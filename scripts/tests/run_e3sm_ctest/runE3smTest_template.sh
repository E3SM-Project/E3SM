...
if [ ! -d "${SCRATCH}/E3SM_simulations" ]; then
...
run_e3sm_log="${public_scratch}/run_e3sm.log"

#Verify the case is already deleted
rm -rf ${case_scratch}

# Change to E3SM dir, where script must be run, using CMake for absolute path
cd @RUN_E3SM_DIR@
...
echo
echo "**********************************************"
echo "Running run_e3sm.template.csh :"
echo

./run_e3sm.template.csh > ${run_e3sm_log}
exitcode=$?
cat ${run_e3sm_log}
...
echo
jobid=`grep "Submitted job id is " ${run_e3sm_log} | cut -d' ' -f5 | tail -n1`
...
# 
# Paranoid delay to ensure we get the most up to date e3sm and coupler logs
sleep 5
sync
