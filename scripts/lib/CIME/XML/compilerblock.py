...
class CompilerBlock(object):
    ...
    def _handle_references(self, elem, set_up, tear_down, depends):
        ...
        for child in elem:
            if self.name(child) == "env":
                ...
                expect(False,
                       "Unexpected tag "+self.name(child)+" encountered in "
    ...
    def _elem_to_setting(self, elem):
        ...
        setting = ValueSetting(value_text, self.name(elem) == "append",
    ...
    def add_settings_to_lists(self, flag_vars, value_lists):
        ...
        for elem in self._compiler_elem:
            ...
            if self.name(elem) in flag_vars:
                for child in elem:
                    self._add_elem_to_lists(self.name(elem), child, value_lists)
            else:
                self._add_elem_to_lists(self.name(elem), elem, value_lists)
