...
my $cesmTest = new CESMTest(compset => "B", grid => 'f19_g16', testname => 'ERS', machine => 'frankfurt', 
							compiler => 'intel', 'testmods' => 'clm/Default', comment => 'always test your code');
...
my @sexpectedauxclmshort = sort { $a->{compset} cmp $b->{compset} ||
                                  $a->{grid} cmp $b->{grid}       ||
                                  $a->{testname} cmp $b->{testname} ||
                                  $a->{machine} cmp $b->{machine} ||
                                  $a->{compiler} cmp $b->{compiler} 
								} @expectedauxclmshort;
my @sactualauxclmshort = sort { $a->{compset} cmp $b->{compset} ||
                                  $a->{grid} cmp $b->{grid}       ||
                                  $a->{testname} cmp $b->{testname} ||
                                  $a->{machine} cmp $b->{machine} ||
                                  $a->{compiler} cmp $b->{compiler} 
