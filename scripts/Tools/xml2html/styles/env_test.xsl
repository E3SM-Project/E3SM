...
<xsl:output method="html" indent="yes" 
  ...
  <xsl:template match="//file">
    <html>
      <head>
	<title>CESM CASEROOT env_test.xml file</title>
	<style type="text/css">
	  body {
	...
	function init() {
      ...
      <body onload="init()">
	<div class="container">
	  <h1>CESM CASEROOT env_test.xml file</h1>
	  ...
	  <h3>Description</h3>
	  <p>
	    The CASEROOT scripts read the XML settings in the env_test.xml file in order to<br/>
	    determine case specific settings when running a regression test..<br/>
	  </p>

	  <h3>Interface Tools</h3>
	  <p>
	    CESM2.0 supports a single test group with id="test" in env_test.xml which can be queried but not changed.
	    The <a href="http://www.cesm.ucar.edu/models/cesm2.0/cesm/xmlquery.html">xmlquery</a> script tool<br/>
	  </p>
	  <h4>Examples</h4>
  ...
  <xsl:template match="group">
    <div class="accordionItem">
      <h2>Test Group Settings <span id="variable"><b> &lt;group id="<xsl:value-of select="@id" />"&gt; : </b></span></h2>
      <div>
  ...
  <xsl:template match="test">
    <div class="accordionItem">
      <h2>Specific Test Settings <span id="variable"><b> &lt;test NAME="<xsl:value-of select="@NAME" />"&gt; : </b></span></h2>
      <div>
	<ul>
	  <xsl:apply-templates select="DESC"/>
	  <xsl:apply-templates select="INFO_DBUG"/>
	  <xsl:apply-templates select="STOP_OPTION"/>
	  <xsl:apply-templates select="STOP_N"/>
	  <xsl:apply-templates select="REST_OPTION"/>
	  <xsl:apply-templates select="DOUT_S"/>
	  <xsl:apply-templates select="CONTINUE_RUN"/>
	</ul>
      </div>
    </div>
  </xsl:template>
  ...
  <xsl:template match="INFO_DBUG">
    <li>Information Debug Level &lt;INFO_DBUG&gt; : <b><xsl:value-of select="." /></b></li>
  </xsl:template>

  <xsl:template match="STOP_OPTION">
    <li>Test Run Length &lt;STOP_OPTION&gt; : <b><xsl:value-of select="." /></b></li>
  </xsl:template>

  <xsl:template match="STOP_N">
    <li>Test Run Count for STOP_OPTION &lt;STOP_N&gt; : <b><xsl:value-of select="." /></b></li>
  </xsl:template>

  <xsl:template match="REST_OPTION">
    <li>Restart Length &lt;REST_OPTION&gt; : <b><xsl:value-of select="." /></b></li>
  </xsl:template>

  <xsl:template match="DOUT_S">
    <li>Short-term Archiving &lt;DOUT_S&gt; : <b><xsl:value-of select="." /></b></li>
  </xsl:template>

  <xsl:template match="CONTINUE_RUN">
    <li>Continuation Run &lt;CONTINUE_RUN&gt; : <b><xsl:value-of select="." /></b></li>
  </xsl:template>
