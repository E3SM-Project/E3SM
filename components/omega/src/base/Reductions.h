...
GlobalSum(const MachEnv *InEnv,
          yakl::Array<T, dim, yakl::memHost, yakl::styleC> const arr, T *val) {
   T LocalSum = yakl::intrinsics::sum(arr);
   return MPI_Allreduce(&LocalSum, val, 1, MPI_INT64_T, MPI_SUM,
                        InEnv->getComm());
}

template <int dim>
int GlobalSum(const MachEnv *InEnv,
              yakl::Array<R4, dim, yakl::memHost, yakl::styleC> const arr,
              R4 *val) {
   R4 LocalSum = yakl::intrinsics::sum(arr);
   return MPI_Allreduce(&LocalSum, val, 1, MPI_FLOAT, MPI_SUM,
                        InEnv->getComm());
}

template <int dim>
int GlobalSum(const MachEnv *InEnv,
              yakl::Array<R8, dim, yakl::memHost, yakl::styleC> const arr,
              R8 *val) {
   R8 LocalSum = yakl::intrinsics::sum(arr);
   return MPI_Allreduce(&LocalSum, val, 1, MPI_DOUBLE, MPI_SUM,
                        InEnv->getComm());
}
