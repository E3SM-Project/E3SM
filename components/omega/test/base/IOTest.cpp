...
int main(int argc, char *argv[]) {
   ...
   int DimVrtxID;
   int DimVertID;
   Err = OMEGA::IO::defineDim(OutFileID, "NVertLevels", NVertLevels, DimVertID);
   if (Err != 0)
   ...
   Err =
       OMEGA::IO::defineDim(OutFileID, "NVertices", NVerticesGlobal, DimVrtxID);
   if (Err != 0)
   ...
   OMEGA::I4 NVertLevelsNew = OMEGA::IO::getDimLength(InFileID, "NVertLevels");
