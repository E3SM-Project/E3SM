...
integer :: so4dryvol_idx = -1 !pbuf id for so4dryvol 
...
logical :: modal_strat_sulfate_aod = .true.           ! !flag to turn on/off stratospheric H2SO4 AOD treatment
...
subroutine modal_aero_calcsize_init( pbuf2d, species_class)
   ...
   if (is_first_step()) then ! bsingh- Do we need this conditional in a init routine?
      ...
      call pbuf_set_field(pbuf2d, so4dryvol_idx, 0.0_r8) 
   endif
...
subroutine modal_aero_calcsize_diag(state, pbuf, list_idx_in, dgnum_m)
   ...
   if (modal_strat_sulfate_aod) then
      call modal_aero_calcdry(state, pbuf, list_idx_in)! to calculate so4dryvol
   end if
