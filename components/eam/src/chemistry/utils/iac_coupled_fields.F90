...
contains
  ...
  subroutine iac_coupled_fields_init(state, pbuf2d)
    ...
    !------------------------------------------------------------------
    if (.not. iac_present) return !FIXME: uncomment it!

    call set_time_float_from_date( second_year, 2, 1, 1, 0 )
    call set_time_float_from_date( first_year,  1, 1, 1, 0 )
  ...
  subroutine iac_coupled_timeinterp(mon_spec, day_spec, tod_spec, & !input
    ...
    ! Dumb
    integer, parameter   :: cyclic_year       = 1   ! Repeat calculations assuming only 1 year
    ...
    !Get current model time in "days".Note: (we assume a cyclic year of 1)
    ...
    call set_time_float_from_date( curr_model_time, & !output
         cyclic_year , mon_spec, day_spec, tod_spec ) !input
