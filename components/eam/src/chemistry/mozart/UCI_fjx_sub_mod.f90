...
!   USE RRSW_FASTJ_CMN, only: MXLAY, ngptsw
      ...
      use cam_logfile,      only : iulog    ! For writing to cam log file
...
!         write(iulog,'(a,I5,a,I5,a,I5,a,I5)')'W_r=    ', W_r, 'W_rrtmg=', W_rrtmg, 'W_clirad=', W_clirad, 'W_LLNL=', W_LLNL

  !SJ    endif

      if (LPRTJ) then
         write(iulog,*)'Fast-J v7.6c ---PHOTO_JX internal print: fjx_sub_mod.f90'
         write(iulog,'(a,3i8)') ' NWBIN / NSBIN:',NWBIN,NSBIN
         write(iulog,'(a,3i8)') ' g-bin super-bin  L-flux: W_  S_  W_+W_r ',W_,S_,W_+W_r
         write(iulog,'(3i8)')   (KR, KDOKR(KR), LDOKR(KR), KR = 1,W_+W_r)
         write(iulog,*) '     L   P1      P2        T  //WPath, Reff, OD'
      endif
...
!>>>diagnostic print of Rayleigh data:
         if (LPRTJ) then
            write(iulog,'(a,i3,2f8.2,f8.1,1p,e12.4)') &
...
!>>>diagnostic print of cloud data:
            if (LPRTJ) then
               write(iulog,'(a,i3,2f8.2,f8.1,f8.4,f8.2,f8.4)') &
...
!>>>diagnostic print of SSA data:
                  if (LPRTJ) then
                     write(iulog,'(a,i3,2f8.2,f8.1,2f8.5,1x,a12)') &
...
!>>>diagnostic print of GEOMIP data:
                  if (LPRTJ) then
                     write(iulog,'(a,i3,2f8.2,f8.1,3f8.5,i10)') &
...
!>>>diagnostic print of OPTICA data:
                  if (LPRTJ) then
                     write(iulog,'(a,i3,2f8.2,f8.1,2f8.5,i5)') &
...
!---Add O2 & O3 absorbers to get final optical properties (Fast-J bins only 1:18)
         do K = 1,W_
            ...
            OD(K,L)  = OD(K,L)  + ODABS
!SJ!       if (LPRTJ) then
!SJ!          write(iulog,'(a,i3,2f8.2,f8.1,f12.4)') &
...
!---diagnostics/variables below are JUST for PRINT and NOT returned to the CTM code
      if (LPRTJ) then
         do L=1,L1U
            ...
            enddo
         enddo
         write(iulog,'(a)') 'Fast-J  v7.6 ---PHOTO_JX internal print: Atmosphere--'
...
!SJ!            write(iulog,'(a)') ParaSummary(1:200)
!SJ!         endif
         write(iulog,'(a)') 'Fast-J  v7.6 ---PHOTO_JX internal print: Solar fluxes (W/m2)--'
         write(iulog,'(a11,f12.4)')    ' inc TOTAL ',SWMSQ(1)
         write(iulog,'(a11,f12.4)')    ' rfl outtop',SWMSQ(2)
         write(iulog,'(a11,f12.4)')    ' abs in atm',SWMSQ(3)
         write(iulog,'(a11,f12.4)')    ' abs at srf',SWMSQ(4)
         write(iulog,'(a11,1p,e12.4)') ' PAR direct',SWMSQ(5)
         write(iulog,'(a11,1p,e12.4)') ' PAR diffus',SWMSQ(6)
         NW1=1
         NS1=1
         NW2=W_
         NS2=S_
         write(iulog,'(a)') 'Spectral budget scaled to 1.0, wavelengths 27:-1:1'
         write(iulog,'(a11/(9f8.4))') ' inc TOTAL ',(FFXNET(K,3), K=NS2,NS1,-1)
         write(iulog,'(a11/(9f8.4))') ' rfl outtop',(FFXNET(K,4), K=NS2,NS1,-1)
         write(iulog,'(a11/(9f8.4))') ' abs in atm',(FFXNET(K,5), K=NS2,NS1,-1)
         write(iulog,'(a11/(9f8.4))') ' abs at srf',(FFXNET(K,6), K=NS2,NS1,-1)
         do J = 1,8
            do K = 1,S_
         ...
         if (.not.(LRRTMG .or. LCLIRAD)) then
            write(iulog,'(a,f10.2,f10.6,3f10.4)') 'FJX: SZA/u0/F-incd/F-refl/', &
               SZA,U0,FREFI,FREFL,FREFS
            write(iulog,'(a,5f10.4)') 'FJX80: albedos@600nm u(1:4) & u0(5)', &
               RFL(1:5,18)
            write(iulog,'(a5,20i8)')   ' bin:',(K, K=NW1,NW2)
            write(iulog,'(a5,20f8.1)') ' wvl:',(WL(K), K=NW1,NW2)
            write(iulog,'(a)') ' ---- 100000=Fsolar   MEAN INTENSITY per wvl bin'
                 RATIO(:) = 0.d0
            do L = LU,1,-1
               do K=NW1,NW2
                  if (LDOKR(K) .gt. 0) then
               ...
               enddo
               write(iulog,'(i3,2x,18i8)') L,(RATIO(K),K=NW1,NW2)
            enddo
            write(iulog,'(a)') 'specific intensity onto surface @ 4 quad angles'
            do I = 1,4
               write(iulog,'(f5.2,20f8.5)') EMU(I),(FIBOT(I,K), K=NW1,NW2)
            enddo
            write(iulog,'(a)') 'specific intensity up from lower bndry'
            write(iulog,'(f5.2,20f8.5)') U0,(FIBOT(5,K), K=NW1,NW2)
            write(iulog,'(a)') 'direct flux at lower bndry'
            write(iulog,'(f5.2,20f8.5)') U0,(FSBOT(K), K=NW1,NW2)
            write(iulog,'(a)') 'diffus flux at lower bndry'
            write(iulog,'(f5.2,20f8.5)') U0,(FJBOT(K), K=NW1,NW2)
            NSS2 = min(S_,NW2+32)  !  limt for large # of super bins: high-res clouds
            write(iulog,'(a5,32i8)')   ' bin:',(K, K=NW2+1,NSS2)
            write(iulog,'(a5,32f8.1)') ' wvl:',(WL(K), K=NW2+1,NSS2)
            write(iulog,'(a)') 'specific intensity to surface at the 4 quad angles'
            do I = 1,4
               write(iulog,'(f5.2,32f8.5)') EMU(I),(FIBOT(I,K), K=NW2+1,NSS2)
            enddo
            write(iulog,'(a)') 'specific intensity up from lower bndry'
            write(iulog,'(f5.2,32f8.5)') U0,(FIBOT(5,K), K=NW2+1,NSS2)
            write(iulog,'(a)') 'direct flux at lower bndry'
            write(iulog,'(f5.2,32f8.5)') U0,(FSBOT(K), K=NW2+1,NSS2)
            write(iulog,'(a)') 'diffus flux at lower bndry'
            write(iulog,'(f5.2,32f8.5)') U0,(FJBOT(K), K=NW2+1,NSS2)
            write(iulog,*)
            write(iulog,*)'Fast-J v7.6 ---PHOTO_JX Net Fluxes include SZA & solar dist'
            write(iulog,'(a,2f8.2)') ' ---NET FLUXES--- solar < 700 or 778 nm ',FREF1,FREF1+FREF2
            write(iulog,'(a11,18i8)')   'bins:',(K, K=NW1,NW2)
            write(iulog,'(a11,18f8.1)') 'wavl:',(WL(K), K=NW1,NW2)
            write(iulog,'(a11,18f8.2)') 'watt:',(FW(K), K=NW1,NW2)
            write(iulog,'(a11,18f8.2)') ' sol atm+sf',(FFXNET(K,3), K=NW1,NW2)
            write(iulog,'(a11,18f8.2)') ' sol in atm',(FFXNET(K,1), K=NW1,NW2)
            write(iulog,'(a11,18f8.2)') ' sol at srf',(FFXNET(K,2), K=NW1,NW2)
            write(iulog,'(a11,18f8.2)') ' dif outtop',(FFXNET(K,4), K=NW1,NW2)
            write(iulog,'(a11,18f8.2)') ' abs in atm',(FFXNET(K,5), K=NW1,NW2)
            write(iulog,'(a11,18f8.2)') ' abs at srf',(FFXNET(K,6), K=NW1,NW2)
            write(iulog,'(a11,18f8.2)') ' srf direct',(FFXNET(K,7), K=NW1,NW2)
            write(iulog,'(a11,18f8.2)') ' srf diffus',(FFXNET(K,8), K=NW1,NW2)
            write(iulog,'(a11,11f8.2,7f8.3)') ' tau absorb',(FFXTAU(K,1), K=NW1,NW2)
            write(iulog,'(a11,11f8.2,7f8.3)') ' tau total ',(FFXTAU(K,2), K=NW1,NW2)
            write(iulog,'(a11,11f8.2,7f8.3)') ' cld absorb',(FFXTAU(K,3), K=NW1,NW2)
            write(iulog,'(a11,11f8.2,7f8.3)') ' cld total ',(FFXTAU(K,4), K=NW1,NW2)
            write(iulog,'(a11,1p,e10.3)') ' PAR direct',PREF1
            write(iulog,'(a11,1p,e10.3)') ' PAR diffus',PREF2
            write(iulog,'(a,3f8.2)') ' ---NET FLUXES--- solar <700, >700, total: ',&
                             FREF1,FREF2,FREF1+FREF2
            write(iulog,'(a11,32i8)')   'bins:',(K, K=NW2+1,NSS2)
            write(iulog,'(a11,32f8.1)') 'wavl:',(WL(K), K=NW2+1,NSS2)
            write(iulog,'(a11,32f8.2)') 'watt:',(FW(K), K=NW2+1,NSS2)
            write(iulog,'(a11,33f8.2)') ' sol atm+sf',(FFXNET(K,3), K=NW2+1,NSS2),FFXNETS(3)
            write(iulog,'(a11,33f8.2)') ' sol in atm',(FFXNET(K,1), K=NW2+1,NSS2),FFXNETS(1)
            write(iulog,'(a11,33f8.2)') ' sol at srf',(FFXNET(K,2), K=NW2+1,NSS2),FFXNETS(2)
            write(iulog,'(a11,33f8.2)') ' dif outtop',(FFXNET(K,4), K=NW2+1,NSS2),FFXNETS(4)
            write(iulog,'(a11,33f8.2)') ' abs in atm',(FFXNET(K,5), K=NW2+1,NSS2),FFXNETS(5)
            write(iulog,'(a11,33f8.2)') ' abs at srf',(FFXNET(K,6), K=NW2+1,NSS2),FFXNETS(6)
            write(iulog,'(a11,33f8.2)') ' srf direct',(FFXNET(K,7), K=NW2+1,NSS2),FFXNETS(7)
            write(iulog,'(a11,33f8.2)') ' srf diffus',(FFXNET(K,8), K=NW2+1,NSS2),FFXNETS(8)
            write(iulog,'(a11,32f8.3)') ' tau absorb',(FFXTAU(K,1), K=NW2+1,NSS2)
            write(iulog,'(a11,32f8.3)') ' tau total ',(FFXTAU(K,2), K=NW2+1,NSS2)
            write(iulog,'(a11,32f8.3)') ' cld absorb',(FFXTAU(K,3), K=NW2+1,NSS2)
            write(iulog,'(a11,32f8.3)') ' cld total ',(FFXTAU(K,4), K=NW2+1,NSS2)
            write(iulog,'(a)') 'heating rate profiles in K/day v7.6  180-778nm '
            write(iulog, '(a4, 32f7.1)')'wvl ',(WL(I),I=NW1,NW2)
            SKPERD(:,:)=NINT( SKPERD(:,:)*100 ) / 100. !Rounding to 2 decimal places for consistent output.
            do L = LU,1,-1
               write(iulog,'(i4,32f7.2)') L,(SKPERD(I,L), I=NW1,NW2)
            enddo
            write(iulog,'(a)') 'heating rate profiles in K/day v7.6 778-...nm plus 1:18 19:27 1:27'
            write(iulog, '(a4,32f7.1)')'wvl ',(WL(I),I=NW2+1,NSS2)
            do L = LU,1,-1
               write(iulog,'(i4,35f7.2)') L,(SKPERD(I,L), I=NW2+1,NSS2+2),SKPERD(S_+1,L)+SKPERD(S_+2,L)
            enddo
            write(iulog,'(a)') ' Fast-J  v7.6 ----J-values----'
            write(iulog,'(1x,a,72(a6,3x))') 'L=  ',(TITLEJX(K), K=1,NJX)
            do L = LU,1,-1
               write(iulog,'(i3,1p, 72e9.2)') L,(VALJXX(L,K),K=1,NJX)
            enddo
...
!-----------------------------------------------------------------------
      ...
      if (NJXU .lt. NJX) then
        write(iulog,'(A,2I5)')  'NJXU<NJX',NJXU,NJX
...
!-----------------------------------------------------------------------
      ...
      write(iulog,'(4a)') '   L z(km)     p      T   ', &
       '    d(air)   d(O3)','  col(O2)  col(O3)     d-TAU   SS-alb', &
       '  g(cos) CTM lyr=>'
      L = LU+2
      write(iulog,'(1x,i3,0p,f6.2,f10.3,f7.2,1p,4e9.2,0p,f10.4,2f8.5,2i3)') &
            L,ZZJ(L)*1.d-5,PPJ(L)
      XCOLO2 = 0.d0
      XCOLO3 = 0.d0
      ZTOP = ZZJ(LU+2)
...
!---check on overflow of arrays, cut off JXTRA at lower L if too many levels
      ...
      do L = L1X,1,-1
         ...
         if (JTOTL*2 .gt. NX)  then
            write(iulog,'(A,3I5)') 'cutoff EXTRAL1 @L: NX/L1X/L: ',NX,L1X,L
            do LL = L,1,-1
               JXTRA(LL) = 0
            enddo
...
!2012 HITRAN H2O
      real*8, dimension(10,3), parameter  :: WGF = reshape([ &
...
!  WGF and WGTOT are tabulated FJX_spec_GGLLNL.dat
      ...
      real*8, dimension(7,3), parameter  :: WGF = reshape([ &
...
!WGK cross sections unit cm2 g-1
      real*8, dimension(7,3), parameter  :: WGK = reshape([ &
...
!               write(iulog,'(A10, 2I5, f12.8)')'LLNL tau', L, INDKG, TAUG_LLNL(L,indkg)
            enddo
         enddo
      enddo
