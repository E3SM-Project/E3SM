...
#if (defined MODAL_AERO_9MODE || MODAL_AERO_4MODE_MOM || defined MODAL_AERO_5MODE_AGEDCARBON || defined MODAL_AERO_7MODE_BB_ACARBON)
       dummy = 'SSTSFMBL_OM'
...
!      if(nspc > 0 .and. .not.cnst_name_cw(nspc) == ' ') then   ! REASTER 08/04/2015
    ...
    index_chm_mass(5,3) = get_spc_ndx('bc_a5' )
    !
    ! biomass burning carbon mode
    index_tot_mass(6,1) = get_spc_ndx('pom_a6')
    index_tot_mass(6,2) = get_spc_ndx('bc_a6')
    index_chm_mass(6,1) = get_spc_ndx('bc_a6' )
    !
    ! aged biomass burning carbon mode
    !
    index_tot_mass(7,1) = get_spc_ndx('so4_a7')
    index_tot_mass(7,2) = get_spc_ndx('pom_a7')
    index_tot_mass(7,3) = get_spc_ndx('soa_a7')
    index_tot_mass(7,4) = get_spc_ndx('bc_a7' )
    index_tot_mass(7,5) = get_spc_ndx('ncl_a7')
    index_tot_mass(7,6) = get_spc_ndx('mom_a7')
    index_chm_mass(7,1) = get_spc_ndx('so4_a7')
    index_chm_mass(7,2) = get_spc_ndx('soa_a7')
    index_chm_mass(7,3) = get_spc_ndx('bc_a7' )
    !
...
#if ( defined MODAL_AERO_3MODE ) || ( defined MODAL_AERO_4MODE ) || ( defined MODAL_AERO_4MODE_MOM ) || ( defined MODAL_AERO_5MODE_AGEDCARBON ) || ( defined MODAL_AERO_7MODE_BB_ACARBON )
       ntoo = modeptr_coarse
#else
...
! REASTER 08/11/2015 BEGIN
    do mtmp = 1, ntot_amode ! main loop over aerosol modes
       ...
       if (ntot_amode == 7) then
          ! for mam7, do accum, aitken, pcarbon, acarbon,  
          ! pbb, abb, then coarse 
          if (mtmp == ntot_amode) then
             m = modeptr_coarse
          else if (mtmp >= modeptr_coarse) then
