...
!---------------------------------------------------------------------------
  ...
  type cam_out_t 
     ...
     real(r8), allocatable :: tresp(:)    ! first-order response of tbot to surface fluxes
     real(r8), allocatable :: qresp(:)    ! first-order response of qbot to surface fluxes
...
!
    ...
    do c = begchunk,endchunk 
       ...
       allocate (cam_out(c)%tresp(pcols), stat=ierror)
       if ( ierror /= 0 ) call endrun('ATM2HUB_ALLOC error: allocation error tresp')

       allocate (cam_out(c)%qresp(pcols), stat=ierror)
       if ( ierror /= 0 ) call endrun('ATM2HUB_ALLOC error: allocation error qresp')
    ...
    do c = begchunk,endchunk
       ...
       cam_out(c)%tresp(:)    = 0._r8
       cam_out(c)%qresp(:)    = 0._r8
  ...
  subroutine atm2hub_deallocate(cam_out)
    ...
    if(associated(cam_out)) then
       do c = begchunk,endchunk
          ...
          deallocate(cam_out(c)%tresp)
          deallocate(cam_out(c)%qresp)
...
!-----------------------------------------------------------------------
   ...
   integer :: vmag_gust_idx, tresp_idx, qresp_idx, wsresp_idx, u_diff_idx, v_diff_idx
   ...
   real(r8), pointer :: tresp(:)                   ! First-order response of tbot to surface stress
   real(r8), pointer :: qresp(:)                   ! First-order response of qbot to surface stress
...
!PMA adds gustiness to surface scheme c20181128
   ...
   do i=1,ncol
      ...
      cam_out%tresp(i) = max(tresp(i), 0._r8)
      cam_out%qresp(i) = max(qresp(i), 0._r8)
