module crm_output_module
   ...
   type crm_output_type
      ...
      real(crm_rknd), allocatable :: wtke         (:,:)  ! vertical velocity variance  [kg/m/s2]
...
contains
   ...
   subroutine crm_output_initialize(output, ncol, nlev)
      ...
      if (present(ncol)) then
         ...
         if (.not. allocated(output%wtke         )) allocate(output%wtke         (ncol,nlev))
   ...
   subroutine crm_output_finalize(output)
      ...
      if (allocated(output%wtke)) deallocate(output%wtke)
