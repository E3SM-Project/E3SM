...
void advect2_mom_xy() {
  ...
  if (RUN3D) {
    ...
    parallel_for( "advect2_mom_xy 1" , SimpleBounds<4>(nzm,ny,nx,ncrms) , YAKL_LAMBDA (int k, int j, int i, int icrm) {
      int kc= k+1;
    ...
    //    for (int i=0; i<nx; i++) {
    //       for (int icrm=0; icrm<ncrms; icrm++) {
    parallel_for( "advect2_mom_xy 2" , SimpleBounds<3>(nzm,nx,ncrms) , YAKL_LAMBDA (int k, int i, int icrm) {
      int j=0;
      int kc= k+1;
      int kcu =min(kc, nzm-1);
      real irho = 1.0/(rhow(kc,icrm)*adzw(kc,icrm));
