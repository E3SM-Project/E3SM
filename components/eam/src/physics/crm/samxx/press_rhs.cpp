...
void press_rhs() {
  ...
  if (dowallx && rank%nsubdomains_x == 0) {
    ...
    parallel_for( "press_rhs 1" , SimpleBounds<3>(nzm,ny,ncrms) , YAKL_LAMBDA (int k, int j, int icrm) {
  ...
  if (dowally && RUN3D == 1 && rank < nsubdomains_x) {
    ...
    //  for (int i=0; i<nx; i++) {
    //    for (int icrm=0; icrm<ncrms; icrm++) {
    parallel_for( "press_rhs 2" , SimpleBounds<3>(nzm,nx,ncrms) , YAKL_LAMBDA (int k, int i, int icrm) {
  ...
  if (RUN3D) {
    ...
    //     for (int i=0; i<nx; i++) {
    //       for (int icrm=0; icrm<ncrms; icrm++) {
    parallel_for( "press_rhs 3" , SimpleBounds<4>(nzm,ny,nx,ncrms) , YAKL_LAMBDA (int k, int j, int i, int icrm) {
      int kc=k+1;
    ...
    //     for (int i=0; i<nx; i++) {
    //       for (int icrm=0; icrm<ncrms; icrm++) {
    parallel_for( "press_rhs 4" , SimpleBounds<3>(nzm,nx,ncrms) , YAKL_LAMBDA (int k, int i, int icrm) {
      int kc=k+1;
      real rdz=1.0/(adz(k,icrm)*dz(icrm));
      real rup = rhow(kc,icrm)/rho(k,icrm)*rdz;
      real rdn = rhow(k,icrm)/rho(k,icrm)*rdz;
