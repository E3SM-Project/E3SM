...
void press_grad() {
  ...
  parallel_for( "press_grad 1" , SimpleBounds<4>(nzm,ny,nx,ncrms) , YAKL_LAMBDA (int k, int j, int i, int icrm) {
    int kb=max(0,k-1);
  ...
  //     for (int i=0; i<nx; i++) {
  //       for (int icrm=0; icrm<ncrms; icrm++) {
  parallel_for( "press_grad 2" , SimpleBounds<4>(nzm,ny+YES3D,nx+1,ncrms) , YAKL_LAMBDA (int k, int j, int i, int icrm) {
  ...
  if(dowallx && rank%nsubdomains_x == 0) {
    ...
    //  for (int j=0; j<ny; j++) {
    //    for (int icrm=0; icrm<ncrms; icrm++) {
    parallel_for( "press_grad 3" , SimpleBounds<3>(nzm,ny,ncrms) , YAKL_LAMBDA (int k, int j, int icrm) {
  ...
  if(dowally && RUN3D == 1 && rank < nsubdomains_x) {
    ...
    //  for (int i=0; i<nx; i++) {
    //    for (int icrm=0; icrm<ncrms; icrm++) {
    parallel_for( "press_grad 4" , SimpleBounds<3>(nzm,nx,ncrms) , YAKL_LAMBDA (int k, int i, int icrm) {
