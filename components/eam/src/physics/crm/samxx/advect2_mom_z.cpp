...
void advect2_mom_z() {
  ...
  parallel_for( "advect2_mom_z 1" , SimpleBounds<2>(nz,ncrms) , YAKL_LAMBDA (int k, int icrm) {
    uwle(k,icrm) = 0.0;
    vwle(k,icrm) = 0.0;
  });
  ...
  //  for (int i=0; i<nx; i++) {
  //    for (int icrm=0; icrm<ncrms; icrm++) {
  parallel_for( "advect2_mom_z 2" , SimpleBounds<3>(ny,nx,ncrms) , YAKL_LAMBDA (int j, int i, int icrm) {
  ...
  if (RUN3D) {
    ...
    //     for (int i=0; i<nx; i++) {
    //       for (int icrm=0; icrm<ncrms; icrm++) {
    parallel_for( "advect2_mom_z 3" , SimpleBounds<4>(nzm-1,ny,nx,ncrms) , YAKL_LAMBDA (int k, int j, int i, int icrm) {
      real dz25=1.0/(4.0*dz(icrm));
      int kb = k-1;
    ...
    //     for (int i=0; i<nx; i++) {
    //       for (int icrm=0; icrm<ncrms; icrm++) {
    parallel_for( "advect2_mom_z 4" , SimpleBounds<4>(nzm-1,ny,nx,ncrms) , YAKL_LAMBDA (int k, int j, int i, int icrm) {
      real dz25=1.0/(4.0*dz(icrm));
      int kb = k-1;
      real rhoi = dz25 * rhow(k+1,icrm);
  ...
  //     for (int i=0; i<nx; i++) {
  //       for (int icrm=0; icrm<ncrms; icrm++) {
  parallel_for( "advect2_mom_z 5" , SimpleBounds<4>(nzm,ny,nx,ncrms) , YAKL_LAMBDA (int k, int j, int i, int icrm) {
    real dz25=1.0/(4.0*dz(icrm));
    int kc = k+1;
  ...
  //     for (int i=0; i<nx; i++) {
  //       for (int icrm=0; icrm<ncrms; icrm++) {
  parallel_for( "advect2_mom_z 6" , SimpleBounds<4>(nzm-1,ny,nx,ncrms) , YAKL_LAMBDA (int k, int j, int i, int icrm) {
    int kb=k-1;
