...
void post_icycle() {
  ...
  parallel_for( "post_icycle 1" , SimpleBounds<3>(ny,nx,ncrms) , YAKL_LAMBDA (int j, int i, int icrm) {
    cwp     (j,i,icrm) = 0.0;
    cwph    (j,i,icrm) = 0.0;
    cwpm    (j,i,icrm) = 0.0;
    cwpl    (j,i,icrm) = 0.0;
  ...
  //  for (int i=0; i<nx; i++) {
  //    for (int icrm=0; icrm<ncrms; icrm++) {
  parallel_for( "post_icycle 2" , SimpleBounds<3>(ny,nx,ncrms) , YAKL_LAMBDA (int j, int i, int icrm) {
    for (int k=0; k<nzm; k++) {
      int l = plev-(k+1);
  ...
  //     for (int i=0; i<nx; i++) {
  //       for (int icrm=0; icrm<ncrms; icrm++) {
  parallel_for( "post_icycle 3" , SimpleBounds<4>(nzm,ny,nx,ncrms) , YAKL_LAMBDA (int k, int j, int i, int icrm) {
  ...
  parallel_for( "post_icycle 4" , SimpleBounds<4>(nzm+1,ny,nx,ncrms) , YAKL_LAMBDA (int k, int j, int i, int icrm) {
    int l=plev+1-(k+1);
    int kx;
    real qsat;
    real tmp;
  ...
  //  for (int i=0; i<nx; i++) {
  //    for (int icrm=0; icrm<ncrms; icrm++) {
  parallel_for( "post_icycle 5" , SimpleBounds<3>(ny,nx,ncrms) , YAKL_LAMBDA (int j, int i, int icrm) {
