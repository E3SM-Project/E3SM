...
void advect_scalar2D(real4d &f, real2d &flux) {
  ...
  parallel_for( "advect_scalar2D 1" , SimpleBounds<2>(nx+4,ncrms) , YAKL_LAMBDA (int i, int icrm) {
  ...
  if (dowallx) {
    if (rank%nsubdomains_x == 0) {
      ...
      //    for (int icrm=0; icrm<ncrms; icrm++) {
      parallel_for( "advect_scalar2D 2" , SimpleBounds<3>(nzm,nx,ncrms) , YAKL_LAMBDA (int k, int i, int icrm) {
        u(k,j,i,icrm) = 0.0;
      });
    }
    if (rank%nsubdomains_x==nsubdomains_x-1) {
      ...
      //    for (int icrm=0; icrm<ncrms; icrm++) {
      parallel_for( "advect_scalar2D 3" , SimpleBounds<3>(nzm,nx,ncrms) , YAKL_LAMBDA (int k, int i, int icrm) {
        int iInd = i+ (nx+2);
        u(k,j,iInd,icrm) = 0.0;
      });
    }
  ...
  if (nonos) {
    ...
    //    for (int icrm=0; icrm<ncrms; icrm++) {
    parallel_for( "advect_scalar2D 4" , SimpleBounds<3>(nzm,nx+2,ncrms) , YAKL_LAMBDA (int k, int i, int icrm) {
  ...
  //    for (int icrm=0; icrm<ncrms; icrm++) {
  parallel_for( "advect_scalar2D 5" , SimpleBounds<3>(nzm,nx+5,ncrms) , YAKL_LAMBDA (int k, int i, int icrm) {
    int kb=max(0,k-1);
  ...
  // for (int k=0; k<nzm; k++) {
  //  for (int icrm=0; icrm<ncrms; icrm++) {
  parallel_for( "advect_scalar2D 6" , SimpleBounds<2>(nzm,ncrms) , YAKL_LAMBDA (int k, int icrm) {
  ...
  //  for (int i=0; i<nx+4; i++) {
  //    for (int icrm=0; icrm<ncrms; icrm++) {
  parallel_for( "advect_scalar2D 7" , SimpleBounds<3>(nzm,nx+4,ncrms) , YAKL_LAMBDA (int k, int i, int icrm) {
    if (i >= 2 && i <= nx+1) {
  ...
  //    for (int icrm=0; icrm<ncrms; icrm++) {
  parallel_for( "advect_scalar2D 8" , SimpleBounds<3>(nzm,nx+3,ncrms) , YAKL_LAMBDA (int k, int i, int icrm) {
    int kc=min(nzm-1,k+1);
    int kb=max(0,k-1);
  ...
  //  for (int i=0; i<nx+4; i++) {
  //    for (int icrm=0; icrm<ncrms; icrm++) {
  parallel_for( "advect_scalar2D 9" , SimpleBounds<2>(nx+4,ncrms) , YAKL_LAMBDA (int i, int icrm) {
    www(0,j,i,icrm) = 0.0;
  });
  ...
  if (nonos) {
    ...
    //  for (int i=0; i<nx+2; i++) {
    //    for (int icrm=0; icrm<ncrms; icrm++) {
    parallel_for( "advect_scalar2D 10" , SimpleBounds<3>(nzm,nx+2,ncrms) , YAKL_LAMBDA (int k, int i, int icrm) {
      int kc=min(nzm-1,k+1);
      int kb=max(0,k-1);
      int ib=i-1;
      int ic=i+1;
    ...
    //  for (int i=0; i<nx+2; i++) {
    //    for (int icrm=0; icrm<ncrms; icrm++) {
    parallel_for( "advect_scalar2D 11" , SimpleBounds<3>(nzm,nx+2,ncrms) , YAKL_LAMBDA (int k, int i, int icrm) {
      int kc=min(nzm-1,k+1);
      int ic=i+1;
    ...
    //    for (int icrm=0; icrm<ncrms; icrm++) {
    parallel_for( "advect_scalar2D 12" , SimpleBounds<3>(nzm,nx+1,ncrms) , YAKL_LAMBDA (int k, int i, int icrm) {
      int ib=i-1;
  ...
  //     for (int i=0; i<nx; i++) {
  //       for (int icrm=0; icrm<ncrms; icrm++) {
  parallel_for( "advect_scalar2D 13" , SimpleBounds<3>(nzm,nx,ncrms) , YAKL_LAMBDA (int k, int i, int icrm) {
    int kc=k+1;
...
void advect_scalar2D(real5d &f, int ind_f, real2d &flux) {
  ...
  // for (int i=0; i<nx+4; i++) {
  //  for (int icrm=0; icrm<ncrms; icrm++) {
  parallel_for( "advect_scalar2D 14" , SimpleBounds<2>(nx+4,ncrms) , YAKL_LAMBDA (int i, int icrm) {
    www(nz-1,j,i,icrm)=0.0;
  });
  ...
  if (dowallx) {
    if (rank%nsubdomains_x == 0) {
      ...
      //  for (int i=0; i<1-dimx1_u+1; i++) {
      //    for (int icrm=0; icrm<ncrms; icrm++) {
      parallel_for( "advect_scalar2D 15" , SimpleBounds<3>(nzm,nx,ncrms) , YAKL_LAMBDA (int k, int i, int icrm) {
        u(k,j,i,icrm) = 0.0;
      });
    }
    if (rank%nsubdomains_x==nsubdomains_x-1) {
      ...
      //  for (int i=0; i<dimx2_u-(nx+1)+1; i++) {
      //    for (int icrm=0; icrm<ncrms; icrm++) {
      parallel_for( "advect_scalar2D 16" , SimpleBounds<3>(nzm,nx,ncrms) , YAKL_LAMBDA (int k, int i, int icrm) {
        int iInd = i+ (nx+2);
        u(k,j,iInd,icrm) = 0.0;
      });
    }
  ...
  if (nonos) {
    ...
    //  for (int i=0; i<nx+2; i++) {
    //    for (int icrm=0; icrm<ncrms; icrm++) {
    parallel_for( "advect_scalar2D 17" , SimpleBounds<3>(nzm,nx+2,ncrms) , YAKL_LAMBDA (int k, int i, int icrm) {
      int kc=min(nzm-1,k+1);
      int kb=max(0,k-1);
      int ib=i-1;
      int ic=i+1;
  ...
  //  for (int i=0; i<nx+5; i++) {
  //    for (int icrm=0; icrm<ncrms; icrm++) {
  parallel_for( "advect_scalar2D 18" , SimpleBounds<3>(nzm,nx+5,ncrms) , YAKL_LAMBDA (int k, int i, int icrm) {
    int kb=max(0,k-1);
  ...
  // for (int k=0; k<nzm; k++) {
  //  for (int icrm=0; icrm<ncrms; icrm++) {
  parallel_for( "advect_scalar2D 19" , SimpleBounds<2>(nzm,ncrms) , YAKL_LAMBDA (int k, int icrm) {
    irho(k,icrm) = 1.0/rho(k,icrm);
    iadz(k,icrm) = 1.0/adz(k,icrm);
    irhow(k,icrm) = 1.0/(rhow(k,icrm)*adz(k,icrm));
  });
  ...
  //  for (int i=0; i<nx+4; i++) {
  //    for (int icrm=0; icrm<ncrms; icrm++) {
  parallel_for( "advect_scalar2D 20" , SimpleBounds<3>(nzm,nx+4,ncrms) , YAKL_LAMBDA (int k, int i, int icrm) {
    if (i >= 2 && i <= nx+1) {
      yakl::atomicAdd(flux(k,icrm),www(k,j,i,icrm));
    }
  ...
  //  for (int i=0; i<nx+3; i++) {
  //    for (int icrm=0; icrm<ncrms; icrm++) {
  parallel_for( "advect_scalar2D 21" , SimpleBounds<3>(nzm,nx+3,ncrms) , YAKL_LAMBDA (int k, int i, int icrm) {
    int kc=min(nzm-1,k+1);
    int kb=max(0,k-1);
    real dd=2.0/(kc-kb)/adz(k,icrm);
    int ib=i-1;
  ...
  //  for (int i=0; i<nx+4; i++) {
  //    for (int icrm=0; icrm<ncrms; icrm++) {
  parallel_for( "advect_scalar2D 22" , SimpleBounds<2>(nx+4,ncrms) , YAKL_LAMBDA (int i, int icrm) {
    www(0,j,i,icrm) = 0.0;
  });
  ...
  if (nonos) {
    ...
    //  for (int i=0; i<nx+2; i++) {
    //    for (int icrm=0; icrm<ncrms; icrm++) {
    parallel_for( "advect_scalar2D 23" , SimpleBounds<3>(nzm,nx+2,ncrms) , YAKL_LAMBDA (int k, int i, int icrm) {
      int kc=min(nzm-1,k+1);
      int kb=max(0,k-1);
      int ib=i-1;
      int ic=i+1;
    ...
    //  for (int i=0; i<nx+2; i++) {
    //    for (int icrm=0; icrm<ncrms; icrm++) {
    parallel_for( "advect_scalar2D 24" , SimpleBounds<3>(nzm,nx+2,ncrms) , YAKL_LAMBDA (int k, int i, int icrm) {
      int kc=min(nzm-1,k+1);
      int ic=i+1;
    ...
    //  for (int i=0; i<nx+1; i++) {
    //    for (int icrm=0; icrm<ncrms; icrm++) {
    parallel_for( "advect_scalar2D 25" , SimpleBounds<3>(nzm,nx+1,ncrms) , YAKL_LAMBDA (int k, int i, int icrm) {
      int ib=i-1;
  ...
  //     for (int i=0; i<nx; i++) {
  //       for (int icrm=0; icrm<ncrms; icrm++) {
  parallel_for( "advect_scalar2D 26" , SimpleBounds<3>(nzm,nx,ncrms) , YAKL_LAMBDA (int k, int i, int icrm) {
    int kc=k+1;
    // MK: added fix for very small negative values (relative to positive values)
    //     especially  when such large numbers as
    //     hydrometeor concentrations are advected. The reason for negative values is
...
void advect_scalar2D(real5d &f, int ind_f, real3d &flux, int ind_flux) {
  ...
  // for (int i=0; i<nx+4; i++) {
  //  for (int icrm=0; icrm<ncrms; icrm++) {
  parallel_for( "advect_scalar2D 27" , SimpleBounds<2>(nx+4,ncrms) , YAKL_LAMBDA (int i, int icrm) {
    www(nz-1,j,i,icrm)=0.0;
  });
  ...
  if (dowallx) {
    if (rank%nsubdomains_x == 0) {
      ...
      //  for (int i=0; i<1-dimx1_u+1; i++) {
      //    for (int icrm=0; icrm<ncrms; icrm++) {
      parallel_for( "advect_scalar2D 28" , SimpleBounds<3>(nzm,nx,ncrms) , YAKL_LAMBDA (int k, int i, int icrm) {
        u(k,j,i,icrm) = 0.0;
      });
    }
    if (rank%nsubdomains_x==nsubdomains_x-1) {
      ...
      //  for (int i=0; i<dimx2_u-(nx+1)+1; i++) {
      //    for (int icrm=0; icrm<ncrms; icrm++) {
      parallel_for( "advect_scalar2D 29" , SimpleBounds<3>(nzm,nx,ncrms) , YAKL_LAMBDA (int k, int i, int icrm) {
        int iInd = i+ (nx+2);
        u(k,j,iInd,icrm) = 0.0;
      });
    }
  ...
  if (nonos) {
    ...
    //  for (int i=0; i<nx+2; i++) {
    //    for (int icrm=0; icrm<ncrms; icrm++) {
    parallel_for( "advect_scalar2D 30" , SimpleBounds<3>(nzm,nx+2,ncrms) , YAKL_LAMBDA (int k, int i, int icrm) {
      int kc=min(nzm-1,k+1);
      int kb=max(0,k-1);
      int ib=i-1;
      int ic=i+1;
  ...
  //  for (int i=0; i<nx+5; i++) {
  //    for (int icrm=0; icrm<ncrms; icrm++) {
  parallel_for( "advect_scalar2D 31" , SimpleBounds<3>(nzm,nx+5,ncrms) , YAKL_LAMBDA (int k, int i, int icrm) {
    int kb=max(0,k-1);
    uuu(k,j,i,icrm)=max(0.0,u(k,j,i,icrm))*f(ind_f,k,j,i-1+offx_s-2,icrm)+
                    min(0.0,u(k,j,i,icrm))*f(ind_f,k,j,i+offx_s-2,icrm);
    if (i <= nx+3) {
  ...
  // for (int k=0; k<nzm; k++) {
  //  for (int icrm=0; icrm<ncrms; icrm++) {
  parallel_for( "advect_scalar2D 32" , SimpleBounds<2>(nzm,ncrms) , YAKL_LAMBDA (int k, int icrm) {
    irho(k,icrm) = 1.0/rho(k,icrm);
    iadz(k,icrm) = 1.0/adz(k,icrm);
    irhow(k,icrm) = 1.0/(rhow(k,icrm)*adz(k,icrm));
  });
  ...
  //  for (int i=0; i<nx+4; i++) {
  //    for (int icrm=0; icrm<ncrms; icrm++) {
  parallel_for( "advect_scalar2D 33" , SimpleBounds<3>(nzm,nx+4,ncrms) , YAKL_LAMBDA (int k, int i, int icrm) {
    if (i >= 2 && i <= nx+1) {
  ...
  //  for (int i=0; i<nx+3; i++) {
  //    for (int icrm=0; icrm<ncrms; icrm++) {
  parallel_for( "advect_scalar2D 34" , SimpleBounds<3>(nzm,nx+3,ncrms) , YAKL_LAMBDA (int k, int i, int icrm) {
    int kc=min(nzm-1,k+1);
    int kb=max(0,k-1);
    real dd=2.0/(kc-kb)/adz(k,icrm);
    int ib=i-1;
  ...
  //  for (int i=0; i<nx+4; i++) {
  //    for (int icrm=0; icrm<ncrms; icrm++) {
  parallel_for( "advect_scalar2D 35" , SimpleBounds<2>(nx+4,ncrms) , YAKL_LAMBDA (int i, int icrm) {
    www(0,j,i,icrm) = 0.0;
  });
  ...
  if (nonos) {
    ...
    //  for (int i=0; i<nx+2; i++) {
    //    for (int icrm=0; icrm<ncrms; icrm++) {
    parallel_for( "advect_scalar2D 36" , SimpleBounds<3>(nzm,nx+2,ncrms) , YAKL_LAMBDA (int k, int i, int icrm) {
      int kc=min(nzm-1,k+1);
      int kb=max(0,k-1);
      int ib=i-1;
      int ic=i+1;
    ...
    //  for (int i=0; i<nx+2; i++) {
    //    for (int icrm=0; icrm<ncrms; icrm++) {
    parallel_for( "advect_scalar2D 37" , SimpleBounds<3>(nzm,nx+2,ncrms) , YAKL_LAMBDA (int k, int i, int icrm) {
      int kc=min(nzm-1,k+1);
      int ic=i+1;
      mx(k,j,i,icrm)=rho(k,icrm)*(mx(k,j,i,icrm)-f(ind_f,k,j,i+offx_s-1,icrm))/(pn2(uuu(k,j,ic+offx_uuu-1,icrm)) +
                     pp2(uuu(k,j,i+offx_uuu-1,icrm))+iadz(k,icrm)*(pn2(www(kc,j,i+offx_www-1,icrm)) +
    ...
    //  for (int i=0; i<nx+1; i++) {
    //    for (int icrm=0; icrm<ncrms; icrm++) {
    parallel_for( "advect_scalar2D 38" , SimpleBounds<3>(nzm,nx+1,ncrms) , YAKL_LAMBDA (int k, int i, int icrm) {
      int ib=i-1;
      uuu(k,j,i+offx_uuu,icrm)= pp2(uuu(k,j,i+offx_uuu,icrm))*min(1.0,min(mx(k,j,i+offx_m,icrm), mn(k,j,ib+offx_m,icrm))) -
                                pn2(uuu(k,j,i+offx_uuu,icrm))*min(1.0,min(mx(k,j,ib+offx_m,icrm),mn(k,j,i+offx_m,icrm)));
      if (i <= nx-1) {
  ...
  //     for (int i=0; i<nx; i++) {
  //       for (int icrm=0; icrm<ncrms; icrm++) {
  parallel_for( "advect_scalar2D 39" , SimpleBounds<3>(nzm,nx,ncrms) , YAKL_LAMBDA (int k, int i, int icrm) {
    int kc=k+1;
    // MK: added fix for very small negative values (relative to positive values)
    //     especially  when such large numbers as
    //     hydrometeor concentrations are advected. The reason for negative values is
