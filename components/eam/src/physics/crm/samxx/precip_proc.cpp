...
void precip_proc(real5d &q, int ind_q, real5d &qp, int ind_qp) {
  ...
  parallel_for( "precip_proc 1" , SimpleBounds<2>(nzm,ncrms) , YAKL_LAMBDA (int k, int icrm) {
    qpsrc(k,icrm)=0.0;
    qpevp(k,icrm)=0.0;
  });
  ...
  //     for (int i=0; i<nx; i++) {
  //       for (int icrm=0; icrm<ncrms; icrm++) {
  parallel_for( "precip_proc 2" , SimpleBounds<4>(nzm,ny,nx,ncrms) , YAKL_LAMBDA (int k, int j, int i, int icrm) {
