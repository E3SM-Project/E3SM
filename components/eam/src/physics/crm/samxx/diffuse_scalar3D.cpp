...
void diffuse_scalar3D(real4d &field, real3d &fluxb, real3d &fluxt, real5d &tkh,
  ...
  if (dosgs) {
    ...
    parallel_for( "diffuse_scalar3D 1" , SimpleBounds<4>(nzm,ny,nx,ncrms) , YAKL_LAMBDA (int k, int j, int i, int icrm) {
      dfdt(k,j,i,icrm)=0.0;
    });
    ...
    //       for (int icrm=0; icrm<ncrms; icrm++) {
    parallel_for( "diffuse_scalar3D 2" , SimpleBounds<4>(nzm,ny+1,nx+1,ncrms) , YAKL_LAMBDA (int k, int j, int i, int icrm) {
      if (j >= 1) {
        int ic=i+1;
    ...
    //     for (int i=0; i<nx; i++) {
    //       for (int icrm=0; icrm<ncrms; icrm++) {
    parallel_for( "diffuse_scalar3D 3" , SimpleBounds<4>(nzm,ny,nx,ncrms) , YAKL_LAMBDA (int k, int j, int i, int icrm) {
      int ib=i-1;
    ...
    // for (int k=0; k<nzm; k++) {
    //  for (int icrm=0; icrm<ncrms; icrm++) {
    parallel_for( "diffuse_scalar3D 4" , SimpleBounds<2>(nzm,ncrms) , YAKL_LAMBDA (int k, int icrm) {
      flux(k,icrm) = 0.0;
    });
    ...
    //     for (int i=0; i<nx; i++) {
    //       for (int icrm=0; icrm<ncrms; icrm++) {
    parallel_for( "diffuse_scalar3D 5" , SimpleBounds<4>(nzm,ny,nx,ncrms) , YAKL_LAMBDA (int k, int j, int i, int icrm) {
      if (k <= nzm-2) {
        int kc=k+1;
    ...
    //     for (int i=0; i<nx; i++) {
    //       for (int icrm=0; icrm<ncrms; icrm++) {
    parallel_for( "diffuse_scalar3D 6" , SimpleBounds<4>(nzm,ny,nx,ncrms) , YAKL_LAMBDA (int k, int j, int i, int icrm) {
      int kb=k-1;
...
void diffuse_scalar3D(real5d &field, int ind_field, real3d &fluxb, real3d &fluxt, real5d &tkh,
  ...
  if (dosgs) {
    ...
    //     for (int i=0; i<nx; i++) {
    //       for (int icrm=0; icrm<ncrms; icrm++) {
    parallel_for( "diffuse_scalar3D 7" , SimpleBounds<4>(nzm,ny,nx,ncrms) , YAKL_LAMBDA (int k, int j, int i, int icrm) {
      dfdt(k,j,i,icrm)=0.0;
    });
    ...
    //     for (int i=0; i<nx+1; i++) {
    //       for (int icrm=0; icrm<ncrms; icrm++) {
    parallel_for( "diffuse_scalar3D 8" , SimpleBounds<4>(nzm,ny+1,nx+1,ncrms) , YAKL_LAMBDA (int k, int j, int i, int icrm) {
      if (j >= 1) {
        int ic=i+1;
        real rdx5=0.5*rdx2 * grdf_x(k,icrm);
        real tkx=rdx5*(tkh(ind_tkh,k,j+offy_d-1,i+offx_d-1,icrm)+tkh(ind_tkh,k,j+offy_d-1,ic+offx_d-1,icrm));
    ...
    //     for (int i=0; i<nx; i++) {
    //       for (int icrm=0; icrm<ncrms; icrm++) {
    parallel_for( "diffuse_scalar3D 9" , SimpleBounds<4>(nzm,ny,nx,ncrms) , YAKL_LAMBDA (int k, int j, int i, int icrm) {
      int ib=i-1;
      dfdt(k,j,i,icrm)=dfdt(k,j,i,icrm)-(flx_x(k+offz_flx,j+offy_flx,i+offx_flx,icrm)-
                                         flx_x(k+offz_flx,j+offy_flx,ib+offx_flx,icrm));
      int jb=j-1;
    ...
    // for (int k=0; k<nzm; k++) {
    //  for (int icrm=0; icrm<ncrms; icrm++) {
    parallel_for( "diffuse_scalar3D 10" , SimpleBounds<2>(nzm,ncrms) , YAKL_LAMBDA (int k, int icrm) {
      flux(k,icrm) = 0.0;
    });
    ...
    //     for (int i=0; i<nx; i++) {
    //       for (int icrm=0; icrm<ncrms; icrm++) {
    parallel_for( "diffuse_scalar3D 11" , SimpleBounds<4>(nzm,ny,nx,ncrms) , YAKL_LAMBDA (int k, int j, int i, int icrm) {
      if (k <= nzm-2) {
        int kc=k+1;
        real rhoi = rhow(kc,icrm)/adzw(kc,icrm);
        real rdz2 = 1.0/(dz(icrm)*dz(icrm));
    ...
    //     for (int i=0; i<nx; i++) {
    //       for (int icrm=0; icrm<ncrms; icrm++) {
    parallel_for( "diffuse_scalar3D 12" , SimpleBounds<4>(nzm,ny,nx,ncrms) , YAKL_LAMBDA (int k, int j, int i, int icrm) {
      int kb=k-1;
      real rhoi = 1.0/(adz(k,icrm)*rho(k,icrm));
      dfdt(k,j,i,icrm)=dtn*(dfdt(k,j,i,icrm)-(flx_z(k+offz_flx,j+offy_flx,i+offx_flx,icrm)-
                                              flx_z(kb+offz_flx,j+offy_flx,i+offx_flx,icrm))*rhoi);
...
void diffuse_scalar3D(real5d &field, int ind_field, real4d &fluxb, int ind_fluxb, real4d &fluxt,
  ...
  if (dosgs) {
    ...
    //     for (int i=0; i<nx; i++) {
    //       for (int icrm=0; icrm<ncrms; icrm++) {
    parallel_for( "diffuse_scalar3D 13" , SimpleBounds<4>(nzm,ny,nx,ncrms) , YAKL_LAMBDA (int k, int j, int i, int icrm) {
      dfdt(k,j,i,icrm)=0.0;
    });
    ...
    //     for (int i=0; i<nx+1; i++) {
    //       for (int icrm=0; icrm<ncrms; icrm++) {
    parallel_for( "diffuse_scalar3D 14" , SimpleBounds<4>(nzm,ny+1,nx+1,ncrms) , YAKL_LAMBDA (int k, int j, int i, int icrm) {
      if (j >= 1) {
        int ic=i+1;
        real rdx5=0.5*rdx2 * grdf_x(k,icrm);
        real tkx=rdx5*(tkh(ind_tkh,k,j+offy_d-1,i+offx_d-1,icrm)+tkh(ind_tkh,k,j+offy_d-1,ic+offx_d-1,icrm));
    ...
    //     for (int i=0; i<nx; i++) {
    //       for (int icrm=0; icrm<ncrms; icrm++) {
    parallel_for( "diffuse_scalar3D 15" , SimpleBounds<4>(nzm,ny,nx,ncrms) , YAKL_LAMBDA (int k, int j, int i, int icrm) {
      int ib=i-1;
      dfdt(k,j,i,icrm)=dfdt(k,j,i,icrm)-(flx_x(k+offz_flx,j+offy_flx,i+offx_flx,icrm)-
                                         flx_x(k+offz_flx,j+offy_flx,ib+offx_flx,icrm));
      int jb=j-1;
    ...
    // for (int k=0; k<nzm; k++) {
    //  for (int icrm=0; icrm<ncrms; icrm++) {
    parallel_for( "diffuse_scalar3D 16" , SimpleBounds<2>(nzm,ncrms) , YAKL_LAMBDA (int k, int icrm) {
    ...
    //     for (int i=0; i<nx; i++) {
    //       for (int icrm=0; icrm<ncrms; icrm++) {
    parallel_for( "diffuse_scalar3D 17" , SimpleBounds<4>(nzm,ny,nx,ncrms) , YAKL_LAMBDA (int k, int j, int i, int icrm) {
      if (k <= nzm-2) {
        int kc=k+1;
        real rhoi = rhow(kc,icrm)/adzw(kc,icrm);
        real rdz2 = 1.0/(dz(icrm)*dz(icrm));
    ...
    //     for (int i=0; i<nx; i++) {
    //       for (int icrm=0; icrm<ncrms; icrm++) {
    parallel_for( "diffuse_scalar3D 18" , SimpleBounds<4>(nzm,ny,nx,ncrms) , YAKL_LAMBDA (int k, int j, int i, int icrm) {
      int kb=k-1;
      real rhoi = 1.0/(adz(k,icrm)*rho(k,icrm));
      dfdt(k,j,i,icrm)=dtn*(dfdt(k,j,i,icrm)-(flx_z(k+offz_flx,j+offy_flx,i+offx_flx,icrm)-
                                              flx_z(kb+offz_flx,j+offy_flx,i+offx_flx,icrm))*rhoi);
