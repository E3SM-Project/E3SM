...
void advect_scalar3D(real4d &f, real2d &flux) {
  ...
  parallel_for( "advect_scalar3D 1" , SimpleBounds<4>(nzm,ny+4,nx+4,ncrms) , YAKL_LAMBDA (int k, int j, int i, int icrm) {
  ...
  if (dowallx) {
    if (rank%nsubdomains_x == 0) {
      ...
      //       for (int icrm=0; icrm<ncrms; icrm++) {
      parallel_for( "advect_scalar3D 2" , SimpleBounds<4>(nzm,dimy_u,1-dimx1_u+1,ncrms) , YAKL_LAMBDA (int k, int j, int i, int icrm) {
        u(k,j,i,icrm) = 0.0;
      });
    }
    if (rank%nsubdomains_x == nsubdomains_x-1) {
      ...
      //       for (int icrm=0; icrm<ncrms; icrm++) {
      parallel_for( "advect_scalar3D 3" , SimpleBounds<4>(nzm,dimy_u,dimx2_u-(nx+1)+1,ncrms) , YAKL_LAMBDA (int k, int j, int i, int icrm) {
        int iInd = i+(nx+2);
        u(k,j,iInd,icrm) = 0.0;
      });
    }
  ...
  if (dowally) {
    if (rank < nsubdomains_x) {
      ...
      //       for (int icrm=0; icrm<ncrms; icrm++) {
      parallel_for( "advect_scalar3D 4" , SimpleBounds<4>(nzm,1-dimy1_v+1,dimx_v,ncrms) , YAKL_LAMBDA (int k, int j, int i, int icrm) {
        v(k,j,i,icrm) = 0.0;
      });
    }
    if (rank > nsubdomains-nsubdomains_x-1) {
      ...
      //     for (int i=0; i<dimx_v; i++) {
      //       for (int icrm=0; icrm<ncrms; icrm++) {
      parallel_for( "advect_scalar3D 5" , SimpleBounds<4>(nzm,dimy2_v-(ny+1)+1,dimx_v,ncrms) , YAKL_LAMBDA (int k, int j, int i, int icrm) {
        int jInd = j+(ny+2);
        v(k,jInd,i,icrm) = 0.0;
      });
    }
  ...
  if (nonos) {
    ...
    //       for (int icrm=0; icrm<ncrms; icrm++) {
    parallel_for( "advect_scalar3D 6" , SimpleBounds<4>(nzm,ny+2,nx+2,ncrms) , YAKL_LAMBDA (int k, int j, int i, int icrm) {
  ...
  //       for (int icrm=0; icrm<ncrms; icrm++) {
  parallel_for( "advect_scalar3D 7" , SimpleBounds<4>(nzm,ny+5,nx+5,ncrms) , YAKL_LAMBDA (int k, int j, int i, int icrm) {
    int kb=max(0,k-1);
    if (j <= ny+3){
  ...
  // for (int k=0; k<nzm; k++) {
  //  for (int icrm=0; icrm<ncrms; icrm++) {
  parallel_for( "advect_scalar3D 8" , SimpleBounds<2>(nzm,ncrms) , YAKL_LAMBDA (int k, int icrm) {
  ...
  //     for (int i=0; i<nx+4; i++) {
  //       for (int icrm=0; icrm<ncrms; icrm++) {
  parallel_for( "advect_scalar3D 9" , SimpleBounds<4>(nzm,ny+4,nx+4,ncrms) , YAKL_LAMBDA (int k, int j, int i, int icrm) {
  ...
  //       for (int icrm=0; icrm<ncrms; icrm++) {
  parallel_for( "advect_scalar3D 10" , SimpleBounds<4>(nzm,ny+3,nx+3,ncrms) , YAKL_LAMBDA (int k, int j, int i, int icrm) {
    if (j <= ny+1) {
      int kc=min(nzm-1,k+1);
      int kb=max(0,k-1);
  ...
  //     for (int i=0; i<nx+4; i++) {
  //       for (int icrm=0; icrm<ncrms; icrm++) {
  parallel_for( "advect_scalar3D 11" , SimpleBounds<4>(nzm,ny+4,nx+4,ncrms) , YAKL_LAMBDA (int k, int j, int i, int icrm) {
    www(0,j,i,icrm) = 0.0;
  });
  ...
  if (nonos) {
    ...
    //     for (int i=0; i<nx+2; i++) {
    //       for (int icrm=0; icrm<ncrms; icrm++) {
    parallel_for( "advect_scalar3D 12" , SimpleBounds<4>(nzm,ny+2,nx+2,ncrms) , YAKL_LAMBDA (int k, int j, int i, int icrm) {
      int kc=min(nzm-1,k+1);
      int kb=max(0,k-1);
      int jb=j-1;
      int jc=j+1;
    ...
    //     for (int i=0; i<nx+2; i++) {
    //       for (int icrm=0; icrm<ncrms; icrm++) {
    parallel_for( "advect_scalar3D 13" , SimpleBounds<4>(nzm,ny+2,nx+2,ncrms) , YAKL_LAMBDA (int k, int j, int i, int icrm) {
      int kc=min(nzm-1,k+1);
      int jc=j+1;
      int ic=i+1;
    ...
    //       for (int icrm=0; icrm<ncrms; icrm++) {
    parallel_for( "advect_scalar3D 14" , SimpleBounds<4>(nzm,ny+1,nx+1,ncrms) , YAKL_LAMBDA (int k, int j, int i, int icrm) {
      if (j <= ny-1) {
        int ib=i-1;
  ...
  //     for (int i=0; i<nx; i++) {
  //       for (int icrm=0; icrm<ncrms; icrm++) {
  parallel_for( "advect_scalar3D 15" , SimpleBounds<4>(nzm,ny,nx,ncrms) , YAKL_LAMBDA (int k, int j, int i, int icrm) {
...
void advect_scalar3D(real5d &f, int ind_f, real2d &flux) {
  ...
  //     for (int i=0; i<nx+4; i++) {
  //       for(int icrm=0; icrm<ncrms; icrm++) {
  parallel_for( "advect_scalar3D 16" , SimpleBounds<4>(nzm,ny+4,nx+4,ncrms) , YAKL_LAMBDA (int k, int j, int i, int icrm) {
    www(nz-1,j,i,icrm)=0.0;
  });
  ...
  if (dowallx) {
    if (rank%nsubdomains_x == 0) {
      ...
      //     for (int i=0; i<1-dimx1_u+1; i++) {
      //       for (int icrm=0; icrm<ncrms; icrm++) {
      parallel_for( "advect_scalar3D 17" , SimpleBounds<4>(nzm,dimy_u,1-dimx1_u+1,ncrms) , YAKL_LAMBDA (int k, int j, int i, int icrm) {
        u(k,j,i,icrm) = 0.0;
      });
    }
    if (rank%nsubdomains_x == nsubdomains_x-1) {
      ...
      //     for (int i=0; i<dimx2_u-(nx+1)+1; i++) {
      //       for (int icrm=0; icrm<ncrms; icrm++) {
      parallel_for( "advect_scalar3D 18" , SimpleBounds<4>(nzm,dimy_u,dimx2_u-(nx+1)+1,ncrms) , YAKL_LAMBDA (int k, int j, int i, int icrm) {
        int iInd = i+(nx+2);
        u(k,j,iInd,icrm) = 0.0;
      });
    }
  ...
  if (dowally) {
    if (rank < nsubdomains_x) {
      ...
      //     for (int i=0; i<dimx_v; i++) {
      //       for (int icrm=0; icrm<ncrms; icrm++) {
      parallel_for( "advect_scalar3D 19" , SimpleBounds<4>(nzm,1-dimy1_v+1,dimx_v,ncrms) , YAKL_LAMBDA (int k, int j, int i, int icrm) {
        v(k,j,i,icrm) = 0.0;
      });
    }
    if (rank > nsubdomains-nsubdomains_x-1) {
      ...
      //     for (int i=0; i<dimx_v; i++) {
      //       for (int icrm=0; icrm<ncrms; icrm++) {
      parallel_for( "advect_scalar3D 20" , SimpleBounds<4>(nzm,dimy2_v-(ny+1)+1,dimx_v,ncrms) , YAKL_LAMBDA (int k, int j, int i, int icrm) {
        int jInd = j+(ny+2);
        v(k,jInd,i,icrm) = 0.0;
      });
    }
  ...
  if (nonos) {
    ...
    //     for (int i=0; i<nx+2; i++) {
    //       for (int icrm=0; icrm<ncrms; icrm++) {
    parallel_for( "advect_scalar3D 21" , SimpleBounds<4>(nzm,ny+2,nx+2,ncrms) , YAKL_LAMBDA (int k, int j, int i, int icrm) {
      int kc=min(nzm-1,k+1);
      int kb=max(0,k-1);
      int jb=j-1;
      int jc=j+1;
  ...
  //     for (int i=0; i<nx+5; i++) {
  //       for (int icrm=0; icrm<ncrms; icrm++) {
  parallel_for( "advect_scalar3D 22" , SimpleBounds<4>(nzm,ny+5,nx+5,ncrms) , YAKL_LAMBDA (int k, int j, int i, int icrm) {
    int kb=max(0,k-1);
    if (j <= ny+3){
  ...
  // for (int k=0; k<nzm; k++) {
  //  for (int icrm=0; icrm<ncrms; icrm++) {
  parallel_for( "advect_scalar3D 23" , SimpleBounds<2>(nzm,ncrms) , YAKL_LAMBDA (int k, int icrm) {
    irho(k,icrm) = 1.0/rho(k,icrm);
    iadz(k,icrm) = 1.0/adz(k,icrm);
    irhow(k,icrm) = 1.0/(rhow(k,icrm)*adz(k,icrm));
  });
  ...
  //     for (int i=0; i<nx+4; i++) {
  //       for (int icrm=0; icrm<ncrms; icrm++) {
  parallel_for( "advect_scalar3D 24" , SimpleBounds<4>(nzm,ny+4,nx+4,ncrms) , YAKL_LAMBDA (int k, int j, int i, int icrm) {
    if (i >= 2 && i <= nx+1 && j >= 2 && j <= ny+1) {
      yakl::atomicAdd(flux(k,icrm),www(k,j,i,icrm));
    }
  ...
  //     for (int i=0; i<nx+3; i++) {
  //       for (int icrm=0; icrm<ncrms; icrm++) {
  parallel_for( "advect_scalar3D 25" , SimpleBounds<4>(nzm,ny+3,nx+3,ncrms) , YAKL_LAMBDA (int k, int j, int i, int icrm) {
    if (j <= ny+1) {
      int kc=min(nzm-1,k+1);
      int kb=max(0,k-1);
      real dd=2.0/(kc-kb)/adz(k,icrm);
  ...
  //     for (int i=0; i<nx+4; i++) {
  //       for (int icrm=0; icrm<ncrms; icrm++) {
  parallel_for( "advect_scalar3D 26" , SimpleBounds<4>(nzm,ny+4,nx+4,ncrms) , YAKL_LAMBDA (int k, int j, int i, int icrm) {
    www(0,j,i,icrm) = 0.0;
  });
  ...
  if (nonos) {
    ...
    //     for (int i=0; i<nx+2; i++) {
    //       for (int icrm=0; icrm<ncrms; icrm++) {
    parallel_for( "advect_scalar3D 27" , SimpleBounds<4>(nzm,ny+2,nx+2,ncrms) , YAKL_LAMBDA (int k, int j, int i, int icrm) {
      int kc=min(nzm-1,k+1);
      int kb=max(0,k-1);
      int jb=j-1;
      int jc=j+1;
    ...
    //     for (int i=0; i<nx+2; i++) {
    //       for (int icrm=0; icrm<ncrms; icrm++) {
    parallel_for( "advect_scalar3D 28" , SimpleBounds<4>(nzm,ny+2,nx+2,ncrms) , YAKL_LAMBDA (int k, int j, int i, int icrm) {
      int kc=min(nzm-1,k+1);
      int jc=j+1;
      int ic=i+1;
    ...
    //     for (int i=0; i<nx+1; i++) {
    //       for (int icrm=0; icrm<ncrms; icrm++) {
    parallel_for( "advect_scalar3D 29" , SimpleBounds<4>(nzm,ny+1,nx+1,ncrms) , YAKL_LAMBDA (int k, int j, int i, int icrm) {
      if (j <= ny-1) {
        int ib=i-1;
        uuu(k,j+offy_uuu,i+offx_uuu,icrm) = 
              pp3(uuu(k,j+offy_uuu,i+offx_uuu,icrm))*min(1.0,min(mx(k,j+offy_m,i+offx_m,icrm), mn(k,j+offy_m,ib+offx_m,icrm)))
  ...
  //     for (int i=0; i<nx; i++) {
  //       for (int icrm=0; icrm<ncrms; icrm++) {
  parallel_for( "advect_scalar3D 30" , SimpleBounds<4>(nzm,ny,nx,ncrms) , YAKL_LAMBDA (int k, int j, int i, int icrm) {
    // MK: added fix for very small negative values (relative to positive values)
    //     especially  when such large numbers as
    //     hydrometeor concentrations are advected. The reason for negative values is
    //     most likely truncation error.
...
void advect_scalar3D(real5d &f, int ind_f, real3d &flux, int ind_flux) {
  ...
  //     for (int i=0; i<nx+4; i++) {
  //       for(int icrm=0; icrm<ncrms; icrm++) {
  parallel_for( "advect_scalar3D 31" , SimpleBounds<4>(nzm,ny+4,nx+4,ncrms) , YAKL_LAMBDA (int k, int j, int i, int icrm) {
    www(nz-1,j,i,icrm)=0.0;
  });
  ...
  if (dowallx) {
    if (rank%nsubdomains_x == 0) {
      ...
      //     for (int i=0; i<1-dimx1_u+1; i++) {
      //       for (int icrm=0; icrm<ncrms; icrm++) {
      parallel_for( "advect_scalar3D 32" , SimpleBounds<4>(nzm,dimy_u,1-dimx1_u+1,ncrms) , YAKL_LAMBDA (int k, int j, int i, int icrm) {
        u(k,j,i,icrm) = 0.0;
      });
    }
    if (rank%nsubdomains_x == nsubdomains_x-1) {
      ...
      //     for (int i=0; i<dimx2_u-(nx+1)+1; i++) {
      //       for (int icrm=0; icrm<ncrms; icrm++) {
      parallel_for( "advect_scalar3D 33" , SimpleBounds<4>(nzm,dimy_u,dimx2_u-(nx+1)+1,ncrms) , YAKL_LAMBDA (int k, int j, int i, int icrm) {
        int iInd = i+(nx+2);
        u(k,j,iInd,icrm) = 0.0;
      });
    }
  ...
  if (dowally) {
    if (rank < nsubdomains_x) {
      ...
      //     for (int i=0; i<dimx_v; i++) {
      //       for (int icrm=0; icrm<ncrms; icrm++) {
      parallel_for( "advect_scalar3D 34" , SimpleBounds<4>(nzm,1-dimy1_v+1,dimx_v,ncrms) , YAKL_LAMBDA (int k, int j, int i, int icrm) {
        v(k,j,i,icrm) = 0.0;
      });
    }
    if (rank > nsubdomains-nsubdomains_x-1) {
      ...
      //     for (int i=0; i<dimx_v; i++) {
      //       for (int icrm=0; icrm<ncrms; icrm++) {
      parallel_for( "advect_scalar3D 35" , SimpleBounds<4>(nzm,dimy2_v-(ny+1)+1,dimx_v,ncrms) , YAKL_LAMBDA (int k, int j, int i, int icrm) {
        int jInd = j+(ny+2);
        v(k,jInd,i,icrm) = 0.0;
      });
    }
  ...
  if (nonos) {
    ...
    //     for (int i=0; i<nx+2; i++) {
    //       for (int icrm=0; icrm<ncrms; icrm++) {
    parallel_for( "advect_scalar3D 36" , SimpleBounds<4>(nzm,ny+2,nx+2,ncrms) , YAKL_LAMBDA (int k, int j, int i, int icrm) {
      int kc=min(nzm-1,k+1);
      int kb=max(0,k-1);
      int jb=j-1;
      int jc=j+1;
  ...
  //     for (int i=0; i<nx+5; i++) {
  //       for (int icrm=0; icrm<ncrms; icrm++) {
  parallel_for( "advect_scalar3D 37" , SimpleBounds<4>(nzm,ny+5,nx+5,ncrms) , YAKL_LAMBDA (int k, int j, int i, int icrm) {
    int kb=max(0,k-1);
    if (j <= ny+3){
      uuu(k,j,i,icrm)=max(0.0,u(k,j,i,icrm))*f(ind_f,k,j+offy_s-2,i-1+offx_s-2,icrm)+
                      min(0.0,u(k,j,i,icrm))*f(ind_f,k,j+offy_s-2,i+offx_s-2,icrm);
  ...
  // for (int k=0; k<nzm; k++) {
  //  for (int icrm=0; icrm<ncrms; icrm++) {
  parallel_for( "advect_scalar3D 37" , SimpleBounds<2>(nzm,ncrms) , YAKL_LAMBDA (int k, int icrm) {
    irho(k,icrm) = 1.0/rho(k,icrm);
    iadz(k,icrm) = 1.0/adz(k,icrm);
    irhow(k,icrm) = 1.0/(rhow(k,icrm)*adz(k,icrm));
  });
  ...
  //     for (int i=0; i<nx+4; i++) {
  //       for (int icrm=0; icrm<ncrms; icrm++) {
  parallel_for( "advect_scalar3D 38" , SimpleBounds<4>(nzm,ny+4,nx+4,ncrms) , YAKL_LAMBDA (int k, int j, int i, int icrm) {
    if (i >= 2 && i <= nx+1 && j >= 2 && j <= ny+1) {
  ...
  //     for (int i=0; i<nx+3; i++) {
  //       for (int icrm=0; icrm<ncrms; icrm++) {
  parallel_for( "advect_scalar3D 39" , SimpleBounds<4>(nzm,ny+3,nx+3,ncrms) , YAKL_LAMBDA (int k, int j, int i, int icrm) {
    if (j <= ny+1) {
      int kc=min(nzm-1,k+1);
      int kb=max(0,k-1);
      real dd=2.0/(kc-kb)/adz(k,icrm);
  ...
  //     for (int i=0; i<nx+4; i++) {
  //       for (int icrm=0; icrm<ncrms; icrm++) {
  parallel_for( "advect_scalar3D 40" , SimpleBounds<4>(nzm,ny+4,nx+4,ncrms) , YAKL_LAMBDA (int k, int j, int i, int icrm) {
    www(0,j,i,icrm) = 0.0;
  });
  ...
  if (nonos) {
    ...
    //     for (int i=0; i<nx+2; i++) {
    //       for (int icrm=0; icrm<ncrms; icrm++) {
    parallel_for( "advect_scalar3D 41" , SimpleBounds<4>(nzm,ny+2,nx+2,ncrms) , YAKL_LAMBDA (int k, int j, int i, int icrm) {
      int kc=min(nzm-1,k+1);
      int kb=max(0,k-1);
      int jb=j-1;
      int jc=j+1;
    ...
    //     for (int i=0; i<nx+2; i++) {
    //       for (int icrm=0; icrm<ncrms; icrm++) {
    parallel_for( "advect_scalar3D 42" , SimpleBounds<4>(nzm,ny+2,nx+2,ncrms) , YAKL_LAMBDA (int k, int j, int i, int icrm) {
      int kc=min(nzm-1,k+1);
      int jc=j+1;
      int ic=i+1;
      mx(k,j,i,icrm)=rho(k,icrm)*(mx(k,j,i,icrm)-f(ind_f,k,j+offy_s-1,i+offx_s-1,icrm))/
    ...
    //     for (int i=0; i<nx+1; i++) {
    //       for (int icrm=0; icrm<ncrms; icrm++) {
    parallel_for( "advect_scalar3D 43" , SimpleBounds<4>(nzm,ny+1,nx+1,ncrms) , YAKL_LAMBDA (int k, int j, int i, int icrm) {
      if (j <= ny-1) {
        int ib=i-1;
        uuu(k,j+offy_uuu,i+offx_uuu,icrm) = 
  ...
  //     for (int i=0; i<nx; i++) {
  //       for (int icrm=0; icrm<ncrms; icrm++) {
  parallel_for( "advect_scalar3D 44" , SimpleBounds<4>(nzm,ny,nx,ncrms) , YAKL_LAMBDA (int k, int j, int i, int icrm) {
    // MK: added fix for very small negative values (relative to positive values)
    //     especially  when such large numbers as
    //     hydrometeor concentrations are advected. The reason for negative values is
    //     most likely truncation error.
