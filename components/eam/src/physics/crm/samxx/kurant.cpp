...
void kurant () {
  ...
  ncycle = 1;
  parallel_for( "kurant 1" , SimpleBounds<2>(nz,ncrms) , YAKL_LAMBDA (int k, int icrm) {
    wm(k,icrm) = 0.0;
    uhm(k,icrm) = 0.0;
  });
  ...
  parallel_for( "kurant 2" , SimpleBounds<4>(nzm,ny,nx,ncrms) , YAKL_LAMBDA (int k, int j, int i, int icrm) {
    real tmp;
  ...
  // for (int k=0; k<nzm; k++) {
  //  for (int icrm=0; icrm<ncrms; icrm++) {
  parallel_for( "kurant 3" , SimpleBounds<2>(nzm,ncrms) , YAKL_LAMBDA (int k, int icrm) {
