...
void advect_all_scalars() {
  ...
  micro_precip_fall();
    ...
    parallel_for( "advect_all_scalars 1" , SimpleBounds<4>(nzm,ny,nx,ncrms) , YAKL_LAMBDA (int k, int j, int i, int icrm) {
       esmt_min(icrm) = min(min(u_esmt(k,j+offy_s,i+offx_s,icrm), v_esmt(k,j+offy_s,i+offx_s,icrm)), esmt_min(icrm));
    });

    parallel_for( "advect_all_scalars 2" , SimpleBounds<4>(nzm,dimy_s,dimx_s,ncrms) , YAKL_LAMBDA (int k, int j, int i, int icrm) {
    ...
    parallel_for( "advect_all_scalars 3" , SimpleBounds<4>(nzm,dimy_s,dimx_s,ncrms), YAKL_LAMBDA (int k, int j, int i, int icrm) {
      u_esmt(k,j,i,icrm) = u_esmt(k,j,i,icrm) - esmt_offset(icrm);
      v_esmt(k,j,i,icrm) = v_esmt(k,j,i,icrm) - esmt_offset(icrm);
   });
#endif
