...
void diffuse_scalar2D(real4d &field, real3d &fluxb, real3d &fluxt, real5d &tkh,
  ...
  if (dosgs || docolumn) {
    ...
    parallel_for( "diffuse_scalar2D 1" , SimpleBounds<3>(nzm,nx,ncrms) , YAKL_LAMBDA (int k, int i, int icrm) {
      dfdt(k,j,i,icrm)=0.0;
    });
    ...
    if (!docolumn) {
      ...
      //    for (int icrm=0; icrm<ncrms; icrm++) {
      parallel_for( "diffuse_scalar2D 2" , SimpleBounds<3>(nzm,nx+1,ncrms) , YAKL_LAMBDA (int k, int i, int icrm) {
      ...
      //  for (int i=0; i<nx; i++) {
      //    for (int icrm=0; icrm<ncrms; icrm++) {
      parallel_for( "diffuse_scalar2D 3" , SimpleBounds<3>(nzm,nx,ncrms) , YAKL_LAMBDA (int k, int i, int icrm) {
        int ib=i-1;
    ...
    // for (int k=0; k<nzm; k++) {
    //  for (int icrm=0; icrm<ncrms; icrm++) {
    parallel_for( "diffuse_scalar2D 4" , SimpleBounds<2>(nzm,ncrms) , YAKL_LAMBDA (int k, int icrm) {
      flux(k,icrm) = 0.0;
    });
    ...
    //  for (int i=0; i<nx; i++) {
    //    for (int icrm=0; icrm<ncrms; icrm++) {
    parallel_for( "diffuse_scalar2D 5" , SimpleBounds<3>(nzm,nx,ncrms) , YAKL_LAMBDA (int k, int i, int icrm) {
      if (k <= nzm-2) {
        int kc=k+1;
    ...
    //     for (int i=0; i<nx; i++) {
    //       for (int icrm=0; icrm<ncrms; icrm++) {
    parallel_for( "diffuse_scalar2D 6" , SimpleBounds<3>(nzm,nx,ncrms) , YAKL_LAMBDA (int k, int i, int icrm) {
      int kb=k-1;
...
void diffuse_scalar2D(real5d &field, int ind_field, real3d &fluxb, real3d &fluxt,
  ...
  if (dosgs || docolumn) {
    ...
    //  for (int i=0; i<nx; i++) {
    //    for (int icrm=0; icrm<ncrms; icrm++) {
    parallel_for( "diffuse_scalar2D 7" , SimpleBounds<3>(nzm,nx,ncrms) , YAKL_LAMBDA (int k, int i, int icrm) {
      dfdt(k,j,i,icrm)=0.0;
    });
    ...
    if (!docolumn) {
      ...
      //  for (int i=0; i<nx+1; i++) {
      //    for (int icrm=0; icrm<ncrms; icrm++) {
      parallel_for( "diffuse_scalar2D 8" , SimpleBounds<3>(nzm,nx+1,ncrms) , YAKL_LAMBDA (int k, int i, int icrm) {
        real rdx5=0.5*rdx2 * grdf_x(k,icrm);
        int ic=i+1;
        real tkx=rdx5*(tkh(ind_tkh,k,j,i+offx_d-1,icrm)+tkh(ind_tkh,k,j,ic+offx_d-1,icrm));
      ...
      //  for (int i=0; i<nx; i++) {
      //    for (int icrm=0; icrm<ncrms; icrm++) {
      parallel_for( "diffuse_scalar2D 9" , SimpleBounds<3>(nzm,nx,ncrms) , YAKL_LAMBDA (int k, int i, int icrm) {
        int ib=i-1;
        dfdt(k,j,i,icrm)=dfdt(k,j,i,icrm)-(flx(k+offz_flx,j,i+offx_flx,icrm)-flx(k+offz_flx,j,ib+offx_flx,icrm));
      });
    }
    ...
    // for (int k=0; k<nzm; k++) {
    //  for (int icrm=0; icrm<ncrms; icrm++) {
    parallel_for( "diffuse_scalar2D 10" , SimpleBounds<2>(nzm,ncrms) , YAKL_LAMBDA (int k, int icrm) {
      flux(k,icrm) = 0.0;
    });
    ...
    //  for (int i=0; i<nx; i++) {
    //    for (int icrm=0; icrm<ncrms; icrm++) {
    parallel_for( "diffuse_scalar2D 11" , SimpleBounds<3>(nzm,nx,ncrms) , YAKL_LAMBDA (int k, int i, int icrm) {
      if (k <= nzm-2) {
        int kc=k+1;
        real rhoi = rhow(kc,icrm)/adzw(kc,icrm);
        real rdz2=1.0/(dz(icrm)*dz(icrm));
    ...
    //     for (int i=0; i<nx; i++) {
    //       for (int icrm=0; icrm<ncrms; icrm++) {
    parallel_for( "diffuse_scalar2D 12" , SimpleBounds<3>(nzm,nx,ncrms) , YAKL_LAMBDA (int k, int i, int icrm) {
      int kb=k-1;
      real rhoi = 1.0/(adz(k,icrm)*rho(k,icrm));
      dfdt(k,j,i,icrm)=dtn*(dfdt(k,j,i,icrm)-(flx(k+offz_flx,j,i+offx_flx,icrm)-flx(kb+offz_flx,j,i+offx_flx,icrm))*rhoi);
...
void diffuse_scalar2D(real5d &field, int ind_field, real4d &fluxb, int ind_fluxb, real4d &fluxt,
  ...
  if (dosgs || docolumn) {
    ...
    //  for (int i=0; i<nx; i++) {
    //    for (int icrm=0; icrm<ncrms; icrm++) {
    parallel_for( "diffuse_scalar2D 13" , SimpleBounds<3>(nzm,nx,ncrms) , YAKL_LAMBDA (int k, int i, int icrm) {
      dfdt(k,j,i,icrm)=0.0;
    });
    ...
    if (!docolumn) {
      ...
      //  for (int i=0; i<nx+1; i++) {
      //    for (int icrm=0; icrm<ncrms; icrm++) {
      parallel_for( "diffuse_scalar2D 14" , SimpleBounds<3>(nzm,nx+1,ncrms) , YAKL_LAMBDA (int k, int i, int icrm) {
        real rdx5=0.5*rdx2 * grdf_x(k,icrm);
        int ic=i+1;
        real tkx=rdx5*(tkh(ind_tkh,k,j,i+offx_d-1,icrm)+tkh(ind_tkh,k,j,ic+offx_d-1,icrm));
        flx(k+offz_flx,j,i,icrm)=-tkx*(field(ind_field,k,j,ic+offx_s-1,icrm)-field(ind_field,k,j,i+offx_s-1,icrm));
      ...
      //  for (int i=0; i<nx; i++) {
      //    for (int icrm=0; icrm<ncrms; icrm++) {
      parallel_for( "diffuse_scalar2D 15" , SimpleBounds<3>(nzm,nx,ncrms) , YAKL_LAMBDA (int k, int i, int icrm) {
        int ib=i-1;
        dfdt(k,j,i,icrm)=dfdt(k,j,i,icrm)-(flx(k+offz_flx,j,i+offx_flx,icrm)-flx(k+offz_flx,j,ib+offx_flx,icrm));
      });
    }
    ...
    // for (int k=0; k<nzm; k++) {
    //  for (int icrm=0; icrm<ncrms; icrm++) {
    parallel_for( "diffuse_scalar2D 16" , SimpleBounds<2>(nzm,ncrms) , YAKL_LAMBDA (int k, int icrm) {
    ...
    //  for (int i=0; i<nx; i++) {
    //    for (int icrm=0; icrm<ncrms; icrm++) {
    parallel_for( "diffuse_scalar2D 17" , SimpleBounds<3>(nzm,nx,ncrms) , YAKL_LAMBDA (int k, int i, int icrm) {
      if (k <= nzm-2) {
        int kc=k+1;
        real rhoi = rhow(kc,icrm)/adzw(kc,icrm);
        real rdz2=1.0/(dz(icrm)*dz(icrm));
    ...
    //     for (int i=0; i<nx; i++) {
    //       for (int icrm=0; icrm<ncrms; icrm++) {
    parallel_for( "diffuse_scalar2D 18" , SimpleBounds<3>(nzm,nx,ncrms) , YAKL_LAMBDA (int k, int i, int icrm) {
      int kb=k-1;
      real rhoi = 1.0/(adz(k,icrm)*rho(k,icrm));
      dfdt(k,j,i,icrm)=dtn*(dfdt(k,j,i,icrm)-(flx(k+offz_flx,j,i+offx_flx,icrm)-flx(kb+offz_flx,j,i+offx_flx,icrm))*rhoi);
      field(ind_field,k,j,i+offx_s,icrm)=field(ind_field,k,j,i+offx_s,icrm) + dfdt(k,j,i,icrm);
