...
void periodic(int flag) {
  ...
  if (flag == 0) {
    ...
    parallel_for( "periodic 1" , SimpleBounds<3>(ny,nx,ncrms) , YAKL_LAMBDA (int j, int i, int icrm) {
    ...
    //       for (int icrm=0; icrm<ncrms; icrm++) {
    parallel_for( "periodic 2" , SimpleBounds<3>(ny+2*YES3D,nxp3,ncrms) , YAKL_LAMBDA (int j, int i, int icrm) {
    ...
    //     for (int i=0; i<nxp3; i++) {
    //       for (int icrm=0; icrm<ncrms; icrm++) {
    parallel_for( "periodic 3" , SimpleBounds<3>(ny+2*YES3D,nxp3,ncrms) , YAKL_LAMBDA (int j, int i, int icrm) {
      int jStart = 1-YES3D-1;
      int jInd = 1-YES3D-1+j;
      int jEnd = ny+YES3D-1;
      int iStart = 0-1;
