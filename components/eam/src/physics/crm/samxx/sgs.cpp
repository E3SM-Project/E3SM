...
void kurant_sgs(real &cfl) {
  ...
  parallel_for( "sgs 1" , SimpleBounds<2>(nzm,ncrms) , YAKL_LAMBDA (int k, int icrm) {
    tkhmax(k,icrm) = 0.;
  });
  ...
  //     for (int i=0; i<nx; i++) {
  //       for (int icrm=0; icrm<ncrms; icrm++) {
  parallel_for( "sgs 2" , SimpleBounds<4>(nzm,ny,nx,ncrms) , YAKL_LAMBDA (int k, int j, int i, int icrm) {
    yakl::atomicMax( tkhmax(k,icrm) , sgs_field_diag(1,k,offy_d+j,offx_d+i,icrm) );
  });

  // for (int k=0; k<nzm; k++) {
  //   for (int icrm=0; icrm < ncrms; icrm++) {
  parallel_for( "sgs 3" , SimpleBounds<2>(nzm,ncrms) , YAKL_LAMBDA (int k, int icrm) {
...
void sgs_proc() {
  ...
  //       for (int icrm=0; icrm<ncrms; icrm++) {
  parallel_for( "sgs 4" , SimpleBounds<4>(nzm,dimy_s,dimx_s,ncrms) , YAKL_LAMBDA (int k, int j, int i, int icrm) {
  ...
  //       for (int icrm=0; icrm<ncrms; icrm++) {
  parallel_for( "sgs 5" , SimpleBounds<4>(nzm,dimy_d,dimx_d,ncrms) , YAKL_LAMBDA (int k, int j, int i, int icrm) {
...
void sgs_init() {
  ...
  if (nrestart == 0) {
    ...
    //      for (int i=0; i<dimx_s; i++) {
    //        for (int icrm=0; icrm<ncrms; icrm++) {
    parallel_for( "sgs 6" , SimpleBounds<5>(nsgs_fields,nzm,dimy_s,dimx_s,ncrms) , YAKL_LAMBDA (int l, int k, int j, int i, int icrm) {
    ...
    //      for (int i=0; i<dimx_d; i++) {
    //        for (int icrm=0; icrm<ncrms; icrm++) {
    parallel_for( "sgs 7" , SimpleBounds<5>(nsgs_fields_diag,nzm,dimy_d,dimx_d,ncrms) , 
  ...
  if (les) {
    // for (int k=0; k<nzm; k++) {
    //  for (int icrm=0; icrm<ncrms; icrm++) {
    parallel_for( "sgs 8" , SimpleBounds<2>(nzm,ncrms) , YAKL_LAMBDA (int k, int icrm) {
    ...
    // for (int k=0; k<nzm; k++) {
    //  for (int icrm=0; icrm<ncrms; icrm++) {
    parallel_for( "sgs 9" , SimpleBounds<2>(nzm,ncrms) , YAKL_LAMBDA (int k, int icrm) {
      real tmp1 = (adz(k,icrm)*dz(icrm));
      real tmp2 = (adz(k,icrm)*dz(icrm));
