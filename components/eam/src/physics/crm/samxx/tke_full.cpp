...
void tke_full(real5d &tke, int ind_tke, real5d &tk, int ind_tk, real5d &tkh, int ind_tkh) {
  ...
  YAKL_SCOPE( tkelediss      , :: tkelediss );
  YAKL_SCOPE( tkesbdiss      , :: tkesbdiss );
  YAKL_SCOPE( tkesbshear     , :: tkesbshear );
  YAKL_SCOPE( tkesbbuoy      , :: tkesbbuoy );
  YAKL_SCOPE( grdf_x         , :: grdf_x );
  YAKL_SCOPE( grdf_y         , :: grdf_y );
  YAKL_SCOPE( grdf_z         , :: grdf_z );
  YAKL_SCOPE( dosmagor       , :: dosmagor );
  ...
  parallel_for( "tke_full 1" , SimpleBounds<3>(ny,nx,ncrms) , YAKL_LAMBDA (int j, int i, int icrm) {
  ...
  //     for (int i=0; i<nx; i++) {
  //       for (int icrm=0; icrm<ncrms; icrm++) {
  parallel_for( "tke_full 2" , SimpleBounds<4>(nzm-1,ny,nx,ncrms) , YAKL_LAMBDA (int k, int j, int i, int icrm) {
    int kb,kc;
  ...
  // for (int k=0; k<nzm-1; k++) {
  //  for (int icrm=0; icrm<ncrms; icrm++) {
  parallel_for( "tke_full 3" , SimpleBounds<2>(nz,ncrms) , YAKL_LAMBDA (int k, int icrm) {
  ...
  //     for (int i=0; i<nx; i++) {
  //       for (int icrm=0; icrm<ncrms; icrm++) {
  parallel_for( "tke_full 4" , SimpleBounds<4>(nzm-1,ny,nx,ncrms) , YAKL_LAMBDA (int k, int j, int i, int icrm) {
