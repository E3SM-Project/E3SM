...
inline void pam_feedback_compute_crm_mean_state( pam::PamCoupler &coupler ) {
  ...
  //------------------------------------------------------------------------------------------------
  // We will be essentially reducing a summation to these variables, so initialize them to zero
  parallel_for("Initialize horzontal means", SimpleBounds<2>(gcm_nlev,nens), YAKL_LAMBDA (int k, int iens) {
    nc_mean(k,iens) = 0;
    ni_mean(k,iens) = 0;
    qr_mean(k,iens) = 0;
    nr_mean(k,iens) = 0;
  ...
  parallel_for("Horz mean of CRM state", SimpleBounds<4>(nz,ny,nx,nens), YAKL_LAMBDA (int k, int j, int i, int iens) {
    ...
    // yakl::atomicAdd ensures only one thread performs an update at a time to avoid data races and wrong answers
    atomicAdd( nc_mean(k_gcm,iens), nc(k,j,i,iens) * r_nx_ny );
    atomicAdd( ni_mean(k_gcm,iens), ni(k,j,i,iens) * r_nx_ny );
    atomicAdd( qr_mean(k_gcm,iens), qr(k,j,i,iens) * r_nx_ny );
    atomicAdd( nr_mean(k_gcm,iens), nr(k,j,i,iens) * r_nx_ny );
    atomicAdd( qm_mean(k_gcm,iens), qm(k,j,i,iens) * r_nx_ny );
    atomicAdd( bm_mean(k_gcm,iens), bm(k,j,i,iens) * r_nx_ny );
  });
  //------------------------------------------------------------------------------------------------
