...
inline void copy_input_rad_to_coupler( pam::PamCoupler &coupler ) {
  ...
  auto gcm_qrad = dm_host.get<real const,2>("rad_qrad").createDeviceCopy();
  //------------------------------------------------------------------------------------------------
...
inline void copy_output_rad_to_gcm( pam::PamCoupler &coupler ) {
  ...
  //------------------------------------------------------------------------------------------------
  // get the coupler rad tendency variable
  auto crm_rad_temperature = dm_device.get<real,4>("crm_rad_temperature");
  auto crm_rad_qv          = dm_device.get<real,4>("crm_rad_qv");
  auto crm_rad_qc          = dm_device.get<real,4>("crm_rad_qc");
  auto crm_rad_qi          = dm_device.get<real,4>("crm_rad_qi");
  auto crm_rad_cld         = dm_device.get<real,4>("crm_rad_cld");
  //------------------------------------------------------------------------------------------------
  // wrap the host CRM state data in YAKL arrays
  auto gcm_rad_temperature = dm_host.get<real const,2>("rad_temperature").createDeviceCopy();
  auto gcm_rad_qv          = dm_host.get<real const,2>("rad_qv").createDeviceCopy();
  auto gcm_rad_qc          = dm_host.get<real const,2>("rad_qc").createDeviceCopy();
  auto gcm_rad_qi          = dm_host.get<real const,2>("rad_qi").createDeviceCopy();
  auto gcm_rad_cld         = dm_host.get<real const,2>("rad_cld").createDeviceCopy();
  //------------------------------------------------------------------------------------------------
