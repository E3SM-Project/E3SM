...
!+++ AeroCOM IND3 output
   ...
   do i = 1, ncol
      ...
      if (do_mmf_local) tau_cld_regenerate = 3600.0_r8 * 24.0_r8 * 365.0_r8
   ...
   if (do_mmf_local) then
      do m=1, pcnst
         if(species_class(m).eq.spec_class_gas) then
            ...
            fieldnamegas = trim('MMF_'//cnst_name(m)) // '_mixnuc1'
