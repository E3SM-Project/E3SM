...
!================================================================================================
  ...
  subroutine output_rad_data(  pbuf, state, cam_in, landm, coszen )
    ...
    call outfld(asdir_fldn, cam_in%asdir_mi, pcols, lchnk   )
    call outfld(asdif_fldn, cam_in%asdif_mi, pcols, lchnk   )
    call outfld(aldir_fldn, cam_in%aldir_mi, pcols, lchnk   )
    call outfld(aldif_fldn, cam_in%aldif_mi, pcols, lchnk   )
![ no need to output *_pos_mi]
    ...
    call outfld(lwup_fldn,  cam_in%lwup_mi,  pcols, lchnk   )
    call outfld(ts_fldn,    cam_in%ts_mi,    pcols, lchnk   )
#else    
    do icol = 1, ncol
       coszrs_pos(icol)  = max(coszen(icol),0._r8)
    end do
