   ...
   if(.not.use_MAML) then
      ...
      do i = 1,ncol
         ...
         if(rcol.eq.223) then! .or. rcol.eq.237) then
            ...
            nstep = get_nstep()
            write(iulog,'("CAM_EXPORT: nstep=",I5," inst_index=",I4," chunk_index=",I3," icol=",I3," rcol=",I5, &
                          "lat=",F8.3," lon=",F8.3, &
                          " soll=",F7.2," sols=",F7.2," solld=",F7.2," solsd=",F7.2, " flwds=",F7.2, &
                          " tbot=",F7.2," thot=",F7.2," qbot=",F5.3," ubot=",F7.3," vbot=",F7.3)') &
                          ,inst_index,lchnk,i,rcol,&
                          clat,clon,&
                          cam_out%soll(i),cam_out%sols(i),cam_out%solld(i),cam_out%solsd(i),cam_out%flwds(i),&
                          cam_out%tbot(i), cam_out%thbot(i), cam_out%qbot(i,1),&
                          cam_out%ubot(i), cam_out%vbot(i)
            do j = 1, num_inst_atm
               write(iulog,'("      j=",I3," soll=",F7.2," sols=",F7.2," solld=",F7.2," solsd=",F7.2," flwds=",F7.2,&
                                    " tbot=",F7.2," thbot=",F7.2," qbot=",F5.3," ubot=",F7.3," vbot=",F7.3)') &
                           j,cam_out%soll_mi(i,j),cam_out%sols_mi(i,j),cam_out%solld_mi(i,j),cam_out%solsd_mi(i,j), cam_out%flwds_mi(i,j), &
                           cam_out%tbot_mi(i,j), cam_out%thbot_mi(i,j),cam_out%qbot_mi(i,1,j),&
                           cam_out%ubot_mi(i,j), cam_out%vbot_mi(i,j)
