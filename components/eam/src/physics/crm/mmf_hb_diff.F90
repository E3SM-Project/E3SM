module mmf_hb_diff
  ...
  real(r8), parameter :: ricr  =  0.05_r8       ! Critical richardson number
...
subroutine compute_mmf_hb_diff(lchnk, ncol,             &
  ...
  !-----------------------------------------------------------------------------
  ! Purpose: calcualtion and diatnostics of turbulence related coefficients
  !-----------------------------------------------------------------------------
...
subroutine trbintd(ncol, thv, z, u, v, s2, n2, ri )
  !---------------------------------------------------------------------------
  ! Purpose:  Time dependent initialization
  ! 
  ! Method: Diagnosis of variables that do not depend on mixing assumptions or PBL depth
  !---------------------------------------------------------------------------
  ! Interface Arguments
  integer,  intent(in)  :: ncol                    ! number of atmospheric columns
  ...
  !---------------------------------------------------------------------------
  ! Compute shear squared (s2), brunt vaisaila frequency (n2) and related 
  ! richardson number (ri). Use virtual temperature to compute n2.
...
!===============================================================================
!===============================================================================
subroutine pblintd(ncol, thv, z, u, v, ustar, obklen, kbfs, pblh, wstar, zi, cldn, bge )
  !-----------------------------------------------------------------------------
  ! Purpose: Diagnose standard PBL variables
  ! 
  ! Method: 
  ...
  do i=1,ncol
    if (cldn(i,pver).ge.0.0_r8) pblh(i) = max(pblh(i),zi(i,pver) + 50._r8)
  end do
  return
...
subroutine austausch_pbl(lchnk ,ncol    ,  &
                         ...
                         ktopbl,tke     ,bge     )
  !----------------------------------------------------------------------- 
  ! Purpose: Atmospheric Boundary Layer Computation
  ! 
  ! Method: 
