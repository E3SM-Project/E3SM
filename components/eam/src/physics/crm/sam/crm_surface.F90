...
contains
  ...
  subroutine crm_surface(ncrms,bflx,z0_loc)
    use vars
    use params
    use grid, only:nx,ny
    implicit none
    integer, intent(in) :: ncrms
    real(crm_rknd), intent (in) :: bflx(ncrms,nx,ny), z0_loc(ncrms,nx,ny)
    ...
    do j=1,ny
      do i=1,nx
        do icrm = 1 , ncrms
          ...
          tau00 = rho(icrm,1) * diag_ustar(z(icrm,1),bflx(icrm,i,j),u_h0,z0_loc(icrm,i,j))**2
