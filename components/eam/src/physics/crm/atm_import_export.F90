    ...
    do c=begchunk,endchunk
       ...
       do i =1,ncols                                                               
         ...
         !! Jungmin
             ...
             ! Modified for MAML
             if(overwrite_flds) then
                cam_in(c)%cflx1_mi(i,inst_index) = -x2a(index_x2a_Faxx_evap,ig)                
                cam_in(c)%shf_mi (i,inst_index)   = -x2a(index_x2a_Faxx_sen, ig)    
                cam_in(c)%lhf_mi (i,inst_index)   = -x2a(index_x2a_Faxx_lat, ig)   
             endif
             cam_in(c)%wsx_mi      (i,inst_index) = -x2a(index_x2a_Faxx_taux,ig)
             cam_in(c)%wsy_mi      (i,inst_index) = -x2a(index_x2a_Faxx_tauy,ig)
             cam_in(c)%lwup_mi     (i,inst_index) = -x2a(index_x2a_Faxx_lwup,ig)
             cam_in(c)%asdir_mi    (i,inst_index) =  x2a(index_x2a_Sx_avsdr, ig)
             cam_in(c)%aldir_mi    (i,inst_index) =  x2a(index_x2a_Sx_anidr, ig)
             cam_in(c)%asdif_mi    (i,inst_index) =  x2a(index_x2a_Sx_avsdf, ig)
  ...
  subroutine atm_export( cam_out, a2x )
    ...
    do c=begchunk, endchunk
       ...
       do i=1,ncols
          ...
          a2x(index_a2x_Sa_u      ,ig) = cam_out(c)%ubot_mi(i,inst_index)   
          a2x(index_a2x_Sa_v      ,ig) = cam_out(c)%vbot_mi(i,inst_index)   
          a2x(index_a2x_Sa_tbot   ,ig) = cam_out(c)%tbot_mi(i,inst_index)   
          a2x(index_a2x_Sa_ptem   ,ig) = cam_out(c)%thbot_mi(i,inst_index)  
          a2x(index_a2x_Sa_pbot   ,ig) = cam_out(c)%pbot(i)   
          a2x(index_a2x_Sa_shum   ,ig) = cam_out(c)%qbot_mi(i,1,inst_index) 
          a2x(index_a2x_Sa_dens   ,ig) = cam_out(c)%rho_mi(i,inst_index)
          a2x(index_a2x_Faxa_rainc,ig) = (cam_out(c)%precc_mi(i,inst_index)-cam_out(c)%precsc_mi(i,inst_index))*1000._r8
