...
subroutine   papampollu_init ( )
  ...
  do ichem=param_first_ecpp, pcnst
    if(.not. (cnst_name_cw(ichem) == ' ')) then
      call addfld(trim(cnst_name_cw(ichem))//'EP',      (/'lev'/),'A','kg/kg/s',trim(cnst_name_cw(ichem))//' tendency from ECPP' )
      call addfld(trim(cnst_name_cw(ichem))//'ACHEM_EP',(/'lev'/),'A','kg/kg/s',trim(cnst_name_cw(ichem))//' tendency from aqueous chemistry in ECPP' )
      call addfld(trim(cnst_name_cw(ichem))//'RENM_EP', (/'lev'/),'A','kg/kg/s',trim(cnst_name_cw(ichem))//' tendency from renaming in ECPP' )
      call addfld(trim(cnst_name_cw(ichem))//'ACT_EP',  (/'lev'/),'A','kg/kg/s',trim(cnst_name_cw(ichem))//' tendency from activation/resuspension in ECPP' )
      call addfld(trim(cnst_name_cw(ichem))//'WET_EP',  (/'lev'/),'A','kg/kg/s',trim(cnst_name_cw(ichem))//' tendency from wet removable in ECPP' )
      call addfld(trim(cnst_name_cw(ichem))//'WRESU_EP',(/'lev'/),'A','kg/kg/s',trim(cnst_name_cw(ichem))//' tendency from resuspension in wet removable in ECPP' )
      call addfld(trim(cnst_name_cw(ichem))//'CONV_EP', (/'lev'/),'A','kg/kg/s',trim(cnst_name_cw(ichem))//' tendency from convective tansport in ECPP' )
  ...
  do ichem = param_first_ecpp, pcnst
    ...
    if ( trim(cnst_name(ichem)) == 'DMS' &
    .or. trim(cnst_name(ichem)) == 'SO2' &
    .or. trim(cnst_name(ichem)) == 'so4_a1') then
      if(.not. (cnst_name_cw(ichem) == ' ')) then
...
!===================================================================================================
!===================================================================================================
subroutine parampollu_driver2(state, ptend,  pbuf, dtstep_in, dtstep_pp_in,         &
                              acen_3d, abnd_3d, acen_tf_3d, abnd_tf_3d,             &
                              massflxbnd_3d, rhcen_3d, qcloudcen_3d, qlsinkcen_3d,  &
                              precrcen_3d, precsolidcen_3d, acldy_cen_tbeg_3d       )
  !---------------------------------------------------------------------------
  ! parampollu_driver2 is the interface between chemistry routines and the
  ...
  real(r8),                  intent(in) :: dtstep_pp_in  ! time step (s) for "parameterized pollutants" calculations
