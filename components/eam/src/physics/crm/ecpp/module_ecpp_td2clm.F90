...
subroutine parampollu_td240clm(                       &
    ...
    integer :: icc, iccy, idiag, ipass_area_change, ipass_check_adjust_inputs, itstep_hybrid
    ...
    if (lun62 > 0) write(lun62,'(a,2i10)') &
        'parampollu_td240clm - ktau, ntstep_hybrid', ktau, ntstep_hybrid
        ...
        do iccy = 1, 2
            ...
            if (ido_actres_tmp == 0) then
                ...
                if (inmw_of_aerosol(la) == 1) then
                    tmp_fmnact = fnact_horz(m,n,jcls,iccy,jclsy,k) * frc_ent_act
                else
                    tmp_fmnact = fmact_horz(m,n,jcls,iccy,jclsy,k) * frc_ent_act
                end if
            ...
            if (lc > 0)  tmphoriz(k,jcls,lc) = tmphoriz(k,jcls,lc) + tmp_del_ardzqc
...
vert_botqu_iccy_loop:   &
                do iccy = 1, 2
                    ...
                    if (icc == 1) then
                        ...
                        if (lc > 0) tmpverta(k,jcls,lc) = tmpverta(k,jcls,lc) + tmp_del_ardzqc
                    end if
...
vert_topqu_iccy_loop:   &
                do iccy = 1, 2
                    ...
                    ! with "pgf90 -O2", code seg-faulted until following statement 
                    !    was added.  (note that it is do-nothing, since la>0 always)
                    if (la < 0) write(*,*)  'vert_topqu gggg - icc,iupdn,ido', iccy, iupdn, ido_actres_tmp
...
subroutine parampollu_tdx_area_change(                    &
    ...
    !-----------------------------------------------------------------------    
    ! parampollu_tdx_area_change does horizontal exchange associated with sub-class area changes
    !
    ! incoming chem_sub_new holds current sub-class mixing ratios
    ! outgoing chem_sub_new holds updated sub-class mixing ratios
    !-----------------------------------------------------------------------
...
subroutine set_of_aerosol_stuff(is_aerosol, &
    ...
    use module_data_mosaic_asect, only:  ai_phase, cw_phase,   &
        massptr_aer, ncomp_aer, nphase_aer, nsize_aer, ntype_aer, numptr_aer
...
subroutine parampollu_tdx_startup(                        &
    ...
    ! arguments
    integer, intent(in) :: ktau, ktau_pp, it, jt, kts, ktebnd, ktecen
    ! ktau - time step number
    ! ktau_pp - time step number for "parameterized pollutants" calculations
    ! [its:ite, kts:kte, jts:jte] - spatial (x,z,y) indices for "tile"
    ! chem_driver and routines under it do calculations
...
subroutine parampollu_check_adjust_inputs(            &
    ...
    ! check that the mtype_updnenv_use are valid
...
30000   continue
    ...
    do k = kts+1, ktecen
        ...
        do ja = 1, 2
            jb = 3-ja
            tmpa = 0.0
            if (tmpvecaa(ja) > 0.0) tmpa = min(tmpvecbb(ja),tmpvecaa(ja))/tmpvecaa(ja)
            tmpa = max( 0.0_r8, min( 1.0_r8, tmpa ) )
...
subroutine parampollu_1clm_dumpaa(                        &
    ...
    !-----------------------------------------------------------------------
    ! this does a diagnostic print of numerous ecpp arrays
    !-----------------------------------------------------------------------
    use module_data_ecpp1
    use module_ecpp_util, only:  ecpp_error_fatal, ecpp_message
