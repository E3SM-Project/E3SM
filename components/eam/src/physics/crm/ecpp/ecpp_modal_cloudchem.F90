...
subroutine parampollu_tdx_cldchem(                     &
  ...
  use chem_mods,            only: gas_pcnst, nfs, indexm, adv_mass
  ...
  ! arguments
  integer, intent(in) :: ktau, ktau_pp, itstep_sub,   &
                         it, jt, kts, ktebnd, ktecen, itstep_hybrid
  ...
  real(r8), intent(in), dimension( kts:ktecen ) :: tcen_bar, pcen_bar, rhocen_bar, dzcen
  real(r8), intent(in), dimension( kts:ktebnd ) :: rhobnd_bar, wbnd_bar, zbnd
  ...
  real(r8), intent(in), dimension( kts:ktecen, 1:num_chem_ecpp ) :: chem_bar
        ...
        do icc = 1, 2
          ...
          if(rename_onoff_ecpp > 0 ) then
            if ((tmpa1 > afrac_cut_0p5) .and. (tmpa2 > afrac_cut_0p5)) then
              ...
              tmpb2 = 1.0_r8 - tmpb1
              tmpq1 = chem_tmpa(iccpp1,k,jcls,l)*tmpb1 + chem_tmpa(iccpp2,k,jcls,l)*tmpb2
              tmpq2 = chem_tmpc(iccpp1,k,jcls,l)*tmpb1 + chem_tmpc(iccpp2,k,jcls,l)*tmpb2
  ...
  deallocate( p_tmp, t_tmp, rho_tmp, alt_tmp, cldfra_tmp, qlsink_tmp )
  deallocate( precr_tmp, precs_tmp, precg_tmp, preci_tmp )
  ...
  deallocate( mbar, cldnum, invariants, mmr_3d, mmrcw_3d, mbar_3d )
  deallocate( qsrflx_full, qqcwsrflx_full )
