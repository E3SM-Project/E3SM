...
subroutine zm_conv_main(pcols, ncol, pver, pverp, is_first_step, time_step, &
   ...
   real(r8), dimension(pcols,pver), intent(  out) :: ent_upd         ! updraft entrainment                     [?]
   real(r8), dimension(pcols,pver), intent(  out) :: det_upd         ! updraft detrainment                     [?]
   real(r8), dimension(pcols,pver), intent(  out) :: mfx_dnd         ! downdraft mass flux                     [?]
   real(r8), dimension(pcols,pver), intent(  out) :: ent_dnd         ! downdraft entrainment                   [?]
   ...
   do i = 1,lengath
      ...
      if (mfx_upd_max(i) > 0._r8) then
         cld_bass_mass_flux(i) = min(cld_bass_mass_flux(i),1/(time_step*mfx_upd_max(i)))
      else
   ...
   do i = 1,lengath
      ...
      ! scale variables
      do k = msg+1,pver
         mfx_upd(i,k)   = mfx_upd(i,k)  *cld_bass_mass_flux(i)
         mfx_dnd(i,k)   = mfx_dnd(i,k)  *cld_bass_mass_flux(i)
         mfx_net(i,k)   = mfx_net(i,k)  *cld_bass_mass_flux(i)
         det_upd(i,k)   = det_upd(i,k)  *cld_bass_mass_flux(i)
         ent_upd(i,k)   = ent_upd(i,k)  *cld_bass_mass_flux(i)
         ent_dnd(i,k)   = ent_dnd(i,k)  *cld_bass_mass_flux(i)
