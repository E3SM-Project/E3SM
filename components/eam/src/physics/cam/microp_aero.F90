...
use ndrop,            only: ndrop_init, dropmixnuc,&
                            lnd_ocean_cdnc,so4_cdnc   !XZ prescribed CDNC
...
integer :: cdncst_idx !prescribed CDNC XZ
integer :: micro_prescribed_cdnc_opt !prescribed CDNC XZ
...
subroutine microp_aero_register
   ...
   call pbuf_add_field('CDNC_PRESC', 'physpkg',dtype_r8,(/pcols,pver/), cdncst_idx)  !prescribed cdnc XZ
   ...
   call phys_getopts(eddy_scheme_out          = eddy_scheme, &
        ...
        if(micro_do_nccons) then
            if(micro_prescribed_cdnc_opt .le. 0) then
                write(iulog,*) 'The prescribed in-cloud CDNC setting is wrong: ',&
                        micro_prescribed_cdnc_opt, micro_do_nccons     
                call endrun('double check micro_mg_nl')
            elseif( micro_prescribed_cdnc_opt.eq. 1 .and. (micro_nccons_land .le. 0.0_r8 .or. micro_nccons_ocean .le. 0.0_r8)) then
                write(iulog,*) 'Errors in constant in-cloud CDNCs over land and ocean: ',&
                         micro_nccons_land, micro_nccons_ocean
                call endrun('double check micro_mg_nl')    
            endif    
        elseif (micro_prescribed_cdnc_opt .gt. 0) then
                 write(iulog,*) 'Errors in the prescribed in-cloud CDNC setting:', &
                        micro_prescribed_cdnc_opt, micro_do_nccons
                 call endrun('double check micro_mg_nl')
        endif         
...
subroutine microp_aero_run ( &
   ...
   real(r8), pointer :: cdncst(:,:)      ! prescribed liquid in-cloud NC !XZ
   ...
   call pbuf_get_field(pbuf, cdncst_idx, cdncst)  !XZ
   ...
   if (clim_modal_aero) then
      ...
      if(micro_prescribed_cdnc_opt .eq. 1) then  !XZ land-ocean CDNC
        call t_startf('lnd_ocean_cdnc')
        call lnd_ocean_cdnc(landfrac,micro_nccons_land,micro_nccons_ocean,cdncst)
