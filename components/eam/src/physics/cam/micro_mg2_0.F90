...
subroutine micro_mg_tend ( &
     ...
     qcsinksum_rate1ord,                                         &
     naai,                         npccn,                        &
     cdncst,                               & !XZ prescribed in-cloud nc
     rndst,                        nacon,                        &
     tlat,                         qvlat,                        &
     qctend,                       qitend,                       &
     nctend,                       nitend,                       &
  ...
  real(r8), intent(in) :: cdncst(:,:)   ! (#/m3)  !!XZ 
  ...
  !--------------------------------------------------
  !! check if prescribed CDNC  XZ
  if(nccons) then 
        where (qc >= qsmall)
          nc = max(cdncst/rho*lcldm, 0._r8)
  ...
  endif !! end of the prescribed CDNC setting  !!XZ
  ...
  if(nccons) then
    nc =  ncn
    nctend = (cdncst/rho*lcldm-nc)/deltat
  else !default 
    nc = ncn
    nctend = nctend + npccn
  end if  !end of prescribed CDNC XZ
  ...
  sed_col_loop: do i=1,mgncol
   ...
   if (.not. precip_off) then
     ...
     do k=1,nlev
        ...
        !XZ dumnc(i,k) = max((nc(i,k)+nctend(i,k)*deltat)/lcldm(i,k),0._r8)
        ...
        !!prescribed CDNC setting  XZ
        !! specify droplet concentration  XZ
        if (nccons) then
          dumnc(i,k) = max(cdncst(i,k)/rho(i,k),0._r8)
        else ! default
        ...
        !XZ-- dumnc(i,k) = max((nc(i,k)+nctend(i,k)*deltat),0._r8)
        ...
        !XZ-- if (dumc(i,k).lt.qsmall) dumnc(i,k)=0._r8
     ...
     do k=1,nlev
        ...
        !XZ --dumnc(i,k) = max(nc(i,k)+nctend(i,k)*deltat,0._r8)/lcldm(i,k)
        ...
        if (dumc(i,k).ge.qsmall) then
           ...
           if (nccons) then
              ...
              nctend(i,k)=(cdncst(i,k)/rho(i,k)*lcldm(i,k)-nc(i,k))/deltat
           else  !default
