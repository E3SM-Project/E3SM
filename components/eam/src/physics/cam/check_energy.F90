...
! Public methods
  ...
  public :: energy_helper_eam_def1d, energy_helper_eam_def
...
!-----------------------------------------------------------------------
    ...
    do k = 1, pver
       do i = 1, ncol
          ...
          se(i) = se(i) +         state%t(i,k)*cpairv_loc(i,k,lchnk)*state%pdel(i,k)/gravit
          wv(i) = wv(i) + state%q(i,k,1       )*state%pdel(i,k)/gravit
!if ((lchnk == 6092).and.(i == 13).and.(name == "clubb_tend")) then
!print *, 'in Check, begin, k,wv',k,wv(i)
!endif
...
#if 1
!chunk 5659, col1   rliq 
do i = 1, ncol
!if (i == icol) then
if ((lchnk == 6092).and.(i == 13).and.(name == "clubb_tend")) then
print *, 'lat lon ------------------------------CHECK', state%lat(i), state%lon(i)
print *, 'nstep',nstep,'te', te(i), 'te_tnd*dt', te_tnd(i)*ztodt
print *, 'te_xpd',te_xpd(i),'te_diff',te_dif(i)
print *, 'explicit diff',te_xpd(i) - te(i)
print *, 'te_rer',te_rer(i), 'te+te_tnd*dt', te(i)+te_tnd(i)*ztodt
print *, 'F parts1', flx_vap(i)*(latvap+latice)*ztodt
print *, 'F parts2', - (flx_cnd(i) - flx_ice(i))*1000._r8*latice*ztodt
print *, 'F parts3', flx_sen(i)*ztodt

print *, 'in check ke',ke(i)
print *,'in check se',se(i)
print *, 'in check wv*(L+L)',(latvap+latice)*wv(i) 
print *, 'in check, (wl+wr)*L',latice*( wl(i) + wr(i) )

!do k=1,72
!!print *,'check pdel',k, state%pdel(i,k)
!print *,'checkq',k,state%q(i,k,1)
!enddo
       ...
       if (any(abs(te_rer(1:ncol)) > 1.E-14_r8 )) then
          ...
          do i = 1,ncol
             if (abs(te_rer(i)) > 1.E-14_r8 ) then 
                ...
                write(iulog,"(i5,2f10.6,6e15.7)") i, state%lat(i), state%lon(i), te(i),te_xpd(i),te_dif(i),  &
                      te_tnd(i)*ztodt,te_rer(i), tend%te_tnd(i)*ztodt

if ((lchnk == 6092).and.(i == 13).and.(name == "clubb_tend")) then              
print *, 'INSIDE aaa', i, lchnk,te_rer(i)
stop
endif
...
!===============================================================================
  subroutine energy_helper_eam_def1d(u,v,T,q,ps,pdel,phis, &
...
!#endif
    ...
    if (irain   > 1  .and.  isnow   > 1 ) then
       ...
       tw = wv + wl + wi + wr + ws
  end subroutine energy_helper_eam_def1d
