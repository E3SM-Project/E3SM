...
module zm_conv
   ...
   ! May need to change to use local variable as passed via dummy argument. 
   ! For now, making it threadprivate as follows,
...
subroutine zmconv_readnl(nlfile)
   ...
   if (masterproc) then
      ...
      if (trigdcape_ull)   write(iulog,*)'**** ZM uses DCAPE trigger and unrestricted launch level:', trigdcape_ull
      if (trig_dcape_only) write(iulog,*)'**** ZM uses only DCAPE trigger:', trig_dcape_only
      if (trig_ull_only)   write(iulog,*)'**** ZM uses only unrestricted launch level:', trig_ull_only
...
subroutine zm_convr(lchnk   ,ncol    , &
   ...
   ! (performs deep convective adjustment using a mass-flux closure)
   ...
   !  wg * mu       upward   cloud mass flux (positive up). specified at interface
   ...
   real(r8), intent(in), pointer, dimension(:,:) :: t_star ! intermediate T between time n and n-1 for DCAPE
   real(r8), intent(in), pointer, dimension(:,:) :: q_star ! intermediate q between time n and n-1 for DCAPE
   !----------------------------------------------------------------------------
   ! output arguments
   ...
   real(r8), intent(out) :: dlf(pcols,pver)    ! scattered version of the detraining cld h2o tend
   ...
   real(r8), intent(out) :: prec(pcols)        ! precipitation
   ...
   real(r8) mumax(pcols)               ! upper bound for cloud base mass flux
   ...
   real(r8) ql(pcols,pver)             ! w grid slice of cloud liquid water.
   ...
   real(r8) precip
   !----------------------------------------------------------------------------
   ! gathered work variables
   real(r8) qg(pcols,pver)             ! wg grid slice of gathered values of q.
   real(r8) tg(pcols,pver)             ! wg grid slice of temperature at interface.
   ...
   ! work variables arising from gathered calculations
   ...
   real(r8) qlg(pcols,pver)            ! wg liq water of updraft
   ...
   ! calculate local pressure (mbs) and height (m) 
   ! for both interface and mid-layer locations.
   do i = 1,ncol
      zs(i) = geos(i)*rgrav
   ...
   ! define array of factors which defines interfacial
...
subroutine zm_conv_evap(ncol,lchnk, t,pmid,pdel,q, &
   ...
   real(r8) :: work1                     ! temp variable
   real(r8) :: work2                     ! temp variable
   ...
   do k = 1, pver
      do i = 1, ncol
         ...
         ! Layer may already be saturated.
...
subroutine convtran(lchnk   , &
   ...
   !----------------------------------------------------------------------------
   ! Local Variables
   integer i,m                         ! Work indices
   integer k,km1,kp1                   ! Work indices
   integer kk,kkp1                     ! Work indices
   ...
   real(r8) minc,maxc                  ! Work variables
   real(r8) fluxin,fluxout             ! Work variables
...
subroutine momtran(lchnk, ncol, &
   ...
   !----------------------------------------------------------------------------
   ! Local variables
   integer i,m,ii                      ! Work indices
   integer kk,kkp1,kkm1                ! Work indices
   integer k,kp1,km1                   ! Work indices
   integer kbm                         ! Highest altitude index of cloud base
   integer ktm                         ! Highest altitude index of cloud top
   real(r8) cabv                       ! Mix ratio of constituent above
   real(r8) cbel                       ! Mix ratio of constituent below
   ...
   !----------------------------------------------------------------------------
   ! Loop ever each wind component - start at m=1 to transport momentum
   do m = 1, ncnst
      if (domomtran(m)) then
...
subroutine cldprp(lchnk   , &
                  ...
                  landfrac,  tpertg )
   !----------------------------------------------------------------------------
   ! Author: may 09/91 guang jun zhang, m.lazare, n.mcfarlane, modified by P. Rasch
   !----------------------------------------------------------------------------
   implicit none
   !----------------------------------------------------------------------------
   ! Interface arguments
   ...
   integer, intent(in ) :: il2g                 ! length of gathered array
...
subroutine buoyan_dilute(lchnk   ,ncol    , &
                  ...
                  tpert   ,iclosure)
   !---------------------------------------------------------------------------- 
   ! Purpose: Calculates CAPE the lifting condensation level and the convective top
   ...
   ! 
   ! References: Raymond and Blythe (1992) JAS 
   ! Author: Richard Neale - September 2004
   !----------------------------------------------------------------------------
   implicit none
   !----------------------------------------------------------------------------
   ! Interface arguments
   ...
   !----------------------------------------------------------------------------
   ! main buoyancy calculation using dilute entraining plume
...
subroutine parcel_dilute (lchnk, ncol, msg, klaunch, p, t, q, tpert, tp, tpv, qstp, pl, tl, lcl)
   ...
   integer, intent(in   ) :: msg
   integer, intent(in   ), dimension(pcols)      :: klaunch
