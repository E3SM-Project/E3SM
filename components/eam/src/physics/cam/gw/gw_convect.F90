...
end subroutine gw_heating_depth
...
subroutine gw_storm_speed(ncol, storm_speed_min, ubm, mini, maxi, &
  ...
  ! Bottom/top heating range index.
  integer, intent(in)  :: mini(ncol), maxi(ncol)

  ! Speed of convective cells relative to storm.
  integer, intent(out) :: storm_speed(ncol)

  ! Mean wind in heating region.
  real(r8), intent(out) :: uh(ncol)

  ! Min/max projected wind value in each column.
  real(r8), intent(out) :: Umin(ncol), Umax(ncol)
...
subroutine gw_convect_gw_sources(ncol, ngwv, lat, hdepth_min, hdepth, mini, maxi, netdt, uh, storm_speed, &
  ...
  ! Mean wind in heating region.
  real(r8), intent(in) :: uh(ncol)

  ! Speed of convective cells relative to storm.
  integer, intent(in) :: storm_speed(ncol)

  ! Maximum heating rate.
  real(r8), intent(in) :: maxq0(ncol)

  ! Min/max projected wind value in each column.
  real(r8), intent(in) :: Umin(ncol), Umax(ncol)
