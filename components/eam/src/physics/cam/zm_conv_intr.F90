...
subroutine zm_conv_tend(pblh    ,mcon    ,cme     , &
   ...
   real(r8),intent(out):: dsubcld(pcols)     ! wg layer thickness in mbs (between upper/lower interface).
   integer, intent(out):: jt(pcols)          ! wg layer thickness in mbs between lcl and maxi.    
   integer, intent(out):: maxg(pcols)        ! wg top  level index of deep cumulus convection.
   integer, intent(out):: ideep(pcols)       ! wg gathered values of maxi.
   ...
   ! dpdry is not used in this call to convtran 
   ! since the cloud liquid and ice mixing ratios are moist
   fake_dpdry(:,:) = 0._r8
...
subroutine zm_conv_tend_2( state,  ptend,  ztodt, pbuf,mu, eu, &
   ...
   real(r8),intent(in):: dsubcld(pcols)   ! wg layer thickness in mbs (between upper/lower interface).
   integer, intent(in):: jt(pcols)        ! wg layer thickness in mbs between lcl and maxi.    
   integer, intent(in):: maxg(pcols)      ! wg top  level index of deep cumulus convection.
   integer, intent(in):: ideep(pcols)     ! wg gathered values of maxi.
   integer, intent(in):: lengath          ! w holds position of gathered points
