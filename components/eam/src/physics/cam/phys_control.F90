...
logical           :: state_debug_checks   = .false.    ! Extra checks for validity of physics_state objects
                                                       ! in physics_update.
!character(len=128):: p3_lookup_dir        = unset_str  ! location of p3 input files ! ASD-DELETE
...
subroutine phys_ctl_readnl(nlfile)
   ...
   namelist /phys_ctl_nl/ cam_physpkg, cam_chempkg, waccmx_opt, deep_scheme, shallow_scheme, &
      ...
      rrtmg_temp_fix !, p3_lookup_dir ASD-DELETE
   !-----------------------------------------------------------------------------
...
!   call mpibcast(p3_lookup_dir,  len(p3_lookup_dir) , mpichar, 0, mpicom) !  ASD-DELETE
#endif
...
!   character(len=128),intent(out), optional :: p3_lookup_dir_out ! ASD-DELETE
...
!   if ( present(p3_lookup_dir_out       ) ) p3_lookup_dir_out        = p3_lookup_dir ! ASD-DELETE
