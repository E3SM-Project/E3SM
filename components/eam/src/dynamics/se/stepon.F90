...
! from SE
   ...
   use scamMod,        only: use_iop, doiopupdate, single_column, &
                             setiopupdate, readiopdata, uniform_grid_mode
...
!-----------------------------------------------------------------------
  ...
  if (single_column) then
    ...
    call scm_broadcast()
    if (.not. uniform_grid_mode) call scm_setfield(elem,iop_update_phase1)
  endif
...
subroutine stepon_run2(phys_state, phys_tend, dyn_in, dyn_out )
   ...
   if (.not. single_column .or. uniform_grid_mode) then
      do ie=1,nelemd
