...
| Equation term  | ELM Variable  | Units| Description |
| ----------- | ----------- | ----------- | ----------- |
| $A^{sun}$   |  `veg_cf%psnsun_to_cpool` | $gC~m^{2}~sec^{-1}$ |  C fixation from sunlit canopy |
| $A^{shade}$   |  `veg_cf%psnshade_to_cpool` | $gC~m^{2}~sec^{-1}$ | C fixation from shaded canopy |
| $F^C_{GPP}$   |  `veg_cf%gpp_before_downreg` | $gC~m^{2}~sec^{-1}$ | GPP flux before downregulation |  
| $F^C_{MR}$   |  mr | $gC~m^{2}~sec^{-1}$ | maintenance respiration |
| $F^C_{avail}$   |  `veg_cf%availc` | $gC~m^{2}~sec^{-1}$ | C flux available for allocation |
| $C_{allom}$ | `cnstate_vars%c_allometry_patch` | -  | C allocation index |
| $N_{allom}$ | `cnstate_vars%n_allometry_patch` | -  | N allocation index |
| $P_{allom}$ | `cnstate_vars%p_allometry_patch` | -  | P allocation index |
| $F^N_{plant\_demand}$ | `veg_nf%plant_ndemand` | $gN~m^{2}~sec^{-1}$ | N flux required to support initial GPP |
| $F^P_{plant\_demand}$ | `veg_pf%plant_pdemand` | $gP~m^{2}~sec^{-1}$ | P flux required to support initial GPP |
| $CN_{leaf}$ | `veg_vp%leafcn` | $gC~gN^{-1}$ | leaf C:N |
| $CN_{fr}$ | `veg_vp%frootcn` | $gC~gN^{-1}$ | fine root C:N |
| $CN_{lw}$ | `veg_vp%livewdcn` | $gC~gN^{-1}$ | live wood C:N |
| $CN_{dw}$ | `veg_vp%deadwdcn` | $gC~gN^{-1}$ | dead wood C:N |
| $CN_{g}$ | `veg_vp%graincn` | $gC~gN^{-1}$ | grain C:N |
| $CP_{leaf}$ | `veg_vp%leafcp` | $gC~gP^{-1}$ | leaf C:P |
| $CP_{fr}$ | `veg_vp%frootcp` | $gC~gP^{-1}$ | fine root C:P |
| $CP_{lw}$ | `veg_vp%livewdcp` | $gC~gP^{-1}$ | live wood C:P |
| $CP_{dw}$ | `veg_vp%deadwdcp` | $gC~gP^{-1}$ | dead wood C:P |
| $CP_{g}$ | `veg_vp%graincp` | $gC~gP^{-1}$ | grain C:P |
| $N_{retrans}$ | `veg_nf%retransn_to_npool` | $gN~m^{2}~sec^{-1}$ | deployment of retranslocated N |
| $P_{retrans}$ | `veg_pf%retransp_to_ppool` | $gP~m^{2}~sec^{-1}$ | deployment of retranslocated P |
| $F^C_{alloc}$ | `veg_cf%plant_calloc` | $gC~m^{2}~sec^{-1}$ | total allocated C flux |
| $F^N_{alloc}$ | `veg_nf%plant_nalloc` | $gN~m^{2}~sec^{-1}$ | total allocated N flux |
| $F^P_{alloc}$ | `veg_pf%plant_palloc` | $gP~m^{2}~sec^{-1}$ | total allocated P flux |
| $f_{cur}$ | `veg_vp%fcur` | - | fraction of allocation that goes to currently displayed growth, remainder to storage |
| $F^C_{alloc,leaf}$ | `veg_cf%cpool_to_leafc` | $gC~m^{2}~sec^{-1}$ | allocation to leaf C |
| $F^C_{alloc,leaf\_ stor}$ | `veg_cf%cpool_to_leafc_storage` | $gC~m^{2}~sec^{-1}$ | allocation to leaf C storage |
| $F^C_{alloc,froot}$ | `veg_cf%cpool_to_frootc` | $gC~m^{2}~sec^{-1}$ | allocation to fine root C |
| $F^C_{alloc,froot\_ stor}$ | `veg_cf%cpool_to_frootc_storage` | $gC~m^{2}~sec^{-1}$ | allocation to fine root C storage |
| $F^C_{alloc,livestem}$ | `veg_cf%cpool_to_livestemc` | $gC~m^{2}~sec^{-1}$ | allocation to live stem C |
| $F^C_{alloc,livestem\_ stor}$ | `veg_cf%cpool_to_livestemc_storage` | $gC~m^{2}~sec^{-1}$ | allocation to live stem C storage |
| $F^C_{alloc,deadstem}$ | `veg_cf%cpool_to_deadstemc` | $gC~m^{2}~sec^{-1}$ | allocation to dead stem C |
| $F^C_{alloc,deadstem\_ stor}$ | `veg_cf%cpool_to_deadstemc_storage` | $gC~m^{2}~sec^{-1}$ | allocation to dead stem C storage |
| $F^C_{alloc,livecroot}$ | `veg_cf%cpool_to_livecrootc` | $gC~m^{2}~sec^{-1}$ | allocation to live coarse root C |
| $F^C_{alloc,livecroot\_ stor}$ | `veg_cf%cpool_to_livecrootc_storage` | $gC~m^{2}~sec^{-1}$ | allocation to live coarse root C storage |
| $F^C_{alloc,deadcroot}$ | `veg_cf%cpool_to_deadcrootc` | $gC~m^{2}~sec^{-1}$ | allocation to dead coarse root C |
| $F^C_{alloc,deadcroot\_ stor}$ | `veg_cf%cpool_to_deadcrootc_storage` | $gC~m^{2}~sec^{-1}$ | allocation to dead coarse root C storage |
| $F^C_{alloc,grain}$ | `veg_cf%cpool_to_grainc` | $gC~m^{2}~sec^{-1}$ | allocation to grain C for prognostic crop |
| $F^C_{alloc,grain\_ stor}$ | `veg_cf%cpool_to_grainc_storage` | $gC~m^{2}~sec^{-1}$ | allocation to grain C storage for prognostic crop |
...
$$
\begin{equation}
if \left( F^C_{storage} < 0 \right) then
    \begin{cases}
          F^C_{storage\_recov} = -\frac{F^C_{storage}}{86400\tau _{xs}}
    \end{cases}
\end{equation}
$$
...
$$
\begin{equation}
if \left(F^C_{storage\_recov} < F^C_{avail} \right) then
    \begin{cases}
       F^C_{avail} = F^C_{avail} - F^C_{storage\_recov}
    \end{cases}
\end{equation}
$$
...
if \left(F^C_{storage\_recov} > F^C_{avail} \right) then
    \begin{cases}
       F^C_{storage\_recov} = F^C_{avail} \\
       F^C_{avail} = 0 \\
    \end{cases}
\end{equation}
$$
