module Method_procs_acc
  ...
  use elm_varctl      , only : pf_hmode, nu_com, use_erosion
  ...
  contains
    ...
    subroutine colcf_setvalues_acc (colcf, num_column, filter_column, value_column)
      ...
      do k = 1, ndecomp_pools
         do j = 1, nlevdecomp_full
            do fi = 1,num_column
               ...
               colcf%decomp_cpools_yield_vr(i,j,k) = value_column
            end do
         end do
      end do
      ...
      do k = 1, ndecomp_pools
         do fi = 1,num_column
            ...
            colcf%decomp_cpools_erode(i,k) = value_column
            colcf%decomp_cpools_deposit(i,k) = value_column
            colcf%decomp_cpools_yield(i,k) = value_column
      ...
      do fi = 1,num_column
         ...
         colcf%cinputs(i)                   = value_column
         colcf%coutputs(i)                  = value_column
         ...
         colcf%somc_erode(i)                = value_column
         colcf%somc_deposit(i)              = value_column
         colcf%somc_yield(i)                = value_column
    ...
    subroutine colnf_setvalues_acc( colnf, num_column, filter_column, value_column)
      ...
      do fi = 1,num_column
         ...
         colnf%somn_erode(i)                = value_column
         colnf%somn_deposit(i)              = value_column
         colnf%somn_yield(i)                = value_column
         ! Zero p2c column fluxes
         colnf%fire_nloss(i) = value_column
      ...
      do k = 1, ndecomp_pools
         do fi = 1,num_column
            ...
            colnf%decomp_npools_erode(i,k) = value_column
            colnf%decomp_npools_deposit(i,k) = value_column
            colnf%decomp_npools_yield(i,k) = value_column
      ...
      do k = 1, ndecomp_pools
         do j = 1, nlevdecomp_full
            do fi = 1,num_column
               ...
               colnf%decomp_npools_yield_vr(i,j,k) = value_column
            end do
         end do
      end do
    ...
    subroutine colnf_summary_acc(colnf, bounds, num_soilc, filter_soilc, dtime)
      ...
      do fc = 1,num_soilc
         ...
         colnf%somn_erode(c)          = 0._r8
         colnf%somn_deposit(c)        = 0._r8
         colnf%somn_yield(c)          = 0._r8
      end do
      ...
      if ( use_erosion ) then
         do l = 1, ndecomp_pools
            do j = 1, nlev
               do fc = 1, num_soilc
                  c = filter_soilc(fc)
                  colnf%decomp_npools_yield(c,l) = &
                       colnf%decomp_npools_yield(c,l) + &
                       colnf%decomp_npools_yield_vr(c,j,l) * dzsoi_decomp(j)
               end do
            end do
         end do
      end if
      ...
      if ( use_erosion ) then
         do k = 1, ndecomp_pools
            if ( decomp_cascade_con%is_soil(k) ) then
               do fc = 1, num_soilc
                  c = filter_soilc(fc)
                  colnf%somn_erode(c) = colnf%somn_erode(c) + &
                       colnf%decomp_npools_erode(c,k)
                  colnf%somn_deposit(c) = colnf%somn_deposit(c) + &
                       colnf%decomp_npools_deposit(c,k)
                  colnf%somn_yield(c) = colnf%somn_yield(c) + &
                       colnf%decomp_npools_yield(c,k)
               end do
            end if
         end do
      end if
    ...
    subroutine colpf_setvalues_acc( colpf, num_column, filter_column, value_column)
      ...
      do j = 1, nlevdecomp_full
         do fi = 1,num_column
            ...
            colpf%labilep_yield_vr(i,j)                 = value_column
            colpf%secondp_yield_vr(i,j)                 = value_column
            colpf%occlp_yield_vr(i,j)                   = value_column
            colpf%primp_yield_vr(i,j)                   = value_column
      ...
      do fi = 1,num_column
         ...
         colpf%somp_erode(i)                = value_column
         colpf%somp_deposit(i)              = value_column
         colpf%somp_yield(i)                = value_column
         colpf%labilep_erode(i)             = value_column
         colpf%labilep_deposit(i)           = value_column
         colpf%labilep_yield(i)             = value_column
         colpf%secondp_erode(i)             = value_column
         colpf%secondp_deposit(i)           = value_column
         colpf%secondp_yield(i)             = value_column
         colpf%occlp_erode(i)               = value_column
         colpf%occlp_deposit(i)             = value_column
         colpf%occlp_yield(i)               = value_column
         colpf%primp_erode(i)               = value_column
         colpf%primp_deposit(i)             = value_column
         colpf%primp_yield(i)               = value_column
      ...
      do k = 1, ndecomp_pools
         do fi = 1,num_column
            ...
            colpf%decomp_ppools_erode(i,k) = value_column
            colpf%decomp_ppools_deposit(i,k) = value_column
            colpf%decomp_ppools_yield(i,k) = value_column
      ...
      do k = 1, ndecomp_pools
         do j = 1, nlevdecomp_full
            do fi = 1,num_column
               ...
               colpf%decomp_ppools_yield_vr(i,j,k) = value_column
            end do
         end do
      end do
    ...
    subroutine colcf_summary_acc(colcf, bounds, num_soilc, filter_soilc, isotope, dtime)
      ...
      do fc = 1,num_soilc
         ...
         colcf%somc_erode(c)         = 0._r8
         colcf%somc_deposit(c)       = 0._r8
         colcf%somc_yield(c)         = 0._r8
      end do
      ...
      if (use_erosion) then
         do l = 1, ndecomp_pools
            do j = 1, nlev
               do fc = 1, num_soilc
                  c = filter_soilc(fc)
                  colcf%decomp_cpools_yield(c,l) = &
                       colcf%decomp_cpools_yield(c,l) + &
                       colcf%decomp_cpools_yield_vr(c,j,l)*dzsoi_decomp(j)
               end do
            end do
         end do
      end if
      ...
      if ( use_erosion ) then
         do l = 1, ndecomp_pools
            if ( is_soil(l) ) then
               do fc = 1, num_soilc
                  c = filter_soilc(fc)
                  colcf%somc_erode(c) = colcf%somc_erode(c) + &
                       colcf%decomp_cpools_erode(c,l)
                  colcf%somc_deposit(c) = colcf%somc_deposit(c) + &
                       colcf%decomp_cpools_deposit(c,l)
                  colcf%somc_yield(c) = colcf%somc_yield(c) + &
                       colcf%decomp_cpools_yield(c,l)
               end do
            end if
         end do
    ...
    subroutine colpf_summary_acc(colpf, bounds, num_soilc, filter_soilc, dtime)
      ...
      do fc = 1,num_soilc
         ...
         colpf%somp_erode(c)          = 0._r8
         colpf%somp_deposit(c)        = 0._r8
         colpf%somp_yield(c)          = 0._r8
         colpf%labilep_yield(c)       = 0._r8
         colpf%secondp_yield(c)       = 0._r8
         colpf%occlp_yield(c)         = 0._r8
         colpf%primp_yield(c)         = 0._r8
      end do
      ...
      if (use_erosion) then
         do j = 1, nlevdecomp
            do fc = 1,num_soilc
               c = filter_soilc(fc)
               colpf%labilep_yield(c) = &
                     colpf%labilep_yield(c) + &
                     colpf%labilep_yield_vr(c,j) * dzsoi_decomp(j)
               colpf%secondp_yield(c) = &
                     colpf%secondp_yield(c) + &
                     colpf%secondp_yield_vr(c,j) * dzsoi_decomp(j)
               colpf%occlp_yield(c) = &
                     colpf%occlp_yield(c) + &
                     colpf%occlp_yield_vr(c,j) * dzsoi_decomp(j)
               colpf%primp_yield(c) = &
                     colpf%primp_yield(c) + &
                     colpf%primp_yield_vr(c,j) * dzsoi_decomp(j)
            end do
         end do
      end if
      ...
      if (use_erosion) then
         do l = 1, ndecomp_pools
            do j = 1, nlevdecomp
               do fc = 1, num_soilc
                  c = filter_soilc(fc)
                  colpf%decomp_ppools_yield(c,l) = &
                       colpf%decomp_ppools_yield(c,l) + &
                       colpf%decomp_ppools_yield_vr(c,j,l) * dzsoi_decomp(j)
               end do
            end do
         end do
      end if
      ...
      if ( use_erosion ) then
         do k = 1, ndecomp_pools
            if ( decomp_cascade_con%is_soil(k) ) then
               do fc = 1, num_soilc
                  c = filter_soilc(fc)
                  colpf%somp_erode(c) = colpf%somp_erode(c) + &
                       colpf%decomp_ppools_erode(c,k)
                  colpf%somp_deposit(c) = colpf%somp_deposit(c) + &
                       colpf%decomp_ppools_deposit(c,k)
                  colpf%somp_yield(c) = colpf%somp_yield(c) + &
                       colpf%decomp_ppools_yield(c,k)
               end do
            end if
         end do
      end if
         ...
         do p = bounds%begp, bounds%endp
          ...
          if (use_c13) then
            ...
            do c = bounds%begc,bounds%endc
               ...
               c13_col_cf%dwt_crop_productc_gain(c)   = 0._r8
          ...
          if (use_c14) then
            ...
            do c = bounds%begc,bounds%endc
               ...
               c14_col_cf%dwt_crop_productc_gain(c)   = 0._r8
