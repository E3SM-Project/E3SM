  ...
  !-----------------------------------Meteorological forcing  -----------------------------------
        ...
        if (atm2lnd_vars%loaded_bypassdata == 0) then
          ...
          if (atm2lnd_vars%metsource == 0) then 
            ...
            ierr = nf90mpi_open(MPI_COMM_SELF, trim(metdata_bypass) // '/all_hourly.nc', nf90_nowrite, MPI_INFO_NULL, ncid)
...
10          continue
          ...
          do v=1,met_nvars
            ...
            ierr = nf90mpi_open(MPI_COMM_SELF, trim(metdata_bypass) // '/' // trim(metdata_fname), NF90_NOWRITE, MPI_INFO_NULL, met_ncids(v))
            ...
            ierr = nf90mpi_begin_indep_data(met_ncids(v))
            ierr = nf90mpi_get_var(met_ncids(v), varid, timetemp, starti(1:1), counti(1:1))
            ierr = nf90mpi_end_indep_data(met_ncids(v))
        ...
        model_filter: if (use_cn .or. use_fates) then 
          if (atm2lnd_vars%loaded_bypassdata == 0 .or. (mon .eq. 1 .and. day .eq. 1 .and. tod .eq. 0)) then  
            if (masterproc .and. i .eq. 1) then 
              ...
              ierr = nf90mpi_open(MPI_COMM_SELF, trim(stream_fldFileName_popdens), NF90_NOWRITE, MPI_INFO_NULL, ncid)
          ...
          if (atm2lnd_vars%loaded_bypassdata .eq. 0 .and. masterproc .and. i .eq. 1) then 
            ...
            ierr = nf90mpi_open(MPI_COMM_SELF, trim(stream_fldFileName_lightng), NF90_NOWRITE, MPI_INFO_NULL, ncid)
            ierr = nf90mpi_inq_varid(ncid, 'lat', varid)
            ierr = nf90mpi_begin_indep_data(ncid)
   ...
   !------------------------------------Nitrogen deposition----------------------------------------------
          ...
          if (atm2lnd_vars%loaded_bypassdata .eq. 0 .or. (mon .eq. 1 .and. day .eq. 1 .and. tod .eq. 0)) then 
            if (masterproc .and. i .eq. 1) then 
              ...
              ierr = nf90mpi_open(MPI_COMM_SELF, trim(stream_fldFileName_ndep), nf90_nowrite, MPI_INFO_NULL, ncid)
              ierr = nf90mpi_inq_varid(ncid, 'lat', varid)
              ierr = nf90mpi_begin_indep_data(ncid)
   ...
   !------------------------------------Aerosol forcing--------------------------------------------------
        if (atm2lnd_vars%loaded_bypassdata .eq. 0 .or. (mon .eq. 1 .and. day .eq. 1 .and. tod .eq. 0)) then 
          if (masterproc .and. i .eq. 1) then 
            ...
            ierr = nf90mpi_open(MPI_COMM_SELF, trim(aero_file), nf90_nowrite, MPI_INFO_NULL, ncid)
            ierr = nf90mpi_inq_varid(ncid, 'lat', varid)
            ierr = nf90mpi_begin_indep_data(ncid)
            ierr = nf90mpi_get_var(ncid, varid, smap2_lat)
            ierr = nf90mpi_end_indep_data(ncid)
  ...
  !-----------------------------------------------------------------------------------------------------
       ...
       if (co2_type_idx == 1) then
        ...
        if (atm2lnd_vars%loaded_bypassdata .eq. 0) then 
          ierr = nf90mpi_open(MPI_COMM_SELF, trim(co2_file), nf90_nowrite, MPI_INFO_NULL, ncid)
