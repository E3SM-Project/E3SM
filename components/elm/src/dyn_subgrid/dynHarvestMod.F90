...
contains
  ...
  subroutine dynHarvest_init(bounds, harvest_filename)
    ...
    allocate(harvest_rates(num_harvest_vars,1:max_topounits,bounds%begg:bounds%endg),stat=ier)  ! TKT added max_topounits
...
!-----------------------------------------------------------------------
  subroutine dynHarvest_interp_harvest_types(bounds)
    ...
    if (use_cn .or. use_fates) then
       harvest_rates(1:num_harvest_vars,1:max_topounits,bounds%begg:bounds%endg) = 0._r8
       ...
       if (dynHarvest_file%time_info%is_before_time_series()) then
          ...
          allocate(this_data(1:max_topounits,bounds%begg:bounds%endg))   ! TKT introduced max_topounits
          do varnum = 1, num_harvest_vars
   ...
   do fp = 1,num_soilp
      ...
      if (woody(ivt(p)) == 1.0_r8) then
         ...
         if (do_cn_harvest) then
            am = 0._r8
            do varnum = 1, num_harvest_vars
               am = am + harvest_rates(varnum,ti,g)  ! TKT added ti
            end do
