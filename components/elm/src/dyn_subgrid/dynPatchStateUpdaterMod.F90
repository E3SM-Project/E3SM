...
contains
  ...
  subroutine set_old_patch_weights(this, bounds)
    ...
    do p = bounds%begp, bounds%endp
       c = veg_pp%column(p)
       this%pwtgcell_old(p) = veg_pp%wtgcell(p) !veg_pp%wttopounit(p) ! TKT from veg_pp%wtgcell(p) to veg_pp%wttopounit(p)
       this%cwtgcell_old(c) = col_pp%wtgcell(c) !col_pp%wttopounit(c) ! TKT from col_pp%wtgcell(c) to col_pp%wttopounit(c)
  ...
  subroutine set_new_patch_weights(this, bounds)
    ...
    do p = bounds%begp, bounds%endp
       this%pwtgcell_new(p) = veg_pp%wtgcell(p) !veg_pp%wttopounit(p) ! TKT from veg_pp%wtgcell(p) to veg_pp%wttopounit(p)
