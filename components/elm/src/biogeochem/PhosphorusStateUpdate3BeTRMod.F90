module PhosphorusStateUpdate3BeTRMod
  ...
  public:: PhosphorusStateUpdate3Soil
  public:: PhosphorusStateUpdate3Veg
  !-----------------------------------------------------------------------
     ...
     ldecomp_on=.true.; if(present(is_decomp_on))ldecomp_on=is_decomp_on
      ...
      if(ldecomp_on)then
        ...
        do j = 1, nlevdecomp
          ...
          do fc = 1,num_soilc
            c = filter_soilc(fc)
            flux_mineralization(c,j) = col_pf%net_mineralization_p_vr(c,j)*dt
          enddo
        enddo
      endif
...
!          print*,'temp_solutionp',temp_solutionp(c,j)
          if (temp_solutionp(c,j) < 0.0_r8) then
            col_pf%labilep_to_secondp_vr(c,j) =col_pf%labilep_to_secondp_vr(c,j)/ &
                   (col_pf%labilep_to_secondp_vr(c,j)+col_pf%sminp_leached_vr(c,j)+tiny_val)* &
  ...
  end subroutine PhosphorusStateUpdate3Soil
  !-----------------------------------------------------------------------
  subroutine PhosphorusStateUpdate3Veg(bounds,num_soilc, filter_soilc, num_soilp, filter_soilp, &
  ...
  end subroutine PhosphorusStateUpdate3Veg
