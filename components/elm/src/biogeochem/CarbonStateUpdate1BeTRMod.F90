module CarbonStateUpdate1BeTRMod
  ...
  public :: CarbonStateUpdate1Soil
  public :: CarbonStateUpdate1Veg
  !-----------------------------------------------------------------------
...
contains
  ...
  !-----------------------------------------------------------------------
  subroutine CarbonStateUpdate1Soil(bounds, &
       num_soilc, filter_soilc, &
       num_soilp, filter_soilp, &
       crop_vars, col_cs, col_cf, dt, is_decomp_on)
  ...
  subroutine CarbonStateUpdate1(bounds, &
       num_soilc, filter_soilc, &
       num_soilp, filter_soilp, &
       crop_vars, col_cs, veg_cs, col_cf, veg_cf, dt, ldecomp_on)
    !
    ! !DESCRIPTION:
    ! On the radiation time step, update all the prognostic carbon state
    ! variables (except for gap-phase mortality and fire fluxes)
  ...
  call CarbonStateUpdate1Soil(bounds, num_soilc, filter_soilc, num_soilp, filter_soilp, &
            crop_vars, col_cs, col_cf, dt, is_decomp_on=is_decomp_on)
