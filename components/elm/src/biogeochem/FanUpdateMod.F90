  ...
  subroutine handle_storage(bounds, frictionvel_vars, dt,  &
    ...
    do g = begg, endg
       ...
       if (l /= ispval) then
          ...
          do c = lun_pp%coli(l), lun_pp%colf(l)
             if (.not. col_pp%active(c) .or. col_pp%wtgcell(c) < 1.e-6_r8) cycle

             if (lun_pp%wtgcell(l) < 1.e-8_r8) cycle ! Ignore if landunit is very tiny
       ...
       if (col_grass /= ispval .and. col_pp%wtgcell(col_grass) > 1.e-6_r8) then
