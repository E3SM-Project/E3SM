...
!
  public mksoiltexInit  ! Initialization
  public mksoiltex      ! Set soil texture
  public mkorganic     ! Set organic soil
  public mksoilcolInit  ! Initialization
...
!EOP
  ...
  integer  :: kmax_max = 50                 ! this can be changed
...
!-----------------------------------------------------------------------
  ...
  allocate(kmap(0:kmax_max,ns_o), &
           kwgt(0:kmax_max,ns_o),&
           kmax(ns_o),&
           wst(0:kmax_max), stat=ier)
  if (ier/=0) call abort()
  ...
  do n = 1,tgridmap%ns
     ...
     if (.not. found) then
        kmax(no) = kmax(no) + 1
        if (kmax(no) > kmax_max) then
           write(6,*)'kmax is > kmax_max= ',kmax_max,' for no = ',no
           write(6,*)'reset kmax_max in mksoilMod to a greater value'
           stop
        end if
        kmap(kmax(no),no) = k
        kwgt(kmax(no),no) = wt
