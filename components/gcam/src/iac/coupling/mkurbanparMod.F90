...
!-----------------------------------------------------------------------
  ...
  if ( all_urban .or. zero_out )then
     ...
     do no = 1, ldomain%ns
        if (urbn_o(no) < 0.1_r8) urbn_o(no) = 0._r8
     enddo
...
!EOP
  type(domain1_type)    :: tdomain              ! local domain 
  type(domain1_type)    :: tdomain_mask         ! local domain that contains "mask"
  type(gridmap_type)    :: tgridmap             ! local gridmap
...
!-----------------------------------------------------------------------
  ...
  call domain1_read(tdomain     , locfn)
  call domain1_read(tdomain_mask, locfn, readmask=.true.)
  ...
  call domain1_clean(tdomain)
  call domain1_clean(tdomain_mask)
  call gridmap_clean(tgridmap)
