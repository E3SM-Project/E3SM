...
!      seq_timemgr_alarm_barrier
   ...
   integer(SHR_KIND_IN),private,parameter :: &
      ...
      seq_timemgr_nalarm_iacrun     =18 , & ! driver only clock alarm
...
!-------------------------------------------------------------------------------
    ...
    if (iam == 0) then
       ...
       if (iac_cpl_dt == 0) iac_cpl_dt = iac_cpl_dt ! Copy atm coupling time into esp
       ...
       if ( atm_cpl_dt <= 0 .or. &
          ...
          write(logunit,*) trim(subname),' ERROR: aliogrwe _cpl_dt = ', &
             ...
             rof_cpl_dt, wav_cpl_dt, iac_cpl_dt, esp_cpl_dt
       ...
       if ( abs(atm_cpl_offset) > atm_cpl_dt .or. &
          ...
          write(logunit,*) trim(subname),' ERROR: aliogrwe _cpl_offset = ', &
             ...
             glc_cpl_offset, rof_cpl_offset, wav_cpl_offset, iac_cpl_offset, &
             esp_cpl_offset
...
!-------------------------------------------------------------------------------
   ...
   if (ESMF_AlarmIsRinging(SyncClock%EAlarm(seq_timemgr_nclock_drv,seq_timemgr_nalarm_iacrun))) then
      call ESMF_ClockAdvance(SyncClock%ECP(seq_timemgr_nclock_iac)%EClock, rc=rc )
      call seq_timemgr_ESMFCodeCheck(rc, msg=subname//"Error from iac ESMF_ClockAdvance")
   endif
