...
#define ML_V1    // MultiLevelPreconditioner Version
//#define ML_V2  // Advanced Version
...
// ==============================================================================
// Constructor for Block Preconditioner ML
// ==============================================================================
BlockPreconditioner_SparseML::BlockPreconditioner_SparseML
  ...
  firstfill(true)
{
  ...
  if (printproc) 
    cout << " >>> Constructing BlockPreconditioner_SparseML <<< " << endl;
  ...
  // ============================================================================
#ifdef ML_V1
  if (printproc) cout << "Setting ML Parameter List: \n" << endl;
  ...
  MLAsolve = rcp(new ML_Epetra::MultiLevelPreconditioner(*Ablock, MLList, false));
  // MLSsolve = rcp(new ML_Epetra::MultiLevelPreconditioner(*Smatrix, MLList, false));
  // MLPsolve = rcp(new ML_Epetra::MultiLevelPreconditioner(*Hblock, MLList, false));
  
  if (printproc) cout << "ML Parameter List: \n" << endl;
  if (printproc) cout << MLList << endl;
#endif
}
...
// ==============================================================================
int BlockPreconditioner_SparseML::computePreconditioner(RCP<const Epetra_Vector> xVec, void* StateData_)
{
  ...
  try{
...
#ifdef ML_V1
    {
      ...
      // A solve
      if (MLAsolve->IsPreconditionerComputed()) {
	ierr = MLAsolve->DestroyPreconditioner();
	check_flag(&ierr, "ML DestroyPreconditioner: A solve", 1);
      }     
      ierr = MLAsolve->ComputePreconditioner();
      check_flag(&ierr, "ML ComputePreconditioner: A solve", 1);

      // // S solve
      // if (MLSsolve->IsPreconditionerComputed()) {
      // 	ierr = MLSsolve->DestroyPreconditioner();
      // 	check_flag(&ierr, "ML DestroyPreconditioner: S solve", 1);
      // }     
      // ierr = MLSsolve->ComputePreconditioner();
      // check_flag(&ierr, "ML ComputePreconditioner: S solve", 1);

      // // P solve
      // if (MLPsolve->IsPreconditionerComputed()) {
      // 	ierr = MLPsolve->DestroyPreconditioner();
      // 	check_flag(&ierr, "ML DestroyPreconditioner: P solve", 1);
      // }     
      // ierr = MLPsolve->ComputePreconditioner();
      // check_flag(&ierr, "ML ComputePreconditioner: P solve", 1);
    }
#endif

#ifdef DEBUG_PRINT_ON      
    if (printproc) cout << "Preconditioner Assembly Complete" << endl;
#endif
  }
...
// ------------------------------------------------------------------------------
#ifdef PREC_IDENT
int BlockPreconditioner_SparseML::ApplyInverse(const Epetra_MultiVector& X, 
...
{  
  ...
  try{
...
#endif
    {
...
#if defined(ML_V1)
      MLAsolve->ApplyInverse(*VelVec, *Ax);
      ...
      ierr = ML_Create(&ml_object, N_grids); // always returns 0
      
      ierr = EpetraMatrix2MLMatrix(ml_object, 0, &(*Ablock));
      check_flag(&ierr, "EpetraMatrix2MLMatrix", 1);
      
      N_levels = ML_Gen_MGHierarchy_UsingAggregation(ml_object, 0, ML_INCREASING, NULL);
      
      ierr = ML_Gen_Smoother_Jacobi(ml_object, ML_ALL_LEVELS, ML_PRESMOOTHER, 1, ML_DEFAULT);
      check_flag(&ierr, "ML_Gen_Smoother_Jacobi", 1);
      
      ierr = ML_Gen_Solver(ml_object, ML_MGV, 0, N_levels-1); // always returns 0
      
      if (printproc) cout << "ML iterate" << endl;
      
      ierr = ML_Iterate(ml_object, Ax->Values(), VelVec->Values()); // always returns 0
      
      ierr = ML_Destroy(&ml_object); // always returns 0
      
      if (printproc) cout << "ML complete" << endl;
#else
#error Unknown ML interface version in BlockPreconditioner_SparseML.cpp
#endif
    }
...
#endif
    {
      ...
      // MLSsolve->ApplyInverse(*PsVec, *Sx);
