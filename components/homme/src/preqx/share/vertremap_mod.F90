...
! HHLEE 20190521
  ...
  real (kind=real_kind), dimension(np,np,pver,qsize) :: Qdp_test
     ...
     if (qsize>0) then
      ...
      do q = 1, qsize
         elem(ie)%state%Qdp(:,:,:,q,np1_qdp) = elem(ie)%state%Qdp(:,:,:,q,np1_qdp) + &
                   Qtmp_host(:,:,:,q) *dp(:,:,:) * dt
         !Qdp_test(:,:,:,q) = elem(ie)%state%Qdp(:,:,:,q,np1_qdp) + &
         !          Qtmp_host(:,:,:,q) *dp(:,:,:) * dt
      enddo
      elem(ie)%state%Qdp(:,:,:,:,np1_qdp) = max(elem(ie)%state%Qdp(:,:,:,:,np1_qdp),0.) 
      !Qdp_test = max(Qdp_test,0.)
#else
