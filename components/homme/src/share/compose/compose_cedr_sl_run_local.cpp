...
void solve_local (const Int ie, const Int k, const Int q,
                  ...
                  const Real Qm, V4& qdp_c, V3& q_c) {
  if (limiter_option == 8)
    solve_local<np_,8>(ie, k, q, np1, n1_qdp, np, scalar_bounds, spheremp,
                       dp3d_c, q_min, q_max, Qm, qdp_c, q_c);
  else
    solve_local<np_,9>(ie, k, q, np1, n1_qdp, np, scalar_bounds, spheremp,
                       dp3d_c, q_min, q_max, Qm, qdp_c, q_c);
}
