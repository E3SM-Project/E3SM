...
#ifdef CAM
    ! be sure we don't try to run cam with more procs than elems
    if (par%nprocs > nelem) then
       write(errormsg, *) 'prim_driver_mod: CAM requires nprocs .lte. nelem ', &
                          'but nprocs = ', par%nprocs, ' and nelem = ', nelem
