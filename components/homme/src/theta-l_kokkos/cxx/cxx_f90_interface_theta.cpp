...
void cxx_push_results_to_f90(F90Ptr &elem_state_v_ptr,         F90Ptr &elem_state_w_i_ptr,
  ...
  elements.m_state.push_to_f90_pointers(elem_state_v_ptr, elem_state_w_i_ptr, elem_state_vtheta_dp_ptr,
...
void init_functors_c ()
{
  ...
  auto& diag = Context::singleton().get<DiagnosticsTheta>();
...
{
  Elements& elements = Context::singleton().get<Elements> ();
  elements.m_state.pull_from_f90_pointers(elem_state_v_ptr,elem_state_w_i_ptr,elem_state_vtheta_dp_ptr,
...
{
  Elements& elements = Context::singleton().get<Elements> ();
  DiagnosticsTheta& diagnostics = Context::singleton().create_with_base<DiagnosticsBase,DiagnosticsTheta> ();
