!----------------------------------------------------------------------------!
! Main File for the fvm project in HOMME                                   !
...
program fvm_main
  ...
  use fvm_bench_mod, only: cslam_run_bench
  ! -----------------------------------------------
...
9 format("process: ",i2,1x,"thread: ",i2,4x,"element limits: ",i4,i4)
  ...
  write(numtrac_char,*) ntrac
   ...
   call t_prf('CSLAMTime-'//trim(adjustl(numproc_char))//'-'//trim(adjustl(numtrac_char)),par%comm)
