...
! =========== PRIMITIVE-EQUATION DATA-STRUCTURES =====================
  ...
  type, public :: elem_accum_t
    ...
    real (kind=real_kind) :: pair3b(np,np)
    real (kind=real_kind) :: pair4a(np,np)
    real (kind=real_kind) :: pair4b(np,np)
    real (kind=real_kind) :: pair5a(np,np)
    real (kind=real_kind) :: pair5b(np,np)
    real (kind=real_kind) :: pair6a(np,np)
    real (kind=real_kind) :: pair6b(np,np)
    real (kind=real_kind) :: pair7a(np,np)
    real (kind=real_kind) :: pair7b(np,np)
    real (kind=real_kind) :: pair8a(np,np)
