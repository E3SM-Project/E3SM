...
contains
  ...
  subroutine prim_advance_exp(elem, deriv, hvcoord, hybrid,dt, tl,  nets, nete, compute_diagnostics)
    ...
    call t_startf('thetah_advance_exp')
    nm1   = tl%nm1
    n0    = tl%n0
    np1   = tl%np1
    nstep = tl%nstep
...
!$omp parallel do private(k,i,j,Qt)
     ...
     if (rsplit>0) then
        ...
        ! ==============================================
        call preq_vertadv(elem(ie)%state%theta(:,:,:,n0),elem(ie)%state%v(:,:,:,:,n0), &
...
!$omp parallel do private(k,i,j,v1,v2,E,Ephi,vtemp,vgrad_theta,gpterm,glnps1,glnps2,u_m_umet,v_m_vmet,t_m_tmet)
     ...
     vertloop: do k=1,nlev
        ...
        ! ================================================
        vtemp(:,:,:)   = gradient_sphere(elem(ie)%state%theta(:,:,k,n0),deriv,elem(ie)%Dinv)
        do j=1,np
           do i=1,np
              v1     = elem(ie)%state%v(i,j,1,k,n0)
              v2     = elem(ie)%state%v(i,j,2,k,n0)
     ...
     ! grad_phi(), dp(), p(), theta_vadv(), v_vadv(), divdp()
     ! =========================================================
