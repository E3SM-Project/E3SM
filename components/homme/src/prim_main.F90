...
! main routine for stand-alone preqx executable
...
program prim_main
  ...
  use prim_driver_mod,  only: prim_init1, prim_init2, prim_run, prim_finalize,leapfrog_bootstrap, prim_run_subcycle
  use prim_state_mod,   only: prim_printstate_par
  use thread_mod,       only: nthreads, vert_num_threads, omp_get_thread_num, omp_set_num_threads,&
                              omp_get_nested, omp_get_max_threads
  ...
  ! Read namelist, initialize and partition the mesh
...
9 format("process: ",i2,1x,"horiz thread id: ",i2,1x,"# vert threads: ",i2,1x,"element limits: ",i5,"-",i5)
  ...
  ! Read vertical levels from file  (cam initializes hvcoord externally)
  ...
  ! Set initial conditions, timesteps, and filters
  ...
  ! Output initial state for new runs
  ...
  ! Use leapfrog bootstrap procedure for semi-implicit integration
