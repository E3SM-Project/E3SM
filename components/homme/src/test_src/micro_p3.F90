...
!       mu_r = real(ii-1)  ! values of mu
       ...
       meansize_loop: do jj = 1,300
          ...
          revap_table_vals(jj,ii) = 10.**(log10(dum5)+(mu_r+1.)*log10(lamr)-(3.*mu_r))
  ...
  SUBROUTINE p3_main_part1(kts, kte, kbot, ktop, kdir, do_predict_nc, do_prescribed_CCN, dt,                &
    ...
    logical, intent(out) :: is_nucleat_possible, is_hydromet_present
    ...
    k_loop_1: do k = kbot,ktop,kdir
      ...
      !    prescribe that value
          ...
          if (do_prescribed_CCN) then
             ...
             nc(k) = max(nc(k) + nc_nuceat_tend(k) * dt,0.0)
          else
       ...
       if (qi(k).lt.qsmall .or. (qi(k).lt.1.e-8 .and.             &
       ...
       if (qi(k).ge.qsmall .and. qi(k).lt.1.e-8 .and.             &
...
!    real :: qcevp   ! cloud droplet evaporation
    ...
    logical :: log_exitlevel, log_wetgrowth
   ...
   k_loop_main: do k = kbot,ktop,kdir
      ...
      if (log_exitlevel .and.                                                           &
         (t_atm(k).lt.T_zerodegc .and. qv_supersat_i(k).lt.-0.05)) goto 555   !i.e. skip all process rates
      ...
      qc2qr_accret_tend   = 0.;     qr2qv_evap_tend   = 0.;     qc2qr_autoconv_tend   = 0.;
      nc_accret_tend   = 0.;     nc_selfcollect_tend   = 0.;
      nc2nr_autoconv_tend  = 0.;     nr_selfcollect_tend   = 0.;
      nr_evap_tend   = 0.;     ncautr  = 0.

      ! initialize ice-phase  process rates
      qi2qv_sublim_tend   = 0.;     nr_ice_shed_tend  = 0.
      qc2qi_hetero_freeze_tend  = 0.;     qrcol   = 0.;     qc2qr_ice_shed_tend   = 0.
      qi2qr_melt_tend   = 0.;     qccol   = 0.
      qr2qi_immers_freeze_tend  = 0.;     qinuc   = 0.;     ni2nr_melt_tend   = 0.
      nc_collect_tend   = 0.;     ncshdc  = 0.
      nc2ni_immers_freeze_tend  = 0.;     nr_collect_tend   = 0.;     ni_selfcollect_tend   = 0.
      ni_nucleat_tend   = 0.;     qidep   = 0.;     qiberg  = 0.
      nr2ni_immers_freeze_tend  = 0.;     ni_sublim_tend   = 0.;     qwgrth  = 0.

      ncheti_cnt = 0.0; qcheti_cnt = 0.0; nicnt = 0.0 
      qicnt = 0.0;      ninuc_cnt = 0.0;  qinuc_cnt = 0.0
      qi_wetDepos = 0.0
      ...
      if (qi_incld(k).ge.qsmall) then
         ...
         if (qr_incld(k).ge.qsmall) then
            ...
            table_val_nr_collect = 0.
            table_val_qr2qi_collect = 0.
         endif
...
444   continue
      ...
      p3_tend_out(k,34) = 0.                    ! used to be qcmul, but that has been removed.  Kept at 0.0 as placeholder.
...
555   continue
 ...
 subroutine p3_main_part3(kts, kte, kbot, ktop, kdir, p3_max_mean_rain_size,                                                &
   ...
   k_loop_final_diagnostics:  do k = kbot,ktop,kdir
      ...
      if (qc(k).ge.qsmall) then
         ...
         call get_cloud_dsd2(qc_incld,nc_incld,mu_c(k),rho(k),nu(k),dnu,lamc(k),  &
              tmp1,tmp2)
       if (mincdnc.gt.0.) nc_incld = max(nc_incld,mincdnc/rho(k)) 
         diag_eff_radius_qc(k) = 0.5*(mu_c(k)+3.)/lamc(k)
      ...
      qi_not_small:  if (qi(k).ge.qsmall) then
         ...
         ze_ice(k) = ze_ice(k) + 0.1892*table_val_ice_reflectivity*ni_incld*rho(k)   ! sum contribution from each ice category (note: 0.1892 = 0.176/0.93)
         ze_ice(k) = max(ze_ice(k),1.e-22)

         !above formula for ze only makes sense for in-cloud vals, but users expect cell-ave output.
         ze_ice(k) = ze_ice(k)*cld_frac_i(k)
         diag_ze_ice(k) = 10.*bfb_log10(ze_ice(k)*1.e18)
      else
         ...
         bm(k) = 0.
         diag_diam_qi(k) = 0.0
         diag_eff_radius_qi(k) = 0.0
         diag_vm_qi(k) = 0.0

      endif qi_not_small

      ! sum ze components and convert to dBZ
      diag_equiv_reflectivity(k) = 10.*bfb_log10((ze_rain(k) + ze_ice(k))*1.e18)
  ...
  SUBROUTINE p3_main(qc,nc,qr,nr,th_atm,qv, & !inout 4 tracers, theta, qv
    ...
    inv_dz    = 1./dz  ! inverse of thickness of layers
    ...
    timeScaleFactor = min(1./120., inv_dt)
    ...
    prec      = 0.
    mu_r      = 0.
    diag_equiv_reflectivity   = -99.
    diag_ze_rain = -99.0 
    diag_ze_ice  = -99.0

    ze_ice    = 1.e-22
    ze_rain   = 1.e-22
    diag_eff_radius_qc = 10.e-6 ! default value
    diag_eff_radius_qi = 25.e-6 ! default value
    diag_vm_qi  = 0.
    diag_diam_qi   = 0.
    rho_qi = 0.
...
!here was col loop
      ...
      ! if (debug_ON) call check_values(qv,T,i,it,debug_ABORT,100,col_location)
       ...
       call p3_main_part1(kts, kte, kbot, ktop, kdir, do_predict_nc, do_prescribed_CCN, dt, &
            ...
            rhofaci(:), acn(:), qv(:), th_atm(:), qc(:), nc(:), qr(:), nr(:), &
       ...
       call p3_main_part2(kts, kte, kbot, ktop, kdir, do_predict_nc, do_prescribed_CCN, dt, inv_dt, &
            p3_autocon_coeff,p3_accret_coeff,p3_qc_autocon_expon,p3_nc_autocon_expon,p3_qc_accret_expon, &
            p3_wbf_coeff,p3_embryonic_rain_size, p3_max_mean_rain_size, &
            pres(:), dpres(:), dz(:), nc_nuceat_tend(:), exner(:), inv_exner(:), &
            inv_cld_frac_l(:), inv_cld_frac_i(:), inv_cld_frac_r(:), ni_activated(:), inv_qc_relvar(:), &
            cld_frac_i(:), cld_frac_l(:), cld_frac_r(:), qv_prev(:), t_prev(:), &
            t_atm(:), rho(:), inv_rho(:), qv_sat_l(:), &
            qv_sat_i(:), qv_supersat_i(:), rhofacr(:), rhofaci(:), acn(:), qv(:), th_atm(:), &
            qc(:), nc(:), qr(:), nr(:), qi(:), ni(:), qm(:), &
            bm(:), latent_heat_vapor(:), latent_heat_sublim(:), latent_heat_fusion(:), qc_incld(:), qr_incld(:), &
            qi_incld(:), qm_incld(:), nc_incld(:), nr_incld(:), ni_incld(:), &
            bm_incld(:), mu_c(:), nu(:), lamc(:), cdist(:), cdist1(:), &
            cdistr(:), mu_r(:), lamr(:), logn0r(:), qv2qi_depos_tend(:), precip_total_tend(:), &
       ...
       p3_tend_out(:,43) = ( nc(:)      - nc_old(:) ) * inv_dt       ! Liq. # microphysics tendency, measure
       p3_tend_out(:,44) = ( qr(:)      - qr_old(:) ) * inv_dt       ! Rain microphysics tendency, measure
       p3_tend_out(:,45) = ( nr(:)      - nr_old(:) ) * inv_dt       ! Rain # microphysics tendency, measure
       p3_tend_out(:,46) = ( qi(:)      - qi_old(:) ) * inv_dt       ! Ice  microphysics tendency, measure
       p3_tend_out(:,47) = ( ni(:)      - ni_old(:) ) * inv_dt       ! Ice  # microphysics tendency, measure
       p3_tend_out(:,48) = ( qv(:)      - qv_old(:) ) * inv_dt       ! Vapor  microphysics tendency, measure
       p3_tend_out(:,49) = ( th_atm(:)  - th_atm_old(:) ) * inv_dt   ! Pot. Temp. microphysics tendency, measure
       ...
       !      if (debug_ON) then
       !         tmparr1(:) = th(:)*inv_exner(:)!(pres(:)*1.e-5)**(rd*inv_cp)
       ...
       p3_tend_out(:,36) = qc(:) ! Liq. sedimentation tendency, initialize
       p3_tend_out(:,37) = nc(:) ! Liq. # sedimentation tendency, initialize
       ...
       p3_tend_out(:,39) = nr(:) ! Rain # sedimentation tendency, initialize
       ...
       p3_tend_out(:,41) = ni(:) ! Ice # sedimentation tendency, initialize
       ...
       call p3_main_part3(kts, kte, kbot, ktop, kdir, p3_max_mean_rain_size,                                                  &
            exner(:), cld_frac_l(:), cld_frac_r(:), cld_frac_i(:),                                                    &
            rho(:), inv_rho(:), rhofaci(:), qv(:), th_atm(:), qc(:), nc(:), qr(:), nr(:), qi(:), ni(:), &
...
! end col loop  enddo i_loop_main
  ...
  subroutine find_lookupTable_indices_1b(dumj,dum3,rcollsize,qr,nr)
    ...
    if (qr.ge.qsmall .and. nr.gt.0.) then
       ...
       dum3  = (bfb_log10(1.*dumlr)+5.)*10.70415
       dumj  = int(dum3)
       ! set limits
  ...
  subroutine get_cloud_dsd2(qc,nc,mu_c,rho,nu,dnu,lamc,cdist,cdist1)
    ...
    if (qc.ge.qsmall) then
       ...
       ! calculate lamc
       lamc = bfb_cbrt(cons1*nc*(mu_c+3.)*(mu_c+2.)*(mu_c+1.)/qc)
       ...
       if (lamc.lt.lammin) then
          lamc = lammin
          nc   = 6.*(lamc*lamc*lamc)*qc/(pi*rho_h2o*(mu_c+3.)*(mu_c+2.)*(mu_c+1.))
  ...
  subroutine get_rain_dsd2(qr,p3_max_mean_rain_size,nr,mu_r,lamr,cdistr,logn0r)
    ...
    if (qr.ge.qsmall) then
       ...
       nr      = max(nr,nsmall)
       inv_dum = bfb_cbrt(qr/(cons1*nr*6.))

       ! Apply constant mu_r:  Recall the switch to v4 tables means constant mu_r
       mu_r = mu_r_constant
       lamr   = bfb_cbrt(cons1*nr*(mu_r+3.)*(mu_r+2.)*(mu_r+1.)/(qr))  ! recalculate slope based on mu_r
       lammax = (mu_r+1.)*1.e+5   ! check for slope
       lammin = (mu_r+1.)*(1.)/p3_max_mean_rain_size   ! breakup is explicitly included (mean size set by namelist parameter)

       ! apply lambda limiters for rain
       if (lamr.lt.lammin) then
          lamr = lammin
          nr   = bfb_exp(3.*bfb_log(lamr)+bfb_log(qr)+bfb_log(bfb_gamma(mu_r+1.))-bfb_log(bfb_gamma(mu_r+4.)))/(cons1)
       elseif (lamr.gt.lammax) then
          lamr = lammax
          nr   = bfb_exp(3.*bfb_log(lamr)+bfb_log(qr)+bfb_log(bfb_gamma(mu_r+1.))-bfb_log(bfb_gamma(mu_r+4.)))/(cons1)
       endif

       cdistr  = nr/bfb_gamma(mu_r+1.)
       logn0r  = bfb_log10(nr)+(mu_r+1.)*bfb_log10(lamr)-bfb_log10(bfb_gamma(mu_r+1.)) !note: logn0r is calculated as log10(n0r)
...
table_val_qi2qr_melting,table_val_qi2qr_vent_melt,latent_heat_vapor,latent_heat_fusion,dv,sc,mu,kap,qv,qi_incld,ni_incld,    &
   ...
   if (qi_incld .ge.qsmall .and. t_atm.gt.T_zerodegc) then
      ...
      T_zerodegc)*kap-rho*latent_heat_vapor*dv*(qsat0-qv))*2.*pi/latent_heat_fusion)*ni_incld
...
qv,qc_incld,qi_incld,ni_incld,qr_incld,    &
   ...
   if (qi_incld.ge.qsmall .and. qc_incld+qr_incld.ge.1.e-6 .and. t_atm.lt.T_zerodegc) then
      ...
      2.*pi*(rho*latent_heat_vapor*dv*(qsat0-qv)-(t_atm-T_zerodegc)*           &
      kap)/(latent_heat_fusion+cpw*(t_atm-T_zerodegc)))*ni_incld

      qwgrth = max(qwgrth,0.)
      dum    = max(0.,(qccol+qrcol)-qwgrth)
      if (dum.ge.1.e-10) then
         nr_ice_shed_tend = nr_ice_shed_tend + dum*1.923e+6   ! 1/5.2e-7, 5.2e-7 is the mass of a 1 mm raindrop
...
table_val_qi_fallspd,acn,lamc, mu_c,qc_incld,qccol,    &
   ...
   if (qccol.ge.qsmall .and. t_atm.lt.T_zerodegc) then
      ...
      iTc   = 1./min(-0.001,t_atm-T_zerodegc)

             ! cloud:
      if (qc_incld.ge.qsmall) then
         ! droplet fall speed
         ! (use Stokes' formulation (thus use analytic solution)
         Vt_qc = acn*bfb_gamma(4.+bcn+mu_c)/(bfb_pow(lamc,bcn)*bfb_gamma(mu_c+4.))
         ...
         Ri        = max(1.,min(Ri,12.))
         if (Ri.le.8.) then
            rho_qm_cloud  = (0.051 + 0.114*Ri - 0.0055*bfb_square(Ri))*1000.
         else
            ...
            rho_qm_cloud  = 611.+72.25*(Ri-8.)
         endif
      else
         rho_qm_cloud = 400.
      endif    !if qc>qsmall
   else
      vtrmi1 = 0. ! no velocity if no ice
      rho_qm_cloud = 400.
...
qr2qi_immers_freeze_tend, nr2ni_immers_freeze_tend)
   ...
   if (qr_incld.ge.qsmall .and. t_atm.le.T_rainfrz) then
      ...
      Q_nuc = cons6*bfb_exp(bfb_log(cdistr) + bfb_log(bfb_gamma(7.+mu_r)) - 6.*bfb_log(lamr))*bfb_exp(aimm*(T_zerodegc-t_atm))
      N_nuc = cons5*bfb_exp(bfb_log(cdistr) + bfb_log(bfb_gamma(mu_r+4.)) - 3.*bfb_log(lamr))*bfb_exp(aimm*(T_zerodegc-t_atm))
...
!         ! dum = exp(-0.639+0.1296*100.*qv_supersat_i(i,k))*1000.*inv_rho(i,k)  !Meyers et al. (1992)
      ...
      ! +++ [JS]: Here we add the cirrus/aerosol contribution to the Cooper scheme under dedicated switch +++
            if(do_Cooper_inP3)then
               ...
               ni_nucleat_tend = ni_nucleat_tend + max(0., (ni_activated - ni)*inv_dt)
               qinuc = ni_nucleat_tend * mi0
            else          
               ni_nucleat_tend = max(0., (ni_activated - ni)*inv_dt)
               qinuc = ni_nucleat_tend * mi0
            endif   
      endif
   endif
...
subroutine droplet_self_collection(rho,inv_rho,qc_incld,mu_c,nu,nc2nr_autoconv_tend,    &
   ...
   if (qc_incld.ge.qsmall) then
      ...
      if (iparam.eq.1) then
         ...
         nc_selfcollect_tend = -kc*(1.e-3*rho*qc_incld)**2*(nu+2.)/(nu+1.)*         &
              1.e+6*inv_rho+nc2nr_autoconv_tend
      elseif (iparam.eq.2) then
         !Beheng (994)
         nc_selfcollect_tend = -5.5e+16*inv_rho*mu_c**(-0.63)*(1.e-3*rho*qc_incld)**2
...
subroutine cloud_rain_accretion(rho,inv_rho,qc_incld,nc_incld,qr_incld,inv_qc_relvar,    &
  ...
  if (qr_incld.ge.qsmall .and. qc_incld.ge.qsmall) then
     ...
     if (iparam.eq.1) then
        ...
        qc2qr_accret_tend = kr*rho*0.001*qc_incld*qr_incld*dum1
        nc_accret_tend = qc2qr_accret_tend*rho*0.001*(nc_incld*rho*1.e-6)/(qc_incld*rho*   &
             0.001)*1.e+6*inv_rho
     elseif (iparam.eq.2) then
        !Beheng (994)
        qc2qr_accret_tend = 6.*rho*(qc_incld*qr_incld)
        nc_accret_tend = qc2qr_accret_tend*rho*1.e-3*(nc_incld*rho*1.e-6)/(qc_incld*rho*1.e-3)* &
             1.e+6*inv_rho
     elseif (iparam.eq.3) then
        !Khroutdinov and Kogan (2000)
     ...
     if (qc2qr_accret_tend.eq.0.) nc_accret_tend = 0.
     if (nc_accret_tend.eq.0.) qc2qr_accret_tend = 0.
...
subroutine rain_self_collection(rho,qr_incld,nr_incld,    &
   ...
   if (qr_incld.ge.qsmall) then
      ...
      if (dum2.lt.dum1) then
         ...
         dum = 2. - bfb_exp(2300.*(dum2-dum1))
      endif

      if (iparam.eq.1) then
         nr_selfcollect_tend = dum*kr*1.e-3*qr_incld*nr_incld*rho
      elseif (iparam.eq.2 .or. iparam.eq.3) then
         nr_selfcollect_tend = dum*5.78*nr_incld*qr_incld*rho
      endif
...
subroutine cloud_water_autoconversion(rho,qc_incld,nc_incld,inv_qc_relvar,          &
   ...
   qc_not_small: if (qc_incld.ge.1.e-8) then
      ...
      if (nc2nr_autoconv_tend.eq.0. .or. do_precip_off) qc2qr_autoconv_tend  = 0.
      if (do_precip_off) ncautr = 0.
...
subroutine prevent_ice_overdepletion(pres,t_atm,qv,latent_heat_vapor,latent_heat_sublim,inv_dt,dt,qidep,qinuc,qinuc_cnt,    &
   ...
   if(qtmp_all > qv_sat_l)then
      ...
      qrevp_satadj = (q_sink-dumqv_sat_l)/(1. + bfb_square(latent_heat_vapor)*dumqv_sat_l/(cp*rv* bfb_square(t_sink) ))*inv_dt
      qr2qv_evap_tend    = qr2qv_evap_tend*min(1.,max(0.,-qrevp_satadj)/max(qv_source_evp, 1.e-20))

      ! ... Next, ice-sublimation is limited in the same way but with sublim LH 
      dumqv_sat_i = qv_sat(t_sink,pres,1)
      qi2qv_sublim_satadj = (q_sink-dumqv_sat_i)/(1. + bfb_square(latent_heat_sublim)*dumqv_sat_i/(cp*rv* bfb_square(t_sink) ))*inv_dt
      qi2qv_sublim_tend  = qi2qv_sublim_tend*min(1.,max(0.,-qi2qv_sublim_satadj)/max(qv_source_evp, 1.e-20))
...
subroutine water_vapor_conservation(qv,qidep,qinuc,qi2qv_sublim_tend,qr2qv_evap_tend,qinuc_cnt,dt)
  ...
  if (qv_sink > qv_avail .and. qv_sink>1.e-20) then
...
subroutine cloud_water_conservation(qc,dt,    &
   ...
   if (sinks .gt. qc .and. sinks.ge.1.e-20) then
      ratio  = qc/sinks
   ...
   !water to begin with.
   if (qc .gt. 1.e-20) then
      qidep  = qidep*(1.-ratio)
      qi2qv_sublim_tend  = qi2qv_sublim_tend*(1.-ratio)
   end if
...
subroutine update_prognostic_liquid(qc2qr_accret_tend,nc_accret_tend,qc2qr_autoconv_tend,nc2nr_autoconv_tend, &
   ...
   if (iparam.eq.1 .or. iparam.eq.2) then
      nr = nr + (0.5*nc2nr_autoconv_tend-nr_selfcollect_tend-nr_evap_tend)*dt
   else
...
qr2qv_evap_tend,nr_evap_tend)
  ...
  !https://doi.org/10.1175/2007JAS2374.1 for details.
   ...
   if (qc_incld + qi_incld < 1.e-6) then
         cld_frac = 0.
   else
         cld_frac = cld_frac_l
   end if
   ...
   if (cld_frac_r > cld_frac .and. ssat_r<0. .and. qr_incld >= qsmall ) then
      ...
      if (t < 273.15) then
         eps_eff   = epsr + epsi_tot*(1.0 + latent_heat_sublim*inv_cp*dqsdt)/abi
      else
         eps_eff   = epsr
      endif

      !Set lower bound on eps_eff to prevent division by zero
      eps_eff  = max(1.e-20,eps_eff)
      tau_eff = 1.0/eps_eff
      ...
      if (t < 273.15) then
         A_c = (qv - qv_prev)*inv_dt - dqsdt*(t-t_prev)*inv_dt - (qv_sat_l - qv_sat_i)* &
               (1.0 + latent_heat_sublim*inv_cp*dqsdt)/abi*epsi_tot
      else
      ...
      if (qr_incld < 1e-12 .and. qv/qv_sat_l < 0.999) then
      ...
      qr2qv_evap_tend = max(0., qr2qv_evap_tend)
...
mu,dv,sc,dqsdt,dqsidt,ab,abi,kap,eii)
   ...
   ab     = 1.+dqsdt*latent_heat_vapor*inv_cp
   abi    = 1.+dqsidt*latent_heat_sublim*inv_cp
   kap    = 1.414e+3*mu

   ! very simple temperature dependent aggregation efficiency
   if (t_atm.lt.253.15) then
      eii = 0.001
   else if (t_atm.ge.253.15.and.t_atm.lt.273.15) then
      eii = 0.001 + (t_atm-253.15)*(0.3 - 0.001)/20.  ! linear ramp from 0.1 to 1 between 253.15 and 268.15 K
   else
      eii = 0.3
   end if
         ...
         substep_sedi_c2: do while (dt_left.gt.1.e-4)
            ...
            kloop_sedi_c2: do k = k_qxtop,k_qxbot,-kdir
               ...
               qc_notsmall_c2: if (qc_incld(k)>qsmall) then
                  ...
                  dum = 1. / bfb_pow(lamc(k), bcn)
                  V_qc(k) = acn(k)*bfb_gamma(4.+bcn+mu_c(k))*dum/(bfb_gamma(mu_c(k)+4.))
                  V_nc(k) = acn(k)*bfb_gamma(1.+bcn+mu_c(k))*dum/(bfb_gamma(mu_c(k)+1.))
   ...
   k_loop_mlt:  do k = kbot,ktop,kdir
      if (qi(k).ge.qsmall .and. (th_atm(k)/exner(k)) > t_snow_melt) then
         ...
         if(th_atm(k)/exner(k) - del_mass*latent_heat_fusion(k)/cp <  t_snow_melt)then
            ...
            frac_mlt = max(0.0,frac_mlt)
            frac_mlt = min(1.0,frac_mlt)
         else
            frac_mlt = 1.0
         endif
         rv_tmp = 3.0*qi(k)/ni(k)/4.0/pi/900.0  ! density of pure ice [kg/m3]
         rv = 1.0e6*bfb_cbrt(rv_tmp)                        ! in [um]
         if((qm(k)/qi(k)) < 0.1)then
            if(rv < 100.0)then
               ! ... Ice crystas melt into cld droplets
               qi(k) = max((1.0 - frac_mlt)*del_mass,0.0)
               qm(k) = max((1.0 - frac_mlt)*del_mass,0.0)
               ni(k) = max((1.0 - frac_mlt)*del_num,0.0)
