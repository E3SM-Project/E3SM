 ...
 *  ----------------------------------------------------------
 *  This file has been merged from multiple headers. Please
 * don't edit it directly
 *  Copyright (c) 2012 Two Blue Cubes Ltd. All rights
 * reserved.
 *
...
#define INTERNAL_CATCH_STRINGIFY(expr) \
  INTERNAL_CATCH_STRINGIFY2(expr)
...
// Detect a number of compiler features - mostly C++11/14
// conformance - by compiler
// The following features are defined:
//
// CATCH_CONFIG_CPP11_NULLPTR : is nullptr supported?
// CATCH_CONFIG_CPP11_NOEXCEPT : is noexcept supported?
// CATCH_CONFIG_CPP11_GENERATED_METHODS : The delete and
// default keywords for compiler generated methods
// CATCH_CONFIG_CPP11_IS_ENUM : std::is_enum is supported?
// CATCH_CONFIG_CPP11_TUPLE : std::tuple is supported
// CATCH_CONFIG_CPP11_LONG_LONG : is long long supported?
// CATCH_CONFIG_CPP11_OVERRIDE : is override supported?
// CATCH_CONFIG_CPP11_UNIQUE_PTR : is unique_ptr supported
// (otherwise use auto_ptr)

// CATCH_CONFIG_CPP11_OR_GREATER : Is C++11 supported?

// CATCH_CONFIG_VARIADIC_MACROS : are variadic macros
// supported?
// CATCH_CONFIG_COUNTER : is the __COUNTER__ macro
// supported?
// ****************
// Note to maintainers: if new toggles are added please
// document them
// in configuration.md, too
// ****************

// In general each macro has a _NO_<feature name> form
// (e.g. CATCH_CONFIG_CPP11_NO_NULLPTR) which disables the
// feature.
// Many features, at point of detection, define an
// _INTERNAL_ macro, so they
// can be combined, en-mass, with the _NO_ forms later.

// All the C++11 features can be disabled with
...
// - otherwise more recent versions define __cplusplus >=
// 201103L
...
// Now set the actual defines based on the above + anything
...
#define CATCH_CONFIG_CPP11_LONG_LONG
    ...
    !defined(CATCH_CONFIG_CPP11_OVERRIDE) &&         \
    !defined(CATCH_CONFIG_NO_CPP11)
#define CATCH_CONFIG_CPP11_OVERRIDE
#endif
#if defined(CATCH_INTERNAL_CONFIG_CPP11_UNIQUE_PTR) && \
    !defined(CATCH_CONFIG_NO_UNIQUE_PTR) &&            \
    !defined(CATCH_CONFIG_CPP11_UNIQUE_PTR) &&         \
    !defined(CATCH_CONFIG_NO_CPP11)
...
// noexcept support:
#if defined(CATCH_CONFIG_CPP11_NOEXCEPT) && \
    !defined(CATCH_NOEXCEPT)
...
inline void deleteAll(ContainerT &container) {
  typename ContainerT::const_iterator it =
      container.begin();
  typename ContainerT::const_iterator itEnd =
      container.end();
  for(; it != itEnd; ++it) delete *it;
}
template <typename AssociativeContainerT>
inline void deleteAllValues(
    AssociativeContainerT &container) {
...
// This is just here to avoid compiler warnings with macro
...
#define CATCH_NOT_IMPLEMENTED           \
  throw Catch::NotImplementedException( \
...
struct IGeneratorsForTest {
  ...
  virtual IGeneratorInfo &getGeneratorInfo(
      std::string const &fileInfo, std::size_t size) = 0;
  virtual bool moveNext() = 0;
};
...
struct IContext {
  ...
  virtual size_t getGeneratorIndex(
      std::string const &fileInfo, size_t totalSize) = 0;
...
struct IMutableContext : IContext {
  ...
  virtual void setConfig(
      Ptr<IConfig const> const &config) = 0;
};
...
struct ITestCaseRegistry {
  ...
  virtual std::vector<TestCase> const &getAllTests()
      const = 0;
...
std::vector<TestCase> const &getAllTestCasesSorted(
    IConfig const &config);
}
...
struct AutoReg {
  ...
  AutoReg(void (C::*method)(), char const *className,
    ...
    registerTestCase(new MethodTestCase<C>(method),
                     className, nameAndDesc, lineInfo);
  }
...
void registerTestCaseFunction(
    TestFunction function, SourceLineInfo const &lineInfo,
    NameAndDesc const &nameAndDesc);
...
#define INTERNAL_CATCH_TESTCASE2(TestName, ...)          \
  ...
  Catch::AutoReg INTERNAL_CATCH_UNIQUE_NAME(             \
      autoRegistrar)(&TestName, CATCH_INTERNAL_LINEINFO, \
...
#define INTERNAL_CATCH_TEST_CASE_METHOD2(TestName,       \
  ...
  Catch::AutoReg INTERNAL_CATCH_UNIQUE_NAME(             \
      autoRegistrar)(&TestName::test, #ClassName,        \
                     Catch::NameAndDesc(__VA_ARGS__),    \
                     CATCH_INTERNAL_LINEINFO);           \
  }                                                      \
  void TestName::test()
...
#define INTERNAL_CATCH_TEST_CASE_METHOD2(                \
    TestCaseName, ClassName, TestName, Desc)             \
  namespace {                                            \
  struct TestCaseName : ClassName {                      \
    void test();                                         \
  };                                                     \
  Catch::AutoReg INTERNAL_CATCH_UNIQUE_NAME(             \
      autoRegistrar)(&TestCaseName::test, #ClassName,    \
...
#define INTERNAL_CATCH_TEST_CASE_METHOD(ClassName,      \
  ...
  INTERNAL_CATCH_TEST_CASE_METHOD2(                     \
      INTERNAL_CATCH_UNIQUE_NAME(                       \
          ____C_A_T_C_H____T_E_S_T____),                \
      ClassName, TestName, Desc)

///////////////////////////////////////////////////////////////////////////////
#define INTERNAL_CATCH_REGISTER_TESTCASE(Function, Name, \
                                         Desc)           \
  Catch::AutoReg(Function, CATCH_INTERNAL_LINEINFO,      \
                 Catch::NameAndDesc(Name, Desc));
#endif
...
struct ResultDisposition {
  enum Flags {
    ...
    ContinueOnFailure = 0x02,  // Failures fail test, but
                               // execution continues
...
inline ResultDisposition::Flags operator|(
...
class AssertionResult {
 public:
  ...
  AssertionResult &operator=(AssertionResult const &) =
      default;
...
struct Matcher : SharedImpl<IShared> {
  ...
  Generic::AllOf<ExpressionT> operator&&(
      Matcher<ExpressionT> const &other) const;
  Generic::AnyOf<ExpressionT> operator||(
      Matcher<ExpressionT> const &other) const;
...
template <typename ExpressionT>
Generic::Not<ExpressionT> Matcher<ExpressionT>::operator!()
    const {
...
struct StartsWith : MatcherImpl<StartsWith, std::string> {
  ...
  virtual bool match(std::string const &expr) const {
    return startsWith(m_data.adjustString(expr),
                      m_data.m_str);
  }
  virtual std::string toString() const {
...
struct EndsWith : MatcherImpl<EndsWith, std::string> {
  ...
  virtual bool match(std::string const &expr) const {
    return endsWith(m_data.adjustString(expr),
                    m_data.m_str);
  }
  virtual std::string toString() const {
...
// The following functions create the actual matcher
// objects.
// This allows the types to be inferred
template <typename ExpressionT>
inline Impl::Generic::Not<ExpressionT> Not(
    Impl::Matcher<ExpressionT> const &m) {
...
inline Impl::Generic::AllOf<ExpressionT> AllOf(
  ...
  return Impl::Generic::AllOf<ExpressionT>()
      .add(m1)
      .add(m2)
      .add(m3);
}
...
inline Impl::Generic::AnyOf<ExpressionT> AnyOf(
    ...
    Impl::Matcher<ExpressionT> const &m2,
    Impl::Matcher<ExpressionT> const &m3) {
  return Impl::Generic::AnyOf<ExpressionT>()
      .add(m1)
      .add(m2)
      .add(m3);
}
...
inline Impl::StdString::Equals Equals(
    const char *str, CaseSensitive::Choice caseSensitivity =
                         CaseSensitive::Yes) {
  return Impl::StdString::Equals(
      Impl::StdString::makeString(str), caseSensitivity);
}
...
inline Impl::StdString::StartsWith StartsWith(
    std::string const &substr) {
...
inline Impl::StdString::EndsWith EndsWith(
    std::string const &substr) {
...
#pragma warning(push)
#pragma warning( \
    disable : 4389)  // '==' : signed/unsigned mismatch
#endif
...
// nullptr_t support based on pull request #154 from
// Konstantin Baumann
#ifdef CATCH_CONFIG_CPP11_NULLPTR
...
// So the compare overloads can be operator agnostic we
// convey the operator as a template
// enum, which is used to specialise an Evaluator for doing
// the comparison.
...
// This level of indirection allows us to specialise for
// integer types
...
bool compare(unsigned int lhs, long rhs) {
  return applyEvaluator<Op>(
      lhs, static_cast<unsigned long>(rhs));
}
template <Operator Op>
...
std::string toString(
    NSString *CATCH_ARC_STRONG const &nsstring);
...
FalseType operator<<(std::ostream const &,
                     BorgType const &);
...
struct IsStreamInsertable {
  ...
  enum {
    value =
        sizeof(testStreamable(s << t)) == sizeof(TrueType)
  };
};
...
struct StringMaker
    : Detail::StringMakerBase<
          Detail::IsStreamInsertable<T>::value> {};
...
//    static std::string convert( std::vector<T,Allocator>
//    const& v ) {
...
struct StringMaker<std::tuple<Types...>> {
  static std::string convert(
    ...
    os << '{';
    TupleDetail::ElementPrinter<
        std::tuple<Types...>>::print(tuple, os);
    os << " }";
    return os.str();
  }
};
...
/// except when an
/// ostringstream overload does not exist - in which case it
/// attempts to detect
/// that and writes {?}.
/// Overload (not specialise) this template for custom typs
/// that you don't want
...
// Wraps the LHS of an expression and captures the operator
// and RHS (if any) -
...
class ExpressionLhs {
 ...
 public:
  ...
  template <typename RhsT>
  ResultBuilder &operator<=(RhsT const &rhs) {
    return captureExpression<Internal::IsLessThanOrEqualTo>(
        rhs);
  }
  ...
  ResultBuilder &operator>=(RhsT const &rhs) {
    return captureExpression<
        Internal::IsGreaterThanOrEqualTo>(rhs);
  }
  ...
  void endExpression() {
    ...
    m_rb.setLhs(Catch::toString(value))
        .setResultType(value)
        .endExpression();
  }

  // Only simple binary expressions are allowed on the LHS.
  // If more complex compositions are required then place
...
template <typename T>
inline ExpressionLhs<T const &> ResultBuilder::operator<=(
    T const &operand) {
  return ExpressionLhs<T const &>(*this, operand);
}

inline ExpressionLhs<bool> ResultBuilder::operator<=(
    bool value) {
...
struct IResultCapture {
  ...
  virtual void assertionEnded(
      AssertionResult const &result) = 0;
  ...
  virtual void sectionEndedEarly(
      SectionEndInfo const &endInfo) = 0;
  virtual void pushScopedMessage(
      MessageInfo const &message) = 0;
  virtual void popScopedMessage(
      MessageInfo const &message) = 0;

  virtual std::string getCurrentTestName() const = 0;
  virtual const AssertionResult *getLastResult() const = 0;

  virtual void handleFatalErrorCondition(
...
extern "C" __declspec(
    dllimport) void __stdcall DebugBreak();
#define CATCH_BREAK_INTO_DEBUGGER() \
  if(Catch::isDebuggerActive()) {   \
    DebugBreak();                   \
  }
...
///////////////////////////////////////////////////////////////////////////////
// In the event of a failure works out if the debugger needs
// to be invoked
// and/or an exception thrown and takes appropriate action.
// This needs to be done as a macro so the debugger will
// stop in the user
...
///////////////////////////////////////////////////////////////////////////////
#define INTERNAL_CATCH_TEST(expr, resultDisposition, \
                            macroName)               \
  do {                                               \
    Catch::ResultBuilder __catchResult(              \
        macroName, CATCH_INTERNAL_LINEINFO, #expr,   \
        resultDisposition);                          \
    try {                                            \
      CATCH_INTERNAL_SUPPRESS_PARENTHESES_WARNINGS(  \
          __catchResult <= expr)                     \
    ...
    INTERNAL_CATCH_REACT(__catchResult)              \
      ...
      false && !!(expr)))  // expr here is never evaluated
                           // at runtime but it forces the
                           // compiler to give it a look

///////////////////////////////////////////////////////////////////////////////
#define INTERNAL_CATCH_IF(expr, resultDisposition,         \
                          macroName)                       \
  INTERNAL_CATCH_TEST(expr, resultDisposition, macroName); \
  if(Catch::getResultCapture().getLastResult()->succeeded())

///////////////////////////////////////////////////////////////////////////////
#define INTERNAL_CATCH_ELSE(expr, resultDisposition,       \
                            macroName)                     \
  INTERNAL_CATCH_TEST(expr, resultDisposition, macroName); \
...
///////////////////////////////////////////////////////////////////////////////
#define INTERNAL_CATCH_NO_THROW(expr, resultDisposition,   \
                                macroName)                 \
  do {                                                     \
    Catch::ResultBuilder __catchResult(                    \
        macroName, CATCH_INTERNAL_LINEINFO, #expr,         \
...
///////////////////////////////////////////////////////////////////////////////
#define INTERNAL_CATCH_THROWS(expr, resultDisposition,   \
                              matcher, macroName)        \
  do {                                                   \
    Catch::ResultBuilder __catchResult(                  \
        macroName, CATCH_INTERNAL_LINEINFO, #expr,       \
...
///////////////////////////////////////////////////////////////////////////////
#define INTERNAL_CHECK_THAT(arg, matcher,                  \
    ...
    try {                                                  \
      ...
      __catchResult.setLhs(Catch::toString(arg))           \
          .setRhs(matcherAsString ==                       \
                          Catch::Detail::unprintableString \
                      ? #matcher                           \
                      : matcherAsString)                   \
          .setOp("matches")                                \
...
#define INTERNAL_CATCH_SECTION(name, desc)                 \
  if(Catch::Section const &INTERNAL_CATCH_UNIQUE_NAME(     \
         catch_internal_Section) =                         \
         Catch::SectionInfo(CATCH_INTERNAL_LINEINFO, name, \
                            desc))
#endif
...
class CompositeGenerator {
 public:
  ...
  operator T() const {
    size_t overallIndex =
        getCurrentContext().getGeneratorIndex(m_fileInfo,
                                              m_totalSize);

    typename std::vector<
        const IGenerator<T> *>::const_iterator it =
        m_composed.begin();
    typename std::vector<
        const IGenerator<T> *>::const_iterator itEnd =
        m_composed.end();
    ...
    return T();  // Suppress spurious "not all control paths
                 // return a value" warning in Visual Studio
...
#define INTERNAL_CATCH_LINESTR(line) \
  INTERNAL_CATCH_LINESTR2(line)

#define INTERNAL_CATCH_GENERATE(expr) \
  expr.setFileInfo(                   \
      __FILE__ "(" INTERNAL_CATCH_LINESTR(__LINE__) ")")
...
struct IRegistryHub {
  ...
  virtual IReporterRegistry const &getReporterRegistry()
      const = 0;
  virtual ITestCaseRegistry const &getTestCaseRegistry()
      const = 0;
...
struct IExceptionTranslator;
typedef std::vector<const IExceptionTranslator *>
...
class Approx {
 public:
  ...
  friend bool operator==(double lhs, Approx const &rhs) {
    // Thanks to Richard Harris for his help refining this
    // formula
...
struct ITagAliasRegistry {
  ...
  virtual Option<TagAlias> find(
      std::string const &alias) const = 0;
...
// NB. Any general catch headers included here must be
// included
// in catch.hpp first to make sure they are included by the
// single
// header for non obj-usage

///////////////////////////////////////////////////////////////////////////////
// This protocol is really only here for (self) documenting
// purposes, since
...
inline std::string getAnnotation(
    Class cls, std::string const &annotationName,
...
inline Impl::NSStringMatchers::Equals Equals(
    NSString *substr) {
  return Impl::NSStringMatchers::Equals(substr);
}

inline Impl::NSStringMatchers::Contains Contains(
    NSString *substr) {
  return Impl::NSStringMatchers::Contains(substr);
}

inline Impl::NSStringMatchers::StartsWith StartsWith(
    NSString *substr) {
  return Impl::NSStringMatchers::StartsWith(substr);
}

inline Impl::NSStringMatchers::EndsWith EndsWith(
    NSString *substr) {
...
#define OC_TEST_CASE(name, desc)           \
  +(NSString *)INTERNAL_CATCH_UNIQUE_NAME( \
      Catch_Name_test) {                   \
    return @name;                          \
  }                                        \
  +(NSString *)INTERNAL_CATCH_UNIQUE_NAME( \
...
// These are the equivalent of what would usually be cpp
// files
...
class TestSpec {
  ...
  struct Filter {
    ...
    bool matches(TestCaseInfo const &testCase) const {
      // All patterns in a filter must match for the filter
      // to be a match
 ...
 public:
  ...
  Config(ConfigData const &data)
    ...
    if(!data.testsOrTags.empty()) {
      ...
      for(std::size_t i = 0; i < data.testsOrTags.size();
          ++i)
...
// Use Catch's value for console width (store Clara's off to
// the side, if present)
...
// Only use header guard if we are not using an outer
// namespace
...
#define STITCH_TBC_TEXT_FORMAT_OPEN_NAMESPACE \
...
const unsigned int consoleWidth =
...
struct TextAttributes {
  ...
  std::size_t
      initialIndent;   // indent of first line, or npos
...
class Text {
 public:
  Text(std::string const &_str,
    ...
    std::size_t indent =
        _attr.initialIndent != std::string::npos
            ? _attr.initialIndent
            : _attr.indent;
  ...
  inline friend std::ostream &operator<<(
      std::ostream &_stream, Text const &_text) {
...
// CLARA_CONFIG_CPP11_GENERATED_METHODS : The delete and
// default keywords for compiler generated methods
// CLARA_CONFIG_CPP11_OVERRIDE : is override supported?
// CLARA_CONFIG_CPP11_UNIQUE_PTR : is unique_ptr supported
// (otherwise use auto_ptr)

// CLARA_CONFIG_CPP11_OR_GREATER : Is C++11 supported?

// CLARA_CONFIG_VARIADIC_MACROS : are variadic macros
// supported?

// In general each macro has a _NO_<feature name> form
// (e.g. CLARA_CONFIG_CPP11_NO_NULLPTR) which disables the
// feature.
// Many features, at point of detection, define an
// _INTERNAL_ macro, so they
// can be combined, en-mass, with the _NO_ forms later.
...
#define CLARA_CONFIG_CPP11_GENERATED_METHODS
    ...
    !defined(CLARA_CONFIG_CPP11_OVERRIDE) &&         \
    !defined(CLARA_CONFIG_NO_CPP11)
#define CLARA_CONFIG_CPP11_OVERRIDE
#endif
#if defined(CLARA_INTERNAL_CONFIG_CPP11_UNIQUE_PTR) && \
    !defined(CLARA_CONFIG_NO_UNIQUE_PTR) &&            \
    !defined(CLARA_CONFIG_CPP11_UNIQUE_PTR) &&         \
    !defined(CLARA_CONFIG_NO_CPP11)
#define CLARA_CONFIG_CPP11_UNIQUE_PTR
#endif

// noexcept support:
#if defined(CLARA_CONFIG_CPP11_NOEXCEPT) && \
    !defined(CLARA_NOEXCEPT)
...
inline void convertInto(std::string const &_source,
                        std::string &_dest) {
  _dest = _source;
}
inline void convertInto(std::string const &_source,
  ...
  if(sourceLC == "y" || sourceLC == "1" ||
     sourceLC == "true" || sourceLC == "yes" ||
     sourceLC == "on")
    _dest = true;
 ...
 public:
  ...
  BoundArgFunction(BoundArgFunction const &other)
      : functionObj(other.functionObj
                        ...
                        : CLARA_NULL) {}
  BoundArgFunction &operator=(
      BoundArgFunction const &other) {
    IArgFunction<ConfigT> *newFunctionObj =
        other.functionObj ? other.functionObj->clone()
                          : CLARA_NULL;
    delete functionObj;
...
inline std::vector<std::string> argsToVector(
    int argc, char const *const *const argv) {
  std::vector<std::string> args(
      static_cast<std::size_t>(argc));
  for(std::size_t i = 0; i < static_cast<std::size_t>(argc);
      ++i)
    args[i] = argv[i];
...
class Parser {
 ...
 public:
  ...
  void parseIntoTokens(std::vector<std::string> const &args,
    ...
    for(std::size_t i = 1;
        i < args.size() && args[i] != doubleDash; ++i)
      parseIntoTokens(args[i], tokens);
  }

  void parseIntoTokens(std::string const &arg,
                       std::vector<Token> &tokens) {
  ...
  Mode handleMode(std::size_t i, char c,
  ...
  Mode handleOpt(std::size_t i, char c,
                 std::string const &arg,
                 std::vector<Token> &tokens) {
    ...
    if(mode == ShortOpt)
      for(std::size_t j = 0; j < optName.size(); ++j)
        tokens.push_back(
            Token(Token::ShortOpt, optName.substr(j, 1)));
  ...
  Mode handlePositional(std::size_t i, char c,
                        std::string const &arg,
                        std::vector<Token> &tokens) {
    if(inQuotes ||
       std::string("\0", 1).find(c) == std::string::npos)
      return mode;
...
struct CommonArgProperties {
  ...
  std::string detail;
  std::string
      placeholder;  // Only value if boundField takes an arg
...
class CommandLine {
  struct Arg : CommonArgProperties<ConfigT>,
    ...
    Arg() {}
    Arg(Detail::BoundArgFunction<ConfigT> const
            &_boundField)
        : CommonArgProperties<ConfigT>(_boundField) {}

    using CommonArgProperties<
  ...
  friend void addOptName(Arg &arg,
  ...
  class ArgBuilder {
   public:
    ...
    template <typename C, typename M>
    void bind(void (C::*unaryMethod)(M),
    ...
    void bind(void (C::*unaryMethod)(bool)) {
      m_arg->boundField =
          new Detail::BoundUnaryMethod<C, bool>(
              unaryMethod);
    }

    // Bind a method that takes no arguments (will be called
    // if opt is present)
    template <typename C>
    ...
    // Bind a free function taking a single argument - the
    // object to operate on (no placeholder string required)
    template <typename C>
    ...
    // Bind a free function taking a single argument - the
    // object to operate on (requires a placeholder string)
    template <typename C, typename T>
    void bind(void (*binaryFunction)(C &, T),
              std::string const &placeholder) {
      m_arg->boundField =
          new Detail::BoundBinaryFunction<C, T>(
              binaryFunction);
      m_arg->placeholder = placeholder;
    }
 ...
 public:
  ...
  CommandLine &setThrowOnUnrecognisedTokens(
  ...
  void optUsage(
    ...
    for(it = itBegin; it != itEnd; ++it) {
      ...
      Detail::Text desc(it->description,
                        Detail::TextAttributes().setWidth(
                            width - maxWidth - 3));

      for(std::size_t i = 0;
          i < (std::max)(usage.size(), desc.size()); ++i) {
        std::string usageCol =
            i < usage.size() ? usage[i] : "";
        os << usageCol;
  ...
  void argSynopsis(std::ostream &os) const {
    for(int i = 1; i <= m_highestSpecifiedArgPosition;
        ++i) {
      if(i > 1) os << " ";
      ...
      if(it != m_positionalArgs.end())
        ...
        throw std::logic_error(
            "non consecutive positional arguments with no "
            "floating args");
    }
  ...
  void usage(std::ostream &os,
             std::string const &procName) const {
    validate();
    os << "usage:\n  " << procName << " ";
    argSynopsis(os);
    if(!m_options.empty()) {
  ...
  std::vector<Parser::Token> parseInto(
  ...
  std::vector<Parser::Token> populate(
    ...
    unusedTokens =
        populateFloatingArgs(unusedTokens, config);
    return unusedTokens;
  }
...
inline void abortAfterX(ConfigData &config, int x) {
  if(x < 1)
    throw std::runtime_error(
        "Value after -x or --abortAfter must be greater "
        "than zero");
  config.abortAfter = x;
}
inline void addTestOrTags(ConfigData &config,
                          std::string const &_testSpec) {
  config.testsOrTags.push_back(_testSpec);
}
inline void addReporterName(
    ConfigData &config, std::string const &_reporterName) {
  config.reporterNames.push_back(_reporterName);
}

inline void addWarning(ConfigData &config,
                       std::string const &_warning) {
  if(_warning == "NoAssertions")
    config.warnings = static_cast<WarnAbout::What>(
        config.warnings | WarnAbout::NoAssertions);
  else
    throw std::runtime_error("Unrecognised warning: '" +
                             _warning + "'");
}
inline void setOrder(ConfigData &config,
  ...
  if(startsWith("declared", order))
    ...
    throw std::runtime_error("Unrecognised ordering: '" +
                             order + "'");
}
inline void setRngSeed(ConfigData &config,
  ...
  if(seed == "time") {
    ...
    if(ss.fail())
      throw std::runtime_error(
          "Argment to --rng-seed should be the word 'time' "
          "or a number");
  }
}
...
inline void setShowDurations(ConfigData &config,
                             bool _showDurations) {
  config.showDurations = _showDurations
                             ? ShowDurations::Always
                             : ShowDurations::Never;
}
inline void setUseColour(ConfigData &config,
...
inline void loadTestNamesFromFile(
    ConfigData &config, std::string const &_filename) {
...
makeCommandLineParser() {
  ...
  //            .describe( "level of verbosity (0=no
  //            output)" )
  //            .shortOpt( "v")
  //            .longOpt( "verbosity" )
  //            .placeholder( "level" );
...
#define TBC_TEXT_FORMAT_CONSOLE_WIDTH \
  CATCH_CONFIG_CONSOLE_WIDTH
...
struct ReporterConfig {
  explicit ReporterConfig(
      Ptr<IConfig const> const &_fullConfig)
      : m_stream(&_fullConfig->stream()),
        m_fullConfig(_fullConfig) {}

  ReporterConfig(Ptr<IConfig const> const &_fullConfig,
                 std::ostream &_stream)
...
struct AssertionStats {
  ...
  AssertionStats &operator=(AssertionStats const &) =
      default;
...
struct IStreamingReporter : IShared {
  ...
  // Implementing class must also provide the following
  // static method:
  ...
  virtual void testGroupStarting(
  ...
  // The return value indicates if the messages buffer
  // should be cleared:
...
struct IReporterRegistry {
  typedef std::map<std::string, Ptr<IReporterFactory>>
      FactoryMap;
...
inline std::size_t listTestsNamesOnly(
    Config const &config) {
  TestSpec testSpec = config.testSpec();
  if(!config.testSpec().hasFilters())
    testSpec = TestSpecParser(ITagAliasRegistry::get())
  ...
  for(std::vector<TestCase>::const_iterator
    ...
    for(std::set<std::string>::const_iterator
      ...
      if(countIt == tagCounts.end())
        countIt = tagCounts
                      .insert(std::make_pair(lcaseTagName,
                                             TagInfo()))
                      .first;
...
inline std::size_t listReporters(
    Config const & /*config*/) {
  Catch::cout() << "Available reporters:\n";
  IReporterRegistry::FactoryMap const &factories =
      getRegistryHub().getReporterRegistry().getFactories();
  IReporterRegistry::FactoryMap::const_iterator
      itBegin = factories.begin(),
  ...
  for(it = itBegin; it != itEnd; ++it) {
    Text wrapper(it->second->getDescription(),
                 TextAttributes()
                     ...
                     .setWidth(CATCH_CONFIG_CONSOLE_WIDTH -
                               maxNameLen - 8));
    Catch::cout() << "  " << it->first << ":"
                  << std::string(
                         maxNameLen - it->first.size() + 2,
                         ' ')
                  << wrapper << "\n";
  }
  Catch::cout() << std::endl;
...
inline Option<std::size_t> list(Config const &config) {
  ...
  if(config.listTests())
    listedCount =
        listedCount.valueOr(0) + listTests(config);
  ...
  if(config.listReporters())
    listedCount =
        listedCount.valueOr(0) + listReporters(config);
  return listedCount;
}
...
struct ITracker : SharedImpl<> {
  ...
  virtual bool isOpen()
      const = 0;  // Started but not complete
...
class TrackerBase : public ITracker {
 ...
 public:
  ...
  virtual bool isSuccessfullyCompleted() const
      CATCH_OVERRIDE {
    return m_runState == CompletedSuccessfully;
  }
  ...
  virtual void addChild(Ptr<ITracker> const &child)
      CATCH_OVERRIDE {
    m_children.push_back(child);
  }

  virtual ITracker *findChild(std::string const &name)
      CATCH_OVERRIDE {
    Children::const_iterator it =
        std::find_if(m_children.begin(), m_children.end(),
                     TrackerHasName(name));
    return (it != m_children.end()) ? it->get()
                                    : CATCH_NULL;
  }
  virtual ITracker &parent() CATCH_OVERRIDE {
    assert(m_parent);  // Should always be non-null except
                       // for root
    return *m_parent;
  }
...
class SectionTracker : public TrackerBase {
 public:
  SectionTracker(std::string const &name,
  ...
  static SectionTracker &acquire(TrackerContext &ctx,
    ...
    if(ITracker *childTracker =
           currentTracker.findChild(name)) {
      assert(childTracker);
      assert(childTracker->isSectionTracker());
      section = static_cast<SectionTracker *>(childTracker);
    } else {
      section =
          new SectionTracker(name, ctx, &currentTracker);
 ...
 public:
  ...
  static IndexTracker &acquire(TrackerContext &ctx,
    ...
    if(ITracker *childTracker =
      ...
      tracker = new IndexTracker(name, ctx, &currentTracker,
                                 size);
...
inline ITracker &TrackerContext::startRun() {
  m_rootTracker =
      new SectionTracker("{root}", *this, CATCH_NULL);
  m_currentTracker = CATCH_NULL;
  m_runState = Executing;
...
class RunContext : public IResultCapture, public IRunner {
 ...
 public:
  explicit RunContext(
      Ptr<IConfig const> const &_config,
  ...
  Totals runTest(TestCase const &testCase) {
    ...
    do {
      ...
      do {
        ...
        m_testCaseTracker = &SectionTracker::acquire(
            m_trackerContext, testInfo.name);
        runCurrentTest(redirectedCout, redirectedCerr);
      } while(
          !m_testCaseTracker->isSuccessfullyCompleted() &&
          !aborting());
    }
    // !TBD: deprecated - this will be replaced by indexed
    // trackers
    ...
    m_reporter->testCaseEnded(
        TestCaseStats(testInfo, deltaTotals, redirectedCout,
 ...
 private:  // IResultCapture
  ...
  virtual void sectionEnded(SectionEndInfo const &endInfo) {
    ...
    bool missingAssertions =
        testForMissingAssertions(assertions);
    ...
    m_reporter->sectionEnded(SectionStats(
        endInfo.sectionInfo, assertions,
  ...
  virtual void handleFatalErrorCondition(
    ...
    // Recreate section for test case (as we will lose the
    // one that was in scope)
    TestCaseInfo const &testCaseInfo =
 ...
 private:
  ...
  void handleUnfinishedSections() {
    // If sections ended prematurely due to an exception we
    // stored their
    // infos here so we can tear them down outside the
    // unwind process.
...
Ptr<IStreamingReporter> createReporter(
    std::string const &reporterName,
    Ptr<Config> const &config) {
  Ptr<IStreamingReporter> reporter =
      getRegistryHub().getReporterRegistry().create(
          reporterName, config.get());
  if(!reporter) {
    std::ostringstream oss;
...
Ptr<IStreamingReporter> makeReporter(
    Ptr<Config> const &config) {
  ...
  for(std::vector<std::string>::const_iterator
      ...
      it != itEnd; ++it)
    reporter =
        addReporter(reporter, createReporter(*it, config));
  return reporter;
}
Ptr<IStreamingReporter> addListeners(
  ...
  for(IReporterRegistry::Listeners::const_iterator
          ...
          itEnd = listeners.end();
      it != itEnd; ++it)
    reporters = addReporter(
        reporters, (*it)->create(ReporterConfig(config)));
  return reporters;
}
...
Totals runTests(Ptr<Config> const &config) {
  ...
  std::vector<TestCase> const &allTestCases =
      getAllTestCasesSorted(*iconfig);
  for(std::vector<TestCase>::const_iterator
          it = allTestCases.begin(),
 ...
 public:
  ...
  Session() : m_cli(makeCommandLineParser()) {
    if(alreadyInstantiated) {
      std::string msg =
          "Only one instance of Catch::Session can ever be "
          "used";
  ...
  void showHelp(std::string const &processName) {
    ...
    Catch::cout() << "For more detail usage please see the "
                     "project docs\n"
                  << std::endl;
  }
  ...
  int applyCommandLine(
    ...
    try {
      m_cli.setThrowOnUnrecognisedTokens(
          unusedOptionBehaviour == OnUnusedOptions::Fail);
      m_unusedTokens = m_cli.parseInto(
          Clara::argsToVector(argc, argv), m_configData);
  ...
  int run() {
    ...
    try {
      ...
      return static_cast<int>(
          runTests(m_config).assertions.failed);
    } catch(std::exception &ex) {
  ...
  std::vector<Clara::Parser::Token> const &unusedTokens()
      const {
...
void enforceNoDuplicateTestCases(
  ...
  for(std::vector<TestCase>::const_iterator
          ...
          itEnd = functions.end();
      it != itEnd; ++it) {
    std::pair<std::set<TestCase>::const_iterator, bool>
        prev = seenFunctions.insert(*it);
    if(!prev.second) {
      std::ostringstream ss;

      ss << Colour(Colour::Red) << "error: TEST_CASE( \""
...
class ReporterRegistry : public IReporterRegistry {
 public:
  ...
  virtual FactoryMap const &getFactories() const
      CATCH_OVERRIDE {
    return m_factories;
  }
  virtual Listeners const &getListeners() const
      CATCH_OVERRIDE {
    return m_listeners;
  }
...
#ifndef CATCH_CONFIG_NOSTDOUT  // If you #define this you
...
class Context : public IMutableContext {
 ...
 public:  // IContext
  ...
  virtual size_t getGeneratorIndex(
      std::string const &fileInfo, size_t totalSize) {
 ...
 private:
  IGeneratorsForTest *findGeneratorsForCurrentTest() {
    ...
    std::map<std::string,
             IGeneratorsForTest *>::const_iterator it =
        m_generatorsByTestName.find(testName);
    return it != m_generatorsByTestName.end() ? it->second
                                              : CATCH_NULL;
  }
 ...
 private:
  ...
  std::map<std::string, IGeneratorsForTest *>
      m_generatorsByTestName;
};
...
IColourImpl *platformColourInstance() {
  ...
  if(colourMode == UseColour::Auto)
    colourMode = !isDebuggerActive() ? UseColour::Yes
                                     : UseColour::No;
...
IColourImpl *platformColourInstance() {
  ...
  if(colourMode == UseColour::Auto)
    colourMode =
        (!isDebuggerActive() && isatty(STDOUT_FILENO))
            ? UseColour::Yes
            : UseColour::No;
  return colourMode == UseColour::Yes
...
class GeneratorsForTest : public IGeneratorsForTest {
 public:
  ...
  IGeneratorInfo &getGeneratorInfo(
      std::string const &fileInfo, std::size_t size) {
    std::map<std::string, IGeneratorInfo *>::const_iterator
        it = m_generatorsByName.find(fileInfo);
    if(it == m_generatorsByName.end()) {
      IGeneratorInfo *info = new GeneratorInfo(size);
      m_generatorsByName.insert(
          std::make_pair(fileInfo, info));
 ...
 private:
  std::map<std::string, IGeneratorInfo *>
      m_generatorsByName;
...
inline TestCaseInfo::SpecialProperties parseSpecialTag(
...
inline void enforceNotReservedTag(
    std::string const &tag,
    SourceLineInfo const &_lineInfo) {
  if(isReservedTag(tag)) {
    {
      Colour colourGuard(Colour::Red);
      Catch::cerr() << "Tag name [" << tag
                    << "] not allowed.\n"
                    << "Tag names starting with non "
                       "alpha-numeric characters are "
                       "reserved\n";
    }
    {
...
TestCase makeTestCase(ITestCase *_testCase,
  ...
  TestCaseInfo info(_name, _className, desc, tags,
                    _lineInfo);
...
TestCaseInfo::TestCaseInfo(
    std::string const &_name, std::string const &_className,
    std::string const &_description,
    std::set<std::string> const &_tags,
    SourceLineInfo const &_lineInfo)
    : name(_name),
...
struct IReporter : IShared {
  ...
  virtual void NoAssertionsInSection(
      std::string const &sectionName) = 0;
  virtual void NoAssertionsInTestCase(
      std::string const &testName) = 0;
  virtual void Aborted() = 0;
  virtual void Result(AssertionResult const &result) = 0;
};

class LegacyReporterAdapter
...
ReporterPreferences LegacyReporterAdapter::getPreferences()
    const {
...
void LegacyReporterAdapter::noMatchingTestCases(
    std::string const &) {}
void LegacyReporterAdapter::testRunStarting(
    TestRunInfo const &) {
  m_legacyReporter->StartTesting();
}
void LegacyReporterAdapter::testGroupStarting(
    GroupInfo const &groupInfo) {
  m_legacyReporter->StartGroup(groupInfo.name);
}
void LegacyReporterAdapter::testCaseStarting(
    TestCaseInfo const &testInfo) {
  m_legacyReporter->StartTestCase(testInfo);
}
void LegacyReporterAdapter::sectionStarting(
    SectionInfo const &sectionInfo) {
  m_legacyReporter->StartSection(sectionInfo.name,
                                 sectionInfo.description);
}
void LegacyReporterAdapter::assertionStarting(
    AssertionInfo const &) {
...
void LegacyReporterAdapter::sectionEnded(
    SectionStats const &sectionStats) {
  if(sectionStats.missingAssertions)
    m_legacyReporter->NoAssertionsInSection(
        sectionStats.sectionInfo.name);
  m_legacyReporter->EndSection(
      sectionStats.sectionInfo.name,
      sectionStats.assertions);
}
void LegacyReporterAdapter::testCaseEnded(
    TestCaseStats const &testCaseStats) {
  m_legacyReporter->EndTestCase(
      testCaseStats.testInfo, testCaseStats.totals,
...
void LegacyReporterAdapter::testRunEnded(
    TestRunStats const &testRunStats) {
...
uint64_t getCurrentTicks() {
  ...
  uint64_t t;
  QueryPerformanceCounter(
      reinterpret_cast<LARGE_INTEGER *>(&t));
...
unsigned int Timer::getElapsedMicroseconds() const {
  return static_cast<unsigned int>(getCurrentTicks() -
                                   m_ticks);
}
...
SourceLineInfo::SourceLineInfo(char const *_file,
                               std::size_t _line)
...
bool SourceLineInfo::operator==(
    SourceLineInfo const &other) const {
...
void throwLogicError(std::string const &message,
                     SourceLineInfo const &locationInfo) {
  std::ostringstream oss;
  oss << locationInfo << ": Internal Catch error: '"
      << message << "'";
...
// This indicates whether the section should be executed or
// not
...
bool isDebuggerActive() {
  ...
  if(sysctl(mib, sizeof(mib) / sizeof(*mib), &info, &size,
            CATCH_NULL, 0) != 0) {
    Catch::cerr() << "\n** Call to sysctl failed - unable "
                     "to determine if debugger is active "
                     "**\n"
                  << std::endl;
    return false;
  }
...
#elif defined(_MSC_VER)
extern "C" __declspec(
    dllimport) int __stdcall IsDebuggerPresent();
namespace Catch {
...
#ifdef CATCH_PLATFORM_WINDOWS
extern "C" __declspec(
    dllimport) void __stdcall OutputDebugStringA(const char
                                                     *);
namespace Catch {
void writeToDebugConsole(std::string const &text) {
...
ResultBuilder::ResultBuilder(
    char const *macroName, SourceLineInfo const &lineInfo,
    char const *capturedExpression,
    ResultDisposition::Flags resultDisposition,
    char const *secondArg)
...
ResultBuilder &ResultBuilder::setResultType(
...
ResultBuilder &ResultBuilder::setLhs(
    std::string const &lhs) {
  m_exprComponents.lhs = lhs;
  return *this;
}
ResultBuilder &ResultBuilder::setRhs(
...
void ResultBuilder::captureResult(
    ResultWas::OfType resultType) {
...
void ResultBuilder::captureExpectedException(
  ...
  if(expectedMessage.empty())
    ...
    captureExpectedException(
        Matchers::Equals(expectedMessage));
}
...
void ResultBuilder::handleResult(
    AssertionResult const &result) {
...
std::string ResultBuilder::reconstructExpression() const {
  if(m_exprComponents.op == "")
    ...
    if(m_exprComponents.lhs.size() +
               m_exprComponents.rhs.size() <
           40 &&
    ...
    return "{can't expand - use " +
           m_assertionInfo.macroName + "_FALSE( " +
           m_assertionInfo.capturedExpression.substr(1) +
...
Option<TagAlias> TagAliasRegistry::find(
    std::string const &alias) const {
...
class MultipleReporters
 ...
 public:  // IStreamingReporter
  ...
  virtual void testRunStarting(
      TestRunInfo const &testRunInfo) CATCH_OVERRIDE {
    for(Reporters::const_iterator it = m_reporters.begin(),
                                  itEnd = m_reporters.end();
        it != itEnd; ++it)
  ...
  // should be cleared:
  virtual bool assertionEnded(
      AssertionStats const &assertionStats) CATCH_OVERRIDE {
  ...
  virtual void sectionEnded(
      SectionStats const &sectionStats) CATCH_OVERRIDE {
    for(Reporters::const_iterator it = m_reporters.begin(),
                                  itEnd = m_reporters.end();
        it != itEnd; ++it)
  ...
  virtual void testCaseEnded(
      TestCaseStats const &testCaseStats) CATCH_OVERRIDE {
    for(Reporters::const_iterator it = m_reporters.begin(),
                                  itEnd = m_reporters.end();
        it != itEnd; ++it)
  ...
  virtual void testGroupEnded(
      TestGroupStats const &testGroupStats) CATCH_OVERRIDE {
    for(Reporters::const_iterator it = m_reporters.begin(),
                                  itEnd = m_reporters.end();
        it != itEnd; ++it)
  ...
  virtual void testRunEnded(
      TestRunStats const &testRunStats) CATCH_OVERRIDE {
    for(Reporters::const_iterator it = m_reporters.begin(),
                                  itEnd = m_reporters.end();
        it != itEnd; ++it)
...
struct StreamingReporterBase
  ...
  virtual void testRunStarting(
      TestRunInfo const &_testRunInfo) CATCH_OVERRIDE {
    currentTestRunInfo = _testRunInfo;
  }
  virtual void testGroupStarting(
      GroupInfo const &_groupInfo) CATCH_OVERRIDE {
...
class LegacyReporterRegistrar {
  class ReporterFactory : public IReporterFactory {
    virtual IStreamingReporter *create(
        ReporterConfig const &config) const {
...
class ReporterRegistrar {
  class ReporterFactory
    ...
    // *** Please Note ***:
    // - If you end up here looking at a compiler error
    // because it's trying to register
    // your custom reporter class be aware that the native
    // reporter interface has changed
    // to IStreamingReporter. The "legacy" interface,
    // IReporter, is still supported via
    // an adapter. Just use REGISTER_LEGACY_REPORTER to take
    // advantage of the adapter.
    // However please consider updating to the new interface
    // as the old one is now
    // deprecated and will probably be removed quite soon!
    // Please contact me via github if you have any
    // questions at all about this.
    // In fact, ideally, please contact me anyway to let me
    // know you've hit this - as I have
...
class XmlEncode {
 public:
  ...
  void encodeTo(std::ostream &os) const {
    // Apostrophe escaping not necessary if we always use "
    // to write attributes
    ...
    for(std::size_t i = 0; i < m_str.size(); ++i) {
      ...
      switch(c) {
        ...
        default:
          // Escape control chars - based on contribution by
          // @espenalb in PR #465 and
          // by @mrpi PR #588
...
class XmlReporter : public StreamingReporterBase {
 ...
 public:  // StreamingReporterBase
  ...
  virtual void testGroupStarting(GroupInfo const &groupInfo)
    ...
    m_xml.startElement("Group").writeAttribute(
        "name", groupInfo.name);
  }
  ...
  virtual bool assertionEnded(
    ...
    // Drop out if result was successful but we're not
    // printing them.
  ...
  virtual void testGroupEnded(
    ...
    // TODO: Check testGroupStats.aborting and act
    // accordingly.
    m_xml.scopedElement("OverallResults")
        .writeAttribute(
            "successes",
...
class JunitReporter : public CumulativeReporterBase {
 public:
  ...
  static std::string getDescription() {
    return "Reports test results in an XML format that "
           "looks like Ant's junitreport target";
  }

  virtual void noMatchingTestCases(
      std::string const & /*spec*/) CATCH_OVERRIDE {}
  ...
  void writeTestCase(TestCaseNode const &testCaseNode) {
    ...
    // All test cases have exactly one section - which
    // represents the
    // test case itself. That section may have 0-n nested
    // sections
...
struct ConsoleReporter : StreamingReporterBase {
  ...
  virtual bool assertionEnded(
    ...
    // Drop out if result was successful and we're not
    // printing those
    if(!m_config->includeSuccessfulResults() &&
       result.isOk()) {
  ...
  virtual void sectionEnded(
      SectionStats const &_sectionStats) CATCH_OVERRIDE {
  ...
  virtual void testCaseEnded(
      TestCaseStats const &_testCaseStats) CATCH_OVERRIDE {
  ...
  virtual void testRunEnded(
      TestRunStats const &_testRunStats) CATCH_OVERRIDE {
 ...
 private:
  ...
  // if string has a : in first line will set indent to
  // follow it on
  // subsequent lines
  ...
  struct SummaryColumn {
    SummaryColumn(std::string const &_label,
                  Colour::Code _colour)
  ...
  void printTotals(Totals const &totals) {
    if(totals.testCases.total() == 0) {
      ...
      stream << Colour(Colour::ResultSuccess)
             << "All tests passed";
      stream << " (" << pluralise(totals.assertions.passed,
                                  "assertion")
             << " in " << pluralise(totals.testCases.passed,
                                    "test case")
             << ")"
             << "\n";
    } else {
      ...
      columns.push_back(
          SummaryColumn("failed", Colour::ResultError)
  ...
  static std::size_t makeRatio(std::size_t number,
                               std::size_t total) {
    std::size_t ratio =
        total > 0
            ? CATCH_CONFIG_CONSOLE_WIDTH * number / total
            : 0;
    return (ratio == 0 && number > 0) ? 1 : ratio;
  }
  static std::size_t &findMax(std::size_t &i,
                              std::size_t &j,
                              std::size_t &k) {
    if(i > j && i > k)
      return i;
  ...
  void printTotalsDivider(Totals const &totals) {
    if(totals.testCases.total() > 0) {
      std::size_t failedRatio =
          makeRatio(totals.testCases.failed,
                    totals.testCases.total());
      std::size_t failedButOkRatio =
          makeRatio(totals.testCases.failedButOk,
                    totals.testCases.total());
      std::size_t passedRatio =
          makeRatio(totals.testCases.passed,
                    totals.testCases.total());
...
struct CompactReporter : StreamingReporterBase {
  ...
  static std::string getDescription() {
    return "Reports test results on a single line, "
           "suitable for IDEs";
  }
 ...
 private:
  class AssertionPrinter {
   ...
   private:
    ...
    void printRemainingMessages(
      ...
      // using messages.end() directly yields compilation
      // error:
  ...
  // - white: No tests ran.
  // -   red: Failed [both/all] N test cases, failed
  // [both/all] M assertions.
  // - white: Passed [both/all] N test cases (no
  // assertions).
  // -   red: Failed N tests cases, failed M assertions.
  // - green: Passed [both/all] N tests cases with M
  // assertions.
...
namespace Catch {
// These are all here to avoid warnings about not having any
// out of line
// virtual methods
...
int main(int argc, char *const argv[]) {
  ...
  int result =
      Catch::Session().run(argc, (char *const *)argv);
...
// If this config identifier is defined then all CATCH
// macros are prefixed with CATCH_
...
#define CATCH_REQUIRE_FALSE(expr)                    \
  INTERNAL_CATCH_TEST(                               \
      expr, Catch::ResultDisposition::Normal |       \
...
#define CATCH_REQUIRE_THROWS_WITH(expr, matcher)       \
  INTERNAL_CATCH_THROWS(                               \
      expr, Catch::ResultDisposition::Normal, matcher, \
...
#define CATCH_CHECK(expr)                                \
  INTERNAL_CATCH_TEST(                                   \
      expr, Catch::ResultDisposition::ContinueOnFailure, \
      "CATCH_CHECK")
...
#define CATCH_INFO(msg) \
  INTERNAL_CATCH_INFO(msg, "CATCH_INFO")
#define CATCH_WARN(msg)                            \
  INTERNAL_CATCH_MSG(                              \
      Catch::ResultWas::Warning,                   \
      Catch::ResultDisposition::ContinueOnFailure, \
      "CATCH_WARN", msg)
#define CATCH_SCOPED_INFO(msg) \
  INTERNAL_CATCH_INFO(msg, "CATCH_INFO")
...
#define CATCH_TEST_CASE_METHOD(className, name,    \
                               description)        \
  INTERNAL_CATCH_TEST_CASE_METHOD(className, name, \
                                  description)
#define CATCH_METHOD_AS_TEST_CASE(method, name,    \
                                  description)     \
  INTERNAL_CATCH_METHOD_AS_TEST_CASE(method, name, \
                                     description)
#define CATCH_REGISTER_TEST_CASE(function, name,   \
                                 description)      \
  INTERNAL_CATCH_REGISTER_TESTCASE(function, name, \
                                   description)
...
#define CATCH_REGISTER_LEGACY_REPORTER(name, reporterType) \
  INTERNAL_CATCH_REGISTER_LEGACY_REPORTER(name,            \
                                          reporterType)
...
#else
#define CATCH_SCENARIO(name, tags) \
  CATCH_TEST_CASE("Scenario: " name, tags)
#define CATCH_SCENARIO_METHOD(className, name, tags) \
  INTERNAL_CATCH_TEST_CASE_METHOD(className,         \
                                  "Scenario: " name, tags)
#endif
#define CATCH_GIVEN(desc) \
  CATCH_SECTION(std::string("Given: ") + desc, "")
#define CATCH_WHEN(desc) \
  CATCH_SECTION(std::string(" When: ") + desc, "")
#define CATCH_AND_WHEN(desc) \
  CATCH_SECTION(std::string("  And: ") + desc, "")
#define CATCH_THEN(desc) \
  CATCH_SECTION(std::string(" Then: ") + desc, "")
...
#define REQUIRE(expr)  \
  INTERNAL_CATCH_TEST( \
      expr, Catch::ResultDisposition::Normal, "REQUIRE")
...
#define CAPTURE(msg) \
  INTERNAL_CATCH_INFO(#msg " := " << msg, "CAPTURE")
...
#define REGISTER_TEST_CASE(method, name, description) \
  INTERNAL_CATCH_REGISTER_TESTCASE(method, name,      \
                                   description)
...
#endif
#define GIVEN(desc) \
  SECTION(std::string("   Given: ") + desc, "")
#define WHEN(desc) \
  SECTION(std::string("    When: ") + desc, "")
#define AND_WHEN(desc) \
  SECTION(std::string("And when: ") + desc, "")
#define THEN(desc) \
  SECTION(std::string("    Then: ") + desc, "")
#define AND_THEN(desc) \
  SECTION(std::string("     And: ") + desc, "")
