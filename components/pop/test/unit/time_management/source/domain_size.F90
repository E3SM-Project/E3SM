...
!
! !REVISION HISTORY:
!  SVN:$Id: gx3v5_domain_size.F90 12674 2008-10-31 22:21:32Z njn01 $
...
! !DEFINED PARAMETERS:
   ...
   integer (int_kind), parameter, public ::  &  ! model size parameters
      nx_global =  30 ,&! extent of horizontal axis in i direction
      ny_global =  31 ,&! extent of horizontal axis in j direction
      km =  3          ,&! number of vertical levels
      nt =  2            ! total number of tracers

   integer (int_kind), parameter, public :: &
      block_size_x = 4, &! size of block in 1st horizontal dimension
      block_size_y = 4   ! size of block in 2nd horizontal dimension
   ...
   integer (int_kind), parameter, public :: &
      max_blocks_clinic = 65,  &! max number of blocks per processor
      max_blocks_tropic = 65    !   in each distribution
