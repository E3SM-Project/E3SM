...
!       NSEAL  = 1 + (NSEA-IAPROC)/NAPROC
        ...
        WRITE(NDSE,*) "Number of points for this processor :", IAPROC,NXL(IAPROC)
              DISTSWITCH = .FALSE.
        DISTSWITCH = .TRUE.
          CALL MPI_TYPE_VECTOR ( TIDESIZ, 1, 1, MPI_REAL, TIDALSTUFF, IERR_MPI )
          CALL MPI_TYPE_COMMIT ( TIDALSTUFF, IERR_MPI )
!
...
!
       CALL MPI_SEND ( TIDALSTUFFA, 1, TIDALSTUFF, NAPOUT-1, 0,  &
                       MPI_COMM_WORLD, IERR_MPI )
           CALL MPI_RECV ( TIDALSTUFFA, 1, TIDALSTUFF, JPC-1, MPI_ANY_TAG,  &
                             MPI_COMM_WORLD, STAT_MPI, IERR_MPI )
 
        END DO ! IY=1,NY
 
       CALL MPI_TYPE_FREE(TIDALSTUFF, IERR_MPI)
