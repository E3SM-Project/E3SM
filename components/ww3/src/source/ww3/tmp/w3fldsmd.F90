...
!
 ...
 1009 FORMAT (/' *** WAVEWATCH III ERROR IN W3FLDO : '/         &
         '     INCOMPATIBLE GRID DATA : ',3(1X,I4)/             &
         '                  SHOULD BE : ',3(1X,I4)/)
...
!                        'LEV', 'CUR', 'WND', 'WNS', 'ICE', 'ISI', and 'DTn'.
!       NDS     Int.  I  Dataset number for fields file.
!       NDST    Int.  I  Dataset number for test output.
!       NDSE    Int.  I  Dataset number for error output.
!                        (No output if NDSE < 0).
...
! Formats
!
 1001 FORMAT (/' *** WAVEWATCH III ERROR IN W3FLDTIDE : '/         &
               '     ILLEGAL INXOUT STRING : ',A/)
 1002 FORMAT (/' *** WAVEWATCH III ERROR IN W3FLDTIDE : '/         &
               '     ILLEGAL FIELD ID STRING : ',A/)
 1004 FORMAT (/' *** WAVEWATCH III ERROR IN W3FLDTIDE : '/         &
               '     ERROR IN WRITING TO ',A,' FILE, IOSTAT =',I6/)
 1005 FORMAT (/' *** WAVEWATCH III ERROR IN W3FLDTIDE : '/         &
               '     ERROR IN READING ',A,' FILE, IOSTAT =',I6/)
 
 1006 FORMAT (/' *** WAVEWATCH III ERROR IN W3FLDTIDE : '/         &
               '     PREMATURE END OF ',A,' FILE'/)
!/
!/ End of W3FLDO  ---------------------------------------------------- /
!/
...
!
      ...
      IF ( IDFLD.NE.'IC1' .AND. IDFLD.NE.'IC2' .AND.                  &
           ...
           IDFLD.NE.'LEV' .AND. IDFLD.NE.'CUR' .AND.                  &
           IDFLD.NE.'WND' .AND. IDFLD.NE.'WNS' .AND.                  &
           IDFLD.NE.'ICE' .AND. IDFLD.NE.'ISI'  )    GOTO 802
!
! Set internal variables --------------------------------------------- *
!
      WRITE  = INXOUT .EQ. 'WRITE'
...
!
      PLAT => ALAT
      PLON => ALON
      GSU = W3GSUC( .TRUE., FLAGLL, ICLO, MXI, MYI, PLON, PLAT )
!
! 2.  Loop over output grid ------------------------------------------ *
!
      DO 500, IY=1, NY
...
!/
      ...
      INTEGER, INTENT(OUT)    :: IERR
      REAL, INTENT(INOUT)     :: HA(NHM,-7:4), HD(NHM,-7:4), HS(NHM,-7:4),  &
...
!
!old: IF ( J.LE.0 .OR. J .GT.3 ) GOTO 801
      IF ( J.LT.-7 .OR. J .GT.3 ) GOTO 801
      FLFRST = TFN(1) .EQ. -1
!
! Loop over times / fields ========================================== *
!
...
!
        DTTST  = DSEC21 ( T0 , TFN )
        IF ( J.GE.(-7).AND.(J.LE.1).AND.FLFRST .AND. DTTST.EQ.0. ) EXIT
        IF ( DTTST .GT. 0. ) EXIT
        END DO
!
