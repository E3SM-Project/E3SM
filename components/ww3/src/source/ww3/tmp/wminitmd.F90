...
!/
      ...
      USE W3IDATMD, ONLY: INFLAGS1, INFLAGS2, INPUTS, IINIT, IIDATA,  &
                          FLCUR, FLWIND, JFIRST
...
!
            ...
            CALL MPI_BCAST ( YGRD, NX*NY, MPI_REAL, 0,           &
                             MPI_COMM_BCT, IERR_MPI )
            IF ( MPI_COMM_GRD .EQ. MPI_COMM_NULL )               &
                 GSU = W3GSUC( .FALSE., FLAGLL, ICLOSE, NX, NY,  &
                               XGRD, YGRD )
...
!
            ...
            CALL MPI_BCAST ( NBI  , 1, MPI_INTEGER, 0,           &
                             MPI_COMM_BCT, IERR_MPI )
!
            CALL MPI_BCAST ( FLOUT,  7, MPI_LOGICAL, 0,          &
                             MPI_COMM_BCT, IERR_MPI )
            CALL MPI_BCAST ( DTOUT , 7, MPI_REAL, 0,             &
                             MPI_COMM_BCT, IERR_MPI )
            CALL MPI_BCAST ( TONEXT,14, MPI_INTEGER, 0,          &
                             MPI_COMM_BCT, IERR_MPI )
            CALL MPI_BCAST ( TOLAST,14, MPI_INTEGER, 0,          &
                             MPI_COMM_BCT, IERR_MPI )
!
          END IF
        END DO
...
!/
      ...
      USE W3WDATMD, ONLY: TIME
      USE W3ADATMD, ONLY: WADATS
      USE W3IDATMD, ONLY: INFLAGS1, INPUTS, IINIT, IIDATA, FLCUR,     &
                          FLWIND, JFIRST, INFLAGS2
      USE W3ODATMD, ONLY: NOGRP, NGRPP, IDOUT, FLOUT, TONEXT, FLBPI,  &
                          FLBPO, NFBPO, NBI, NDS, IAPROC, NAPLOG,     &
                          NAPFLD, NAPPNT, NAPTRK, NAPRST, NAPBPT,     &
                          NAPPRT, NAPROC, FNMPRE, OUTPTS, NDST, NDSE, &
...
!/ Local parameters
!/
      INTEGER                 :: MDSE2, IERR, I,J,K, NMOVE, TTIME(2), &
                                 ILOOP, SCRATCH, RNKMIN,              &
                                 RNKMAX, RNKTMP, GRPMIN, GRPMAX, II,  &
                                 NDSREC, NDSFND, NPTS, JJ, IP1, IPN,  &
                                 MPI_COMM_LOC, NMPSC2, JJJ, TOUT(2),  &
                                 TLST(2), NCPROC, NPOUTT, NAPLOC,     &
...
!
      type(domain_def_t)                      :: domain_def
      type(input_grid_t), allocatable         :: input_grid(:)
      type(model_grid_t), allocatable         :: model_grid(:)
      type(output_type_t), allocatable        :: output_type(:)
      type(output_date_t), allocatable        :: output_date(:)
      type(homogeneous_input_t), allocatable  :: homogeneous_input(:)
!
      TYPE OT2TPE
        INTEGER                    :: NPTS
        REAL, POINTER              :: X(:), Y(:)
...
!
      ...
      CALL namlmultidef (MPI_COMM, MDSI, trim(FNMPRE)//IFNAME, domain_def, ierr)
      ALLOCATE(input_grid(domain_def%nrinp))
      ALLOCATE(model_grid(domain_def%nrgrd))
      ALLOCATE(output_type(domain_def%nrgrd))
      ALLOCATE(output_date(domain_def%nrgrd))
      ALLOCATE(homogeneous_input(domain_def%nmove))
      CALL namlmulticonf (MPI_COMM, MDSI, trim(FNMPRE)//IFNAME, &
                          domain_def, input_grid, model_grid, output_type, &
                          output_date, homogeneous_input, ierr)
      IF (ierr.ne.0) THEN
...
!     Processor set as in W3INIT to minimize communication in WMIOPO
!
      NRINP = domain_def%nrinp
      NRGRD = domain_def%nrgrd
      NMOVE = domain_def%nmove
      UNIPTS = domain_def%unipts
      IOSTYP = domain_def%iostyp
      UPPROC = domain_def%upproc
      PSHARE = domain_def%pshare
...
!
        CALL W3SETI ( -I, MDSE, MDST )
        INFLAGS1 = .FALSE.
        MNAMES(-I) = input_grid(I)%name
        INFLAGS1(-7) = input_grid(I)%forcing%ice_param1
        INFLAGS1(-6) = input_grid(I)%forcing%ice_param2
        INFLAGS1(-5) = input_grid(I)%forcing%ice_param3
        INFLAGS1(-4) = input_grid(I)%forcing%ice_param4
        INFLAGS1(-3) = input_grid(I)%forcing%ice_param5
        INFLAGS1(-2) = input_grid(I)%forcing%mud_density
        INFLAGS1(-1) = input_grid(I)%forcing%mud_thickness
        INFLAGS1(0) = input_grid(I)%forcing%mud_viscosity
        INFLAGS1(1) = input_grid(I)%forcing%water_levels
        INFLAGS1(2) = input_grid(I)%forcing%currents
        INFLAGS1(3) = input_grid(I)%forcing%winds
        INFLAGS1(4) = input_grid(I)%forcing%ice_conc
        INFLAGS1(5) = input_grid(I)%assim%mean
        INFLAGS1(6) = input_grid(I)%assim%spec1d
        INFLAGS1(7) = input_grid(I)%assim%spec2d
      END DO
!
! 3.a.2 Unified point output grid.
!
...
!
          ...
          NDSE   = MDSE
!
          MNAMES(0) = output_type(1)%point%name
!
          IF ( IOSTYP .LE. 1 ) THEN
              NMPUPT = MAX(1,NMPROC-2)
            ELSE
...
!
      DO I=1,NRGRD
        MNAMES(NRGRD+I) = model_grid(I)%name
        INAMES(NRGRD+I,-7) = model_grid(I)%forcing%ice_param1
        INAMES(NRGRD+I,-6) = model_grid(I)%forcing%ice_param2
        INAMES(NRGRD+I,-5) = model_grid(I)%forcing%ice_param3
        INAMES(NRGRD+I,-4) = model_grid(I)%forcing%ice_param4
        INAMES(NRGRD+I,-3) = model_grid(I)%forcing%ice_param5
        INAMES(NRGRD+I,-2) = model_grid(I)%forcing%mud_density
        INAMES(NRGRD+I,-1) = model_grid(I)%forcing%mud_thickness
        INAMES(NRGRD+I,0) = model_grid(I)%forcing%mud_viscosity
        INAMES(NRGRD+I,1) = model_grid(I)%forcing%water_levels
        INAMES(NRGRD+I,2) = model_grid(I)%forcing%currents
        INAMES(NRGRD+I,3) = model_grid(I)%forcing%winds
        INAMES(NRGRD+I,4) = model_grid(I)%forcing%ice_conc
        INAMES(NRGRD+I,5) = model_grid(I)%assim%mean
        INAMES(NRGRD+I,6) = model_grid(I)%assim%spec1d
        INAMES(NRGRD+I,7) = model_grid(I)%assim%spec2d
        TMPRNK(NRGRD+I) = model_grid(I)%resource%rank_id
        TMPGRP(NRGRD+I) = model_grid(I)%resource%group_id
        RP1(NRGRD+I) = model_grid(I)%resource%comm_frac(1)
        RPN(NRGRD+I) = model_grid(I)%resource%comm_frac(2)
        BCDTMP(NRGRD+I) = model_grid(i)%resource%bound_flag
!
...
!
! 3.b Assign input file unit numbers
!     Also, set INFLAGS2. This is the initial value of INFLAGS1.
!
      DO I=-NRINP, NRGRD
        IF ( I .EQ. 0 ) CYCLE
        CALL W3SETI ( I, MDSE, MDST )
...
!
      IF ( MDSS.NE.MDSO .AND. NMPSCR.EQ.IMPROC ) WRITE (MDSS,940)
!
      READ(domain_def%start, *) STIME
      READ(domain_def%stop, *) ETIME
      CALL STME21 ( STIME , DTME21 )
      IF ( MDSS.NE.MDSO .AND. NMPSCR.EQ.IMPROC ) WRITE (MDSS,941) DTME21
      CALL STME21 ( ETIME , DTME21 )
      IF ( MDSS.NE.MDSO .AND. NMPSCR.EQ.IMPROC ) WRITE (MDSS,942) DTME21
...
!
      IF ( MDSS.NE.MDSO .AND. NMPSCR.EQ.IMPROC ) WRITE (MDSS,943)
!
      FLGHG1 = domain_def%flghg1
      FLGHG2 = domain_def%flghg2
      FLGHG2 = FLGHG1 .AND. FLGHG2
!
      IF ( MDSS.NE.MDSO .AND. NMPSCR.EQ.IMPROC ) THEN
          IF ( FLGHG1 ) THEN
...
!
      DO I=1, NRGRD
        ...
        READ(output_date(I)%field%start, *)   ODAT(1,I), ODAT(2,I)
        READ(output_date(I)%field%stride, *)  ODAT(3,I)
        READ(output_date(I)%field%stop, *)    ODAT(4,I), ODAT(5,I)
        READ(output_date(I)%point%start, *)   ODAT(6,I), ODAT(7,I)
        READ(output_date(I)%point%stride, *)  ODAT(8,I)
        READ(output_date(I)%point%stop, *)    ODAT(9,I), ODAT(10,I)
        READ(output_date(I)%track%start, *)   ODAT(11,I), ODAT(12,I)
        READ(output_date(I)%track%stride, *)  ODAT(13,I)
        READ(output_date(I)%track%stop, *)    ODAT(14,I), ODAT(15,I)
        READ(output_date(I)%restart%start, *)   ODAT(16,I), ODAT(17,I)
        READ(output_date(I)%restart%stride, *)  ODAT(18,I)
        READ(output_date(I)%restart%stop, *)    ODAT(19,I), ODAT(20,I)
        READ(output_date(I)%boundary%start, *)   ODAT(21,I), ODAT(22,I)
        READ(output_date(I)%boundary%stride, *)  ODAT(23,I)
        READ(output_date(I)%boundary%stop, *)    ODAT(24,I), ODAT(25,I)
        READ(output_date(I)%partition%start, *)   ODAT(26,I), ODAT(27,I)
        READ(output_date(I)%partition%stride, *)  ODAT(28,I)
        READ(output_date(I)%partition%stop, *)    ODAT(29,I), ODAT(30,I)
...
!
  ...
  !
                ...
                FLDOUT = output_type(I)%field%list
  ...
  !
                ...
                IF ( trim(output_type(I)%point%file).EQ.'unset' ) THEN
...
!
                IF ( UNIPTS .AND. I.GE.2 ) THEN
                  DO K=1,I-1
                    IF ( output_type(K)%point%file.NE.output_type(I)%point%file ) GOTO 2053
                  END DO
                END IF
                OPEN (MDSI, file=trim(FNMPRE)//trim(output_type(I)%point%file), &
...
!
                  IF ( ILOOP.EQ.2) THEN
                    IF ( SIZE(OT2(I)%X).NE.OT2(I)%NPTS .AND. OT2(I)%NPTS.GT.0 ) THEN
                      ALLOCATE ( OT2(I)%X(OT2(I)%NPTS),               &
                                 OT2(I)%Y(OT2(I)%NPTS),               &
                                 OT2(I)%PNAMES(OT2(I)%NPTS) )
...
! 5.e Type 3: track output
!
                TFLAGI = output_type(I)%track%format
                IF ( TFLAGI ) THEN
                    MDS(11,I) =  ABS(MDS(11,I))
                  ELSE
                    MDS(11,I) = -ABS(MDS(11,I))
...
! 5.h Type 6: partitioned wave field data
!
                IPRT(1,I) = output_type(I)%partition%x0
                IPRT(2,I) = output_type(I)%partition%xn
                IPRT(3,I) = output_type(I)%partition%nx
                IPRT(4,I) = output_type(I)%partition%y0
                IPRT(5,I) = output_type(I)%partition%yn
                IPRT(6,I) = output_type(I)%partition%ny
                LPRT(I) = output_type(I)%partition%format
                IF ( MDSS.NE.MDSO .AND. NMPSCR.EQ.IMPROC ) THEN
                    WRITE (MDSS,961) IPRT(:,I)
                    IF ( .NOT. LPRT(I) ) THEN
                        WRITE (MDSS,960) 'output', 'UNFORMATTED'
...
!
          ...
          ALLOCATE ( TMOVE(2,NMOVE), AMOVE(NMOVE), DMOVE(NMOVE) )
          DO I=1,NMOVE
            READ(homogeneous_input(I)%moving%start, * ) TMOVE(:,I)
            AMOVE(I) = homogeneous_input(I)%moving%speed
            DMOVE(I) = homogeneous_input(I)%moving%direction
            IF ( MDSS.NE.MDSO .AND. NMPSCR.EQ.IMPROC )                &
