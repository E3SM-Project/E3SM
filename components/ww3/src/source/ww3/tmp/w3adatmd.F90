...
!      TAUWX/Y   R.A.  Public   Stresses particularly for PALM.
!
...
!
        REAL, POINTER         ::  SXX(:),  SYY(:),  SXY(:),  TAUOX(:),&
                                  ...
                                  USSY(:),  PRMS(:),  TPMS(:),        &
                                  PHICE(:), TAUICE(:,:)
        REAL, POINTER         ::  P2SMS(:,:),  US3D(:,:)
        REAL, POINTER         :: XSXX(:), XSYY(:), XSXY(:), XTAUOX(:),&
                                 XTAUOY(:), XBHD(:), XPHIOC(:),       &
                                 XTUSX(:), XTUSY(:), XUSSX(:),        &
                                 XUSSY(:), XPRMS(:), XTPMS(:),        &
                                 XPHICE(:), XTAUICE(:,:)
        REAL, POINTER         :: XP2SMS(:,:), XUS3D(:,:)
!
...
!
        REAL, POINTER         ::  MSSX(:), MSSY(:), MSCX(:), MSCY(:)
        REAL, POINTER         :: XMSSX(:), XMSSY(:), XMSCX(:),        &
                                 XMSCY(:)
!
...
!/ ------------------------------------------------------------------- /
      USE W3GDATMD, ONLY: NGRIDS, IGRID, W3SETG, NK, NX, NY, NSEA,    &
                          NSEAL, NSPEC, NTH, E3DF, P2MSF, US3DF, GTYPE, UNGTYPE
...
!
      ALLOCATE ( WADATS(IMOD)%HS   (NSEALM), WADATS(IMOD)%WLM (NSEALM), &
                 ...
                 WADATS(IMOD)%HCMAXD(NSEALM), STAT=ISTAT )
      CHECK_ALLOC_STATUS ( ISTAT )
!
...
!
      ...
      USE W3GDATMD, ONLY: E3DF, P2MSF, US3DF, GTYPE, UNGTYPE
!
      USE W3SERVMD, ONLY: EXTCDE
!
      IMPLICIT NONE
