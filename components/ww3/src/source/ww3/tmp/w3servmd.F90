...
!/ ------------------------------------------------------------------- /
      SUBROUTINE EXTCDE ( IEXIT, UNIT, MSG, FILE, LINE )
!/
!/                  +-----------------------------------+
!/                  | WAVEWATCH III           NOAA/NCEP |
!/                  |           H. L. Tolman            |
...
! Subroutine arguments
        ...
        REAL,    INTENT(IN) :: Alpha                ! Turning angle in degree
        REAL, INTENT(INOUT) :: Spectr(NFreq,NDirc)  ! Wave spectrum in and out
...
! So shift nsft bins anticlockwise results in local bin number Decreases by nsft
              jj=ii - nsft
...
! Subroutine arguments
        ...
        INTEGER, INTENT(IN) :: NFreq, NDirc         ! No. frequ and direc bins
        REAL,    INTENT(IN) :: Alpha                ! Turning angle in degree
        REAL, INTENT(INOUT) :: Spectr(NDirc, NFreq)  ! Wave action in and out
...
! Scale lambda pole to range -180 to 180 degs
      ...
      IF (LAMBDA_POLE.LE.-180.0) LAMBDA_POLE=LAMBDA_POLE+360.0
      IF (LAMBDA_POLE.GT. 180.0) LAMBDA_POLE=LAMBDA_POLE-360.0
...
! Local varables:------------------------------------------------------
      REAL :: E_LAMBDA, E_PHI, A_LAMBDA, A_PHI,   &
...
!*********************************************************************
   SUBROUTINE DIAGONALIZE(a,d,v,nrot)
!*********************************************************************
   IMPLICIT NONE
