...
!
          ...
          ELSE
            ...
            IF ( NAPROC.GT.1 ) THEN
               CLOSE (NDSR)
               OPEN (NDSR,FILE=FNMPRE(:J)//FNAME, ACCESS='STREAM',     &
                    FORM='UNFORMATTED', STATUS='OLD',                  &
                    ERR=800, IOSTAT=IERR, ACTION='READ')
               ...
               DO JSEA = 1, NSEAL_MIN
                 ...
                 CALL MPI_SCATTER(VGBUFF, NSIZE, MPI_REAL,             &
                                  ...
                                  NAPROC-1, MPI_COMM_WAVE, IERR        )
                 VA(1:NSPEC,JSEA) = VLBUFF(1:NSPEC)
                 END DO
               CLOSE (NDSR)
               OPEN (NDSR,FILE=FNMPRE(:J)//FNAME, ACCESS='DIRECT',     &
...
!
                  ...
                  DO IY=1, NY
                    DO IPART=1,NPRTX2
                      NREC  = NREC + 1
                      WRITE (NDSR,REC=NREC) (MAPTMP(IY,IXL),          &
                             IXL=1+(IPART-1)*NSIZE,MIN(NX,IPART*NSIZE))
                      END DO
                    END DO
                  DEALLOCATE ( MAPTMP )
                  DO IPART=1,NPART
