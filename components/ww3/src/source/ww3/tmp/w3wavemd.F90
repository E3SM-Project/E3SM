...
!
  !!Li   Broadcast local SPCBAC(:,IK) to all other PEs.
         IF( ARCTIC ) THEN
         CALL MPI_BCAST(SPCBAC(1,IK),NSPEC,MPI_REAL,ISPEC,MPI_COMM_WORLD,IERR_MPI)
...
!
            ...
            NRQMAX = 0
!
            IF ( (FLOUT(1) .OR.  FLOUT(7)) .AND. NRQGO.NE.0 ) THEN
                IF ( ( DSEC21(TIME,TONEXT(:,1)).EQ.0. ) .OR. &
                     ( DSEC21(TIME,TONEXT(:,7)).EQ.0. ) ) THEN
              ...
              END IF
!
            IF ( (FLOUT(1) .OR.  FLOUT(7)) .AND. NRQGO2.NE.0 ) THEN
                IF ( ( DSEC21(TIME,TONEXT(:,1)).EQ.0. ) .OR. &
                     ( DSEC21(TIME,TONEXT(:,7)).EQ.0. ) ) THEN
