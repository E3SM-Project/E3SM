...
!
          ...
          !ELSE
          !  IF (ISP.EQ.1) WRITE(6,*) 'EXCLUDED POINT FOR CURRENTS:',IXY,ISEA,IOBP(IXY)
            END IF
          END DO
        END IF
...
!
          ...
          DO I = INDEX_CELL(IP), INDEX_CELL(IP+1)-1
            ...
            KTMP        = KELEM ! Copy
            NM          = - 1.D0/MIN(-THR8,SUM(MIN(0.d0,KTMP))) ! N-Values
            KELEM       = MAX(0.d0,KTMP)
...
!/
       USE W3GDATMD, ONLY : NK, NTH, NTRI, NX, CCON, IE_CELL,POS_CELL, SI, &
                            IEN, TRIGP, CLATS, MAPSF, IOBPD, IOBP, IOBPA, XYB
       USE W3WDATMD, ONLY: TIME
...
! Does take into account boundary points in CFL calculation
             IF (IOBP(IP) .NE. 0) CYCLE
             DTMAXEXP = SI(IP)/MAX(DBLE(10.E-10),KKSUM(IP))
...
!
               U(IP) = MAX(0.d0,U(IP)-DTSI(IP)*ST(IP))*DBLE(IOBPD(ITH,IP)) &
                       + AC(IP)*(1-IOBPD(ITH,IP))  ! restores reflected boundary values
               END DO
!
             AC = U
!
! 5 Update boundaries: performs interpolation in time as done in rect grids (e.g. w3pro1md.ftn)
!
             IF ( FLBPI ) THEN
...
!
              DO IBI=1, NBI
                 ...
                 BOUNDARY_FORCING = ( RD1*BBPI0(ISP,IBI) + RD2*BBPIN(ISP,IBI) )   &
                             / CG(IK,ISBPI(IBI)) * CLATS(ISBPI(IBI))
                 AC(IP) = AC(IP)*DBLE(IOBPD(ITH,IP)) +   &
                          BOUNDARY_FORCING *IOBPA(IP)*(1-IOBPD(ITH,IP))
                 END DO
               END IF
           END DO ! IT
!      CALL EXTCDE ( 99 )
...
!/
       USE W3GDATMD, ONLY : NK, NTH, NTRI, NX, CCON, IE_CELL,POS_CELL, SI, &
                            IEN, TRIGP, CLATS, MAPSF, IOBPA, IOBPD, IOBP, NNZ
       USE W3WDATMD, ONLY: TIME
       USE W3ADATMD, ONLY: CG, ITER
       USE W3ODATMD, ONLY: NDSE, NDST, FLBPI, NBI, TBPI0, TBPIN, ISBPI, BBPI0, BBPIN
       USE W3TIMEMD, ONLY: DSEC21
...
!
         ...
         END DO ! NTRI
           ...
           IF (LCALC) THEN ! If the current field or water level changes estimate the iteration number based on the new flow field a
             ...
             END DO ! IE
             DO IP = 1, NX
!               DTMAXEXP = MAX( ABS(DBLE(IOBP(IP))*DBLE(10.E10)),SI(IP)/MAX(DBLE(10.E-10),KKSUM(IP)*DBLE(IOBPD(ITH,IP)))) ! This cor
               DTMAXEXP = SI(IP)/MAX(DBLE(10.E-10),KKSUM(IP))
               DTMAXGL  = MIN( DTMAXGL, DTMAXEXP)
             END DO ! IP
             CFLXY = DBLE(DT)/DTMAXGL
           ...
           DO IT = 1, ITER(IK,ITH)
             ...
             END DO
              ...
              DO IP = 1, NX
               U(IP) = MAX(0.d0,U(IP)-DTSI(IP)*ST(IP))*IOBPD(ITH,IP)  &
                       + AC(IP)*(1-IOBPD(ITH,IP))  ! restores reflected data
               END DO
!
             AC = U
!
...
!
               ...
               ENDDO
             END IF
!          WRITE(6,*) 'ITER:',IK, ITH, LCALC, ITER(IK,ITH),IT,RD10,RD20,RD1,RD2
           END DO ! IT
!      CALL EXTCDE ( 99 )
!/
!/ End of W3XYPFSN ----------------------------------------------------- /
...
!         IF (SUM(AC) .GT. 0. ) THEN
         ...
         END IF
          ...
          DO IP = 1,NX
            U(IP) = MAX(0.d0,X(IP))*IOBPD(ITH,IP)  &
                       + DBLE(AC(IP))*(1-IOBPD(ITH,IP))  ! restores reflected boundary
...
!/
       USE W3GDATMD, ONLY : NK, NTH, NTRI, NX, CCON, IE_CELL,POS_CELL, SI, &
                            IEN, TRIGP, CLATS, MAPSF, IOBPD, IOBPA, TRIA
       USE W3WDATMD, ONLY: TIME
       USE W3ADATMD, ONLY: CG, ITER
       USE W3ODATMD, ONLY: NDSE, NDST, FLBPI, NBI, TBPI0, TBPIN, ISBPI, BBPI0, BBPIN
       USE W3TIMEMD, ONLY: DSEC21
...
!
 70   if (ipar(12).ne.0) then
      ...
      if ((ipar(7).lt.ipar(6) .or. ipar(6).le.0) .and. ((ipar(3).eq.999.and.ipar(11).eq.0) .or.  &
