...
TEST_CASE("rrtmgp_scream_standalone_k", "") {
  ...
  // Create kokkos arrays to store the input data
  ...
  // Grab views from field manager and copy in values from kokkos arrays. Making
  // copies is necessary since the kokkos::View take in data arranged with ncol
  ...
  // Finalize the driver. Kokkos will be finalized inside
