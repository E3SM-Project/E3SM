...
###############################################################################
    ...
    ###########################################################################
        self._run         = run
        self._commit      = commit
        self._machines    = [get_machine(m) for m in machines]
    ...
    ###########################################################################
        ...
        root_dir = pathlib.Path(str(self._root_dir).replace("$machine", machine.name))
        ...
        if self._local:
            ...
            if self._scream_docs:
                scream_docs_repo = root_dir
                scream_repo      = pathlib.Path(f"~/scream-perf-{machine.name}/components/eamxx")
            else:
                scream_docs_repo = None
                scream_repo      = root_dir
        ...
        local_cmd = local_cmd.\
            ...
            replace("$f90_compiler", machine.ftn_compiler).\
            replace("$c_compiler", machine.c_compiler).\
        ...
        cmd = "{setup}cd {repo} && {env_setup} && {repo_setup} && {batch} {local_cmd}".\
              format(setup=setup, repo=repo, env_setup=env_setup_str, repo_setup=repo_setup, batch=machine.batch, local_cmd=local_cmd)
    ...
    ###########################################################################
        ...
        print(f"Starting analysis on {machine.name} with cmd: {cmd}")
        ...
        if self._local:
            ...
            try:
                ssh_cmd = f"ssh -o StrictHostKeyChecking=no {machine.name} '{cmd}'"
                output = run_cmd_no_fail(ssh_cmd, dry_run=self._dry_run, exc_type=RuntimeError, combine_output=True)
            except RuntimeError as e:
                output = str(e)
                raise
            finally:
                result_path = os.path.join("gather-all-results", self._commit, machine.name)
        ...
        print(f"Completed analysis on {machine.name}")
