...
`test-all-scream` has an informative help dump, displaying some examples, important information, and options flags/descriptions (provided below in the collapsed text box).
...
??? note "Output from Calling `test-all-scream --help`"
    ...
    For batch machines, this script expects to already be on a compute node; it does not do batch submissions.
