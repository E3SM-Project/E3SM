...
- The standard we maintain is largely identical to the
  ...
  enumerated in the configuration file [`$EAMXX_ROOT/.clang-format`](https://github.com/E3SM-Project/E3SM/blob/master/components/eamxx/.clang-format).
    - See this [How-to Guide](resources/clang-format_HOWTO.md)
      for more details on how to configure `clang-format` on your chosen
      development machine.
  ...
  your preference.
    - The one situation for which opening a pre-formatted PR may not be
      ...
      and requires a large number of changes.
        - I.e., touching the majority of lines in a file for format-only changes
          will make it difficult for a reviewer to determine which lines were
          changed for substantive reasons.
        - In this case, please refrain from formatting the code prior to opening
          the PR, and, instead, run `clang-format` once the PR is approved to be
          merged and make that the final commit.
...
is in the main pane of the page with the title **clang-format-linter summary**.
