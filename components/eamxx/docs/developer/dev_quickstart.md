...
configure, build, and test a project
that will run on the system being employed.
...
More details on this are provided below in the
[Testing Section](#code-build-test-repeat), and full details may be found in
[EAMxx Automated Standalone Testing](dev_testing/test_all_scream.md)

### Note on Computing Platforms

EAMxx is tested and ==(unofficially?) supported== on the
...
For the purposes of this quick-start guide, we will employ the working example
of adding a new [processes](../common/glossary.md#atmospheric-process)
...
??? Quote "Only Run Requested Test Cases"
    ...
    [^regex-tool]:
        For those that are not regex ninjas (yet), there are useful tools only to
        construct or decode regex strings--e.g., [RegExr](https://regexr.com).
...
??? Success "Baseline Tests"
    ...
    new **AshInjection**-related modifications as compared to the code before
    we started working.

    ***Note***, however, that any new **AshInjection**-specific tests or tests
    involving processes that interact with **AshInjection** may give different
...
??? Tip ""Quick-rerunning" Tests"
    ...
    of options to rerun tests quickly and avoiding configure or build costs.
...
set the proper "machine ID" flag indicating the machine name, and potentially
desired device type (CPU, GPU).
...
==FIXME: can be run via scheduler as long as it's on a compute node==
...
??? Warning "Must Be Run Interactively on a Machine with Batch Scheduling"
    ...
    This can be achieved most easily by launching the job manually from an
    interactive session.
...
To fully cover all the potential interactions of the **AshInjection** process,
...
??? Abstract "Thorough and Careful Testing Script"
    ...
    # run full eamxx test suite against pre-AshInjection baselines for all
...
Finally, because the **AshInjection** process is not called in isolation,
we must validate the results generated when **AshInjection** is called in
