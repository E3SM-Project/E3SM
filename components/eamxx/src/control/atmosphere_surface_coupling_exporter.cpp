...
namespace scream {
// =========================================================================================
SurfaceCouplingExporter::SurfaceCouplingExporter(
    const ekat::Comm &comm, const ekat::ParameterList &params)
    : AtmosphereProcess(comm, params) {}
// =========================================================================================
void SurfaceCouplingExporter::set_grids(
  ...
  // TODO: Switch horiz_winds to using U and V, note right now there is an issue
  // with when the subfields are created, so can't switch yet.
  ...
  add_field<Required>("precip_liq_surf_mass", scalar2d_layout, kg / m2,
                      grid_name);
  add_field<Required>("precip_ice_surf_mass", scalar2d_layout, kg / m2,
                      grid_name);
...
// =========================================================================================
void SurfaceCouplingExporter::create_helper_field(
    const std::string &name, const FieldLayout &layout,
...
size_t SurfaceCouplingExporter::requested_buffer_size_in_bytes() const {
  ...
  return Buffer::num_2d_vector_mid * m_num_cols *
             ekat::npack<Spack>(m_num_levs) * sizeof(Spack) +
         Buffer::num_2d_vector_int * m_num_cols *
             ekat::npack<Spack>(m_num_levs + 1) * sizeof(Spack);
}
// =========================================================================================
void SurfaceCouplingExporter::init_buffers(
  ...
  size_t used_mem =
      (reinterpret_cast<Real *>(s_mem) - buffer_manager.get_memory()) *
      sizeof(Real);

  EKAT_REQUIRE_MSG(
      used_mem == requested_buffer_size_in_bytes(),
      "Error! Used memory != requested memory for SurfaceCouplingExporter.");
}
// =========================================================================================
void SurfaceCouplingExporter::setup_surface_coupling_data(
  ...
  // The export data is of size ncols,num_cpl_exports. All other data is of size
  // num_scream_exports
  m_cpl_exports_view_h = decltype(m_cpl_exports_view_h)(
      sc_data_manager.get_field_data_ptr(), m_num_cols, m_num_cpl_exports);
  m_cpl_exports_view_d =
      Kokkos::create_mirror_view(DefaultDevice(), m_cpl_exports_view_h);

#ifdef HAVE_MOAB
  // The export data is of size num_cpl_exports,ncols. All other data is of size
  // num_scream_exports
  ...
  m_cpl_indices_view = decltype(m_cpl_indices_view)(
      sc_data_manager.get_field_cpl_indices_ptr(), m_num_scream_exports);
  ...
  m_do_export_during_init_view = decltype(m_do_export_during_init_view)(
      sc_data_manager.get_field_transfer_during_init_ptr(),
...
void SurfaceCouplingExporter::initialize_impl(const RunType /* run_type */) {
  ...
  for (int i = 0; i < m_num_scream_exports; ++i) {
    ...
    EKAT_REQUIRE_MSG(has_helper_field(fname),
                     "Error! Attempting to export " + fname +
    ...
    get_col_info_for_surface_values(
        field.get_header_ptr(), m_vector_components_view(i),
    ...
    m_column_info_h(i).transfer_during_initialization =
        m_do_export_during_init_view(i);
    if (m_do_export_during_init_view(i))
      any_initial_exports = true;
  ...
  // Set the number of exports from eamxx or set to a constant, default type =
  // FROM_MODEL
  using vos_type = std::vector<std::string>;
  using vor_type = std::vector<Real>;
  m_export_source =
      view_1d<DefaultDevice, ExportType>("", m_num_scream_exports);
  m_export_source_h = Kokkos::create_mirror_view(m_export_source);
  Kokkos::deep_copy(m_export_source_h,
                    FROM_MODEL); // The default is that all export variables
                                 // will be derived from the EAMxx state.
  ...
  if (m_params.isSublist("prescribed_from_file")) {
    ...
    EKAT_REQUIRE_MSG(export_from_file_params.isParameter("fields"),
                     "Error! surface_coupling_exporter::init - "
                     "prescribed_from_file does not have "
                     "'fields' parameter.");
    auto export_from_file_fields =
        export_from_file_params.get<vos_type>("fields");

    EKAT_REQUIRE_MSG(export_from_file_params.isParameter("files"),
                     "Error! surface_coupling_exporter::init - "
                     "prescribed_from_file does not have "
                     "'files' parameter.");
    auto export_from_file_names =
        export_from_file_params.get<vos_type>("files");
    ...
    EKAT_REQUIRE_MSG(are_files_present == are_fields_present,
                     "ERROR!! When prescribing export fields from file, you "
                     "must provide both "
    ...
    if (do_export_from_file) {
      ...
      // Check if alternative names have been provided.  This is useful for
      // source data files that don't use the conventional EAMxx ATM->SRFC
      // variable names.
      auto alt_names =
          export_from_file_params.get<vos_type>("fields_alt_name", {});
      for (auto entry : alt_names) {
        ...
        EKAT_REQUIRE_MSG(
            it != export_from_file_fields.end(),
            "Error! surface_coupling_exporter::init - LHS of entry '" + entry +
                "' in field_alt_names does not match a valid EAMxx field.\n");
        // Make sure that a user hasn't accidentally copy/pasted
        auto chk = ekat::find(export_from_file_reg_names, tokens[1]);
        EKAT_REQUIRE_MSG(
            chk == export_from_file_reg_names.end(),
            "Error! surface_coupling_exporter::init - RHS of entry '" + entry +
                "' in field_alt_names has already been used for a different "
                "field.\n");
      ...
      for (size_t ii = 0; ii < export_from_file_fields.size(); ++ii) {
        ...
        EKAT_REQUIRE_MSG(
            ...
            "ERROR!! surface_coupling_exporter::init - prescribed_from_file "
            "has field with name "
                << fname
                << " which can't be found in set of exported fields\n.");
        ...
        EKAT_REQUIRE_MSG(
            ...
            "Error! surface_coupling_exporter::init - attempting to set field "
                << fname
                << " export type, which has already been set. Please check "
                   "namelist options");
        m_export_source_h(idx) = FROM_FILE;
        ++m_num_from_file_exports;
        --m_num_from_model_exports;
        auto &f_helper = m_helper_fields.at(fname);
        // We want to add the field as a deep copy so that the helper_fields are
  ...
  if (m_params.isSublist("prescribed_constants")) {
    ...
    EKAT_REQUIRE_MSG(export_constant_params.isParameter("fields"),
                     "Error! surface_coupling_exporter::init - "
                     "prescribed_constants does not have "
                     "'fields' parameter.");
    EKAT_REQUIRE_MSG(export_constant_params.isParameter("values"),
                     "Error! surface_coupling_exporter::init - "
                     "prescribed_constants does not have "
                     "'values' parameter.");
    auto export_constant_fields =
        export_constant_params.get<vos_type>("fields");
    auto export_constant_values =
        export_constant_params.get<vor_type>("values");
    EKAT_REQUIRE_MSG(export_constant_fields.size() ==
                         export_constant_values.size(),
                     "Error! surface_coupling_exporter::init - "
                     "prescribed_constants 'fields' and "
    ...
    if (are_fields_present) {
      ...
      for (size_t ii = 0; ii < export_constant_fields.size(); ii++) {
        ...
        EKAT_REQUIRE_MSG(
            v_loc != m_export_field_names_vector.end(),
            "ERROR!! surface_coupling_exporter::init - prescribed_constants "
            "has field with name "
                << fname
                << " which can't be found in set of exported fields\n.");
        auto idx = v_loc - m_export_field_names_vector.begin();
        // This field should not have been set to anything else yet (recall
        // FROM_MODEL is the default)
        EKAT_REQUIRE_MSG(m_export_source_h(idx) == FROM_MODEL,
                         "Error! surface_coupling_exporter::init - attempting "
                         "to set field " +
                             fname +
                             " export type, which has already been set.  "
                             "Please check namelist options");
  ...
  // Final sanity check
  EKAT_REQUIRE_MSG(m_num_scream_exports = m_num_from_file_exports +
                                          m_num_const_exports +
                                          m_num_from_model_exports,
                   "Error! surface_coupling_exporter - Something went wrong "
                   "set the type of export "
                   "for all variables.");
  EKAT_REQUIRE_MSG(
      m_num_from_model_exports >= 0,
      "Error! surface_coupling_exporter - The number of exports derived from "
      "EAMxx < "
      "0, something must "
  ...
  if (any_initial_exports)
    do_export(0, true);
}
// =========================================================================================
void SurfaceCouplingExporter::run_impl(const double dt) { do_export(dt); }
// =========================================================================================
void SurfaceCouplingExporter::do_export(
    const double dt, const bool called_during_initialization) {
...
void SurfaceCouplingExporter::set_constant_exports() {
  ...
  // Gotta catch em all
  EKAT_REQUIRE_MSG(
      num_set == m_num_const_exports,
      "ERROR! SurfaceCouplingExporter::set_constant_exports() - Number of "
      "fields set "
      "to a constant (" +
...
// =========================================================================================
// This compute_eamxx_exports routine  handles all export variables that are
// derived from the EAMxx state. Important! This setup assumes the numerical
// order of export_cpl_indices as listed in
// /src/mct_coupling/eamxx_cpl_indices.F90
//
// If this order is changed or a new variable is added it is important to update
// the corresponding index query in the below.
void SurfaceCouplingExporter::compute_eamxx_exports(
    const double dt, const bool called_during_initialization) {
  using PC = physics::Constants<Real>;

  const auto &p_int = get_field_in("p_int").get_view<const Real **>();
  const auto &pseudo_density =
      get_field_in("pseudo_density").get_view<const Spack **>();
  const auto &qv    = get_field_in("qv").get_view<const Spack **>();
  const auto &T_mid = get_field_in("T_mid").get_view<const Spack **>();
  // TODO: This will need to change if we ever switch from horiz_winds to U and
  // V
  const auto &horiz_winds =
      get_field_in("horiz_winds").get_view<const Real ***>();
  const auto &p_mid = get_field_in("p_mid").get_view<const Spack **>();
  const auto &phis  = get_field_in("phis").get_view<const Real *>();
  const auto &sfc_flux_dir_nir =
      get_field_in("sfc_flux_dir_nir").get_view<const Real *>();
  const auto &sfc_flux_dir_vis =
      get_field_in("sfc_flux_dir_vis").get_view<const Real *>();
  const auto &sfc_flux_dif_nir =
      get_field_in("sfc_flux_dif_nir").get_view<const Real *>();
  const auto &sfc_flux_dif_vis =
      get_field_in("sfc_flux_dif_vis").get_view<const Real *>();
  const auto &sfc_flux_sw_net =
      get_field_in("sfc_flux_sw_net").get_view<const Real *>();
  const auto &sfc_flux_lw_dn =
      get_field_in("sfc_flux_lw_dn").get_view<const Real *>();
  ...
  const auto setup_policy = ekat::ExeSpaceUtils<
      KT::ExeSpace>::get_thread_range_parallel_scan_team_policy(num_cols,
                                                                num_levs);
  Kokkos::parallel_for(
      setup_policy,
      KOKKOS_LAMBDA(const Kokkos::TeamPolicy<KT::ExeSpace>::member_type &team) {
        ...
        // These views are needed by more than one export variable so we declare
        // them here.
        ...
        // Compute vertical layer heights (relative to ground surface rather
        // than from sea level). Use z_int(nlevs) = z_surf = 0.0. Currently only
        // needed for Sa_z, Sa_dens and Sa_pslv
        const bool calculate_z_vars =
            export_source(idx_Sa_z) == FROM_MODEL ||
            export_source(idx_Sa_dens) == FROM_MODEL ||
            export_source(idx_Sa_pslv) == FROM_MODEL;
        if (calculate_z_vars) {
          PF::calculate_dz(team, pseudo_density_i, p_mid_i, T_mid_i, qv_i,
                           dz_i);
          team.team_barrier();
        ...
        // Set the values in the helper fields which correspond to the exported
        // variables
        ...
        if (export_source(idx_Sa_u) == FROM_MODEL) {
          const auto u_wind_i =
              ekat::subview(horiz_winds, i,
                            0); // TODO, when U and V work switch to using here
                                // instead of horiz_winds.
        ...
        if (export_source(idx_Sa_ptem) == FROM_MODEL) {
          Sa_ptem(i) = PF::calculate_theta_from_T(s_T_mid_i(num_levs - 1),
                                                  s_p_mid_i(num_levs - 1));
        }
        ...
        if (export_source(idx_Sa_dens) == FROM_MODEL) {
          ...
          Sa_dens(i) = PF::calculate_density(s_pseudo_density_i(num_levs - 1),
                                             s_dz_i(num_levs - 1));
        }
        ...
        if (export_source(idx_Sa_pslv) == FROM_MODEL) {
          ...
          const auto s_z_mid_i = ekat::scalarize(z_mid_i);
          // Calculate air temperature at bottom of cell closest to the ground
          // for PSL
          const Real T_int_bot = PF::calculate_surface_air_T(
              s_T_mid_i(num_levs - 1), s_z_mid_i(num_levs - 1));
        ...
        if (not called_during_initialization) {
          ...
          if (export_source(idx_Faxa_rainl) == FROM_MODEL) {
            Faxa_rainl(i) =
                precip_liq_surf_mass(i) / dt * (1000.0 / PC::RHO_H2O);
          }
          if (export_source(idx_Faxa_snowl) == FROM_MODEL) {
            Faxa_snowl(i) =
                precip_ice_surf_mass(i) / dt * (1000.0 / PC::RHO_H2O);
          }
        }
      });
  // Variables that are already surface vars in the ATM can just be copied
  // directly.
  if (m_export_source_h(idx_Faxa_swndr) == FROM_MODEL) {
    Kokkos::deep_copy(Faxa_swndr, sfc_flux_dir_nir);
  }
  if (m_export_source_h(idx_Faxa_swvdr) == FROM_MODEL) {
    Kokkos::deep_copy(Faxa_swvdr, sfc_flux_dir_vis);
  }
  if (m_export_source_h(idx_Faxa_swndf) == FROM_MODEL) {
    Kokkos::deep_copy(Faxa_swndf, sfc_flux_dif_nir);
  }
  if (m_export_source_h(idx_Faxa_swvdf) == FROM_MODEL) {
    Kokkos::deep_copy(Faxa_swvdf, sfc_flux_dif_vis);
  }
  if (m_export_source_h(idx_Faxa_swnet) == FROM_MODEL) {
    Kokkos::deep_copy(Faxa_swnet, sfc_flux_sw_net);
  }
  if (m_export_source_h(idx_Faxa_lwdn) == FROM_MODEL) {
    Kokkos::deep_copy(Faxa_lwdn, sfc_flux_lw_dn);
  }
}
// =========================================================================================
void SurfaceCouplingExporter::do_export_to_cpl(
  ...
  Kokkos::parallel_for(
      export_policy, KOKKOS_LAMBDA(const int &i) {
        ...
        // if this is during initialization, check whether or not the field
        // should be exported
        bool do_export = (not called_during_initialization ||
                          info.transfer_during_initialization);
        if (do_export) {
          cpl_exports_view_d(icol, info.cpl_indx) =
              info.constant_multiple * info.data[offset];
        }
      });
#ifdef HAVE_MOAB
  Kokkos::parallel_for(
      export_policy, KOKKOS_LAMBDA(const int &i) {
        ...
        bool do_export = (not called_during_initialization ||
                          info.transfer_during_initialization);
        if (do_export) {
          moab_cpl_exports_view_d(info.cpl_indx, icol) =
              info.constant_multiple * info.data[offset];
        }
      });
#endif
