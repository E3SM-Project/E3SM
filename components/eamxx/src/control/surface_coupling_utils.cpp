...
void get_col_info_for_surface_values(
  ...
  EKAT_REQUIRE_MSG(scalar || vector, "Error! Support for tensor fields not yet "
                                     "implemented in surface coupling.\n");
  EKAT_REQUIRE_MSG((vecComp < 0) == scalar,
                   "Error! You can and must specify a vector component only "
                   "for vector fields.\n");
  ...
  // in the larger field. We then must treat the child field as having
  // col_stride and col_offset of a vector field with component as its
  // subview_idx.
  ...
  if (parent != nullptr) {
    ...
    // Recall: idx = (idim,k) = (dimension where slice happened, index along
    // said dimension). Field class only allows idim=0,1. But we should never be
    // in the case of idim=0, here. If we have idim=0, it means that the parent
    // field did not have COL as tag[0].
    EKAT_REQUIRE_MSG(
        sv_info.dim_idx == 1,
    ...
    col_offset +=
        sv_info.slice_idx * parent->get_alloc_properties().get_last_extent();
