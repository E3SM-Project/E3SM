...
void AtmosphereDriver::create_fields()
{
  ...
  auto add_component = [&](const Field& f, int cmp, const std::string& suffix) {
    auto sf_name = f.name() + "_" + suffix;
    ...
    const auto& f_track = f.get_header().get_tracking();
  ...
  for (auto it : m_grids_manager->get_repo()) {
    ...
    if (m_field_mgr->has_field("surf_mom_flux", gn)) {
      ...
      add_component(smf,0,"U");
      add_component(smf,1,"V");
    }
  }
