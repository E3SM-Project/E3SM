...
inline void dummy_atm_init() {
  ...
  // Need to register products in the factory *before* we create any
  // AtmosphereProcessGroup, which rely on factory for process creation. The
  // initialize method of the AD does that. While we're at it, check that the
  // case insensitive key of the factory works.
  auto &proc_factory = AtmosphereProcessFactory::instance();
  proc_factory.register_product("Dummy",
                                &create_atmosphere_process<DummyProcess>);
