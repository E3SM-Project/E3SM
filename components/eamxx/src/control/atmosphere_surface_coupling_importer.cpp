...
namespace scream {
// =========================================================================================
SurfaceCouplingImporter::SurfaceCouplingImporter(
    const ekat::Comm &comm, const ekat::ParameterList &params)
    : AtmosphereProcess(comm, params) {}
// =========================================================================================
void SurfaceCouplingImporter::set_grids(
...
// =========================================================================================
void SurfaceCouplingImporter::setup_surface_coupling_data(
  ...
  // The import data is of size ncols,num_cpl_imports. All other data is of size
  // num_scream_imports
  m_cpl_imports_view_h = decltype(m_cpl_imports_view_h)(
      sc_data_manager.get_field_data_ptr(), m_num_cols, m_num_cpl_imports);
  m_cpl_imports_view_d = Kokkos::create_mirror_view_and_copy(
      DefaultDevice(), m_cpl_imports_view_h);
#ifdef HAVE_MOAB
  // The import data is of size num_cpl_imports, ncol. All other data is of size
  // num_scream_imports
  ...
  m_moab_cpl_imports_view_d = Kokkos::create_mirror_view_and_copy(
      DefaultDevice(), m_moab_cpl_imports_view_h);
#endif
  ...
  m_cpl_indices_view = decltype(m_cpl_indices_view)(
      sc_data_manager.get_field_cpl_indices_ptr(), m_num_scream_imports);
...
void SurfaceCouplingImporter::initialize_impl(const RunType /* run_type */) {
  ...
  for (int i = 0; i < m_num_scream_imports; ++i) {
    ...
    m_column_info_h(i).transfer_during_initialization =
        m_do_import_during_init_view(i);
    if (m_do_import_during_init_view(i))
      any_initial_imports = true;
  ...
  add_postcondition_check<FieldWithinIntervalCheck>(
      get_field_out("sfc_alb_dir_vis"), m_grid, 0.0, 1.0, true);
  add_postcondition_check<FieldWithinIntervalCheck>(
      get_field_out("sfc_alb_dir_nir"), m_grid, 0.0, 1.0, true);
  add_postcondition_check<FieldWithinIntervalCheck>(
      get_field_out("sfc_alb_dif_vis"), m_grid, 0.0, 1.0, true);
  add_postcondition_check<FieldWithinIntervalCheck>(
      get_field_out("sfc_alb_dif_nir"), m_grid, 0.0, 1.0, true);
...
// =========================================================================================
void SurfaceCouplingImporter::run_impl(const double /* dt */) { do_import(); }
// =========================================================================================
void SurfaceCouplingImporter::do_import(
  ...
  Kokkos::parallel_for(
      unpack_policy, KOKKOS_LAMBDA(const int &i) {
        ...
        // should be imported
        bool do_import = (not called_during_initialization ||
                          info.transfer_during_initialization);
        if (do_import) {
          info.data[offset] =
              cpl_imports_view_d(icol, info.cpl_indx) * info.constant_multiple;
        }
      });
  ...
  Kokkos::parallel_for(
      unpack_policy, KOKKOS_LAMBDA(const int &i) {
        ...
        bool do_import = (not called_during_initialization ||
                          info.transfer_during_initialization);
        if (do_import) {
          info.data[offset] = moab_cpl_imports_view_d(info.cpl_indx, icol) *
                              info.constant_multiple;
        }
      });
#endif
...
// =========================================================================================
void SurfaceCouplingImporter::overwrite_iop_imports(
    const bool called_during_initialization) {
  using policy_type = KokkosTypes<DefaultDevice>::RangePolicy;
  ...
  for (int ifield = 0; ifield < m_num_scream_imports; ++ifield) {
    ...
    if (called_during_initialization &&
        not info_h.transfer_during_initialization) {
      continue;
    }
