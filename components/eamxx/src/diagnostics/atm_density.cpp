...
void AtmDensityDiagnostic::set_grids(
...
void AtmDensityDiagnostic::compute_diagnostic_impl() {
  ...
  Kokkos::parallel_for(
      "AtmosphereDensityDiagnostic",
      Kokkos::RangePolicy<>(0, m_num_cols * nlevs),
      KOKKOS_LAMBDA(const int &idx) {
        const int icol = idx / nlevs;
        const int ilev = idx % nlevs;
        auto dz =
            PF::calculate_dz(pseudo_density_mid(icol, ilev), p_mid(icol, ilev),
                             T_mid(icol, ilev), qv_mid(icol, ilev));
        atm_dens(icol, ilev) =
            PF::calculate_density(pseudo_density_mid(icol, ilev), dz);
      });
  Kokkos::fence();
}
