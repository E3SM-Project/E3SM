...
VertContractDiag::VertContractDiag(const ekat::Comm &comm,
  ...
  // Let's support unweighted/weighted mean/sum
  // unweighted mean --> verg_uavg
  // weighted mean --> vert_wavg
  // unweighted sum --> vert_usum
  // weighted sum --> vert_wsum
  m_contract_method = m_params.get<std::string>("contract_method");
  m_diag_name       = fname + m_contract_method;
}
...
void VertContractDiag::initialize_impl(const RunType /*run_type*/) {
  ...
  // scale the weighting field (pseudo density) by g if weighting, else set all
  // to 1
  m_weighting = get_field_in("pseudo_density");
  if(m_contract_method == "vert_wavg" || m_contract_method == "vert_wsum") {
