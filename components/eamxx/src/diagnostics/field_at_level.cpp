...
// =========================================================================================
FieldAtLevel::FieldAtLevel(const ekat::Comm &comm,
...
void FieldAtLevel::initialize_impl(const RunType /*run_type*/) {
  ...
  EKAT_REQUIRE_MSG(tag == LEV || tag == ILEV,
                   "Error! FieldAtLevel diagnostic expects a layout ending "
                   "with 'LEV'/'ILEV' tag.\n"
                   " - field name  : " +
                       fid.name() +
                       "\n"
  ...
  if (ekat::starts_with(location, "lev_")) {
    ...
    EKAT_REQUIRE_MSG(
        lev.find_first_not_of("0123456789") == std::string::npos,
    ...
    EKAT_REQUIRE_MSG(
        m_field_level < layout.dims().back(),
        "Error! Invalid level specification. Level index out of bounds.\n"
        "  - input level: " +
            std::to_string(m_field_level) +
            "\n"
            "  - number of levels: " +
            std::to_string(layout.dims().back()) + "\n");
    ...
    EKAT_ERROR_MSG(
        ...
        location +
        "'\n"
        "  - expected: 'model_top','model_bot', or 'levN', with N an "
        "integer.\n");
  }
...
void FieldAtLevel::compute_diagnostic_impl() {
  const auto &f = get_fields_in().front();
  const auto &diag_layout =
      m_diagnostic_output.get_header().get_identifier().get_layout();
  ...
  case 2: {
    ...
    Kokkos::parallel_for(
        m_diagnostic_output.name(), policy, KOKKOS_LAMBDA(const int idx) {
