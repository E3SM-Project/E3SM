...
RelativeHumidityDiagnostic::RelativeHumidityDiagnostic(
...
void RelativeHumidityDiagnostic::set_grids(
  ...
  add_field<Required>("pseudo_density", scalar3d, Pa, grid_name,
                      SCREAM_PACK_SIZE);
  add_field<Required>("pseudo_density_dry", scalar3d, Pa, grid_name,
                      SCREAM_PACK_SIZE);
...
void RelativeHumidityDiagnostic::compute_diagnostic_impl() {
  ...
  Kokkos::parallel_for(
      ...
      KOKKOS_LAMBDA(const int &idx) {
        ...
        auto qv_sat_l         = physics::qv_sat_wet(
            T_mid(icol, jpack), p_dry_mid(icol, jpack), true, range_mask,
