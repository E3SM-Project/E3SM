...
void HommeDynamics::init_buffers(const ATMBufferManager &buffer_manager)
{
  ...
  if (params.do_3d_turbulence || do_leonard) {
    ...
    assign_scratch(m_dsdx_thl_all);
    assign_scratch(m_dsdy_thl_all);
...
void HommeDynamics::set_computed_group_impl (const FieldGroup& group)
{
  ...
  if (group.name()=="tracers") {
    ...
    const auto& fields = group.individual_fields();
    m_qc_idx = -1;
    if (fields.count("qc") != 0) {
      m_qc_idx = fields.at("qc").get_header().get_alloc_properties().get_subview_info().slice_idx;
    }
  }
}
