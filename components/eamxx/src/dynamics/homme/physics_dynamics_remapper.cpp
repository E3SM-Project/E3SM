...
void PhysicsDynamicsRemapper::registration_ends_impl() {
...
void PhysicsDynamicsRemapper::initialize_device_variables() {
  ...
  for (auto which : {'P', 'D'}) {
    ...
    auto get_view = [&](const int i, const Field &f) {
      ...
      switch (rank) {
      case 1:
        repo.h_cviews[i].v1d = f.get_view<const Real *>();
        break;
      case 2:
        repo.h_cviews[i].v2d = f.get_view<const Real **>();
        break;
      case 3:
        repo.h_cviews[i].v3d = f.get_view<const Real ***>();
        break;
      case 4:
        repo.h_cviews[i].v4d = f.get_view<const Real ****>();
        break;
      case 5:
        repo.h_cviews[i].v5d = f.get_view<const Real *****>();
        break;
      }
      if (not f.is_read_only()) {
        switch (rank) {
        case 1:
          repo.h_views[i].v1d = f.get_view<Real *>();
          break;
        case 2:
          repo.h_views[i].v2d = f.get_view<Real **>();
          break;
        case 3:
          repo.h_views[i].v3d = f.get_view<Real ***>();
          break;
        case 4:
          repo.h_views[i].v4d = f.get_view<Real ****>();
          break;
        case 5:
          repo.h_views[i].v5d = f.get_view<Real *****>();
          break;
        }
      }
    };
...
void PhysicsDynamicsRemapper::remap_fwd_impl() {
...
void PhysicsDynamicsRemapper::remap_bwd_impl() {
...
void PhysicsDynamicsRemapper::setup_boundary_exchange() {
...
void PhysicsDynamicsRemapper::create_p2d_map() {
  ...
  Kokkos::parallel_for(
      policy, KOKKOS_LAMBDA(const int idof) {
