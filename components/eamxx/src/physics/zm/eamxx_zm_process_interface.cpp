...
void ZMDeepConvection::create_requests ()
{
  ...
  add_field<Computed>("deep_conv_t_tend",            scalar3d_mid, K/s,    grid_name, pack_size);
...
void ZMDeepConvection::initialize_impl (const RunType)
{
  ...
  add_postcondition_check<FieldWithinIntervalCheck>(get_field_out("nc"),m_grid,0.0,1.e12,false);
  add_postcondition_check<FieldWithinIntervalCheck>(get_field_out("ni"),m_grid,0.0,1.e12,false);
...
void ZMDeepConvection::run_impl (const double dt)
{
  ...
  const auto& deep_conv_t_tend  = get_field_out("deep_conv_t_tend") .get_view<Real**>();
  Kokkos::parallel_for("zm_update_prognostic",
    ...
    deep_conv_t_tend(i,k) = loc_zm_output_tend_out_t(i,k);
  });
