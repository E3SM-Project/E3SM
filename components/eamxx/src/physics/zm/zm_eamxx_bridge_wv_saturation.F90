! this module is a version of components/eam/src/physics/cam/wv_saturation.F90
! modified to facilitate bridging to the fortran ZM deep convection scheme
module zm_eamxx_bridge_wv_saturation
...
use zm_eamxx_bridge_physconst, only: epsilo, &
                                     latvap, &
                                     latice, &
                                     rh2o,   &
                                     cpair,  &
...
subroutine wv_sat_init
  ...
  !   SVP lookup table).                                             !
  !------------------------------------------------------------------!
  use zm_eamxx_bridge_params, only: masterproc
...
end module zm_eamxx_bridge_wv_saturation
