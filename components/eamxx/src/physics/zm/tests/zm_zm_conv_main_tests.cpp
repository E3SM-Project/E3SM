...
struct UnitWrap::UnitTest<D>::TestZmConvMain : public UnitWrap::UnitTest<D>::Base {
  ...
  void run_bfb()
  {
    ...
    ZmConvMainData baseline_data[] = {
      ...
      ZmConvMainData(    4,    4,   72,    73,       2.0,         false, 0),
      ZmConvMainData(    4,    4,   72,    73,       3.0,         true , 0),
      ZmConvMainData(    4,    4,  128,   129,       4.0,         false, 0),
      ZmConvMainData(    4,    4,  128,   129,       5.0,         true , 0),
    };
    ...
    if (SCREAM_BFB_TESTING && this->m_baseline_action == COMPARE) {
      for (Int i = 0; i < num_runs; ++i) {
        ...
        REQUIRE(d_baseline.total(d_baseline.prec) == d_test.total(d_test.msemax_klev_g));
        ...
        for (Int k = 0; k < d_baseline.total(d_baseline.prec); ++k) {
          ...
          REQUIRE(d_baseline.dcape[k] == d_test.dcape[k]);
          ...
          REQUIRE(d_baseline.msemax_klev_g[k] == d_test.msemax_klev_g[k]);
