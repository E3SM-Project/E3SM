...
void SPA::initialize_impl (const RunType /* run_type */)
{
  ...
  util::TimeStamp ref_ts (1,1,1,0,0,0); // Beg of any year, since we use yearli periodic timeline
  ...
  remap_data.hremap_file = spa_map_file;
  if (m_iop_data_manager!=nullptr) {
    ...
    // TODO: expose tgt lat/lon in IOPDataManager, to avoid injencting knowledge
