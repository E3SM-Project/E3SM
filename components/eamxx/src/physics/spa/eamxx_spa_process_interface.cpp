...
void SPA::set_grids(const std::shared_ptr<const GridsManager> grids_manager) {
  ...
  this->log(LogLevel::info, "SPA data file bands dimensions:\n"
                            "  - num sw bands: " +
                                std::to_string(nswbands) +
                                "\n"
                                "  - num lw bands: " +
                                std::to_string(nlwbands) + "\n");
  ...
  auto scalar3d_swband =
      m_model_grid->get_3d_vector_layout(true, nswbands, "swband");
  auto scalar3d_lwband =
      m_model_grid->get_3d_vector_layout(true, nlwbands, "lwband");
...
void SPA::initialize_impl(const RunType /* run_type */) {
  ...
  // NOTE: SPA does not have an internal persistent state, so run_type is
  // irrelevant
  ...
  // SPA doesn't really *need* pint, but DataInterpolation does. It's important
  // to stress that NO FIELD VALUES from p_int are accessed in the
  // DataInterpolation we build, since we don't remap any field on interfaces.
  // But when the VerticalRemapper in the DataInterpolation is setup, pint must
  // store the correct layout of a field defined at interfaces; it does not have
  // to have COL dimension. It just need to have the ILEV tag in the layout AND
  // have alloc properties compatible with SCREAM_PACK_SIZE. NOTE: we could just
  // add p_int as a required field, but that would be misleading in the DAG
  auto pmid = get_field_in("p_mid");
  Field pint(FieldIdentifier("p_int", m_model_grid->get_vertical_layout(false),
                             Pa, m_model_grid->name()));
  ...
  m_data_interpolation =
      std::make_shared<DataInterpolation>(m_model_grid, spa_fields);
  m_data_interpolation->setup_time_database(
      {spa_data_file}, util::TimeLine::YearlyPeriodic, ref_ts);

  DataInterpolation::RemapData remap_data;
  remap_data.hremap_file = spa_map_file == "none" ? "" : spa_map_file;
  if (m_iop_data_manager != nullptr) {
    // IOP cases cannot have a remap file. We will create a IOPRemapper as the
    // horiz remapper
    EKAT_REQUIRE_MSG(
        spa_map_file == "" or spa_map_file == "none",
        "Error! Cannot define spa_remap_file for cases with an Intensive "
        "Observation "
        "Period defined. "
    ...
    remap_data.iop_lon =
        m_iop_data_manager->get_params().get<Real>("target_longitude");
  ...
  add_postcondition_check<FWI>(get_field_out("nccn"), m_model_grid, 0, 1e11,
                               true, -1e11 * eps);

  // TODO: add an epslon to max possible upper bound of aero_ssa_sw?
  add_postcondition_check<FWI>(get_field_out("aero_g_sw"), m_model_grid, 0.0,
                               1.0, true);
  add_postcondition_check<FWI>(get_field_out("aero_ssa_sw"), m_model_grid, 0.0,
                               1.0, true);
  add_postcondition_check<FWI>(get_field_out("aero_tau_sw"), m_model_grid, 0.0,
                               1.0, true);
  add_postcondition_check<FWI>(get_field_out("aero_tau_lw"), m_model_grid, 0.0,
                               1.0, true);
}
