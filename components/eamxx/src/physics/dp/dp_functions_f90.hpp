...
struct AdvanceIopNudgingData : public PhysicsTestData {
  static constexpr size_t NUM_ARRAYS = 10;

  // Inputs
  Int plev;
  Real scm_dt, ps_in;
  Real *t_in, *q_in, *hyai, *hyam, *hybi, *hybm;

  // Outputs
  Real *t_update, *q_update, *relaxt, *relaxq;

  AdvanceIopNudgingData(Int plev_, Real scm_dt_, Real ps_in_) :
    PhysicsTestData({{ plev_ }}, {{ &t_in, &q_in, &hyai, &hyam, &hybi, &hybm, &t_update, &q_update, &relaxt, &relaxq }}),
