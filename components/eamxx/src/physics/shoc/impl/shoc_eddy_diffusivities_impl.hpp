...
KOKKOS_FUNCTION void Functions<S, D>::eddy_diffusivities(
    ...
    const Scalar &Ckm, const Scalar &pblh, const uview_1d<const Spack> &zt_grid,
  ...
  Kokkos::parallel_for(
      Kokkos::TeamVectorRange(team, nlev_pack), [&](const Int &k) {
        ...
        const Smask condition =
            (zt_grid(k) < pblh + pbl_trans) && (s_tabs(nlev - 1) < tabs_crit);
        tkh(k).set(condition,
                   Ckh_s * ekat::square(shoc_mix(k)) * ekat::sqrt(sterm_zt(k)));
        tk(k).set(condition,
                  Ckm_s * ekat::square(shoc_mix(k)) * ekat::sqrt(sterm_zt(k)));
