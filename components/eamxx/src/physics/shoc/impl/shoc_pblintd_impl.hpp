...
KOKKOS_FUNCTION void Functions<S, D>::pblintd(
    const MemberType &team, const Int &nlev, const Int &nlevi, const Int &npbl,
  ...
  workspace.template take_many_contiguous_unsafe<2>({"rino", "thv"},
                                                    {&rino, &thv});
  ...
  // Compute Obukhov length virtual temperature flux and various arrays for use
  // later:
  ...
  team.team_barrier();
  pblintd_height(team, nlev, npbl, z, u, v, ustar, thv, s_thv(nlev - 1), pblh,
                 rino, check);

  // Estimate an effective surface temperature to account for surface
  // fluctuations
  Scalar tlv;
  pblintd_surf_temp(nlev, nlevi, npbl, z, ustar, obklen, kbfs, thv, tlv, pblh,
                    check, rino);
