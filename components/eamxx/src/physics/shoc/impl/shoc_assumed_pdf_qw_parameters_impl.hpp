...
KOKKOS_INLINE_FUNCTION void Functions<S, D>::shoc_assumed_pdf_qw_parameters(
    const Spack &wqwsec, const Spack &sqrtw2, const Spack &Skew_w,
  ...
  const Smask condition =
      qwsec > (rt_tol * rt_tol) && ekat::abs(w1_2 - w1_1) > w_thresh;

  const Spack corrtest2 =
      ekat::max(-1, ekat::min(1, wqwsec / (sqrtw2 * sqrtqt)));
