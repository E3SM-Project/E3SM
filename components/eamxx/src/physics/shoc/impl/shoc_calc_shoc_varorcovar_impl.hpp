...
KOKKOS_FUNCTION void Functions<S, D>::calc_shoc_varorcovar(
    ...
    const uview_1d<const Spack> &tkh_zi, const uview_1d<const Spack> &dz_zi,
    const uview_1d<const Spack> &invar1, const uview_1d<const Spack> &invar2,
  ...
  Kokkos::parallel_for(
      Kokkos::TeamVectorRange(team, nlev_pack), [&](const Int &k) {
        ...
        range_pack2.set(range_pack1 < 1,
                        1); // don't want the shift to go below zero. we mask
                            // out that result anyway
        ...
        if (active_range.any()) {
          ...
          const Spack grid_dz2 =
              ekat::square(grid_dz); // vertical grid diff squared

          // Compute the variance or covariance
          varorcovar(k).set(active_range,
                            tunefac * (isotropy_zi(k) * tkh_zi(k)) * grid_dz2 *
                                (invar1_sm1 - invar1_s) *
                                (invar2_sm1 - invar2_s));
        }
      });
