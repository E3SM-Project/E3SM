...
KOKKOS_FUNCTION void Functions<S, D>::compute_diag_third_shoc_moment(
    ...
    const uview_1d<const Spack> &thetal_zi, const uview_1d<Spack> &w3) {
  ...
  Kokkos::parallel_for(Kokkos::TeamVectorRange(team, nlev_pack), [&](const Int
                                                                         &k) {
    // Constants
    const Scalar a0 = (sp(0.52) * (1 / (c_diag_3rd_mom * c_diag_3rd_mom))) /
                      (c_diag_3rd_mom - 2);
    ...
    // Calculate shifts
    Spack dz_zt_k, dz_zt_km1, wthl_sec_k, wthl_sec_km1, wthl_sec_kp1, thl_sec_k,
        thl_sec_km1, thl_sec_kp1, w_sec_k, w_sec_km1, tke_k, tke_km1;
    ...
    ekat::index_and_shift<-1>(s_wthl_sec, range_pack_m1, wthl_sec_k,
                              wthl_sec_km1);
    ekat::index_and_shift<1>(s_wthl_sec, range_pack_m2, wthl_sec_k,
                             wthl_sec_kp1);
    ...
    if (active_range.any()) {
      ...
      const auto f0 = thedz2 * ekat::cube(bet2) * ((iso * iso) * (iso * iso)) *
                      wthl_sec_k * thl_sec_diff;
      const auto f1 =
          thedz2 * ekat::square(bet2) * ekat::cube(iso) *
      ...
      const auto x0 = (a2 * buoy_sgs2 * (Spack(1) - a3 * buoy_sgs2)) /
                      (Spack(1) - (a1 + a3) * buoy_sgs2);
      const auto y0 = (2 * a2 * buoy_sgs2 * x0) / (Spack(1) - a3 * buoy_sgs2);
      const auto x1 =
          (a0 * f0 + a1 * f1 + a2 * (Spack(1) - a3 * buoy_sgs2) * f2) /
          (Spack(1) - (a1 + a3) * buoy_sgs2);
      const auto y1 = (2 * a2 * (buoy_sgs2 * x1 + (a0 / a1) * f0 + f1)) /
                      (Spack(1) - a3 * buoy_sgs2);
      ...
      w3(k).set(active_range, (aa1 - sp(1.2) * x1 - sp(1.5) * f5) /
                                  (Spack(c_diag_3rd_mom) - sp(1.2) * x0 + aa0));
    }
  });
