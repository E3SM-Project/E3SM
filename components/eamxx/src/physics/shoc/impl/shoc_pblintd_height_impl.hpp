...
KOKKOS_FUNCTION void Functions<S, D>::pblintd_height(
    const MemberType &team, const Int &nlev, const Int &npbl,
  ...
  Kokkos::parallel_reduce(
      Kokkos::TeamVectorRange(team, lower_pack_indx, upper_pack_indx + 1),
      [&](const Int &k, Int &local_max) {
        auto indices_pack = ekat::range<IntSmallPack>(k * Spack::n);
        const auto in_range =
            (indices_pack < nlev - 1 && indices_pack >= nlev - npbl);
        ...
        if (in_range.any()) {
          rino(k).set(in_range, ggr * (thv(k) - thv_ref) *
                                    (z(k) - s_z(nlev - 1)) /
                                    (s_thv(nlev - 1) * vvk));
        }
        ...
        indices_pack.set(rino(k) < ricr || !in_range,
                         Kokkos::reduction_identity<Int>::max());
