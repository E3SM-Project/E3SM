...
KOKKOS_FUNCTION void Functions<S, D>::compute_shr_prod(
    const MemberType &team, const Int &nlevi, const Int &nlev,
  ...
  Kokkos::parallel_for(
      Kokkos::TeamVectorRange(team, nlev_pack), [&](const Int &k) {
        ...
        if (active_range.any()) {
          ...
          ekat::index_and_shift<-1>(sclr_uwind, range_pack2, u_grid,
                                    u_up_grid); // for u_wind
          ekat::index_and_shift<-1>(sclr_vwind, range_pack2, v_grid,
                                    v_up_grid); // for v_wind
          ...
          sterm(k).set(active_range,
                       Ck_sh * (u_grad * u_grad + v_grad * v_grad));
        }
      });
  /*
