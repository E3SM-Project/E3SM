...
KOKKOS_FUNCTION void Functions<S, D>::calc_shoc_vertflux(
    const MemberType &team, const Int &nlev,
    const uview_1d<const Spack> &tkh_zi, const uview_1d<const Spack> &dz_zi,
    const uview_1d<const Spack> &invar, const uview_1d<Spack> &vertflux) {
  ...
  Kokkos::parallel_for(
      Kokkos::TeamVectorRange(team, nlev_pack), [&](const Int &k) {
        ...
        if (active_range.any()) {
          ...
          vertflux(k).set(active_range,
                          -(tkh_zi(k) * grid_dz * (up_grid - grid)));
        }
      });
}
