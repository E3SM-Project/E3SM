...
KOKKOS_FUNCTION void Functions<S, D>::shoc_assumed_pdf(
  ...
  uview_1d<Spack> wthl_sec_zt, wqw_sec_zt, w3_zt, thl_sec_zt, qwthl_sec_zt,
      qw_sec_zt;
  workspace.template take_many_contiguous_unsafe<6>(
      {"wthl_sec_zt", "wqw_sec_zt", "w3_zt", "thl_sec_zt", "qwthl_sec_zt",
       "qw_sec_zt"},
      {&wthl_sec_zt, &wqw_sec_zt, &w3_zt, &thl_sec_zt, &qwthl_sec_zt,
       &qw_sec_zt});
  ...
  linear_interp(team, zi_grid, zt_grid, wthl_sec, wthl_sec_zt, nlevi, nlev,
                largeneg);
  linear_interp(team, zi_grid, zt_grid, qwthl_sec, qwthl_sec_zt, nlevi, nlev,
                largeneg);
  linear_interp(team, zi_grid, zt_grid, wqw_sec, wqw_sec_zt, nlevi, nlev,
                largeneg);
  ...
  // gnu and std=c++14. The macro ConstExceptGnu is defined in
  ...
  Kokkos::parallel_for(
      Kokkos::TeamVectorRange(team, nlev_pack), [&](const Int &k) {
        ...
        // Compute square roots of some variables so we don't have to compute
        // these again
        ...
        shoc_assumed_pdf_vv_parameters(w_first, w2sec, w3var, w_tol_sqd, Skew_w,
                                       w1_1, w1_2, w2_1, w2_2, a);

        // Find parameters for thetal
        Spack thl1_1, thl1_2, thl2_1, thl2_2, sqrtthl2_1, sqrtthl2_2;
        shoc_assumed_pdf_thl_parameters(wthlsec, sqrtw2, sqrtthl, thlsec,
                                        thl_first, w1_1, w1_2, Skew_w, a,
                                        thl_tol, w_thresh, thl1_1, thl1_2,
                                        thl2_1, thl2_2, sqrtthl2_1, sqrtthl2_2);
        ...
        shoc_assumed_pdf_qw_parameters(
            ...
            rt_tol, w_thresh, qw1_1, qw1_2, qw2_1, qw2_2, sqrtqw2_1, sqrtqw2_2);
        ...
        shoc_assumed_pdf_inplume_correlations(
            sqrtqw2_1, sqrtthl2_1, a, sqrtqw2_2, sqrtthl2_2, qwthlsec, qw1_1,
            qw_first, thl1_1, thl_first, qw1_2, thl1_2, r_qwthl_1);
          ...
          for (int i = 0; i < is_nan_Tl1_1.n; i++) {
            if (is_nan_Tl1_1[i] || is_nan_Tl1_2[i]) {
              Kokkos::printf(
                  ...
                  index_range[i], Tl1_1[i], Tl1_2[i], thl_first[i], qw_first[i],
                  pval[i], thlsec[i], qwsec[i], w2sec[i], w_first[i], w3var[i],
                  qwthlsec[i], wqwsec[i], wthlsec[i], a[i], w1_1[i], w1_2[i],
                  w2_1[i], w2_2[i], thl1_1[i], thl1_2[i], thl2_1[i], thl2_2[i],
                  qw1_1[i], qw1_2[i], qw2_1[i], qw2_2[i]);
            }
          }
        }

        // Check to ensure Tl1_1 and Tl1_2 are not excessively small, set to
        ...
        if (is_small_Tl1_1.any()) {
          ...
          Kokkos::printf("WARNING: Tl1_1 has %d values <= allowable value.  "
                         "Resetting to minimum value.\n",
                         n_mask);
        }
        if (is_small_Tl1_2.any()) {
          ...
          Kokkos::printf("WARNING: Tl1_2 has %d values <= allowable value.  "
                         "Resetting to minimum value.\n",
                         n_mask);
        }
        ...
        shoc_assumed_pdf_compute_qs(Tl1_1, Tl1_2, pval, active_entries, qs1,
                                    beta1, qs2, beta2);
        ...
        shoc_assumed_pdf_compute_s(qw1_1, qs1, beta1, pval, thl2_1, qw2_1,
                                   sqrtthl2_1, sqrtqw2_1, r_qwthl_1, s1, std_s1,
                                   qn1, C1);

        // Second plume
        // Only compute variables of the second plume if the two plumes are not
        // equal
        ...
        if (nequal.any()) {
          shoc_assumed_pdf_compute_s(qw1_2, qs2, beta2, pval, thl2_2, qw2_2,
                                     sqrtthl2_2, sqrtqw2_2, r_qwthl_1, s2,
                                     std_s2, qn2, C2);
        }
        ...
        // Compute cloud liquid variance (CLUBB formulation, adjusted to SHOC
        // parameters based)
        ...
        shoc_assumed_pdf_compute_liquid_water_flux(a, w1_1, w_first, ql1, w1_2,
                                                   ql2, wqls(k));
  ...
  workspace.template release_many_contiguous<6>({&wthl_sec_zt, &wqw_sec_zt,
                                                 &w3_zt, &thl_sec_zt,
                                                 &qwthl_sec_zt, &qw_sec_zt});
}
