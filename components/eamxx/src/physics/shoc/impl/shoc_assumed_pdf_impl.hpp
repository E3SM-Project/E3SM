...
KOKKOS_INLINE_FUNCTION constexpr Real safe_min() { return Kokkos::Experimental::norm_min_v<Real>; }
#else
KOKKOS_INLINE_FUNCTION constexpr Real safe_min() { return std::numeric_limits<Real>::min(); }
#endif
...
KOKKOS_FUNCTION void Functions<S, D>::shoc_assumed_pdf(
  ...
  uview_1d<Spack> wthl_sec_zt, wqw_sec_zt, w3_zt, thl_sec_zt, qwthl_sec_zt, qw_sec_zt;
  ...
  Kokkos::parallel_for(Kokkos::TeamVectorRange(team, nlev_pack), [&](const Int &k) {
    ...
    if (is_nan_Tl1_1.any() || is_nan_Tl1_2.any()) {
      ...
      for (int i = 0; i < is_nan_Tl1_1.n; i++) {
        if (is_nan_Tl1_1[i] || is_nan_Tl1_2[i]) {
          Kokkos::printf("Tl1 NaN Detected: lev, Tl1_1, Tl1_2: %d, %16.9e, %16.9e\n"
    ...
    shoc_assumed_pdf_compute_buoyancy_flux(wthlsec, wqwsec, pval, wqls(k), wthv_sec(k));
  });
