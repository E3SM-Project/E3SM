...
KOKKOS_FUNCTION void Functions<S, D>::isotropic_ts(
    const MemberType &team, const Int &nlev, const Scalar &lambda_low_in,
  ...
  Kokkos::parallel_for(
      Kokkos::TeamVectorRange(team, nlev_pack), [&](const Int &k) {
        ...
        Spack lambda(lambda_low +
                     ((brunt_int / ggr) - lambda_thresh) * lambda_slope);
        lambda =
            ekat::max(Spack(lambda_low), ekat::min(Spack(lambda_high), lambda));

        const Spack buoy_sgs_save = brunt(k);
        lambda.set(buoy_sgs_save <= 0,
                   0); // set lambda to zero where buoy_sgs_save <=0

        // Compute the return to isotropic timescale
        isotropy(k) =
            ekat::min(Spack(maxiso), tscale / (1 + lambda * buoy_sgs_save *
                                                       ekat::square(tscale)));
      });
}
