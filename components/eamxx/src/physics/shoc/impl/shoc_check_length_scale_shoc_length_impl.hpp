...
KOKKOS_FUNCTION void Functions<S, D>::check_length_scale_shoc_length(
    const MemberType &team, const Int &nlev, const Scalar &dx, const Scalar &dy,
  ...
  Kokkos::parallel_for(
      Kokkos::TeamVectorRange(team, nlev_pack), [&](const Int &k) {
        ...
        shoc_mix(k) =
            ekat::min(std::sqrt(dx * dy), ekat::max(minlen, shoc_mix(k)));
      });
}
