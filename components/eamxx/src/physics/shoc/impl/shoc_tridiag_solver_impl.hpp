...
KOKKOS_FUNCTION void Functions<S, D>::vd_shoc_decomp(
    ...
    const uview_1d<const Spack> &kv_term, const uview_1d<const Spack> &tmpi,
    const uview_1d<const Spack> &rdp_zt, const Scalar &dtime,
  ...
  Kokkos::parallel_for(
      Kokkos::TeamVectorRange(team, nlev_pack), [&](const Int &k) {
        ...
        // Original code was: auto shift_range = range_pack; but that caused
        // mysterious test failures on blake.
        IntSmallPack shift_range;
        vector_simd for (int s = 0; s < Spack::n; ++s) {
          shift_range[s] = range_pack[s];
        }

        shift_range.set(range_pack > nlev - 1,
                        1); // don't calculate shift above nlev-1
        ...
        // The diagonal elements are a combination of du and dl (d=1-du-dl).
        // Surface fluxes are applied explicitly in the diagonal at the top
        // level.
        d_k = 1 - du_k - dl_k;
...
template <typename S, typename D>
KOKKOS_FUNCTION void Functions<S, D>::vd_shoc_solve(
    const MemberType &team, const uview_1d<Scalar> &du,
