...
void Functions<Real, DefaultDevice>::diag_second_shoc_moments_disp(
    const Int &shcol, const Int &nlev, const Int &nlevi, const Real &thl2tune,
    ...
    const view_2d<Spack> &qw_sec, const view_2d<Spack> &wthl_sec,
    const view_2d<Spack> &wqw_sec, const view_2d<Spack> &qwthl_sec,
  ...
  Kokkos::parallel_for(
      policy, KOKKOS_LAMBDA(const MemberType &team) {
        ...
        diag_second_shoc_moments(
            team, nlev, nlevi, thl2tune, qw2tune, qwthl2tune, w2tune,
            ekat::subview(thetal, i), ekat::subview(qw, i),
            ekat::subview(u_wind, i), ekat::subview(v_wind, i),
            ekat::subview(tke, i), ekat::subview(isotropy, i),
            ekat::subview(tkh, i), ekat::subview(tk, i),
            ekat::subview(dz_zi, i), ekat::subview(zt_grid, i),
            ekat::subview(zi_grid, i), ekat::subview(shoc_mix, i), wthl_sfc(i),
            wqw_sfc(i), uw_sfc(i), vw_sfc(i), ustar2(i), wstar(i), workspace,
            ekat::subview(thl_sec, i), ekat::subview(qw_sec, i),
            ekat::subview(wthl_sec, i), ekat::subview(wqw_sec, i),
            ekat::subview(qwthl_sec, i), ekat::subview(uw_sec, i),
            ekat::subview(vw_sec, i), ekat::subview(wtke_sec, i),
