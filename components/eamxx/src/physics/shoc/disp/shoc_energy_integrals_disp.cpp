...
void Functions<Real, DefaultDevice>::shoc_energy_integrals_disp(
    ...
    const uview_2d<const Spack> &v_wind, const view_1d<Scalar> &se_b,
    const view_1d<Scalar> &ke_b, const view_1d<Scalar> &wv_b,
  ...
  Kokkos::parallel_for(
      policy, KOKKOS_LAMBDA(const MemberType &team) {
        ...
        shoc_energy_integrals(team, nlev, ekat::subview(host_dse, i),
                              ekat::subview(pdel, i), ekat::subview(rtm, i),
                              ekat::subview(rcm, i), ekat::subview(u_wind, i),
                              ekat::subview(v_wind, i), se_b(i), ke_b(i),
                              wv_b(i), wl_b(i));
      });
}
