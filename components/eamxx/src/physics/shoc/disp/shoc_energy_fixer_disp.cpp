...
void Functions<Real, DefaultDevice>::shoc_energy_fixer_disp(
  ...
  Kokkos::parallel_for(
      policy, KOKKOS_LAMBDA(const MemberType &team) {
        ...
        shoc_energy_fixer(
            team, nlev, nlevi, dtime, nadv, ekat::subview(zt_grid, i),
            ekat::subview(zi_grid, i), se_b(i), ke_b(i), wv_b(i), wl_b(i),
            se_a(i), ke_a(i), wv_a(i), wl_a(i), wthl_sfc(i), wqw_sfc(i),
            ekat::subview(rho_zt, i), ekat::subview(tke, i),
            ekat::subview(pint, i), workspace, ekat::subview(host_dse, i));
      });
}
