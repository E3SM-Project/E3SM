...
class SHOCMacrophysics : public scream::AtmosphereProcess {
  ...
  using sview_2d_const =
      typename KokkosTypes<DefaultDevice>::template view_2d<const Real>;
...
public:
  ...
  // run_impl in the main .cpp code. Structure to handle the local generation of
  // data needed by shoc_main in run_impl
  struct SHOCPreprocess {
    ...
    void operator()(
      ...
      Kokkos::parallel_for(
          Kokkos::TeamVectorRange(team, nlev_packs), [&](const Int &k) {
            ...
            // Inverse of Exner. In non-rel builds, assert that exner != 0 when
            // in range before computing.
            const Spack exner   = PF::exner_function(p_mid(i, k));
            const Smask nonzero = (exner != 0);
            EKAT_KERNEL_ASSERT(
                (nonzero || !(ekat::range<IntSmallPack>(k * Spack::n) < nlev))
                    .all());
            inv_exner(i, k).set(nonzero, 1 / exner);

            tke(i, k) = ekat::max(mintke, tke(i, k));

            // Tracers are updated as a group. The tracers tke and qc act as
            // separate inputs to shoc_main() and are therefore updated
            // differently to the tracers group's monolithic field. Here, we
            // make a copy if each of these tracers and pass to shoc_main() so
            // that changes to the tracer group does not alter tke or qc values.
            // Then during post processing, we copy back correct values of tke
            // and qc to tracer group in postprocessing.
            // TODO: remove *_copy views once SHOC can request a subset of
            // tracers.
            ...
            const auto theta_zt =
                PF::calculate_theta_from_T(T_mid(i, k), p_mid(i, k));
            thlm(i, k) = PF::calculate_thetal_from_theta(theta_zt, T_mid(i, k),
                                                         qc(i, k));
            thv(i, k)  = theta_zt * (1 + zvir * qv(i, k) - qc(i, k));

            // Vertical layer thickness
            dz(i, k) = PF::calculate_dz(pseudo_density(i, k), p_mid(i, k),
                                        T_mid(i, k), qv(i, k));
      ...
      SHF::linear_interp(team, zt_grid_s, zi_grid_s, rrho_s, rrho_i_s, nlev,
                         nlev + 1, 0);
      team.team_barrier();
      ...
      wpthlp_sfc(i) =
          (surf_sens_flux(i) / (cpair * rrho_i(i, nlevi_v)[nlevi_p])) *
          inv_exner_int_surf;
    ...
    void set_variables(
        const int ncol_, const int nlev_, const int num_qtracers_,
        const Real z_surf_, const view_2d_const &T_mid_,
        const view_2d_const &p_mid_, const view_2d_const &p_int_,
        const view_2d_const &pseudo_density_, const view_2d_const &omega_,
        const view_1d_const &phis_, const view_1d_const &surf_sens_flux_,
        const view_1d_const &surf_evap_, const sview_2d_const &surf_mom_flux_,
        const view_3d_strided &qtracers_, const view_2d &qv_,
        const view_2d_const &qc_, const view_2d &qc_copy_, const view_2d &tke_,
        const view_2d &tke_copy_, const view_2d &z_mid_, const view_2d &z_int_,
        const view_2d &dse_, const view_2d &rrho_, const view_2d &rrho_i_,
        const view_2d &thv_, const view_2d &dz_, const view_2d &zt_grid_,
        const view_2d &zi_grid_, const view_1d &wpthlp_sfc_,
        const view_1d &wprtp_sfc_, const view_1d &upwp_sfc_,
        const view_1d &vpwp_sfc_, const view_2d &wtracer_sfc_,
        const view_2d &wm_zt_, const view_2d &inv_exner_, const view_2d &thlm_,
        const view_2d &qw_, const view_2d &cldfrac_liq_,
  ...
  // based on output from shoc_main.
  struct SHOCPostprocess {
    ...
    void operator()(
      ...
      Kokkos::parallel_for(
          Kokkos::TeamVectorRange(team, nlev_packs), [&](const Int &k) {
            // See comment in SHOCPreprocess::operator() about the necessity of
            // *_copy views
            ...
            // wet<->dry conversion is a constant factor that cancels out in
            // mean(qc)^2/mean(qc'*qc').
            inv_qc_relvar(i, k)  = 1;
            ...
            T_mid(i, k) =
                PF::calculate_temperature_from_dse(dse_ik, z_mid_ik, phis_i);
          });

      // If necessary, set appropriate boundary fluxes for energy and mass
      // conservation checks. Any boundary fluxes not included in SHOC interface
      // are set to 0.
    ...
    void set_variables(const int ncol_, const int nlev_,
                       const int num_qtracers_, const view_2d_const &rrho_,
                       const view_2d &qv_, const view_2d_const &qw_,
                       const view_2d &qc_, const view_2d_const &qc_copy_,
                       const view_2d &tke_, const view_2d_const &tke_copy_,
