...
// =========================================================================================
SHOCMacrophysics::SHOCMacrophysics(const ekat::Comm &comm,
                                   const ekat::ParameterList &params)
    : AtmosphereProcess(comm, params) {
  /* Anything that can be initialized without grid information can be
   * initialized here. Like universal constants, shoc options.
   */
}

// =========================================================================================
void SHOCMacrophysics::set_grids(
  ...
  // Note: shoc_main is organized by a set of 5 structures, variables below are
  // organized
  ...
  if (m_params.get<bool>("apply_tms", false)) {
    add_field<Required>("surf_drag_coeff_tms", scalar2d, kg / (m2 * s),
                        grid_name);
  }
  ...
  add_field<Computed>("inv_qc_relvar", scalar3d_mid, pow(kg / kg, 2), grid_name,
                      ps);
  ...
  if (m_params.get<bool>("extra_shoc_diags", false)) {
    ...
    add_field<Computed>("wqw_sec", scalar3d_int, (kg / kg) * (m / s), grid_name,
                        ps);
  ...
  // Tracer group
  add_group<Updated>("turbulence_advected_tracers", grid_name, ps,
...
void SHOCMacrophysics::set_computed_group_impl(const FieldGroup &group) {
  ...
  EKAT_REQUIRE_MSG(name == "turbulence_advected_tracers",
                   "Error! We were not expecting a field group called '"
                       << name << "\n");
...
size_t SHOCMacrophysics::requested_buffer_size_in_bytes() const {
  ...
  const size_t wsm_request = WSM::get_total_bytes_needed(
...
void SHOCMacrophysics::init_buffers(const ATMBufferManager &buffer_manager) {
  ...
  for (int i = 0; i < Buffer::num_2d_vector_mid; ++i) {
    *_2d_spack_mid_view_ptrs[i] =
        spack_2d_view_t(s_mem, m_num_cols, nlev_packs);
    s_mem += _2d_spack_mid_view_ptrs[i]->size();
  }

  for (int i = 0; i < Buffer::num_2d_vector_int; ++i) {
    *_2d_spack_int_view_ptrs[i] =
        spack_2d_view_t(s_mem, m_num_cols, nlevi_packs);
    s_mem += _2d_spack_int_view_ptrs[i]->size();
  }
  m_buffer.wtracer_sfc =
      decltype(m_buffer.wtracer_sfc)(s_mem, m_num_cols, num_tracer_packs);
  ...
  const int n_trac_slots = ekat::npack<Spack>(m_num_tracers + 3) * Spack::n;
  const int wsm_size =
      WSM::get_total_bytes_needed(nlevi_packs,
                                  14 + (n_wind_slots + n_trac_slots), policy) /
      sizeof(Spack);
  s_mem += wsm_size;
  ...
  EKAT_REQUIRE_MSG(
      ...
      "Error! Used memory != requested memory for SHOCMacrophysics.");
}
...
void SHOCMacrophysics::initialize_impl(const RunType run_type) {
  ...
  const auto &surf_sens_flux =
      get_field_in("surf_sens_flux").get_view<const Real *>();
  const auto &surf_evap = get_field_in("surf_evap").get_view<const Real *>();
  const auto &surf_mom_flux =
      get_field_in("surf_mom_flux").get_view<const Real **>();
  ...
  const auto &cldfrac_liq_prev =
      get_field_out("cldfrac_liq_prev").get_view<Spack **>();
  const auto &sgs_buoy_flux =
      get_field_out("sgs_buoy_flux").get_view<Spack **>();
  const auto &tk = get_field_out("eddy_diff_mom").get_view<Spack **>();
  const auto &inv_qc_relvar =
      get_field_out("inv_qc_relvar").get_view<Spack **>();
  ...
  shoc_preprocess.set_variables(
      m_num_cols, m_num_levs, m_num_tracers, z_surf, T_mid, p_mid, p_int,
      pseudo_density, omega, phis, surf_sens_flux, surf_evap, surf_mom_flux,
      qtracers, qv, qc, qc_copy, tke, tke_copy, z_mid, z_int, dse, rrho, rrho_i,
      thv, dz, zt_grid, zi_grid, wpthlp_sfc, wprtp_sfc, upwp_sfc, vpwp_sfc,
  ...
  shoc_postprocess.set_variables(m_num_cols, m_num_levs, m_num_tracers, rrho,
                                 qv, qw, qc, qc_copy, tke, tke_copy, qtracers,
                                 shoc_ql2, cldfrac_liq, inv_qc_relvar, T_mid,
                                 dse, z_mid, phis);
  ...
  if (has_column_conservation_check()) {
    ...
    shoc_postprocess.set_mass_and_energy_fluxes(
        surf_evap, surf_sens_flux, vapor_flux, water_flux, ice_flux, heat_flux);
  }
  ...
  add_postcondition_check<Interval>(get_field_out("T_mid"), m_grid, 100.0,
                                    500.0, false);
  add_postcondition_check<Interval>(get_field_out("qc"), m_grid, 0.0, 0.1,
                                    false);
  ...
  add_postcondition_check<Interval>(get_field_out("cldfrac_liq"), m_grid, 0.0,
                                    1.0, false);
  ...
  const auto default_policy =
      ekat::ExeSpaceUtils<KT::ExeSpace>::get_default_team_policy(m_num_cols,
                                                                 nlev_packs);
  workspace_mgr.setup(m_buffer.wsm_data, nlevi_packs,
                      14 + (n_wind_slots + n_trac_slots), default_policy);
  ...
  Kokkos::parallel_for(
      Kokkos::RangePolicy<>(0, m_num_levs), KOKKOS_LAMBDA(const int lev) {
  ...
  if (m_grid->has_geometry_data("dx_short")) {
    ...
    auto dx =
        m_grid->get_geometry_data("dx_short").get_view<const Real, Host>()();
    Kokkos::deep_copy(cell_length, dx * 1000); // convert km -> m
  } else {
    const auto area =
        m_grid->get_geometry_data("area").get_view<const Real *>();
...
void SHOCMacrophysics::run_impl(const double dt) {
  EKAT_REQUIRE_MSG(dt <= 300, "Error! SHOC is intended to run with a timestep "
                              "no longer than 5 minutes.\n"
                              "       Please, reduce timestep (perhaps "
                              "increasing subcycling iterations).\n");
  ...
  Kokkos::parallel_for("shoc_preprocess", scan_policy, shoc_preprocess);
  Kokkos::fence();
  ...
  // Run shoc main
  SHF::shoc_main(m_num_cols, m_num_levs, m_num_levs + 1, m_npbl, m_nadv,
                 m_num_tracers, dt, workspace_mgr, runtime_options, input,
                 input_output, output, history_output
#ifdef SCREAM_SHOC_SMALL_KERNELS
                 ,
                 temporaries
#endif
  ...
  Kokkos::parallel_for("shoc_postprocess", default_policy, shoc_postprocess);
  Kokkos::fence();
...
void SHOCMacrophysics::apply_turbulent_mountain_stress() {
  auto surf_drag_coeff_tms =
      get_field_in("surf_drag_coeff_tms").get_view<const Real *>();
  ...
  Kokkos::parallel_for(
      "apply_tms", KT::RangePolicy(0, m_num_cols), KOKKOS_LAMBDA(const int i) {
        upwp_sfc(i) -= surf_drag_coeff_tms(i) *
                       horiz_winds(i, 0, nlev_v)[nlev_p] /
                       rrho_i(i, nlevi_v)[nlevi_p];
        vpwp_sfc(i) -= surf_drag_coeff_tms(i) *
                       horiz_winds(i, 1, nlev_v)[nlev_p] /
                       rrho_i(i, nlevi_v)[nlevi_p];
      });
}
// =========================================================================================
void SHOCMacrophysics::check_flux_state_consistency(const double dt) {
  ...
  const Real qmin =
      1e-12; // minimum permitted constituent concentration (kg/kg)
  ...
  const auto policy =
      ekat::ExeSpaceUtils<KT::ExeSpace>::get_default_team_policy(m_num_cols,
                                                                 nlev_packs);
  Kokkos::parallel_for(
      "check_flux_state_consistency", policy,
      KOKKOS_LAMBDA(const KT::MemberType &team) {
        ...
        if (condition < 0) {
          ...
          Real mm = ekat::ExeSpaceUtils<KT::ExeSpace>::view_reduction(
              team, 0, nlevs, tracer_mass);

          EKAT_KERNEL_ASSERT_MSG(mm >= cc,
                                 "Error! Total mass of column vapor should be "
                                 "greater than mass of surf_evap.\n");

          Kokkos::parallel_for(
              Kokkos::TeamVectorRange(team, nlev_packs), [&](const int &k) {
                const auto adjust = cc * qv_i(k) * pseudo_density_i(k) / mm;
                qv_i(k)           = (qv_i(k) * pseudo_density_i(k) - adjust) /
                          pseudo_density_i(k);
              });
