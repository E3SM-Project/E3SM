...
void compute_w0_and_rho(haero::ThreadTeamPolicy team_policy,
  ...
  Kokkos::parallel_for(
      team_policy, KOKKOS_LAMBDA(const haero::ThreadTeam &team) {
        ...
        Kokkos::parallel_for(Kokkos::TeamVectorRange(team, 0u, top_lev), [&](int kk) {
          w0(icol, kk)  = 0;
          rho(icol, kk) = -999.0;
        });
        Kokkos::parallel_for(Kokkos::TeamVectorRange(team, top_lev, nlev), [&](int kk) {
...
void call_function_dropmixnuc(
    ...
    // ## work arrays ##
    MAMAci::view_2d raercol_cw[mam4::ndrop::pver][2], MAMAci::view_2d raercol[mam4::ndrop::pver][2],
    MAMAci::view_3d state_q_work, MAMAci::view_3d nact, MAMAci::view_3d mact,
          ...
          for (int m = 0; m < mam_coupling::num_aero_modes(); ++m) {
            ...
            for (int a = 0; a < mam_coupling::num_aero_species(); ++a) {
              if (dry_aero.cld_aero_mmr[m][a].data()) {
                qqcw_view[ind_qqcw](klev) = dry_aero.cld_aero_mmr[m][a](icol, klev);
                ++ind_qqcw;
              }
            }
          }
...
void update_interstitial_aerosols(haero::ThreadTeamPolicy team_policy,
  ...
  for (int m = 0; m < mam_coupling::num_aero_modes(); ++m) {
    ...
    auto aero_nmr         = dry_aero.int_aero_nmr[m]; // number mixing ratio for mode "m"
    const auto ptend_view = ptend_q[s_idx];
    Kokkos::parallel_for(
        team_policy, KOKKOS_LAMBDA(const haero::ThreadTeam &team) {
          const int icol = team.league_rank();
...
void call_hetfrz_compute_tendencies(haero::ThreadTeamPolicy team_policy, mam4::Hetfrz &hetfrz_,
  ...
  Kokkos::parallel_for(
      team_policy, KOKKOS_LAMBDA(const haero::ThreadTeam &team) {
        ...
        diags.activation_fraction[accum_idx] = ekat::subview(factnum, icol, accum_idx);

        diags.activation_fraction[coarse_idx] = ekat::subview(factnum, icol, coarse_idx);
        ...
        diags.stratiform_cloud_fraction = ekat::subview(diagnostic_scratch[42], icol);
        ...
        const mam4::AeroConfig aero_config;
        const Real t = 0; // not used
        hetfrz.compute_tendencies(aero_config, team, t, dt, haero_atm, surf, progs, diags, tends);
      });
}
} // namespace
} // namespace scream
