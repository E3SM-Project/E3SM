...
void compute_nucleate_ice_tendencies(
    ...
    MAMAci::view_2d nihf, 
    MAMAci::view_2d niim, 
    MAMAci::view_2d nidep, 
    MAMAci::view_2d nimey, 
    ...
    const int nlev)
{
  ...
  Kokkos::parallel_for(team_policy, KOKKOS_LAMBDA(const haero::ThreadTeam &team) {
    ...
    haero::Surface surf{};
    mam4::Prognostics progs = mam_coupling::aerosols_for_column(aerosol_state, icol);
...
void call_function_dropmixnuc(
  ...
  MAMAci::view_2d ndropmix,
  MAMAci::view_2d nsource,
  MAMAci::view_2d wtke,
  MAMAci::view_3d ccn,
  MAMAci::view_3d nact,
  MAMAci::view_3d mact,
  ...
  Kokkos::parallel_for(team_policy, KOKKOS_LAMBDA(const haero::ThreadTeam &team) {
    ...
    */
    mam4::ndrop::dropmixnuc(
     ...
     ekat::subview(cloud_frac_new, icol),
  ...
  const int nlev)
{
  ...
  Kokkos::parallel_for(team_policy, KOKKOS_LAMBDA(const haero::ThreadTeam &team) {
    ...
    const mam4::AeroConfig aero_config;
    const Real t=0, dt=0;
    hetfrz.compute_tendencies(aero_config, team, t, dt, atmos, surf, progs, diags, tends);
  });
}
}
...
void MAMAci::set_grids(const std::shared_ptr<const GridsManager> grids_manager) {
  ...
  add_field<Updated> ("qv",             scalar3d_layout_mid, q_unit, grid_name); // Water vapor mixing ratio [kg vapor / kg dry air]
  ...
  //
  add_field<Updated> ("w_updraft",      scalar3d_layout_mid, q_unit, grid_name); // updraft velocity [m/s]
  ...
  add_field<Computed>("icenuc_num_hetfrz",scalar3d_layout_mid , 1/m/m/m,   grid_name); // number conc of ice nuclei due to heterogeneous freezing [1/m3]
  add_field<Computed>("icenuc_num_immfrz",scalar3d_layout_mid , 1/m/m/m,   grid_name); // number conc of ice nuclei due to immersion freezing (hetero nuc) [1/m3]
  add_field<Computed>("icenuc_num_depnuc",scalar3d_layout_mid , 1/m/m/m,   grid_name); // number conc of ice nuclei due to deposition nucleation (hetero nuc)[1/m3]
  add_field<Computed>("icenuc_num_meydep",scalar3d_layout_mid , 1/m/m/m,   grid_name); // number conc of ice nuclei due to meyers deposition [1/m3]
  add_field<Computed>("num_act_aerosol_ice_nucle_hom",scalar3d_layout_mid , n_unit, grid_name); // number of activated aerosol for ice nucleation (homogeneous freezing only) [#/kg]
  add_field<Computed>("num_act_aerosol_ice_nucle",scalar3d_layout_mid , n_unit, grid_name); // number of activated aerosol for ice nucleation[#/kg]
  ...
  add_field<Computed>("factnum", FieldLayout{ {COL, LEV, CMP}, {ncol_, nlev_, mam_coupling::num_aero_modes()}}, nondim, grid_name); // activation fraction for aerosol number [fraction]
  ...
  FieldLayout scalar4d_layout_mid{ {COL, LEV, NUM_MODES}, {ncol_, nlev_, num_aero_modes} }; // mid points
  add_field<Required>("dgnum", scalar4d_layout_mid, m, grid_name); // dry diameter of aerosols
 

  auto cm = m/100;
  auto frz_unit = 1/(cm*cm*cm*s); // units of number mixing ratios of tracers
  n_unit.set_string("1(cm^-3 s^-1)");

  add_field<Computed>("hetfrz_immersion_nucleation_tend", scalar3d_layout_mid, frz_unit, grid_name); // dry diameter of aerosols
  add_field<Computed>("hetfrz_contact_nucleation_tend", scalar3d_layout_mid, frz_unit, grid_name); // dry diameter of aerosols
  add_field<Computed>("hetfrz_depostion_nucleation_tend", scalar3d_layout_mid, frz_unit, grid_name); // dry diameter of aerosols
  //
  /* 
...
void MAMAci::initialize_impl(const RunType run_type) {
  ...
  naai_hom_ = get_field_out("num_act_aerosol_ice_nucle_hom").get_view<Real**>();
  ...
  hetfrz_depostion_nucleation_tend_ = get_field_out("hetfrz_depostion_nucleation_tend").get_view<Real**>();
...
void MAMAci::run_impl(const double dt) {
  ...
  compute_w0_and_rho(team_policy, w0_, rho_, wet_atmosphere_, dry_atmosphere_, top_lev_, nlev_);
  ...
  //-------------------------------------------------------------
  /*NOTE: We probably need to store indices for the select few cloud borne aerosols
  Fortran code:
