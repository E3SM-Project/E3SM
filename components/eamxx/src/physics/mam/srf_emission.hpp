...
template <typename ScalarType, typename DeviceType> struct srfEmissFunctions {
  ...
  struct srfEmissData {
    ...
    srfEmissData(const int ncol_, const int nsectors_) : ncols(ncol_), nsectors(nsectors_) {
  ...
  struct srfEmissInput {
    ...
    srfEmissInput(const int ncols_, const int nsectors_) { init(ncols_, nsectors_); }

    void init(const int ncols_, const int nsectors_) { data.init(ncols_, nsectors_, true); }
  ...
  static void srfEmiss_main(const srfEmissTimeState &time_state, const srfEmissInput &data_beg,
