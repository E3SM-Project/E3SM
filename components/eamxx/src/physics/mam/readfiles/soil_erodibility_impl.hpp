...
soilErodibilityFunctions<S, D>::create_horiz_remapper(const std::shared_ptr<const AbstractGrid> &model_grid,
  ...
  // code)
  auto horiz_interp_tgt_grid = model_grid->clone("soil_erodibility_horiz_interp_tgt_grid", true);
  ...
  Field soil_erodibility(FieldIdentifier(field_name, layout_2d, nondim, tgt_grid->name()));
...
void soilErodibilityFunctions<S, D>::update_soil_erodibility_data_from_file(
  ...
  // 1. Read from file
  start_timer("EAMxx::soilErodibility::update_soil_erodibility_data_from_file::read_"
              "data");
  scorpio_reader->read_variables();
  stop_timer("EAMxx::soilErodibility::update_soil_erodibility_data_from_file::read_"
             "data");

  // 2. Run the horiz remapper (it is a do-nothing op if soilErodibility data is
  // on same grid as model)
  start_timer("EAMxx::soilErodibility::update_soil_erodibility_data_from_file::horiz_"
              "remap");
  horiz_interp.remap_fwd();
  stop_timer("EAMxx::soilErodibility::update_soil_erodibility_data_from_file::horiz_"
             "remap");

  // 3. Get the tgt field of the remapper
  start_timer("EAMxx::soilErodibility::update_soil_erodibility_data_from_file::get_"
              "field");
  ...
  stop_timer("EAMxx::soilErodibility::update_soil_erodibility_data_from_file::get_"
             "field");
