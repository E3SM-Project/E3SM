...
std::shared_ptr<AbstractRemapper> fracLandUseFunctions<S, D>::create_horiz_remapper(
  ...
  Field fractional_land_use(FieldIdentifier(field_name, layout_2d, nondim, tgt_grid->name()));
...
void fracLandUseFunctions<S, D>::update_frac_land_use_data_from_file(
  ...
  // 1. Read from file
  start_timer("EAMxx::FracLandUse::update_frac_land_use_data_from_file::read_data");
  scorpio_reader->read_variables();
  stop_timer("EAMxx::FracLandUse::update_frac_land_use_data_from_file::read_data");

  // 2. Run the horiz remapper (it is a do-nothing op if FracLandUse data is on
  // same grid as model)
  start_timer("EAMxx::FracLandUse::update_frac_land_use_data_from_file::horiz_remap");
  horiz_interp.remap_fwd();
  stop_timer("EAMxx::FracLandUse::update_frac_land_use_data_from_file::horiz_remap");

  // 3. Get the tgt field of the remapper
  start_timer("EAMxx::FracLandUse::update_frac_land_use_data_from_file::get_field");
  ...
  stop_timer("EAMxx::FracLandUse::update_frac_land_use_data_from_file::get_field");
