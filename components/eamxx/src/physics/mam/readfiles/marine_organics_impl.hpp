...
std::shared_ptr<AbstractRemapper> marineOrganicsFunctions<S, D>::create_horiz_remapper(
  ...
  //  code)
  auto horiz_interp_tgt_grid = model_grid->clone("marine_organics_horiz_interp_tgt_grid", true);
  ...
  if (ncols_data == ncols_model) {
    ...
    EKAT_REQUIRE_MSG(map_file != "", "ERROR: marine organics data is on a different grid "
                                     "than the model one,\n"
...
void marineOrganicsFunctions<S, D>::update_marine_organics_data_from_file(
  ...
  // 1. Read from file
  start_timer("EAMxx::marineOrganics::update_marine_organics_data_from_file::read_"
              "data");
  scorpio_reader->read_variables();
  stop_timer("EAMxx::marineOrganics::update_marine_organics_data_from_file::read_"
             "data");

  // 2. Run the horiz remapper (it is a do-nothing op if marineOrganics data is
  // on same grid as model)
  start_timer("EAMxx::marineOrganics::update_marine_organics_data_from_file::horiz_"
              "remap");
  horiz_interp.remap_fwd();
  stop_timer("EAMxx::marineOrganics::update_marine_organics_data_from_file::horiz_"
             "remap");

  // 3. Get the tgt field of the remapper
  start_timer("EAMxx::marineOrganics::update_marine_organics_data_from_file::get_"
              "field");
  ...
  stop_timer("EAMxx::marineOrganics::update_marine_organics_data_from_file::get_"
             "field");
...
void marineOrganicsFunctions<S, D>::update_marine_organics_timestate(
  ...
  if (month != time_state.current_month) {
    ...
    update_marine_organics_data_from_file(scorpio_reader, ts, next_month, horiz_interp, end);
  }
...
void marineOrganicsFunctions<S, D>::perform_time_interpolation(const marineOrganicsTimeState &time_state,
  ...
  Kokkos::parallel_for(
      policy, KOKKOS_LAMBDA(const MemberType &team) {
        ...
        Kokkos::parallel_for(Kokkos::TeamVectorRange(team, 0u, nsectors), [&](int isec) {
          const auto beg                     = data_beg.data.emiss_sectors(isec, icol);
          const auto end                     = data_end.data.emiss_sectors(isec, icol);
          data_out.emiss_sectors(isec, icol) = linear_interp(beg, end, delta_t_fraction);
        });
      });
  Kokkos::fence();
...
// -------------------------------------------------------------------------------------------
template <typename S, typename D>
void marineOrganicsFunctions<S, D>::marineOrganics_main(const marineOrganicsTimeState &time_state,
                                                        const marineOrganicsInput &data_beg,
                                                        const marineOrganicsInput &data_end,
                                                        const marineOrganicsOutput &data_out) {
...
void marineOrganicsFunctions<S, D>::init_marine_organics_file_read(
  ...
  marineOrganicsDataReader = create_data_reader(marineOrganicsHorizInterp, data_file);
