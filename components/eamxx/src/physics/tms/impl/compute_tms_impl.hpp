...
void Functions<S, D>::compute_tms(
  ...
  const Scalar horomin =
      1; // Minimum value of subgrid orographic height for mountain stress [ m ]
  const Scalar z0max  = 100;  // Maximum value of z_0 for orography [ m ]
  const Scalar dv2min = 0.01; // Minimum shear squared [ m2/s2 ]
  const Scalar orocnst =
      C::orocnst; // Converts from standard deviation to height [ no unit ]
  const Scalar z0fac = C::z0fac;   // Factor determining z_0 from orographic
  ...
  Kokkos::parallel_for(
      policy, KOKKOS_LAMBDA(const int &i) {
        // Subview on column, scalarize since we only care about last 2 levels
        // (never loop over levels)
        ...
        if (horo < horomin) {
          ...
          const auto tmp =
              karman / std::log((z_mid_i(nlevs - 1) + z0oro) / z0oro);
          auto cd = tmp * tmp;
          ...
          const auto dv2 =
              ekat::impl::max(tmp_u * tmp_u + tmp_v * tmp_v, dv2min);

          const auto ri =
              2 * gravit *
              (t_mid_i(kt) * exner_i(kt) - t_mid_i(kb) * exner_i(kb)) *
              (z_mid_i(kt) - z_mid_i(kb)) /
              ((t_mid_i(kt) * exner_i(kt) + t_mid_i(kb) * exner_i(kb)) * dv2);

          // Calculate the instability function and modify the neutral drag
          // cofficient. We should probably follow more elegant approach like
          // Louis et al (1982) or Bretherton and Park (2009) but for now we use
          // very crude approach : just 1 for ri < 0, 0 for ri > 1, and linear
          // ramping.
          const auto stabfri =
              ekat::impl::max(0.0, ekat::impl::min(1.0, 1.0 - ri));
          cd *= stabfri;

          // Compute density, velocity magnitude and stress using bottom level
          // properties
          const auto rho = p_mid_i(nlevs - 1) / (rair * t_mid_i(nlevs - 1));
          const auto vmag =
              std::sqrt(u_wind_i(nlevs - 1) * u_wind_i(nlevs - 1) +
