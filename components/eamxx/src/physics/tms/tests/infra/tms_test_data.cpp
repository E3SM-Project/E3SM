...
//
void compute_tms_f(int ncols, int nlevs, Real *u_wind, Real *v_wind,
                   Real *t_mid, Real *p_mid, Real *exner, Real *z_mid,
                   Real *sgh, Real *landfrac, Real *ksrf, Real *taux,
                   Real *tauy) {
  ...
  ekat::host_to_device({u_wind, v_wind, t_mid, p_mid, exner, z_mid}, ncols,
                       nlevs, temp_d_2d, true);

  view_1d sgh_d(temp_d_1d[0]), landfrac_d(temp_d_1d[1]),
      ksrf_d("ksrf_d", ncols), taux_d("taux_d", ncols), tauy_d("tauy_d", ncols);
  ...
  const auto policy =
      ekat::ExeSpaceUtils<ExeSpace>::get_default_team_policy(ncols, nlev_packs);
  Kokkos::parallel_for(
  ...
  TMSFunc::compute_tms(ncols, nlevs, ekat::scalarize(horiz_wind_d),
                       ekat::scalarize(t_mid_d), ekat::scalarize(p_mid_d),
                       ekat::scalarize(exner_d), ekat::scalarize(z_mid_d),
