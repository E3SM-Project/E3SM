...
// =========================================================================================
TurbulentMountainStress::TurbulentMountainStress(
...
// =========================================================================================
void TurbulentMountainStress::set_grids(
  ...
  add_field<Computed>("surf_drag_coeff_tms", scalar2d, kg / (m2 * s),
                      grid_name);
...
void TurbulentMountainStress::run_impl(const double /* dt */) {
  ...
  const auto horiz_winds =
      get_field_in("horiz_winds").get_view<const Spack ***>();
  ...
  // Output views
  const auto surf_drag_coeff_tms =
      get_field_out("surf_drag_coeff_tms").get_view<Real *>();
  const auto wind_stress_tms =
      get_field_out("wind_stress_tms").get_view<Real **>();
  ...
  // special policy
  const auto scan_policy = ekat::ExeSpaceUtils<TMSFunctions::KT::ExeSpace>::
      get_thread_range_parallel_scan_team_policy(ncols, nlev_packs);
  Kokkos::parallel_for(
  ...
  // Compute TMS
  TMSFunctions::compute_tms(
      ncols, nlevs, ekat::scalarize(horiz_winds), ekat::scalarize(T_mid),
...
// =========================================================================================
void TurbulentMountainStress::init_buffers(
