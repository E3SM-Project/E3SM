...
OpticalProps2str get_cloud_optics_sw(const int ncol, const int nlay,
                                     CloudOptics &cloud_optics,
                                     GasOpticsRRTMGP &kdist, real2d &lwp,
                                     real2d &iwp, real2d &rel, real2d &rei) {
  ...
  limit_to_bounds(rel, cloud_optics.radliq_lwr, cloud_optics.radliq_upr,
                  rel_limited);
  limit_to_bounds(rei, cloud_optics.radice_lwr, cloud_optics.radice_upr,
                  rei_limited);

  // Calculate cloud optics
  cloud_optics.cloud_optics(ncol, nlay, lwp, iwp, rel_limited, rei_limited,
                            clouds);
...
OpticalProps1scl get_cloud_optics_lw(const int ncol, const int nlay,
                                     CloudOptics &cloud_optics,
                                     GasOpticsRRTMGP &kdist, real2d &lwp,
                                     real2d &iwp, real2d &rel, real2d &rei) {
  ...
  clouds.alloc_1scl(
      ncol,
      nlay); // this is dumb, why do we need to init and alloc separately?!
...
OpticalProps2str get_subsampled_clouds(const int ncol, const int nlay,
                                       const int nbnd, const int ngpt,
                                       OpticalProps2str &cloud_optics,
                                       GasOpticsRRTMGP &kdist, real2d &cld,
                                       real2d &p_lay) {
  // Initialized subsampled optics
  OpticalProps2str subsampled_optics;
  subsampled_optics.init(kdist.get_band_lims_wavenumber(),
                         kdist.get_band_lims_gpoint(), "subsampled_optics");
  subsampled_optics.alloc_2str(ncol, nlay);
  // Check that we do not have clouds with no optical properties; this would get
  // corrected when we assign optical props, but we want to use a "radiative
  // cloud fraction" for the subcolumn sampling too because otherwise we can get
  // vertically-contiguous cloud mask profiles with no actual cloud properties
  // in between, which would just further overestimate the vertical correlation
  // of cloudy layers. I.e., cloudy layers might look maximally overlapped even
  // when separated by layers with no cloud properties, when in fact those
  // layers should be randomly overlapped.
  auto cldfrac_rad = real2d("cldfrac_rad", ncol, nlay);
  memset(cldfrac_rad, 0.0); // Start with all zeros
  TIMED_KERNEL(parallel_for(
      SimpleBounds<3>(nbnd, nlay, ncol),
      YAKL_LAMBDA(int ibnd, int ilay, int icol) {
        if (cloud_optics.tau(icol, ilay, ibnd) > 0) {
          cldfrac_rad(icol, ilay) = cld(icol, ilay);
        }
      }));
  // Get subcolumn cloud mask; note that get_subcolumn_mask exposes overlap
  // assumption as an option, but the only currently supported options are 0
  // (trivial all-or-nothing cloud) or 1 (max-rand), so overlap has not been
  // exposed as an option beyond this subcolumn. In the future, we should
  // support generalized overlap as well, with parameters derived from DPSCREAM
  // simulations with very high resolution.
  int overlap = 1;
  // Get unique seeds for each column that are reproducible across different MPI
  // rank layouts; use decimal part of pressure for this, consistent with the
  // implementation in EAM
  ...
  auto cldmask =
      get_subcolumn_mask(ncol, nlay, ngpt, cldfrac_rad, overlap, seeds);

  // Assign optical properties to subcolumns (note this implements MCICA)
  auto gpoint_bands = kdist.get_gpoint_bands();
  TIMED_KERNEL(parallel_for(
      SimpleBounds<3>(ngpt, nlay, ncol),
      YAKL_LAMBDA(int igpt, int ilay, int icol) {
        auto ibnd = gpoint_bands(igpt);
        if (cldmask(icol, ilay, igpt) == 1) {
          subsampled_optics.tau(icol, ilay, igpt) =
              cloud_optics.tau(icol, ilay, ibnd);
          subsampled_optics.ssa(icol, ilay, igpt) =
              cloud_optics.ssa(icol, ilay, ibnd);
          subsampled_optics.g(icol, ilay, igpt) =
              cloud_optics.g(icol, ilay, ibnd);
        } else {
...
OpticalProps1scl get_subsampled_clouds(const int ncol, const int nlay,
                                       const int nbnd, const int ngpt,
  ...
  // Get unique seeds for each column that are reproducible across different MPI
  // rank layouts; use decimal part of pressure for this, consistent with the
  // implementation in EAM; use different seed values for longwave and shortwave
  auto seeds = int1d("seeds", ncol);
  TIMED_KERNEL(parallel_for(
      SimpleBounds<1>(ncol), YAKL_LAMBDA(int icol) {
        seeds(icol) =
            1e9 * (p_lay(icol, nlay - 1) - int(p_lay(icol, nlay - 1)));
      }));
  auto cldmask =
      get_subcolumn_mask(ncol, nlay, ngpt, cldfrac_rad, overlap, seeds);
  // Assign optical properties to subcolumns (note this implements MCICA)
...
void compute_band_by_band_surface_albedos(
    const int ncol, const int nswbands, real1d &sfc_alb_dir_vis,
    real1d &sfc_alb_dir_nir, real1d &sfc_alb_dif_vis, real1d &sfc_alb_dif_nir,
    real2d &sfc_alb_dir, real2d &sfc_alb_dif) {

  EKAT_ASSERT_MSG(initialized, "Error! rrtmgp_initialize must be called before "
                               "GasOpticsRRTMGP object can be used.");
  ...
  TIMED_KERNEL(parallel_for(
      SimpleBounds<2>(nswbands, ncol),
      YAKL_LAMBDA(const int ibnd, const int icol) {
        ...
        const bool is_visible_wave1 =
            (wavenumber_limits(1, ibnd) > visible_wavenumber_threshold ? true
                                                                       : false);
        const bool is_visible_wave2 =
            (wavenumber_limits(2, ibnd) > visible_wavenumber_threshold ? true
                                                                       : false);
        ...
        if (is_visible_wave1 && is_visible_wave2) {
          ...
          // broadband albedos
          sfc_alb_dir(icol, ibnd) =
              0.5 * (sfc_alb_dir_vis(icol) + sfc_alb_dir_nir(icol));
          sfc_alb_dif(icol, ibnd) =
              0.5 * (sfc_alb_dif_vis(icol) + sfc_alb_dif_nir(icol));
        }
      }));
}

void compute_broadband_surface_fluxes(
    const int ncol, const int ktop, const int nswbands, real3d &sw_bnd_flux_dir,
    real3d &sw_bnd_flux_dif, real1d &sfc_flux_dir_vis, real1d &sfc_flux_dir_nir,
    real1d &sfc_flux_dif_vis, real1d &sfc_flux_dif_nir) {
  // Band 10 straddles the near-IR and visible, so divide contributions from
  // band 10 between both broadband sums
  // TODO: Hard-coding these band indices is really bad practice. If the bands
  // ever were to change (like when the RRTMG bands were re-ordered for RRTMGP),
  // we would be using the wrong bands for the IR and UV/VIS. This should be
  // refactored to grab the correct bands by specifying appropriate wavenumber
  // rather than index. sfc_flux_dir_nir(i) = sum(sw_bnd_flux_dir(i+1,kbot,1:9))
  // + 0.5 * sw_bnd_flux_dir(i+1,kbot,10); sfc_flux_dir_vis(i) =
  ...
  // sw_bnd_flux_dif(i+1,kbot,10); sfc_flux_dif_vis(i) =
  // sum(sw_bnd_flux_dif(i+1,kbot,11:14)) + 0.5
  ...
  auto wavenumber_limits = k_dist_sw.get_band_lims_wavenumber();
  TIMED_KERNEL(parallel_for(
      SimpleBounds<1>(ncol), YAKL_LAMBDA(const int icol) {
        for (int ibnd = 1; ibnd <= nswbands; ++ibnd) {
          // Wavenumber is in the visible if it is above the visible wavenumber
          // threshold, and in the infrared if it is below the threshold
          const bool is_visible_wave1 =
              (wavenumber_limits(1, ibnd) > visible_wavenumber_threshold
                   ? true
                   : false);
          const bool is_visible_wave2 =
              (wavenumber_limits(2, ibnd) > visible_wavenumber_threshold
                   ? true
                   : false);
          ...
          if (is_visible_wave1 && is_visible_wave2) {
            ...
            // Band straddles the visible to near-infrared transition, so put
            // half the flux in visible and half in near-infrared fluxes
...
void rrtmgp_main(
    const int ncol, const int nlay, real2d &p_lay, real2d &t_lay, real2d &p_lev,
    real2d &t_lev, GasConcs &gas_concs, real2d &sfc_alb_dir,
    real2d &sfc_alb_dif, real1d &mu0, real2d &lwp, real2d &iwp, real2d &rel,
    real2d &rei, real2d &cldfrac, real3d &aer_tau_sw, real3d &aer_ssa_sw,
    real3d &aer_asm_sw, real3d &aer_tau_lw, real3d &cld_tau_sw_bnd,
    real3d &cld_tau_lw_bnd, real3d &cld_tau_sw_gpt, real3d &cld_tau_lw_gpt,
    real2d &sw_flux_up, real2d &sw_flux_dn, real2d &sw_flux_dn_dir,
    real2d &lw_flux_up, real2d &lw_flux_dn, real2d &sw_clnclrsky_flux_up,
    real2d &sw_clnclrsky_flux_dn, real2d &sw_clnclrsky_flux_dn_dir,
    real2d &sw_clrsky_flux_up, real2d &sw_clrsky_flux_dn,
    real2d &sw_clrsky_flux_dn_dir, real2d &sw_clnsky_flux_up,
    real2d &sw_clnsky_flux_dn, real2d &sw_clnsky_flux_dn_dir,
  ...
  check_range(t_lay, k_dist_sw.get_temp_min(), k_dist_sw.get_temp_max(),
              "rrtmgp_main::t_lay");
  check_range(t_lev, k_dist_sw.get_temp_min(), k_dist_sw.get_temp_max(),
              "rrtmgp_main::t_lev");
  check_range(p_lay, k_dist_sw.get_press_min(), k_dist_sw.get_press_max(),
              "rrtmgp_main::p_lay");
  check_range(p_lev, k_dist_sw.get_press_min(), k_dist_sw.get_press_max(),
              "rrtmgp_main::p_lev");
  ...
  TIMED_KERNEL(parallel_for(
      SimpleBounds<3>(nswbands, nlay, ncol),
      YAKL_LAMBDA(int ibnd, int ilay, int icol) {
  ...
  // NOTE: these should already have been checked by precondition checks, but
  // someday we might have non-trivial aerosol optics, so this is still good to
  // do here.
  check_range(aerosol_sw.tau, 0, 1e3, "rrtmgp_main:aerosol_sw.tau");
  check_range(aerosol_sw.ssa, 0, 1,
              "rrtmgp_main:aerosol_sw.ssa"); //, "aerosol_optics_sw.ssa");
  check_range(aerosol_sw.g, -1, 1,
              "rrtmgp_main:aerosol_sw.g  "); //, "aerosol_optics_sw.g"  );
  ...
  OpticalProps2str clouds_sw = get_cloud_optics_sw(
      ncol, nlay, cloud_optics_sw, k_dist_sw, lwp, iwp, rel, rei);
  OpticalProps1scl clouds_lw = get_cloud_optics_lw(
      ncol, nlay, cloud_optics_lw, k_dist_lw, lwp, iwp, rel, rei);
  clouds_sw.tau.deep_copy_to(cld_tau_sw_bnd);
  clouds_lw.tau.deep_copy_to(cld_tau_lw_bnd);

  // Do subcolumn sampling to map bands -> gpoints based on cloud fraction and
  // overlap assumption; This implements the Monte Carlo Independing Column
  // Approximation by mapping only a single subcolumn (cloud state) to each
  // gpoint.
  auto nswgpts       = k_dist_sw.get_ngpt();
  auto clouds_sw_gpt = get_subsampled_clouds(
      ncol, nlay, nswbands, nswgpts, clouds_sw, k_dist_sw, cldfrac, p_lay);
  // Longwave
  auto nlwgpts       = k_dist_lw.get_ngpt();
  auto clouds_lw_gpt = get_subsampled_clouds(
      ncol, nlay, nlwbands, nlwgpts, clouds_lw, k_dist_lw, cldfrac, p_lay);

  // Copy cloud properties to outputs (is this needed, or can we just use
  // pointers?) Alternatively, just compute and output a subcolumn cloud mask
  TIMED_KERNEL(parallel_for(
      SimpleBounds<3>(nswgpts, nlay, ncol),
      YAKL_LAMBDA(int igpt, int ilay, int icol) {
        cld_tau_sw_gpt(icol, ilay, igpt) = clouds_sw_gpt.tau(icol, ilay, igpt);
      }));
  TIMED_KERNEL(parallel_for(
      SimpleBounds<3>(nlwgpts, nlay, ncol),
      YAKL_LAMBDA(int igpt, int ilay, int icol) {
        cld_tau_lw_gpt(icol, ilay, igpt) = clouds_lw_gpt.tau(icol, ilay, igpt);
      }));

#ifdef SCREAM_RRTMGP_DEBUG
  // Perform checks on optics; these would be caught by RRTMGP_EXPENSIVE_CHECKS
  // in the RRTMGP code, but we might want to provide additional debug info
  // here. NOTE: we may actually want to move this up higher in the code, I
  // think optical props should go up higher since optical props are kind of a
  // parameterization of their own, and we might want to swap different choices.
  // These checks go here only because we need to run them on computed optical
  // props, so if the optical props themselves get computed up higher, then
  // perform these checks higher as well
  check_range(clouds_sw.tau, 0, std::numeric_limits<Real>::max(),
              "rrtmgp_main:clouds_sw.tau");
  ...
  // Do shortwave
  rrtmgp_sw(ncol, nlay, k_dist_sw, p_lay, t_lay, p_lev, t_lev, gas_concs,
            sfc_alb_dir, sfc_alb_dif, mu0, aerosol_sw, clouds_sw_gpt, fluxes_sw,
            clnclrsky_fluxes_sw, clrsky_fluxes_sw, clnsky_fluxes_sw,
            tsi_scaling, logger, extra_clnclrsky_diag, extra_clnsky_diag);

  // Do longwave
  rrtmgp_lw(ncol, nlay, k_dist_lw, p_lay, t_lay, p_lev, t_lev, gas_concs,
            aerosol_lw, clouds_lw_gpt, fluxes_lw, clnclrsky_fluxes_lw,
            clrsky_fluxes_lw, clnsky_fluxes_lw, extra_clnclrsky_diag,
            extra_clnsky_diag);
}

int3d get_subcolumn_mask(const int ncol, const int nlay, const int ngpt,
                         real2d &cldf, const int overlap_option, int1d &seeds) {

  // Routine will return subcolumn mask with values of 0 indicating no cloud, 1
  // indicating cloud
  ...
  if (overlap_option == 0) { // Dummy mask, always cloudy
    ...
    // Uses essentially the algorithm described in eq (14) in Raisanen et al.
    // 2004, https://rmets.onlinelibrary.wiley.com/doi/epdf/10.1256/qj.03.99.
    // Also the same algorithm used in RRTMG implementation of maximum-random
    // overlap (see
    // https://github.com/AER-RC/RRTMG_SW/blob/master/src/mcica_subcol_gen_sw.f90)
    //
    // First, fill cldx with random numbers. Need to use a unique seed for each
    // column!
    TIMED_KERNEL(parallel_for(
        SimpleBounds<1>(ncol), YAKL_LAMBDA(int icol) {
    ...
    TIMED_KERNEL(parallel_for(
        SimpleBounds<2>(ngpt, ncol), YAKL_LAMBDA(int igpt, int icol) {
          for (int ilay = 2; ilay <= nlay; ilay++) {
            // Check cldx in level above and see if it satisfies conditions to
            // create a cloudy subcolumn
            if (cldx(icol, ilay - 1, igpt) > 1.0 - cldf(icol, ilay - 1)) {
              // Cloudy subcolumn above, use same random number here so that
              // clouds in these two adjacent layers are maximimally overlapped
              cldx(icol, ilay, igpt) = cldx(icol, ilay - 1, igpt);
            } else {
              // Cloud-less above, use new random number so that clouds are
              // distributed randomly in this layer. Need to scale new random
              // number to range [0, 1.0 - cldf(ilay-1)] because we have
              // artifically changed the distribution of random numbers in this
              // layer with the above branch of the conditional, which would
              // otherwise inflate cloud fraction in this layer.
              cldx(icol, ilay, igpt) =
                  cldx(icol, ilay, igpt) * (1.0 - cldf(icol, ilay - 1));
            }
          }
        }));
  }
...
void rrtmgp_sw(const int ncol, const int nlay, GasOpticsRRTMGP &k_dist,
               real2d &p_lay, real2d &t_lay, real2d &p_lev, real2d &t_lev,
               GasConcs &gas_concs, real2d &sfc_alb_dir, real2d &sfc_alb_dif,
               real1d &mu0, OpticalProps2str &aerosol, OpticalProps2str &clouds,
               FluxesByband &fluxes, FluxesBroadband &clnclrsky_fluxes,
  ...
  TIMED_KERNEL(parallel_for(
      SimpleBounds<1>(nday),
      YAKL_LAMBDA(int iday) { mu0_day(iday) = mu0(dayIndices(iday)); }));

  // subset state variables
  auto p_lay_day = real2d("p_lay_day", nday, nlay);
  auto t_lay_day = real2d("t_lay_day", nday, nlay);
  TIMED_KERNEL(parallel_for(
      SimpleBounds<2>(nlay, nday), YAKL_LAMBDA(int ilay, int iday) {
  ...
  TIMED_KERNEL(parallel_for(
      SimpleBounds<3>(nbnd, nlay, nday),
      YAKL_LAMBDA(int ibnd, int ilay, int iday) {
        aerosol_day.tau(iday, ilay, ibnd) =
            aerosol.tau(dayIndices(iday), ilay, ibnd);
        aerosol_day.ssa(iday, ilay, ibnd) =
            aerosol.ssa(dayIndices(iday), ilay, ibnd);
        aerosol_day.g(iday, ilay, ibnd) =
            aerosol.g(dayIndices(iday), ilay, ibnd);
      }));
  ...
  TIMED_KERNEL(parallel_for(
      SimpleBounds<3>(ngpt, nlay, nday),
      YAKL_LAMBDA(int igpt, int ilay, int iday) {
        clouds_day.tau(iday, ilay, igpt) =
            clouds.tau(dayIndices(iday), ilay, igpt);
        clouds_day.ssa(iday, ilay, igpt) =
            clouds.ssa(dayIndices(iday), ilay, igpt);
  ...
  TIMED_KERNEL(parallel_for(
      SimpleBounds<2>(nbnd, nday), YAKL_LAMBDA(int ibnd, int icol) {
  ...
  auto bnd_flux_dn_dir_day =
      real3d("bnd_flux_dn_dir_day", nday, nlay + 1, nbnd);
  ...
  limit_to_bounds(t_lay_day, k_dist_sw.get_temp_min(), k_dist_sw.get_temp_max(),
                  t_lay_limited);
  ...
  k_dist.gas_optics(nday, nlay, top_at_1, p_lay_day, p_lev_day, t_lay_limited,
                    gas_concs_day, optics, toa_flux);
  if (extra_clnsky_diag) {
    k_dist.gas_optics(nday, nlay, top_at_1, p_lay_day, p_lev_day, t_lay_limited,
                      gas_concs_day, optics_no_aerosols, toa_flux);
  }

#ifdef SCREAM_RRTMGP_DEBUG
  // Check gas optics
  check_range(optics.tau, 0, std::numeric_limits<Real>::max(),
              "rrtmgp_sw:optics.tau");
  ...
  if (extra_clnclrsky_diag) {
    ...
    rte_sw(optics, top_at_1, mu0_day, toa_flux, sfc_alb_dir_T, sfc_alb_dif_T,
           fluxes_day);
  ...
  TIMED_KERNEL(parallel_for(
      SimpleBounds<3>(nbnd, nlay + 1, nday),
      YAKL_LAMBDA(int ibnd, int ilev, int iday) {
        int icol                      = dayIndices(iday);
        bnd_flux_up(icol, ilev, ibnd) = bnd_flux_up_day(iday, ilev, ibnd);
        bnd_flux_dn(icol, ilev, ibnd) = bnd_flux_dn_day(iday, ilev, ibnd);
        bnd_flux_dn_dir(icol, ilev, ibnd) =
            bnd_flux_dn_dir_day(iday, ilev, ibnd);
      }));
  ...
  if (extra_clnsky_diag) {
    ...
    rte_sw(optics_no_aerosols, top_at_1, mu0_day, toa_flux, sfc_alb_dir_T,
           sfc_alb_dif_T, fluxes_day);
    // Expand daytime fluxes to all columns
    TIMED_KERNEL(parallel_for(
        SimpleBounds<2>(nlay + 1, nday), YAKL_LAMBDA(int ilev, int iday) {
          int icol                       = dayIndices(iday);
...
void rrtmgp_lw(const int ncol, const int nlay, GasOpticsRRTMGP &k_dist,
               real2d &p_lay, real2d &t_lay, real2d &p_lev, real2d &t_lev,
               GasConcs &gas_concs, OpticalProps1scl &aerosol,
               OpticalProps1scl &clouds, FluxesByband &fluxes,
  ...
  gauss_Ds_host(1, 1) = 1.66_wp;
  gauss_Ds_host(2, 1) = 0._wp;
  gauss_Ds_host(3, 1) = 0._wp;
  gauss_Ds_host(4, 1) = 0._wp;
  gauss_Ds_host(1, 2) = 1.18350343_wp;
  gauss_Ds_host(2, 2) = 2.81649655_wp;
  gauss_Ds_host(3, 2) = 0._wp;
  gauss_Ds_host(4, 2) = 0._wp;
  gauss_Ds_host(1, 3) = 1.09719858_wp;
  gauss_Ds_host(2, 3) = 1.69338507_wp;
  gauss_Ds_host(3, 3) = 4.70941630_wp;
  gauss_Ds_host(4, 3) = 0._wp;
  gauss_Ds_host(1, 4) = 1.06056257_wp;
  gauss_Ds_host(2, 4) = 1.38282560_wp;
  gauss_Ds_host(3, 4) = 2.40148179_wp;
  gauss_Ds_host(4, 4) = 7.15513024_wp;

  realHost2d gauss_wts_host("gauss_wts", max_gauss_pts, max_gauss_pts);
  gauss_wts_host(1, 1) = 0.5_wp;
  gauss_wts_host(2, 1) = 0._wp;
  gauss_wts_host(3, 1) = 0._wp;
  gauss_wts_host(4, 1) = 0._wp;
  gauss_wts_host(1, 2) = 0.3180413817_wp;
  gauss_wts_host(2, 2) = 0.1819586183_wp;
  gauss_wts_host(3, 2) = 0._wp;
  gauss_wts_host(4, 2) = 0._wp;
  gauss_wts_host(1, 3) = 0.2009319137_wp;
  gauss_wts_host(2, 3) = 0.2292411064_wp;
  gauss_wts_host(3, 3) = 0.0698269799_wp;
  gauss_wts_host(4, 3) = 0._wp;
  gauss_wts_host(1, 4) = 0.1355069134_wp;
  gauss_wts_host(2, 4) = 0.2034645680_wp;
  gauss_wts_host(3, 4) = 0.1298475476_wp;
  gauss_wts_host(4, 4) = 0.0311809710_wp;
  ...
  limit_to_bounds(t_lay, k_dist_lw.get_temp_min(), k_dist_lw.get_temp_max(),
                  t_lay_limited);
  limit_to_bounds(t_lev, k_dist_lw.get_temp_min(), k_dist_lw.get_temp_max(),
                  t_lev_limited);

  // Do gas optics
  k_dist.gas_optics(ncol, nlay, top_at_1, p_lay, p_lev, t_lay_limited, t_sfc,
                    gas_concs, optics, lw_sources, real2d(), t_lev_limited);
  if (extra_clnsky_diag) {
    k_dist.gas_optics(ncol, nlay, top_at_1, p_lay, p_lev, t_lay_limited, t_sfc,
                      gas_concs, optics_no_aerosols, lw_sources, real2d(),
                      t_lev_limited);
  }
  ...
  // Check gas optics
  check_range(optics.tau, 0, std::numeric_limits<Real>::max(),
              "rrtmgp_lw:optics.tau");
#endif

  if (extra_clnclrsky_diag) {
    // Compute clean-clear-sky fluxes before we add in aerosols and clouds
    rte_lw(max_gauss_pts, gauss_Ds, gauss_wts, optics, top_at_1, lw_sources,
           emis_sfc, clnclrsky_fluxes);
  }
  ...
  if (extra_clnsky_diag) {
    ...
    rte_lw(max_gauss_pts, gauss_Ds, gauss_wts, optics_no_aerosols, top_at_1,
           lw_sources, emis_sfc, clnsky_fluxes);
  }
}

void compute_cloud_area(int ncol, int nlay, int ngpt, const Real pmin,
                        const Real pmax, const real2d &pmid,
                        const real3d &cld_tau_gpt, real1d &cld_area) {
  // Subcolumn binary cld mask; if any layers with pressure between pmin and
  // pmax are cloudy then 2d subcol mask is 1, otherwise it is 0
  auto subcol_mask = real2d("subcol_mask", ncol, ngpt);
  memset(subcol_mask, 0);
  TIMED_KERNEL(yakl::fortran::parallel_for(
      SimpleBounds<3>(ngpt, nlay, ncol),
      YAKL_LAMBDA(int igpt, int ilay, int icol) {
        // NOTE: using plev would need to assume level ordering (top to bottom
        // or bottom to top), but using play/pmid does not
...
int get_wavelength_index(OpticalProps &kdist, double wavelength) {
  ...
  // Note that bands are stored in wavenumber space, units of cm-1, so if we are
  // passed wavelength in units of meters, we need a conversion factor of 10^2
