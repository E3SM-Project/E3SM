...
OpticalProps2str get_subsampled_clouds(const int ncol, const int nlay, const int nbnd,
  ...
  TIMED_KERNEL(parallel_for(
      SimpleBounds<3>(nbnd, nlay, ncol), YAKL_LAMBDA(int ibnd, int ilay, int icol) {
  ...
  TIMED_KERNEL(parallel_for(
      SimpleBounds<1>(ncol),
      YAKL_LAMBDA(int icol) { seeds(icol) = 1e9 * (p_lay(icol, nlay) - int(p_lay(icol, nlay))); }));
...
void compute_band_by_band_surface_albedos(const int ncol, const int nswbands,
  ...
  TIMED_KERNEL(parallel_for(
      SimpleBounds<2>(nswbands, ncol), YAKL_LAMBDA(const int ibnd, const int icol) {
...
void rrtmgp_main(
  ...
  TIMED_KERNEL(parallel_for(
      SimpleBounds<3>(nswbands, nlay, ncol), YAKL_LAMBDA(int ibnd, int ilay, int icol) {
  ...
  TIMED_KERNEL(parallel_for(
      SimpleBounds<3>(nswgpts, nlay, ncol), YAKL_LAMBDA(int igpt, int ilay, int icol) {
        cld_tau_sw_gpt(icol, ilay, igpt) = clouds_sw_gpt.tau(icol, ilay, igpt);
      }));
  TIMED_KERNEL(parallel_for(
      SimpleBounds<3>(nlwgpts, nlay, ncol), YAKL_LAMBDA(int igpt, int ilay, int icol) {
...
void rrtmgp_sw(const int ncol, const int nlay, GasOpticsRRTMGP &k_dist, real2d &p_lay,
  ...
  TIMED_KERNEL(parallel_for(
      SimpleBounds<3>(nbnd, nlay + 1, ncol), YAKL_LAMBDA(int ibnd, int ilev, int icol) {
  ...
  for (int igas = 0; igas < ngas; igas++) {
    ...
    TIMED_KERNEL(parallel_for(
        SimpleBounds<2>(nlay, nday),
        YAKL_LAMBDA(int ilay, int iday) { vmr_day(iday, ilay) = vmr(dayIndices(iday), ilay); }));
  ...
  TIMED_KERNEL(parallel_for(
      SimpleBounds<3>(nbnd, nlay, nday), YAKL_LAMBDA(int ibnd, int ilay, int iday) {
  ...
  TIMED_KERNEL(parallel_for(
      SimpleBounds<3>(ngpt, nlay, nday), YAKL_LAMBDA(int igpt, int ilay, int iday) {
  ...
  TIMED_KERNEL(parallel_for(
      SimpleBounds<3>(nbnd, nlay + 1, nday), YAKL_LAMBDA(int ibnd, int ilev, int iday) {
        int icol                          = dayIndices(iday);
