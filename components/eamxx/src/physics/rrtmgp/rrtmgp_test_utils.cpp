...
void dummy_atmos(std::string inputfile, int ncol, real2d &p_lay, real2d &t_lay,
  ...
  dummy_clouds(scream::rrtmgp::cloud_optics_sw, p_lay, t_lay, lwp, iwp, rel,
               rei, cld);
}
...
void dummy_clouds(CloudOptics &cloud_optics, real2d &p_lay, real2d &t_lay,
  ...
  // Restrict clouds to troposphere (> 100 hPa = 100*100 Pa) and not very close
  // to the ground (< 900 hPa), and put them in 2/3 of the columns since that's
  // roughly the total cloudiness of earth. Set sane values for liquid and ice
  // water path. NOTE: these "sane" values are in g/m2!
  parallel_for(
      SimpleBounds<2>(nlay, ncol), YAKL_LAMBDA(int ilay, int icol) {
        ...
        lwp(icol, ilay) =
            merge(10._wp, 0._wp,
                  cloud_mask(icol, ilay) && t_lay(icol, ilay) > 263._wp);
        iwp(icol, ilay) =
            merge(10._wp, 0._wp,
                  cloud_mask(icol, ilay) && t_lay(icol, ilay) < 273._wp);
