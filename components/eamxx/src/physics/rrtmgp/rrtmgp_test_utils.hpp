...
void dummy_clouds(CloudOptics &cloud_optics, real2d &p_lay, real2d &t_lay,
...
void dummy_atmos(std::string inputfile, int ncol, real2d &p_lay, real2d &t_lay,
                 ...
                 real1d &sfc_alb_dif_vis, real1d &sfc_alb_dif_nir, real1d &mu0,
                 real2d &lwp, real2d &iwp, real2d &rel, real2d &rei,
                 real2d &cld);

void read_fluxes(std::string inputfile, real2d &sw_flux_up, real2d &sw_flux_dn,
                 real2d &sw_flux_dir, real2d &lw_flux_up, real2d &lw_flux_dn);

void write_fluxes(std::string outputfile, real2d &sw_flux_up,
                  real2d &sw_flux_dn, real2d &sw_flux_dir, real2d &lw_flux_up,
                  real2d &lw_flux_dn);
#endif
...
struct rrtmgp_test_utils {
  ...
  static void dummy_clouds(CloudOpticsK<RealT, LayoutT, DeviceT> &cloud_optics,
                           real2dk &p_lay, real2dk &t_lay, real2dk &lwp,
                           real2dk &iwp, real2dk &rel, real2dk &rei,
                           real2dk &cloud_mask) {
    // Problem sizes
    int ncol = t_lay.extent(0);
    int nlay = t_lay.extent(1);

    // Generate some fake liquid and ice water data. We pick values to be midway
    // between the min and max of the valid lookup table values for effective
    // radii
    real rel_val = 0.5 * (cloud_optics.get_min_radius_liq() +
                          cloud_optics.get_max_radius_liq());
    real rei_val = 0.5 * (cloud_optics.get_min_radius_ice() +
                          cloud_optics.get_max_radius_ice());

    // Restrict clouds to troposphere (> 100 hPa = 100*100 Pa) and not very
    // close to the ground (< 900 hPa), and put them in 2/3 of the columns since
    // that's roughly the total cloudiness of earth. Set sane values for liquid
    // and ice water path. NOTE: these "sane" values are in g/m2!
    Kokkos::parallel_for(
        MDRP::template get<2>({ncol, nlay}), KOKKOS_LAMBDA(int icol, int ilay) {
          ...
          lwp(icol, ilay) = conv::merge(
              10., 0., cloud_mask(icol, ilay) && t_lay(icol, ilay) > 263.);
          iwp(icol, ilay) = conv::merge(
              10., 0., cloud_mask(icol, ilay) && t_lay(icol, ilay) < 273.);
          rel(icol, ilay) = conv::merge(rel_val, 0., lwp(icol, ilay) > 0.);
          rei(icol, ilay) = conv::merge(rei_val, 0., iwp(icol, ilay) > 0.);
        });
  }

  static void dummy_atmos(std::string inputfile, int ncol, real2dk &p_lay,
                          real2dk &t_lay, real1dk &sfc_alb_dir_vis,
                          real1dk &sfc_alb_dir_nir, real1dk &sfc_alb_dif_vis,
                          real1dk &sfc_alb_dif_nir, real1dk &mu0, real2dk &lwp,
                          real2dk &iwp, real2dk &rel, real2dk &rei,
                          real2dk &cld) {
    ...
    dummy_clouds(*interface_t::cloud_optics_sw_k, p_lay, t_lay, lwp, iwp, rel,
                 rei, cld);
  }

  static void read_fluxes(std::string inputfile, real2dk &sw_flux_up,
                          real2dk &sw_flux_dn, real2dk &sw_flux_dir,
  ...
  static void write_fluxes(std::string outputfile, real2dk &sw_flux_up,
                           real2dk &sw_flux_dn, real2dk &sw_flux_dir,
                           real2dk &lw_flux_up, real2dk &lw_flux_dn) {
    conv::SimpleNetCDF io;
