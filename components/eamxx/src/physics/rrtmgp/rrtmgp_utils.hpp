...
void compute_heating_rate(yakl::Array<T, 2, myMem, myStyle> const &flux_up,
                          yakl::Array<T, 2, myMem, myStyle> const &flux_dn,
                          yakl::Array<T, 2, myMem, myStyle> const &pdel,
...
bool check_range(T x, Real xmin, Real xmax, std::string msg, std::ostream &out = std::cout) {
  ...
  if (_xmin < xmin or _xmax > xmax) {
    ...
    memset(bad_mask, 0);
    yakl::c::parallel_for(
        yakl::c::SimpleBounds<1>(x.totElems()), YAKL_LAMBDA(int i) {
    ...
    if (num_bad > 0) {
      pass = false;
      out << msg << ": " << num_bad << " values outside range "
          << "[" << xmin << "," << xmax << "]"
          << "; minval = " << _xmin << "; maxval = " << _xmax << "\n";
    }
  }
  return pass;
}
