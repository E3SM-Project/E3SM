...
void rrtmgp_main(const int ncol, const int nlay, real2d &p_lay, real2d &t_lay,
                 real2d &p_lev, real2d &t_lev, GasConcs &gas_concs,
                 real2d &sfc_alb_dir, real2d &sfc_alb_dif, real1d &mu0,
                 real2d &lwp, real2d &iwp, real2d &rel, real2d &rei,
                 real2d &cldfrac, real3d &aer_tau_sw, real3d &aer_ssa_sw,
                 real3d &aer_asm_sw, real3d &aer_tau_lw, real3d &cld_tau_sw_bnd,
                 real3d &cld_tau_lw_bnd, real3d &cld_tau_sw_gpt,
                 real3d &cld_tau_lw_gpt, real2d &sw_flux_up, real2d &sw_flux_dn,
                 real2d &sw_flux_dn_dir, real2d &lw_flux_up, real2d &lw_flux_dn,
                 real2d &sw_clnclrsky_flux_up, real2d &sw_clnclrsky_flux_dn,
                 real2d &sw_clnclrsky_flux_dn_dir, real2d &sw_clrsky_flux_up,
                 real2d &sw_clrsky_flux_dn, real2d &sw_clrsky_flux_dn_dir,
                 real2d &sw_clnsky_flux_up, real2d &sw_clnsky_flux_dn,
                 real2d &sw_clnsky_flux_dn_dir, real2d &lw_clnclrsky_flux_up,
                 real2d &lw_clnclrsky_flux_dn, real2d &lw_clrsky_flux_up,
                 real2d &lw_clrsky_flux_dn, real2d &lw_clnsky_flux_up,
                 real2d &lw_clnsky_flux_dn, real3d &sw_bnd_flux_up,
                 real3d &sw_bnd_flux_dn, real3d &sw_bnd_flux_dn_dir,
                 real3d &lw_bnd_flux_up, real3d &lw_bnd_flux_dn,
                 const Real tsi_scaling,
                 const std::shared_ptr<spdlog::logger> &logger,
                 const bool extra_clnclrsky_diag = false,
...
void compute_cloud_area(int ncol, int nlay, int ngpt, Real pmin, Real pmax,
                        const real2d &pmid, const real3d &cld_tau_gpt,
                        real1d &cld_area);
...
void mixing_ratio_to_cloud_mass(
  ...
  TIMED_KERNEL(yakl::fortran::parallel_for(
      ...
      YAKL_LAMBDA(int ilay, int icol) {
        // Compute in-cloud mixing ratio (mixing ratio of the cloudy part of the
        // layer) NOTE: these thresholds (from E3SM) seem arbitrary, but
        // included here for consistency This limits in-cloud mixing ratio to
        // 0.005 kg/kg. According to note in cloud_diagnostics in EAM, this is
        // consistent with limits in MG2. Is this true for P3?
        if (cloud_fraction(icol, ilay) > 0) {
          // Compute layer-integrated cloud mass (per unit area)
          auto incloud_mixing_ratio =
              std::min(mixing_ratio(icol, ilay) /
                           std::max(0.0001, cloud_fraction(icol, ilay)),
                       0.005);
          cloud_mass(icol, ilay) =
              incloud_mixing_ratio * dp(icol, ilay) / physconst::gravit;
        } else {
...
void limit_to_bounds(S const &arr_in, T const lower, T const upper,
                     S &arr_out) {
...
struct rrtmgp_interface {
  ...
  template <typename T> using view_t = Kokkos::View<T, LayoutT, DeviceT>;

  template <typename T> using hview_t = Kokkos::View<T, LayoutT, HostDevice>;
  ...
  /*
   * Initialize data for RRTMGP driver. Increase multiplier to allocate more
   * pool space.
   */
  ...
  static void compute_band_by_band_surface_albedos(
    ...
    TIMED_KERNEL(FLATTEN_MD_KERNEL2(
        ...
        const bool is_visible_wave1 =
            (wavenumber_limits(0, ibnd) > visible_wavenumber_threshold ? true
                                                                       : false);
  ...
  static void compute_broadband_surface_fluxes(
    ...
    // ever were to change (like when the RRTMG bands were re-ordered for
    // RRTMGP), we would be using the wrong bands for the IR and UV/VIS. This
    // should be refactored to grab the correct bands by specifying appropriate
    // wavenumber rather than index. sfc_flux_dir_nir(i) =
    // sum(sw_bnd_flux_dir(i+1,kbot,1:9))   + 0.5 *
    // sw_bnd_flux_dir(i+1,kbot,10); sfc_flux_dir_vis(i) =
    // sum(sw_bnd_flux_dir(i+1,kbot,11:14)) + 0.5 *
    // sw_bnd_flux_dir(i+1,kbot,10); sfc_flux_dif_nir(i) =
    // sum(sw_bnd_flux_dif(i+1,kbot,1:9))   + 0.5 *
    ...
    TIMED_KERNEL(Kokkos::parallel_for(
        ncol, KOKKOS_LAMBDA(const int icol) {
          for (int ibnd = 0; ibnd < nswbands; ++ibnd) {
            // Wavenumber is in the visible if it is above the visible
            // wavenumber threshold, and in the infrared if it is below the
            // threshold
            const bool is_visible_wave1 =
                (wavenumber_limits(0, ibnd) > visible_wavenumber_threshold
                     ? true
                     : false);
            const bool is_visible_wave2 =
  ...
  static void rrtmgp_main(
      const int ncol, const int nlay, const creal2dk &p_lay,
      const creal2dk &t_lay, const creal2dk &p_lev, const creal2dk &t_lev,
      gas_concs_t &gas_concs, const creal2dk &sfc_alb_dir,
      const creal2dk &sfc_alb_dif, const real1dk &mu0, const real2dk &lwp,
      const real2dk &iwp, const creal2dk &rel, const creal2dk &rei,
      const real2dk &cldfrac, const real3dk &aer_tau_sw,
      const real3dk &aer_ssa_sw, const real3dk &aer_asm_sw,
      const real3dk &aer_tau_lw, const real3dk &cld_tau_sw_bnd,
      const real3dk &cld_tau_lw_bnd, const real3dk &cld_tau_sw_gpt,
      const real3dk &cld_tau_lw_gpt, const real2dk &sw_flux_up,
      const real2dk &sw_flux_dn, const real2dk &sw_flux_dn_dir,
    ...
    check_range_k(t_lay, k_dist_sw_k->get_temp_min(),
                  k_dist_sw_k->get_temp_max(), "rrtmgp_main::t_lay");
    check_range_k(t_lev, k_dist_sw_k->get_temp_min(),
                  k_dist_sw_k->get_temp_max(), "rrtmgp_main::t_lev");
    check_range_k(p_lay, k_dist_sw_k->get_press_min(),
                  k_dist_sw_k->get_press_max(), "rrtmgp_main::p_lay");
    check_range_k(p_lev, k_dist_sw_k->get_press_min(),
                  k_dist_sw_k->get_press_max(), "rrtmgp_main::p_lev");
    ...
    check_range_k(lwp, 0, std::numeric_limits<RealT>::max(),
                  "rrtmgp_main::lwp");
    check_range_k(iwp, 0, std::numeric_limits<RealT>::max(),
                  "rrtmgp_main::iwp");
    check_range_k(rel, 0, std::numeric_limits<RealT>::max(),
                  "rrtmgp_main::rel");
    check_range_k(rei, 0, std::numeric_limits<RealT>::max(),
                  "rrtmgp_main::rei");
#endif
    ...
    auto sw_subcloud_tau_mem =
        pool_t::template alloc<RealT>(ncol, nlay, sw_ngpt);
    auto sw_subcloud_ssa_mem =
        pool_t::template alloc<RealT>(ncol, nlay, sw_ngpt);
    auto sw_subcloud_g_mem = pool_t::template alloc<RealT>(ncol, nlay, sw_ngpt);
    auto lw_subcloud_tau_mem =
        pool_t::template alloc<RealT>(ncol, nlay, lw_ngpt);
    ...
    aerosol_sw.init_no_alloc(k_dist_sw_k->get_band_lims_wavenumber(),
                             sw_band2gpt_mem, sw_gpt2band_mem);
    aerosol_sw.alloc_2str_no_alloc(ncol, nlay, sw_tau_mem, sw_ssa_mem,
                                   sw_g_mem);
    TIMED_KERNEL(FLATTEN_MD_KERNEL3(
        ncol, nlay, nswbands, icol, ilay, ibnd,
    ...
    aerosol_lw.init_no_alloc(k_dist_lw_k->get_band_lims_wavenumber(),
                             lw_band2gpt_mem, lw_gpt2band_mem);
    aerosol_lw.alloc_1scl_no_alloc(ncol, nlay, lw_tau_mem);
    TIMED_KERNEL(FLATTEN_MD_KERNEL3(ncol, nlay, nlwbands, icol, ilay, ibnd,
                                    aerosol_lw.tau(icol, ilay, ibnd) =
                                        aer_tau_lw(icol, ilay, ibnd);));

#ifdef SCREAM_RRTMGP_DEBUG
    // Check aerosol optical properties
    // NOTE: these should already have been checked by precondition checks, but
    // someday we might have non-trivial aerosol optics, so this is still good
    // to do here.
    check_range_k(aerosol_sw.tau, 0, 1e3, "rrtmgp_main:aerosol_sw.tau");
    check_range_k(aerosol_sw.ssa, 0, 1,
                  "rrtmgp_main:aerosol_sw.ssa"); //, "aerosol_optics_sw.ssa");
    check_range_k(aerosol_sw.g, -1, 1,
                  "rrtmgp_main:aerosol_sw.g  "); //, "aerosol_optics_sw.g"  );
    check_range_k(aerosol_lw.tau, 0, 1e3, "rrtmgp_main:aerosol_lw.tau");
#endif

    // Convert cloud physical properties to optical properties for input to
    // RRTMGP
    optical_props2_t clouds_sw = get_cloud_optics_sw(
        ncol, nlay, *cloud_optics_sw_k, *k_dist_sw_k, lwp, iwp, rel, rei,
        sw_cloud_band2gpt_mem, sw_cloud_gpt2band_mem, sw_cloud_tau_mem,
        sw_cloud_ssa_mem, sw_cloud_g_mem);
    optical_props1_t clouds_lw = get_cloud_optics_lw(
        ncol, nlay, *cloud_optics_lw_k, *k_dist_lw_k, lwp, iwp, rel, rei,
    ...
    auto clouds_sw_gpt = get_subsampled_clouds(
        ncol, nlay, nswbands, nswgpts, clouds_sw, *k_dist_sw_k, cldfrac, p_lay,
    ...
    auto clouds_lw_gpt = get_subsampled_clouds(
        ...
        lw_subcloud_band2gpt_mem, lw_subcloud_gpt2band_mem,
        lw_subcloud_tau_mem);
...
#ifdef SCREAM_RRTMGP_DEBUG
    // Perform checks on optics; these would be caught by
    // RRTMGP_EXPENSIVE_CHECKS in the RRTMGP code, but we might want to provide
    // additional debug info here. NOTE: we may actually want to move this up
    // higher in the code, I think optical props should go up higher since
    // optical props are kind of a parameterization of their own, and we might
    // want to swap different choices. These checks go here only because we need
    // to run them on computed optical props, so if the optical props themselves
    // get computed up higher, then perform these checks higher as well
    check_range_k(clouds_sw.tau, 0, std::numeric_limits<RealT>::max(),
    ...
    // Do shortwave
    rrtmgp_sw(ncol, nlay, *k_dist_sw_k, p_lay, t_lay, p_lev, t_lev, gas_concs,
              sfc_alb_dir, sfc_alb_dif, mu0, aerosol_sw, clouds_sw_gpt,
              fluxes_sw, clnclrsky_fluxes_sw, clrsky_fluxes_sw,
              clnsky_fluxes_sw, tsi_scaling, logger, extra_clnclrsky_diag,
              extra_clnsky_diag);

    // Do longwave
    rrtmgp_lw(ncol, nlay, *k_dist_lw_k, p_lay, t_lay, p_lev, t_lev, gas_concs,
  ...
  static void rrtmgp_sw(
      const int ncol, const int nlay, gas_optics_t &k_dist,
      const creal2dk &p_lay, const creal2dk &t_lay, const creal2dk &p_lev,
      const creal2dk &t_lev, gas_concs_t &gas_concs,
      const creal2dk &sfc_alb_dir, const creal2dk &sfc_alb_dif,
      const real1dk &mu0, optical_props2_t &aerosol, optical_props2_t &clouds,
    ...
    TIMED_KERNEL(FLATTEN_MD_KERNEL2(
        ncol, nlay + 1, icol, ilev, flux_up(icol, ilev) = 0;
        flux_dn(icol, ilev) = 0; flux_dn_dir(icol, ilev) = 0;
        clnclrsky_flux_up(icol, ilev) = 0; clnclrsky_flux_dn(icol, ilev) = 0;
        clnclrsky_flux_dn_dir(icol, ilev) = 0; clrsky_flux_up(icol, ilev) = 0;
        clrsky_flux_dn(icol, ilev) = 0; clrsky_flux_dn_dir(icol, ilev) = 0;
    ...
    Kokkos::parallel_reduce(
        1,
        KOKKOS_LAMBDA(int, int &nday_inner) {
    ...
    auto bnd_flux_dn_dir_day =
        pool_t::template alloc<RealT>(nday, nlay + 1, nbnd);

    auto col_gas =
        pool_t::template alloc<RealT>(ncol, nlay, k_dist.get_ngas() + 1);
    ...
    TIMED_KERNEL(Kokkos::parallel_for(
        nday,
        KOKKOS_LAMBDA(int iday) { mu0_day(iday) = mu0(dayIndices(iday)); }));
    ...
    for (int igas = 0; igas < ngas; igas++) {
      ...
      TIMED_KERNEL(FLATTEN_MD_KERNEL2(nday, nlay, iday, ilay,
                                      vmr_day(iday, ilay) =
                                          vmr(dayIndices(iday), ilay);));
      gas_concs_day.set_vmr(gas_names[igas], vmr_day);
    }

    // Subset aerosol optics
    optical_props2_t aerosol_day;
    aerosol_day.init_no_alloc(k_dist.get_band_lims_wavenumber(),
                              sw_aero_band2gpt_mem, sw_aero_gpt2band_mem);
    aerosol_day.alloc_2str_no_alloc(nday, nlay, sw_aero_tau_mem,
                                    sw_aero_ssa_mem, sw_aero_g_mem);
    TIMED_KERNEL(
        FLATTEN_MD_KERNEL3(nday, nlay, nbnd, iday, ilay, ibnd,
    ...
    clouds_day.init_no_alloc(k_dist.get_band_lims_wavenumber(),
                             k_dist.get_band_lims_gpoint(),
                             sw_cloud_band2gpt_mem, sw_cloud_gpt2band_mem);
    clouds_day.alloc_2str_no_alloc(nday, nlay, sw_cloud_tau_mem,
                                   sw_cloud_ssa_mem, sw_cloud_g_mem);
    TIMED_KERNEL(
        FLATTEN_MD_KERNEL3(nday, nlay, ngpt, iday, ilay, igpt,
                           ...
                           clouds_day.g(iday, ilay, igpt) =
                               clouds.g(dayIndices(iday), ilay, igpt);));
    ...
    optics.alloc_2str_no_alloc(nday, nlay, k_dist, sw_optics_band2gpt_mem,
                               sw_optics_gpt2band_mem, sw_optics_tau_mem,
    ...
    if (extra_clnsky_diag) {
      ...
      optics_no_aerosols.alloc_2str_no_alloc(
          nday, nlay, k_dist, sw_noaero_band2gpt_mem, sw_noaero_gpt2band_mem,
          sw_noaero_tau_mem, sw_noaero_ssa_mem, sw_noaero_g_mem);
    }

    // Limit temperatures for gas optics look-up tables
    limit_to_bounds_k(t_lay_day, k_dist_sw_k->get_temp_min(),
    ...
    k_dist.gas_optics(nday, nlay, top_at_1, p_lay_day, p_lev_day, t_lay_limited,
                      gas_concs_day, col_gas, optics, toa_flux);
    if (extra_clnsky_diag) {
      k_dist.gas_optics(nday, nlay, top_at_1, p_lay_day, p_lev_day,
                        t_lay_limited, gas_concs_day, col_gas,
                        optics_no_aerosols, toa_flux);
    }

#ifdef SCREAM_RRTMGP_DEBUG
    // Check gas optics
    check_range_k(optics.tau, 0, std::numeric_limits<RealT>::max(),
    ...
    TIMED_KERNEL(FLATTEN_MD_KERNEL2(nday, ngpt, iday, igpt,
                                    toa_flux(iday, igpt) =
                                        tsi_scaling * toa_flux(iday, igpt);));
    ...
    if (extra_clnclrsky_diag) {
      ...
      TIMED_KERNEL(FLATTEN_MD_KERNEL2(
          nday, nlay + 1, iday, ilev, const int icol = dayIndices(iday);
    ...
    TIMED_KERNEL(FLATTEN_MD_KERNEL3(
        nday, nlay + 1, nbnd, iday, ilev, ibnd,
   ...
   */
  static void rrtmgp_lw(const int ncol, const int nlay, gas_optics_t &k_dist,
                        const creal2dk &p_lay, const creal2dk &t_lay,
                        const creal2dk &p_lev, const creal2dk &t_lev,
                        gas_concs_t &gas_concs, optical_props1_t &aerosol,
                        optical_props1_t &clouds, fluxes_t &fluxes,
    ...
    auto gauss_wts =
        pool_t::template alloc<RealT>(max_gauss_pts, max_gauss_pts);
    ...
    TIMED_KERNEL(FLATTEN_MD_KERNEL2(
        ncol, nlay + 1, icol, ilev, flux_up(icol, ilev) = 0;
        flux_dn(icol, ilev) = 0; clnclrsky_flux_up(icol, ilev) = 0;
        clnclrsky_flux_dn(icol, ilev) = 0; clrsky_flux_up(icol, ilev) = 0;
        clrsky_flux_dn(icol, ilev) = 0; clnsky_flux_up(icol, ilev) = 0;
    ...
    optics.alloc_1scl_no_alloc(ncol, nlay, k_dist, lw_optics_band2gpt_mem,
    ...
    if (extra_clnsky_diag) {
      ...
      optics_no_aerosols.alloc_1scl_no_alloc(
          ncol, nlay, k_dist, lw_noaero_band2gpt_mem, lw_noaero_gpt2band_mem,
          lw_noaero_tau_mem);
    }

    // Boundary conditions
    source_func_t lw_sources;
    lw_sources.alloc_no_alloc(
        ncol, nlay, k_dist, lw_source_band2gpt_mem, lw_source_gpt2band_mem,
        sfc_source_mem, lay_source_mem, lev_source_inc_mem, lev_source_dec_mem);
    ...
    RealT gauss_Ds_host_raw[max_gauss_pts][max_gauss_pts] = {
        ...
        {0., 0., 0., 7.15513024}};
    hview_t<RealT **> gauss_Ds_host(&gauss_Ds_host_raw[0][0], max_gauss_pts,
                                    max_gauss_pts);
    ...
    hview_t<RealT **> gauss_wts_host(&gauss_wts_host_raw[0][0], max_gauss_pts,
                                     max_gauss_pts);
    ...
    // Limit temperatures for gas optics look-up tables
    limit_to_bounds_k(t_lay, k_dist_lw_k->get_temp_min(),
                      k_dist_lw_k->get_temp_max(), t_lay_limited);
    limit_to_bounds_k(t_lev, k_dist_lw_k->get_temp_min(),
                      k_dist_lw_k->get_temp_max(), t_lev_limited);

    // Do gas optics
    k_dist.gas_optics(ncol, nlay, top_at_1, p_lay, p_lev, t_lay_limited, t_sfc,
                      gas_concs, col_gas, optics, lw_sources,
                      view_t<RealT **>(), t_lev_limited);
    if (extra_clnsky_diag) {
      k_dist.gas_optics(ncol, nlay, top_at_1, p_lay, p_lev, t_lay_limited,
                        t_sfc, gas_concs, col_gas, optics_no_aerosols,
                        lw_sources, view_t<RealT **>(), t_lev_limited);
    }
  ...
  static void get_subcolumn_mask(const int ncol, const int nlay, const int ngpt,
                                 const CldfT &cldf, const int overlap_option,
                                 const SeedsT &seeds,
    ...
    if (overlap_option == 0) { // Dummy mask, always cloudy
      ...
      //
      // First, fill cldx with random numbers. Need to use a unique seed for
      // each column! auto seeds_host = Kokkos::create_mirror_view(seeds);
   ...
   */
  static void compute_cloud_area(int ncol, int nlay, int ngpt, Real pmin,
                                 Real pmax, const creal2dk &pmid,
                                 const real3dk &cld_tau_gpt,
  ...
  static void compute_aerocom_cloudtop(
      ...
      const real1dk &cldfrac_ice_at_cldtop,
      const real1dk &cldfrac_liq_at_cldtop,
      const real1dk &cldfrac_tot_at_cldtop, const real1dk &cdnc_at_cldtop,
      const real1dk &eff_radius_qc_at_cldtop,
   ...
   * mass per unit area (what E3SM refers to as "in-cloud water paths", a
   * terminology we shun here to avoid confusion with the standard practice of
   * using "water path" to refer to the total column-integrated quantities).
   */
  template <class View1, class View2, class View3, class View4>
  static void mixing_ratio_to_cloud_mass(View1 const &mixing_ratio,
  ...
  static void limit_to_bounds_k(InT const &arr_in, T const lower, T const upper,
                                OutT &arr_out) {
    TIMED_KERNEL(Kokkos::parallel_for(
        arr_out.size(), KOKKOS_LAMBDA(int i) {
  ...
  static int get_wavelength_index(optical_props_t &kdist, RealT wavelength) {
    ...
    auto wavelength_bounds = pool_t::template alloc<RealT>(
        band_lims_wvn.extent(0), band_lims_wvn.extent(1));
    ...
    // Note that bands are stored in wavenumber space, units of cm-1, so if we
    // are passed wavelength in units of meters, we need a conversion factor of
    // 10^2
    const int nbnds = kdist.get_nband();
    int band_index  = -1;
    TIMED_KERNEL(Kokkos::parallel_reduce(
        nbnds,
  ...
  static optical_props2_t get_cloud_optics_sw(
      const int ncol, const int nlay, cloud_optics_t &cloud_optics,
    ...
    clouds.init_no_alloc(kdist.get_band_lims_wavenumber(), sw_band2gpt_mem,
                         sw_gpt2band_mem);
    ...
    limit_to_bounds_k(rel, cloud_optics.radliq_lwr, cloud_optics.radliq_upr,
                      rel_limited);
    limit_to_bounds_k(rei, cloud_optics.radice_lwr, cloud_optics.radice_upr,
                      rei_limited);
  ...
  static optical_props1_t get_cloud_optics_lw(
    ...
    clouds.init_no_alloc(kdist.get_band_lims_wavenumber(), lw_band2gpt_mem,
                         lw_gpt2band_mem);
    clouds.alloc_1scl_no_alloc(ncol, nlay,
                               lw_tau_mem); // this is dumb, why do we need to
                                            // init and alloc separately?!
  ...
  static optical_props2_t get_subsampled_clouds(
    ...
    subsampled_optics.init_no_alloc(
        kdist.get_band_lims_wavenumber(), kdist.get_band_lims_gpoint(),
        sw_band2gpt_mem, sw_gpt2band_mem, "subsampled_optics");
    subsampled_optics.alloc_2str_no_alloc(ncol, nlay, sw_tau_mem, sw_ssa_mem,
                                          sw_g_mem);
    ...
    // Check that we do not have clouds with no optical properties; this would
    // get corrected when we assign optical props, but we want to use a
    // "radiative cloud fraction" for the subcolumn sampling too because
    // otherwise we can get vertically-contiguous cloud mask profiles with no
    // actual cloud properties in between, which would just further overestimate
    // the vertical correlation of cloudy layers. I.e., cloudy layers might look
    // maximally overlapped even when separated by layers with no cloud
    // properties, when in fact those layers should be randomly overlapped.
    ...
    // support generalized overlap as well, with parameters derived from
    // DPSCREAM simulations with very high resolution.
    int overlap = 1;
    // Get unique seeds for each column that are reproducible across different
    // MPI rank layouts; use decimal part of pressure for this, consistent with
    // the implementation in EAM
  ...
  static optical_props1_t get_subsampled_clouds(
    ...
    // Get unique seeds for each column that are reproducible across different
    // MPI rank layouts; use decimal part of pressure for this, consistent with
    // the implementation in EAM; use different seed values for longwave and
    // shortwave
    auto seeds = pool_t::template alloc<int>(ncol);
    TIMED_KERNEL(Kokkos::parallel_for(
        ncol, KOKKOS_LAMBDA(int icol) {
          seeds(icol) =
              1e9 * (p_lay(icol, nlay - 2) - int(p_lay(icol, nlay - 2)));
        }));
    get_subcolumn_mask(ncol, nlay, ngpt, cldfrac_rad, overlap, seeds, cldmask);
    // Assign optical properties to subcolumns (note this implements MCICA)
    auto gpoint_bands = kdist.get_gpoint_bands();
