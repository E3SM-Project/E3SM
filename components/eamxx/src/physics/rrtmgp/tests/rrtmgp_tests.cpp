...
// NOTE: using old (full gpoint resolution) coefficient data for consistency
// with reference problem
...
int run_yakl(int argc, char **argv) {
  ...
  rrtmgpTest::read_fluxes(inputfile, sw_flux_up_ref, sw_flux_dn_ref,
                          sw_flux_dir_ref, lw_flux_up_ref, lw_flux_dn_ref);
  ...
  rrtmgpTest::dummy_atmos(inputfile, ncol, p_lay, t_lay, sfc_alb_dir_vis,
                          sfc_alb_dir_nir, sfc_alb_dif_vis, sfc_alb_dif_nir,
                          mu0, lwp, iwp, rel, rei, cld);
  ...
  scream::rrtmgp::rrtmgp_main(
      ...
      aer_asm_sw, aer_tau_lw, cld_tau_sw_bnd,
      cld_tau_lw_bnd,         // outputs
      cld_tau_sw, cld_tau_lw, // outputs
      sw_flux_up, sw_flux_dn, sw_flux_dir, lw_flux_up, lw_flux_dn,
  ...
  rrtmgpTest::read_fluxes(baseline, sw_flux_up_ref, sw_flux_dn_ref,
                          sw_flux_dir_ref, lw_flux_up_ref, lw_flux_dn_ref);
  int nerr = 0;
  ...
  // Because the aerosol optical properties are all set to zero, these fluxes
  // must be equal
  if (!rrtmgpTest::all_close(sw_flux_up, sw_clnsky_flux_up, 0.0000000001))
    nerr++;
  if (!rrtmgpTest::all_close(sw_clrsky_flux_up, sw_clnclrsky_flux_up,
                             0.0000000001))
    nerr++;
  if (!rrtmgpTest::all_close(sw_flux_dn, sw_clnsky_flux_dn, 0.0000000001))
    nerr++;
  if (!rrtmgpTest::all_close(sw_clrsky_flux_dn, sw_clnclrsky_flux_dn,
                             0.0000000001))
    nerr++;
  if (!rrtmgpTest::all_close(sw_flux_dir, sw_clnsky_flux_dir, 0.0000000001))
    nerr++;
  if (!rrtmgpTest::all_close(sw_clrsky_flux_dir, sw_clnclrsky_flux_dir,
                             0.0000000001))
    nerr++;
  if (!rrtmgpTest::all_close(lw_flux_up, lw_clnsky_flux_up, 0.0000000001))
    nerr++;
  if (!rrtmgpTest::all_close(lw_clrsky_flux_up, lw_clnclrsky_flux_up,
                             0.0000000001))
    nerr++;
  if (!rrtmgpTest::all_close(lw_flux_dn, lw_clnsky_flux_dn, 0.0000000001))
    nerr++;
  if (!rrtmgpTest::all_close(lw_clrsky_flux_dn, lw_clnclrsky_flux_dn,
                             0.0000000001))
    nerr++;
...
int run_kokkos(int argc, char **argv) {
  ...
  Kokkos::parallel_for(
      MDRP::template get<3>({ncol, nlay, nswbands}),
  ...
  // Check values against baseline
  logger->info("Check values...\n");
  utils_t::read_fluxes(baseline, sw_flux_up_ref, sw_flux_dn_ref,
                       sw_flux_dir_ref, lw_flux_up_ref, lw_flux_dn_ref);
  int nerr = 0;
  ...
  if (!utils_t::all_close(sw_flux_up, sw_clnsky_flux_up, 0.0000000001))
    nerr++;
  if (!utils_t::all_close(sw_clrsky_flux_up, sw_clnclrsky_flux_up,
                          0.0000000001))
    nerr++;
  if (!utils_t::all_close(sw_flux_dn, sw_clnsky_flux_dn, 0.0000000001))
    nerr++;
  if (!utils_t::all_close(sw_clrsky_flux_dn, sw_clnclrsky_flux_dn,
                          0.0000000001))
    nerr++;
  if (!utils_t::all_close(sw_flux_dir, sw_clnsky_flux_dir, 0.0000000001))
    nerr++;
  if (!utils_t::all_close(sw_clrsky_flux_dir, sw_clnclrsky_flux_dir,
                          0.0000000001))
    nerr++;
  if (!utils_t::all_close(lw_flux_up, lw_clnsky_flux_up, 0.0000000001))
    nerr++;
  if (!utils_t::all_close(lw_clrsky_flux_up, lw_clnclrsky_flux_up,
                          0.0000000001))
    nerr++;
  if (!utils_t::all_close(lw_flux_dn, lw_clnsky_flux_dn, 0.0000000001))
    nerr++;
  if (!utils_t::all_close(lw_clrsky_flux_dn, lw_clnclrsky_flux_dn,
                          0.0000000001))
    nerr++;
