...
void RRTMGPRadiation::init_buffers(const ATMBufferManager &buffer_manager) {
  ...
  m_buffer.lw_clrsky_flux_dn =
      decltype(m_buffer.lw_clrsky_flux_dn)("lw_clrsky_flux_dn", mem, m_col_chunk_size, m_nlay + 1);
  mem += m_buffer.lw_clrsky_flux_dn.totElems();
  m_buffer.lw_clnsky_flux_up =
      decltype(m_buffer.lw_clnsky_flux_up)("lw_clnsky_flux_up", mem, m_col_chunk_size, m_nlay + 1);
  mem += m_buffer.lw_clnsky_flux_up.totElems();
  m_buffer.lw_clnsky_flux_dn =
  ...
  m_buffer.cld_tau_sw_gpt =
      decltype(m_buffer.cld_tau_sw_gpt)("cld_tau_sw_gpt", mem, m_col_chunk_size, m_nlay, m_nswgpts);
...
void RRTMGPRadiation::run_impl(const double dt) {
  ...
  auto d_cldfrac_ice_at_cldtop   = get_field_out("cldfrac_ice_at_cldtop").get_view<Real *>();
  auto d_cldfrac_liq_at_cldtop   = get_field_out("cldfrac_liq_at_cldtop").get_view<Real *>();
  auto d_cldfrac_tot_at_cldtop   = get_field_out("cldfrac_tot_at_cldtop").get_view<Real *>();
  auto d_cdnc_at_cldtop          = get_field_out("cdnc_at_cldtop").get_view<Real *>();
  auto d_eff_radius_qc_at_cldtop = get_field_out("eff_radius_qc_at_cldtop").get_view<Real *>();
  auto d_eff_radius_qi_at_cldtop = get_field_out("eff_radius_qi_at_cldtop").get_view<Real *>();
  ...
  if (update_rad) {
    ...
    shr_orb_params_c2f(&orbital_year, &eccen, &obliq, &mvelp, &obliqr, &lambm0, &mvelpp);
    ...
    for (int ic = 0; ic < m_num_col_chunks; ++ic) {
      ...
      TIMED_INLINE_KERNEL(
          ...
          auto p_lay = subview_2d(m_buffer.p_lay); auto t_lay = subview_2d(m_buffer.t_lay);
          auto p_lev = subview_2d(m_buffer.p_lev); auto z_del = subview_2d(m_buffer.z_del);
      ...
      COMPARE_ALL_WRAP(std::vector<real2d>({lwp, iwp}), std::vector<real2dk>({lwp_k, iwp_k}));
#endif
