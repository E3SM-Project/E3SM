...
struct ConvertToRrtmgpSubview {
  ...
  // items from the field manager views due to simd packs. If we don't trim,
  // then check_range_k will fail due to looking at unused values. Once rrtmgp
  // can handle packs, this won't be necessary.
  template <typename FieldView, typename BufferView>
  FieldView subview2d(const FieldView &field_view, const BufferView &,
                      const int inner_dim) const {
    return subview2d_impl(field_view, inner_dim);
  }
#endif

  template <typename View> View subview3d(const View &v) const {
...
RRTMGPRadiation::RRTMGPRadiation(const ekat::Comm &comm,
                                 const ekat::ParameterList &params)
    : AtmosphereProcess(comm, params) {
  // Gather the active gases from the rrtmgp parameter list and assign to the
  // m_gas_names vector.
  const auto &active_gases =
      m_params.get<std::vector<std::string>>("active_gases");
  for (auto &it : active_gases) {
    // Make sure only unique names are added
    if (not ekat::contains(m_gas_names, it)) {
      m_gas_names.push_back(it);
      if (it == "o3") {
        TraceGasesWorkaround::singleton().add_active_gas(it +
                                                         "_volume_mix_ratio");
      }
    }
  }
...
void RRTMGPRadiation::set_grids(
  ...
  m_col_chunk_size =
      std::min(m_params.get("column_chunk_size", m_ncol), m_ncol);
  ...
  for (int i = 0; i < m_num_col_chunks; ++i) {
    m_col_chunk_beg[i + 1] =
        std::min(m_ncol, m_col_chunk_beg[i] + m_col_chunk_size);
  }
  ...
  FieldLayout scalar3d_swband =
      m_grid->get_3d_vector_layout(true, m_nswbands, "swband");
  FieldLayout scalar3d_lwband =
      m_grid->get_3d_vector_layout(true, m_nlwbands, "lwband");
  FieldLayout scalar3d_swgpts =
      m_grid->get_3d_vector_layout(true, m_nswgpts, "swgpt");
  FieldLayout scalar3d_lwgpts =
      m_grid->get_3d_vector_layout(true, m_nlwgpts, "lwgpt");
  ...
  for (auto &it : m_gas_names) {
    // Add gas VOLUME mixing ratios (moles of gas / moles of air; what actually
    // gets input to RRTMGP)
    if (it == "o3") {
      // o3 is read from file, or computed by chemistry
      add_field<Required>(it + "_volume_mix_ratio", scalar3d_mid, mol / mol,
                          grid_name);
    } else {
      // the rest are computed by RRTMGP from prescribed surface values
      // NOTE: this may change at some point
      add_field<Computed>(it + "_volume_mix_ratio", scalar3d_mid, mol / mol,
                          grid_name);
    }
  }
  ...
  add_field<Computed>("SW_clnclrsky_flux_dn_dir", scalar3d_int, W / m2,
                      grid_name);
  ...
  for (std::string prefix : {"sw", "lw"}) {
    ...
    if (not m_grid->has_geometry_data(prefix + "band_bounds")) {
      ...
      // NOTE: use append, so we get builtin name for (CMP,2) dim, without
      // hard-coding it here
      FieldLayout layout({CMP}, {nbands}, {prefix + "band"});
      layout.append_dim(CMP, 2);
      Field bounds(
          FieldIdentifier(prefix + "band_bounds", layout, 1 / cm, grid_name));
      bounds.allocate_view();

      std::string fname =
          m_params.get<std::string>("rrtmgp_coefficients_file_" + prefix);
      scorpio::register_file(fname, scorpio::FileMode::Read);
      scorpio::read_var(fname, "bnd_limits_wavenumber",
                        bounds.get_view<Real **, Host>().data());
    ...
    // If no bounds were in the grid, the bands centerpoint likely wouldn't
    // either. Still, let's check...
...
size_t RRTMGPRadiation::requested_buffer_size_in_bytes() const {
  const size_t interface_request =
      ...
      Buffer::num_3d_nlev_nswbands * m_col_chunk_size * (m_nlay + 1) *
          m_nswbands +
      Buffer::num_3d_nlev_nlwbands * m_col_chunk_size * (m_nlay + 1) *
          m_nlwbands +
...
void RRTMGPRadiation::init_buffers(const ATMBufferManager &buffer_manager) {
  ...
  m_buffer.sfc_alb_dir_vis = decltype(m_buffer.sfc_alb_dir_vis)(
      "sfc_alb_dir_vis", mem, m_col_chunk_size);
  mem += m_buffer.sfc_alb_dir_vis.totElems();
  m_buffer.sfc_alb_dir_nir = decltype(m_buffer.sfc_alb_dir_nir)(
      "sfc_alb_dir_nir", mem, m_col_chunk_size);
  mem += m_buffer.sfc_alb_dir_nir.totElems();
  m_buffer.sfc_alb_dif_vis = decltype(m_buffer.sfc_alb_dif_vis)(
      "sfc_alb_dif_vis", mem, m_col_chunk_size);
  mem += m_buffer.sfc_alb_dif_vis.totElems();
  m_buffer.sfc_alb_dif_nir = decltype(m_buffer.sfc_alb_dif_nir)(
      "sfc_alb_dif_nir", mem, m_col_chunk_size);
  mem += m_buffer.sfc_alb_dif_nir.totElems();
  m_buffer.sfc_flux_dir_vis = decltype(m_buffer.sfc_flux_dir_vis)(
      "sfc_flux_dir_vis", mem, m_col_chunk_size);
  mem += m_buffer.sfc_flux_dir_vis.totElems();
  m_buffer.sfc_flux_dir_nir = decltype(m_buffer.sfc_flux_dir_nir)(
      "sfc_flux_dir_nir", mem, m_col_chunk_size);
  mem += m_buffer.sfc_flux_dir_nir.totElems();
  m_buffer.sfc_flux_dif_vis = decltype(m_buffer.sfc_flux_dif_vis)(
      "sfc_flux_dif_vis", mem, m_col_chunk_size);
  mem += m_buffer.sfc_flux_dif_vis.totElems();
  m_buffer.sfc_flux_dif_nir = decltype(m_buffer.sfc_flux_dif_nir)(
      "sfc_flux_dif_nir", mem, m_col_chunk_size);
  mem += m_buffer.sfc_flux_dif_nir.totElems();
  m_buffer.cosine_zenith =
      decltype(m_buffer.cosine_zenith)(mem, m_col_chunk_size);
  mem += m_buffer.cosine_zenith.size();

  // 2d arrays
  m_buffer.p_lay =
      decltype(m_buffer.p_lay)("p_lay", mem, m_col_chunk_size, m_nlay);
  mem += m_buffer.p_lay.totElems();
  m_buffer.t_lay =
      decltype(m_buffer.t_lay)("t_lay", mem, m_col_chunk_size, m_nlay);
  mem += m_buffer.t_lay.totElems();
  m_buffer.z_del =
      decltype(m_buffer.z_del)("z_del", mem, m_col_chunk_size, m_nlay);
  mem += m_buffer.z_del.totElems();
  m_buffer.p_del =
      decltype(m_buffer.p_del)("p_del", mem, m_col_chunk_size, m_nlay);
  ...
  m_buffer.cldfrac_tot = decltype(m_buffer.cldfrac_tot)(
      "cldfrac_tot", mem, m_col_chunk_size, m_nlay);
  mem += m_buffer.cldfrac_tot.totElems();
  m_buffer.eff_radius_qc = decltype(m_buffer.eff_radius_qc)(
      "eff_radius_qc", mem, m_col_chunk_size, m_nlay);
  mem += m_buffer.eff_radius_qc.totElems();
  m_buffer.eff_radius_qi = decltype(m_buffer.eff_radius_qi)(
      "eff_radius_qi", mem, m_col_chunk_size, m_nlay);
  mem += m_buffer.eff_radius_qi.totElems();
  m_buffer.tmp2d =
      decltype(m_buffer.tmp2d)("tmp2d", mem, m_col_chunk_size, m_nlay);
  ...
  m_buffer.sw_heating = decltype(m_buffer.sw_heating)("sw_heating", mem,
                                                      m_col_chunk_size, m_nlay);
  mem += m_buffer.sw_heating.totElems();
  m_buffer.lw_heating = decltype(m_buffer.lw_heating)("lw_heating", mem,
                                                      m_col_chunk_size, m_nlay);
  mem += m_buffer.lw_heating.totElems();
  m_buffer.p_lev =
      decltype(m_buffer.p_lev)("p_lev", mem, m_col_chunk_size, m_nlay + 1);
  mem += m_buffer.p_lev.totElems();
  m_buffer.t_lev =
      decltype(m_buffer.t_lev)("t_lev", mem, m_col_chunk_size, m_nlay + 1);
  mem += m_buffer.t_lev.totElems();
  m_buffer.d_tint =
      decltype(m_buffer.d_tint)(mem, m_col_chunk_size, m_nlay + 1);
  mem += m_buffer.d_tint.size();
  m_buffer.d_dz = decltype(m_buffer.d_dz)(mem, m_col_chunk_size, m_nlay);
  mem += m_buffer.d_dz.size();
  // 3d arrays
  m_buffer.sw_flux_up = decltype(m_buffer.sw_flux_up)(
      "sw_flux_up", mem, m_col_chunk_size, m_nlay + 1);
  mem += m_buffer.sw_flux_up.totElems();
  m_buffer.sw_flux_dn = decltype(m_buffer.sw_flux_dn)(
      "sw_flux_dn", mem, m_col_chunk_size, m_nlay + 1);
  mem += m_buffer.sw_flux_dn.totElems();
  m_buffer.sw_flux_dn_dir = decltype(m_buffer.sw_flux_dn_dir)(
      "sw_flux_dn_dir", mem, m_col_chunk_size, m_nlay + 1);
  mem += m_buffer.sw_flux_dn_dir.totElems();
  m_buffer.lw_flux_up = decltype(m_buffer.lw_flux_up)(
      "lw_flux_up", mem, m_col_chunk_size, m_nlay + 1);
  mem += m_buffer.lw_flux_up.totElems();
  m_buffer.lw_flux_dn = decltype(m_buffer.lw_flux_dn)(
      "lw_flux_dn", mem, m_col_chunk_size, m_nlay + 1);
  ...
  m_buffer.sw_clnclrsky_flux_dn_dir =
      decltype(m_buffer.sw_clnclrsky_flux_dn_dir)(
          "sw_clnclrsky_flux_dn_dir", mem, m_col_chunk_size, m_nlay + 1);
  mem += m_buffer.sw_clnclrsky_flux_dn_dir.totElems();
  m_buffer.sw_clrsky_flux_up = decltype(m_buffer.sw_clrsky_flux_up)(
      "sw_clrsky_flux_up", mem, m_col_chunk_size, m_nlay + 1);
  mem += m_buffer.sw_clrsky_flux_up.totElems();
  m_buffer.sw_clrsky_flux_dn = decltype(m_buffer.sw_clrsky_flux_dn)(
      "sw_clrsky_flux_dn", mem, m_col_chunk_size, m_nlay + 1);
  ...
  m_buffer.sw_clnsky_flux_up = decltype(m_buffer.sw_clnsky_flux_up)(
      "sw_clnsky_flux_up", mem, m_col_chunk_size, m_nlay + 1);
  mem += m_buffer.sw_clnsky_flux_up.totElems();
  m_buffer.sw_clnsky_flux_dn = decltype(m_buffer.sw_clnsky_flux_dn)(
      "sw_clnsky_flux_dn", mem, m_col_chunk_size, m_nlay + 1);
  ...
  m_buffer.lw_clrsky_flux_up = decltype(m_buffer.lw_clrsky_flux_up)(
      "lw_clrsky_flux_up", mem, m_col_chunk_size, m_nlay + 1);
  mem += m_buffer.lw_clrsky_flux_up.totElems();
  m_buffer.lw_clrsky_flux_dn = decltype(m_buffer.lw_clrsky_flux_dn)(
      "lw_clrsky_flux_dn", mem, m_col_chunk_size, m_nlay + 1);
  mem += m_buffer.lw_clrsky_flux_dn.totElems();
  m_buffer.lw_clnsky_flux_up = decltype(m_buffer.lw_clnsky_flux_up)(
      "lw_clnsky_flux_up", mem, m_col_chunk_size, m_nlay + 1);
  mem += m_buffer.lw_clnsky_flux_up.totElems();
  m_buffer.lw_clnsky_flux_dn = decltype(m_buffer.lw_clnsky_flux_dn)(
      "lw_clnsky_flux_dn", mem, m_col_chunk_size, m_nlay + 1);
  ...
  m_buffer.sfc_alb_dir = decltype(m_buffer.sfc_alb_dir)(
      "sfc_alb_dir", mem, m_col_chunk_size, m_nswbands);
  mem += m_buffer.sfc_alb_dir.totElems();
  m_buffer.sfc_alb_dif = decltype(m_buffer.sfc_alb_dif)(
      "sfc_alb_dif", mem, m_col_chunk_size, m_nswbands);
  mem += m_buffer.sfc_alb_dif.totElems();
  // 3d arrays with extra band dimension (aerosol optics by band)
  m_buffer.aero_tau_sw = decltype(m_buffer.aero_tau_sw)(
      "aero_tau_sw", mem, m_col_chunk_size, m_nlay, m_nswbands);
  mem += m_buffer.aero_tau_sw.totElems();
  m_buffer.aero_ssa_sw = decltype(m_buffer.aero_ssa_sw)(
      "aero_ssa_sw", mem, m_col_chunk_size, m_nlay, m_nswbands);
  mem += m_buffer.aero_ssa_sw.totElems();
  m_buffer.aero_g_sw = decltype(m_buffer.aero_g_sw)(
      "aero_g_sw", mem, m_col_chunk_size, m_nlay, m_nswbands);
  mem += m_buffer.aero_g_sw.totElems();
  m_buffer.aero_tau_lw = decltype(m_buffer.aero_tau_lw)(
      "aero_tau_lw", mem, m_col_chunk_size, m_nlay, m_nlwbands);
  mem += m_buffer.aero_tau_lw.totElems();
  // 3d arrays with extra ngpt dimension (cloud optics by gpoint; primarily for
  // debugging)
  m_buffer.cld_tau_sw_gpt = decltype(m_buffer.cld_tau_sw_gpt)(
      "cld_tau_sw_gpt", mem, m_col_chunk_size, m_nlay, m_nswgpts);
  mem += m_buffer.cld_tau_sw_gpt.totElems();
  m_buffer.cld_tau_lw_gpt = decltype(m_buffer.cld_tau_lw_gpt)(
      "cld_tau_lw_gpt", mem, m_col_chunk_size, m_nlay, m_nlwgpts);
  mem += m_buffer.cld_tau_lw_gpt.totElems();
  m_buffer.cld_tau_sw_bnd = decltype(m_buffer.cld_tau_sw_bnd)(
      "cld_tau_sw_bnd", mem, m_col_chunk_size, m_nlay, m_nswbands);
  mem += m_buffer.cld_tau_sw_bnd.totElems();
  m_buffer.cld_tau_lw_bnd = decltype(m_buffer.cld_tau_lw_bnd)(
      "cld_tau_lw_bnd", mem, m_col_chunk_size, m_nlay, m_nlwbands);
  ...
  m_buffer.sfc_alb_dir_vis_k =
      decltype(m_buffer.sfc_alb_dir_vis_k)(mem, m_col_chunk_size);
  mem += m_buffer.sfc_alb_dir_vis_k.size();
  m_buffer.sfc_alb_dir_nir_k =
      decltype(m_buffer.sfc_alb_dir_nir_k)(mem, m_col_chunk_size);
  mem += m_buffer.sfc_alb_dir_nir_k.size();
  m_buffer.sfc_alb_dif_vis_k =
      decltype(m_buffer.sfc_alb_dif_vis_k)(mem, m_col_chunk_size);
  mem += m_buffer.sfc_alb_dif_vis_k.size();
  m_buffer.sfc_alb_dif_nir_k =
      decltype(m_buffer.sfc_alb_dif_nir_k)(mem, m_col_chunk_size);
  mem += m_buffer.sfc_alb_dif_nir_k.size();
  m_buffer.sfc_flux_dir_vis_k =
      decltype(m_buffer.sfc_flux_dir_vis_k)(mem, m_col_chunk_size);
  mem += m_buffer.sfc_flux_dir_vis_k.size();
  m_buffer.sfc_flux_dir_nir_k =
      decltype(m_buffer.sfc_flux_dir_nir_k)(mem, m_col_chunk_size);
  mem += m_buffer.sfc_flux_dir_nir_k.size();
  m_buffer.sfc_flux_dif_vis_k =
      decltype(m_buffer.sfc_flux_dif_vis_k)(mem, m_col_chunk_size);
  mem += m_buffer.sfc_flux_dif_vis_k.size();
  m_buffer.sfc_flux_dif_nir_k =
      decltype(m_buffer.sfc_flux_dif_nir_k)(mem, m_col_chunk_size);
  ...
  m_buffer.cldfrac_tot_k =
      decltype(m_buffer.cldfrac_tot_k)(mem, m_col_chunk_size, m_nlay);
  mem += m_buffer.cldfrac_tot_k.size();
  m_buffer.eff_radius_qc_k =
      decltype(m_buffer.eff_radius_qc_k)(mem, m_col_chunk_size, m_nlay);
  mem += m_buffer.eff_radius_qc_k.size();
  m_buffer.eff_radius_qi_k =
      decltype(m_buffer.eff_radius_qi_k)(mem, m_col_chunk_size, m_nlay);
  ...
  m_buffer.sw_heating_k =
      decltype(m_buffer.sw_heating_k)(mem, m_col_chunk_size, m_nlay);
  mem += m_buffer.sw_heating_k.size();
  m_buffer.lw_heating_k =
      decltype(m_buffer.lw_heating_k)(mem, m_col_chunk_size, m_nlay);
  mem += m_buffer.lw_heating_k.size();
  m_buffer.p_lev_k =
      decltype(m_buffer.p_lev_k)(mem, m_col_chunk_size, m_nlay + 1);
  mem += m_buffer.p_lev_k.size();
  m_buffer.t_lev_k =
      decltype(m_buffer.t_lev_k)(mem, m_col_chunk_size, m_nlay + 1);
  ...
  mem += m_buffer.d_dz.size();
  // 3d arrays
  m_buffer.sw_flux_up_k =
      decltype(m_buffer.sw_flux_up_k)(mem, m_col_chunk_size, m_nlay + 1);
  mem += m_buffer.sw_flux_up_k.size();
  m_buffer.sw_flux_dn_k =
      decltype(m_buffer.sw_flux_dn_k)(mem, m_col_chunk_size, m_nlay + 1);
  ...
  m_buffer.lw_flux_up_k =
      decltype(m_buffer.lw_flux_up_k)(mem, m_col_chunk_size, m_nlay + 1);
  mem += m_buffer.lw_flux_up_k.size();
  m_buffer.lw_flux_dn_k =
      decltype(m_buffer.lw_flux_dn_k)(mem, m_col_chunk_size, m_nlay + 1);
  mem += m_buffer.lw_flux_dn_k.size();
  m_buffer.sw_clnclrsky_flux_up_k = decltype(m_buffer.sw_clnclrsky_flux_up_k)(
      mem, m_col_chunk_size, m_nlay + 1);
  mem += m_buffer.sw_clnclrsky_flux_up_k.size();
  m_buffer.sw_clnclrsky_flux_dn_k = decltype(m_buffer.sw_clnclrsky_flux_dn_k)(
      mem, m_col_chunk_size, m_nlay + 1);
  mem += m_buffer.sw_clnclrsky_flux_dn_k.size();
  m_buffer.sw_clnclrsky_flux_dn_dir_k =
      decltype(m_buffer.sw_clnclrsky_flux_dn_dir_k)(mem, m_col_chunk_size,
                                                    m_nlay + 1);
  ...
  m_buffer.sw_clrsky_flux_dn_dir_k = decltype(m_buffer.sw_clrsky_flux_dn_dir_k)(
      mem, m_col_chunk_size, m_nlay + 1);
  ...
  m_buffer.sw_clnsky_flux_dn_dir_k = decltype(m_buffer.sw_clnsky_flux_dn_dir_k)(
      mem, m_col_chunk_size, m_nlay + 1);
  mem += m_buffer.sw_clnsky_flux_dn_dir_k.size();
  m_buffer.lw_clnclrsky_flux_up_k = decltype(m_buffer.lw_clnclrsky_flux_up_k)(
      mem, m_col_chunk_size, m_nlay + 1);
  mem += m_buffer.lw_clnclrsky_flux_up_k.size();
  m_buffer.lw_clnclrsky_flux_dn_k = decltype(m_buffer.lw_clnclrsky_flux_dn_k)(
      mem, m_col_chunk_size, m_nlay + 1);
  ...
  // 3d arrays with nswbands dimension (shortwave fluxes by band)
  m_buffer.sw_bnd_flux_up_k = decltype(m_buffer.sw_bnd_flux_up_k)(
      mem, m_col_chunk_size, m_nlay + 1, m_nswbands);
  mem += m_buffer.sw_bnd_flux_up_k.size();
  m_buffer.sw_bnd_flux_dn_k = decltype(m_buffer.sw_bnd_flux_dn_k)(
      mem, m_col_chunk_size, m_nlay + 1, m_nswbands);
  mem += m_buffer.sw_bnd_flux_dn_k.size();
  m_buffer.sw_bnd_flux_dir_k = decltype(m_buffer.sw_bnd_flux_dir_k)(
      mem, m_col_chunk_size, m_nlay + 1, m_nswbands);
  mem += m_buffer.sw_bnd_flux_dir_k.size();
  m_buffer.sw_bnd_flux_dif_k = decltype(m_buffer.sw_bnd_flux_dif_k)(
      mem, m_col_chunk_size, m_nlay + 1, m_nswbands);
  mem += m_buffer.sw_bnd_flux_dif_k.size();
  // 3d arrays with nlwbands dimension (longwave fluxes by band)
  m_buffer.lw_bnd_flux_up_k = decltype(m_buffer.lw_bnd_flux_up_k)(
      mem, m_col_chunk_size, m_nlay + 1, m_nlwbands);
  mem += m_buffer.lw_bnd_flux_up_k.size();
  m_buffer.lw_bnd_flux_dn_k = decltype(m_buffer.lw_bnd_flux_dn_k)(
      mem, m_col_chunk_size, m_nlay + 1, m_nlwbands);
  mem += m_buffer.lw_bnd_flux_dn_k.size();
  // 2d arrays with extra nswbands dimension (surface albedos by band)
  m_buffer.sfc_alb_dir_k =
      decltype(m_buffer.sfc_alb_dir_k)(mem, m_col_chunk_size, m_nswbands);
  mem += m_buffer.sfc_alb_dir_k.size();
  m_buffer.sfc_alb_dif_k =
      decltype(m_buffer.sfc_alb_dif_k)(mem, m_col_chunk_size, m_nswbands);
  mem += m_buffer.sfc_alb_dif_k.size();
  // 3d arrays with extra band dimension (aerosol optics by band)
  m_buffer.aero_tau_sw_k = decltype(m_buffer.aero_tau_sw_k)(
      mem, m_col_chunk_size, m_nlay, m_nswbands);
  mem += m_buffer.aero_tau_sw_k.size();
  m_buffer.aero_ssa_sw_k = decltype(m_buffer.aero_ssa_sw_k)(
      mem, m_col_chunk_size, m_nlay, m_nswbands);
  mem += m_buffer.aero_ssa_sw_k.size();
  m_buffer.aero_g_sw_k =
      decltype(m_buffer.aero_g_sw_k)(mem, m_col_chunk_size, m_nlay, m_nswbands);
  mem += m_buffer.aero_g_sw_k.size();
  m_buffer.aero_tau_lw_k = decltype(m_buffer.aero_tau_lw_k)(
      mem, m_col_chunk_size, m_nlay, m_nlwbands);
  mem += m_buffer.aero_tau_lw_k.size();
  // 3d arrays with extra ngpt dimension (cloud optics by gpoint; primarily for
  // debugging)
  m_buffer.cld_tau_sw_gpt_k = decltype(m_buffer.cld_tau_sw_gpt_k)(
      mem, m_col_chunk_size, m_nlay, m_nswgpts);
  mem += m_buffer.cld_tau_sw_gpt_k.size();
  m_buffer.cld_tau_lw_gpt_k = decltype(m_buffer.cld_tau_lw_gpt_k)(
      mem, m_col_chunk_size, m_nlay, m_nlwgpts);
  mem += m_buffer.cld_tau_lw_gpt_k.size();
  m_buffer.cld_tau_sw_bnd_k = decltype(m_buffer.cld_tau_sw_bnd_k)(
      mem, m_col_chunk_size, m_nlay, m_nswbands);
  mem += m_buffer.cld_tau_sw_bnd_k.size();
  m_buffer.cld_tau_lw_bnd_k = decltype(m_buffer.cld_tau_lw_bnd_k)(
      mem, m_col_chunk_size, m_nlay, m_nlwbands);
  ...
  EKAT_REQUIRE_MSG(
      ...
      "Error! Used memory != requested memory for RRTMGPRadiation.");
...
void RRTMGPRadiation::initialize_impl(const RunType /* run_type */) {
  ...
  // irradiance at TOA).  Used for idealized experiments such as RCE. Disabled
  // when value is less than 0.
  m_fixed_total_solar_irradiance =
      m_params.get<double>("fixed_total_solar_irradiance", -9999);

  // Determine whether or not we are using a fixed solar zenith angle (positive
  // value)
  m_fixed_solar_zenith_angle =
      m_params.get<double>("fixed_solar_zenith_angle", -9999);
  ...
  std::string coefficients_file_sw =
      m_params.get<std::string>("rrtmgp_coefficients_file_sw");
  std::string coefficients_file_lw =
      m_params.get<std::string>("rrtmgp_coefficients_file_lw");
  std::string cloud_optics_file_sw =
      m_params.get<std::string>("rrtmgp_cloud_optics_file_sw");
  std::string cloud_optics_file_lw =
      m_params.get<std::string>("rrtmgp_cloud_optics_file_lw");
#ifdef RRTMGP_ENABLE_YAKL
  m_gas_concs.init(gas_names_yakl_offset, m_col_chunk_size, m_nlay);
  rrtmgp::rrtmgp_initialize(m_gas_concs, coefficients_file_sw,
  ...
  interface_t::rrtmgp_initialize(
      m_gas_concs_k, coefficients_file_sw, coefficients_file_lw,
      cloud_optics_file_sw, cloud_optics_file_lw, m_atm_logger, multiplier);
  ...
  add_invariant_check<FieldWithinIntervalCheck>(get_field_out("T_mid"), m_grid,
                                                100.0, 500.0, false);

  // VMR of n2 and co is currently prescribed as a constant value, read from
  // file
...
void RRTMGPRadiation::run_impl(const double dt) {
  ...
  auto d_sfc_alb_dir_vis =
      get_field_in("sfc_alb_dir_vis").get_view<const Real *>();
  auto d_sfc_alb_dir_nir =
      get_field_in("sfc_alb_dir_nir").get_view<const Real *>();
  auto d_sfc_alb_dif_vis =
      get_field_in("sfc_alb_dif_vis").get_view<const Real *>();
  auto d_sfc_alb_dif_nir =
      get_field_in("sfc_alb_dif_nir").get_view<const Real *>();
  ...
  auto d_surf_lw_flux_up =
      get_field_in("surf_lw_flux_up").get_view<const Real *>();
  // Output fields
  auto d_tmid        = get_field_out("T_mid").get_view<Real **>();
  auto d_cldfrac_rad = get_field_out("cldfrac_rad").get_view<Real **>();

  // Aerosol optics only exist if m_do_aerosol_rad is true, so declare views and
  // copy from FM if so
  ...
  auto d_sw_clnclrsky_flux_up =
      get_field_out("SW_clnclrsky_flux_up").get_view<Real **>();
  auto d_sw_clnclrsky_flux_dn =
      get_field_out("SW_clnclrsky_flux_dn").get_view<Real **>();
  auto d_sw_clnclrsky_flux_dn_dir =
      get_field_out("SW_clnclrsky_flux_dn_dir").get_view<Real **>();
  auto d_sw_clrsky_flux_up =
      get_field_out("SW_clrsky_flux_up").get_view<Real **>();
  auto d_sw_clrsky_flux_dn =
      get_field_out("SW_clrsky_flux_dn").get_view<Real **>();
  auto d_sw_clrsky_flux_dn_dir =
      get_field_out("SW_clrsky_flux_dn_dir").get_view<Real **>();
  auto d_sw_clnsky_flux_up =
      get_field_out("SW_clnsky_flux_up").get_view<Real **>();
  auto d_sw_clnsky_flux_dn =
      get_field_out("SW_clnsky_flux_dn").get_view<Real **>();
  auto d_sw_clnsky_flux_dn_dir =
      get_field_out("SW_clnsky_flux_dn_dir").get_view<Real **>();
  auto d_lw_clnclrsky_flux_up =
      get_field_out("LW_clnclrsky_flux_up").get_view<Real **>();
  auto d_lw_clnclrsky_flux_dn =
      get_field_out("LW_clnclrsky_flux_dn").get_view<Real **>();
  auto d_lw_clrsky_flux_up =
      get_field_out("LW_clrsky_flux_up").get_view<Real **>();
  auto d_lw_clrsky_flux_dn =
      get_field_out("LW_clrsky_flux_dn").get_view<Real **>();
  auto d_lw_clnsky_flux_up =
      get_field_out("LW_clnsky_flux_up").get_view<Real **>();
  auto d_lw_clnsky_flux_dn =
      get_field_out("LW_clnsky_flux_dn").get_view<Real **>();
  auto d_rad_heating_pdel =
      get_field_out("rad_heating_pdel").get_view<Real **>();
  auto d_sfc_flux_dir_vis =
      get_field_out("sfc_flux_dir_vis").get_view<Real *>();
  auto d_sfc_flux_dir_nir =
      get_field_out("sfc_flux_dir_nir").get_view<Real *>();
  auto d_sfc_flux_dif_vis =
      get_field_out("sfc_flux_dif_vis").get_view<Real *>();
  auto d_sfc_flux_dif_nir =
      get_field_out("sfc_flux_dif_nir").get_view<Real *>();
  ...
  auto update_rad =
      scream::rrtmgp::radiation_do(m_rad_freq_in_steps, ts.get_num_steps());

  if (update_rad) {
    // On each chunk, we internally "reset" the GasConcs object to subview the
    // concs 3d array with the correct ncol dimension. So let's keep a copy of
    // the original (ref-counted) array, to restore at the end inside the
    // m_gast_concs object.
#ifdef RRTMGP_ENABLE_YAKL
    ...
    // Precompute VMR for all gases, on all cols, before starting the chunks
    // loop
    //
    // h2o is taken from qv
    // o3 is computed elsewhere (either read from file or computed by
    // chemistry); n2 and co are set to constants and are not handled by trcmix;
    ...
    for (int igas = 0; igas < m_ngas; igas++) {
      ...
      if (name == "o3" or name == "n2" or name == "co")
        continue;

      auto d_vmr =
          get_field_out(name + "_volume_mix_ratio").get_view<Real **>();
      if (name == "h2o") {
        // h2o is (wet) mass mixing ratio in FM, otherwise known as "qv", which
        // we've already read in above Convert to vmr
        const auto policy =
            ekat::ExeSpaceUtils<ExeSpace>::get_default_team_policy(m_ncol,
                                                                   m_nlay);
        Kokkos::parallel_for(
            policy, KOKKOS_LAMBDA(const MemberType &team) {
              const int icol = team.league_rank();
              Kokkos::parallel_for(
                  Kokkos::TeamVectorRange(team, nlay), [&](const int &k) {
                    d_vmr(icol, k) = PF::calculate_vmr_from_mmr(
                        gas_mol_weights[igas], d_qv(icol, k), d_qv(icol, k));
                  });
            });
        Kokkos::fence();
      } else {
        // This gives (dry) mass mixing ratios
        scream::physics::trcmix(name, m_nlay, m_lat.get_view<const Real *>(),
                                d_pmid, d_vmr, m_co2vmr, m_n2ovmr, m_ch4vmr,
                                m_f11vmr, m_f12vmr);
        ...
        Kokkos::parallel_for(
            policy, KOKKOS_LAMBDA(const MemberType &team) {
              ...
              Kokkos::parallel_for(
                  Kokkos::TeamVectorRange(team, nlay), [&](const int &k) {
                    d_vmr(i, k) =
                        air_mol_weight / gas_mol_weights[igas] * d_vmr(i, k);
                  });
            });
      }
    }
    ...
    for (int ic = 0; ic < m_num_col_chunks; ++ic) {
      ...
      this->log(LogLevel::debug,
                "[RRTMGP::run_impl] Col chunk beg,end: " + std::to_string(beg) +
                    ", " + std::to_string(beg + ncol) + "\n");

      // d_tint and d_dz are used in eamxx calls and therefore
      // must be layout right
      ulrreal2dk d_tint =
          ulrreal2dk(m_buffer.d_tint.data(), m_col_chunk_size, m_nlay + 1);
      ulrreal2dk d_dz =
          ulrreal2dk(m_buffer.d_dz.data(), m_col_chunk_size, m_nlay);
      auto d_mu0 = m_buffer.cosine_zenith;
#ifdef RRTMGP_ENABLE_YAKL
      TIMED_INLINE_KERNEL(
          init_views,
          // Create YAKL arrays. RRTMGP expects YAKL arrays with styleFortran,
          // i.e., data has ncol as the fastest index. For this reason we must
          // copy the data.
          ...
          auto subview_3d = [&](const real3d v) -> real3d {
            return real3d(v.label(), v.myData, ncol, v.dimension[1],
                          v.dimension[2]);
          };
      ...
      TIMED_INLINE_KERNEL(
          ...
          // Note, ncol will not necessary be m_col_chunk_size because the
          // number of cols will not always be evenly divided by
          // m_col_chunk_size. In most cases, the extra space will not cause any
          // problems, but it does sometimes.
          ...
          auto sw_flux_up_k =
              conv.subview2d(d_sw_flux_up, m_buffer.sw_flux_up_k, m_nlay + 1);
          auto sw_flux_dn_k =
              conv.subview2d(d_sw_flux_dn, m_buffer.sw_flux_dn_k, m_nlay + 1);
          auto sw_flux_dn_dir_k = conv.subview2d(
              d_sw_flux_dn_dir, m_buffer.sw_flux_dn_dir_k, m_nlay + 1);
          auto lw_flux_up_k =
              conv.subview2d(d_lw_flux_up, m_buffer.lw_flux_up_k, m_nlay + 1);
          auto lw_flux_dn_k =
              conv.subview2d(d_lw_flux_dn, m_buffer.lw_flux_dn_k, m_nlay + 1);
          auto sw_clnclrsky_flux_up_k =
              conv.subview2d(d_sw_clnclrsky_flux_up,
                             m_buffer.sw_clnclrsky_flux_up_k, m_nlay + 1);
          auto sw_clnclrsky_flux_dn_k =
              conv.subview2d(d_sw_clnclrsky_flux_dn,
                             m_buffer.sw_clnclrsky_flux_dn_k, m_nlay + 1);
          auto sw_clnclrsky_flux_dn_dir_k =
              conv.subview2d(d_sw_clnclrsky_flux_dn_dir,
                             m_buffer.sw_clnclrsky_flux_dn_dir_k, m_nlay + 1);
          auto sw_clrsky_flux_up_k = conv.subview2d(
              d_sw_clrsky_flux_up, m_buffer.sw_clrsky_flux_up_k, m_nlay + 1);
          auto sw_clrsky_flux_dn_k = conv.subview2d(
              d_sw_clrsky_flux_dn, m_buffer.sw_clrsky_flux_dn_k, m_nlay + 1);
          auto sw_clrsky_flux_dn_dir_k =
              conv.subview2d(d_sw_clrsky_flux_dn_dir,
                             m_buffer.sw_clrsky_flux_dn_dir_k, m_nlay + 1);
          auto sw_clnsky_flux_up_k = conv.subview2d(
              d_sw_clnsky_flux_up, m_buffer.sw_clnsky_flux_up_k, m_nlay + 1);
          auto sw_clnsky_flux_dn_k = conv.subview2d(
              d_sw_clnsky_flux_dn, m_buffer.sw_clnsky_flux_dn_k, m_nlay + 1);
          auto sw_clnsky_flux_dn_dir_k =
              conv.subview2d(d_sw_clnsky_flux_dn_dir,
                             m_buffer.sw_clnsky_flux_dn_dir_k, m_nlay + 1);
          auto lw_clnclrsky_flux_up_k =
              conv.subview2d(d_lw_clnclrsky_flux_up,
                             m_buffer.lw_clnclrsky_flux_up_k, m_nlay + 1);
          auto lw_clnclrsky_flux_dn_k =
              conv.subview2d(d_lw_clnclrsky_flux_dn,
                             m_buffer.lw_clnclrsky_flux_dn_k, m_nlay + 1);
          auto lw_clrsky_flux_up_k = conv.subview2d(
              d_lw_clrsky_flux_up, m_buffer.lw_clrsky_flux_up_k, m_nlay + 1);
          auto lw_clrsky_flux_dn_k = conv.subview2d(
              d_lw_clrsky_flux_dn, m_buffer.lw_clrsky_flux_dn_k, m_nlay + 1);
          auto lw_clnsky_flux_up_k = conv.subview2d(
              d_lw_clnsky_flux_up, m_buffer.lw_clnsky_flux_up_k, m_nlay + 1);
          auto lw_clnsky_flux_dn_k = conv.subview2d(
              d_lw_clnsky_flux_dn, m_buffer.lw_clnsky_flux_dn_k, m_nlay + 1);
      ...
      // Copy data from the FieldManager to the YAKL arrays
      {
        ...
        // NOTE: Since we are bridging to F90 arrays this must be done on HOST
        // and then
        ...
        if (m_fixed_solar_zenith_angle > 0) {
          ...
          for (int i = 0; i < ncol; i++) {
            double lat =
                h_lat(i + beg) * PC::Pi / 180.0; // Convert lat/lon to radians
            double lon = h_lon(i + beg) * PC::Pi / 180.0;
            h_mu0(i)   = shr_orb_cosz_c2f(calday, lat, lon, delta,
                                          m_rad_freq_in_steps * dt);
          }
        }
        Kokkos::deep_copy(d_mu0, h_mu0);

        const auto policy =
            ekat::ExeSpaceUtils<ExeSpace>::get_default_team_policy(ncol,
                                                                   m_nlay);
        TIMED_KERNEL(
            Kokkos::parallel_for(
                policy, KOKKOS_LAMBDA(const MemberType &team) {
                  ...
                  PF::calculate_dz<Real>(team, pseudo_density, p_mid, T_mid, qv,
                                         dz);
                  team.team_barrier();

                  // Calculate T_int from longwave flux up from the surface,
                  // assuming blackbody emission with emissivity of 1.
                  // TODO: Does land model assume something other than
                  // emissivity of 1? If so we should use that here rather than
                  // assuming perfect blackbody emission. NOTE: RRTMGP can
                  // accept vertical ordering surface to toa, or toa to surface.
                  // The input data for the standalone test is ordered surface
                  // to toa, but SCREAM in general assumes data is toa to
                  // surface. We account for this here by swapping bc_top and
                  // bc_bot in the case that the input data is ordered surface
                  // to toa.
                  const auto T_int = ekat::subview(d_tint, i);
                  const int itop = (p_mid(0) < p_mid(nlay - 1)) ? 0 : nlay - 1;
                  const Real bc_top = T_mid(itop);
                  const Real bc_bot =
                      sqrt(sqrt(d_surf_lw_flux_up(icol) / stebol));
                  if (itop == 0) {
                    CO::compute_interface_values_linear(team, nlay, T_mid, dz,
                                                        bc_top, bc_bot, T_int);
                  } else {
                    CO::compute_interface_values_linear(team, nlay, T_mid, dz,
                                                        bc_bot, bc_top, T_int);
                  ...
                  Kokkos::parallel_for(Kokkos::TeamVectorRange(team, nlay),
                                       [&](const int &k) {
                  ...
                  // Note that RRTMGP expects ordering (col,lay,bnd) but the FM
                  // keeps things in (col,bnd,lay) order
                  if (do_aerosol_rad) {
                    Kokkos::parallel_for(
                        Kokkos::TeamVectorRange(team, nswbands * nlay),
                        [&](const int &idx) {
                          auto b = idx / nlay;
                          auto k = idx % nlay;
                          aero_tau_sw(i + 1, k + 1, b + 1) =
                              d_aero_tau_sw(icol, b, k);
                          aero_ssa_sw(i + 1, k + 1, b + 1) =
                              d_aero_ssa_sw(icol, b, k);
                          aero_g_sw(i + 1, k + 1, b + 1) =
                              d_aero_g_sw(icol, b, k);
                        });
                    Kokkos::parallel_for(
                        Kokkos::TeamVectorRange(team, nlwbands * nlay),
                        [&](const int &idx) {
                          auto b = idx / nlay;
                          auto k = idx % nlay;
                          aero_tau_lw(i + 1, k + 1, b + 1) =
                              d_aero_tau_lw(icol, b, k);
                        });
                  } else {
                    Kokkos::parallel_for(
                        Kokkos::TeamVectorRange(team, nswbands * nlay),
                  ...
                  if (do_aerosol_rad) {
                    ...
                    // cuda complains (in warning only) about these being not
                    // allowed... Kokkos::deep_copy(aero_tau_sw_k, 0);
      ...
      Kokkos::fence();
#ifdef RRTMGP_ENABLE_KOKKOS
      COMPARE_ALL_WRAP(std::vector<real3d>(
                           {aero_tau_sw, aero_ssa_sw, aero_g_sw, aero_tau_lw}),
                       std::vector<real3dk>({aero_tau_sw_k, aero_ssa_sw_k,
                                             aero_g_sw_k, aero_tau_lw_k}));
#endif
      ...
      for (int igas = 0; igas < m_ngas; igas++) {
        ...
        // 'o3' is marked as 'Required' rather than 'Computed', so we need to
        // get the proper field
        auto f =
            name == "o3" ? get_field_in(full_name) : get_field_out(full_name);
        ...
        Kokkos::parallel_for(
            policy, KOKKOS_LAMBDA(const MemberType &team) {
              ...
              Kokkos::parallel_for(
                  Kokkos::TeamVectorRange(team, nlay), [&](const int &k) {
                    tmp2d(i + 1, k + 1) =
                        d_vmr(icol, k); // Note that for YAKL arrays i and k
                                        // start with index 1
                  });
            });
        Kokkos::fence();
      ...
      //
      // If not doing subcolumn sampling for mcica, we want to make sure we use
      // grid-mean condensate for computing cloud optical properties, because we
      // are assuming the entire column is completely clear or cloudy. Thus, in
      // this case we want to set cloud fraction to 0 or 1. Note that we could
      // choose an alternative threshold criteria here, like qc + qi > 1e-5 or
      // something.
      //
      // If we *are* doing subcolumn sampling for MCICA, then keep cloud
      // fraction as input from cloud fraction parameterization, wherever that
      // is computed.
...
#endif
#ifdef RRTMGP_ENABLE_KOKKOS
      interface_t::mixing_ratio_to_cloud_mass(qc_k, cldfrac_tot_k, p_del_k,
                                              lwp_k);
      interface_t::mixing_ratio_to_cloud_mass(qi_k, cldfrac_tot_k, p_del_k,
                                              iwp_k);
      ...
      TIMED_KERNEL(rrtmgp::compute_band_by_band_surface_albedos(
                       ncol, nswbands, sfc_alb_dir_vis, sfc_alb_dir_nir,
                       sfc_alb_dif_vis, sfc_alb_dif_nir, sfc_alb_dir,
                       sfc_alb_dif););
#endif
#ifdef RRTMGP_ENABLE_KOKKOS
      TIMED_KERNEL(interface_t::compute_band_by_band_surface_albedos(
                       ncol, nswbands, sfc_alb_dir_vis_k, sfc_alb_dir_nir_k,
                       sfc_alb_dif_vis_k, sfc_alb_dif_nir_k, sfc_alb_dir_k,
                       sfc_alb_dif_k););
      ...
      TIMED_KERNEL(
          rrtmgp::rrtmgp_main(
              ncol, m_nlay, p_lay, t_lay, p_lev, t_lev, m_gas_concs,
              sfc_alb_dir, sfc_alb_dif, mu0, lwp, iwp, rel, rei, cldfrac_tot,
              aero_tau_sw, aero_ssa_sw, aero_g_sw, aero_tau_lw, cld_tau_sw_bnd,
              cld_tau_lw_bnd, cld_tau_sw_gpt, cld_tau_lw_gpt, sw_flux_up,
              sw_flux_dn, sw_flux_dn_dir, lw_flux_up, lw_flux_dn,
              sw_clnclrsky_flux_up, sw_clnclrsky_flux_dn,
              sw_clnclrsky_flux_dn_dir, sw_clrsky_flux_up, sw_clrsky_flux_dn,
              sw_clrsky_flux_dn_dir, sw_clnsky_flux_up, sw_clnsky_flux_dn,
              sw_clnsky_flux_dn_dir, lw_clnclrsky_flux_up, lw_clnclrsky_flux_dn,
              lw_clrsky_flux_up, lw_clrsky_flux_dn, lw_clnsky_flux_up,
              lw_clnsky_flux_dn, sw_bnd_flux_up, sw_bnd_flux_dn,
              sw_bnd_flux_dir, lw_bnd_flux_up, lw_bnd_flux_dn, eccf,
              m_atm_logger, m_extra_clnclrsky_diag, m_extra_clnsky_diag););
#endif
#ifdef RRTMGP_ENABLE_KOKKOS
      TIMED_KERNEL(
          interface_t::rrtmgp_main(
              ncol, m_nlay, p_lay_k, t_lay_k, p_lev_k, t_lev_k, m_gas_concs_k,
              sfc_alb_dir_k, sfc_alb_dif_k, d_mu0, lwp_k, iwp_k, rel_k, rei_k,
              cldfrac_tot_k, aero_tau_sw_k, aero_ssa_sw_k, aero_g_sw_k,
              aero_tau_lw_k, cld_tau_sw_bnd_k, cld_tau_lw_bnd_k,
              cld_tau_sw_gpt_k, cld_tau_lw_gpt_k, sw_flux_up_k, sw_flux_dn_k,
              sw_flux_dn_dir_k, lw_flux_up_k, lw_flux_dn_k,
              sw_clnclrsky_flux_up_k, sw_clnclrsky_flux_dn_k,
              sw_clnclrsky_flux_dn_dir_k, sw_clrsky_flux_up_k,
              sw_clrsky_flux_dn_k, sw_clrsky_flux_dn_dir_k, sw_clnsky_flux_up_k,
              sw_clnsky_flux_dn_k, sw_clnsky_flux_dn_dir_k,
              lw_clnclrsky_flux_up_k, lw_clnclrsky_flux_dn_k,
              lw_clrsky_flux_up_k, lw_clrsky_flux_dn_k, lw_clnsky_flux_up_k,
              lw_clnsky_flux_dn_k, sw_bnd_flux_up_k, sw_bnd_flux_dn_k,
              sw_bnd_flux_dir_k, lw_bnd_flux_up_k, lw_bnd_flux_dn_k, eccf,
              m_atm_logger, m_extra_clnclrsky_diag, m_extra_clnsky_diag););
      COMPARE_ALL_WRAP(std::vector<real2d>({sw_flux_up,
                                            ...
                                            lw_clnsky_flux_up,
                                            lw_clnsky_flux_dn}),
                       std::vector<real2dk>({sw_flux_up_k,
                                             sw_flux_dn_k,
                                             sw_flux_dn_dir_k,
                                             lw_flux_up_k,
                                             lw_flux_dn_k,
                                             sw_clnclrsky_flux_up_k,
                                             sw_clnclrsky_flux_dn_k,
                                             sw_clnclrsky_flux_dn_dir_k,
                                             sw_clrsky_flux_up_k,
                                             sw_clrsky_flux_dn_k,
                                             sw_clrsky_flux_dn_dir_k,
                                             sw_clnsky_flux_up_k,
                                             sw_clnsky_flux_dn_k,
                                             sw_clnsky_flux_dn_dir_k,
                                             lw_clnclrsky_flux_up_k,
                                             lw_clnclrsky_flux_dn_k,
                                             lw_clrsky_flux_up_k,
                                             lw_clrsky_flux_dn_k,
                                             lw_clnsky_flux_up_k,
                                             lw_clnsky_flux_dn_k}));
      COMPARE_ALL_WRAP(
          std::vector<real3d>({sw_bnd_flux_up, sw_bnd_flux_dn, sw_bnd_flux_dir,
                               lw_bnd_flux_up, lw_bnd_flux_dn}),
          std::vector<real3dk>({sw_bnd_flux_up_k, sw_bnd_flux_dn_k,
                                sw_bnd_flux_dir_k, lw_bnd_flux_up_k,
                                lw_bnd_flux_dn_k}));
#endif
      ...
      TIMED_INLINE_KERNEL(
          ...
          auto lw_heating = m_buffer.lw_heating; rrtmgp::compute_heating_rate(
              sw_flux_up, sw_flux_dn, p_del, sw_heating);
          rrtmgp::compute_heating_rate(lw_flux_up, lw_flux_dn, p_del,
                                       lw_heating);
          {
            const auto policy =
                ekat::ExeSpaceUtils<ExeSpace>::get_default_team_policy(ncol,
            ...
            Kokkos::parallel_for(
                policy, KOKKOS_LAMBDA(const MemberType &team) {
                  ...
                  Kokkos::parallel_for(Kokkos::TeamVectorRange(team, nlay),
                                       [&](const int &ilay) {
                                         // Combine SW and LW heating into a net
                                         // heating tendency; use
                                         // d_rad_heating_pdel temporarily Note
                                         // that for YAKL arrays i and k start
                                         // with index 1
                                         d_rad_heating_pdel(icol, ilay) =
                                             sw_heating(idx + 1, ilay + 1) +
                                             lw_heating(idx + 1, ilay + 1);
                                       });
                });
          } Kokkos::fence(););
#endif
      ...
      TIMED_INLINE_KERNEL(
          ...
          rrtmgp::compute_heating_rate(sw_flux_up_k, sw_flux_dn_k, p_del_k,
                                       sw_heating_k);
          rrtmgp::compute_heating_rate(lw_flux_up_k, lw_flux_dn_k, p_del_k,
                                       lw_heating_k);
          {
            const auto policy =
                ekat::ExeSpaceUtils<ExeSpace>::get_default_team_policy(ncol,
                ...
                policy, KOKKOS_LAMBDA(const MemberType &team) {
                  ...
                  Kokkos::parallel_for(Kokkos::TeamVectorRange(team, nlay),
                                       [&](const int &ilay) {
                                         ...
                                         // with index 1
                                         d_rad_heating_pdel(icol, ilay) =
                                             sw_heating_k(idx, ilay) +
                                             lw_heating_k(idx, ilay);
                                       });
                });
          } Kokkos::fence(););
      ...
      TIMED_KERNEL(
          ...
          Kokkos::parallel_for(
              ...
              KOKKOS_LAMBDA(const int idx) {
                // CAREFUL: these are YAKL arrays, with "LayoutLeft". So make
                // the indices stride accordingly, and add 1.
                const int ibnd = (idx / ncol) / (nlay + 1) + 1;
                const int ilev = (idx / ncol) % (nlay + 1) + 1;
                const int icol = idx % ncol + 1;
                sw_bnd_flux_dif(icol, ilev, ibnd) =
                    sw_bnd_flux_dn(icol, ilev, ibnd) -
                    sw_bnd_flux_dir(icol, ilev, ibnd);
              });
          // Compute surface fluxes
          rrtmgp::compute_broadband_surface_fluxes(
              ncol, kbot, nswbands, sw_bnd_flux_dir, sw_bnd_flux_dif,
      ...
      TIMED_KERNEL(
          ...
          Kokkos::parallel_for(
              ...
              KOKKOS_LAMBDA(const int idx) {
                ...
                sw_bnd_flux_dif_k(icol, ilev, ibnd) =
                    sw_bnd_flux_dn_k(icol, ilev, ibnd) -
                    sw_bnd_flux_dir_k(icol, ilev, ibnd);
              });
          // Compute surface fluxes
          interface_t::compute_broadband_surface_fluxes(
              ncol, kbot_k, nswbands, sw_bnd_flux_dir_k, sw_bnd_flux_dif_k,
              sfc_flux_dir_vis_k, sfc_flux_dir_nir_k, sfc_flux_dif_vis_k,
              sfc_flux_dif_nir_k););
      COMPARE_ALL_WRAP(
          std::vector<real1d>({sfc_flux_dir_vis, sfc_flux_dir_nir,
      ...
      TIMED_KERNEL(
          ...
          // NOTE: limits for low, mid, and high clouds are mostly taken from
          // EAM F90 source, with the exception that I removed the restriction
          // on low clouds to be above (numerically lower pressures) 1200 hPa,
          // and on high clouds to be below (numerically high pressures) 50 hPa.
          // This probably does not matter in practice, as clouds probably
          // should not be produced above 50 hPa and we should not be
          // encountering surface pressure above 1200 hPa, but in the event that
          // things go off the rails we might want to look at these still.
          rrtmgp::compute_cloud_area(ncol, nlay, nlwgpts, 700e2,
                                     std::numeric_limits<Real>::max(), p_lay,
                                     cld_tau_lw_gpt, cldlow);
          rrtmgp::compute_cloud_area(ncol, nlay, nlwgpts, 400e2, 700e2, p_lay,
                                     cld_tau_lw_gpt, cldmed);
          rrtmgp::compute_cloud_area(ncol, nlay, nlwgpts, 0, 400e2, p_lay,
                                     cld_tau_lw_gpt, cldhgh);
          rrtmgp::compute_cloud_area(ncol, nlay, nlwgpts, 0,
                                     std::numeric_limits<Real>::max(), p_lay,
      ...
      TIMED_KERNEL(
          ...
          interface_t::compute_cloud_area(ncol, nlay, nlwgpts, 700e2,
                                          ...
                                          p_lay_k, cld_tau_lw_gpt_k, cldlow_k);
          interface_t::compute_cloud_area(ncol, nlay, nlwgpts, 400e2, 700e2,
                                          p_lay_k, cld_tau_lw_gpt_k, cldmed_k);
          interface_t::compute_cloud_area(ncol, nlay, nlwgpts, 0, 400e2,
                                          p_lay_k, cld_tau_lw_gpt_k, cldhgh_k);
          interface_t::compute_cloud_area(
              ncol, nlay, nlwgpts, 0, std::numeric_limits<Real>::max(), p_lay_k,
              cld_tau_lw_gpt_k, cldtot_k););
      COMPARE_ALL_WRAP(
          std::vector<real1d>({cldlow, cldmed, cldhgh, cldtot}),
      ...
      TIMED_INLINE_KERNEL(
          ...
          // Compute cloud-top diagnostics following AeroCom recommendation
          real1d T_mid_at_cldtop("T_mid_at_cldtop",
                                 d_T_mid_at_cldtop.data() + m_col_chunk_beg[ic],
                                 ncol);
          real1d p_mid_at_cldtop("p_mid_at_cldtop",
                                 d_p_mid_at_cldtop.data() + m_col_chunk_beg[ic],
                                 ncol);
          ...
          real1d cdnc_at_cldtop("cdnc_at_cldtop",
                                d_cdnc_at_cldtop.data() + m_col_chunk_beg[ic],
                                ncol);
          ...
          rrtmgp::compute_aerocom_cloudtop(
              ncol, nlay, t_lay, p_lay, p_del, z_del, qc, qi, rel, rei,
      ...
      TIMED_INLINE_KERNEL(
          ...
          real1dk T_mid_at_cldtop_k(
              d_T_mid_at_cldtop.data() + m_col_chunk_beg[ic], ncol);
          real1dk p_mid_at_cldtop_k(
              d_p_mid_at_cldtop.data() + m_col_chunk_beg[ic], ncol);
          real1dk cldfrac_ice_at_cldtop_k(
              d_cldfrac_ice_at_cldtop.data() + m_col_chunk_beg[ic], ncol);
          real1dk cldfrac_liq_at_cldtop_k(
              d_cldfrac_liq_at_cldtop.data() + m_col_chunk_beg[ic], ncol);
          real1dk cldfrac_tot_at_cldtop_k(
              d_cldfrac_tot_at_cldtop.data() + m_col_chunk_beg[ic], ncol);
          real1dk cdnc_at_cldtop_k(
              d_cdnc_at_cldtop.data() + m_col_chunk_beg[ic], ncol);
          real1dk eff_radius_qc_at_cldtop_k(
              d_eff_radius_qc_at_cldtop.data() + m_col_chunk_beg[ic], ncol);
          real1dk eff_radius_qi_at_cldtop_k(
              d_eff_radius_qi_at_cldtop.data() + m_col_chunk_beg[ic], ncol);

          interface_t::compute_aerocom_cloudtop(
              ncol, nlay, t_lay_k, p_lay_k, p_del_k, z_del_k, qc_k, qi_k, rel_k,
              rei_k, cldfrac_tot_k, nc_k, T_mid_at_cldtop_k, p_mid_at_cldtop_k,
              cldfrac_ice_at_cldtop_k, cldfrac_liq_at_cldtop_k,
              cldfrac_tot_at_cldtop_k, cdnc_at_cldtop_k,
      ...
      COMPARE_ALL_WRAP(
          ...
          std::vector<real1dk>(
              {T_mid_at_cldtop_k, p_mid_at_cldtop_k, cldfrac_ice_at_cldtop_k,
               ...
               cdnc_at_cldtop_k, eff_radius_qc_at_cldtop_k,
               eff_radius_qi_at_cldtop_k}));
#endif
      ...
      TIMED_KERNEL(
          Kokkos::parallel_for(
              policy, KOKKOS_LAMBDA(const MemberType &team) {
                ...
                d_sfc_flux_sw_net(icol) =
                    sw_flux_dn(i + 1, kbot) - sw_flux_up(i + 1, kbot);
                d_sfc_flux_lw_dn(icol) = lw_flux_dn(i + 1, kbot);
                Kokkos::parallel_for(
                    Kokkos::TeamVectorRange(team, nlay + 1), [&](const int &k) {
                      ...
                      d_sw_clnclrsky_flux_up(icol, k) =
                          sw_clnclrsky_flux_up(i + 1, k + 1);
                      d_sw_clnclrsky_flux_dn(icol, k) =
                          sw_clnclrsky_flux_dn(i + 1, k + 1);
                      d_sw_clnclrsky_flux_dn_dir(icol, k) =
                          sw_clnclrsky_flux_dn_dir(i + 1, k + 1);
                      d_sw_clrsky_flux_up(icol, k) =
                          sw_clrsky_flux_up(i + 1, k + 1);
                      d_sw_clrsky_flux_dn(icol, k) =
                          sw_clrsky_flux_dn(i + 1, k + 1);
                      d_sw_clrsky_flux_dn_dir(icol, k) =
                          sw_clrsky_flux_dn_dir(i + 1, k + 1);
                      d_sw_clnsky_flux_up(icol, k) =
                          sw_clnsky_flux_up(i + 1, k + 1);
                      d_sw_clnsky_flux_dn(icol, k) =
                          sw_clnsky_flux_dn(i + 1, k + 1);
                      d_sw_clnsky_flux_dn_dir(icol, k) =
                          sw_clnsky_flux_dn_dir(i + 1, k + 1);
                      d_lw_clnclrsky_flux_up(icol, k) =
                          lw_clnclrsky_flux_up(i + 1, k + 1);
                      d_lw_clnclrsky_flux_dn(icol, k) =
                          lw_clnclrsky_flux_dn(i + 1, k + 1);
                      d_lw_clrsky_flux_up(icol, k) =
                          lw_clrsky_flux_up(i + 1, k + 1);
                      d_lw_clrsky_flux_dn(icol, k) =
                          lw_clrsky_flux_dn(i + 1, k + 1);
                      d_lw_clnsky_flux_up(icol, k) =
                          lw_clnsky_flux_up(i + 1, k + 1);
                      d_lw_clnsky_flux_dn(icol, k) =
                          lw_clnsky_flux_dn(i + 1, k + 1);
                    });
                // Extract optical properties for COSP
                Kokkos::parallel_for(
                    Kokkos::TeamVectorRange(team, nlay), [&](const int &k) {
                      d_dtau067(icol, k) =
                          cld_tau_sw_bnd(i + 1, k + 1, idx_067);
                      d_dtau105(icol, k) =
                          cld_tau_lw_bnd(i + 1, k + 1, idx_105);
                    });
      ...
      TIMED_KERNEL(
          Kokkos::parallel_for(
              policy, KOKKOS_LAMBDA(const MemberType &team) {
                ...
                const int icol = i + beg;
                d_sfc_flux_sw_net(icol) =
                    sw_flux_dn_k(i, kbot_k) - sw_flux_up_k(i, kbot_k);
                d_sfc_flux_lw_dn(icol) = lw_flux_dn_k(i, kbot_k);
#ifdef RRTMGP_LAYOUT_LEFT
                // Copy from layout left buffer views back to layout right
                // fields
                Kokkos::parallel_for(
                    Kokkos::TeamVectorRange(team, nlay + 1), [&](const int &k) {
      ...
      real3dk temp(gas_concs_k, std::make_pair(0, ncol), Kokkos::ALL,
                   Kokkos::ALL);
  ...
  // Apply temperature tendency; if we updated radiation this timestep, then
  // d_rad_heating_pdel should contain actual heating rate, not pdel scaled
  // heating rate. Otherwise, if we have NOT updated the radiative heating, then
  // we need to back out the heating from the rad_heating*pdel term that we
  // carry across timesteps to conserve energy.
  const int ncols = m_ncol;
  const int nlays = m_nlay;
  const auto policy =
      ekat::ExeSpaceUtils<ExeSpace>::get_default_team_policy(ncols, nlays);
  Kokkos::parallel_for(
      policy, KOKKOS_LAMBDA(const MemberType &team) {
        const int i = team.league_rank();
        Kokkos::parallel_for(
            Kokkos::TeamVectorRange(team, nlays), [&](const int &k) {
              if (update_rad) {
                d_tmid(i, k) = d_tmid(i, k) + d_rad_heating_pdel(i, k) * dt;
                d_rad_heating_pdel(i, k) =
                    d_pdel(i, k) * d_rad_heating_pdel(i, k);
              } else {
  ...
  // conservation checks. Any boundary fluxes not included in radiation
  ...
  if (has_column_conservation_check()) {
    ...
    Kokkos::parallel_for(
        policy, KOKKOS_LAMBDA(const MemberType &team) {
          ...
          const auto fsns =
              d_sw_flux_dn(icol, nlays) - d_sw_flux_up(icol, nlays);
          const auto fsnt = d_sw_flux_dn(icol, 0) - d_sw_flux_up(icol, 0);
          const auto flns =
              d_lw_flux_up(icol, nlays) - d_lw_flux_dn(icol, nlays);
