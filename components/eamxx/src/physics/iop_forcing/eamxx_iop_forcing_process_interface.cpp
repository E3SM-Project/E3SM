...
void IOPForcing::set_grids(
  ...
  add_group<Updated>("tracers", grid_name, pack_size,
                     MonolithicAlloc::Required);
  ...
  const auto iop_nudge_tq =
      m_iop_data_manager->get_params().get<bool>("iop_nudge_tq");
  const auto iop_nudge_uv =
      m_iop_data_manager->get_params().get<bool>("iop_nudge_uv");
...
void IOPForcing::set_computed_group_impl(const FieldGroup &group) {
  ...
  EKAT_REQUIRE_MSG(name == "tracers",
                   "Error! IOPForcing was not expecting a field group called '"
                       << name << "\n");

  EKAT_REQUIRE_MSG(
      group.m_info->m_monolithic_allocation,
...
size_t IOPForcing::requested_buffer_size_in_bytes() const {
  ...
  const size_t wsm_bytes = WorkspaceMgr::get_total_bytes_needed(
      nlevi_packs, 7 + m_num_tracers, policy);
...
void IOPForcing::init_buffers(const ATMBufferManager &buffer_manager) {
  ...
  const auto policy = ESU::get_default_team_policy(m_num_cols, nlevi_packs);
  const size_t wsm_npacks = WorkspaceMgr::get_total_bytes_needed(
                                nlevi_packs, 7 + m_num_tracers, policy) /
                            sizeof(Pack);
  mem += wsm_npacks;

  size_t used_mem =
      (reinterpret_cast<Real *>(mem) - buffer_manager.get_memory()) *
      sizeof(Real);
...
void IOPForcing::initialize_impl(const RunType run_type) {
  ...
  add_postcondition_check<Interval>(get_field_out("T_mid"), m_grid, 100.0,
                                    500.0, false);
  add_postcondition_check<Interval>(get_field_out("horiz_winds"), m_grid,
                                    -400.0, 400.0, false);
  // For qv, ensure it doesn't get negative, by allowing repair of any neg
  // value.
  ...
  const auto policy = ESU::get_default_team_policy(m_num_cols, nlevi_packs);
  m_workspace_mgr.setup(m_buffer.wsm_data, nlevi_packs, 7 + m_num_tracers,
                        policy);
  ...
  if (iop_nudge_tq or iop_nudge_uv)
    m_helper_fields.at("horiz_mean_weights").deep_copy(one_over_num_dofs);
}
// =========================================================================================
KOKKOS_FUNCTION
void IOPForcing::advance_iop_subsidence(
    const MemberType &team, const int nlevs, const Real dt, const Real ps,
  ...
  workspace.take_many_contiguous_unsafe<4>(
  ...
  // Compute delta views for u, v, T, and Q (e.g., u(k+1) - u(k),
  // k=0,...,nlevs-2)
  ...
  Kokkos::parallel_for(
      Kokkos::TeamVectorRange(team, nlev_packs), [&](const int k) {
        auto range_pack       = ekat::range<IntPack>(k * Pack::n);
        ...
        Pack delta_u_k, delta_u_km1, delta_v_k, delta_v_km1, delta_T_k,
            delta_T_km1;
        ekat::index_and_shift<-1>(s_delta_u, range_pack_for_m1_shift, delta_u_k,
                                  delta_u_km1);
        ekat::index_and_shift<-1>(s_delta_v, range_pack_for_m1_shift, delta_v_k,
                                  delta_v_km1);
        ekat::index_and_shift<-1>(s_delta_T, range_pack_for_m1_shift, delta_T_k,
                                  delta_T_km1);
        ...
        ekat::index_and_shift<1>(s_omega_int, range_pack, omega_int_k,
                                 omega_int_kp1);
        ...
        for (int iq = 0; iq < n_q_tracers; ++iq) {
          ...
          ekat::index_and_shift<-1>(s_delta_tracer, range_pack_for_m1_shift,
                                    delta_tracer_k, delta_tracer_km1);
          if (any_at_top)
            delta_tracer_k.set(at_top, s_delta_tracer(0));
          if (any_at_bot)
            delta_tracer_km1.set(at_bot, s_delta_tracer(nlevs - 2));

          Q(iq, k).update(not_at_bot, fac * omega_int_kp1 * delta_tracer_k, -1,
                          1);
          Q(iq, k).update(not_at_top, fac * omega_int_k * delta_tracer_km1, -1,
                          1);
        }
      });
  ...
  workspace.release_many_contiguous<4>(
      {&omega_int, &delta_u, &delta_v, &delta_T});
}
// =========================================================================================
KOKKOS_FUNCTION
void IOPForcing::advance_iop_forcing(const MemberType &team, const int nlevs,
                                     const Real dt,
...
// =========================================================================================
KOKKOS_FUNCTION
void IOPForcing::iop_apply_coriolis(const MemberType &team, const int nlevs,
...
void IOPForcing::run_impl(const double dt) {
  ...
  const auto Q =
      get_group_out("tracers").m_monolithic_field->get_view<Pack ***>();

  // Load data from IOP files, if necessary
  // TODO: this is using the TS from the beg of the step. Should it use
  // end_of_step_ts() instead?
  m_iop_data_manager->read_iop_file_data(start_of_step_ts());

  // Define local IOP param values
  const auto iop_dosubsidence =
      m_iop_data_manager->get_params().get<bool>("iop_dosubsidence");
  const auto iop_coriolis =
      m_iop_data_manager->get_params().get<bool>("iop_coriolis");
  const auto iop_nudge_tq =
      m_iop_data_manager->get_params().get<bool>("iop_nudge_tq");
  const auto iop_nudge_uv =
      m_iop_data_manager->get_params().get<bool>("iop_nudge_uv");
  ...
  const auto use_3d_forcing =
      m_iop_data_manager->get_params().get<bool>("use_3d_forcing");
  const auto target_lat =
      m_iop_data_manager->get_params().get<Real>("target_latitude");
  const auto iop_nudge_tscale =
      m_iop_data_manager->get_params().get<Real>("iop_nudge_tscale");
  const auto iop_nudge_tq_low =
      m_iop_data_manager->get_params().get<Real>("iop_nudge_tq_low");
  const auto iop_nudge_tq_high =
      m_iop_data_manager->get_params().get<Real>("iop_nudge_tq_high");

  // Define local IOP field views
  const Real ps_iop =
      m_iop_data_manager->get_iop_field("Ps").get_view<const Real, Host>()();
  view_1d<const Pack> omega, divT, divq, u_ls, v_ls, qv_iop, t_iop, u_iop,
      v_iop;
  divT =
      use_3d_forcing
          ? m_iop_data_manager->get_iop_field("divT3d").get_view<const Pack *>()
          : m_iop_data_manager->get_iop_field("divT").get_view<const Pack *>();
  divq =
      use_3d_forcing
          ? m_iop_data_manager->get_iop_field("divq3d").get_view<const Pack *>()
  ...
  // Apply IOP forcing
  Kokkos::parallel_for(
      "apply_iop_forcing", policy_iop, KOKKOS_LAMBDA(const MemberType &team) {
        ...
        ws.take_many_contiguous_unsafe<3>(
        ...
        Kokkos::parallel_for(
            Kokkos::TeamVectorRange(team, num_levs + 1), [&](const int &k) {
        ...
        if (iop_dosubsidence) {
          ...
          advance_iop_subsidence(team, num_levs, dt, ps_i, ref_p_mid, ref_p_int,
                                 ref_p_del, omega, ws, u_i, v_i, T_mid_i, Q_i);
        }

        // Update T and qv according to large scale forcing as specified in IOP
        // file.
        advance_iop_forcing(team, num_levs, dt, divT, divq, T_mid_i, qv_i);

        if (iop_coriolis) {
          // Apply coriolis forcing to u and v winds
          iop_apply_coriolis(team, num_levs, dt, target_lat, u_ls, v_ls, u_i,
                             v_i);
        }
  ...
  if (iop_nudge_tq or iop_nudge_uv) {
    ...
    if (iop_nudge_tq) {
      horiz_contraction<Real>(
          m_helper_fields.at("qv_mean"), get_field_out("qv"),
          m_helper_fields.at("horiz_mean_weights"), &m_comm);
      qv_mean = m_helper_fields.at("qv_mean").get_view<Pack *>();

      horiz_contraction<Real>(
          m_helper_fields.at("t_mean"), get_field_out("T_mid"),
          m_helper_fields.at("horiz_mean_weights"), &m_comm);
      t_mean = m_helper_fields.at("t_mean").get_view<Pack *>();
    }
    if (iop_nudge_uv) {
      horiz_contraction<Real>(
          m_helper_fields.at("horiz_winds_mean"), get_field_out("horiz_winds"),
          m_helper_fields.at("horiz_mean_weights"), &m_comm);
      horiz_winds_mean =
          m_helper_fields.at("horiz_winds_mean").get_view<Pack **>();
    }

    // Apply relaxation
    const auto rtau = std::max(dt, iop_nudge_tscale);
    Kokkos::parallel_for(
        "apply_domain_relaxation", policy_iop,
        KOKKOS_LAMBDA(const MemberType &team) {
          ...
          auto s_ref_p_mid = ekat::scalarize(ref_p_mid);
          Kokkos::parallel_for(
              Kokkos::TeamVectorRange(team, num_levs), [&](const int &k) {
                s_ref_p_mid(k) = hyam(k) * ps0 + hybm(k) * ps_i;
              });
          team.team_barrier();
          ...
          Kokkos::parallel_for(
              Kokkos::TeamVectorRange(team, nlev_packs), [&](const int &k) {
                if (iop_nudge_tq) {
                  // Restrict nudging of T and qv to certain levels if requested
                  // by user IOP pressure variable is in unitis of [Pa], while
                  // iop_nudge_tq_low/high is in units of [hPa], thus convert
                  // iop_nudge_tq_low/high
                  Mask nudge_level(false);
                  int max_size = hyam.size();
                  for (int lev = k * Pack::n, p = 0;
                       p < Pack::n && lev < max_size; ++lev, ++p) {
                    const auto pressure_from_iop =
                        hyam(lev) * ps0 + hybm(lev) * ps_iop;
                    nudge_level.set(
                        p, pressure_from_iop <= iop_nudge_tq_low * 100 and
                               pressure_from_iop >= iop_nudge_tq_high * 100);
                  }

                  qv_i(k).update(nudge_level, qv_mean(k) - qv_iop(k),
                                 -dt / rtau, 1.0);
                  T_mid_i(k).update(nudge_level, t_mean(k) - t_iop(k),
                                    -dt / rtau, 1.0);
                }
                if (iop_nudge_uv) {
                  u_i(k).update(horiz_winds_mean(0, k) - u_iop(k), -dt / rtau,
                                1.0);
                  v_i(k).update(horiz_winds_mean(1, k) - v_iop(k), -dt / rtau,
                                1.0);
                }
              });
