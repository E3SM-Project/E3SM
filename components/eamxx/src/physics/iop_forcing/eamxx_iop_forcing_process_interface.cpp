...
void IOPForcing::set_grids(const std::shared_ptr<const GridsManager> grids_manager) {
  ...
  EKAT_REQUIRE_MSG(m_iop_data_manager, "Error! IOPDataManager not setup by driver, but IOPForcing"
...
void IOPForcing::advance_iop_subsidence(const MemberType &team, const int nlevs, const Real dt, const Real ps,
  ...
  Kokkos::parallel_for(Kokkos::TeamVectorRange(team, nlev_packs), [&](const int k) {
    ...
    Pack delta_u_k, delta_u_km1, delta_v_k, delta_v_km1, delta_T_k, delta_T_km1;
...
void IOPForcing::run_impl(const double dt) {
  ...
  if (iop_nudge_tq or iop_nudge_uv) {
    ...
    Kokkos::parallel_for(
        "apply_domain_relaxation", policy_iop, KOKKOS_LAMBDA(const MemberType &team) {
          const int icol = team.league_rank();
          ...
          Kokkos::parallel_for(Kokkos::TeamVectorRange(team, nlev_packs), [&](const int &k) {
            if (iop_nudge_tq) {
              ...
              for (int lev = k * Pack::n, p = 0; p < Pack::n && lev < max_size; ++lev, ++p) {
                ...
                nudge_level.set(p, pressure_from_iop <= iop_nudge_tq_low * 100 and
