...
Functions<S, D>::check_temperature(const Spack &t_atm, const char *caller,
                                   const Smask &range_mask) {

  //"range_mask" masks out packs which are padded with uninitialized values

  /*Future work:
  Make error message dynamic so that it tells the user the function name
  (func_name) from where this error is coming from. Currently func_name is not
  used in this function */

  // NOTE: EKAT_KERNEL_REQUIRE_MSG requires first argument to be "False" to exit
  // with an error message

  // find out if there are any negative temperatures in the pack
  const auto is_neg_t_atm = (t_atm <= 0) && range_mask;
  EKAT_KERNEL_REQUIRE_MSG(!(is_neg_t_atm.any()),
                          caller); // exit with an error message

  // find out if there are any NaN temperatures in the pack
  const auto is_nan_t_atm = isnan(t_atm) && range_mask;
  EKAT_KERNEL_REQUIRE_MSG(!(is_nan_t_atm.any()),
                          caller); // exit with an error message
}
...
Functions<S, D>::MurphyKoop_svp(const Spack &t_atm, const bool ice,
  ...
  if (ice_mask.any()) {
    ...
    const Spack ice_result =
        exp(ic[0] - (ic[1] / t_atm) + (ic[2] * log(t_atm)) - (ic[3] * t_atm));
  ...
  if (liq_mask.any()) {
    ...
    static constexpr Scalar lq[] = {54.842763, 6763.22, 4.210,  0.000367,
                                    0.0415,    218.8,   53.878, 1331.22,
                                    9.44523,   0.014025};
...
Functions<S, D>::polysvp1(const Spack &t, const bool ice,
                          const Smask &range_mask, const char *caller) {
  // REPLACE GOFF-GRATCH WITH FASTER FORMULATION FROM FLATAU ET AL. 1992, TABLE
  // 4 (RIGHT-HAND COLUMN)
...
Functions<S, D>::qv_sat_dry(const Spack &t_atm, const Spack &p_atm_dry,
  ...
  range_mask: is a mask which masks out padded values in the packs, which are
  uninitialized func_idx is an optional argument to decide which scheme is to be
  called for saturation vapor pressure Currently default is set to
  "MurphyKoop_svp" func_idx = Polysvp1 (=0) --> polysvp1 (Flatau et al. 1992)
  func_idx = MurphyKoop (=1) --> MurphyKoop_svp (Murphy, D. M., and T. Koop
  2005)*/
...
template <typename S, typename D>
KOKKOS_FUNCTION typename Functions<S, D>::Spack
Functions<S, D>::qv_sat_wet(const Spack &t_atm, const Spack &p_atm_dry,
                            const bool ice, const Smask &range_mask,
  ...
  Spack qsatdry =
      qv_sat_dry(t_atm, p_atm_dry, ice, range_mask, func_idx, caller);
