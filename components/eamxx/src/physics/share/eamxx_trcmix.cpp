...
void trcmix(
    const std::string &name, const int nlevs,
  ...
  // Constants map: gas_name -> [trop_mmr, scale1_base, scale1_fact,
  // scale2_base, scale2_fact]
  ...
  const auto policy =
      ekat::ExeSpaceUtils<ExeSpace>::get_default_team_policy(ncols, nlevs);
  ...
  if (name == "o2" || name == "co2") {
    ...
    EKAT_REQUIRE_MSG(it != const_map.end(),
                     "trcmix: Unhandled gas_name: " + name);
    ...
    Kokkos::parallel_for(
        policy, KOKKOS_LAMBDA(const MemberType &team) {
          const Int i = team.league_rank();
          Kokkos::parallel_for(
              Kokkos::TeamVectorRange(team, nlevs), [&](const Int &k) {
                // set stratospheric scale height factor for gases. Need to
                // convert clat to radians
                const auto clat_r = clat(i) * C::Pi / 180.0;
                const auto dlat   = std::abs(57.2958 * clat_r);
                const auto scale =
                    dlat <= 45.0 ? scale1_base + scale1_fact * dlat
                                 : scale2_base + scale2_fact * (dlat - 45);

                // pressure of tropopause
                const auto ptrop =
                    250.0e2 - 150.0e2 * std::pow(std::cos(clat_r), 2);
