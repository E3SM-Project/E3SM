...
void Functions<Real, DefaultDevice>::check_values_disp(
    const uview_2d<const Spack> &qv, const uview_2d<const Spack> &temp,
  ...
  Kokkos::parallel_for(
      "p3_check_values", policy, KOKKOS_LAMBDA(const MemberType &team) {
        ...
        check_values(ekat::subview(qv, i), ekat::subview(temp, i), ktop, kbot,
                     timestepcount, force_abort, 900, team,
