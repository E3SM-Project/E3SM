...
void Functions<Real, DefaultDevice>::p3_main_init_disp(
  ...
  Kokkos::parallel_for(
      "p3_main_init", policy, KOKKOS_LAMBDA(const MemberType &team) {
...
Int Functions<Real, DefaultDevice>::p3_main_internal_disp(
  ...
  auto hetfrz_immersion_nucleation_tend =
      diagnostic_inputs.hetfrz_immersion_nucleation_tend;
  auto hetfrz_contact_nucleation_tend =
      diagnostic_inputs.hetfrz_contact_nucleation_tend;
  auto hetfrz_deposition_nucleation_tend =
      diagnostic_inputs.hetfrz_deposition_nucleation_tend;
  ...
  // initialize
  p3_main_init_disp(
      nj, nk_pack, cld_frac_i, cld_frac_l, cld_frac_r, inv_exner, th, dz,
      ...
      mu_r, lamr, logn0r, nu, cdist, cdist1, cdistr, qc_incld, qr_incld,
      qi_incld, qm_incld, nc_incld, nr_incld, ni_incld, bm_incld, inv_rho, prec,
      rho, rhofacr, rhofaci, acn, qv_sat_l, qv_sat_i, sup, qv_supersat_i,
      qtend_ignore, ntend_ignore, mu_c, lamc, rho_qi, qv2qi_depos_tend,
      precip_total_tend, nevapr, precip_liq_flux, precip_ice_flux);

  p3_main_part1_disp(
      nj, nk, infrastructure.predictNc, infrastructure.prescribedCCN,
      infrastructure.dt, pres, dpres, dz, nc_nuceat_tend, nccn_prescribed,
      inv_exner, exner, inv_cld_frac_l, inv_cld_frac_i, inv_cld_frac_r, T_atm,
      rho, inv_rho, qv_sat_l, qv_sat_i, qv_supersat_i, rhofacr, rhofaci, acn,
      qv, th, qc, nc, qr, nr, qi, ni, qm, bm, qc_incld, qr_incld, qi_incld,
  ...
  p3_main_part2_disp(
      nj, nk, runtime_options.max_total_ni, infrastructure.predictNc,
      infrastructure.prescribedCCN, infrastructure.dt, inv_dt,
      hetfrz_immersion_nucleation_tend, hetfrz_contact_nucleation_tend,
      hetfrz_deposition_nucleation_tend, lookup_tables.dnu_table_vals,
      lookup_tables.ice_table_vals, lookup_tables.collect_table_vals,
      lookup_tables.revap_table_vals, pres, dpres, dz, nc_nuceat_tend,
      inv_exner, exner, inv_cld_frac_l, inv_cld_frac_i, inv_cld_frac_r,
      ni_activated, inv_qc_relvar, cld_frac_i, cld_frac_l, cld_frac_r, qv_prev,
      t_prev, T_atm, rho, inv_rho, qv_sat_l, qv_sat_i, qv_supersat_i, rhofacr,
      rhofaci, acn, qv, th, qc, nc, qr, nr, qi, ni, qm, bm, qc_incld, qr_incld,
      qi_incld, qm_incld, nc_incld, nr_incld, ni_incld, bm_incld, mu_c, nu,
      lamc, cdist, cdist1, cdistr, mu_r, lamr, logn0r, qv2qi_depos_tend,
      precip_total_tend, nevapr, qr_evap_tend, vap_liq_exchange,
      vap_ice_exchange, liq_ice_exchange, qr2qv_evap, qi2qv_sublim,
      ...
      qi2qr_melt, pratot, prctot, nucleationPossible, hydrometeorsPresent,
      runtime_options);

  // NOTE: At this point, it is possible to have negative (but small) nc, nr,
  // ni.  This is not
  //       a problem; those values get clipped to zero in the sedimentation
  //       section (if necessary). (This is not done above simply for efficiency
  //       purposes.)
  ...
  ice_sedimentation_disp(
      rho, inv_rho, rhofaci, cld_frac_i, inv_dz, workspace_mgr, nj, nk, ktop,
      kbot, kdir, infrastructure.dt, inv_dt, qi, qi_incld, ni, ni_incld, qm,
      qm_incld, bm, bm_incld, qi_sed, ntend_ignore,
      lookup_tables.ice_table_vals, diagnostic_outputs.precip_ice_surf,
      nucleationPossible, hydrometeorsPresent, runtime_options);
  ...
  //
  p3_main_part3_disp(
      nj, nk_pack, runtime_options.max_total_ni, lookup_tables.dnu_table_vals,
      lookup_tables.ice_table_vals, inv_exner, cld_frac_l, cld_frac_r,
      cld_frac_i, rho, inv_rho, rhofaci, qv, th, qc, nc, qr, nr, qi, ni, qm, bm,
      mu_c, nu, lamc, mu_r, lamr, vap_liq_exchange, ze_rain, ze_ice, diag_vm_qi,
      diag_eff_radius_qi, diag_diam_qi, rho_qi, diag_equiv_reflectivity,
      diag_eff_radius_qc, diag_eff_radius_qr, nucleationPossible,
      hydrometeorsPresent, runtime_options);
  ...
  check_values_disp(qv, tmparr2, ktop, kbot, infrastructure.it, debug_ABORT,
                    900, col_location, nj, nk);
#endif
  Kokkos::fence();
