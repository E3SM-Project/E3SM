...
public:
  ...
  /*--------------------------------------------------------------------------------------------*/
  // Most individual processes have a pre-processing step that constructs needed
  // variables from the set of fields stored in the field manager.  A structure
  // like this defines those operations, which can then be called during
  // run_impl in the main .cpp code. NOTE: the use of the ekat command "set" to
  // copy local values into the variables of interest. This is an important step
  // to avoid having those variables just share pointers to memory. Structure to
  // handle the local generation of data needed by p3_main in run_impl
  struct p3_preamble {
    p3_preamble() = default;
    ...
    void operator()(const KT::MemberType &team) const {
      ...
      Kokkos::parallel_for(
          Kokkos::TeamVectorRange(team, npack), [&](const Int &ipack) {
            ...
            const Spack &pseudo_density_dry_pack(
                pseudo_density_dry(icol, ipack));

            // compute dz from full pressure
            dz(icol, ipack) = PF::calculate_dz(pseudo_density_pack, pmid_pack,
                                               T_atm_pack, qv(icol, ipack));

            /*----------------------------------------------------------------------------------------------------------------------
             *Wet to dry mixing ratios:
             *-------------------------
             *Since state constituents from the host model (or AD) are  wet
             *mixing ratios and P3 needs these constituents in dry mixing
             *ratios, we convert the wet mixing ratios to dry mixing ratios.
             *----------------------------------------------------------------------------------------------------------------------
             */

            // Units of all constituents below are [kg/kg(dry-air)] for mass and
            // [#/kg(dry-air)] for number
            qc(icol, ipack) = PF::calculate_drymmr_from_wetmmr_dp_based(
                qc(icol, ipack), pseudo_density_pack,
            ...
            qr(icol, ipack) = PF::calculate_drymmr_from_wetmmr_dp_based(
                qr(icol, ipack), pseudo_density_pack,
                pseudo_density_dry_pack); // Rain mass
            nr(icol, ipack) = PF::calculate_drymmr_from_wetmmr_dp_based(
                nr(icol, ipack), pseudo_density_pack,
                pseudo_density_dry_pack); // Rain number
            qi(icol, ipack) = PF::calculate_drymmr_from_wetmmr_dp_based(
                qi(icol, ipack), pseudo_density_pack,
                pseudo_density_dry_pack); // Cloud ice mass
            ni(icol, ipack) = PF::calculate_drymmr_from_wetmmr_dp_based(
                ni(icol, ipack), pseudo_density_pack,
                pseudo_density_dry_pack); // Cloud ice number
            qm(icol, ipack) = PF::calculate_drymmr_from_wetmmr_dp_based(
                qm(icol, ipack), pseudo_density_pack,
                pseudo_density_dry_pack); // Rimmed ice mass
            bm(icol, ipack) = PF::calculate_drymmr_from_wetmmr_dp_based(
                bm(icol, ipack), pseudo_density_pack,
            ...
            qv_prev(icol, ipack) = PF::calculate_drymmr_from_wetmmr_dp_based(
                qv_prev(icol, ipack), pseudo_density_pack,
            ...
            th_atm(icol, ipack) =
                PF::calculate_theta_from_T(T_atm_pack, pmid_pack);
            // Cloud fraction
            // Set minimum cloud fraction - avoids division by zero
            // Alternatively set fraction to 1 everywhere to disable subgrid
            // effects
            if (runtime_opts.use_separate_ice_liq_frac) {
              cld_frac_l(icol, ipack) =
                  runtime_opts.set_cld_frac_l_to_one
                      ? 1
                      : ekat::max(cld_frac_l_in_pack, mincld);
              cld_frac_i(icol, ipack) =
                  runtime_opts.set_cld_frac_i_to_one
                      ? 1
                      : ekat::max(cld_frac_i_in_pack, mincld);
            } else {
              cld_frac_l(icol, ipack) =
                  runtime_opts.set_cld_frac_l_to_one
                      ? 1
                      : ekat::max(cld_frac_t_in_pack, mincld);
              cld_frac_i(icol, ipack) =
                  runtime_opts.set_cld_frac_i_to_one
                      ? 1
                      : ekat::max(cld_frac_t_in_pack, mincld);
            ...
            cld_frac_r(icol, ipack) =
                runtime_opts.set_cld_frac_r_to_one
                    ? 1
                    : ekat::max(cld_frac_t_in_pack, mincld);

            // update rain cloud fraction given neighboring levels using
            // max-overlap approach.
            if (not runtime_opts.set_cld_frac_r_to_one) {
              // Get the cld_frac_t_in(icol, ilev-1) entries
              const auto &cld_frac_t_in_s =
                  ekat::scalarize(ekat::subview(cld_frac_t_in, icol));
              ...
              range_pack2.set(range_pack1 < 1,
                              1); // don't want the shift to go below zero. we
                                  // mask out that result anyway
              ekat::index_and_shift<-1>(cld_frac_t_in_s, range_pack2,
                                        cld_frac_t_in_k, cld_frac_t_in_km1);

              // Hard-coded max-overlap cloud fraction calculation.  Cycle
              // through the layers from top to bottom and determine if the rain
              // fraction needs to be updated to match the cloud fraction in the
              // layer above.
              const auto active_range = range_pack1 > 0 && range_pack1 < m_nlev;
              if (active_range.any()) {
                const auto set_to_t_in =
                    cld_frac_t_in_km1 > cld_frac_r(icol, ipack);
                cld_frac_r(icol, ipack)
                    .set(active_range and set_to_t_in, cld_frac_t_in_km1);
              }
            }
          });
    } // operator
    ...
    // Local variables
    int m_ncol, m_nlev;
    Real mincld = 0.0001; // TODO: These should be stored somewhere as more
                          // universal constants.  Or maybe in the P3 class hpp
    view_2d_const pmid;
    ...
    void
    set_variables(const int ncol, const int nlev, const view_2d_const &pmid_,
                  const view_2d_const &pmid_dry_,
                  const view_2d_const &pseudo_density_,
                  const view_2d_const &pseudo_density_dry_,
                  const view_2d &T_atm_, const view_2d_const &cld_frac_t_in_,
                  const view_2d_const &cld_frac_l_in_,
                  const view_2d_const &cld_frac_i_in_, const view_2d &qv_,
                  const view_2d &qc_, const view_2d &nc_, const view_2d &qr_,
                  const view_2d &nr_, const view_2d &qi_, const view_2d &qm_,
                  const view_2d &ni_, const view_2d &bm_,
                  const view_2d &qv_prev_, const view_2d &inv_exner_,
                  const view_2d &th_atm_, const view_2d &cld_frac_l_,
                  const view_2d &cld_frac_i_, const view_2d &cld_frac_r_,
                  const view_2d &dz_, const P3F::P3Runtime &runtime_options) {
      m_ncol = ncol;
      m_nlev = nlev;
      // IN
      pmid               = pmid_;
  ...
  /* --------------------------------------------------------------------------------------------*/
  // Most individual processes have a post-processing step that derives
  // variables needed by the rest of the model, using outputs from this process.
  // Structure to handle the generation of data needed by the rest of the model
  // based on output from p3_main.
  struct p3_postamble {
    ...
    void operator()(const KT::MemberType &team) const {
      ...
      Kokkos::parallel_for(
          Kokkos::TeamVectorRange(team, npack), [&](const Int &ipack) {
            ...
            // Update the atmospheric temperature and the previous temperature.
            {
              ...
              T_atm(icol, ipack) =
                  (PF::calculate_T_from_theta(th_atm(icol, ipack),
                                              pmid(icol, ipack)) -
                   T_atm_before_p3) *
             ...
             *DRY-TO-WET MMRs:
             *-----------------
             *Since the host model (or AD) needs wet mixing ratios, we need to
             *convert dry mixing ratios from P3 to wet mixing ratios.
             *----------------------------------------------------------------------------------------------------------------------
             */
            // Units of all constituents below are [kg/kg(wet-air)] for mass and
            // [#/kg(wet-air)] for number
            qc(icol, ipack) = PF::calculate_wetmmr_from_drymmr_dp_based(
                qc(icol, ipack), pseudo_density_pack,
                pseudo_density_dry_pack); // Cloud liquid mass
            nc(icol, ipack) = PF::calculate_wetmmr_from_drymmr_dp_based(
                nc(icol, ipack), pseudo_density_pack,
                pseudo_density_dry_pack); // Cloud liquid number
            qr(icol, ipack) = PF::calculate_wetmmr_from_drymmr_dp_based(
                qr(icol, ipack), pseudo_density_pack,
                pseudo_density_dry_pack); // Rain mass
            nr(icol, ipack) = PF::calculate_wetmmr_from_drymmr_dp_based(
                nr(icol, ipack), pseudo_density_pack,
                pseudo_density_dry_pack); // Rain number
            qi(icol, ipack) = PF::calculate_wetmmr_from_drymmr_dp_based(
                qi(icol, ipack), pseudo_density_pack,
                pseudo_density_dry_pack); // Cloud ice mass
            ni(icol, ipack) = PF::calculate_wetmmr_from_drymmr_dp_based(
                ni(icol, ipack), pseudo_density_pack,
                pseudo_density_dry_pack); // Cloud ice number
            qm(icol, ipack) = PF::calculate_wetmmr_from_drymmr_dp_based(
                qm(icol, ipack), pseudo_density_pack,
                pseudo_density_dry_pack); // Rimmed ice mass
            bm(icol, ipack) = PF::calculate_wetmmr_from_drymmr_dp_based(
                bm(icol, ipack), pseudo_density_pack,
                pseudo_density_dry_pack); // Rimmed ice number
      ...
      // Microphysics can be subcycled together during a single physics
      // timestep, therefore we must accumulate these fluxes. Note: we need to
      // ensure that only a single thread within the team is
      //       updating the mass value.
      Kokkos::single(Kokkos::PerTeam(team), [&] {
        precip_liq_surf_mass(icol) +=
            precip_liq_surf_flux(icol) * PC::RHO_H2O * m_dt;
        precip_ice_surf_mass(icol) +=
            precip_ice_surf_flux(icol) * PC::RHO_H2O * m_dt;
      });
      ...
      if (compute_mass_and_energy_fluxes) {
        vapor_flux(icol) = 0.0;
        water_flux(icol) =
            precip_liq_surf_flux(icol) + precip_ice_surf_flux(icol);
    ...
    void
    set_variables(const int ncol, const int npack, const view_2d &th_atm_,
                  const view_2d_const &pmid_, const view_2d_const &pmid_dry_,
                  const view_2d &T_atm_, const view_2d &T_prev_,
                  const view_2d_const &pseudo_density_,
                  const view_2d_const &pseudo_density_dry_, const view_2d &qv_,
                  const view_2d &qc_, const view_2d &nc_, const view_2d &qr_,
                  const view_2d &nr_, const view_2d &qi_, const view_2d &qm_,
                  const view_2d &ni_, const view_2d &bm_,
                  const view_2d &qv_prev_, const view_2d &diag_eff_radius_qc_,
                  const view_2d &diag_eff_radius_qi_,
                  const view_2d &diag_eff_radius_qr_,
                  const view_1d_const &precip_liq_surf_flux_,
                  const view_1d_const &precip_ice_surf_flux_,
                  const view_1d &precip_liq_surf_mass_,
                  const view_1d &precip_ice_surf_mass_) {
      m_ncol  = ncol;
      m_npack = npack;
      // IN
      th_atm               = th_atm_;
      ...
      // TODO: This is a list of variables not yet defined for post-processing,
      // but are defined in the F90 p3 interface code.  So this list will need
      // to be checked as new processes come online to make sure their
      // requirements from p3 are being met. qme, vap_liq_exchange ENERGY
      // Conservation: prec_str, snow_str RAD Vars: icinc, icwnc, icimrst,
      // icwmrst COSP Vars: flxprc, flxsnw, flxprc, flxsnw, cvreffliq,
      // cvreffice, reffsnow
    } // set_variables

    void set_mass_and_energy_fluxes(const view_1d &vapor_flux_,
...
#ifdef SCREAM_P3_SMALL_KERNELS
    uview_2d mu_r, T_atm, lamr, logn0r, nu, cdist, cdist1, cdistr,
        inv_cld_frac_i, inv_cld_frac_l, inv_cld_frac_r, qc_incld, qr_incld,
        qi_incld, qm_incld, nc_incld, nr_incld, ni_incld, bm_incld, inv_dz,
        inv_rho, ze_ice, ze_rain, prec, rho, rhofacr, rhofaci, acn, qv_sat_l,
        qv_sat_i, sup, qv_supersat_i, tmparr2, exner, diag_vm_qi, diag_diam_qi,
        pratot, prctot, qtend_ignore, ntend_ignore, mu_c, lamc, qr_evap_tend,
        v_qc, v_nc, flux_qx, flux_nx, v_qit, v_nit, flux_nit, flux_bir,
        flux_qir, flux_qit, v_qr, v_nr;
#endif
...
protected:
  ...
  // Iteration count is internal to P3 and keeps track of the number of times
  // p3_main has been called. infrastructure.it is passed as an arguement to
  // p3_main and is used for identifying which iteration an error occurs.
