...
struct UnitWrap::UnitTest<D>::TestCloudRainAccretion
  ...
  void run_bfb() {
    ...
    CloudRainAccretionData cloud_rain_acc_data[max_pack_size] = {
        // rho, inv_rho, qc_incld, nc_incld, qr_incld, qc2qr_accret_tend,
        // nc_accret_tend,
        // inv_qc_relvar
    ...
    // Sync to device
    view_1d<CloudRainAccretionData> device_data("cloud_rain_acc",
                                                max_pack_size);
    ...
    if (SCREAM_BFB_TESTING && this->m_baseline_action == COMPARE) {
      for (Int s = 0; s < max_pack_size; ++s) {
        REQUIRE(cloud_rain_acc_data[s].qc2qr_accret_tend ==
                host_data[s].qc2qr_accret_tend);
        REQUIRE(cloud_rain_acc_data[s].nc_accret_tend ==
                host_data[s].nc_accret_tend);
      }
...
TEST_CASE("p3_cloud_rain_accretion", "[p3_functions]") {
  using T = scream::p3::unit_test::UnitWrap::UnitTest<
      scream::DefaultDevice>::TestCloudRainAccretion;
