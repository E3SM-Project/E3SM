...
struct UnitWrap::UnitTest<D>::TestBackToCellAverage
  ...
  void run_bfb() {
    ...
    // Sync to device.
    view_1d<BackToCellAverageData> device_data("back_to_cell_average",
                                               max_pack_size);
    const auto host_data = Kokkos::create_mirror_view(device_data);
    std::copy(&back_to_cell_average_data[0],
              &back_to_cell_average_data[0] + max_pack_size, host_data.data());
    ...
    Kokkos::parallel_for(
        num_test_itrs, KOKKOS_LAMBDA(const Int &i) {
          ...
          // Init pack inputs
          Spack cld_frac_l, cld_frac_r, cld_frac_i, qc2qr_accret_tend,
              ...
              qv2qi_nucleat_tend, ni_nucleat_tend, qc2qi_berg_tend, ncheti_cnt,
          ...
          for (Int s = 0, vs = offset; s < Spack::n; ++s, ++vs) {
            ...
            qc2qi_hetero_freeze_tend[s] =
                device_data[s].qc2qi_hetero_freeze_tend;
            ...
            qr2qi_immers_freeze_tend[s] =
                device_data[s].qr2qi_immers_freeze_tend;
            ...
            nc2ni_immers_freeze_tend[s] =
                device_data[s].nc2ni_immers_freeze_tend;
            ...
            nr2ni_immers_freeze_tend[s] =
                device_data[s].nr2ni_immers_freeze_tend;
          ...
          Functions::back_to_cell_average(
              cld_frac_l, cld_frac_r, cld_frac_i, qc2qr_accret_tend,
              qr2qv_evap_tend, qc2qr_autoconv_tend, nc_accret_tend,
              nc_selfcollect_tend, nc2nr_autoconv_tend, nr_selfcollect_tend,
              nr_evap_tend, ncautr, qi2qv_sublim_tend, nr_ice_shed_tend,
              qc2qi_hetero_freeze_tend, qr2qi_collect_tend, qc2qr_ice_shed_tend,
              ...
              qv2qi_vapdep_tend, nr2ni_immers_freeze_tend, ni_sublim_tend,
              qv2qi_nucleat_tend, ni_nucleat_tend, qc2qi_berg_tend, ncheti_cnt,
              qcheti_cnt, nicnt, qicnt, ninuc_cnt, qinuc_cnt, context);
    ...
    if (SCREAM_BFB_TESTING && this->m_baseline_action == COMPARE) {
      for (Int s = 0; s < max_pack_size; ++s) {
        REQUIRE(back_to_cell_average_data[s].qc2qr_accret_tend ==
                host_data[s].qc2qr_accret_tend);
        REQUIRE(back_to_cell_average_data[s].qr2qv_evap_tend ==
                host_data[s].qr2qv_evap_tend);
        REQUIRE(back_to_cell_average_data[s].qc2qr_autoconv_tend ==
                host_data[s].qc2qr_autoconv_tend);
        REQUIRE(back_to_cell_average_data[s].nc_accret_tend ==
        ...
        REQUIRE(back_to_cell_average_data[s].nr_evap_tend ==
                host_data[s].nr_evap_tend);
        REQUIRE(back_to_cell_average_data[s].ncautr == host_data[s].ncautr);
        REQUIRE(back_to_cell_average_data[s].qi2qv_sublim_tend ==
                host_data[s].qi2qv_sublim_tend);
        REQUIRE(back_to_cell_average_data[s].nr_ice_shed_tend ==
                host_data[s].nr_ice_shed_tend);
        REQUIRE(back_to_cell_average_data[s].qc2qi_hetero_freeze_tend ==
                host_data[s].qc2qi_hetero_freeze_tend);
        REQUIRE(back_to_cell_average_data[s].qr2qi_collect_tend ==
                host_data[s].qr2qi_collect_tend);
        REQUIRE(back_to_cell_average_data[s].qc2qr_ice_shed_tend ==
                host_data[s].qc2qr_ice_shed_tend);
        REQUIRE(back_to_cell_average_data[s].qi2qr_melt_tend ==
                host_data[s].qi2qr_melt_tend);
        REQUIRE(back_to_cell_average_data[s].qc2qi_collect_tend ==
                host_data[s].qc2qi_collect_tend);
        REQUIRE(back_to_cell_average_data[s].qr2qi_immers_freeze_tend ==
                host_data[s].qr2qi_immers_freeze_tend);
        REQUIRE(back_to_cell_average_data[s].ni2nr_melt_tend ==
                host_data[s].ni2nr_melt_tend);
        REQUIRE(back_to_cell_average_data[s].nc_collect_tend ==
                host_data[s].nc_collect_tend);
        ...
        REQUIRE(back_to_cell_average_data[s].nr_collect_tend ==
                host_data[s].nr_collect_tend);
        REQUIRE(back_to_cell_average_data[s].ni_selfcollect_tend ==
                host_data[s].ni_selfcollect_tend);
        REQUIRE(back_to_cell_average_data[s].qv2qi_vapdep_tend ==
                host_data[s].qv2qi_vapdep_tend);
        REQUIRE(back_to_cell_average_data[s].nr2ni_immers_freeze_tend ==
                host_data[s].nr2ni_immers_freeze_tend);
        REQUIRE(back_to_cell_average_data[s].ni_sublim_tend ==
                host_data[s].ni_sublim_tend);
        REQUIRE(back_to_cell_average_data[s].qv2qi_nucleat_tend ==
                host_data[s].qv2qi_nucleat_tend);
        REQUIRE(back_to_cell_average_data[s].ni_nucleat_tend ==
                host_data[s].ni_nucleat_tend);
        REQUIRE(back_to_cell_average_data[s].qc2qi_berg_tend ==
                host_data[s].qc2qi_berg_tend);
      }
...
TEST_CASE("p3_back_to_cell_average", "[p3_functions]") {
  using T = scream::p3::unit_test::UnitWrap::UnitTest<
      scream::DefaultDevice>::TestBackToCellAverage;
