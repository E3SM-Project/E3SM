...
struct UnitWrap::UnitTest<D>::TestP3Main : public UnitWrap::UnitTest<D>::Base {
  ...
  void run_bfb_p3_main_part1() {
    ...
    constexpr Scalar qsmall =
        C::QSMALL; // PMC wouldn't it make more sense to define qsmall at a
    ...
    P3MainPart1Data isds_baseline[] = {
        //            kts, kte, ktop, kbot, kdir, do_predict_nc,
        //            do_prescribed_CCN,       dt
    ...
    static constexpr Int num_runs =
        sizeof(isds_baseline) / sizeof(P3MainPart1Data);

    for (auto &d : isds_baseline) {
      const auto qsmall_r = std::make_pair(
          0, qsmall * 2); // PMC this range seems inappropriately small
      d.randomize(engine,
    ...
    for (auto &d : isds_cxx) {
      p3_main_part1_host(
          d.kts, d.kte, d.ktop, d.kbot, d.kdir, d.do_predict_nc,
          d.do_prescribed_CCN, d.dt, d.pres, d.dpres, d.dz, d.nc_nuceat_tend,
          d.nccn_prescribed, d.inv_exner, d.exner, d.inv_cld_frac_l,
          d.inv_cld_frac_i, d.inv_cld_frac_r, d.T_atm, d.rho, d.inv_rho,
          d.qv_sat_l, d.qv_sat_i, d.qv_supersat_i, d.rhofacr, d.rhofaci, d.acn,
          d.qv, d.th_atm, d.qc, d.nc, d.qr, d.nr, d.qi, d.ni, d.qm, d.bm,
          d.qc_incld, d.qr_incld, d.qi_incld, d.qm_incld, d.nc_incld,
          d.nr_incld, d.ni_incld, d.bm_incld, &d.is_nucleat_possible,
    ...
    if (SCREAM_BFB_TESTING && this->m_baseline_action == COMPARE) {
      for (Int i = 0; i < num_runs; ++i) {
        ...
        for (Int k = start; k < end; ++k) {
          ...
          REQUIRE(isds_baseline[i].qv_supersat_i[k] ==
                  isds_cxx[i].qv_supersat_i[k]);
        ...
        REQUIRE(isds_baseline[i].is_hydromet_present ==
                isds_cxx[i].is_hydromet_present);
        REQUIRE(isds_baseline[i].is_nucleat_possible ==
                isds_cxx[i].is_nucleat_possible);
      }
  ...
  void run_bfb_p3_main_part2() {
    ...
    std::vector<Real> qr2qv_evap(72, 0.0), qi2qv_sublim(72, 0.0),
        qc2qr_accret(72, 0.0), qc2qr_autoconv(72, 0.0), qv2qi_vapdep(72, 0.0),
        qc2qi_berg(72, 0.0), qc2qr_ice_shed(72, 0.0), qc2qi_collect(72, 0.0),
        qr2qi_collect(72, 0.0), qc2qi_hetero_freeze(72, 0.0),
        qr2qi_immers_freeze(72, 0.0), qi2qr_melt(72, 0.0);
    static constexpr Int num_runs =
        sizeof(isds_baseline) / sizeof(P3MainPart2Data);
    ...
    for (auto &d : isds_cxx) {
      p3_main_part2_host(
          d.kts, d.kte, d.kbot, d.ktop, d.kdir, d.do_predict_nc,
          d.do_prescribed_CCN, d.dt, d.inv_dt,
          hetfrz_immersion_nucleation_tend.data(),
          hetfrz_contact_nucleation_tend.data(),
          hetfrz_deposition_nucleation_tend.data(), d.pres, d.dpres, d.dz,
          d.nc_nuceat_tend, d.inv_exner, d.exner, d.inv_cld_frac_l,
          d.inv_cld_frac_i, d.inv_cld_frac_r, d.ni_activated, d.inv_qc_relvar,
          d.cld_frac_i, d.cld_frac_l, d.cld_frac_r, d.qv_prev, d.t_prev,
          d.T_atm, d.rho, d.inv_rho, d.qv_sat_l, d.qv_sat_i, d.qv_supersat_i,
          d.rhofacr, d.rhofaci, d.acn, d.qv, d.th_atm, d.qc, d.nc, d.qr, d.nr,
          d.qi, d.ni, d.qm, d.bm, d.qc_incld, d.qr_incld, d.qi_incld,
          d.qm_incld, d.nc_incld, d.nr_incld, d.ni_incld, d.bm_incld, d.mu_c,
          d.nu, d.lamc, d.cdist, d.cdist1, d.cdistr, d.mu_r, d.lamr, d.logn0r,
          d.qv2qi_depos_tend, d.precip_total_tend, d.nevapr, d.qr_evap_tend,
          d.vap_liq_exchange, d.vap_ice_exchange, d.liq_ice_exchange,
          qr2qv_evap.data(), qi2qv_sublim.data(), qc2qr_accret.data(),
          qc2qr_autoconv.data(), qv2qi_vapdep.data(), qc2qi_berg.data(),
          qc2qr_ice_shed.data(), qc2qi_collect.data(), qr2qi_collect.data(),
          qc2qi_hetero_freeze.data(), qr2qi_immers_freeze.data(),
          qi2qr_melt.data(), d.pratot, d.prctot, &d.is_hydromet_present);
    }
    ...
    if (SCREAM_BFB_TESTING && this->m_baseline_action == COMPARE) {
      for (Int i = 0; i < num_runs; ++i) {
        ...
        for (Int k = start; k < end; ++k) {
          ...
          REQUIRE(isds_baseline[i].qv2qi_depos_tend[k] ==
                  isds_cxx[i].qv2qi_depos_tend[k]);
          REQUIRE(isds_baseline[i].precip_total_tend[k] ==
                  isds_cxx[i].precip_total_tend[k]);
          REQUIRE(isds_baseline[i].nevapr[k] == isds_cxx[i].nevapr[k]);
          REQUIRE(isds_baseline[i].qr_evap_tend[k] ==
                  isds_cxx[i].qr_evap_tend[k]);
          REQUIRE(isds_baseline[i].vap_liq_exchange[k] ==
                  isds_cxx[i].vap_liq_exchange[k]);
          REQUIRE(isds_baseline[i].vap_ice_exchange[k] ==
                  isds_cxx[i].vap_ice_exchange[k]);
          REQUIRE(isds_baseline[i].liq_ice_exchange[k] ==
                  isds_cxx[i].liq_ice_exchange[k]);
  ...
  void run_bfb_p3_main_part3() {
    ...
    static constexpr Int num_runs =
        sizeof(isds_baseline) / sizeof(P3MainPart3Data);
    ...
    // Get data from cxx
    for (auto &d : isds_cxx) {
      p3_main_part3_host(d.kts, d.kte, d.kbot, d.ktop, d.kdir, d.inv_exner,
                         d.cld_frac_l, d.cld_frac_r, d.cld_frac_i, d.rho,
                         d.inv_rho, d.rhofaci, d.qv, d.th_atm, d.qc, d.nc, d.qr,
                         d.nr, d.qi, d.ni, d.qm, d.bm, d.mu_c, d.nu, d.lamc,
                         d.mu_r, d.lamr, d.vap_liq_exchange, d.ze_rain,
                         d.ze_ice, d.diag_vm_qi, d.diag_eff_radius_qi,
                         d.diag_diam_qi, d.rho_qi, d.diag_equiv_reflectivity,
    ...
    if (SCREAM_BFB_TESTING && this->m_baseline_action == COMPARE) {
      for (Int i = 0; i < num_runs; ++i) {
        ...
        for (Int k = start; k < end; ++k) {
          ...
          REQUIRE(isds_baseline[i].diag_eff_radius_qi[k] ==
                  isds_cxx[i].diag_eff_radius_qi[k]);
          REQUIRE(isds_baseline[i].diag_diam_qi[k] ==
                  isds_cxx[i].diag_diam_qi[k]);
          ...
          REQUIRE(isds_baseline[i].diag_eff_radius_qc[k] ==
                  isds_cxx[i].diag_eff_radius_qc[k]);
          REQUIRE(isds_baseline[i].diag_eff_radius_qr[k] ==
                  isds_cxx[i].diag_eff_radius_qr[k]);
        }
      }
    } else if (this->m_baseline_action == GENERATE) {
      for (Int i = 0; i < num_runs; ++i) {
  ...
  void run_bfb_p3_main() {
    ...
    for (auto &d : isds_baseline) {
      d.randomize(engine, {{d.pres, {1.00000000E+02, 9.87111111E+04}},
                           ...
                           {d.th_atm,
                            {6.72653866E+02,
                             1.07954335E+03}}, // PMC - this range seems insane
    ...
    for (auto &d : isds_cxx) {
      p3_main_host(
          d.qc, d.nc, d.qr, d.nr, d.th_atm, d.qv, d.dt, d.qi, d.qm, d.ni, d.bm,
          d.pres, d.dz, d.nc_nuceat_tend, d.nccn_prescribed, d.ni_activated,
          d.inv_qc_relvar, d.it, d.precip_liq_surf, d.precip_ice_surf, d.its,
          d.ite, d.kts, d.kte, d.diag_eff_radius_qc, d.diag_eff_radius_qi,
          d.diag_eff_radius_qr, d.rho_qi, d.do_predict_nc, d.do_prescribed_CCN,
          d.use_hetfrz_classnuc, d.dpres, d.inv_exner, d.qv2qi_depos_tend,
          d.precip_liq_flux, d.precip_ice_flux, d.cld_frac_r, d.cld_frac_l,
          d.cld_frac_i, d.liq_ice_exchange, d.vap_liq_exchange,
...
TEST_CASE("p3_main", "[p3_functions]") {
  using T = scream::p3::unit_test::UnitWrap::UnitTest<
      scream::DefaultDevice>::TestP3Main;
