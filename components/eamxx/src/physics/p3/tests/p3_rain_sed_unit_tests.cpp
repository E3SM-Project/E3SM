...
struct UnitWrap::UnitTest<D>::TestRainSed : public UnitWrap::UnitTest<D>::Base {
  ...
  void run_bfb_rain_vel() {
    ...
    std::copy(&crfv_baseline[0], &crfv_baseline[0] + max_pack_size,
              crfv_host.data());
    ...
    Kokkos::parallel_for(
        num_test_itrs, KOKKOS_LAMBDA(const Int &i) {
          ...
          Functions::compute_rain_fall_velocity(
              vn_table_vals, vm_table_vals, qr_incld, rhofacr, nr_incld, mu_r,
              lamr, V_qr, V_nr,
  ...
  void run_bfb_rain_sed() {
    ...
    for (auto &d : rsds_cxx) {
      ...
      inv_dt *= 2;
#endif
      rain_sedimentation_host(d.kts, d.kte, d.ktop, d.kbot, d.kdir, d.qr_incld,
                              d.rho, d.inv_rho, d.rhofacr, d.cld_frac_r,
                              d.inv_dz, d.dt, inv_dt, d.qr, d.nr, d.nr_incld,
                              d.mu_r, d.lamr, &d.precip_liq_surf,
                              d.precip_liq_flux, d.qr_tend, d.nr_tend);
    }

    if (SCREAM_BFB_TESTING && this->m_baseline_action == COMPARE) {
      for (Int i = 0; i < num_runs; ++i) {
        // Due to pack issues, we must restrict checks to the active k space
        Int start = std::min(rsds_baseline[i].kbot, rsds_baseline[i].ktop) -
                    1; // 0-based indx
        Int end = std::max(rsds_baseline[i].kbot,
                           rsds_baseline[i].ktop); // 0-based indx
        for (Int k = start; k < end; ++k) {
          ...
          REQUIRE(rsds_baseline[i].precip_liq_flux[k] ==
                  rsds_cxx[i].precip_liq_flux[k]);
          REQUIRE(rsds_baseline[i].qr_tend[k] == rsds_cxx[i].qr_tend[k]);
          REQUIRE(rsds_baseline[i].nr_tend[k] == rsds_cxx[i].nr_tend[k]);
        }
        REQUIRE(rsds_baseline[i].precip_liq_flux[end] ==
                rsds_cxx[i].precip_liq_flux[end]);
        REQUIRE(rsds_baseline[i].precip_liq_surf ==
                rsds_cxx[i].precip_liq_surf);
      }
    } else if (this->m_baseline_action == GENERATE) {
      for (Int i = 0; i < num_runs; ++i) {
...
TEST_CASE("p3_rain_sed", "[p3_functions]") {
  using T = scream::p3::unit_test::UnitWrap::UnitTest<
      scream::DefaultDevice>::TestRainSed;
