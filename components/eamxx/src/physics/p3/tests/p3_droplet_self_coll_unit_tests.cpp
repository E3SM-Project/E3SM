...
struct UnitWrap::UnitTest<D>::TestDropletSelfCollection
  ...
  void run_bfb() {
    ...
    constexpr Scalar rho1 = 4.056E-03, rho2 = 6.852E-02, rho3 = 8.852E-02,
                     rho4     = 1.902E-01;
    ...
    DropletSelfCollectionData droplet_self_coll_data[max_pack_size] = {
        // rho, inv_rho, qc_incld, mu_c, nu, nc2nr_autoconv_tend,
        // nc_selfcollect_tend
    ...
    // Sync to device
    view_1d<DropletSelfCollectionData> device_data("droplet_self_coll",
                                                   max_pack_size);
    const auto host_data = Kokkos::create_mirror_view(device_data);
    std::copy(&droplet_self_coll_data[0],
              &droplet_self_coll_data[0] + max_pack_size, host_data.data());
    ...
    Kokkos::parallel_for(
        num_test_itrs, KOKKOS_LAMBDA(const Int &i) {
          ...
          Functions::droplet_self_collection(rho, inv_rho, qc_incld, mu_c, nu,
                                             nc2nr_autoconv_tend,
                                             nc_selfcollect_tend);
    ...
    if (SCREAM_BFB_TESTING && this->m_baseline_action == COMPARE) {
      for (Int s = 0; s < max_pack_size; ++s) {
        REQUIRE(droplet_self_coll_data[s].nc_selfcollect_tend ==
...
TEST_CASE("p3_droplet_self_collection", "[p3_functions]") {
  using T = scream::p3::unit_test::UnitWrap::UnitTest<
      scream::DefaultDevice>::TestDropletSelfCollection;
