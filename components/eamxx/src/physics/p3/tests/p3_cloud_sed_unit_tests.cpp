...
struct UnitWrap::UnitTest<D>::TestCloudSed
  ...
  void run_bfb() {
    ...
    static constexpr Int num_runs =
        sizeof(csds_baseline) / sizeof(CloudSedData);
    ...
    // Get data from cxx
    for (auto &d : csds_cxx) {
      cloud_sedimentation_host(d.kts, d.kte, d.ktop, d.kbot, d.kdir, d.qc_incld,
                               d.rho, d.inv_rho, d.cld_frac_l, d.acn, d.inv_dz,
                               d.dt, d.inv_dt, d.do_predict_nc, d.qc, d.nc,
                               d.nc_incld, d.mu_c, d.lamc, &d.precip_liq_surf,
                               d.qc_tend, d.nc_tend);
    }
    ...
    if (SCREAM_BFB_TESTING && this->m_baseline_action == COMPARE) {
      for (Int i = 0; i < num_runs; ++i) {
        ...
        Int start = std::min(csds_baseline[i].kbot, csds_baseline[i].ktop) -
                    1; // 0-based indx
        Int end = std::max(csds_baseline[i].kbot,
                           csds_baseline[i].ktop); // 0-based indx
        ...
        REQUIRE(csds_baseline[i].precip_liq_surf ==
                csds_cxx[i].precip_liq_surf);
      }
...
TEST_CASE("p3_cloud_sed", "[p3_functions]") {
  using T = scream::p3::unit_test::UnitWrap::UnitTest<
      scream::DefaultDevice>::TestCloudSed;
