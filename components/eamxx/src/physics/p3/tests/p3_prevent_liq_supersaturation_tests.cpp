...
struct UnitWrap::UnitTest<D>::TestPreventLiqSupersaturation
  ...
  void run_property()
  // Conceptual tests for prevent_liq_supersaturation. Note many conceptual
  // tests make sense to run on random data, so are included in run_bfb rather
  // than here.
  {
    ...
    // Note that inout variables are initialized for each test to avoid using
    // unintended vals
    ...
    // Next, confirm that initial data is subsaturated at beginning of step and
    // supersaturated at end
    ...
    Spack T_endstep  = t_atm + ((qv_sinks - qi2qv_sublim_tend_tmp) *
                                   (latvap + latice) * inv_cp -
                               qr2qv_evap_tend_tmp * latvap * inv_cp) *
                                  dt;
    Spack qsl =
        physics::qv_sat_dry(T_endstep, pres, false,
                            context); //"false" means NOT sat w/ respect to ice
    // just require index 0 since all entries are identical

    REQUIRE(
        qv[0] <
        qsl[0]); // not a test of prevent_liq_supersat, just a sanity check that
    REQUIRE(qv_endstep[0] >
            qsl[0]); // inputs to this test are testing what we want.
    ...
    T_endstep  = t_atm + ((qv_sinks - qi2qv_sublim_tend_tmp) *
                             (latvap + latice) * inv_cp -
                         qr2qv_evap_tend_tmp * latvap * inv_cp) *
                            dt;
    qsl =
    ...
    Functions::prevent_liq_supersaturation(pres, t_atm, qv_tmp, dt, qidep,
                                           qinuc, qi2qv_sublim_tend_tmp2,
  ...
  void run_bfb() {
    ...
    view_1d<PreventLiqSupersaturationData> cxx_device("cxx_device",
                                                      max_pack_size);
    ...
    // Save copy of inout vars to check that prevent_liq_supersaturation always
    // makes them smaller
    Real qi2qv_sublim_tend_init[max_pack_size],
        qr2qv_evap_tend_init[max_pack_size];
    ...
    // Get data from cxx. Run prevent_liq_supersaturation from a kernel and copy
    ...
    Kokkos::parallel_for(
        num_test_itrs, KOKKOS_LAMBDA(const Int &i) {
          ...
          // Init pack inputs
          Scalar dt;
          Spack pres, qi2qv_sublim_tend, qidep, qinuc, qr2qv_evap_tend, qv,
              t_atm;
          for (Int s = 0, vs = offset; s < Spack::n; ++s, ++vs) {
            dt = cxx_device(vs)
                     .dt; // dt is scalar but PreventLiqSupersaturationData has
                          // diff val for each row.
          ...
          Functions::prevent_liq_supersaturation(pres, t_atm, qv, dt, qidep,
                                                 qinuc, qi2qv_sublim_tend,
                                                 qr2qv_evap_tend);
