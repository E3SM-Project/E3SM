...
P3Data::P3Data(Int ncol_, Int nlev_) : ncol(ncol_), nlev(nlev_) {
  do_predict_nc     = true;
  ...
  qv2qi_depos_tend =
      Array2("qitend due to deposition/sublimation ", ncol, nlev);
  precip_liq_flux = Array2("grid-box average rain flux (kg m^-2 s^-1), pverp",
                           ncol, nlev + 1);
  precip_ice_flux = Array2(
      "grid-box average ice/snow flux (kg m^-2 s^-1), pverp", ncol, nlev + 1);
  ...
  liq_ice_exchange =
      Array2("sum of liq-ice phase change tendenices", ncol, nlev);
  vap_liq_exchange =
      Array2("sum of vap-liq phase change tendenices", ncol, nlev);
  vap_ice_exchange =
      Array2("sum of vap-ice phase change tendenices", ncol, nlev);
  hetfrz_immersion_nucleation_tend =
      Array2("hetfrz_immersion_nucleation_tend", ncol, nlev);
  hetfrz_contact_nucleation_tend =
      Array2("hetfrz_contact_nucleation_tend", ncol, nlev);
  hetfrz_deposition_nucleation_tend =
      Array2("hetfrz_deposition_nucleation_tend", ncol, nlev);
}

P3DataIterator::P3DataIterator(const P3Data::Ptr &d) { init(d); }
...
#define fdipb(name)                                                   \
  ...
  fdipb(pres);
  fdipb(dz);
  fdipb(nc_nuceat_tend);
  fdipb(nccn_prescribed);
  fdipb(ni_activated);
  fdipb(inv_qc_relvar);
  fdipb(qc);
  fdipb(nc);
  ...
  fdipb(qm);
  fdipb(bm);
  fdipb(precip_liq_surf);
  fdipb(precip_ice_surf);
  fdipb(diag_eff_radius_qc);
  fdipb(diag_eff_radius_qi);
  fdipb(diag_eff_radius_qr);
  fdipb(rho_qi);
  fdipb(dpres);
  fdipb(inv_exner);
  fdipb(qv2qi_depos_tend);
  fdipb(precip_liq_flux);
  fdipb(precip_ice_flux);
  fdipb(cld_frac_r);
  fdipb(cld_frac_l);
  fdipb(cld_frac_i);
  fdipb(liq_ice_exchange);
  fdipb(vap_liq_exchange);
...
const P3DataIterator::RawArray &P3DataIterator::getfield(Int i) const {
