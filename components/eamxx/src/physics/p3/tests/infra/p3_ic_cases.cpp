...
P3Data::Ptr make_mixed(const Int ncol, const Int nlev) {
  ...
  for (Int i = 0; i < ncol; ++i) {
    ...
    for (k = 0; k < nk; ++k)
      d.nccn_prescribed(i, k) = (100.0 + i / ncol - k / nk) * 1e6;

    // max cld at ~700mb, decreasing to 0 at 900mb.
    for (k = 0; k < 15; ++k)
      d.qc(i, nk - 20 + k) = 1e-4 * (1 - double(k) / 14);
    for (k = 0; k < nk; ++k)
      d.nc(i, k) = 1e6;
    // max rain at 700mb, decreasing to zero at surf.
    for (k = 0; k < 20; ++k)
      d.qr(i, nk - 20 + k) = 1e-5 * (1 - double(k) / 19);
    for (k = 0; k < nk; ++k)
      d.nr(i, k) = 1e6;

    //                                                      v (in the python)
    for (k = 0; k < 15; ++k)
      d.qi(i, nk - 20 + k) = 1e-4; //*(1 - double(k)/14)
    for (k = 0; k < nk; ++k)
      d.ni(i, k) = 1e6;
    for (k = 0; k < 15; ++k)
    ...
    for (k = 0; k < nk; ++k)
      d.pres(i, k) = 100 + 1e5 / double(nk) * k;
