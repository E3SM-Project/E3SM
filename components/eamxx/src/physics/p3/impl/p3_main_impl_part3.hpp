...
KOKKOS_FUNCTION void Functions<S, D>::p3_main_part3(
  ...
  Kokkos::parallel_for(Kokkos::TeamVectorRange(team, nk_pack), [&](Int k) {
    Spack ignore1(0), ignore2(0), table_val_qi_fallspd(0),
        table_val_ice_eff_radius(0), table_val_ni_lammax(0),
        table_val_ni_lammin(0), table_val_ice_reflectivity(0),
    ...
    // Cloud
    {
      ...
      get_cloud_dsd2(qc_incld, nc_incld, mu_c(k), rho(k), nu(k), dnu, lamc(k),
                     ignore1, ignore2, qc_gt_small);

      if (qc_gt_small.any()) {
        nc(k).set(qc_gt_small,
                  nc_incld * cld_frac_l(k)); // cld_dsd2 might have changed
                                             // incld nc... need consistency.
        diag_eff_radius_qc(k).set(qc_gt_small,
                                  sp(0.5) * (mu_c(k) + 3) / lamc(k));
      }
      if (qc_small.any()) {
        qv(k).set(qc_small, qv(k) + qc(k));
        th_atm(k).set(qc_small,
                      th_atm(k) - inv_exner(k) * qc(k) * latvap * inv_cp);
    ...
    // Rain
    {
      ...
      auto nr_incld =
          nr(k) /
          cld_frac_r(
              k); // nr_incld is updated in get_rain_dsd2 but isn't used again

      get_rain_dsd2(qr_incld, nr_incld, mu_r(k), lamr(k), runtime_options,
                    qr_gt_small);

      // Note that integrating over the drop-size PDF as done here should only
      // be done to in-cloud quantities but radar reflectivity is likely meant
      // to be a cell ave. Thus nr in the next line really should be cld_frac_r
      // * nr/cld_frac_r. Not doing that since cld_frac_r cancels out.
      if (qr_gt_small.any()) {
        nr(k).set(qr_gt_small,
                  nr_incld * cld_frac_r(k)); // rain_dsd2 might have changed
                                             // incld nr... need consistency.
        ze_rain(k).set(
            qr_gt_small,
            nr(k) * (mu_r(k) + 6) * (mu_r(k) + 5) * (mu_r(k) + 4) *
      ...
      if (qr_small.any()) {
        ...
        th_atm(k).set(qr_small,
                      th_atm(k) - inv_exner(k) * qr(k) * latvap * inv_cp);
    ...
    // Ice
    {
      ...
      const auto rhop = calc_bulk_rho_rime(qi_incld, qm_incld, bm_incld,
      ...
      // note that reflectivity from lookup table is normalized, so we need to
      // multiply by N
      diag_vm_qi(k).set(qi_gt_small, table_val_qi_fallspd * rhofaci(k));
      diag_eff_radius_qi(k).set(qi_gt_small,
                                table_val_ice_eff_radius); // units are in m
      ...
      ze_ice(k).set(qi_gt_small,
                    ze_ice(k) +
                        sp(0.1892) * table_val_ice_reflectivity * ni_incld *
                            rho(k)); // sum contribution from each ice category
                                     // (note: 0.1892 = 0.176/0.93);
      ze_ice(k).set(qi_gt_small, max(ze_ice(k), sp(1.e-22)));

      // above formula for ze only makes sense for in-cloud vals, but users
      // expect cell-ave output.
    ...
    diag_equiv_reflectivity(k) =
        10 * log10((ze_rain(k) + ze_ice(k)) * sp(1.e18));

    // if qr is very small then set Nr to 0 (needs to be done here after call
    // to ice lookup table because a minimum Nr of nsmall will be set otherwise
    // even if qr=0)
