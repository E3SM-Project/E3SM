...
Functions<S, D>::subgrid_variance_scaling(const Spack &relvar,
                                          const Scalar &expon) {
  /* We assume subgrid variations in qc follow a gamma distribution with inverse
     ...
     for a given process is of the form A*qc**expon for a local value of qc,
     then the average process rate over the PDF is
     gamma(relvar+expon)/[gamma(relvar)*relvar**expon]*A*average(qc)**expon.
     This function calculates the local process rate => cell-average process
     rate scaling factor gamma(relvar+expon)/[gamma(relvar)*relvar**expon]. See
     Morrison and Gettelman (2008; JCLI) eq 9 for details.
  */
  ...
  const auto relvar_exceeds_bounds  = !(relvar > relvar_min && relvar <
  relvar_max);
  ...
  //============================================
  if (expon < 0.0){
    const auto msg = "expon<0. This might be ok, but isn't unit tested and can
  drive subgrid_variance_scaling negative. Be careful if you proceed.";
