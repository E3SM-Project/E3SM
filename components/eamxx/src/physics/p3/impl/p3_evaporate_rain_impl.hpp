...
Functions<S, D>::rain_evap_tscale_weight(const Spack &dt_over_tau,
...
template <typename S, typename D>
KOKKOS_FUNCTION void Functions<S, D>::rain_evap_equilib_tend(
    const Spack &A_c, const Spack &ab, const Spack &tau_eff, const Spack &tau_r,
    Spack &tend, const Smask &context) {
  /*
    In equilibrium, the total evaporation must balance the tendency A_c from
    all other processes. The rain evaporation is the fraction
    (1/tau_r)/(1/tau_eff) of the total tendency and ab corrects for saturation
    changes due to evaporative cooling.
  */

  // Sign convention: Negative A_c causes a supersaturation deficit which needs
  // to be removed by evaporation (which is signed positive when active) to
  // maintain equilibrium. Thus A_c needs a negative sign here since other terms
  // are always positive.
...
template <typename S, typename D>
KOKKOS_FUNCTION void
Functions<S, D>::rain_evap_instant_tend(const Spack &ssat_r, const Spack &ab,
  ...
  */
  ...
  // sign convention: ssat_r must be <0 for evap, other terms are always
  // positive, and we want evap rate positive... so put a minus sign in front.
  tend.set(context, -ssat_r / (ab * tau_r));
}

template <typename S, typename D>
KOKKOS_FUNCTION void Functions<S, D>::evaporate_rain(
    const Spack &qr_incld, const Spack &qc_incld, const Spack &nr_incld,
    ...
    const Spack &qv_sat_i, const Spack &ab, const Spack &abi, const Spack &epsr,
    const Spack &epsi_tot, const Spack &t_atm, const Spack &t_atm_prev,
    const Spack &dqsdt, const Scalar &dt, Spack &qr2qv_evap_tend,
  ...
  const Smask is_rain_evap =
      qr_ge_qsmall && is_subsat && is_evap_area && context;
  if (is_rain_evap.any()) {

    // if qr_incld<QSMALL, epsr=0 causes div by 0 error for tau_r even though it
    // isn't used.
    Spack tau_r(0);
    tau_r.set(is_rain_evap, 1 / epsr);

    // Compute total effective inverse saturation removal timescale eps_eff
    // qc saturation is handled by macrophysics so the qc saturation removal
    // timescale is not included here. Below freezing, eps_eff is the sum of the
    // inverse saturation removal timescales for liquid and ice. The ice term
    // has extra scaling terms to convert it from being relative to ice to
    // liquid instead. Also compute the constant source/sink term A_c for
    // analytic integration. See Eq C3 and C4 of Morrison+Milbrandt 2015
    ...
    Spack eps_eff, A_c;
    if (is_freezing.any()) {
      eps_eff.set(is_freezing,
                  epsr + epsi_tot * (1 + (latvap + latice) * inv_cp * dqsdt) /
                             abi);
      A_c.set(is_freezing, (qv - qv_prev) * inv_dt -
                               dqsdt * (t_atm - t_atm_prev) * inv_dt -
                               (qv_sat_l - qv_sat_i) *
                                   (1 + (latvap + latice) * inv_cp * dqsdt) /
                                   abi * epsi_tot);
    }
    if (not_freezing.any()) {
      eps_eff.set(not_freezing, epsr);
      A_c.set(not_freezing,
              (qv - qv_prev) * inv_dt - dqsdt * (t_atm - t_atm_prev) * inv_dt);
    }
    ...
    // If qr is reasonably big, compute timestep-averaged evap rate as the
    // weighted ave of instantaneous and equilibrium evap rates with weighting
    // timescale tscale_weight. L'Hospital's rull shows tscale_weight is 1 in
    // the limit of small dt. It approaches 0 as dt gets big.
    if (not_qr_tiny.any()) {
      Spack tscale_weight, equilib_tend, instant_tend;
      rain_evap_tscale_weight(dt / tau_eff, tscale_weight, is_rain_evap);
      rain_evap_equilib_tend(A_c, ab, tau_eff, tau_r, equilib_tend,
                             is_rain_evap);
      rain_evap_instant_tend(ssat_r, ab, tau_r, instant_tend, is_rain_evap);

      qr2qv_evap_tend.set(not_qr_tiny, instant_tend * tscale_weight +
    ...
    const Smask is_overevap =
        qr2qv_evap_tend > -ssat_r * inv_dt / ab && is_rain_evap;
    ...
    // allow rain evap to also condense by forcing qr2qv_evap_tend to be
    // positive
    ...
    qr2qv_evap_tend.set(is_rain_evap,
                        qr2qv_evap_tend * (cld_frac_r - cld_frac) / cld_frac_r);
