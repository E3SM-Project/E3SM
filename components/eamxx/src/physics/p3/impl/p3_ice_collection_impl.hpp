...
KOKKOS_FUNCTION void Functions<S, D>::ice_cldliq_collection(
    ...
    Spack &qc2qr_ice_shed_tend, Spack &ncshdc, const P3Runtime &runtime_options,
    const Smask &context) {
...
KOKKOS_FUNCTION void Functions<S, D>::ice_rain_collection(
    ...
    const Spack &ni_incld, const Spack &qr_incld, Spack &qr2qi_collect_tend,
  ...
  nr_collect_tend.set(both_ge_small && !t_is_negative,
                      pow(ten, table_val_nr_collect + logn0r) * rho * rhofaci *
                          rain_to_ice_collection_factor * ni_incld);
  // for now neglect shedding of ice collecting rain above freezing, since snow
  // is not expected to shed in these conditions (though more hevaily rimed ice
  // would be expected to lead to shedding)
}
...
KOKKOS_FUNCTION void Functions<S, D>::ice_self_collection(
    ...
    const Spack &qm_incld, const Spack &qi_incld, const Spack &ni_incld,
  ...
  if (qi_incld_ge_small.any()) {
    // Determine additional collection efficiency factor to be applied to
    // ice-ice collection. The computed values of qicol and nicol are multipiled
    // by Eii_fact to gradually shut off collection if ice is highly rimed.
    ...
    ni_selfcollect_tend.set(qi_incld_ge_small, table_val_ni_self_collect * rho *
                                                   eii * Eii_fact * rhofaci *
                                                   ni_incld * ni_incld);
  }
}
