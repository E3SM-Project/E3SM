...
KOKKOS_FUNCTION Int Functions<S, D>::find_bottom(
    const MemberType &team, const uview_1d<const Scalar> &v,
  ...
  if (team.team_size() == 1) {
    ...
    if (kdir == -1) {
      Kokkos::parallel_reduce(
          Kokkos::TeamVectorRange(team, kbot - ktop + 1),
          [&](Int k_, int &lmax) {
            const Int k = ktop + k_;
            if (v(k) >= small && k > lmax)
              lmax = k;
      ...
      Kokkos::parallel_reduce(
          Kokkos::TeamVectorRange(team, ktop - kbot + 1),
          [&](Int k_, int &lmin) {
            const Int k = kbot + k_;
            if (v(k) >= small && k < lmin)
              lmin = k;
