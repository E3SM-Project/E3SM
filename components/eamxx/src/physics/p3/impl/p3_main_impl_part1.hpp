...
KOKKOS_FUNCTION void Functions<S, D>::p3_main_part1(
    const MemberType &team, const Int &nk, const bool &predictNc,
    ...
    bool &nucleationPossible, bool &hydrometeorsPresent,
  ...
  Kokkos::parallel_for(Kokkos::TeamVectorRange(team, nk_pack), [&](Int k) {
    ...
    inv_rho(k) = 1 / rho(k);
    qv_sat_l(k) =
        physics::qv_sat_dry(T_atm(k), pres(k), false, range_mask,
                            physics::MurphyKoop, "p3::p3_main_part1 (liquid)");
    qv_sat_i(k) =
        physics::qv_sat_dry(T_atm(k), pres(k), true, range_mask,
                            physics::MurphyKoop, "p3::p3_main_part1 (ice)");
    ...
    Spack dum =
        sp(1.496e-6) * pow(T_atm(k), sp(1.5)) / (T_atm(k) + 120); // this is mu
    acn(k) = g * rho_h2o /
             (18 * dum); // 'a' parameter for droplet fallspeed (Stokes' law)
    ...
    if (not_drymass.any()) {
      ...
      // Apply droplet activation here (before other microphysical processes)
      // for consistency with qc increase by saturation adjustment already
      // applied in macrophysics. If prescribed drop number is used, this is
      // also a good place to prescribe that value
    ...
    th_atm(k).set(drymass, th_atm(k) - inv_exner(k) * qi(k) *
