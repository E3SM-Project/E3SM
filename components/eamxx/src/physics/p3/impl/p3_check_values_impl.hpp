...
/*!------------------------------------------------------------------------------------
  ...
  to allow for Q<qsmall.and.N>nsmall or Q>qsmall.and.N<small which can be
  produced at the leading edges due to sedimentation and whose values are
  accpetable since lambda limiters are later imposed after SEDI (so one does not
  necessarily want to trap for inconsistency after sedimentation has been
  called).
...
KOKKOS_FUNCTION void Functions<S, D>::check_values(
    const uview_1d<const Spack> &qv, const uview_1d<const Spack> &temp,
      ...
      Kokkos::TeamVectorRange(team, kmax - kmin + 1), [&](int pk_) {
        ...
        if (t_out_bounds.any()) {
          for (int s = 0; s < Spack::n; ++s) {
            trap = true;
            // printf ("** WARNING IN P3_MAIN -- src, gcol, lon, lat, lvl,
            // tstep, T: %d, %d, %13.6f, %13.6f, %d, %d, %13.6f\n"
        ...
        if (qv_out_bounds.any()) {
          for (int s = 0; s < Spack::n; ++s) {
            // trap = .true.  !note, tentatively no trap, since Qv could be
            // negative passed in to mp printf ("** WARNING IN P3_MAIN -- src,
            // gcol, lon, lat, lvl, tstep, Qv: %d, %d, %13.6f, %13.6f, %d, %d,
            // %13.6f\n"
