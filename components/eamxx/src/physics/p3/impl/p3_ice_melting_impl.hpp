...
KOKKOS_FUNCTION void Functions<S, D>::ice_melting(
    const Spack &rho, const Spack &T_atm, const Spack &pres,
  ...
  // need to add back accelerated melting due to collection of ice mass by rain
  // (pracsw1) note 'f1pr' values are normalized, so we need to multiply by N
  ...
  if (has_melt_qi.any()) {
    //    Note that qsat0 should be with respect to liquid. Confirmed F90 code
    //    did this.
    const auto qsat0 = physics::qv_sat_dry(
        Spack(Tmelt), pres, false, context, physics::MurphyKoop,
        "p3::ice_melting"); //"false" here means NOT saturation w/ respect to
                            //ice.
    ...
    // Reduce nj in proportion to decrease in qi mass. Prev line makes sure it
