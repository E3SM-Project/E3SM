...
KOKKOS_FUNCTION void Functions<S, D>::update_prognostic_ice(
    ...
    Spack &qinuc_cnt, Spack &th_atm, Spack &qv, Spack &qi, Spack &ni, Spack &qm,
    Spack &bm, Spack &qc, Spack &nc, Spack &qr, Spack &nr,
  ...
  if (use_hetfrz_classnuc) {
    qc.set(context, qc + (-qcheti_cnt - qicnt - qc2qi_collect_tend -
                          qc2qr_ice_shed_tend - qc2qi_berg_tend) *
                             dt);
  } else {
    qc.set(context, qc + (-qc2qi_hetero_freeze_tend - qc2qi_collect_tend -
                          qc2qr_ice_shed_tend - qc2qi_berg_tend) *
                             dt);
  }
  ...
  qr.set(context, qr + (-qr2qi_collect_tend + qi2qr_melt_tend -
                        qr2qi_immers_freeze_tend + qc2qr_ice_shed_tend) *
                           dt);
  ...
  nr.set(context,
         nr + (-nr_collect_tend - nr2ni_immers_freeze_tend +
               nmltratio * ni2nr_melt_tend + nr_ice_shed_tend + ncshdc) *
                  dt);
  ...
  if (qi_not_small.any()) {
    bm.set(qi_not_small,
           bm - ((qi2qv_sublim_tend + qi2qr_melt_tend) / qi) * dt * bm);
    qm.set(qi_not_small,
           qm - ((qi2qv_sublim_tend + qi2qr_melt_tend) * qm / qi) * dt);
    qi.set(qi_not_small, qi - (qi2qv_sublim_tend + qi2qr_melt_tend) * dt);
  }

  if (use_hetfrz_classnuc) {
    const auto dum = (qr2qi_collect_tend + qc2qi_collect_tend +
                      qr2qi_immers_freeze_tend + qcheti_cnt + qicnt) *
                     dt;
    qi.set(context, qi +
                        (qv2qi_vapdep_tend + qv2qi_nucleat_tend +
                         qc2qi_berg_tend + qinuc_cnt) *
                            dt +
                        dum);
    qm.set(context, qm + dum);
    bm.set(context, bm + (qr2qi_collect_tend * INV_RHO_RIMEMAX +
                          qc2qi_collect_tend / rho_qm_cloud +
                          (qr2qi_immers_freeze_tend + qcheti_cnt + qicnt) *
                              INV_RHO_RIMEMAX) *
                             dt);
    ni.set(context, ni + (ni_nucleat_tend - ni2nr_melt_tend - ni_sublim_tend -
                          ni_selfcollect_tend + nr2ni_immers_freeze_tend +
    ...
    qi.set(context,
           qi +
               (qv2qi_vapdep_tend + qv2qi_nucleat_tend + qc2qi_berg_tend) * dt +
               dum);
    qm.set(context, qm + dum);
    bm.set(context,
           bm + (qr2qi_collect_tend * INV_RHO_RIMEMAX +
                 qc2qi_collect_tend / rho_qm_cloud +
                 (qr2qi_immers_freeze_tend + qc2qi_hetero_freeze_tend) *
                     INV_RHO_RIMEMAX) *
                    dt);
  ...
  if (use_hetfrz_classnuc) {
    qv.set(context, qv + (-qv2qi_vapdep_tend + qi2qv_sublim_tend -
               ...
               th_atm + inv_exner *
                            ((qv2qi_vapdep_tend - qi2qv_sublim_tend +
                             ...
                             (qr2qi_collect_tend + qc2qi_collect_tend +
                              qcheti_cnt + qicnt + qr2qi_immers_freeze_tend -
                              qi2qr_melt_tend + qc2qi_berg_tend) *
                                 latice * INV_CP) *
                            dt);
  } else {
    qv.set(context,
           qv + (-qv2qi_vapdep_tend + qi2qv_sublim_tend - qv2qi_nucleat_tend) *
                    dt);
    th_atm.set(
        context,
        th_atm +
            inv_exner *
                ((qv2qi_vapdep_tend - qi2qv_sublim_tend + qv2qi_nucleat_tend) *
                     (latvap + latice) * INV_CP +
                 (qr2qi_collect_tend + qc2qi_collect_tend +
                  qc2qi_hetero_freeze_tend + qr2qi_immers_freeze_tend -
                  qi2qr_melt_tend + qc2qi_berg_tend) *
                     latice * INV_CP) *
                dt);
  }
...
KOKKOS_FUNCTION void Functions<S, D>::update_prognostic_liquid(
  ...
  qr.set(context,
         qr + (qc2qr_accret_tend + qc2qr_autoconv_tend - qr2qv_evap_tend) * dt);

  if (do_predict_nc || do_prescribed_CCN) {
    nc.set(context,
           nc + (-nc_accret_tend - nc2nr_autoconv_tend + nc_selfcollect_tend) *
                    dt);
  } else {
  ...
  if (IPARAM == 1 || IPARAM == 2) {
    nr.set(context, nr + (sp(0.5) * nc2nr_autoconv_tend - nr_selfcollect_tend -
                          nr_evap_tend) *
                             dt);
  } else {
  ...
  th_atm.set(context,
             th_atm + inv_exner * (-qr2qv_evap_tend * latvap * INV_CP) * dt);
}
