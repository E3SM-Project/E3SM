...
KOKKOS_FUNCTION void Functions<S, D>::ice_nucleation(
    const Spack &temp, const Spack &inv_rho, const Spack &ni,
    const Spack &ni_activated, const Spack &qv_supersat_i, const Scalar &inv_dt,
  ...
  const auto any_if_log =
      t_lt_T_icenuc && qv_supersat_i_ge_005 && (!do_log) && context;
  const auto any_if_not_log =
      t_lt_T_icenuc && qv_supersat_i_ge_005 && do_log && context;

  Spack dum{0.0}, N_nuc{0.0}, Q_nuc{0.0};

  if (any_if_not_log.any()) {
    dum = sp(0.005) * exp(sp(deposition_nucleation_exponent) * (tmelt - temp)) *
          sp(1.0e3) * inv_rho;
