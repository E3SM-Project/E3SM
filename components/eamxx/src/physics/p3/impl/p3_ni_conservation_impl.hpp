...
KOKKOS_FUNCTION void Functions<S, D>::ni_conservation(
    const Spack &ni, const Spack &ni_nucleat_tend,
    ...
    Spack &ni2nr_melt_tend, Spack &ni_sublim_tend, Spack &ni_selfcollect_tend,
    const bool &use_hetfrz_classnuc, const Smask &context) {
  const auto sink_ni =
      (ni2nr_melt_tend + ni_sublim_tend + ni_selfcollect_tend) * dt;

  Spack source_ni;
  if (use_hetfrz_classnuc) {
    source_ni = ni + (ni_nucleat_tend + nr2ni_immers_freeze_tend + ncheti_cnt +
                      nicnt + ninuc_cnt) *
                         dt;
  } else {
    source_ni = ni + (ni_nucleat_tend + nr2ni_immers_freeze_tend +
