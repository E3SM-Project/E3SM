...
KOKKOS_FUNCTION void Functions<S, D>::calc_rime_density(
    const Spack &T_atm, const Spack &rhofaci, const Spack &table_val_qi_fallspd,
    const Spack &acn, const Spack &lamc, const Spack &mu_c,
    const Spack &qc_incld, const Spack &qc2qi_collect_tend, Spack &vtrmi1,
    Spack &rho_qm_cloud, const Smask &context) {
  constexpr Scalar qsmall     = C::QSMALL;
  constexpr Scalar T_zerodegc = C::T_zerodegc;
  constexpr Scalar bcn        = C::bcn;

  const auto qc2qi_collect_tend_not_small_and_t_freezing =
      (qc2qi_collect_tend >= qsmall) && (T_atm < T_zerodegc) && context;
  ...
  if (qc2qi_collect_tend_not_small_and_t_freezing.any()) {
    ...
    vtrmi1.set(qc2qi_collect_tend_not_small_and_t_freezing,
               table_val_qi_fallspd * rhofaci);
    ...
    if (qc2qi_collect_tend_and_qc_not_small_and_t_freezing.any()) {
      ...
      Spack Vt_qc =
          acn * tgamma(4 + bcn + mu_c) / (pow(lamc, bcn) * tgamma(4 + mu_c));
      ...
      rho_qm_cloud.set(
          qc2qi_collect_tend_and_qc_not_small_and_t_freezing and Ri_le_8,
      ...
      rho_qm_cloud.set(qc2qi_collect_tend_and_qc_not_small_and_t_freezing and
                           !Ri_le_8,
    ...
    const auto qc2qi_collect_tend_not_small_and_qc_small_and_t_freezing =
        qc2qi_collect_tend_not_small_and_t_freezing && (qc_incld < qsmall);
    rho_qm_cloud.set(qc2qi_collect_tend_not_small_and_qc_small_and_t_freezing,
                     400);
  }

  // Handle the cases we haven't handled above.
  vtrmi1.set(!qc2qi_collect_tend_not_small_and_t_freezing && context,
             0); // no velocity if no ice
  rho_qm_cloud.set(!qc2qi_collect_tend_not_small_and_t_freezing && context,
                   400);
}
