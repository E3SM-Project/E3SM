...
KOKKOS_FUNCTION void Functions<S, D>::p3_main_part2(
    const MemberType &team, const Int &nk_pack, const Scalar &max_total_ni,
    const bool &predictNc, const bool &do_prescribed_CCN, const Scalar &dt,
    const Scalar &inv_dt,
    ...
    const uview_1d<Spack> &prctot, bool &hydrometeorsPresent, const Int &nk,
  ...
  const bool use_separate_ice_liq_frac =
      runtime_options.use_separate_ice_liq_frac;
  ...
  Kokkos::parallel_for(Kokkos::TeamVectorRange(team, nk_pack), [&](Int k) {
    // compute mask to identify padded values in packs, which shouldn't be used
    // in calculations
    const auto range_pack = ekat::range<IntSmallPack>(k * Spack::n);
    const auto range_mask = range_pack < nk;

    // if relatively dry and no hydrometeors at this level, skip to end of
    // k-loop (i.e. skip this level)
    const auto skip_all =
        (!range_mask || (qc(k) < qsmall && qr(k) < qsmall && qi(k) < qsmall &&
    ...
    // All microphysics tendencies will be computed as IN-CLOUD, they will be
    // mapped back to cell-average later.
    ...
    Spack
        ...
        nc_accret_tend(
            0), // change in cloud droplet number from accretion by rain
        nc_selfcollect_tend(0), // change in cloud droplet number from
                                // self-collection  (Not in paper?)
        nc2nr_autoconv_tend(
            0), // change in cloud droplet number from autoconversion
        nr_selfcollect_tend(
            0), // change in rain number from self-collection  (Not in paper?)
        ...
        nr_ice_shed_tend(0),         // source for rain number from collision of
                                     // rain/ice above freezing and shedding
        qc2qi_hetero_freeze_tend(0), // immersion freezing droplets
        qr2qi_collect_tend(0),       // collection rain mass by ice
        qc2qr_ice_shed_tend(
            0), // source for rain mass due to cloud water/ice collision above
        ...
        nc_collect_tend(
            0), // change in cloud droplet number from collection by ice
        ncshdc(
            0), // source for rain number due to cloud water/ice collision above
                // freezing  and shedding (combined with NRSHD in the paper)
        nc2ni_immers_freeze_tend(0), // immersion freezing droplets
        nr_collect_tend(0),     // change in rain number from collection by ice
        ni_selfcollect_tend(0), // change in ice number from collection within a
                                // category (Not in paper?)
        ni_nucleat_tend(
            0), // change in ice number from deposition/cond-freezing nucleation
        ...
        ncheti_cnt(0), qcheti_cnt(0), nicnt(0), qicnt(0), ninuc_cnt(0),
        qinuc_cnt(0),

        // quantities related to process rates/parameters, interpolated from
        // lookup tables: For a more in depth reference to where these came from
        // consult the file "create_p3_lookupTable_1.F90-v4.1".  All line
        // numbers below reference this file.
        table_val_qi_fallspd(0), // mass-weighted fallspeed              See
                                 // lines  731 -  808  ums
        table_val_ni_self_collect(0), // ice collection within a category See
                                      // lines  809 -  928  nagg
        table_val_qc2qi_collect(0),  // collection of cloud water by ice     See
                                     // lines  929 - 1009  nrwat
        table_val_qi2qr_melting(0),  // melting                              See
                                     // lines 1212 - 1279  vdep
        table_val_ice_eff_radius(0), // effective radius                     See
                                     // lines 1281 - 1356  eff
        table_val_nr_collect(0),     // collection of rain number by ice     See
                                     // lines 1010 - 1209  nrrain
        table_val_qr2qi_collect(0),  // collection of rain mass by ice       See
                                     // lines 1010 - 1209  qrrain
        table_val_ni_lammax(0),      // minimum ice number (lambda limiter)  See
                                     // lines  704 -  705 nlarge
        table_val_ni_lammin(0),      // maximum ice number (lambda limiter)  See
                                     // lines  704 -  705 nsmall
        table_val_ice_reflectivity(0), // reflectivity See lines  731 -  808
                                       // refl
        table_val_qi2qr_vent_melt(0), // melting (ventilation term) See lines
                                      // 1212 - 1279  vdep1
        table_val_ice_mean_diam(0), // mass-weighted mean diameter          See
                                    // lines 1212 - 1279  dmm
        table_val_ice_bulk_dens(0), // mass-weighted mean particle density  See
                                    // lines 1212 - 1279  rhomm
        ...
        epsr(0),           // TODO(doc)
        epsc(0),           // TODO(doc)
        epsi_tot(0);       // inverse supersaturation relaxation timescale for
                           // combined ice categories

    Smask wetgrowth(false);

    // skip micro process calculations except nucleation/acvtivation if there no
    // hydrometeors are present
    const auto skip_micro =
        skip_all || !(qc_incld(k) >= qsmall || qr_incld(k) >= qsmall ||
                      qi_incld(k) >= qsmall);
    const auto not_skip_micro = !skip_micro;

    if (not_skip_micro.any()) {
      // time/space varying physical variables
      get_time_space_phys_variables(T_atm(k), pres(k), rho(k), qv_sat_l(k),
                                    qv_sat_i(k), mu, dv, sc, dqsdt, dqsidt, ab,
      ...
      get_rain_dsd2(qr_incld(k), nr_incld(k), mu_r(k), lamr(k), runtime_options,
                    not_skip_micro);
      get_cdistr_logn0r(qr_incld(k), nr_incld(k), mu_r(k), lamr(k), cdistr(k),
                        logn0r(k), not_skip_micro);
      nr(k).set(not_skip_micro, nr_incld(k) * cld_frac_r(k));

      impose_max_total_ni(ni_incld(k), max_total_ni, inv_rho(k),
                          not_skip_micro);
      ...
      if (qi_gt_small.any()) {
        ...
        table_val_nr_collect.set(
            qr_gt_small, apply_table_coll(0, collect_table_vals, table_ice,
                                          table_rain, qi_gt_small));
        table_val_qr2qi_collect.set(
            qr_gt_small, apply_table_coll(1, collect_table_vals, table_ice,
                                          table_rain, qi_gt_small));

        // adjust Ni if needed to make sure mean size is in bounds (i.e. apply
        // lambda limiters) note that the Nmax and Nmin are normalized and thus
        // need to be multiplied by existing N
        ni_incld(k).set(qi_gt_small,
                        min(ni_incld(k), table_val_ni_lammax * ni_incld(k)));
        ni_incld(k).set(qi_gt_small,
                        max(ni_incld(k), table_val_ni_lammin * ni_incld(k)));
      }
      ...
      if (do_ice_production) {
        ...
        if (use_hetfrz_classnuc) {
          ice_classical_nucleation(hetfrz_immersion_nucleation_tend(k),
                                   hetfrz_contact_nucleation_tend(k),
                                   hetfrz_deposition_nucleation_tend(k), rho(k),
                                   qc_incld(k), nc_incld(k), 1, ncheti_cnt,
                                   qcheti_cnt, nicnt, qicnt, ninuc_cnt,
                                   qinuc_cnt);
          ice_classical_nucleation(hetfrz_immersion_nucleation_tend(k),
                                   hetfrz_contact_nucleation_tend(k),
                                   hetfrz_deposition_nucleation_tend(k), rho(k),
                                   qc_incld(k), nc_incld(k), 2, ncheti_cnt,
                                   qcheti_cnt, nicnt, qicnt, ninuc_cnt,
                                   qinuc_cnt);
        } else {
      ...
      calc_liq_relaxation_timescale(revap_table_vals, rho(k), f1r, f2r, dv, mu,
                                    sc, mu_r(k), lamr(k), cdistr(k), cdist(k),
                                    qr_incld(k), qc_incld(k), epsr, epsc,
                                    not_skip_micro);

      evaporate_rain(qr_incld(k), qc_incld(k), nr_incld(k), qi_incld(k),
                     cld_frac_l(k), cld_frac_r(k), qv(k), qv_prev(k),
                     qv_sat_l(k), qv_sat_i(k), ab, abi, epsr, epsi_tot,
                     T_atm(k), t_prev(k), dqsdt, dt, qr2qv_evap_tend,
                     nr_evap_tend, not_skip_micro);
    ...
    cloud_water_autoconversion(
        rho(k), qc_incld(k), nc_incld(k), inv_qc_relvar(k), qc2qr_autoconv_tend,
        nc2nr_autoconv_tend, ncautr, runtime_options, not_skip_all);

    // self-collection of droplets
    droplet_self_collection(rho(k), inv_rho(k), qc_incld(k), mu_c(k), nu(k),
                            nc2nr_autoconv_tend, nc_selfcollect_tend,
                            not_skip_all);

    // accretion of cloud by rain
    cloud_rain_accretion(rho(k), inv_rho(k), qc_incld(k), nc_incld(k),
                         qr_incld(k), inv_qc_relvar(k), qc2qr_accret_tend,
                         nc_accret_tend, runtime_options, not_skip_all);

    // self-collection and breakup of rain
    // (breakup following modified Verlinde and Cotton scheme)
    rain_self_collection(rho(k), qr_incld(k), nr_incld(k), nr_selfcollect_tend,
                         runtime_options, not_skip_all);

    // Here we map the microphysics tendency rates back to CELL-AVERAGE
    // quantities for updating cell-average quantities.
    back_to_cell_average(
        cld_frac_l(k), cld_frac_r(k), cld_frac_i(k), qc2qr_accret_tend,
        qr2qv_evap_tend, qc2qr_autoconv_tend, nc_accret_tend,
        nc_selfcollect_tend, nc2nr_autoconv_tend, nr_selfcollect_tend,
        nr_evap_tend, ncautr, qi2qv_sublim_tend, nr_ice_shed_tend,
        ...
        ni_nucleat_tend, qc2qi_berg_tend, ncheti_cnt, qcheti_cnt, nicnt, qicnt,
        ninuc_cnt, qinuc_cnt, not_skip_all, runtime_options);

    //
    // conservation of water
    //

    // don't bother checking vapor since all sinks already have limits imposed
    // and the sum, therefore, cannot possibly overdeplete qv [PMC: the above
    // argument makes no sense to me. I think we don't check qv because it is
    // typically much greater than zero so seldom goes negative (and if it does
    // catastrophic failure is appropriate)]

    if (use_separate_ice_liq_frac) {
      // cloud
      cloud_water_conservation(
          qc(k), dt, qc2qr_autoconv_tend, qc2qr_accret_tend, qc2qi_collect_tend,
          qc2qi_hetero_freeze_tend, qc2qr_ice_shed_tend, qc2qi_berg_tend,
          qi2qv_sublim_tend, qv2qi_vapdep_tend, qcheti_cnt, qicnt,
          use_hetfrz_classnuc, not_skip_all, cld_frac_l(k), cld_frac_i(k),
          runtime_options);
    } else {
      // cloud
      cloud_water_conservation(
    ...
    // rain
    rain_water_conservation(qr(k), qc2qr_autoconv_tend, qc2qr_accret_tend,
                            qi2qr_melt_tend, qc2qr_ice_shed_tend, dt,
                            qr2qv_evap_tend, qr2qi_collect_tend,
                            qr2qi_immers_freeze_tend, not_skip_all);

    // ice
    ice_water_conservation(
        qi(k), qv2qi_vapdep_tend, qv2qi_nucleat_tend, qc2qi_berg_tend,
    ...
    nc_conservation(nc(k), nc_selfcollect_tend, dt, nc_collect_tend,
                    nc2ni_immers_freeze_tend, nc_accret_tend,
                    nc2nr_autoconv_tend, ncheti_cnt, nicnt, use_hetfrz_classnuc,
                    not_skip_all);
    nr_conservation(nr(k), ni2nr_melt_tend, nr_ice_shed_tend, ncshdc,
                    nc2nr_autoconv_tend, dt, nmltratio, nr_collect_tend,
                    nr2ni_immers_freeze_tend, nr_selfcollect_tend, nr_evap_tend,
                    not_skip_all);
    ni_conservation(ni(k), ni_nucleat_tend, nr2ni_immers_freeze_tend,
                    nc2ni_immers_freeze_tend, ncheti_cnt, nicnt, ninuc_cnt, dt,
    ...
    ice_supersat_conservation(
        qv2qi_vapdep_tend, qv2qi_nucleat_tend, qinuc_cnt, cld_frac_i(k), qv(k),
        qv_sat_i(k), th_atm(k) / inv_exner(k), dt, qi2qv_sublim_tend,
        qr2qv_evap_tend, use_hetfrz_classnuc, not_skip_all);
    // make sure procs don't inappropriately push qv beyond liquid saturation
    prevent_liq_supersaturation(pres(k), T_atm(k), qv(k), dt, qv2qi_vapdep_tend,
                                qv2qi_nucleat_tend, qi2qv_sublim_tend,
    ...
    update_prognostic_ice(
        ...
        ni_selfcollect_tend, ni_sublim_tend, qc2qi_berg_tend, inv_exner(k),
        predictNc, wetgrowth, dt, nmltratio, rho_qm_cloud, ncheti_cnt, nicnt,
        ninuc_cnt, qcheti_cnt, qicnt, qinuc_cnt, th_atm(k), qv(k), qi(k), ni(k),
        qm(k), bm(k), qc(k), nc(k), qr(k), nr(k), use_hetfrz_classnuc,
        not_skip_all);

    //-- warm-phase only processes:
    update_prognostic_liquid(qc2qr_accret_tend, nc_accret_tend,
                             qc2qr_autoconv_tend, nc2nr_autoconv_tend, ncautr,
                             nc_selfcollect_tend, qr2qv_evap_tend, nr_evap_tend,
                             nr_selfcollect_tend, predictNc, do_prescribed_CCN,
                             inv_rho(k), inv_exner(k), dt, th_atm(k), qv(k),
                             qc(k), nc(k), qr(k), nr(k), not_skip_all);

    // AaronDonahue - Add extra variables needed from microphysics by E3SM:
    if (use_hetfrz_classnuc) {
      qv2qi_depos_tend(k).set(not_skip_all, qv2qi_vapdep_tend -
                                                qi2qv_sublim_tend +
    ...
    precip_total_tend(k).set(not_skip_all,
                             qc2qr_accret_tend + qc2qr_autoconv_tend +
    ...
    vap_ice_exchange(k).set(not_skip_all, qv2qi_vapdep_tend -
                                              qi2qv_sublim_tend +
                                              qv2qi_nucleat_tend);
    vap_liq_exchange(k).set(not_skip_all, -qr2qv_evap_tend);
    liq_ice_exchange(k).set(
        not_skip_all, qc2qi_hetero_freeze_tend + qr2qi_immers_freeze_tend -
    ...
    th_atm(k).set(qi_small, th_atm(k) - inv_exner(k) * qi(k) *
    ...
    pratot(k).set(not_skip_all,
                  qc2qr_accret_tend); // cloud drop accretion by rain
    prctot(k).set(not_skip_all,
                  qc2qr_autoconv_tend); // cloud drop autoconversion to rain

    // impose_max_total_ni is meant to operate on in-cloud vals. ni_incld is an
    // output of calculate_incloud_mixingratios below but we need to generate it
    // earlier for impose_max_total_ni
    ...
    calculate_incloud_mixingratios(
        ...
        inv_cld_frac_l(k), inv_cld_frac_i(k), inv_cld_frac_r(k), qc_incld(k),
        qr_incld(k), qi_incld(k), qm_incld(k), nc_incld(k), nr_incld(k),
        ni_incld(k), bm_incld(k), not_skip_all);
  });
  team.team_barrier();
}
