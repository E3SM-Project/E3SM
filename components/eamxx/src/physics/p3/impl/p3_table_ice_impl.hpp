...
void Functions<S, D>::get_global_ice_lookup_tables(view_ice_table &ice_table_vals,
...
template <typename S, typename D>
KOKKOS_FUNCTION void Functions<S, D>::lookup_rain(const Spack &qr, const Spack &nr, TableRain &tab,
                                                  const Smask &context) {
  if (!context.any())
    return;
