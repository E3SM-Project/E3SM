...
void Functions<S, D>::get_global_ice_lookup_tables(
...
Functions<S, D>::lookup_ice(const Spack &qi, const Spack &ni, const Spack &qm,
  ...
  // set limits (to make sure the calculated index doesn't exceed range of
  // lookup table)
...
template <typename S, typename D>
KOKKOS_FUNCTION void
Functions<S, D>::lookup_rain(const Spack &qr, const Spack &nr, TableRain &tab,
                             const Smask &context) {
  if (!context.any())
    return;

  // find index for scaled mean rain size
  // if no rain, then just choose dumj = 1 and do not calculate rain-ice
...
Functions<S, D>::apply_table_ice(const int &idx,
  ...
  auto gproc1 =
      ...
      (tab.dum1 - Spack(tab.dumi) - 1) *
          (index(ice_table_vals, tab.dumjj, tab.dumii + 1, tab.dumi + 1,
                 idxpk) -
           index(ice_table_vals, tab.dumjj, tab.dumii + 1, tab.dumi, idxpk));

  const auto tmp1 =
      iproc1 + (tab.dum4 - Spack(tab.dumii) - 1) * (gproc1 - iproc1);
  ...
  iproc1 =
      ...
      (tab.dum1 - Spack(tab.dumi) - 1) *
          (index(ice_table_vals, tab.dumjj + 1, tab.dumii, tab.dumi + 1,
                 idxpk) -
           index(ice_table_vals, tab.dumjj + 1, tab.dumii, tab.dumi, idxpk));
  ...
  gproc1 =
      ...
      (tab.dum1 - Spack(tab.dumi) - 1) *
          (index(ice_table_vals, tab.dumjj + 1, tab.dumii + 1, tab.dumi + 1,
                 idxpk) -
           index(ice_table_vals, tab.dumjj + 1, tab.dumii + 1, tab.dumi,
                 idxpk));
...
template <typename S, typename D>
KOKKOS_FUNCTION typename Functions<S, D>::Spack
Functions<S, D>::apply_table_coll(const int &idx,
  ...
  auto dproc1 =
      ...
      (ti.dum1 - Spack(ti.dumi) - 1) *
          (index(collect_table_vals, ti.dumjj, ti.dumii, ti.dumi + 1, tr.dumj,
                 idxpk) -
           index(collect_table_vals, ti.dumjj, ti.dumii, ti.dumi, tr.dumj,
                 idxpk));

  auto dproc2 = index(collect_table_vals, ti.dumjj, ti.dumii, ti.dumi,
                      tr.dumj + 1, idxpk) +
                (ti.dum1 - Spack(ti.dumi) - 1) *
                    (index(collect_table_vals, ti.dumjj, ti.dumii, ti.dumi + 1,
                           tr.dumj + 1, idxpk) -
                     index(collect_table_vals, ti.dumjj, ti.dumii, ti.dumi,
                           tr.dumj + 1, idxpk));
  ...
  dproc1 = index(collect_table_vals, ti.dumjj, ti.dumii + 1, ti.dumi, tr.dumj,
                 idxpk) +
           (ti.dum1 - Spack(ti.dumi) - 1) *
               (index(collect_table_vals, ti.dumjj, ti.dumii + 1, ti.dumi + 1,
                      tr.dumj, idxpk) -
                index(collect_table_vals, ti.dumjj, ti.dumii + 1, ti.dumi,
                      tr.dumj, idxpk));

  dproc2 = index(collect_table_vals, ti.dumjj, ti.dumii + 1, ti.dumi,
                 tr.dumj + 1, idxpk) +
           (ti.dum1 - Spack(ti.dumi) - 1) *
               (index(collect_table_vals, ti.dumjj, ti.dumii + 1, ti.dumi + 1,
                      tr.dumj + 1, idxpk) -
  ...
  const auto tmp1 =
      iproc1 + (ti.dum4 - Spack(ti.dumii) - 1) * (gproc1 - iproc1);

  // density index + 1

  // current rime fraction index

  dproc1 = index(collect_table_vals, ti.dumjj + 1, ti.dumii, ti.dumi, tr.dumj,
                 idxpk) +
           (ti.dum1 - Spack(ti.dumi) - 1) *
               (index(collect_table_vals, ti.dumjj + 1, ti.dumii, ti.dumi + 1,
                      tr.dumj, idxpk) -
                index(collect_table_vals, ti.dumjj + 1, ti.dumii, ti.dumi,
                      tr.dumj, idxpk));

  dproc2 = index(collect_table_vals, ti.dumjj + 1, ti.dumii, ti.dumi,
                 tr.dumj + 1, idxpk) +
           (ti.dum1 - Spack(ti.dumi) - 1) *
               (index(collect_table_vals, ti.dumjj + 1, ti.dumii, ti.dumi + 1,
                      tr.dumj + 1, idxpk) -
  ...
  dproc1 = index(collect_table_vals, ti.dumjj + 1, ti.dumii + 1, ti.dumi,
                 tr.dumj, idxpk) +
           (ti.dum1 - Spack(ti.dumi) - 1) *
               (index(collect_table_vals, ti.dumjj + 1, ti.dumii + 1,
                      ti.dumi + 1, tr.dumj, idxpk) -
                index(collect_table_vals, ti.dumjj + 1, ti.dumii + 1, ti.dumi,
                      tr.dumj, idxpk));

  dproc2 = index(collect_table_vals, ti.dumjj + 1, ti.dumii + 1, ti.dumi,
                 tr.dumj + 1, idxpk) +
           (ti.dum1 - Spack(ti.dumi) - 1) *
               (index(collect_table_vals, ti.dumjj + 1, ti.dumii + 1,
                      ti.dumi + 1, tr.dumj + 1, idxpk) -
                index(collect_table_vals, ti.dumjj + 1, ti.dumii + 1, ti.dumi,
                      tr.dumj + 1, idxpk));
