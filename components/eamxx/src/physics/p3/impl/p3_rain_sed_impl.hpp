...
KOKKOS_FUNCTION void Functions<S, D>::rain_sedimentation(
  ...
  if (log_qxpresent) {
    ...
    while (dt_left > C::dt_left_tol) {
      ...
      Kokkos::parallel_for(Kokkos::TeamVectorRange(team, V_qr.extent(0)), [&](Int k) {
        V_qr(k) = 0;
        V_nr(k) = 0;
      });
      team.team_barrier();
      ...
      Kokkos::parallel_reduce(
          ...
          [&](int pk_, Scalar &lmax) {
            ...
            const auto Co_max_local = max(qr_gt_small, 0, V_qr(pk) * dt_left * inv_dz(pk));
      ...
      Kokkos::parallel_for(Kokkos::TeamVectorRange(team, qr.extent(0)), [&](int pk) {
      ...
      Kokkos::parallel_for(Kokkos::TeamVectorRange(team, kmax - kmin + 1), [&](int pk_) {
        const int pk          = kmin + pk_;
        const auto range_pack = ekat::range<IntSmallPack>(pk * Spack::n);
        const auto range_mask = range_pack >= kmin_scalar && range_pack <= kmax_scalar;
  ...
  workspace.template release_many_contiguous<4>({&V_qr, &V_nr, &flux_qx, &flux_nx});
}
