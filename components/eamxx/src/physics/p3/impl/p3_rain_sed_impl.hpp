...
/*
 * Implementation of p3 rain sedimentation functions. Clients should NOT
...
KOKKOS_FUNCTION void Functions<S, D>::rain_sedimentation(
  ...
  const view_1d_ptr_array<Spack, 2> fluxes_ptr = {&flux_qx, &flux_nx},
                                    vs_ptr     = {&V_qr, &V_nr},
                                    qnr_ptr    = {&qr, &nr};
  ...
  bool log_qxpresent;
  const Int k_qxtop =
      find_top(team, sqr, qsmall, kbot, ktop, kdir, log_qxpresent);
  ...
  if (log_qxpresent) {
    ...
    // find bottom
    Int k_qxbot =
        find_bottom(team, sqr, qsmall, kbot, k_qxtop, kdir, log_qxpresent);
    ...
    while (dt_left > C::dt_left_tol) {
      ...
      Kokkos::parallel_for(Kokkos::TeamVectorRange(team, V_qr.extent(0)),
                           [&](Int k) {
                             V_qr(k) = 0;
                             V_nr(k) = 0;
                           });
          ...
          [&](int pk_, Scalar &lmax) {
            ...
            if (qr_gt_small.any()) {
              compute_rain_fall_velocity(
                  vn_table_vals, vm_table_vals, qr_incld(pk), rhofacr(pk),
                  nr_incld(pk), mu_r(pk), lamr(pk), V_qr(pk), V_nr(pk),
                  runtime_options, qr_gt_small);

              // in compute_rain_fall_velocity, get_rain_dsd2 keeps the
              // drop-size distribution within reasonable bounds by modifying
              // nr_incld. The next line maintains consistency between nr_incld
              // and nr
              nr(pk).set(qr_gt_small, nr_incld(pk) * cld_frac_r(pk));
            }
            const auto Co_max_local =
                max(qr_gt_small, 0, V_qr(pk) * dt_left * inv_dz(pk));
      ...
      generalized_sedimentation<2>(rho, inv_rho, inv_dz, team, nk, k_qxtop,
      ...
      Kokkos::parallel_for(Kokkos::TeamVectorRange(team, qr.extent(0)),
                           [&](int pk) {
      ...
      Kokkos::parallel_for(
          Kokkos::TeamVectorRange(team, kmax - kmin + 1), [&](int pk_) {
            ...
            precip_liq_flux(pk).set(range_mask,
                                    precip_liq_flux(pk) + flux_qx_pk);
          });
    }
  ...
  Kokkos::parallel_for(Kokkos::TeamVectorRange(team, nk_pack), [&](int pk) {
    qr_tend(pk) =
        (qr(pk) - qr_tend(pk)) * inv_dt; // Rain sedimentation tendency, measure
    nr_tend(pk) = (nr(pk) - nr_tend(pk)) *
                  inv_dt; // Rain # sedimentation tendency, measure
  });
