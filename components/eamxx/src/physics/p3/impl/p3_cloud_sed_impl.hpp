...
KOKKOS_FUNCTION void Functions<S, D>::cloud_sedimentation(
  ...
  if (log_qxpresent) {
    ...
    while (dt_left > C::dt_left_tol) {
      ...
      Kokkos::parallel_for(Kokkos::TeamVectorRange(team, V_qc.extent(0)), [&](Int k) {
        V_qc(k) = 0;
        if (do_predict_nc) {
          V_nc(k) = 0;
        }
      ...
      Kokkos::parallel_reduce(
          ...
          [&](int pk_, Scalar &lmax) {
            ...
            const auto Co_max_local = max(qc_gt_small, 0, V_qc(pk) * dt_left * inv_dz(pk));
      ...
      Kokkos::parallel_for(Kokkos::TeamVectorRange(team, qc.extent(0)), [&](int pk) {
  ...
  workspace.template release_many_contiguous<4>({&V_qc, &V_nc, &flux_qx, &flux_nx});
}
