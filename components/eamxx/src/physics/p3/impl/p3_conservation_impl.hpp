...
KOKKOS_FUNCTION void Functions<S, D>::cloud_water_conservation(
    ...
    const Smask &context, const Spack &cld_frac_l, const Spack &cld_frac_i,
    const P3Runtime &runtime_options) {

  Spack sinks;
  if (use_hetfrz_classnuc) {
    sinks = (qc2qr_autoconv_tend + qc2qr_accret_tend + qc2qi_collect_tend +
             qcheti_cnt + qc2qr_ice_shed_tend + qc2qi_berg_tend) *
  ...
  Smask enforce_conservation =
      sinks > sources && sinks >= qtendsmall &&
  ...
  if (enforce_conservation.any()) {
    ...
    if (use_hetfrz_classnuc) {
      ...
      qc2qi_hetero_freeze_tend.set(enforce_conservation,
                                   qc2qi_hetero_freeze_tend * ratio);
    }
  ...
  if (nothing_todo.any()) {
    ratio.set(nothing_todo,
              1); // If not limiting sinks on qc then most likely did not run
                  // out of qc
  }
  ...
  //  vapor depositional growth rate that takes place within the mixed phase
  //  cloud after qc is depleted
  ...
  if (enforce_conservation.any()) {
    if (runtime_options.use_separate_ice_liq_frac) {
      qv2qi_vapdep_tend.set(enforce_conservation,
                            qv2qi_vapdep_tend +
                                qv2qi_vapdep_tend * (1 - ratio) *
                                    (il_cldm / cld_frac_glaciated));
      qi2qv_sublim_tend.set(enforce_conservation,
                            qi2qv_sublim_tend +
                                qi2qv_sublim_tend * (1 - ratio) *
                                    (il_cldm / cld_frac_glaciated));
    } else {
      qv2qi_vapdep_tend.set(enforce_conservation,
                            qv2qi_vapdep_tend * (1 - ratio));
...
KOKKOS_FUNCTION void Functions<S, D>::rain_water_conservation(
  ...
  const auto sinks =
      ...
      dt; // Sinks of rain water
  const auto sources = qr + (qc2qr_autoconv_tend + qc2qr_accret_tend +
                             qi2qr_melt_tend + qc2qr_ice_shed_tend) *
  ...
  if (enforce_conservation.any()) {
    ...
    qr2qi_immers_freeze_tend.set(enforce_conservation,
                                 qr2qi_immers_freeze_tend * ratio);
  }
}
...
KOKKOS_FUNCTION void Functions<S, D>::ice_water_conservation(
    ...
    const Smask &context) {
  const auto sinks =
      (qi2qv_sublim_tend + qi2qr_melt_tend) * dt; // Sinks of ice water

  Spack sources;
  if (use_hetfrz_classnuc) {
    sources =
        qi + (qv2qi_vapdep_tend + qv2qi_nucleat_tend + qr2qi_collect_tend +
              qc2qi_collect_tend + qr2qi_immers_freeze_tend + qc2qi_berg_tend +
    ...
    sources =
        qi + (qv2qi_vapdep_tend + qv2qi_nucleat_tend + qr2qi_collect_tend +
              ...
              qc2qi_hetero_freeze_tend + qc2qi_berg_tend) *
                 dt; // Sources of ice water
  }
  Spack ratio;
  constexpr Scalar qtendsmall = C::QTENDSMALL;
