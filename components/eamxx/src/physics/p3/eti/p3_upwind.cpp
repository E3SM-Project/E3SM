...
#define ETI_UPWIND(nfield)                                                    \
  template void                                                               \
  Functions<Real, DefaultDevice>::calc_first_order_upwind_step<nfield>(       \
      ...
      const Scalar &dt_sub, const view_1d_ptr_array<Spack, nfield> &flux,     \
...
#define ETI_GENSED(nfield)                                                    \
  template void                                                               \
  Functions<Real, DefaultDevice>::generalized_sedimentation<nfield>(          \
      const uview_1d<const Spack> &rho, const uview_1d<const Spack> &inv_rho, \
      const uview_1d<const Spack> &inv_dz, const MemberType &team,            \
      const Int &nk, const Int &k_qxtop, Int &k_qxbot, const Int &kbot,       \
      const Int &kdir, const Scalar &Co_max, Scalar &dt_left,                 \
      Scalar &prt_accum, const view_1d_ptr_array<Spack, nfield> &flux,        \
      const view_1d_ptr_array<Spack, nfield> &V,                              \
      const view_1d_ptr_array<Spack, nfield> &r);
ETI_GENSED(1)
ETI_GENSED(2)
