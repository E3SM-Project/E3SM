...
Cosp::Cosp(const ekat::Comm &comm, const ekat::ParameterList &params)
  ...
  m_cosp_frequency_units =
      m_params.get<std::string>("cosp_frequency_units", "steps");
  EKAT_REQUIRE_MSG((m_cosp_frequency_units == "steps") ||
                       (m_cosp_frequency_units == "hours"),
                   "cosp_frequency_units " + m_cosp_frequency_units +
                       " not supported");
...
void Cosp::set_grids(const std::shared_ptr<const GridsManager> grids_manager) {
  ...
  add_field<Required>("dtau067", scalar3d_mid, nondim,
                      grid_name); // 0.67 micron optical depth
  add_field<Required>("dtau105", scalar3d_mid, nondim,
                      grid_name); // 10.5 micron optical depth
  // Effective radii, should be computed in either microphysics or radiation
  // interface
  // TODO: should these be meters or microns? Was meters before, but using "m"
  // instead of "micron" seemed to cause prim_model_finalize to throw error with
  // the following: ABORTING WITH ERROR: Error! prim_init_model_f90 was not
  // called yet (or prim_finalize_f90 was already called). P3 defines this field
  // with micron instead of meters units, so is this a unit conversion issue?
...
void Cosp::initialize_impl(const RunType /* run_type */) {
  ...
  // Add note to output files about processing ISCCP fields that are only valid
  // during daytime. This can go away once I/O can handle masked time averages.
  using stratts_t               = std::map<std::string, std::string>;
  std::list<std::string> vnames = {"isccp_cldtot", "isccp_ctptau",
                                   "modis_ctptau", "misr_cthtau"};
  for (const auto field_name :
    ...
    atts["note"] =
        "Night values are zero; divide by cosp_sunlit to get daytime mean";
  }
}
...
void Cosp::run_impl(const double dt) {
  ...
  if (m_cosp_frequency_units == "steps") {
    ...
    EKAT_ERROR_MSG("cosp_frequency_units " + m_cosp_frequency_units +
                   " not supported");
  }
  ...
  auto update_cosp =
      cosp_do(cosp_freq_in_steps, end_of_step_ts().get_num_steps());

  // Call COSP wrapper routines
  if (update_cosp) {
    // Get fields from field manager; note that we get host views because this
    // interface serves primarily as a wrapper to a c++ to f90 bridge for the
    // COSP all then need to be copied to layoutLeft views to permute the
    // indices for F90.
    ...
    const auto scan_policy =
        ESU::get_thread_range_parallel_scan_team_policy(ncol, nlev);
    const auto g = physics::Constants<Real>::gravit;
    Kokkos::parallel_for(
        scan_policy, KOKKOS_LAMBDA(const KT::MemberType &team) {
          ...
          PF::calculate_dz(team, pseudo_density_s, p_mid_s, T_mid_s, qv_s,
                           dz_s);
          team.team_barrier();
    ...
    const auto skt_h =
        get_field_in("surf_radiative_T").get_view<const Real *, Host>();
    const auto p_int_h = get_field_in("p_int").get_view<const Real **, Host>();
    const auto cldfrac_h =
        get_field_in("cldfrac_rad").get_view<const Real **, Host>();
    const auto reff_qc_h =
        get_field_in("eff_radius_qc").get_view<const Real **, Host>();
    const auto reff_qi_h =
        get_field_in("eff_radius_qi").get_view<const Real **, Host>();
    const auto dtau067_h =
        get_field_in("dtau067").get_view<const Real **, Host>();
    const auto dtau105_h =
        get_field_in("dtau105").get_view<const Real **, Host>();

    auto isccp_cldtot_h =
        get_field_out("isccp_cldtot").get_view<Real *, Host>();
    auto isccp_ctptau_h =
        get_field_out("isccp_ctptau").get_view<Real ***, Host>();
    auto modis_ctptau_h =
        get_field_out("modis_ctptau").get_view<Real ***, Host>();
    auto misr_cthtau_h =
        get_field_out("misr_cthtau").get_view<Real ***, Host>();
    auto cosp_sunlit_h =
        get_field_out("cosp_sunlit")
            .get_view<Real *, Host>(); // Copy of sunlit flag with COSP
                                       // frequency for proper averaging

    Real emsfc_lw = 0.99;
    Kokkos::deep_copy(cosp_sunlit_h, sunlit_h);
    CospFunc::main(m_num_cols, m_num_subcols, m_num_levs, m_num_tau, m_num_ctp,
                   m_num_cth, emsfc_lw, sunlit_h, skt_h, T_mid_h, p_mid_h,
                   p_int_h, z_mid_h, qv_h, qc_h, qi_h, cldfrac_h, reff_qc_h,
                   reff_qi_h, dtau067_h, dtau105_h, isccp_cldtot_h,
                   isccp_ctptau_h, modis_ctptau_h, misr_cthtau_h);
    // Remask night values to ZERO since our I/O does not know how to handle
    // masked/missing values in temporal averages; this is all host data, so we
    // can just use host loops like its the 1980s
    ...
    // If not updating COSP statistics, set these to ZERO; this essentially
    // weights the ISCCP cloud properties by the sunlit mask. What will be
    // output for time-averages then is the time-average mask-weighted
    // statistics; to get true averages, we need to divide by the time-average
    // of the mask. I.e., if M is the sunlit mask, and X is the ISCCP statistic,
    // then
    //
    //     avg(X) = sum(M * X) / sum(M) = (sum(M * X)/N) / (sum(M)/N) = avg(M *
    //     X) / avg(M)
    //
