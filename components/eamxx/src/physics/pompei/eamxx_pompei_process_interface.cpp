...
#include "physics/share/physics_constants.hpp" // in eamxx/src (eamxx/src is in compiler path)
#include "pompei.hpp"
...
set_grids (const std::shared_ptr<const GridsManager> grids_manager)
{
  ...
  // we are declaring a tracer to be advected (will be init'ed to zero later, so Updated)
  // but note add_tracer
  add_tracer<Updated>("ash", m_grid, kg/kg);

  // we are interested in creating a "diagnostic" (in the context of this process) ash_column (a la LWP)
  // hence, Computed
...
void POMPEI::initialize_impl (const RunType /* run_type */)
{
  ...
  auto compute_mask = KOKKOS_LAMBDA (const int icol) {
    ...
    if (dist<radius) {
      emission_view(icol,emission_lev) = 1; // this is the actual calculation!!!!
    }
  };
