...
MLCorrection::MLCorrection(const ekat::Comm &comm,
  ...
  m_ML_model_path_sfc_fluxes =
      m_params.get<std::string>("ml_model_path_sfc_fluxes");
  m_fields_ml_output_variables =
      m_params.get<std::vector<std::string>>("ml_output_fields");
...
void MLCorrection::set_grids(
   ...
   * to be used here which we can then setup using the
   * m_fields_ml_output_variables variable
   */
  add_field<Updated>("T_mid", scalar3d_mid, K, grid_name, ps);
  add_field<Updated>("horiz_winds", vector3d_mid, m / s, grid_name, ps);
  /* Note: we also need to update the precipitation after ML commits any column
   * drying */
...
void MLCorrection::initialize_impl(const RunType /* run_type */) {
  ...
  ML_model_sfc_fluxes =
      py_correction.attr("get_ML_model")(m_ML_model_path_sfc_fluxes);
  ekat::enable_fpes(fpe_mask);

  // Enforce bounds on quantities adjusted by ML using Field Property Checks
  using LowerBound = FieldLowerBoundCheck;
  add_postcondition_check<LowerBound>(get_field_out("precip_liq_surf_mass"),
                                      m_grid, 0, true);
  add_postcondition_check<LowerBound>(get_field_out("precip_ice_surf_mass"),
                                      m_grid, 0, true);
...
void MLCorrection::run_impl(const double dt) {
  ...
  std::string datetime_str =
      current_ts.get_date_string() + " " + current_ts.get_time_string();

  const auto &phis = get_field_in("phis").get_view<const Real *, Host>();
  const auto &sfc_alb_dif_vis =
      get_field_in("sfc_alb_dif_vis").get_view<const Real *, Host>();
  ...
  const auto &SW_flux_dn =
      get_field_out("SW_flux_dn").get_view<Real **, Host>();
  const auto &sfc_flux_sw_net =
      get_field_out("sfc_flux_sw_net").get_view<Real *, Host>();
  const auto &sfc_flux_lw_dn =
      get_field_out("sfc_flux_lw_dn").get_view<Real *, Host>();
  const auto &u =
      get_field_out("horiz_winds").get_component(0).get_view<Real **, Host>();
  const auto &v =
      get_field_out("horiz_winds").get_component(1).get_view<Real **, Host>();

  // For precipitation adjustment we need to track the change in column
  // integrated 'qv' So we clone the original qv before ML changes the state so
  // we can back out a qv_tend to use with precip adjustment.
  ...
  pybind11::object ob1 = py_correction.attr("update_fields")(
      ...
      pybind11::array_t < Real,
      pybind11::array::c_style |
          pybind11::array::forcecast > (m_num_cols * m_num_levs * num_tracers,
                                        qv.data(), pybind11::str{}),
      pybind11::array_t < Real,
      pybind11::array::c_style |
          pybind11::array::forcecast >
      ...
      pybind11::array_t < Real,
      pybind11::array::c_style |
          pybind11::array::forcecast > (m_num_cols * (m_num_levs + 1),
      ...
      m_num_cols, m_num_levs, num_tracers, dt, ML_model_tq, ML_model_uv,
      ML_model_sfc_fluxes, datetime_str);
  pybind11::gil_scoped_release no_gil;
  ekat::enable_fpes(fpe_mask);

  // Now back out the qv change abd apply it to precipitation, only if Tq ML is
  // turned on
  if (m_ML_model_path_tq != "none") {
    ...
    const auto &precip_liq_surf_mass =
        get_field_out("precip_liq_surf_mass").get_view<Real *>();
    const auto &precip_ice_surf_mass =
        get_field_out("precip_ice_surf_mass").get_view<Real *>();
    ...
    Kokkos::parallel_for(
        "Compute WVP diff", policy, KOKKOS_LAMBDA(const MT &team) {
          ...
          // The water vapor path (WVP) is calculated as the integral of d_qv
          // over the vertical column which is converted to the units of
          // precipitation which are kg/m*m
          ...
          //       	gravity = m/s2
          //       d_qv * pseduo_density / gravity = kg/kg * kg/m/s2 * s2/m =
          //       kg/m2
          // Compute WaterVaporPath Difference
          Kokkos::parallel_reduce(
              ...
              [&](const int &ilev, Real &lsum) {
                lsum +=
                    (qnew_icol(ilev) - qold_icol(ilev)) * rho_icol(ilev) / g;
              },
              Kokkos::Sum<Real>(net_column_moistening));
          team.team_barrier();
          // Adjust Precipitation
          //  - Note, we subtract the water vapor path because positive precip
          //  represents
          //    a descrease in qv.
          auto tot_precip =
          ...
          if (tot_precip > 0) {
            ...
            // Apply all the adjustment to a single phase based on surface
            // temperature
            Kokkos::single(Kokkos::PerTeam(team), [&] {
          ...
          // Note, with the above formulation it is possible for the
          // precipitation to go negative. We rely on the field property checker
          // defined in the intialization function to repair any instances of
