...
struct UnitWrap::UnitTest<D>::TestGwdComputeTendenciesFromStressDivergence
  ...
  void run_bfb() {
    ...
    // Set up init data
    GwInit init_data[] = {
        // pver, pgwv,   dc, orog_only, molec_diff, tau_0_ubc, nbot_molec, ktop,
        // kbotbg, fcrit2, kwv
    ...
    GwdComputeTendenciesFromStressDivergenceData baseline_data[] = {
        //                                        ncol, ngwv, do_taper,   dt,
        //                                        effgw, init
        GwdComputeTendenciesFromStressDivergenceData(2, 10, false, 0.4, 0.3,
                                                     init_data[0]),
        GwdComputeTendenciesFromStressDivergenceData(3, 10, false, 0.4, 0.3,
                                                     init_data[0]),
        GwdComputeTendenciesFromStressDivergenceData(4, 10, true, 0.4, 0.3,
                                                     init_data[0]),
        GwdComputeTendenciesFromStressDivergenceData(5, 10, true, 0.4, 0.3,
                                                     init_data[0]),
    };

    static constexpr Int num_runs =
        sizeof(baseline_data) /
    ...
    // Create copies of data for use by test. Needs to happen before read calls
    ...
    if (SCREAM_BFB_TESTING && this->m_baseline_action == COMPARE) {
      for (Int i = 0; i < num_runs; ++i) {
        GwdComputeTendenciesFromStressDivergenceData &d_baseline =
            baseline_data[i];
        ...
        for (Int k = 0; k < d_baseline.total(d_baseline.gwut); ++k) {
          REQUIRE(d_baseline.total(d_baseline.gwut) ==
                  d_test.total(d_test.gwut));
          REQUIRE(d_baseline.gwut[k] == d_test.gwut[k]);
        }
        for (Int k = 0; k < d_baseline.total(d_baseline.utgw); ++k) {
          REQUIRE(d_baseline.total(d_baseline.utgw) ==
                  d_test.total(d_test.utgw));
          REQUIRE(d_baseline.utgw[k] == d_test.utgw[k]);
          REQUIRE(d_baseline.total(d_baseline.utgw) ==
                  d_test.total(d_test.vtgw));
