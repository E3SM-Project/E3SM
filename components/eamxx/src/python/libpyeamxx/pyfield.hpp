...
struct PyField {
  ...
  PyField(const FieldIdentifier &fid, const int pack_size = 1) {
    f = Field(fid);
    f.get_header().get_alloc_properties().request_allocation(pack_size);
    f.allocate_view();
  }

  template <typename FRAMEWORK> nb::ndarray<FRAMEWORK> get() const {
  ...
  void sync_to_dev() { f.sync_to_dev(); }
...
private:
  ...
  nb::dlpack::dtype get_dt_and_set_strides(std::vector<ssize_t> &strides) const {
    ...
    default:
      EKAT_ERROR_MSG("Unsupported field rank in PyField.\n"
                     " - field name: " +
                     f.name() +
                     "\n"
                     " - field rnak: " +
