...
create_test_grid (const ekat::Comm& comm, int num_lcols, int nlevs)
{
  ...
  const auto& lat = grid->create_geometry_data("lat",layout);
  const auto& lon = grid->create_geometry_data("lon",layout);
...
inline Field create_test_field (const std::shared_ptr<const AbstractGrid>& grid)
{
  ...
  FieldIdentifier fid ("field_1", layout);
  Field f(fid);
  f.allocate_view();
  return f;
}
...
inline Field create_data_field (const std::shared_ptr<const AbstractGrid>& grid)
{
  ...
  FieldIdentifier fid ("data", layout);
  Field f(fid);
  f.allocate_view();
  f.deep_copy(1);
  return f;
