...
PropertyCheck::ResultAndMsg FieldWithinIntervalCheck::check_impl() const {
  ...
  case 1: {
    ...
    Kokkos::parallel_reduce(
        size,
        KOKKOS_LAMBDA(int i, minmaxloc_value_t &result) {
  ...
  case 2: {
    ...
    Kokkos::parallel_reduce(
        size,
        KOKKOS_LAMBDA(int idx, minmaxloc_value_t &result) {
          int i, j;
...
template <typename ST> void FieldWithinIntervalCheck::repair_impl() const {
  using nonconst_ST = typename std::remove_const<ST>::type;
