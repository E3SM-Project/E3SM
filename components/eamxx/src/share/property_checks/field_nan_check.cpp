...
FieldNaNCheck::FieldNaNCheck(const Field &f,
  ...
  EKAT_REQUIRE_MSG(
      field_valid_data_types().has_v(f.data_type()),
...
PropertyCheck::ResultAndMsg FieldNaNCheck::check_impl() const {
  ...
  case 2: {
    ...
    Kokkos::parallel_reduce(
        size,
        KOKKOS_LAMBDA(int idx, int &result) {
          int i, j;
  ...
  if (res_and_msg.result == CheckResult::Fail) {
    auto &indices = res_and_msg.fail_loc_indices =
        unflatten_idx(layout.dims(), invalid_idx);
    ...
    if (m_grid) {
      ...
      auto gids = m_grid->get_dofs_gids()
                      .get_strided_view<const AbstractGrid::gid_type *, Host>();

      res_and_msg.msg += "  - indices (w/ global column index): (" +
                         std::to_string(gids(col_lid));
      ...
      if (has_latlon) {
        auto lat = m_grid->get_geometry_data("lat")
                       .get_internal_view_data<const Real, Host>();
        auto lon = m_grid->get_geometry_data("lon")
                       .get_internal_view_data<const Real, Host>();
