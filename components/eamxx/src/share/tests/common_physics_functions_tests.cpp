...
template <typename T, int N, int NumLevels> struct ChecksHelpers<ekat::Pack<T, N>, NumLevels> {
  ...
  static bool equal(const ScalarT &lhs, const ScalarT &rhs) { return (lhs == rhs).all(); }
  static bool approx_equal(const ScalarT &lhs, const ScalarT &rhs, const int k, const T tol) {
    const auto range      = ekat::range<ScalarT>(k * N);
    const auto range_mask = range < NumLevels;
...
//-----------------------------------------------------------------------------------------------//
template <typename ScalarT, typename DeviceT> void run(std::mt19937_64 &engine) {
