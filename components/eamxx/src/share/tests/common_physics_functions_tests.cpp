...
template <typename ScalarT, int NumLevels> struct ChecksHelpers {
  ...
  static bool approx_equal(const ScalarT lhs, const ScalarT rhs, const int k,
                           const ScalarT tol) {
    using std::abs;
    return not(k < NumLevels && abs(lhs - rhs) >= tol);
  }
  static bool approx_equal(const ScalarT computed, const ScalarT expected,
                           const ScalarT tol) {
    using std::abs;
...
void run_scalar_valued_fns(std::mt19937_64 &engine) {
  /*
  Most of the common physics functions are templated to operate on scalars or on
  packs of vertical indices. The functions tested here don't include any
  vertical dimension (e.g. they handle variables only defined at the surface),
  so are only defined for scalar reals. This is fundamentally different than the
  other functions, so these functions get their own run test.
  */
  ...
  RPDF pdf_lat(-pi / 2.0, pi / 2.0), pdf_area(1e-8, pi * pi);
  ...
  REQUIRE(
      Check::approx_equal(PF::calculate_surface_air_T(300, 0), 300, test_tol));
  REQUIRE(Check::approx_equal(PF::calculate_surface_air_T(300, 1000), 306.5,
                              test_tol));
  REQUIRE(Check::approx_equal(PF::calculate_surface_air_T(250.3, -10.2),
                              250.2337, test_tol));

  // lapse_T_for_psl property tests:
  // If T_ground = 0, T_ground_tmp should be 255/2 and lapse should be 0.0065
  // Really cold case. If T_ground = 300 K with phi_ground>0 m2/s2, lapse = 0
  // and T_ground_tmp=0.5*(290.5+T_ground). Really hot case. If T_ground = 290 K
  // and phi_ground=10000 m2/s2, T_ground_tmp=T_ground and lapse
  //    is such that T_sl=T_ground+lapse*phi_ground/gravit is within roundoff of
  //    290.5 K. Marginally hot case.
  // If T_ground = 280 K and phi_ground=100 m2/s2, T_ground_tmp=T_ground and
  // lapse=6.5 K/km (typical conditions)
  RealType lapse;
  ...
  PF::lapse_T_for_psl(T_ground, phi_ground, lapse, T_ground_tmp);
  REQUIRE(
      Check::approx_equal(T_ground_tmp, 0.5 * (290.5 + T_ground), test_tol));
  REQUIRE(Check::equal(lapse, 0));
  T_ground   = 290;
  phi_ground = 10000;
  PF::lapse_T_for_psl(T_ground, phi_ground, lapse, T_ground_tmp);
  REQUIRE(Check::equal(T_ground_tmp, T_ground));
  REQUIRE(Check::approx_equal(T_ground_tmp + lapse * phi_ground / g, 290.5,
                              test_tol));
  T_ground   = 280;
  phi_ground = 100;
  PF::lapse_T_for_psl(T_ground, phi_ground, lapse, T_ground_tmp);
  ...
  // computed value close to exact solution when lapse rate is zero (i.e. very
  // warm conditions) computed value close to exact solution when lapse rate
  // is 6.5 K/km (typical conditions) PSL lower than surface pressure whenever
  // surface height > 0 m PSL greater than surface pressure whenever surface
  // height < 0 m
  RealType p_ground =
      100000; // can't use p0 because that is a ScalarT which could be a pack
  ...
  phi_ground = -100;
  lapse      = 0.0065;
  psl_exact  = p_ground * std::pow(1. + lapse / g * phi_ground / T_ground,
                                   g / (Rd * lapse));
  ...
  lat = 0.0;
  REQUIRE(Check::equal(PF::calculate_dx_from_area(area, lat),
  ...
  REQUIRE(Check::approx_equal(PF::calculate_dx_from_area(area, lat),
                              coeff_1 - coeff_3, test_tol));
  lat = 22.5;
  REQUIRE(Check::approx_equal(PF::calculate_dx_from_area(area, lat),
                              coeff_1 - std::sqrt(2.0) / 2.0 * coeff_2,
                              test_tol));
  lat  = pdf_lat(engine);
  area = pdf_area(engine);
  REQUIRE(Check::equal(PF::calculate_dx_from_area(area, lat),
                       PF::calculate_dx_from_area(area, -lat)));
}
...
void run(std::mt19937_64 &engine) {
  ...
  view_1d temperature("temperature", num_mid_packs),
      height("height", num_mid_packs), qv("qv", num_mid_packs),
      qv_dry("qv_dry", num_mid_packs), qv_wet("qv_wet", num_mid_packs),
  ...
  auto dview_as_real = [&](const view_1d &v) -> rview_1d {
    return rview_1d(reinterpret_cast<RealType *>(v.data()),
                    v.size() * pack_size);
  };
  auto hview_as_real = [&](const typename view_1d::HostMirror &v) ->
      typename rview_1d::HostMirror {
        return typename rview_1d::HostMirror(
            reinterpret_cast<RealType *>(v.data()), v.size() * pack_size);
      };

  // Construct random input data
  using RPDF = std::uniform_real_distribution<RealType>;
  RPDF pdf_qv(1e-6, 1e-3), pdf_dp(1.0, 100.0), pdf_pres(0.0, PC::P0),
      pdf_temp(200.0, 400.0), pdf_height(0.0, 1e5), pdf_dz(1.0, 1e5),
      pdf_surface(100.0, 400.0), pdf_mmr(0, 0.99), pdf_dse(1e5, 5e5);
  ...
  //  - T_from_theta ant theta_from_T are one the inverse of the other (up to
  //  roundoff errors)
  T0     = pdf_temp(engine);
  ...
  REQUIRE(Check::approx_equal(
      PF::calculate_T_from_theta(PF::calculate_theta_from_T(T0, p), p), T0,
      test_tol));
  REQUIRE(Check::approx_equal(
      PF::calculate_theta_from_T(PF::calculate_T_from_theta(theta0, p), p),
      theta0, test_tol));
  ...
  //  - T_virt(T=T0) = T0
  //  - T(T_virt=T0) = T0
  //  - T_from_Tvirt and Tvirt_from_T are one the inverse of the other (up to
  //  roundoff errors)
  qv0 = pdf_qv(engine);
  T0  = pdf_temp(engine);
  ...
  REQUIRE(Check::equal(
      PF::calculate_temperature_from_virtual_temperature(zero, qv0), zero));
  REQUIRE(Check::equal(
      PF::calculate_temperature_from_virtual_temperature(T0, zero), T0));

  tmp = PF::calculate_temperature_from_virtual_temperature(T0, qv0);
  REQUIRE(Check::approx_equal(PF::calculate_virtual_temperature(tmp, qv0), T0,
                              test_tol));
  tmp = PF::calculate_virtual_temperature(T0, qv0);
  REQUIRE(Check::approx_equal(
      PF::calculate_temperature_from_virtual_temperature(tmp, qv0), T0,
      test_tol));

  // DSE property tests:
  //  - calculate_dse(T=0.0, z=0.0) = surf_height
  //  - calculate_dse(T=1/cp, z=1/gravity) = surf_height+2
  //  - calculate_temperature_from_dse(dse=cp, z=cp/gravity) =
  //  -1.0*surf_height/cp (up to roundoff errors)
  //  - calculate_dse and calculate_temperature_from_dse are one the inverse of
  //  the other (up to roundoff errors)
  surf_height = pdf_surface(engine);
  z0          = pdf_height(engine);
  dse0        = pdf_dse(engine);
  T0          = pdf_temp(engine);
  REQUIRE(Check::equal(PF::calculate_dse(zero, zero, surf_height),
                       ScalarT(surf_height)));
  REQUIRE(Check::equal(
      PF::calculate_dse(ScalarT(inv_cp), ScalarT(1 / g), surf_height),
      ScalarT(surf_height + 2.0)));
  REQUIRE(Check::approx_equal(PF::calculate_temperature_from_dse(
                                  ScalarT(cp), ScalarT(cp / g), surf_height),
                              ScalarT(-1.0 * surf_height / cp), test_tol));
  REQUIRE(Check::approx_equal(
      PF::calculate_dse(
          PF::calculate_temperature_from_dse(dse0, z0, surf_height), z0,
          surf_height),
      dse0, test_tol));
  REQUIRE(Check::approx_equal(
      PF::calculate_temperature_from_dse(PF::calculate_dse(T0, z0, surf_height),
                                         z0, surf_height),
      T0, test_tol));
  ...
  // mmr_test3: Compute drymmr from wetmmr0 and then use the result to compute
  // wetmmr, which should be approximately
  //            equal to wetmmr0. NOTE: calculate_wetmmr_from_drymmr takes qv in
  //            dry mmr as an argument and calculate_drymmr_from_wetmmr takes qv
  //            in wet mmr as an argument. Therefore, we need to convert qv from
  //            wet mmr to dry mmr as well as part of these conversions

  REQUIRE(Check::equal(PF::calculate_wetmmr_from_drymmr(zero, qv_dry0),
                       zero)); // mmr_test1
  REQUIRE(Check::equal(PF::calculate_drymmr_from_wetmmr(zero, qv_wet0),
                       zero)); // mmr_test2

  // mmr_test3
  drymmr0 = PF::calculate_drymmr_from_wetmmr(
      wetmmr0, qv_wet0); // get drymmr from wetmmr0 using qv_wet0
  ...
  tmp     = PF::calculate_wetmmr_from_drymmr(drymmr0,
                                             qv_dry0); // convert it back to wetmmr0
  REQUIRE(Check::approx_equal(tmp, wetmmr0,
                              test_tol)); // wetmmr0 should be equal to tmp
  ...
  REQUIRE(Check::equal(PF::calculate_dz(ScalarT(p0), ScalarT(p0), one, zero),
                       ScalarT(Rd / g)));
  REQUIRE(Check::approx_equal(
      PF::calculate_dz(ScalarT(g), ScalarT(Rd), T0, zero), T0, test_tol));

  // MMR and VMR property tests:
  //  - calculate_vmr_from_mmr(mmr=0) = 0
  //  - calculate_mmr_from_vmr(vmr=0) = 0
  //  - calculate_vmr_from_mmr(calculate_mmr_from_vmr(gas_name="h2o",vmr0)) =
  //  vmr
  //  - calculate_vmr_from_mmr(calculate_mmr_from_vmr(gas_name="o2",vmr0)) =
  //  vmr, test that changing gas name changes the result.
  //  - calculate_mmr_from_vmr(calculate_vmr_from_mmr(gas_name="h2o",mmr0)) =
  //  vmr
  //  - calculate_mmr_from_vmr(calculate_vmr_from_mmr(gas_name="o2",mmr0)) !=
  //  vmr, test that changing gas name changes the result.
  mmr0               = pdf_mmr(engine);
  vmr0               = pdf_mmr(engine);
  qv0                = pdf_qv(engine);
  ...
  REQUIRE(Check::approx_equal(PF::calculate_mmr_from_vmr(h2o_mol, qv0, tmp),
                              mmr0, test_tol));
  REQUIRE(!Check::approx_equal(PF::calculate_mmr_from_vmr(o2_mol, qv0, tmp),
                               mmr0, test_tol));
  tmp = PF::calculate_mmr_from_vmr(h2o_mol, qv0, vmr0);
  REQUIRE(Check::approx_equal(PF::calculate_vmr_from_mmr(h2o_mol, qv0, tmp),
                              vmr0, test_tol));
  REQUIRE(!Check::approx_equal(PF::calculate_vmr_from_mmr(o2_mol, qv0, tmp),
                               vmr0, test_tol));

  // --------- Run tests on full columns of data ----------- //
  TeamPolicy policy(
      ekat::ExeSpaceUtils<ExecSpace>::get_default_team_policy(1, 1));
  Kokkos::parallel_for(
      "test_universal_physics", policy, KOKKOS_LAMBDA(const MemberType &team) {
        ...
        PF::calculate_temperature_from_virtual_temperature(team, Tv, qv,
                                                           T_from_Tv);
        ...
        PF::calculate_temperature_from_dse(team, dse, height, surf_height,
                                           T_from_dse);
        ...
        PF::calculate_drymmr_from_wetmmr(team, wetmmr_for_testing, qv_wet,
                                         drymmr);
  ...
  for (int k = 0; k < num_mid_packs; ++k) {
    ...
    REQUIRE(Check::approx_equal(T_from_Theta_host(k), temperature_host(k), k,
                                test_tol));

    // Check T(T_virtual(T))==T (up to roundoff tolerance)
    REQUIRE(Check::approx_equal(T_from_Tv_host(k), temperature_host(k), k,
                                test_tol));

    // Check vmr(mmr(vmr))==mmr (up to roundoff tolerance)
    REQUIRE(
        Check::approx_equal(mmr_host(k), mmr_for_testing_host(k), test_tol));

    // Check wetmmr(drymmr(wetmmr))==wetmmr (up to roundoff tolerance)
    REQUIRE(Check::approx_equal(wetmmr_host(k), wetmmr_for_testing_host(k),
                                test_tol));
  }
} // run()
