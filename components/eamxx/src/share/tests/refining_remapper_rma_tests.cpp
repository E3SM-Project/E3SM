...
public:
  ...
  void test_internals() {
    ...
    REQUIRE(m_remote_lids.size() ==
            static_cast<size_t>(m_ov_coarse_grid->get_num_local_dofs()));
    REQUIRE(m_remote_pids.size() ==
            static_cast<size_t>(m_ov_coarse_grid->get_num_local_dofs()));
    ...
    for (int i = 0; i < m_num_fields; ++i) {
      ...
      if (fh.get_parent()) {
        REQUIRE(m_col_stride[i] ==
                col_alloc_size * fap.get_subview_info().dim_extent);
        REQUIRE(m_col_offset[i] ==
                col_alloc_size * fap.get_subview_info().slice_idx);
      } else {
    ...
    auto col_gids_h    = m_ov_coarse_grid->get_dofs_gids()
                          .get_view<const AbstractGrid::gid_type *, Host>();

    auto row_gids_h = m_tgt_grid->get_dofs_gids()
                          .get_view<const AbstractGrid::gid_type *, Host>();
...
Field create_field(const std::string &name, const LayoutType lt,
  ...
  auto pdf = [&](Engine &e) { return std::pow(2, ipdf(e)); };
