...
TEST_CASE("field_mgr", "") {
  ...
  std::vector<int> dims1 = {ncols1, nlevs1};
  std::vector<int> dims2 = {ncols2, nlevs2};
  std::vector<int> dims3 = {ncols1, 10, nlevs1};
  std::vector<int> dims4 = {ncols2, 10, nlevs2};
...
TEST_CASE("sync_subfields") {
  ...
  for (int c = 1; c < ndims; ++c) {
    ...
    for (int idx = 0; idx < ncols * nlevs; ++idx) {
      ...
      if (shared_mem_space)
        REQUIRE(host_subview(icol, ilev) == c);
      else
        REQUIRE(host_subview(icol, ilev) == ndims);
    }
  }
    ...
    Kokkos::parallel_for(
        ...
        KOKKOS_LAMBDA(const int icol, const int ilev) {
          if (shared_mem_space)
            ...
            EKAT_KERNEL_ASSERT(device_subview(icol, ilev) == ndims);
        });
  }
}
