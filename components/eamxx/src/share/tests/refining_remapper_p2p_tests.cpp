...
Field create_field(const std::string &name, const LayoutType lt, const AbstractGrid &grid) {
  ...
  case LayoutType::Scalar2D:
    f = Field(FieldIdentifier(name, grid.get_2d_scalar_layout(), u, gn));
    break;
  case LayoutType::Vector2D:
    f = Field(FieldIdentifier(name, grid.get_2d_vector_layout(ndims), u, gn));
    break;
  case LayoutType::Scalar3D:
    f = Field(FieldIdentifier(name, grid.get_3d_scalar_layout(true), u, gn));
    break;
  ...
  for (int pid = 0, offset = 0; pid < comm.size(); ++pid) {
    ...
    for (int icol = 0; icol < ncols; ++icol, offset += col_size) {
      ...
      case 3:
        ...
        break;
      default:
        EKAT_ERROR_MSG("Unexpected rank in RefiningRemapperP2P unit test.\n"
                       "  - field name: " +
                       f.name() + "\n");
      }
