...
TEST_CASE("data_interpolation_setup") {
  ...
  EKAT_REQUIRE_MSG(comm.size() == 1,
                   "Error! You should run the data_interpolation_setup test "
                   "with ONE rank.\n");

  // Create grid
  std::shared_ptr<const AbstractGrid> grid =
      create_point_grid("pg", ngcols, nlevs, comm);

  // Create and setup two files, so we can test both YearlyPeriodic and
  // LinearHistory
  std::vector<std::string> files = {"data_interpolation_0",
  ...
  for (auto int_same_as_mid : {true, false}) {
    ...
    for (const std::string &fname : files) {
      ...
      scorpio::define_var(fname + suffix, "s2d", {"ncol"}, "real", true);
      scorpio::define_var(fname + suffix, "v2d", {"ncol", "dim2"}, "real",
                          true);
      scorpio::define_var(fname + suffix, "s3d_m", {"ncol", "lev"}, "real",
                          true);
      scorpio::define_var(fname + suffix, "v3d_m", {"ncol", "dim2", "lev"},
                          "real", true);
      scorpio::define_var(fname + suffix, "s3d_i", {"ncol", ilev}, "real",
                          true);
      scorpio::define_var(fname + suffix, "v3d_i", {"ncol", "dim2", ilev},
                          "real", true);
      ...
      scorpio::define_var(fname + suffix, "p3d", {"ncol", "lev"}, "real",
                          false);
    ...
    // We keep pressures fields NOT time-dep, so we write outside the loop. Also
    // write hyam/hybm here
    ...
    hyam.sync_to_host();
    for (const std::string &fname : files) {
      scorpio::write_var(fname + suffix, p1d.name(),
                         p1d.get_internal_view_data<Real, Host>());
      scorpio::write_var(fname + suffix, p2d.name(),
                         p2d.get_internal_view_data<Real, Host>());
      scorpio::write_var(fname + suffix, p3d.name(),
                         p3d.get_internal_view_data<Real, Host>());
      scorpio::write_var(fname + suffix, hybm.name(),
                         hybm.get_internal_view_data<Real, Host>());
      scorpio::write_var(fname + suffix, hyam.name(),
    ...
    for (int mm = 0; mm < 12; ++mm) {
      std::string file_name =
          "data_interpolation_" + std::to_string(mm / 6) + suffix;
      ...
      for (int i = 0; i < nfields; ++i) {
        ...
        f.sync_to_host();
        scorpio::write_var(file_name, f.name(),
                           f.get_internal_view_data<Real, Host>());
      }
