...
void run(std::mt19937_64& engine)
{
  ...
  auto diag = diag_factory.create("LongwaveCloudForcing",comm,params, grid);
  ...
  // Run tests
  {
    ...
    randomize_uniform(LW_flux_up_f,seed++,0,400);
    ...
    auto LWCF_v = LWCF_f.get_view<Real*>();
    auto LW_flux_up_v = LW_flux_up_f.get_view<const Real**>();
    auto LW_clrsky_flux_up_v = LW_clrsky_flux_up_f.get_view<const Real**>();
    auto manual = KOKKOS_LAMBDA(const int icol) {
      LWCF_v(icol) = LW_clrsky_flux_up_v(icol,0) -  LW_flux_up_v(icol,0);
    };
    RP policy(0,ncols);
