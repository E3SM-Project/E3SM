...
void FieldLayout::compute_type () {
  ...
  const int nvlevs    = count(tags,LEV) + count(tags,ILEV) + count(tags,PLEV);
  ...
  auto is_lev_tag = [](const FieldTag t) {
    return t==LEV or t==ILEV or t==PLEV;
  };
  switch (size) {
    case 0:
      ...
      break;
    case 1:
      // The only tag left should be 'CMP', 'TL', or 'LEV'/'ILEV'/'PLEV'
    ...
    case 3:
      ...
      //  1) <TL,  CMP, LEV|ILEV|PLEV>
      //  2) <CMP, CMP, LEV|ILEV|PLEV>
