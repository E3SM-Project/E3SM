...
inline bool views_are_equal(const Field &f1, const Field &f2,
  ...
  case DataType::IntType:
    ret = impl::views_are_equal<const int>(f1, f2, comm);
    break;
  case DataType::FloatType:
    ret = impl::views_are_equal<const float>(f1, f2, comm);
    break;
  case DataType::DoubleType:
    ret = impl::views_are_equal<const double>(f1, f2, comm);
    break;
  default:
...
void randomize(const Field &f, Engine &engine, PDF &&pdf) {
  ...
  EKAT_REQUIRE_MSG(
      (std::is_same<ST, int>::value && data_type == DataType::IntType) ||
          (std::is_same<ST, float>::value &&
           data_type == DataType::FloatType) ||
          (std::is_same<ST, double>::value &&
           data_type == DataType::DoubleType),
...
//   - level_mask:         Mask (size of the level dimension of f) where
//   f(i0,...,k) is
//                         perturbed if level_mask(k)=true
//   - dof_gids:           Field containing global DoF IDs for columns of f (if
//   applicable)
template <typename Engine, typename PDF, typename MaskType>
void perturb(Field &f, Engine &engine, PDF &&pdf, const int base_seed,
             const MaskType &level_mask, const Field &dof_gids = Field()) {
  EKAT_REQUIRE_MSG(
      f.is_allocated(),
  ...
  const auto data_type = f.data_type();
  EKAT_REQUIRE_MSG(
      (std::is_same_v<ST, int> && data_type == DataType::IntType) or
  ...
  EKAT_REQUIRE_MSG(fl.rank() > 0 &&
                   ...
                   "Error! Trying to perturb field \"" + f.name() +
                       "\", but field "
  ...
  if (fl.has_tag(COL)) {
    ...
    const auto &dof_gids_fl =
        dof_gids.get_header().get_identifier().get_layout();
...
void horiz_contraction(const Field &f_out, const Field &f_in,
  ...
  EKAT_REQUIRE_MSG(
      ...
      "The input field's layout is "
          ...
          << l_out.to_string() << ".\n");
  EKAT_REQUIRE_MSG(f_out.is_allocated() && f_in.is_allocated() &&
                       weight.is_allocated(),
...
ST frobenius_norm(const Field &f, const ekat::Comm *comm = nullptr) {
  ...
  const auto data_type = f.data_type();
  EKAT_REQUIRE_MSG(data_type == DataType::FloatType ||
                       data_type == DataType::DoubleType,
                   "Error! Frobenius norm only allowed for floating-point "
                   "field value types.\n");
...
// Prints the value of a field at a certain location, specified by tags and
// indices. If the field layout contains all the location tags, we will slice
// the field along those tags, and print it. E.g., f might be a <COL,LEV> field,
// and the tags/indices refer to a single column, in which case we'll print a
// whole column worth of data.
inline void print_field_hyperslab(const Field &f,
  ...
  default:
    EKAT_ERROR_MSG(
        "[print_field_hyperslab] Error! Invalid/unsupported data type.\n"
        " - field name: " +
        f.name() + "\n");
  }
}
...
void compute_mask(const Field &x, const ST value, Field &mask) {
  ...
  EKAT_REQUIRE_MSG(
      not is_narrowing_conversion(val_dt, x_dt),
  ...
  switch (x_dt) {
  case DataType::IntType:
    impl::compute_mask<CMP>(x, static_cast<int>(value), mask);
    break;
  case DataType::FloatType:
    impl::compute_mask<CMP>(x, static_cast<float>(value), mask);
    break;
  case DataType::DoubleType:
    impl::compute_mask<CMP>(x, static_cast<double>(value), mask);
    break;
  default:
