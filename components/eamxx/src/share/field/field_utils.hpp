...
//  - if both fields are masked, the masks themselves must agree
//  - if only one is masked, the mask must be nonzero everywhere
//  - if masked, only entries with nonzero mask are compared
