...
public:
  ...
  using repo_type =
      std::map<ci_string, std::map<ci_string, std::shared_ptr<Field>>>;
  using field_group_type =
      std::map<ci_string, std::map<ci_string, std::shared_ptr<FieldGroup>>>;
  ...
  // This can be used to allow atm procs to create some helper fields
  // internally, but still leverage the FM class for certain features (e.g.,
  // I/O). NOTE: the repo must be in closed state, and the FieldManager must not
  // already
  //       store a field with the same name.
  void add_field(const Field &f);

  // Adds $field_name on $grid_name to group $group_name (creating the group, if
  // necessary). NOTE: if $group_name is allocated as a monolithic field, this
  // throws. NOTE: must be called after registration ends
  ...
  bool has_field(const std::string &field_name) const {
    EKAT_ASSERT_MSG(m_grids_mgr->size() == 1,
                    "Error! More than one grid exists for FieldManager, "
    ...
    return has_field(field_name,
                     m_grids_mgr->get_repo().begin()->second->name());
  }
  ...
  Field get_field(const std::string &name) const {
    EKAT_ASSERT_MSG(m_grids_mgr->size() == 1,
                    "Error! More than one grid exists for FieldManager, must "
                    "specify grid name to get field.\n"
                    "  - Field name: " +
                        name +
                        "\n"
                        "  - Grids in FM: " +
  ...
  FieldGroupInfo get_group_info(const std::string &group_name) const {
    EKAT_ASSERT_MSG(m_grids_mgr->size() == 1,
                    ...
                    "  - Group name: " +
                        ...
                        "  - Grids in FM: " +
                        m_grids_mgr->print_available_grids() + "\n");
    return get_group_info(group_name,
                          m_grids_mgr->get_repo().begin()->second->name());
  }
  FieldGroupInfo get_group_info(const std::string &group_name,
                                const std::string &grid_name) const;
  ...
  get_repo(const std::string &grid_name) const {
    EKAT_REQUIRE_MSG(
        m_grids_mgr->has_grid(grid_name),
        "Error! This field manager does not contain data on grid \"" +
            grid_name + "\"\n");
  ...
  const std::shared_ptr<const GridsManager> &get_grids_manager() const {
    return m_grids_mgr;
  }
...
protected:
  ...
  std::map<std::string, std::map<std::string, std::set<GroupRequest>>>
      m_group_requests;
  ...
  // we 'skip' them, hoping that some other request will contain the right
  // specs. If no complete request is given for that field, we need to error out
