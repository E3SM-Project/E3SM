...
FieldManager::FieldManager(const std::shared_ptr<const GridsManager> &gm) : m_grids_mgr(gm) {
  EKAT_REQUIRE_MSG(m_grids_mgr != nullptr, "Error! Input grids manager pointer is not valid.");
...
bool FieldManager::has_field(const std::string &field_name, const std::string &grid_name) const {
  return m_fields.at(grid_name).find(field_name) != m_fields.at(grid_name).end();
}

bool FieldManager::has_group(const std::string &group_name, const std::string &grid_name) const {
...
void FieldManager::registration_ends() {
  ...
  if (groups_with_monolithic_allocation.size() > 0) {
    ...
    auto intersect = [](const std::list<ci_string> &lhs, const std::list<ci_string> &rhs) -> bool {
