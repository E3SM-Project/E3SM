...
FieldManager::FieldManager(const std::shared_ptr<const AbstractGrid> &grid)
    : FieldManager(std::make_shared<LibraryGridsManager>(grid)) {
  // Nothing else to do
}

FieldManager::FieldManager(const std::shared_ptr<const GridsManager> &gm)
    : m_grids_mgr(gm) {
...
void FieldManager::register_field(const FieldRequest &req) {
  ...
  EKAT_REQUIRE_MSG(m_repo_state != RepoState::Clean,
                   "Error! Repo state is not 'Open' yet. You must call "
                   "registration_begins() first.\n");
  EKAT_REQUIRE_MSG(m_repo_state != RepoState::Closed,
                   "Error! Repo state is not 'Open' anymore. You already "
                   "called registration_ends().\n");

  // Make sure this FM contains a grid corresponding to the input grid name
  EKAT_REQUIRE_MSG(m_grids_mgr->has_grid(grid_name),
                   "Error! Attempting to register field on grid not in the "
                   "FM's grids manager:\n"
                   "  - FieldRequest grid: " +
                       grid_name +
                       "\n"
  ...
  EKAT_REQUIRE_MSG(not req.incomplete,
                   "Error! FieldManager does not allow registering incomplete "
                   "FieldRequest.\n");
  ...
  if (!has_field(id.name(), grid_name)) {
    ...
    EKAT_REQUIRE_MSG(
        id.data_type() == field_valid_data_types().at<Real>(),
        "Error! While refactoring, we only allow the Field data type to be "
        "Real.\n"
        "       If you're done with refactoring, go back and fix things.\n");
    m_fields[grid_name][id.name()] = std::make_shared<Field>(id);
  } else {
    // Make sure the input field has the same layout and units as the field
    // already stored.
    ...
    const auto id0 =
        m_fields[grid_name][id.name()]->get_header().get_identifier();
    EKAT_REQUIRE_MSG(id.get_units() == id0.get_units(),
                     "Error! Field '" + id.name() +
                         ...
                         "\n"
                         "       Please, check and make sure all atmosphere "
                         "processes use the same units.\n");
  ...
  m_fields[grid_name][id.name()]
      ->get_header()
      .get_alloc_properties()
      .request_allocation(req.pack_size);
  ...
  for (const auto &group_name : req.groups) {
    ...
    if (not ekat::contains(group_info->m_grid_registered[id.name()],
                           grid_name)) {
      group_info->m_grid_registered[id.name()].push_back(grid_name);
    }

    // Ensure that each group in m_field_group_info also appears in the
    // m_group_requests map by adding a "trivial" GroupRequest for this group,
    // meaning no monolithic allocation and pack size 1.
...
void FieldManager::register_group(const GroupRequest &req) {
  ...
  EKAT_REQUIRE_MSG(m_grids_mgr->has_grid(req.grid),
                   "Error! Attempting to register group on grid not in the "
                   "FM's grids manager:\n"
                   "  - GroupRequest grid:  " +
                       req.grid +
                       "\n"
  ...
  // Groups have to be handled once registration is over, so for now simply
  // store the request, and create an empty group info (if it does not already
  // exist)
  ...
  if (not group_info) {
    ...
    group_info->m_monolithic_allocation =
        (req.monolithic_alloc == MonolithicAlloc::Required);
...
void FieldManager::add_to_group(const std::string &field_name,
                                const std::string &grid_name,
                                const std::string &group_name) {
  EKAT_REQUIRE_MSG(m_repo_state == RepoState::Closed,
                   "Error! You cannot call 'add_to_group' until after "
                   "'registration_ends' has been called.\n");
  auto &group = m_field_group_info[group_name];
  if (not group) {
    group = std::make_shared<FieldGroupInfo>(group_name);
  }
  EKAT_REQUIRE_MSG(
      not group->m_monolithic_allocation,
  ...
  EKAT_REQUIRE_MSG(has_field(field_name, grid_name),
                   "Error! Cannot add field to group, since the field is not "
                   "present in this FieldManager.\n"
                   "   field name: " +
                       field_name +
                       "\n"
                       "   grid name:  " +
...
bool FieldManager::has_field(const std::string &field_name,
                             const std::string &grid_name) const {
  return m_fields.at(grid_name).find(field_name) !=
         m_fields.at(grid_name).end();
}

bool FieldManager::has_group(const std::string &group_name,
                             const std::string &grid_name) const {
...
Field FieldManager::get_field(const std::string &name,
                              const std::string &grid_name) const {
  EKAT_REQUIRE_MSG(m_repo_state == RepoState::Closed,
                   "Error! Cannot get fields from the repo while registration "
                   "has not yet completed.\n");
  EKAT_REQUIRE_MSG(has_field(name, grid_name), "Error! Field " + name +
                                                   " on grid " + grid_name +
                                                   " not found.\n");
...
FieldGroupInfo
FieldManager::get_group_info(const std::string &group_name,
                             const std::string &grid_name) const {
  // Sanity checks
  EKAT_REQUIRE_MSG(m_repo_state == RepoState::Closed,
                   "Error! Cannot get field groups from the repo while "
                   "registration has not yet completed.\n");
  EKAT_REQUIRE_MSG(has_group(group_name, grid_name),
                   "Error! Field group '" + group_name + "' on grid '" +
                       grid_name + "' not found.\n");

  auto info = *m_field_group_info.at(group_name);

  if (info.m_monolithic_allocation) {
    // All fields in a group with a monolithic allocation should exist on any
    // grid in that group
    for (const auto &fname : info.m_fields_names) {
      EKAT_REQUIRE_MSG(
          has_field(fname, grid_name),
          "Internal FieldManager Error! Groups with monolithic allocation must "
          "contain all "
...
FieldGroup FieldManager::get_field_group(const std::string &group_name,
  ...
  if (m_field_groups.at(grid_name).find(group_name) !=
      m_field_groups.at(grid_name).end()) {
  ...
  group->m_info =
      std::make_shared<FieldGroupInfo>(get_group_info(group_name, grid_name));
  ...
  if (group->m_info->m_monolithic_allocation) {
    // All fields in a group have the same parent, get the parent from the 1st
    // one
    ...
    EKAT_REQUIRE_MSG(parent_header != nullptr,
                     "Error! A field belonging to a field group with "
    ...
    EKAT_REQUIRE_MSG(has_field(parent_id.name(), grid_name),
                     "Internal FieldManager Error! Parent field must exist in "
                     "the FieldManager:\n"
                     "  - Group: " +
                         group_name +
                         "\n"
    ...
    for (auto child : parent_header->get_children()) {
      ...
      for (auto f : group->m_individual_fields) {
        if (child.lock()->get_identifier() ==
            f.second->get_header().get_identifier()) {
          in_group = true;
          break;
        }
      }
    ...
    if (all_children_in_group) {
      ...
      size_t span =
          ordered_subview_indices.back() - ordered_subview_indices.front() + 1;
      EKAT_REQUIRE_MSG(
          ordered_subview_indices.size() == span,
          "Error! Non-contiguous subview indices found in group \"" +
              group_name + "\"\n");

      group->m_monolithic_field =
          std::make_shared<Field>(parent_field->subfield(
              group_name, parent_header->get_identifier().get_units(),
              group->m_info->m_subview_dim, ordered_subview_indices.front(),
              ordered_subview_indices.back() + 1));
    }
  }
...
void FieldManager::init_fields_time_stamp(const util::TimeStamp &t0) {
  EKAT_REQUIRE_MSG(m_repo_state == RepoState::Closed,
                   "Error! Cannot set initial time stamp until registration "
                   "has completed.\n");
...
void FieldManager::registration_ends() {
  // This method is responsible of allocating the fields in the repo. The most
  // delicate part is the allocation of fields group, in the case where a
  // monolithic allocation is required. If we are not able to honor requests, we
  // will error out. An example of a scenario where we can't honor all requests
  // is given by the three groups G1=(A,B), G2=(B,C), and G3=(A,C). Clearly,
  // only two of these groups can have contiguous allocation.

  // To understand how we can parse the groups to figure out if/how to
  // accommodate all requests, consider the following GR:  G1=(A,B,C),
  // G2=(A,B,C,D,E), G3=(C,D), G4=(C,D,E,F), G5=((D,E,F,G). The ordering of
  // fields (A,B,C,D,E,F,G) clearly accommodates all GRs, though it might not be
  // obvious if one scrambled the fields within each GR.

  // For group requests on different grids, we consider the group members to be
  // the union of all fields registered to a particular group, over all grids.
  // So if fields A and B are registered as part of group G on grid1, and field
  // C is registered as a part of G on grid2, both G on grid1 and G on grid2
  // will contain fields A,B,C, and we ensure that the subview index matches
  // over grids.

  // This is the sequence of operations that allows us to establish if (and how)
  // we can honor all the requests:
  //  1) ensure all groups with a monolithic allocation contain the desired
  //  members. This means that
  //     we need to loop over GroupRequest (GR), and make sure there are fields
  //     registered in those groups (querying m_field_group_info info structs).
  //  2) Focus only on GR that require a monolithic allocation, discarding
  //  others.
  //     All the remaining group can simply "grab" individual fields later (and
  //     they can even grab some "individual" fields, and some fields that are
  //     slices of another group).
  //  3) Divide the groups found at step 2 into 'clusters'. A cluster is a
  //  collection
  //     of field groups such that each group in it share at least one field
  //     with at least another group in the cluster. If two groups are not in
  //     the same cluster, they cannot share any field. Each cluster of field
  //     groups can be processed individually, since allocations are completely
  //     independent. This allows to reduce the combination of groups to checks
  //     later. Note: to build a cluster, take any group not yet in a cluster.
  //     Then iteratively add any group that intersects the cluster, until no
  //     intersections are found.
  //  4) If there is no group in the cluster that contains all the fields of the
  //  cluster,
  //     create the cluster group C. In order to accommodate all requests for
  //     groups, we must be able to allocate C contiguously.
  //  5) For each cluster, call the function contiguous_superset from
  //  eamxx_utils.hpp
  //     (see that file for details). If the fcn fails to find an ordering of
  //     the cluster's field that accommodate all requests, it will return an
  //     empty list. Otherwise it will return the ordering of all fields in the
  //     cluster that allows all groups of the cluster to be a contiguous subset
  //     of C.
  //  Note: this can be done independent of grid since each grid will contain
  //  the same fields

  // Start by processing group request. This function checks that all group
  // fields are properly registered on the appropriate grid and the
  // FieldGroupInfo is up to date. If group G is requested, we make sure to
  // register the union of all fields in group G on any grids they appear.
  ...
  // moving qv first in "tracers" will do the same for
  // "turbulence_advected_tracers". Only homme requires qv first, so we only
  // need to move qv if "tracers" exist.
  bool qv_must_come_first = false;
  if (ekat::contains(groups_with_monolithic_allocation, "tracers") and
    ...
    // Adding the 'fake' group G=(qv) at the front of
    // groups_with_monolithic_allocation ensures qv won't be put in the middle
    // of the tracers group. We use a highly unlikely group name, to avoid
    // clashing with a real group name. Later, after having found an global
    // ordering for the tracers fields, we will remove this group.
    groups_with_monolithic_allocation.push_front("__qv__");
    m_field_group_info.emplace("__qv__",
                               std::make_shared<FieldGroupInfo>("__qv__"));
    m_field_group_info.at("__qv__")->m_fields_names.push_back("qv");
    qv_must_come_first = true;
  }

  // Do all the monolithic allocation stuff only if there are groups which
  // require it.
  if (groups_with_monolithic_allocation.size() > 0) {
    ...
    // Determine if two lists have elements in common (does not compute the
    // intersection)
    ...
    while (added_to_a_cluster.size() <
           groups_with_monolithic_allocation.size()) {
      cluster_type c;
    ...
    for (auto &cluster : clusters) {
      ...
      if (cluster_ordered_fields.size() == 0) {
        // We were not able to accommodate all the allocation requests for the
        // groups in this cluster. We have to error out. But first, let's print
        // some information, so the developers/users can have a shot at fixing
        // this (e.g., by changing the request for monolithic allocation in some
        // Atm Proc).
        std::cout << "Error! Field manager was not able to accommodate the "
                     "following\n"
                  << "       requests for monolithically allocated groups:\n";
        for (const auto &gn : cluster) {
          std::cout << "   - " << gn << "\n";
        }
        std::cout << "       Consdier modifying the Atm Procs where these "
                     "groups are requested.\n";

        EKAT_ERROR_MSG(" -- ERROR --\n");
      }

      // Ok, if we got here, it means we can allocate the cluster as a field,
      // and then subview all the groups Steps:
      //  - check if there's a group in the cluster containing all the fields.
      //  If yes, use that group
      //    name for the grouped field, otherwise make one up from the names of
      //    all groups in the cluster.
      //  - allocate the cluster field F.
      //  - loop over the groups in the cluster, and subview F at the proper
      //  (contiguous) indices.

      // WARNING: this lines should be removed if we move away from Homme's
      // requirement that
      ...
      if (qv_must_come_first) {
        ...
        if (qv_it != cluster_ordered_fields.end()) {
          // Check that qv comes first or last (if last, reverse the list). If
          // not, error out. NOTE: I *think* this should not happen, unless
          // 'tracers' is a subgroup of a bigger group.
          EKAT_REQUIRE_MSG(qv_it == cluster_ordered_fields.begin() ||
                               std::next(qv_it, 1) ==
                                   cluster_ordered_fields.end(),
                           "Error! The water vapor field has to be the first "
                           "tracer, but it is not.\n");

          if (qv_it != cluster_ordered_fields.begin()) {
            // Note: reversing the order of the fields preserves subgroups
            // contiguity
      ...
      // Check if there is a group with all the fields. Notice that it is enough
      // to check if any list in the LOL has the same length as
      // cluster_ordered_fields. If not, we will set cluster_name = $group1_name
      // | $group2_name | ... Note: cluster_name will be the name of the field
      // allocating all fields in the cluster's groups
      ...
      for (const auto &gn : cluster) {
        for (auto grid_name : m_grids_mgr->get_grid_names()) {
          if (m_group_requests.at(grid_name).find(gn) !=
              m_group_requests.at(grid_name).end()) {
      ...
      for (auto cluster_grid_name : grids_in_cluster) {
        ...
        for (const auto &fname : cluster_ordered_fields) {
          ...
          if (lt == LayoutType::Invalid) {
            ...
            EKAT_REQUIRE_MSG(lt == id.get_layout().type(),
                             "Error! Found a group containing fields with "
                             "different layouts.\n"
                             "       Group name: " +
                                 cluster_name +
                                 "\n"
                                 ...
                                 "\n"
                                 "       Layout 2: " +
                                 e2str(id.get_layout().type()) + "\n");
          }
        }

        EKAT_REQUIRE_MSG(
            lt == LayoutType::Scalar2D || lt == LayoutType::Scalar3D,
        ...
        if (lt == LayoutType::Scalar2D) {
          c_layout =
              cluster_grid->get_2d_vector_layout(cluster_ordered_fields.size());
        } else {
          c_layout = cluster_grid->get_3d_vector_layout(
              f_layout.tags().back() == LEV, cluster_ordered_fields.size());
        }

        // The units for the monolithic field are nondimensional, cause checking
        // whether all fields in the group have the same units so we can use
        // those is too long and pointless.
        ...
        for (const auto &gn : cluster) {
          if (m_group_requests.at(cluster_grid_name).find(gn) !=
            ...
            for (const auto &req :
                 m_group_requests.at(cluster_grid_name).at(gn)) {
        ...
        // Note: as of 02/2021, idim should *always* be 1, but we store it just
        // in case,
        //       to avoid bugs in the future.
        const auto &C_tags =
            C->get_header().get_identifier().get_layout().tags();
        const int idim = std::distance(C_tags.begin(), ekat::find(C_tags, CMP));
        // See below where we assume idim is 1
        EKAT_REQUIRE_MSG(idim == 1, "Error! idim is assumed to be 1 in "
                                    "FieldManager::registration_ends().\n");
        ...
        for (const auto &fn : cluster_ordered_fields) {
          ...
          EKAT_REQUIRE_MSG(
              fid.get_units() != ekat::units::Units::invalid(),
      ...
      if (ekat::contains(cluster, "__qv__")) {
        // Erase the 'fake group' we added (to guarantee qv would be first/last
        // in the tracers)
      ...
      for (const auto &gn : cluster) {
        ...
        auto first = std::find_first_of(
            cluster_ordered_fields.begin(), cluster_ordered_fields.end(),
            info.m_fields_names.begin(), info.m_fields_names.end());
        auto last = std::next(first, n);

        // Some sanity checks: info.m_fields_names should be a rearrangement of
        // what is in cluster_odered_fields[first,last)
        EKAT_REQUIRE_MSG(first != cluster_ordered_fields.end(),
                         "Error! Could not find any field of this group in the "
                         "ordered cluster.\n"
                         "       Group name: " +
                             gn + "\n");
        EKAT_REQUIRE_MSG(std::distance(last, cluster_ordered_fields.end()) >= 0,
                         "Error! Something went wrong while looking for fields "
                         "of this group in "
                         "the ordered cluster.\n"
                         "       Group name: " +
                             gn + "\n");
        EKAT_REQUIRE_MSG(std::is_permutation(first, last,
                                             info.m_fields_names.begin(),
        ...
        // Update the list of fields in the group info, mark it as
        // monolithically allocated, and update the subfield indices too.
        info.m_fields_names.clear();
        for (auto it = first; it != last; ++it) {
          info.m_fields_names.push_back(*it);
          info.m_subview_dim = 1; // Assumption is checked above
          info.m_subview_idx[*it] =
              std::distance(cluster_ordered_fields.begin(), it);
        }
        info.m_monolithic_allocation = true;

        // The subview indices will need to be corrected in the case of this
        // group being itself a subfile of the cluster field. We are guarenteed
        // that the indices are contiguous, so just calculate the min idx and
        // subtract from all fields indices.
  ...
  for (auto grid_name : m_grids_mgr->get_grid_names()) {
    for (auto &it : m_fields.at(grid_name)) {
      if (it.second->is_allocated()) {
        // If the field has been already allocated, then it was in a bunlded
        // group, so skip it.
        continue;
      }
    ...
    for (const auto &it : m_field_group_info) {
      if (m_group_requests.at(grid_name).find(it.first) !=
          m_group_requests.at(grid_name).end()) {
        ...
        for (const auto &fn : fnames) {
          ...
          m_fields.at(grid_name)
              .at(fn)
              ->get_header()
              .get_tracking()
              .add_to_group(group_info);
        }
      }
    }
  }
...
void FieldManager::add_field(const Field &f) {
  ...
  EKAT_REQUIRE_MSG(
      ...
      "The reason for this is that otherwise we *might* have missed some "
      "inclusion dependency\n"
      "when we allocated the fields for one of those groups.\n");

  // All good, add the field to the repo
  m_fields[grid_name][f.get_header().get_identifier().name()] =
      std::make_shared<Field>(f);

  if (m_repo_state == RepoState::Clean)
    m_repo_state = RepoState::Closed;
}
...
void FieldManager::pre_process_monolithic_group_requests() {
  ...
  for (auto [group_name, group_info] : m_field_group_info) {
    if (not group_info->m_monolithic_allocation)
      continue;
    ...
    for (auto field_name : group_info->m_fields_names) {
      for (auto grid_name : group_info->m_grid_registered.at(field_name)) {
        if (not ekat::contains(grids_in_group, grid_name))
          grids_in_group.push_back(grid_name);
      }
    }
    ...
    for (auto field_name : group_info->m_fields_names) {
      for (auto grid_name : grids_in_group) {
        ...
        for (auto other_grid_name :
             group_info->m_grid_registered.at(field_name)) {
          if (other_grid_name == grid_name)
            continue;

          // Check that we haven't found 2 other grids which registered this
          // field
          EKAT_REQUIRE_MSG(registered_grid == "" or
                               registered_grid == other_grid_name,
                           "Error! Before registration_end(), FieldManager "
                           "requires a field within "
                           "a group to be "
                           ...
                           "  - Field name: " +
                               ...
                               "\n"
                               "  - Grids registered: " +
                               other_grid_name + "," + registered_grid + "\n");
        ...
        const auto src_fid = m_fields.at(registered_grid)
                                 .at(field_name)
                                 ->get_header()
                                 .get_identifier();
        const auto fl = m_grids_mgr->get_grid(grid_name)->equivalent_layout(
            src_fid.get_layout());
