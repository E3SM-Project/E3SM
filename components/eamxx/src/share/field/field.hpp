...
enum HostOrDevice { Device = 0, Host };
...
// view can be reshaped as needed to match a desired layout/datatype for a given
// client.
class Field {
public:
...
public:
  ...
  using get_view_type =
      cond_t<HD == Device, view_dev_t<DT, MT>, view_host_t<DT, MT>>;

  template <typename DT, HostOrDevice HD, typename MT = Kokkos::MemoryManaged>
  using get_strided_view_type = cond_t<HD == Device, strided_view_dev_t<DT, MT>,
                                       strided_view_host_t<DT, MT>>;
  ...
  template <typename ViewT, typename = typename std::enable_if<
                                Kokkos::is_view<ViewT>::value>::type>
  Field(const identifier_type &id, const ViewT &view_d);
  ...
  const std::shared_ptr<header_type> &get_header_ptr() const {
    return m_header;
  }
  ...
  get_view_type<DT, HD> get_view() const;

  // Like the method above, but only for rank-1 fields, returning a view with
  // LayoutStride. This is safer to use for fields that could be a subfield of
  // another one, since a rank-1 view that is the subview of a 2d one along the
  // 2nd index cannot have LayoutRight, and must have LayoutStride instead.
  template <typename DT, HostOrDevice HD = Device>
  ...
  DataType data_type() const {
    return get_header().get_identifier().data_type();
  }
  const std::string &name() const {
    return get_header().get_identifier().name();
  }
  int rank() const { return get_header().get_identifier().get_layout().rank(); }

  // WARNING: this is a power-user method. Its implementation, including
  // assumptions
  //          on pre/post conditions, may change in the future. Use at your own
  //          risk! Read carefully the instructions below.
  // Allows to get a raw pointer (host or device) from the view stored in the
  // field. The user must provide the pointed type for the returned pointer.
  // Such type must either be char or the correct data type of this field.
  // Notice that the view stored may contain more data than just the
  // data of the current field. This can happen in two cases (possibly
  // simultaneously).
  ...
  ST *get_internal_view_data() const {
    ...
    EKAT_REQUIRE_MSG((field_valid_data_types().at<nonconst_ST>() ==
                          m_header->get_identifier().data_type() or
                      std::is_same<nonconst_ST, char>::value),
                     "Error! Attempt to access raw field pointer with the "
                     "wrong scalar type.\n"
                     " - field name: " +
                         name() +
                         "\n"
    ...
    EKAT_REQUIRE_MSG(not m_is_read_only || std::is_const<ST>::value,
                     "Error! Cannot get a non-const raw pointer to the field "
                     "data if the field is read-only.\n"
                     " - field name: " +
                         name() + "\n");
  ...
  //          on pre/post conditions, may change in the future. Use at your own
  //          risk! Read carefully the instructions below.
  // Same as above, but does allows the field to be read-only. This is unsafe as
  // one could alter data that should not be changed. An example of where this
  // is needed is in SurfaceCoupling where exports need to access read-only
  // fields via their view.data ptr.
  template <typename ST, HostOrDevice HD = Device>
  ST *get_internal_view_data_unsafe() const {
    ...
    EKAT_REQUIRE_MSG((std::is_same<nonconst_ST, char>::value or
                      ...
                      (field_valid_data_types().has_t<nonconst_ST>() and
                       get_data_type<nonconst_ST>() ==
                           m_header->get_identifier().data_type())),
                     "Error! Attempt to access raw field pointer with the "
                     "wrong scalar type.\n"
                     " - field name: " +
                         name() +
  ...
  // See share/util/eamxx_combine_ops.hpp for more details on CombineMode
  // options NOTE: ST=void is just so we can give a default to HD,
  ...
  template <CombineMode CM = CombineMode::Update, HostOrDevice HD = Device,
            typename ST = void>
  ...
  void scale(const ST beta) {
    update<CombineMode::Update, HD>(*this, ST(0), beta);
  }
  ...
  //   - If dynamic = true, it is possible to "reset" the slice index (k) at
  //   runtime.
  Field subfield(const std::string &sf_name, const ekat::units::Units &sf_units,
                 const int idim, const int index,
  ...
  Field subfield(const FieldTag tag, const int k,
                 const bool dynamic = false) const;
  // extracts a subfield composed of multiple slices in a continuous range of
  // indices e.g., (in matlab syntax) subf = f.subfield(:, 1:3, :) but NOT subf
  // = f.subfield(:, [1, 3, 4], :)
  Field subfield(const std::string &sf_name, const ekat::units::Units &sf_units,
                 const int idim, const int index_beg,
  ...
  Field subfield(const int idim, const int index_beg,
                 const int index_end) const;
  // If this field is a vector field, get a subfield for the ith component.
  // If dynamic = true, it is possible to "reset" the component index at
  // runtime. Note: throws if this is not a vector field.
  ...
  //  - the fields are subfield of fields aliasing each other, with same
  //  subfield info
  ...
  void initialize_contiguous_helper_field() {
    ...
    EKAT_REQUIRE_MSG(
        not host_and_device_share_memory_space(),
...
protected:
  template <HostOrDevice HD>
  const get_view_type<char *, HD> &get_view_impl() const {
  ...
  get_subview_1(const get_view_type<data_nd_t<T, N>, HD> &v,
                const int k) const {
...
// Inform the compiler that we will instantiate some template methods in some
// translation unit (TU). This prevents the decl in field_impl.hpp from being
// compiled for every TU. NOTE: field_impl.hpp is still included, so you can
// call other specializations (e.g., update for CM=Max)
//       and the compiler will implicitly instantiate. However, these are the
//       most common use cases, so it helps to do ETI for those.
// NOTE: for update, we only specialize for CM being Update, Multiply, and
// Divide,
#define EAMXX_FIELD_ETI_DECL_UPDATE(S, T)                          \
  extern template void Field::update<CombineMode::Update, S, T>(   \
      const Field &, const T, const T);                            \
  extern template void Field::update<CombineMode::Multiply, S, T>( \
      const Field &, const T, const T);                            \
  extern template void Field::update<CombineMode::Divide, S, T>(   \
      const Field &, const T, const T)

#define EAMXX_FIELD_ETI_DECL_UPDATE_IMPL(S, T1, T2)            \
  extern template void                                         \
  Field::update_impl<CombineMode::Update, S, true, T1, T2>(    \
      const Field &, const T1, const T1);                      \
  extern template void                                         \
  Field::update_impl<CombineMode::Multiply, S, true, T1, T2>(  \
      const Field &, const T1, const T1);                      \
  extern template void                                         \
  Field::update_impl<CombineMode::Divide, S, true, T1, T2>(    \
      const Field &, const T1, const T1);                      \
  extern template void                                         \
  Field::update_impl<CombineMode::Update, S, false, T1, T2>(   \
      const Field &, const T1, const T1);                      \
  extern template void                                         \
  Field::update_impl<CombineMode::Multiply, S, false, T1, T2>( \
      const Field &, const T1, const T1);                      \
  extern template void                                         \
  Field::update_impl<CombineMode::Divide, S, false, T1, T2>(   \
      const Field &, const T1, const T1)

#define EAMXX_FIELD_ETI_DECL_DEEP_COPY(S, T)                             \
  extern template void Field::deep_copy_impl<S, true, T>(const T,        \
                                                         const Field &); \
  extern template void Field::deep_copy_impl<S, false, T>(const T,       \
                                                          const Field &)

#define EAMXX_FIELD_ETI_DECL_GET_VIEW(S, T)                                    \
  extern template Field::get_view_type<T, S> Field::get_view<T, S>() const;    \
  extern template Field::get_view_type<T *, S> Field::get_view<T *, S>()       \
      const;                                                                   \
  extern template Field::get_view_type<T **, S> Field::get_view<T **, S>()     \
      const;                                                                   \
  extern template Field::get_view_type<T ***, S> Field::get_view<T ***, S>()   \
  ...
  extern template Field::get_view_type<T ****, S> Field::get_view<T ****, S>() \
      const;                                                                   \
  extern template Field::get_view_type<T *****, S>                             \
  Field::get_view<T *****, S>() const;                                         \
  extern template Field::get_view_type<T ******, S>                            \
  Field::get_view<T ******, S>() const;                                        \
  extern template Field::get_strided_view_type<T, S>                           \
  Field::get_strided_view<T, S>() const;                                       \
  extern template Field::get_strided_view_type<T *, S>                         \
  Field::get_strided_view<T *, S>() const;                                     \
  extern template Field::get_strided_view_type<T **, S>                        \
  Field::get_strided_view<T **, S>() const;                                    \
...
//       all the update/update_impl methods use get_view for all 3 types, so
//       just ETI all of them
