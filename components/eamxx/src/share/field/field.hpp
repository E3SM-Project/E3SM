...
public:
  ...
  template <bool c, typename T, typename F> using cond_t = typename std::conditional<c, T, F>::type;

  template <bool c, typename T = void> using if_t = typename std::enable_if<c, T>::type;
  ...
  template <HostOrDevice HD> using get_device = cond_t<HD == Device, device_t, host_device_t>;
...
private:
  ...
  template <typename DT, typename MT = Kokkos::MemoryManaged> struct dual_view_t {
...
public:
  ...
  template <typename ViewT, typename = typename std::enable_if<Kokkos::is_view<ViewT>::value>::type>
  ...
  template <typename DT, HostOrDevice HD = Device> get_view_type<DT, HD> get_view() const;
  ...
  template <HostOrDevice HD = Device, typename ST = void> void deep_copy(const ST value);
  ...
  template <HostOrDevice HD = Device, typename ST = void> void scale(const ST beta) {
  ...
  inline bool host_and_device_share_memory_space() const {
    EKAT_REQUIRE_MSG(is_allocated(), "Error! Must allocate view before querying "
...
protected:
  template <HostOrDevice HD> const get_view_type<char *, HD> &get_view_impl() const {
  ...
  template <HostOrDevice HD, typename T, int N>
  auto get_ND_view() const -> if_t<(N < MaxRank), get_view_type<data_nd_t<T, N>, HD>>;
  ...
  template <HostOrDevice HD, typename T, int N>
  auto get_ND_view() const -> if_t<(N >= MaxRank + 1), get_view_type<data_nd_t<T, N>, HD>>;
