...
template<CombineMode CM, typename LhsView, typename RhsView, typename ST, typename MaskView = void>
struct CombineViewsHelper {

  using exec_space = typename LhsView::traits::execution_space;

  static constexpr int N = LhsView::rank();
  static constexpr bool masked = not std::is_same_v<MaskView,void>;
  ...
  void operator() (int i) const {
    if constexpr (masked) {
      if constexpr (N==0)
        combine<CM>(rhs(),lhs(),alpha,beta,mask());
      else
        combine<CM>(rhs(i),lhs(i),alpha,beta,mask(i));
    } else {
      if constexpr (N==0)
  ...
  void operator() (int i, int j) const {
    if constexpr (masked)
      combine<CM>(rhs(i,j),lhs(i,j),alpha,beta,mask(i,j));
    else
  ...
  void operator() (int i, int j, int k) const {
    if constexpr (masked)
      combine<CM>(rhs(i,j,k),lhs(i,j,k),alpha,beta,mask(i,j,k));
    else
  ...
  void operator() (int i, int j, int k, int l) const {
    if constexpr (masked)
      combine<CM>(rhs(i,j,k,l),lhs(i,j,k,l),alpha,beta,mask(i,j,k,l));
    else
  ...
  void operator() (int i, int j, int k, int l, int m) const {
    if constexpr (masked)
      combine<CM>(rhs(i,j,k,l,m),lhs(i,j,k,l,m),alpha,beta,mask(i,j,k,l,m));
    else
  ...
  void operator() (int i, int j, int k, int l, int m, int n) const {
    if constexpr (masked)
      combine<CM>(rhs(i,j,k,l,m,n),lhs(i,j,k,l,m,n),alpha,beta,mask(i,j,k,l,m,n));
    else
...
cvh (LhsView lhs, RhsView rhs,
     ...
     MaskView mask)
{
  CombineViewsHelper <CM, LhsView, RhsView,  ST, MaskView> helper;
  helper.lhs = lhs;
  helper.rhs = rhs;
  helper.alpha = alpha;
  helper.beta = beta;
...
template<CombineMode CM, typename ST, typename XST, bool masked>
void Field::
update_impl (const Field& x, const ST alpha, const ST beta, const Field* mask)
{
  ...
  switch (layout.rank()) {
    case 0:
    {
      view_dev_t mv = masked ? mask.get_view<const int>() : {};
...
template<typename ST, bool masked>
