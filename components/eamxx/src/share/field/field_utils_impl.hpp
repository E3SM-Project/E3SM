...
struct SetMaskHelper {
  ...
  using ST = typename std::remove_cv<typename ViewT::traits::value_type>::type;
  ...
  void set_mask (const ST& x_val, int& m_val) {
    if constexpr (CMP==Comparison::EQ)
      m_val = x_val==val;
    else if constexpr (CMP==Comparison::NEQ)
      m_val = x_val!=val;
    else if constexpr (CMP==Comparison::GT)
      m_val = x_val>val;
    else if constexpr (CMP==Comparison::GEQ)
      m_val = x_val>=val;
    else if constexpr (CMP==Comparison::LT)
      m_val = x_val<val;
    else if constexpr (CMP==Comparison::LEQ)
      m_val = x_val<=val;
    else
  ...
  KOKKOS_INLINE_FUNCTION
  void operator() (int i, int j, int k, int l, int n) {
    set_mask (x(i,j,k,l,n),m(i,j,k,l,n));
  }
  KOKKOS_INLINE_FUNCTION
  void operator() (int i, int j, int k, int l, int n, int p) {
...
void compute_mask (const Field& x, const ST value, Field& m)
{
  ...
  switch (layout.rank()) {
    case 0:
      if (contiguous)
        setMaskHelper<CMP>(x.get_view<const ST>(),m.get_view<int>(),value,dims);
      else
        setMaskHelper<CMP>(x.get_strided_view<const ST>(),m.get_view<int>(),value,dims);
      break;
    case 1:
      if (contiguous)
        setMaskHelper<CMP>(x.get_view<const ST*>(),m.get_view<int*>(),value,dims);
