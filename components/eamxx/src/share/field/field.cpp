...
void Field::deep_copy (const Field& x)
{
  ...
  if (data_type()==DataType::IntType) {
    update_impl<CM,int,int>(x,1,0);
  } else if (data_type()==DataType::FloatType) {
    if (x.data_type()==DataType::FloatType)
      update_impl<CM,float,float>(x,1,0);
    else
      update_impl<CM,float,int>(x,1,0);
  } else if (data_type()==DataType::DoubleType) {
    if (x.data_type()==DataType::DoubleType)
      update_impl<CM,double,double>(x,1,0);
    else if (x.data_type()==DataType::FloatType)
      update_impl<CM,double,float>(x,1,0);
    else
      update_impl<CM,double,int>(x,1,0);
  } else {
...
void Field::deep_copy (const ScalarWrapper value, const Field& mask)
{
  ...
  switch (my_data_type) {
    case DataType::IntType:
      deep_copy_impl(value.as<int>(),&mask);
      break;
    case DataType::FloatType:
      deep_copy_impl(value.as<float>(),&mask);
      break;
    case DataType::DoubleType:
      deep_copy_impl(value.as<double>(),&mask);
      break;
    default:
      EKAT_ERROR_MSG ("Error! Unrecognized field data type in Field::deep_copy.\n");
  }
...
void Field::scale (const ScalarWrapper beta)
{
  ...
  if (data_type()==DataType::IntType) {
    auto b = beta.as<int>();
    update_impl<CM,int,int>(x,0,b);
  } else if (data_type()==DataType::FloatType) {
    auto b = beta.as<float>();
    update_impl<CM,float,float>(x,0,b);
  } else if (data_type()==DataType::DoubleType) {
    auto b = beta.as<double>();
    update_impl<CM,double,double>(x,0,b);
  } else {
    EKAT_ERROR_MSG ("Error! Unrecognized/unsupported field data type in Field::update.\n");
  }
}
...
void Field::scale (const Field& x)
{
  ...
  if (data_type()==DataType::IntType) {
    return update_impl<CM,int,int>(x,1,1);
  } else if (data_type()==DataType::FloatType) {
    if (x.data_type()==DataType::FloatType)
      return update_impl<CM,float,float>(x,1,1);
    else
      return update_impl<CM,float,int>(x,1,1);
  } else if (data_type()==DataType::DoubleType) {
    if (x.data_type()==DataType::DoubleType)
      return update_impl<CM,double,double>(x,1,1);
    else if (x.data_type()==DataType::FloatType)
      return update_impl<CM,double,float>(x,1,1);
    else
      return update_impl<CM,double,int>(x,1,1);
  } else {
    EKAT_ERROR_MSG ("Error! Unrecognized/unsupported field data type in Field::update.\n");
  }
}
...
update (const Field& x, const ScalarWrapper alpha, const ScalarWrapper beta)
{
  ...
  if (data_type()==DataType::IntType) {
    auto a = alpha.as<int>();
    auto b = beta.as<int>();
    return update_impl<CM,int,int>(x,a,b);
  } else if (data_type()==DataType::FloatType) {
    auto a = alpha.as<float>();
    auto b = beta.as<float>();
    if (x.data_type()==DataType::FloatType)
      return update_impl<CM,float,float>(x,a,b);
    else
      return update_impl<CM,float,int>(x,a,b);
  } else if (data_type()==DataType::DoubleType) {
    auto a = alpha.as<double>();
    auto b = beta.as<double>();
    if (x.data_type()==DataType::DoubleType)
      return update_impl<CM,double,double>(x,a,b);
    else if (x.data_type()==DataType::FloatType)
      return update_impl<CM,double,float>(x,a,b);
    else
      return update_impl<CM,double,int>(x,a,b);
  } else {
    EKAT_ERROR_MSG ("Error! Unrecognized/unsupported field data type in Field::update.\n");
  }
}
