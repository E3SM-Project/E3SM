...
subfield (const std::string& sf_name, const ekat::units::Units& sf_units,
  ...
  if (has_mask()) {
    ...
    sf.set_mask(mask.subfield(mask.name(),idim,index,dynamic));
  }
...
bool Field::has_mask () const {
  return m_header->has_extra_data("mask");
}
...
Field& Field::create_mask (const std::string& mask_name)
{
  EKAT_REQUIRE_MSG (not has_mask(),
...
Field& Field::get_mask ()
{
  return m_header->get_extra_data<Field>("mask");
}

const Field& Field::get_mask () const
{
  return m_header->get_extra_data<Field>("mask");
}
...
void Field::deep_copy (const ScalarWrapper value)
{
  ...
  switch (my_data_type) {
    case DataType::IntType:
      deep_copy_impl<false>(value.as<int>());
      break;
    case DataType::FloatType:
      deep_copy_impl<false>(value.as<float>());
      break;
    case DataType::DoubleType:
      deep_copy_impl<false>(value.as<double>());
      break;
    default:
...
void Field::deep_copy (const ScalarWrapper value, const Field& mask, const bool negate_mask)
{
  ...
  switch (my_data_type) {
    case DataType::IntType:
      if (negate_mask)
        deep_copy_impl<true,true>(value.as<int>(),&mask);
      else
        deep_copy_impl<true,false>(value.as<int>(),&mask);
      break;
    case DataType::FloatType:
      if (negate_mask)
        deep_copy_impl<true,true>(value.as<float>(),&mask);
      else
        deep_copy_impl<true,false>(value.as<float>(),&mask);
      break;
    case DataType::DoubleType:
      if (negate_mask)
        deep_copy_impl<true,true>(value.as<double>(),&mask);
      else
        deep_copy_impl<true,false>(value.as<double>(),&mask);
      break;
    default:
      EKAT_ERROR_MSG ("Error! Unrecognized field data type in Field::deep_copy.\n");
  }
...
void Field::deep_copy (const Field& x)
{
  ...
  if (data_type()==DataType::IntType) {
    update_impl<false,CM,int,int>(x,1,0);
  } else if (data_type()==DataType::FloatType) {
    if (x.data_type()==DataType::FloatType)
      update_impl<false,CM,float,float>(x,1,0);
    else
      update_impl<false,CM,float,int>(x,1,0);
  } else if (data_type()==DataType::DoubleType) {
    if (x.data_type()==DataType::DoubleType)
      update_impl<false,CM,double,double>(x,1,0);
    else if (x.data_type()==DataType::FloatType)
      update_impl<false,CM,double,float>(x,1,0);
    else
      update_impl<false,CM,double,int>(x,1,0);
  } else {
...
void Field::deep_copy (const Field& x, const Field& mask)
{
  ...
  constexpr auto CM = CombineMode::Replace;
  if (data_type()==DataType::IntType) {
    update_impl<true,CM,int,int>(x,1,1,&mask);
  } else if (data_type()==DataType::FloatType) {
    if (x.data_type()==DataType::FloatType)
      update_impl<true,CM,float,float>(x,1,1,&mask);
    else
      update_impl<true,CM,float,int>(x,1,1,&mask);
  } else if (data_type()==DataType::DoubleType) {
    if (x.data_type()==DataType::DoubleType)
      update_impl<true,CM,double,double>(x,1,1,&mask);
    else if (x.data_type()==DataType::FloatType)
      update_impl<true,CM,double,float>(x,1,1,&mask);
    else
      update_impl<true,CM,double,int>(x,1,1,&mask);
  } else {
    EKAT_ERROR_MSG ("Error! Unrecognized/unsupported field data type in Field::update.\n");
  }
}
...
void Field::scale (const ScalarWrapper beta)
{
  ...
  if (data_type()==DataType::IntType) {
    auto b = beta.as<int>();
    update_impl<false,CM,int,int>(x,0,b);
  } else if (data_type()==DataType::FloatType) {
    auto b = beta.as<float>();
    update_impl<false,CM,float,float>(x,0,b);
  } else if (data_type()==DataType::DoubleType) {
    auto b = beta.as<double>();
    update_impl<false,CM,double,double>(x,0,b);
  } else {
    EKAT_ERROR_MSG ("Error! Unrecognized/unsupported field data type in Field::update.\n");
  }
}
...
void Field::scale (const Field& x)
{
  ...
  if (data_type()==DataType::IntType) {
    return update_impl<false,CM,int,int>(x,1,1);
  } else if (data_type()==DataType::FloatType) {
    if (x.data_type()==DataType::FloatType)
      return update_impl<false,CM,float,float>(x,1,1);
    else
      return update_impl<false,CM,float,int>(x,1,1);
  } else if (data_type()==DataType::DoubleType) {
    if (x.data_type()==DataType::DoubleType)
      return update_impl<false,CM,double,double>(x,1,1);
    else if (x.data_type()==DataType::FloatType)
      return update_impl<false,CM,double,float>(x,1,1);
    else
      return update_impl<false,CM,double,int>(x,1,1);
  } else {
    EKAT_ERROR_MSG ("Error! Unrecognized/unsupported field data type in Field::update.\n");
  }
}
...
void Field::scale (const Field& x, const Field& mask)
{
  ...
  constexpr auto CM = CombineMode::Multiply;
  if (data_type()==DataType::IntType) {
    return update_impl<true,CM,int,int>(x,1,1,&mask);
  } else if (data_type()==DataType::FloatType) {
    if (x.data_type()==DataType::FloatType)
      return update_impl<true,CM,float,float>(x,1,1,&mask);
    else
      return update_impl<true,CM,float,int>(x,1,1,&mask);
  } else if (data_type()==DataType::DoubleType) {
    if (x.data_type()==DataType::DoubleType)
      return update_impl<true,CM,double,double>(x,1,1,&mask);
    else if (x.data_type()==DataType::FloatType)
      return update_impl<true,CM,double,float>(x,1,1,&mask);
    else
      return update_impl<true,CM,double,int>(x,1,1,&mask);
  } else {
    EKAT_ERROR_MSG ("Error! Unrecognized/unsupported field data type in Field::update.\n");
  }
}
...
update (const Field& x, const ScalarWrapper alpha, const ScalarWrapper beta)
{
  ...
  if (data_type()==DataType::IntType) {
    auto a = alpha.as<int>();
    auto b = beta.as<int>();
    return update_impl<false,CM,int,int>(x,a,b);
  } else if (data_type()==DataType::FloatType) {
    auto a = alpha.as<float>();
    auto b = beta.as<float>();
    if (x.data_type()==DataType::FloatType)
      return update_impl<false,CM,float,float>(x,a,b);
    else
      return update_impl<false,CM,float,int>(x,a,b);
  } else if (data_type()==DataType::DoubleType) {
    auto a = alpha.as<double>();
    auto b = beta.as<double>();
    if (x.data_type()==DataType::DoubleType)
      return update_impl<false,CM,double,double>(x,a,b);
    else if (x.data_type()==DataType::FloatType)
      return update_impl<false,CM,double,float>(x,a,b);
    else
      return update_impl<false,CM,double,int>(x,a,b);
  } else {
    EKAT_ERROR_MSG ("Error! Unrecognized/unsupported field data type in Field::update.\n");
  }
}
...
update (const Field& x, const ScalarWrapper alpha, const ScalarWrapper beta, const Field& mask)
{
  ...
  if (data_type()==DataType::IntType) {
    auto a = alpha.as<int>();
    auto b = beta.as<int>();
    return update_impl<true,CM,int,int>(x,a,b,&mask);
  } else if (data_type()==DataType::FloatType) {
    auto a = alpha.as<float>();
    auto b = beta.as<float>();
    if (x.data_type()==DataType::FloatType)
      return update_impl<true,CM,float,float>(x,a,b,&mask);
    else
      return update_impl<true,CM,float,int>(x,a,b,&mask);
  } else if (data_type()==DataType::DoubleType) {
    auto a = alpha.as<double>();
    auto b = beta.as<double>();
    if (x.data_type()==DataType::DoubleType)
      return update_impl<true,CM,double,double>(x,a,b,&mask);
    else if (x.data_type()==DataType::FloatType)
      return update_impl<true,CM,double,float>(x,a,b,&mask);
    else
      return update_impl<true,CM,double,int>(x,a,b,&mask);
  } else {
    EKAT_ERROR_MSG ("Error! Unrecognized/unsupported field data type in Field::update.\n");
  }
}
