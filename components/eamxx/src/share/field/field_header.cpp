...
FieldHeader::FieldHeader(const identifier_type &id)
  ...
  m_alloc_prop =
      std::make_shared<FieldAllocProp>(get_type_size(id.data_type()));
...
bool FieldHeader::is_aliasing(const FieldHeader &rhs) const {
  ...
  if (p != nullptr and rhs_p != nullptr) {
    return p->is_aliasing(*rhs_p) and m_alloc_prop->get_subview_info() ==
                                          rhs.m_alloc_prop->get_subview_info();
  }
...
create_subfield_header(const FieldIdentifier &id,
                       ...
                       const int k, const bool dynamic) {
  // Sanity checks
  ...
  fh->m_alloc_prop = std::make_shared<FieldAllocProp>(
      parent->get_alloc_properties().subview(idim, k, dynamic));
