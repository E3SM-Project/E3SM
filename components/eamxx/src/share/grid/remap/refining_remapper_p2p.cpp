...
RefiningRemapperP2P::~RefiningRemapperP2P() { clean_up(); }
...
void RefiningRemapperP2P::setup_mpi_data_structures() {
  ...
  for (int pid = 0; pid < nranks; ++pid) {
    pids_recv_offsets_h(pid + 1) = pids_recv_offsets_h(pid) + ncols_recv_h(pid);
  }
  ...
  for (int pid = 0; pid < nranks; ++pid) {
    pids_send_offsets_h(pid + 1) = pids_send_offsets_h(pid) + ncols_send_h(pid);
  }
