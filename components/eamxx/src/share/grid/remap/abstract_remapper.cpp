...
AbstractRemapper::AbstractRemapper(const grid_ptr_type &src_grid,
...
void AbstractRemapper::register_field_from_src(const Field &src) {
...
void AbstractRemapper::register_field_from_tgt(const Field &tgt) {
...
void AbstractRemapper::remap_fwd() {
  ...
  if (m_state != RepoState::Clean) {
    ...
    EKAT_REQUIRE_MSG(not m_has_read_only_tgt_fields,
                     "Error! Forward remap IS allowed by this remapper, but "
                     "some of the tgt fields "
                     "are read-only\n");
    remap_fwd_impl();
  }
}
...
void AbstractRemapper::remap_bwd() {
  ...
  if (m_state != RepoState::Clean) {
    ...
    EKAT_REQUIRE_MSG(not m_has_read_only_src_fields,
                     "Error! Backward remap IS allowed by this remapper, but "
                     "some of the src "
...
void AbstractRemapper::set_grids(const grid_ptr_type &src_grid,
                                 const grid_ptr_type &tgt_grid) {
  EKAT_REQUIRE_MSG(src_grid != nullptr,
                   "Error! Invalid source grid pointer.\n");
  EKAT_REQUIRE_MSG(tgt_grid != nullptr,
                   "Error! Invalid target grid pointer.\n");
...
const Field &AbstractRemapper::get_src_field(const int i) const {
  EKAT_REQUIRE_MSG(i >= 0 && i < m_num_fields,
...
FieldIdentifier
AbstractRemapper::create_src_fid(const FieldIdentifier &tgt_fid) const {
...
FieldIdentifier
AbstractRemapper::create_tgt_fid(const FieldIdentifier &src_fid) const {
...
FieldLayout
AbstractRemapper::create_src_layout(const FieldLayout &tgt_layout) const {
  EKAT_REQUIRE_MSG(m_src_grid != nullptr,
                   "Error! Cannot create source layout until the source grid "
                   "has been set.\n");

  EKAT_REQUIRE_MSG(is_valid_tgt_layout(tgt_layout),
                   "[HorizInterpRemapperBase] Error! Input target layout is "
                   "not valid for this remapper.\n"
                   " - input layout: " +
                       tgt_layout.to_string());
...
FieldLayout
AbstractRemapper::create_tgt_layout(const FieldLayout &src_layout) const {
  EKAT_REQUIRE_MSG(m_tgt_grid != nullptr,
                   "Error! Cannot create target layout until the target grid "
                   "has been set.\n");

  EKAT_REQUIRE_MSG(is_valid_src_layout(src_layout),
                   "[HorizInterpRemapperBase] Error! Input source layout is "
                   "not valid for this remapper.\n"
                   " - input layout: " +
                       src_layout.to_string());
...
bool AbstractRemapper::compatible_layouts(const FieldLayout &src,
...
FieldLayout
AbstractRemapper::create_layout(const FieldLayout &from_layout,
