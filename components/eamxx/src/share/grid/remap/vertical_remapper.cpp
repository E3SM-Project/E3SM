    ...
    if (src_layout.has_tag(LEV) or src_layout.has_tag(ILEV)) {
      ...
      if (m_etype_top==Mask or m_etype_bot==Mask) {
        ...
        EKAT_REQUIRE_MSG(not tgt.get_header().has_extra_data("mask_field"),
            "[VerticalRemapper::registration_ends_impl] Error! Target field already has mask field assigned.\n"
      ...
      if (src.get_header().may_be_filled()) {
        ...
        EKAT_REQUIRE_MSG(src.get_header().has_extra_data("mask_field"),
            "[VerticalRemapper::registration_ends_impl] Error! Source field may be filled, but no mask field is set.\n"
