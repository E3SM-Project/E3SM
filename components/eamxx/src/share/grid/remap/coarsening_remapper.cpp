...
void CoarseningRemapper::registration_ends_impl() {
  if (not m_track_mask) {
...
template <int PackSize> void CoarseningRemapper::rescale_masked_fields(const Field &x, const Field &mask) const {
  ...
  case 1: {
    ...
    Kokkos::parallel_for(
        RangePolicy(0, ncols), KOKKOS_LAMBDA(const int &icol) {
...
void CoarseningRemapper::pack_and_send() {
  ...
  for (int ifield = 0; ifield < m_num_fields; ++ifield) {
    ...
    case 1: {
      ...
      Kokkos::parallel_for(
          RangePolicy(0, num_send_gids), KOKKOS_LAMBDA(const int &i) {
...
void CoarseningRemapper::recv_and_unpack() {
  ...
  for (int ifield = 0; ifield < m_num_fields; ++ifield) {
    ...
    case 1: {
      ...
      Kokkos::parallel_for(
          RangePolicy(0, num_tgt_dofs), KOKKOS_LAMBDA(const int &lid) {
...
void CoarseningRemapper::setup_mpi_data_structures() {
  ...
  for (const auto &it : pid2lids_send) {
    ...
    MPI_Send_init(send_ptr, n, mpi_real, pid, 0, mpi_comm, &req);
  }
  ...
  for (int pid = 0; pid < m_comm.size(); ++pid) {
    ...
    MPI_Recv_init(recv_ptr, n, mpi_real, pid, 0, mpi_comm, &req);
  }
}
