...
public:
  ...
  enum TopBot { Top = 1, Bot = 2, TopAndBot = Top | Bot };
  ...
  enum ProfileType { Midpoints, Interfaces, Both };

  // Use fixed value as mask value
  VerticalRemapper(const grid_ptr_type &src_grid, const std::string &map_file);
  ...
  void set_extrapolation_type(const ExtrapType etype,
                              const TopBot where = TopAndBot);
...
public:
  ...
  void extrapolate(const Field &f_src, const Field &f_tgt, const Field &p_src,
...
protected:
  ...
  // We need to remap mid/int fields separately, and we want to use packs if
  // possible, so we need to divide input fields into 4 separate categories
  ...
  std::shared_ptr<ekat::LinInterp<Real, SCREAM_PACK_SIZE>>
