 ...
 *
 * An abstract grid represents a 2d grid, vertically extruded. It needs to be
 ...
 *   - the mapping of dof lid to a set of indices (i_1,...,i_N), with N being
 * the rank of the scalar field layout, and i_k less than the i-th dimension in
 * the scalar field layout. This is how a dof is 'naturally' indexed on this
 * grid.
 *
 ...
 * grid interfaces to retrieve the correct layout, based on known field
 * properties, like spatial dimension or "physical" rank.
 */
...
public:
  ...
  AbstractGrid(const std::string &name, const GridType type,
  ...
  //       for a vector 3d field on a Point grid it will be
  //       (ncols,vector_dim,nlevs)
  FieldLayout get_vertical_layout(const bool midpoints) const;
  FieldLayout get_vertical_layout(
  ...
  virtual FieldLayout
  get_2d_vector_layout(const int vector_dim,
                       const std::string &vec_dim_name) const = 0;
  virtual FieldLayout get_2d_tensor_layout(
  ...
  virtual FieldLayout
  get_3d_vector_layout(const bool midpoints, const int vector_dim,
                       const std::string &vec_dim_name) const = 0;
  virtual FieldLayout get_3d_tensor_layout(
      const bool midpoints, const std::vector<int> &cmp_dims,
      const std::vector<std::string> &cmp_dims_names) const = 0;

  // Some shortcut versions of the above ones, where the name of the
  // vector/tensor components are all equal to e2str(CMP)
  ...
  FieldLayout get_3d_vector_layout(const bool midpoints,
  ...
  // When running with multiple ranks, fields are partitioned across ranks along
  // this FieldTag
  ...
  // NOTE: for some grids, this is the same as get_dofs_gids. The SEGrid is a
  // counterexample:
  //       the dofs are the GLL dofs, but the partitioned dim is the element
  //       dimension
  Field get_partitioned_dim_gids();
  Field get_partitioned_dim_gids() const;

  // Get a Field storing 2d data, where (i,j) entry contains the j-th coordinate
  // of the i-th dof in the native dof layout. Const verison returns a read-only
  // field
  ...
  Field create_geometry_data(
    ...
    return create_geometry_data(
        FieldIdentifier(name, layout, units, this->name(), data_type),
        pack_size);
  }

  // Sets pre-existing field as geometry data.
  // NOTE: setter is const, since we do allow adding new data even if grid is
  // const
  ...
  // Get a list of GIDs that are unique across all ranks in the grid comm. That
  // is, if a dof is present on 2+ ranks, it will (globally) appear just once in
  // the view returned by this method.
  std::vector<gid_type> get_unique_gids() const;

  // For each entry in the input list of GIDs, retrieve the process id that owns
  // it
  ...
  // dofs have been set to something that satisfies any requirement of the grid
  // type.
  ...
  std::string get_special_tag_name(const FieldTag t) const {
    return m_special_tag_names.at(t);
  }

  // This member is used mostly by IO: if a field exists on multiple grids
  // with the same name, IO can use this as a suffix to diambiguate the fields
  // in the IO file, by appending each grid's suffix to the fields names. NOTE:
  // we'd need setter/getter for this, so we might as well make it public
...
protected:
  ...
  // The max/min dof GID across all ranks. Mutable, to allow for lazy
  // calculation
  ...
  mutable gid_type m_global_min_partitioned_dim_gid =
      std::numeric_limits<gid_type>::max();
  mutable gid_type m_global_max_partitioned_dim_gid =
      -std::numeric_limits<gid_type>::max();
  ...
  // For thread safety in modifying mutable items (just in case someone ever
  // runs this code in threaded regions)
  mutable std::mutex m_mutex;

  // The MPI comm containing the ranks across which the global mesh is
  // partitioned
  ekat::Comm m_comm;
};
