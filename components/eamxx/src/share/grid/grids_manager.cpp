...
bool GridsManager::has_grid(const std::string &grid_name) const {
  for (const auto &it : m_grids) {
...
auto GridsManager::create_remapper(const grid_ptr_type &from_grid,
  ...
  EKAT_REQUIRE_MSG(has_grid(from_grid->name()), "Error! Source grid '" +
                                                    from_grid->name() +
                                                    "' is not supported.\n");
  EKAT_REQUIRE_MSG(has_grid(to_grid->name()), "Error! Target grid '" +
                                                  to_grid->name() +
                                                  "' is not supported.\n");
  ...
  EKAT_REQUIRE_MSG(remapper != nullptr,
                   "Error! A remapper from grid '" + from_grid->name() +
...
void GridsManager::add_nonconst_grid(nonconstgrid_ptr_type grid) {
  add_grid(grid);
  m_nonconst_grids[grid->name()] = grid;
}

void GridsManager::add_grid(grid_ptr_type grid) {
...
auto GridsManager::get_grid_nonconst(const std::string &name) const
...
void GridsManager::alias_grid(const std::string &grid_name,
  ...
  if (grid_name == grid_alias or
      ekat::contains(get_grid(grid_name)->aliases(), grid_alias)) {
...
std::set<std::string> GridsManager::get_grid_names() const {
...
std::string GridsManager::print_available_grids() const {
  std::string str;
  if (m_grids.size() == 0) {
    return str;
  }
