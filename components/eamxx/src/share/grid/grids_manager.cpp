...
auto GridsManager::get_grid(const std::string &name) const -> grid_ptr_type {
  ...
  for (const auto &it : m_grids) {
    if (it.second->name() == name or ekat::contains(it.second->aliases(), name)) {
      g = it.second;
      break;
    }
  }

  EKAT_REQUIRE_MSG(g != nullptr, "Something went wrong while looking up a grid.\n"
                                 "  - grids manager: " +
                                     this->name() +
                                     "\n"
                                     "  - grid name    : " +
...
void GridsManager::add_grid(grid_ptr_type grid) {
  ...
  EKAT_REQUIRE_MSG(not has_grid(name), "Error! Cannot add grid: grid already added.\n"
                                       "  - grids manager: " +
                                           this->name() +
                                           "\n"
                                           "  - grid name    : " +
...
void GridsManager::alias_grid(const std::string &grid_name, const std::string &grid_alias) {
  EKAT_REQUIRE_MSG(has_grid(grid_name), "Error! Cannot alias grid: grid not stored.\n"
                                        "  - grids manager: " +
                                            this->name() +
                                            "\n"
                                            "  - grid name    : " +
