...
LatLonGrid::
LatLonGrid (const std::string& grid_name,
 ...
 : AbstractGrid(grid_name,GridType::Point,nglat,nglat*my_nlon,num_vertical_levels,comm)
{
  ...
  m_partitioned_dim_gids = Field(FieldIdentifier("lon_gids",FieldLayout({LON},{m_nlon_local}),units,this->name(),DataType::IntType));
  m_partitioned_dim_gids.allocate_view();
}

FieldLayout
LatLonGrid::get_2d_scalar_layout () const
{
  using namespace ShortFieldTagsNames;

  return FieldLayout({LAT,LON},{m_nlat_global,m_nlon_local}).rename_dims(m_special_tag_names);
}

FieldLayout
LatLonGrid::get_2d_vector_layout (const int vector_dim, const std::string& vec_dim_name) const
{
  using namespace ShortFieldTagsNames;
...
FieldLayout
LatLonGrid::get_2d_tensor_layout (const std::vector<int>& cmp_dims,
...
FieldLayout
LatLonGrid::get_3d_scalar_layout (const bool midpoints) const
{
  using namespace ShortFieldTagsNames;
...
FieldLayout
LatLonGrid::get_3d_vector_layout (const bool midpoints, const int vector_dim,
...
FieldLayout
LatLonGrid::get_3d_tensor_layout (const bool midpoints,
                    ...
                    const ekat::Comm& comm)
{
  ...
  auto grid = std::make_shared<LatLonGrid>(grid_name,nglat,nglon,my_nlon,num_vertical_lev,comm);
