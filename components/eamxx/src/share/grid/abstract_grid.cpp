...
FieldLayout AbstractGrid::get_vertical_layout(const bool midpoints) const {
...
FieldLayout AbstractGrid::get_2d_vector_layout(const int vector_dim) const {
  using namespace ShortFieldTagsNames;
...
bool AbstractGrid::is_valid_layout(const FieldLayout &layout) const {
  using namespace ShortFieldTagsNames;
...
auto AbstractGrid::get_global_min_dof_gid() const -> gid_type {
  std::lock_guard<std::mutex> lock(m_mutex); // Lock the mutex
  // Lazy calculation
...
auto AbstractGrid::get_global_max_dof_gid() const -> gid_type {
  std::lock_guard<std::mutex> lock(m_mutex); // Lock the mutex
  // Lazy calculation
...
Field AbstractGrid::get_dofs_gids() const { return m_dofs_gids.get_const(); }
...
std::list<std::string> AbstractGrid::get_geometry_data_names() const {
...
std::vector<AbstractGrid::gid_type> AbstractGrid::get_unique_gids() const {
...
std::vector<int> AbstractGrid::get_owners(const gid_view_h &gids) const {
...
auto AbstractGrid::get_gid2lid_map() const -> const std::map<gid_type, int> & {
  std::lock_guard<std::mutex> lock(m_mutex); // Lock the mutex
