...
{
  FieldIdentifier fid(name,layout,units,data_type);
...
void AbstractGrid::create_dof_fields (const int scalar2d_layout_rank)
{
  ...
  m_dofs_gids = Field(FieldIdentifier("gids",dof_layout,DataType::IntType));
  m_lid_to_idx = Field(FieldIdentifier("lid2idx",lid2idx_layout,DataType::IntType));
