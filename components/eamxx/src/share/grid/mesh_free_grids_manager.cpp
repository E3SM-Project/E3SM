...
MeshFreeGridsManager::MeshFreeGridsManager(const ekat::Comm &comm, const ekat::ParameterList &p)
...
void MeshFreeGridsManager::build_grids() {
  ...
  for (const auto &gname : names) {
    ...
    if (type == "se_grid") {
      ...
      EKAT_ERROR_MSG("[MeshFreeGridsManager::build_grids] Unrecognized grid type.\n"
                     " - grid name: " +
                     gname +
                     "\n"
                     " - grid type: " +
...
void MeshFreeGridsManager::build_se_grid(const std::string &name, ekat::ParameterList &params) {
...
void MeshFreeGridsManager::add_geo_data(const nonconstgrid_ptr_type &grid) const {
  ...
  if (geo_data_source == "CREATE_EMPTY_DATA") {
    ...
    if (scorpio::has_var(filename, "lat") && scorpio::has_var(filename, "lon")) {
      load_lat_lon(grid, filename);
    }

    if (scorpio::has_var(filename, "hyam") && scorpio::has_var(filename, "hybm") &&
        scorpio::has_var(filename, "hyai") && scorpio::has_var(filename, "hybi")) {
...
void MeshFreeGridsManager::load_lat_lon(const nonconstgrid_ptr_type &grid,
  ...
  std::map<std::string, geo_view_host> host_views = {{"lat", lat.get_view<Real *, Host>()},
...
void MeshFreeGridsManager::load_vertical_coordinates(const nonconstgrid_ptr_type &grid,
                                                     const std::string &filename) const {
  using geo_view_host = AtmosphereInput::view_1d_host;
  ...
  // Create host mirrors for reading in data
  std::map<std::string, geo_view_host> host_views = {{"hyam", hyam.get_view<Real *, Host>()},
