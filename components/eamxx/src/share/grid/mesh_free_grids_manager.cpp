...
MeshFreeGridsManager::MeshFreeGridsManager(const ekat::Comm &comm,
                                           const ekat::ParameterList &p)
    : m_params(p), m_comm(comm) {
  // Nothing else to do here
}

MeshFreeGridsManager::remapper_ptr_type
MeshFreeGridsManager::do_create_remapper(
    const grid_ptr_type /* from_grid */,
    const grid_ptr_type /* to_grid */) const {
  EKAT_ERROR_MSG("Error! MeshFreeGridsManager does not offer any remapper.\n");
  return nullptr;
}

void MeshFreeGridsManager::build_grids() {
...
void MeshFreeGridsManager::build_se_grid(const std::string &name,
  ...
  se_grid = std::make_shared<SEGrid>(name, num_local_elems, num_gp,
                                     num_vertical_levels, m_comm);
  ...
  auto host_elems =
      elem_gids.template get_view<AbstractGrid::gid_type *, Host>();
...
void MeshFreeGridsManager::build_point_grid(const std::string &name,
                                            ekat::ParameterList &params) {
  const int num_global_cols     = params.get<int>("number_of_global_columns");
  const int num_vertical_levels = params.get<int>("number_of_vertical_levels");
  auto pt_grid =
      create_point_grid(name, num_global_cols, num_vertical_levels, m_comm);

  const auto units = ekat::units::Units::nondimensional();

  auto area = pt_grid->create_geometry_data(
      "area", pt_grid->get_2d_scalar_layout(), units);
...
void MeshFreeGridsManager::add_geo_data(
    const nonconstgrid_ptr_type &grid) const {
  if (!m_params.isParameter("geo_data_source")) {
    return;
  }

  // Load geo data fields if geo data filename is present, and file contains
  // them
  ...
  if (geo_data_source == "CREATE_EMPTY_DATA") {
    ...
    auto lat =
        grid->create_geometry_data("lat", grid->get_2d_scalar_layout(), units);
    auto lon =
        grid->create_geometry_data("lon", grid->get_2d_scalar_layout(), units);
...
void MeshFreeGridsManager::load_lat_lon(const nonconstgrid_ptr_type &grid,
  ...
  lat_lon_reader_pl.set<std::vector<std::string>>("field_names",
                                                  {"lat", "lon"});
...
void MeshFreeGridsManager::load_vertical_coordinates(
    const nonconstgrid_ptr_type &grid, const std::string &filename) const {
  using geo_view_host = AtmosphereInput::view_1d_host;
  ...
  vcoord_reader_pl.set<std::vector<std::string>>(
      "field_names", {"hyam", "hybm", "hyai", "hybi"});
  ...
  for (auto f : {hyam, hybm, hyai, hybi}) {
    ...
    EKAT_REQUIRE_MSG(nan_check.result == CheckResult::Pass,
                     "ERROR! NaN values detected in " + f.name() + " field.\n" +
                         nan_check.msg);
    auto interval_check =
        std::make_shared<FieldWithinIntervalCheck>(f, grid, 0, 1)->check();
    EKAT_REQUIRE_MSG(nan_check.result == CheckResult::Pass,
                     "ERROR! Field " + f.name() +
                         " has values not in [0,1].\n" + nan_check.msg);
  }
#endif
}
