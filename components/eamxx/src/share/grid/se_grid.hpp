...
public:
  // Constructor
  SEGrid(const std::string &grid_name, const int num_my_elements,
...
protected:
  ...
  // The max/min elem GID across all ranks. Mutable, to allow for lazy
  // calculation
  mutable gid_type m_global_min_elem_gid = std::numeric_limits<gid_type>::max();
  mutable gid_type m_global_max_elem_gid =
