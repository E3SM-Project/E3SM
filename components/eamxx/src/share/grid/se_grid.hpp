...
public:
  // Constructor
  SEGrid(const std::string &grid_name, const int num_my_elements, const int num_gauss_pts,
  ...
  FieldLayout get_3d_tensor_layout(const bool midpoints, const std::vector<int> &cmp_dims,
                                   const std::vector<std::string> &cmp_names) const override;

  FieldTag get_partitioned_dim_tag() const override { return FieldTag::Element; }
  int get_partitioned_dim_local_size() const override { return m_num_local_elem; }
  int get_partitioned_dim_global_size() const override { return m_num_global_elem; }
