...
IOPDataManager::IOPDataManager(const ekat::Comm &comm,
                               const ekat::ParameterList &params,
                               const util::TimeStamp &run_t0,
                               const int model_nlevs, const Field &hyam,
                               const Field &hybm) {
  m_comm   = comm;
  m_params = params;
  EKAT_REQUIRE_MSG(
      m_params.get<bool>("doubly_periodic_mode", false),
      "Error! Currently doubly_periodic_mode is the only use case for "
      "intensive observation period files.\n");

  EKAT_REQUIRE_MSG(
      m_params.isParameter("target_latitude") &&
  ...
  if (not m_params.isParameter("iop_srf_prop"))
    m_params.set<bool>("iop_srf_prop", false);
  if (not m_params.isParameter("iop_dosubsidence"))
    m_params.set<bool>("iop_dosubsidence", false);
  if (not m_params.isParameter("iop_coriolis"))
    m_params.set<bool>("iop_coriolis", false);
  if (not m_params.isParameter("iop_nudge_tq"))
    m_params.set<bool>("iop_nudge_tq", false);
  if (not m_params.isParameter("iop_nudge_uv"))
    m_params.set<bool>("iop_nudge_uv", false);
  if (not m_params.isParameter("iop_nudge_tq_low"))
    m_params.set<Real>("iop_nudge_tq_low", 1050);
  if (not m_params.isParameter("iop_nudge_tq_high"))
    m_params.set<Real>("iop_nudge_tq_high", 0);
  if (not m_params.isParameter("iop_nudge_tscale"))
    m_params.set<Real>("iop_nudge_tscale", 10800);
  if (not m_params.isParameter("zero_non_iop_tracers"))
    m_params.set<bool>("zero_non_iop_tracers", false);
...
IOPDataManager::~IOPDataManager() {
  const auto iop_file = m_params.get<std::string>("iop_file");
  scorpio::release_file(iop_file);
}

void IOPDataManager::initialize_iop_file(const util::TimeStamp &run_t0,
                                         int model_nlevs) {
  EKAT_REQUIRE_MSG(
      m_params.isParameter("iop_file"),
  ...
  // All the calls to register_file made on-the-fly inside has_var will be
  // no-op.
  scorpio::register_file(iop_file, scorpio::FileMode::Read);

  // Lambda for allocating space and storing information for potential iop
  // fields. Inputs:
  //   - varnames:    Vector of possible variable names in the iop file.
  //                  First entry will be the variable name used when accessing
  //                  in class
  //   - fl:          IOP field layout (acceptable ranks: 0, 1)
  //   - srf_varname: Name of surface variable potentially in iop file
  //   associated with iop variable.
  auto setup_iop_field = [&, this](const std::vector<std::string> &varnames,
    ...
    if (has_var) {
      ...
      if (iop_varname != file_varname)
        m_iop_file_varnames.insert({iop_varname, file_varname});
      ...
      EKAT_REQUIRE_MSG(field_rank <= 1, "Error! Unexpected field rank " +
                                            std::to_string(field_rank) +
                                            " for iop file fields.\n");
      Field field(fid);
      if (fl.has_tag(FieldTag::LevelMidPoint) or
          fl.has_tag(FieldTag::LevelInterface)) {
  ...
  // Check for large scale winds and enfore "all-or-nothing" for u and v
  // component
  ...
  if (m_params.get<bool>("iop_coriolis")) {
    EKAT_REQUIRE_MSG(
        m_params.get<bool>("use_large_scale_wind"),
        "Error! Large scale winds required for coriolis forcing.\n");
  }

  // If we have the vertical component of T/Q forcing, define 3d forcing as a
  // computed field.
  ...
  const bool both_3d_forcing =
      (has_iop_field("divT3d") and has_iop_field("divq3d"));
  const bool neither_3d_forcing =
      (not(has_iop_field("divT3d") or has_iop_field("divq3d")));
  EKAT_REQUIRE_MSG(both_3d_forcing or neither_3d_forcing,
                   "Error! Either T and q both have 3d forcing, or neither "
                   "have 3d forcing.\n");
  ...
  if (scorpio::has_var(iop_file, "bdate"))
    bdate_name = "bdate";
  else if (scorpio::has_var(iop_file, "basedate"))
    bdate_name = "basedate";
  else if (scorpio::has_var(iop_file, "nbdate"))
    bdate_name = "nbdate";
  else
    EKAT_ERROR_MSG("Error! No valid name for bdate in " + iop_file + ".\n");
  ...
  if (scorpio::has_dim(iop_file, "time"))
    time_dimname = "time";
  else if (scorpio::has_dim(iop_file, "tsec"))
    time_dimname = "tsec";
  else
    EKAT_ERROR_MSG("Error! No valid dimension for tsec in " + iop_file + ".\n");
  ...
  // we call the record dimension "time" (since that's what it virtually always
  // is)
  ...
  m_time_info.iop_file_times_in_sec =
      view_1d_host<int>("iop_file_times", ntimes);
  for (int t = 0; t < ntimes; ++t) {
    scorpio::read_var(iop_file, "tsec", &m_time_info.iop_file_times_in_sec(t),
                      t);
  }

  // Check that lat/lon from iop file match the targets in parameters. Note that
  // longitude may be negtive in the iop file, we convert to positive before
  // checking.
  ...
  const Real rel_lat_err =
      std::fabs(iop_file_lat - m_params.get<Real>("target_latitude")) /
      std::max(std::fabs(m_params.get<Real>("target_latitude")), (Real)0.1);
  const Real rel_lon_err =
      std::fabs(std::fmod(iop_file_lon + 360.0, 360.0) -
  ...
  EKAT_REQUIRE_MSG(rel_lat_err < std::numeric_limits<float>::epsilon(),
                   "Error! IOP file variable \"lat\" does not match "
                   "target_latitude from IOP parameters.\n");
  EKAT_REQUIRE_MSG(rel_lon_err < std::numeric_limits<float>::epsilon(),
                   "Error! IOP file variable \"lon\" does not match "
                   "target_longitude from IOP parameters.\n");
  ...
  iop_file_pressure.get_header().get_alloc_properties().request_allocation(
      Pack::n);
  ...
  for (int ilev = 0; ilev < file_levs; ++ilev)
    data[ilev] /= 100;
  ...
  model_pressure.get_header().get_alloc_properties().request_allocation(
      Pack::n);
  model_pressure.allocate_view();
  m_helper_fields.insert({"model_pressure", model_pressure});
}

void IOPDataManager::setup_io_info(const std::string &file_name,
                                   const grid_ptr &grid) {
...
void IOPDataManager::read_fields_from_file_for_iop(
  ...
  EKAT_REQUIRE_MSG(fields.size() > 0,
                   "[IOPDataManager::read_fields_from_file_for_iop] Error! "
                   "Input fields list is empty.\n");
  ...
  if (grid->name() == "physics_gll" and scorpio::has_dim(file_name, "ncol_d")) {
    // If we are on GLL grid, and nc file contains "ncol_d" dimension, we need
    // to reset COL dim tag
  ...
  // Read remapper src fields (which may have different layout than fields in
  // input vector)
...
void IOPDataManager::read_iop_file_data(const util::TimeStamp &current_ts) {
  ...
  EKAT_REQUIRE_MSG(
      iop_file_time_idx >= m_time_info.time_idx_of_current_data,
      "Error! Attempting to read previous iop file data time index.\n");
  if (iop_file_time_idx == m_time_info.time_idx_of_current_data)
    return;
  ...
  // Compute values and indices associate with pressure for interpolating data
  // (if necessary).
  ...
  if (has_level_data) {
    ...
    Kokkos::parallel_reduce(
        file_levs + 1,
        KOKKOS_LAMBDA(const int ilev, int &lmin) {
          if (ilev == file_levs) {
    ...
    const auto model_nlevs =
        model_pressure.get_header().get_identifier().get_layout().dim(0);
    ...
    Kokkos::parallel_reduce(
        adjusted_file_levs,
        KOKKOS_LAMBDA(const int &ilev, int &lmax, int &lmin) {
          if (iop_file_pres_v(ilev) <= model_pres_v(0) && ilev > lmax) {
            lmax = ilev;
          }
          if (iop_file_pres_v(ilev) >= model_pres_v(model_nlevs - 1) &&
              ilev + 1 < lmin) {
            lmin = ilev + 1;
          }
        },
    ...
    // If no file pressures are found outide the reference pressure range, set
    // to file level endpoints
    if (iop_file_start == Kokkos::reduction_identity<int>::max())
      iop_file_start = 0;
    if (iop_file_end == Kokkos::reduction_identity<int>::min())
      iop_file_end = adjusted_file_levs;

    // Find model pressure levels just inside range of file pressure levels
    Kokkos::parallel_reduce(
        model_nlevs,
        KOKKOS_LAMBDA(const int &ilev, int &lmin, int &lmax) {
          if (model_pres_v(ilev) >= iop_file_pres_v(iop_file_start) &&
              ilev < lmin) {
            lmin = ilev;
          }
          if (model_pres_v(ilev) <= iop_file_pres_v(iop_file_end - 1) &&
              ilev + 1 > lmax) {
            lmax = ilev + 1;
          }
        },
        Kokkos::Min<int>(model_start), Kokkos::Max<int>(model_end));

    // If not reference pressures are found inside file pressures, set to model
    // level endpoints
    if (model_start == Kokkos::reduction_identity<int>::min())
      model_start = model_nlevs - 1;
    if (model_end == Kokkos::reduction_identity<int>::max())
      model_end = 1;
  }
  ...
  for (auto &it : m_iop_fields) {
    ...
    if (m_iop_field_type.at(fname) == IOPFieldType::Computed)
      continue;

    // File may use different varname than IOP class
    auto file_varname = (m_iop_file_varnames.count(fname) > 0)
                            ? m_iop_file_varnames[fname]
                            : fname;
    ...
    if (field.rank() == 0) {
      ...
      // value for surface.
      FieldIdentifier fid(
          file_varname + "_iop_file",
          FieldLayout({FieldTag::LevelMidPoint}, {adjusted_file_levs}),
          ekat::units::Units::nondimensional(), "");
      Field iop_file_field(fid);
      iop_file_field.get_header().get_alloc_properties().request_allocation(
          Pack::n);
      ...
      for (int ilev = 0; ilev < adjusted_file_levs - 1; ++ilev)
        iop_file_v_h(ilev) = data[ilev];
      ...
      if (has_srf) {
        ...
        scorpio::read_var(iop_file, srf_varname,
                          &iop_file_v_h(adjusted_file_levs - 1),
                          iop_file_time_idx);
      } else {
        // No surface value exists, compute surface value
        const auto dx = iop_file_v_h(adjusted_file_levs - 2) -
                        iop_file_v_h(adjusted_file_levs - 3);
        if (dx == 0)
          iop_file_v_h(adjusted_file_levs - 1) =
              iop_file_v_h(adjusted_file_levs - 2);
        else {
          iop_file_pressure.sync_to_host();
          const auto iop_file_pres_v_h =
              iop_file_pressure.get_view<const Real *, Host>();
          const auto dy = iop_file_pres_v_h(adjusted_file_levs - 2) -
                          iop_file_pres_v_h(adjusted_file_levs - 3);
          const auto scale = dy / dx;

          iop_file_v_h(adjusted_file_levs - 1) =
              (iop_file_pres_v_h(adjusted_file_levs - 1) -
      ...
      const auto total_nlevs =
          field.get_header().get_identifier().get_layout().dim(0);

      ekat::LinInterp<Real, Pack1d::n> vert_interp(1, nlevs_input,
                                                   nlevs_output);
      ...
      Kokkos::parallel_for(
          policy, KOKKOS_LAMBDA(const KT::MemberType &team) {
            const auto x_src = Kokkos::subview(
                iop_file_pres_v,
                Kokkos::pair<int, int>(iop_file_start, iop_file_end));
            const auto x_tgt = Kokkos::subview(
                model_pres_v, Kokkos::pair<int, int>(model_start, model_end));
            const auto input = Kokkos::subview(
                iop_file_v,
                Kokkos::pair<int, int>(iop_file_start, iop_file_end));
            const auto output = Kokkos::subview(
                iop_field_v, Kokkos::pair<int, int>(model_start, model_end));
      ...
      if (fname == "T" || fname == "q" || fname == "u" || fname == "u_ls" ||
          fname == "v" || fname == "v_ls") {
        Kokkos::parallel_for(
  ...
  if (has_iop_field("divT3d")) {
    if (m_iop_field_type.at("divT3d") == IOPFieldType::Computed) {
      ...
      const auto nlevs    = get_iop_field("divT3d")
                             .get_header()
                             .get_identifier()
                             .get_layout()
                             .dim(0);
      Kokkos::parallel_for(
          nlevs, KOKKOS_LAMBDA(const int ilev) {
  ...
  if (has_iop_field("divq3d")) {
    if (m_iop_field_type.at("divq3d") == IOPFieldType::Computed) {
      ...
      const auto nlevs    = get_iop_field("divq3d")
                             .get_header()
                             .get_identifier()
                             .get_layout()
                             .dim(0);
...
void IOPDataManager::set_fields_from_iop_data(const field_mgr_ptr field_mgr,
  ...
  if (m_params.get<bool>("zero_non_iop_tracers") &&
      field_mgr->has_group("tracers", grid_name)) {
    // Zero out all tracers before setting iop tracers (if requested)
    field_mgr->get_field_group("tracers", grid_name)
        .m_monolithic_field->deep_copy(0);
  }
  ...
  const bool set_ps =
      field_mgr->has_field("ps", grid_name) && has_iop_field("Ps");
  const bool set_T_mid =
      field_mgr->has_field("T_mid", grid_name) && has_iop_field("T");
  ...
  const bool set_qv =
      field_mgr->has_field("qv", grid_name) && has_iop_field("q");
  const bool set_nc =
      field_mgr->has_field("nc", grid_name) && has_iop_field("NUMLIQ");
  const bool set_qc =
      field_mgr->has_field("qc", grid_name) && has_iop_field("CLDLIQ");
  const bool set_qi =
      field_mgr->has_field("qi", grid_name) && has_iop_field("CLDICE");
  const bool set_ni =
      field_mgr->has_field("ni", grid_name) && has_iop_field("NUMICE");
  ...
  if (set_horiz_winds_u || set_horiz_winds_v) {
    horiz_winds =
        field_mgr->get_field("horiz_winds", grid_name).get_view<Real ***>();
    if (set_horiz_winds_u)
      u_iop = get_iop_field("u").get_view<Real *>();
    if (set_horiz_winds_v)
      v_iop = get_iop_field("v").get_view<Real *>();
  }
  if (set_qv) {
  ...
  const auto ncols =
      field_mgr->get_grids_manager()->get_grid(grid_name)->get_num_local_dofs();
  const auto nlevs = field_mgr->get_grids_manager()
                         ->get_grid(grid_name)
...
void IOPDataManager::correct_temperature_and_water_vapor(
    const field_mgr_ptr field_mgr, const std::string &grid_name) {
  ...
  if (first_valid_idx > 0) {
    // If we have values of T and q to correct, we must have both T_mid and qv
    // as FM fields
