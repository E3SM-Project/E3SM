...
void IOPDataManager::initialize_iop_file(const util::TimeStamp &run_t0, int model_nlevs) {
  ...
  auto setup_iop_field = [&, this](const std::vector<std::string> &varnames, const FieldLayout &fl,
...
void IOPDataManager::setup_io_info(const std::string &file_name, const grid_ptr &grid) {
...
void IOPDataManager::read_iop_file_data(const util::TimeStamp &current_ts) {
  ...
  for (auto &it : m_iop_fields) {
    if (it.second.rank() == 1 and m_iop_field_type.at(it.first) == IOPFieldType::FromFile) {
      has_level_data = true;
      break;
    }
  }
  ...
  for (auto &it : m_iop_fields) {
    ...
    if (field.rank() == 0) {
      ...
      if (has_srf) {
        ...
        if (dx == 0)
          iop_file_v_h(adjusted_file_levs - 1) = iop_file_v_h(adjusted_file_levs - 2);
        else {
...
void IOPDataManager::set_fields_from_iop_data(const field_mgr_ptr field_mgr, const std::string &grid_name) {
  ...
  EKAT_REQUIRE_MSG(grid_name == "physics_gll", "Error! Attempting to set non-GLL fields using "
...
void IOPDataManager::correct_temperature_and_water_vapor(const field_mgr_ptr field_mgr, const std::string &grid_name) {
  ...
  if (first_valid_idx > 0) {
    ...
    Kokkos::parallel_for(
        Kokkos::RangePolicy<>(0, first_valid_idx), KOKKOS_LAMBDA(const int ilev) {
