...
void AtmosphereProcess::set_field (const Field& f)
{
  ...
  for (const auto& r : m_field_requests) {
    if (r.fid==f.get_header().get_identifier()) {
      usage = r.usage;
      break;
    }
  }
  ...
  // Dereference the right ptr for the derived class, so they get the field as read-only if
  // the usage did not include Compute
  set_field_impl (ptr_out ? *ptr_out : *ptr_in);
...
void AtmosphereProcess::set_group (const FieldGroup& group)
{
  ...
  // Pass *ptr to the derived class, so they get the group as read-only if
  // the usage did not include Compute
  set_group_impl(*ptr);
...
void AtmosphereProcess::init_step_tendencies () {
  ...
  for (auto& [fn_gn,f_beg] : m_start_of_step_fields) {
    ...
    const auto& f     = get_field(fname,gname);
    f_beg.deep_copy(f);
  }
...
void AtmosphereProcess::compute_step_tendencies () {
  ...
  for (auto& [fn_gn,tend] : m_proc_tendencies) {
    ...
    const auto& f_end = get_field(fname,gname);
...
has_field_request(const FieldIdentifier& fid, const int usage_mask) const
{
  return has_field_request(fid.name(),fid.get_grid_name(),usage_mask);
}

bool AtmosphereProcess::
has_field_request(const std::string& name, const std::string& grid, const int usage_mask) const
{
  for (const auto& r : m_field_requests) {
...
bool AtmosphereProcess::
has_group_request(const std::string& name, const std::string& grid, const int usage_mask) const
{
  for (const auto& r : m_group_requests) {
...
get_field(const std::string& field_name, const std::string& grid_name)
{
  EKAT_REQUIRE_MSG (m_fields_pointers.count(field_name)==1 and
                    m_fields_pointers[field_name].count(grid_name)==1,
    "Error! Could not locate input field in this atm proces.\n"
    "   atm proc name: " + this->name() + "\n"
    "   field name: " + field_name + "\n"
    "   grid name: " + grid_name + "\n");
  return *m_fields_pointers.at(field_name).at(grid_name);
}

Field& AtmosphereProcess::
get_field(const std::string& field_name)
{
  EKAT_REQUIRE_MSG (m_fields_pointers.count(field_name)==1,
      "Error! Could not locate input field in this atm proces.\n"
      "   atm proc name: " + this->name() + "\n"
      "   field name: " + field_name + "\n");
  EKAT_REQUIRE_MSG (m_fields_pointers[field_name].size()==1,
    "Error! Attempt to find input field providing only the name,\n"
    "       but multiple copies (on different grids) are present.\n"
    "  field name: " + field_name + "\n"
    "  atm process: " + this->name() + "\n"
    "  number of copies: " + std::to_string(m_fields_pointers[field_name].size()) + "\n");
  return *m_fields_pointers.at(field_name).begin()->second;
}

FieldGroup& AtmosphereProcess::
get_group(const std::string& group_name, const std::string& grid_name)
{
  EKAT_REQUIRE_MSG (m_groups_pointers.count(group_name)==1 and
                    m_groups_pointers[group_name].count(grid_name)==1,
    "Error! Could not locate input group in this atm proces.\n"
    "   atm proc name: " + this->name() + "\n"
    "   group name: " + group_name + "\n"
    "   grid name: " + grid_name + "\n");
  return *m_groups_pointers.at(group_name).at(grid_name);
}

FieldGroup& AtmosphereProcess::
get_group(const std::string& group_name)
{
  EKAT_REQUIRE_MSG (m_groups_pointers.count(group_name)==1,
      "Error! Could not locate input group in this atm proces.\n"
      "   atm proc name: " + this->name() + "\n"
      "   group name: " + group_name + "\n");
  EKAT_REQUIRE_MSG (m_groups_pointers[group_name].size()==1,
    "Error! Attempt to find input group providing only the name,\n"
    "       but multiple copies (on different grids) are present.\n"
    "  group name: " + group_name + "\n"
    "  atm process: " + this->name() + "\n"
    "  number of copies: " + std::to_string(m_groups_pointers[group_name].size()) + "\n");
  return *m_groups_pointers.at(group_name).begin()->second;
}

Field& AtmosphereProcess::
get_internal_field(const std::string& field_name, const std::string& grid_name)
{
  EKAT_REQUIRE_MSG (m_internal_fields_pointers.count(field_name)==1 and
                    m_internal_fields_pointers[field_name].count(grid_name)==1,
...
get_internal_field(const std::string& field_name) {
  EKAT_REQUIRE_MSG (m_internal_fields_pointers.count(field_name)==1,
      "Error! Could not locate internal field in this atm proces.\n"
      "   atm proc name: " + this->name() + "\n"
      "   field name: " + field_name + "\n");
  EKAT_REQUIRE_MSG (m_internal_fields_pointers[field_name].size()==1,
    "Error! Attempt to find internal field providing only the name,\n"
    "       but multiple copies (on different grids) are present.\n"
    "  field name: " + field_name + "\n"
    "  atm process: " + this->name() + "\n"
    "  number of copies: " + std::to_string(m_internal_fields_pointers[field_name].size()) + "\n");
  return *m_internal_fields_pointers.at(field_name).begin()->second;
}
...
add_precondition_check (const prop_check_ptr& pc, const CheckFailHandling cfh)
{
  ...
  for (const auto& ptr : pc->repairable_fields()) {
    ...
    EKAT_REQUIRE_MSG ( has_field_request(fid.name(),fid.get_grid_name(),Computed) or
                       has_group_request(fid.name(),fid.get_grid_name(),Computed),
...
void AtmosphereProcess::
alias_field (const std::string& field_name,
             const std::string& grid_name,
             const std::string& alias_name)
{
  try {
    m_fields_pointers[alias_name][grid_name] = m_fields_pointers.at(field_name).at(grid_name);
...
void AtmosphereProcess::
alias_group (const std::string& group_name,
             const std::string& grid_name,
             const std::string& alias_name)
{
  try {
    m_groups_pointers[alias_name][grid_name] = m_groups_pointers.at(group_name).at(grid_name);
  } catch (const std::out_of_range&) {
    // std::out_of_range message would not help detecting where
    // the exception originated, so print a more meaningful message.
    EKAT_ERROR_MSG (
...
void AtmosphereProcess::
remove_field (const std::string& field_name, const std::string& grid_name, const int usage_mask)
{
  ...
  // rmf(m_fields_in);
  // rmf(m_fields_out);
  // rmf(m_internal_fields);
}

void AtmosphereProcess::
remove_group (const std::string& group_name, const std::string& grid_name, const int usage_mask)
{
  const auto rmg = [&] (auto& groups_list) {
