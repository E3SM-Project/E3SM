...
void AtmosphereDiagnostic::run_impl(const double dt) { compute_diagnostic(dt); }

void AtmosphereDiagnostic::set_required_field_impl(const Field &f) {
    ...
    if (it.fid.name() == f.name()) {
      ...
      EKAT_REQUIRE_MSG(fap.get_largest_pack_size() >= it.pack_size,
                       ...
                       "  - diag field: " +
                           ...
                           "\n"
                           "  - requested pack size: " +
                           std::to_string(it.pack_size) +
                           "\n"
                           "  - field max pack size: " +
                           std::to_string(fap.get_largest_pack_size()) + "\n");
      break;
    }
  }
}
