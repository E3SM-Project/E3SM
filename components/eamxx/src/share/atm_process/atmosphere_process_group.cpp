...
void AtmosphereProcessGroup::add_postcondition_nan_checks () {
  for (auto proc : m_atm_processes) {
    proc->add_postcondition_nan_checks();
  }
}
...
set_required_group_impl (const FieldGroup& group)
{
  ...
  // Now add all fields individually (including monolithic one)
  // This will automatically remove from the inputs FM fields that are computed
  // by an atm proc of the group before being required by another
  if (group.m_monolithic_field) {
    set_required_field_impl(*group.m_monolithic_field);
  }

  for (const auto& [name,fptr] : group.m_individual_fields) {
    set_required_field_impl(*fptr);
  }
}
