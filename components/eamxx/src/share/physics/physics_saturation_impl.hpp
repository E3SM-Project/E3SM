...
KOKKOS_FUNCTION
void  Functions<S,D>
::check_temperature(const Pack& t_atm, const char* caller, const Mask& range_mask)
{
...
Functions<S,D>::MurphyKoop_svp(const Pack& t_atm, const bool ice, const Mask& range_mask, const char* caller)
{
  ...
  const Mask ice_mask = (t_atm < tmelt) && ice;
  const Mask liq_mask = !ice_mask;
  ...
  if (ice_mask.any()) {
    ...
    const Pack ice_result = exp(ic[0] - (ic[1] / t_atm) + (ic[2] * log(t_atm)) - (ic[3] * t_atm));
  ...
  if (liq_mask.any()) {
    ...
    const Pack liq_result = exp(lq[0] - (lq[1] / t_atm) - (lq[2] * logt) + (lq[3] * t_atm) +
...
KOKKOS_FUNCTION
typename Functions<S,D>::Pack
Functions<S,D>::polysvp1(const Pack& t, const bool ice, const Mask& range_mask, const char* caller)
{
  ...
  const Pack dt = max(t - sp(273.15), sp(-80));
  Pack result;
  static constexpr  auto tmelt = C::Tmelt.value;
  const Mask ice_mask = (t < tmelt) && ice;
  const Mask liq_mask = !ice_mask;
  ...
  if (ice_mask.any()) {
    ...
    const Pack ice_result = (ai[0] + dt*(ai[1]+dt*(ai[2]+dt*(ai[3]+dt*(ai[4]+dt*(ai[5]+dt*(ai[6]+dt*(ai[7]+
                                                                                                ai[8]*dt))))))))*100;
    result.set(ice_mask, ice_result);
  }
  if (liq_mask.any()) {
    ...
    const Pack liq_result = (a[0] + dt*(a[1]+dt*(a[2]+dt*(a[3]+dt*(a[4]+dt*(a[5]+dt*(a[6]+dt*(a[7]+a[8]*dt))))))))*100;
    result.set(liq_mask, liq_result);
  }
...
KOKKOS_FUNCTION
typename Functions<S,D>::Pack
Functions<S,D>::qv_sat_dry(const Pack& t_atm, const Pack& p_atm_dry, const bool ice, const Mask& range_mask, const SaturationFcn func_idx, const char* caller)
{
  /*Arguments:
    ----------
  ...
  Pack e_pres; // saturation vapor pressure [Pa]
...
KOKKOS_FUNCTION
typename Functions<S,D>::Pack
Functions<S,D>::qv_sat_wet(const Pack& t_atm, const Pack& p_atm_dry, const bool ice, const Mask& range_mask,
                           const Pack& dp_wet, const Pack& dp_dry, const SaturationFcn func_idx, const char* caller)
{
  ...
  Pack qsatdry = qv_sat_dry(t_atm, p_atm_dry, ice, range_mask, func_idx, caller);
