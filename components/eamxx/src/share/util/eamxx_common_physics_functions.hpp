...
template <typename DeviceT> struct PhysicsFunctions {
  // ---------------------------------------------------------------- //
  ...
  //-----------------------------------------------------------------------------------------------//
  // Determine the length of a square column cell at a specifc latitude given
  // the area in radians
  ...
  // NOTE - Here we assume that the column area is a SQUARE so dx=dy.  We will
  // need a different
  //        routine for a rectangular (or other shape) area.
  //-----------------------------------------------------------------------------------------------//
  template <typename ScalarT>
  KOKKOS_INLINE_FUNCTION static ScalarT
  calculate_dx_from_area(const ScalarT &area, const ScalarT &lat);

  //-----------------------------------------------------------------------------------------------//
  // Determines the density given the definition of pseudo_density passed by the
  // dycore
  //   rho = pseudo_density/dz/g
  // where,
  //   rho            is the density of air, [kg/m3]
  //   pseudo_density is the pressure level thickness given a shallow
  //   atmosphere, [Pa] dz             is the geopotential thickness of the
  //   layer, [m] g              is the gravitational constant, [m/s2] - defined
  //   in physics_constants.hpp
  //-----------------------------------------------------------------------------------------------//
  template <typename ScalarT>
  KOKKOS_INLINE_FUNCTION static ScalarT
  calculate_density(const ScalarT &pseudo_density, const ScalarT &dz);
  ...
  //   g              is the gravitational constant, [m/s2] - defined in
  //   physics_constants.hpp
  //-----------------------------------------------------------------------------------------------//
  template <typename ScalarT>
  KOKKOS_INLINE_FUNCTION static ScalarT
  calculate_vertical_velocity(const ScalarT &omega, const ScalarT &density);
  ...
  // All universal constants, P0, Rd, and Cp, are defined in
  // physics_constants.hpp Note: Another experssion for Exner is,
  //   Exner = T/th
  // where,
  ...
  //   pressure  is the pressure, [Pa].  Used for exners formula, see definition
  //   above, unitless
  //-----------------------------------------------------------------------------------------------//
  template <typename ScalarT>
  KOKKOS_INLINE_FUNCTION static ScalarT
  calculate_theta_from_T(const ScalarT &temperature, const ScalarT &pressure);
  ...
  template <typename ScalarT>
  KOKKOS_INLINE_FUNCTION static ScalarT
  calculate_thetal_from_theta(const ScalarT &theta, const ScalarT &temperature,
                              const ScalarT &qc);
  ...
  template <typename ScalarT>
  KOKKOS_INLINE_FUNCTION static ScalarT
  calculate_T_from_theta(const ScalarT &theta, const ScalarT &pressure);

  //-----------------------------------------------------------------------------------------------//
  // Compute temperature from virtual temperature
  //   temperature = T_virtual * (ep_2*(1+qv)/(qv+ep_2)
  // where
  //   ep_2        is ratio of molecular mass of water to the molecular mass of
  //   dry air T_virtual   is the virtual temperature.  Units in [K]. qv is the
  //   water vapor mass mixing ratio.  Units in [kg/kg] temperature is the
  //   atmospheric temperature.  Units in [K].
  //-----------------------------------------------------------------------------------------------//
  template <typename ScalarT>
  KOKKOS_INLINE_FUNCTION static ScalarT
  calculate_temperature_from_virtual_temperature(const ScalarT &T_virtual,
                                                 const ScalarT &qv);
  ...
  // where
  //   ep_2        is ratio of molecular mass of water to the molecular mass of
  //   dry air temperature is the atmospheric temperature.  Units in [K]. qv is
  //   the water vapor mass mixing ratio.  Units in [kg/kg] T_virtual   is the
  //   virtual temperature.  Units in [K].
  //-----------------------------------------------------------------------------------------------//
  template <typename ScalarT>
  KOKKOS_INLINE_FUNCTION static ScalarT
  calculate_virtual_temperature(const ScalarT &temperature, const ScalarT &qv);
  ...
  //   z                 is the geopotential height above surface at midpoints.
  //   Units in [m]. surf_geopotential is the surface geopotential height. Units
  //   in [m]. DSE               is the dry static energy.  Units in [J/kg]
  //-----------------------------------------------------------------------------------------------//
  template <typename ScalarT>
  KOKKOS_INLINE_FUNCTION static ScalarT
  calculate_dse(const ScalarT &temperature, const ScalarT &z,
  ...
  //   z                 is the geopotential height above surface at midpoints.
  //   Units in [m]. surf_geopotential is the surface geopotential height. Units
  //   in [m]. temperature       is the atmospheric temperature. Units in [K].
  //-----------------------------------------------------------------------------------------------//
  template <typename ScalarT>
  KOKKOS_INLINE_FUNCTION static ScalarT
  ...
  //-----------------------------------------------------------------------------------------------//
  // Computes drymmr (mass of a constituent divided by mass of dry air; commonly
  // known as mixing ratio) for any wetmmr constituent (mass of a constituent
  // divided by mass of dry air plus water vapor) using qv_wet (mass of water
  // vapor divided by mass of dry air plus water vapor; see specific humidity):
  ...
  //-----------------------------------------------------------------------------------------------//
  // Computes wetmmr (mass of a constituent divided by mass of dry air plus
  // water vapor) for any drymmr constituent (mass of a constituent divided by
  // mass of dry air; commonly known as mixing ratio) using qv_dry (mass of
  // water vapor divided by mass of dry air):
  ...
  // for any wetmmr constituent (mass of a constituent divided by mass of wet
  // air; commonly known as mixing ratio) using dry and wet pseudodensities
  ...
  // for any drymmr constituent (mass of a constituent divided by mass of dry
  // air; commonly known as mixing ratio) using dry and wet pseudodensities
  ...
  //   T_virtual      is the virtual temperature - calculated using a separate
  //   function from this suite, [K] p_mid          is the avgerage atmosphere
  //   pressure over the level, [Pa] g is the graviational constant, [m s-2] Rd
  //   is the universal gas constant for dry air, [J/kg/K] T_mid          is the
  //   atmospheric temperature, [K] - needed for T_virtual qv is the water vapor
  //   mass mixing ratio, [kg/kg] - needed for T_virtual
  //
  ...
  template <typename ScalarT>
  KOKKOS_INLINE_FUNCTION static ScalarT
  calculate_dz(const ScalarT &pseudo_density, const ScalarT &p_mid,
  ...
  template <typename ScalarT>
  KOKKOS_INLINE_FUNCTION static ScalarT
  calculate_vmr_from_mmr(const Real &gas_mol_weight, const ScalarT &qv,
                         const ScalarT &mmr);
  ...
  //   mol_weight_air is the molecular weight of dry air
  //   mol_weight_X   is the molecular weight of X
  //   a = mol_weight_X/mol_weight_air is the ratio of the molecular weight of
  //   the gas to the molecular weight of dry air
  //-----------------------------------------------------------------------------------------------//
  template <typename ScalarT>
  KOKKOS_INLINE_FUNCTION static ScalarT
  calculate_mmr_from_vmr(const Real &gas_mol_weight, const ScalarT &qv,
                         const ScalarT &vmr);

  //-----------------------------------------------------------------------------------------------
  // Calculate T at the bottom of the grid cell closest to the surface for use
  // in PSL computation. This is done assuming a 6.5 K/km lapse rate, which is a
  // horrible assumption but avoids problems that computing lapse rate via
  // extrapolation might produce strange answers. It is also what CESM has done
  // for the last 20 yrs so seems to be sufficient. Don't assume this method is
  // appropriate for any other use. INPUTS: T_mid_bot z_mid_bot RETURNS: T at
  // the bottom of the cell nearest the surface (K)
  //-----------------------------------------------------------------------------------------------
  KOKKOS_INLINE_FUNCTION
  static Real calculate_surface_air_T(const Real &T_mid_bot,
                                      const Real &z_mid_bot);

  //-----------------------------------------------------------------------------------------------//
  // Compute the lapse rate and effective ground temperature for use in
  // calculating psl. This function should only be used by calculate_psl.
  // INPUTS: T_ground is the air temperature at the bottom of the cell closest
  // to the surface (aka T_int[nlev+1]; K) phi_ground is the geopotential at
  // surface (aka surf_geopotential; m2/s2) OUTPUTS: lapse (K/m) is the lapse
  // rate T_ground_tmp is the effective ground temperature (K)
  //-----------------------------------------------------------------------------------------------//
  KOKKOS_INLINE_FUNCTION
  ...
  //-----------------------------------------------------------------------------------------------//
  // Calculate sea level pressure assuming dry air between ground and sea level
  // and using a lapse rate of 6.5K/km except in very warm conditions. See
  // docs/tech_doc/physics/psl/psl_doc.tex for details INPUTS: T_ground is the
  // air temperature at the bottom of the cell closest to the surface (aka
  // T_int[nlev+1]; K) p_ground is the pressure at the bottom of the cell
  // closest to the surface (Pa) phi_ground is the geopotential at surface (aka
  // surf_geopotential; m2/s2) OUTPUTS: psl is the sea level pressure (Pa)
  //-----------------------------------------------------------------------------------------------//
  KOKKOS_INLINE_FUNCTION
  static Real calculate_psl(const Real &T_ground, const Real &p_ground,
                            const Real &phi_ground);

  //-----------------------------------------------------------------------------------------------//
  // Apply rayleigh friction. Given the decay rate profile, we compute the
  // tendencies in u and v components of the horizontal wind using an Euler
  // backward scheme, and then apply the negative of the kinetic energy tendency
  // to the dry static energy. Note: We don't actually calculate dse since this
  // is simply a tendancy of cp*T_mid. INPUTS: dt is the physics timestep otau
  // is the decay rate INPUT/OUTPUTS: u_wind is u component of the horizontal
  // wind (m/s) v_wind is v component of the horizontal wind (m/s) T_mid is the
  // atmospheric temperature at the midpoints [K]
  //-----------------------------------------------------------------------------------------------//
  template <typename ScalarT>
  KOKKOS_INLINE_FUNCTION static void
  apply_rayleigh_friction(const Real dt, const ScalarT &otau, ScalarT &u_wind,
  ...
  KOKKOS_INLINE_FUNCTION static void
  calculate_density(const MemberType &team,
  ...
  KOKKOS_INLINE_FUNCTION static void calculate_vertical_velocity(
  ...
  KOKKOS_INLINE_FUNCTION static void
  exner_function(const MemberType &team, const InputProviderP &pressure,
                 const view_1d<ScalarT> &exner);

  template <typename ScalarT, typename InputProviderT, typename InputProviderP>
  KOKKOS_INLINE_FUNCTION static void calculate_theta_from_T(
  ...
  KOKKOS_INLINE_FUNCTION static void calculate_thetal_from_theta(
  ...
  KOKKOS_INLINE_FUNCTION static void
  calculate_temperature_from_virtual_temperature(
      const MemberType &team, const InputProviderT &T_virtual,
      const InputProviderQ &qv, const view_1d<ScalarT> &temperature);

  template <typename ScalarT, typename InputProviderT, typename InputProviderQ>
  KOKKOS_INLINE_FUNCTION static void calculate_virtual_temperature(
      const MemberType &team, const InputProviderT &temperature,
  ...
  KOKKOS_INLINE_FUNCTION static void
  calculate_dse(const MemberType &team, const InputProviderT &temperature,
                const InputProviderZ &z, const Real surf_geopotential,
                const view_1d<ScalarT> &dse);

  template <typename ScalarT, typename InputProviderT, typename InputProviderZ>
  KOKKOS_INLINE_FUNCTION static void calculate_temperature_from_dse(
      const MemberType &team, const InputProviderT &dse,
      const InputProviderZ &z, const Real surf_geopotential,
      const view_1d<ScalarT> &temperature);

  template <typename ScalarT, typename InputProviderX, typename InputProviderQ>
  KOKKOS_INLINE_FUNCTION static void calculate_wetmmr_from_drymmr(
      const MemberType &team, const InputProviderX &drymmr,
      const InputProviderQ &qv, const view_1d<ScalarT> &wetmmr);

  template <typename ScalarT, typename InputProviderX, typename InputProviderQ>
  KOKKOS_INLINE_FUNCTION static void calculate_drymmr_from_wetmmr(
  ...
  template <typename ScalarT, typename InputProviderOtau,
            typename MT = Kokkos::MemoryManaged>
  KOKKOS_INLINE_FUNCTION static void apply_rayleigh_friction(
  ...
  //-----------------------------------------------------------------------------------------------//
  // Determines the vertical layer interface height from the vertical layer
  // thicknesses:
  //   z_int = int_0^z(dz)
  // where
  //   dz             is the vertical layer thickness, [m]
  // Note: because this function does an integral it cannot be run just on a
  // single level.  It requires the full column wise integration.
  //-----------------------------------------------------------------------------------------------//
  template <typename ScalarT, typename InputProviderZ,
            typename MT = Kokkos::MemoryManaged>
  KOKKOS_INLINE_FUNCTION static void
  calculate_z_int(const MemberType &team, const int num_levs,
                  const InputProviderZ &dz, const Real z_surf,
                  const view_1d<ScalarT, MT> &z_int);

  //-----------------------------------------------------------------------------------------------//
  // Determines the vertical layer height on mid points from the vertical layer
  // interface height:
  ...
  template <typename ScalarT, typename InputProviderZ,
            typename MT = Kokkos::MemoryManaged>
  KOKKOS_INLINE_FUNCTION static void
  calculate_z_mid(const MemberType &team, const int num_levs,
