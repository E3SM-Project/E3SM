 ...
 *  parallel region. In other words, you should *not* be inside a
 * TeamVectorRange parallel loop when calling these kernels, since these kernels
 * will attempt to create such loops. Furthermore, these kernels *assume* that
 * the team policy vector length (on CUDA) is 1. We have no way of checking this
 * (the vector length is stored in the policy, but not in the team member), so
 * you must make sure that this is the case.
 *
 *  In the compute_* methods, InputProvider can either be a functor (e.g., a
 * lambda) or a 1d view. The only requirement is that operator()(int) is
 * defined, and returns a Pack<ScalarType,PackSize>. For instance, one could use
 * a lambda to compute the midpoint average of the product of two interface
...
template <typename DeviceType, typename ScalarType> class ColumnOps {
public:
  ...
  debug_checks(const int num_levels, const view_1d<ScalarT, MT> &x) {
    ...
    EKAT_KERNEL_ASSERT_MSG(
        num_levels >= 0 && pack_size<ScalarT>() * x.extent_int(0) >= num_levels,
        "Error! Number of levels out of bounds.\n");

    using ret_type     = decltype(std::declval<InputProvider>()(0));
    using raw_ret_type = typename std::remove_const<
        typename std::remove_reference<ret_type>::type>::type;

    static_assert(
        std::is_same<raw_ret_type, ScalarT>::value,
  ...
  compute_midpoint_values(const TeamMember &team, const int num_mid_levels,
    ...
    compute_midpoint_values<CombineMode::Replace>(team, num_mid_levels, x_i,
                                                  x_m, 1, 0);
  }
  // Compute X at level midpoints, given X at level interfaces
  template <CombineMode CM, typename InputProvider, typename ScalarT,
            typename MT>
  KOKKOS_INLINE_FUNCTION static void compute_midpoint_values(
      const TeamMember &team, const int num_mid_levels,
    ...
    compute_midpoint_values_impl<CM, InputProvider, ScalarT, MT>(
        team, num_mid_levels, x_i, x_m, alpha, beta);
  }

  // Compute X at level interfaces, given X at level midpoints and top and bot
  // bc. Note: with proper bc, and with constant dz, then
  // x_int(x_mid(x_int))==x_int.
  template <typename InputProvider1, typename InputProvider2, typename ScalarT,
            typename MT>
  KOKKOS_INLINE_FUNCTION static void compute_interface_values_linear(
      const TeamMember &team, const int num_mid_levels,
    ...
    compute_interface_values_linear_impl(team, num_mid_levels, x_m, dz, bc_top,
                                         bc_bot, x_i);
  }

  // Compute X at level interfaces, given X at level midpoints and top or bot
  // bc. Notes:
  //  - If FixTop=true, the bc value is imposed at the top, and a scan sum from
  //  the
  ...
  //            "interior" interfaces. E.g., x_m==1, bc=0, yield
  //            x_i=[0,2,0,2,...].
  template <bool FixTop, typename InputProvider, typename ScalarT, typename MT>
  KOKKOS_INLINE_FUNCTION static void compute_interface_values_compatible(
      const TeamMember &team, const int num_mid_levels,
    ...
    compute_interface_values_compatible_impl<FixTop>(team, num_mid_levels, x_m,
                                                     bc, x_i);
  }

  // Given X at level interfaces, compute dX at level midpoints.
  template <typename InputProvider, typename ScalarT, typename MT>
  KOKKOS_INLINE_FUNCTION static void compute_midpoint_delta(
      const TeamMember &team, const int num_mid_levels,
      const InputProvider &x_i, const view_1d<ScalarT, MT> &dx_m,
      const scalar_type alpha = one(), const scalar_type beta = zero()) {
    compute_midpoint_delta<CombineMode::Replace>(team, num_mid_levels, x_i,
                                                 dx_m, alpha, beta);
  }
  ...
  KOKKOS_INLINE_FUNCTION static void compute_midpoint_delta(
    ...
    compute_midpoint_delta_impl<CM>(team, num_mid_levels, x_i, dx_m, alpha,
                                    beta);
  }

  // Scan sum of a quantity defined at midpoints, to retrieve its integral at
  // interfaces. This function is the logical inverse of the one above. Notes:
  //  - FromTop: true means we scan over [0,num_mid_levels), while false means
  //  the scan
  //             is over (num_mid_levels,0]. Recall that ilev=0 is the top of
  //             the model.
  //  - InputProvider: must provide an input al all mid levels
  //  - s0: used as bc value at k=0 (FromTop=true) or k=num_mid_levels
  //  (FromTop=false)
  template <bool FromTop, typename InputProvider, typename ScalarT, typename MT>
  KOKKOS_INLINE_FUNCTION static void
  column_scan(const TeamMember &team, const int num_mid_levels,
              const InputProvider &dx_m, const view_1d<ScalarT, MT> &x_i,
              const scalar_type &s0 = zero()) {
    // Sanity checks
    debug_checks<InputProvider>(num_mid_levels + 1, x_i);

    // Scan's impl is quite lengthy, so split impl into two fcns, depending on
    // pack size.
...
protected:
  ...
  KOKKOS_INLINE_FUNCTION static
      typename std::enable_if<(pack_size<ScalarT>() == 1)>::type
  ...
  KOKKOS_INLINE_FUNCTION static
    ...
    // This lambda can be used in both cases to process packs that have a next
    // pack
    ...
    if (NUM_MID_PACKS == NUM_INT_PACKS) {
      ...
      team_single(team, [&]() {
        // Last level pack treated separately, since int pack k+1 does not
        // exist. Shift's first arg is the scalar to put in the "empty" spot at
        // the end. In this case, we don't need it, so just uze zero.
  ...
  KOKKOS_INLINE_FUNCTION static
    ...
    if (NUM_MID_PACKS == NUM_INT_PACKS) {
      ...
      // Last pack does not have a next one, so needs to be treated separately
      // and serially.
      team_single(team, [&]() {
        // Shift's first arg is the scalar to put in the "empty" spot at the
        // end. In this case, we don't need it, so just uze zero.
        const auto &xi_last = x_i(LAST_PACK);
  ...
  KOKKOS_INLINE_FUNCTION static
      typename std::enable_if<(pack_size<ScalarT>() == 1)>::type
      column_scan_impl(const TeamMember &team, const int num_mid_levels,
  ...
  KOKKOS_INLINE_FUNCTION static
    ...
    EKAT_KERNEL_ASSERT_MSG(pack_size<ScalarT>() <= num_mid_levels,
                           "Error! Currently, column_scan_impl() is not "
                           "implemented for pack_size > num_mid_levels.");
    ...
    if (FromTop) {
      ...
      //  3. Do the scan over the current pack: x_i(k)[i] = s +
      //  dx_m(k)[0,...,i-1]

      // It is easier to read if we check whether #int_packs==#mid_packs.
      // This lambda can be used so long as there is a 'next' pack x_i(k+1);
      auto packed_scan_from_top = [&](const int &k, pack_type &accumulator,
                                      const bool last) {
        accumulator += dx_m(k);
        if (last) {
          x_i(k + 1) = accumulator;
      ...
      if (NUM_MID_PACKS == NUM_INT_PACKS) {
        ...
        team_parallel_for(team, NUM_INT_PACKS, [&](const int k) {
          scalar_type s = s0;
          // If k==0, x_i(k) does not contain any scan sum (and may contain
          // garbage), so ignore it
          if (k != 0) {
        ...
        team_parallel_for(team, NUM_INT_PACKS, [&](const int k) {
          ...
          // Note: for the last interface, this_pack_end==1, so we will *not*
          // access
      ...
      // Strategy:
      //  1. Do a packed reduction of x_m, to get x_i(k) =
      //  dx_m(k)+...+dx_m(num_mid_levs-1)
      //  2. Let s = s0 + reduce(x_i(k)). The rhs is the sum of all dx_m
      ...
      if (NUM_MID_PACKS == NUM_INT_PACKS) {
        ...
        team.team_barrier();
        column_scan_impl<FromTop>(team, (NUM_MID_PACKS - 1) * PackLength, dx_m,
                                  x_i, x_i(NUM_INT_PACKS - 1)[0]);
      } else {
        // In this case, all packs of dx_m are full of meaningful values.
        auto packed_scan_from_bot = [&](const int &k, pack_type &accumulator,
                                        const bool last) {
        ...
        // Now let s = s0 + reduce_sum(x_i(k)). The second term is the sum of
        // all dx_m on all "physical" levels on all packs above current one.
        // Then, do the scan over the current pack: x_i(k)[i] = s +
        // dx_m(k)[i,...,pack_end]

        // The last int pack only needs s0, and the second to last had no
        // "following" midpoints pack, and we didn't write anything in it during
        // the scan sum, so fill it with 0's
        team_single(team, [&]() {
          x_i(LAST_INT_PACK)[0]  = s0;
          x_i(LAST_INT_PACK - 1) = 0;
        });

        team_parallel_for(team, NUM_MID_PACKS, [&](const int k) {
          const auto k_bwd = NUM_MID_PACKS - k - 1;
  ...
  KOKKOS_INLINE_FUNCTION static
    ...
    team_parallel_for(team, num_mid_levels + 1, [&](const int k) {
      if (k == 0)
        x_i(k) = bc_top;
      else if (k == num_mid_levels)
        x_i(k) = bc_bot;
      else
        x_i(k) =
            (x_m(k) * dz(k - 1) + x_m(k - 1) * dz(k)) / (dz(k - 1) + dz(k));
    });
  }
  ...
  KOKKOS_INLINE_FUNCTION static
    ...
    team_parallel_for(team, num_int_packs, [&](const int k) {
      ...
      x_i(k).set(in_interior,
                 (x_m_k * dz_km1 + x_m_km1 * dz_k) / (dz_km1 + dz_k));
    });
  }
  ...
  KOKKOS_INLINE_FUNCTION static
      typename std::enable_if<(pack_size<ScalarT>() == 1)>::type
      compute_interface_values_compatible_impl(
          const TeamMember &team, const int num_mid_levels,
          const InputProvider &x_m, const scalar_type &bc,
          const view_1d<ScalarT, MT> &x_i) {
    ...
    // we do a scan sum of (-1)^n x_m(n), using the alt_sign impl of
    // column_scan.
  ...
  KOKKOS_INLINE_FUNCTION static
    ...
    auto m1_pow_k = [](const int k) -> int { return 1 - 2 * (k % 2); };
    // Store a pack of (-1)^k
    pack_type sign = 0;
    vector_simd for (int i = 0; i < PackLength; ++i) { sign[i] = m1_pow_k(i); }

    // Scanned quantity is (-1)^n x_m(n)
    auto lambda = [&](const int k) -> pack_type { return sign * x_m(k); };
  ...
  // Runs the input lambda with a TeamVectorRange parallel for over [0,count)
  // range
  template <typename Lambda>
  KOKKOS_INLINE_FUNCTION static void
  team_parallel_for(const TeamMember &team, const int count, const Lambda &f) {
    Kokkos::parallel_for(Kokkos::TeamVectorRange(team, count), f);
  }

  // Runs the input lambda with a TeamVectorRange parallel for over [start,end)
  // range
  template <typename Lambda>
  KOKKOS_INLINE_FUNCTION static void
  team_parallel_for(const TeamMember &team, const int start, const int end,
                    const Lambda &f) {
    Kokkos::parallel_for(Kokkos::TeamVectorRange(team, start, end), f);
  }

  // Runs the input lambda with a TeamVectorRange parallel scan over [0,count)
  // range
  template <typename Lambda>
  KOKKOS_INLINE_FUNCTION static void
  team_parallel_scan(const TeamMember &team, const int count, const Lambda &f) {
    team_parallel_scan(team, 0, count, f);
  }

  // Runs the input lambda with a TeamVectorRange parallel scan over [start,end)
  // range
  template <typename Lambda>
  KOKKOS_INLINE_FUNCTION static void
  team_parallel_scan(const TeamMember &team, const int start, const int end,
                     const Lambda &f) {
    auto is_pow_of_2 = [](const int n) -> bool {
      // This seems funky, but write down a pow of 2 and a non-pow of 2 in
      // binary (both positive), and you'll see why it works
      return n > 0 && (n & (n - 1)) == 0;
    };
    EKAT_KERNEL_REQUIRE_MSG(
        !ekat::OnGpu<typename device_type::execution_space>::value ||
            is_pow_of_2(team.team_size()),
        "Error! Team-level parallel_scan on CUDA only works for team sizes "
        "that are power of 2.\n"
        "       You could try to reduce the team size to the previous pow of "
        "2.\n");
  ...
  template <typename Lambda>
  KOKKOS_INLINE_FUNCTION static void team_single(const TeamMember &team,
                                                 const Lambda &f) {
