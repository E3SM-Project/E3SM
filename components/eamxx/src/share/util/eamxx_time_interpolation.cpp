...
/*-----------------------------------------------------------------------------------------------*/
TimeInterpolation::TimeInterpolation(const grid_ptr_type &grid, const vos_type &list_of_files)
...
void TimeInterpolation::set_file_data_triplets(const vos_type &list_of_files) {
  ...
  for (size_t ii = 0; ii < list_of_files.size(); ii++) {
    ...
    if (time_units.find("seconds") != std::string::npos) {
      ...
      EKAT_ERROR_MSG("Error!! TimeInterpolation::set_file_triplets - unsupported units of time = ("
