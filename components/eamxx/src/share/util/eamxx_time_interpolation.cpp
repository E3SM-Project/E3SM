...
/*-----------------------------------------------------------------------------------------------*/
/* A function to perform time interpolation using data from all the fields
 * stored in the local field managers. Conducts a simple linear interpolation
 * between two points using y* = w*y0 + (1-w)*y1 where w = (t1-t*)/(t1-t0)
 * Input:
 *   time_in - A timestamp to interpolate onto.
 * Output
 *   A map of (field name) and (Field) pairs such that each field is the
 * interpolated values and the map makes it possible to quickly query individual
 * outputs.
 */
void TimeInterpolation::perform_time_interpolation(const TimeStamp &time_in) {
  ...
  // If data is handled by files we need to check that the timestamps are still
  // relevant
  if (m_file_data_triplets.size() > 0) {
    check_and_update_data(time_in);
  }

  // Gather weights for interpolation.  Note, timestamp differences are integers
  // and we need a real defined weight.
 ...
 * Input:
 *   field_in - Is a field with the appropriate dimensions and metadata to match
 * the interpolation Output: None
 */
void TimeInterpolation::add_field(const Field &field_in,
                                  const bool store_shallow_copy) {
  // First check that we haven't already added a field with the same name.
  const std::string name = field_in.name();
  EKAT_REQUIRE_MSG(!m_fm_time0->has_field(name) and
                       !m_fm_time1->has_field(name),
  ...
  EKAT_REQUIRE_MSG(field_in.data_type() == DataType::FloatType or
                   ...
                   "[TimeInterpolation] Error! Input field must have "
                   "floating-point data type.\n"
                   " - field name: " +
                       field_in.name() +
                       "\n"
                       " - data type : " +
                       e2str(field_in.data_type()) + "\n");
  ...
  if (store_shallow_copy) {
    // Then we want to store the actual field_in and override it when
    // interpolating
 ...
 *
 * At initialization we assume that only the first timestep of data has been
 * set.  Subsequent timesteps of data are added using update_data and then a
 * call to update_timestamp will shift time0 to time1 and update time1.
 */
 ...
 * Input:
 *   field_in - A field with a name matching one of the fields in the
 * interpolator.  Data will be shifted and copied.
 */
...
/*-----------------------------------------------------------------------------------------------*/
/* Function which will initialize the full set of fields given a list of files
 * pointing to where the data can be found. Input: list_of_files: A vector of
 * strings representing all the files where interpolation data can be gathered.
 */
void TimeInterpolation::initialize_data_from_files() {
  ...
  m_file_data_atm_input =
      std::make_shared<AtmosphereInput>(input_params, m_fm_time1);
  m_file_data_atm_input->set_logger(m_logger);
  // Assign the mask value gathered from the FillValue found in the source file.
  // TODO: Should we make it possible to check if FillValue is in the metadata
  // and only assign mask_value if it is?
  for (auto &name : m_field_names) {
    auto &field0    = m_fm_time0->get_field(name);
    auto &field1    = m_fm_time1->get_field(name);
    auto &field_out = m_interp_fields.at(name);
    ...
    auto set_fill_value = [&](const auto var_fill_value) {
      ...
      if (dt == DataType::FloatType) {
        field0.get_header().set_extra_data("mask_value",
                                           static_cast<float>(var_fill_value));
        field1.get_header().set_extra_data("mask_value",
                                           static_cast<float>(var_fill_value));
        field_out.get_header().set_extra_data(
            "mask_value", static_cast<float>(var_fill_value));
      } else if (dt == DataType::DoubleType) {
        field0.get_header().set_extra_data("mask_value",
                                           static_cast<double>(var_fill_value));
        field1.get_header().set_extra_data("mask_value",
                                           static_cast<double>(var_fill_value));
        field_out.get_header().set_extra_data(
            "mask_value", static_cast<double>(var_fill_value));
      } else {
        EKAT_ERROR_MSG(
    ...
    if (scorpio::refine_dtype(pio_var.nc_dtype) == "float") {
      auto var_fill_value = scorpio::get_attribute<float>(triplet_curr.filename,
                                                          name, "_FillValue");
      set_fill_value(var_fill_value);
    } else if (scorpio::refine_dtype(pio_var.nc_dtype) == "double") {
      auto var_fill_value = scorpio::get_attribute<double>(
          triplet_curr.filename, name, "_FillValue");
      set_fill_value(var_fill_value);
    } else {
      EKAT_ERROR_MSG("Unrecognized/unsupported data type\n"
                     " - filename: " +
                     triplet_curr.filename +
                     "\n"
                     " - varname : " +
                     name +
                     "\n"
...
/*-----------------------------------------------------------------------------------------------*/
/* Function which will update the timestamps by shifting time1 to time0 and
 * setting time1. Input: ts_in - A timestamp for the most recent timestamp of
 * the interpolation data.
 *
 * It is assumed that any updated time is meant to replace time1 and that the
 * time1 should be shifted to time0
 */
...
/*-----------------------------------------------------------------------------------------------*/
/* Function which will update field data given an input field.  Useful for time
 * interpolation related to the EAMxx state. Input: field_in - A field with a
 * name matching one of the fields in the interpolator.  Data will be shifted
 * and copied.
 *
 * It is assumed that any updated data is meant to replace the data at time1 and
 * that the time1 data should be shifted to time0
 */
void TimeInterpolation::update_data_from_field(const Field &field_in) {
  ...
  auto &field1    = m_fm_time1->get_field(name);
  std::swap(field0, field1);
  // Now that we have swapped field0 and field 1 we need to grab field 1 from
  // the field manager again. Alternatively we could just update `field0` which
  // is now inside m_fm_time1, but choosing this approach for code readability.
...
/*-----------------------------------------------------------------------------------------------*/
/* Function to organize the data available from data files by time.  This is
 * necessary to quickly grab the appropriate data each time interpolation is
 * requested. Input: list_of_files - Is a vector of strings representing all
 * files that can be used for data.
 *
...
void TimeInterpolation::set_file_data_triplets(const vos_type &list_of_files) {
  EKAT_REQUIRE_MSG(list_of_files.size() > 0,
                   "ERROR! TimeInterpolation::set_file_data_triplets - "
                   "the list of files is empty. Please check.");
  TimeStamp ts_ref;
  // The first step is to grab all relevant metadata for the DataFromFileTriplet
  // objects. We will store the times in a map and take advantage of maps
  // natural sorting to organize the triplets in chronological order.  This
  // ensures that if the list of files does not represent the chronological
  ...
  for (size_t ii = 0; ii < list_of_files.size(); ii++) {
    ...
    auto time_units =
        scorpio::get_attribute<std::string>(filename, "time", "units");
    int time_mult;
    if (time_units.find("seconds") != std::string::npos) {
      ...
      EKAT_ERROR_MSG(
          ...
          "time = ("
          << time_units << ") in source data file " << filename
    ...
    for (int tt = 0; tt < ntime; tt++) {
      ...
      EKAT_REQUIRE_MSG(map_of_times_to_vector_idx.count(time) == 0,
                       "Error! TimeInterpolation::set_file_data_triplets - The "
                       "same time step has "
                       "been encountered "
  ...
  // Now that we have gathered all of the timesnaps we can arrange them in order
  // as DataFromFileTriplet objects. Taking advantage of maps automatically
  // self-sorting by the first arg.
...
/*-----------------------------------------------------------------------------------------------*/
/* Function to read a new set of data from file using the current iterator
 * pointing to the current DataFromFileTriplet.
 */
void TimeInterpolation::read_data() {
  const auto triplet_curr = m_file_data_triplets[m_triplet_idx];
  if (not m_file_data_atm_input or
      triplet_curr.filename != m_file_data_atm_input->get_filename()) {
    ...
    for (auto &name : m_field_names) {
      ...
      if (dt == DataType::FloatType) {
        auto var_fill_value = scorpio::get_attribute<float>(
            triplet_curr.filename, name, "_FillValue");
...
/*-----------------------------------------------------------------------------------------------*/
/* Function to check the current set of interpolation data against a timestamp
 * and, if needed, update the set of interpolation data to ensure the passed
 * timestamp is within the bounds of the interpolation data. Input: ts_in - A
 * timestamp that we intend to interpolate onto.
 */
void TimeInterpolation::check_and_update_data(const TimeStamp &ts_in) {
  // First check if the passed timestamp is within the bounds of time0 and
  // time1.
  ...
  if (m_time1.seconds_from(ts_in) < 0) {
    ...
    // First cycle through the DataFromFileTriplet's to find a timestamp that is
    // greater than this one.
    bool found = false;
    int step_cnt =
        0; // Track how many triplets we passed to find one that worked.
    ...
    EKAT_REQUIRE_MSG(
        found, "ERROR!! TimeInterpolation::check_and_update_data - timestamp "
                   << ts_in.to_string()
                   << "is outside the bounds of the set of data files." << "\n"
                   << "     TimeStamp time0: " << m_time0.to_string() << "\n"
                   << "     TimeStamp time1: " << m_time1.to_string() << "\n");
    // Now we need to make sure we didn't jump more than one triplet, if we did
    // then the data at time0 is incorrect.
    if (step_cnt > 1) {
      // Then we need to populate data for time1 as the previous triplet before
      // shifting data to time0
      --m_triplet_idx;
      read_data();
      ++m_triplet_idx;
    }
    ...
    read_data();
    // Sanity Check
    bool current_data_check = (ts_in.seconds_from(m_time0) >= 0) and
                              (m_time1.seconds_from(ts_in) >= 0);
    EKAT_REQUIRE_MSG(
        current_data_check,
