...
KOKKOS_INLINE_FUNCTION void hash(const double v_, HashType &accum) {
  static_assert(sizeof(double) == sizeof(HashType), "HashType must have size sizeof(double).");
  HashType v;
  std::memcpy(&v, &v_, sizeof(HashType));
  hash(v, accum);
}

KOKKOS_INLINE_FUNCTION void hash(const float v, HashType &accum) { hash(double(v), accum); }
