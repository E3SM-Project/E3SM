...
DataInterpolation::DataInterpolation(
    ...
    : m_model_grid(model_grid), m_fields(fields) {
...
void DataInterpolation::run(const util::TimeStamp &ts) {
  EKAT_REQUIRE_MSG(m_data_initialized,
                   "[DataInterpolation] Error! You must call "
                   "'init_data_interval' before calling 'run'.\n");
  ...
  //       have t_beg>t_end
  util::TimeInterval beg_to_ts(m_data_interval.beg, ts,
                               m_data_interval.timeline);
  double alpha = beg_to_ts.length / m_data_interval.length;
  EKAT_REQUIRE_MSG(alpha >= 0 and alpha <= 1,
                   "[DataInterpolation] Error! Input timestamp is outside the "
                   "current data time interval.\n"
                   "  data interval beg  ; " +
  ...
  // For Dynamic3D/Dynamic3D profile we also need to compute the source pressure
  // profile NOTE: this can't be done in the loop above, since p_data is not a
  // "remapped"
  //       field in the vertical remapper (also, we need to use ad different
  //       ptr)
  if (m_vr_type == Dynamic3D) {
    ...
    auto hyam = m_vert_remapper->get_src_grid()
                    ->get_geometry_data("hyam")
                    .get_view<const PT *>();
    auto hybm = m_vert_remapper->get_src_grid()
                    ->get_geometry_data("hybm")
                    .get_view<const PT *>();
...
void DataInterpolation::shift_data_interval() {
  ...
  m_curr_interval_idx.second =
      m_time_database.get_next_idx(m_curr_interval_idx.first);

  m_data_interval.advance(
      m_time_database.slices[m_curr_interval_idx.second].time);
...
void DataInterpolation::init_data_interval(const util::TimeStamp &t0) {
  EKAT_REQUIRE_MSG(m_remappers_created,
                   "[DataInterpolation] Error! Cannot call "
                   "'init_data_interval' until after remappers creation.\n");

  // Create a bare reader. Fields and filename are set inside the
  // update_end_fields call
  strvec_t fnames;
  for (auto f : m_fields) {
    fnames.push_back(f.name());
  }

  m_reader = std::make_shared<AtmosphereInput>(
      fnames, m_horiz_remapper_beg->get_src_grid());
  ...
  // We need to read in the beg/end fields for the initial interval. However,
  // our generic framework can only load the end slice (since that's what we
  // need at runtime). So, load end state for t=t_beg, then call
  // shift_data_interval NOTE: don't compute length now, since beg time point is
  // invalid (we don't need length yet).
  m_data_interval            = util::TimeInterval(util::TimeStamp(), t_beg,
                                                  m_time_database.timeline, false);
...
void DataInterpolation::setup_time_database(const strvec_t &input_files,
  ...
  // Log the final list of files, so the user know if something went wrong (e.g.
  // a bad regex)
  ...
  auto num_unique_files =
      std::unordered_set(input_files.begin(), input_files.end()).size();
  EKAT_REQUIRE_MSG(
      num_unique_files == input_files.size(),
  ...
  for (const auto &fname : input_files) {
    ...
    if (not scorpio::has_time_dim(fname)) {
      EKAT_REQUIRE_MSG(scorpio::has_dim(fname, "time"),
                       "[DataInterpolation] Error! Input file does not contain "
                       "a 'time' dimension.\n"
                       " - file name: " +
                           fname + "\n");
    ...
    EKAT_REQUIRE_MSG(std::is_sorted(times.back().begin(), times.back().end()),
                     "[DataInterpolation] Error! One of the input files has "
                     "time slices not sorted.\n"
                     " - file name  : " +
                         fname +
                         "\n"
                         " - time stamps: " +
  ...
  for (int i = 0; i < nfiles; ++i) {
    ...
    if (i > 0) {
      ...
      EKAT_REQUIRE_MSG(prev.back() < next.front(),
                       "[DataInterpolation] Error! The input files contain "
  ...
  EKAT_REQUIRE_MSG(m_time_database.size() >= 2,
                   "[DataInterpolation] Error! Input file(s) only contain 1 "
                   "time slice overall.\n");

  m_time_db_created = true;
}

void DataInterpolation::setup_remappers(const RemapData &data) {
  // 1. Horiz remapper
...
int DataInterpolation::TimeDatabase::get_next_idx(int prev) const {
  ...
  if (next >= size()) {
    EKAT_REQUIRE_MSG(timeline == util::TimeLine::YearlyPeriodic,
                     "[TimeDatabase::get_next_idx] Error! Requesting slice "
                     "that is past the database end.\n");
    next = next % size();
  }
  return next;
}

int DataInterpolation::TimeDatabase::find_interval(
    const util::TimeStamp &t) const {
  EKAT_REQUIRE_MSG(size() > 1, "[TimeDatabase::find_interval] Error! The "
  ...
  EKAT_REQUIRE_MSG(
      ...
      "[TimeDatabase::find_interval] Error! Could not locate interval "
      "containing input timestamp.\n"
      "  - input time : " +
          t.to_string() +
          "\n"
          "  - timeline   : " +
  ...
  // Since we are in YearlyPeriodic timeline, the interval starts at the last
  // slice
  return size() - 1;
}

int DataInterpolation::get_input_files_dimlen(
  ...
  for (const auto &fname : m_time_database.files) {
    ...
    EKAT_REQUIRE_MSG(scorpio::has_dim(fname, dimname),
                     "Error! Input file is missing '" + dimname +
                         "' dimension.\n"
                         "  - input file: " +
                         fname + "\n");
...
void DataInterpolation::setup_horiz_remappers(const RemapData &data) {
  EKAT_REQUIRE_MSG(data.hremap_file == "" or not data.has_iop,
                   "Error! Cannot both use a hremap file and set iop lat/lon "
                   "coordinates.\n");
  ...
  if (data.has_iop) {
    EKAT_REQUIRE_MSG(not ekat::is_invalid(data.iop_lat) and
                         not ekat::is_invalid(data.iop_lon),
                     "Error! At least one between iop_lat and iop_lon appears "
                     "to be valid in RemapData.\n"
                     "  - iop_lat: "
                         << data.iop_lat
                         << "\n"
                            "  - iop_lon: "
    ...
    auto data_grid = create_point_grid("data", ncols_data, nlevs_data,
                                       m_model_grid->get_comm());
    auto lat_f     = data_grid->create_geometry_data(
        "lat", data_grid->get_2d_scalar_layout());
    auto lon_f = data_grid->create_geometry_data(
        "lon", data_grid->get_2d_scalar_layout());
    AtmosphereInput latlon_reader(m_time_database.files.front(), data_grid,
                                  {lat_f, lon_f});
    latlon_reader.read_variables();

    // Create IOP remappers
    m_horiz_remapper_beg = std::make_shared<IOPRemapper>(
        data_grid, m_grid_after_hremap, data.iop_lat, data.iop_lon);
    m_horiz_remapper_end = std::make_shared<IOPRemapper>(
        data_grid, m_grid_after_hremap, data.iop_lat, data.iop_lon);
  } else if (data.hremap_file != "") {
    m_horiz_remapper_beg = std::make_shared<RefiningRemapperP2P>(
        m_grid_after_hremap, data.hremap_file);
    m_horiz_remapper_end = std::make_shared<RefiningRemapperP2P>(
        m_grid_after_hremap, data.hremap_file);
  } else {
    // NO hremap of any kind. 'ncols' from the data must then match the model
    // grid (nlev can differ)
    EKAT_REQUIRE_MSG(ncols_data == m_model_grid->get_num_global_dofs(),
                     "Error! No horiz remap was requested, but the 'ncol' dim "
                     "from file does not "
...
void DataInterpolation::setup_vert_remapper(const RemapData &data) {
  ...
  if (m_vr_type == None) {
    ...
    // If no vert remap is requested, model_grid and grid_after_hremap MUST have
    // same nlevs
    ...
    EKAT_REQUIRE_MSG(model_nlevs == data_nlevs,
                     "Error! No vertical remap was requested, but the 'lev' "
                     "dim from file does not "
  ...
  auto s2et = [](const std::string &s) {
    if (s == "P0") {
      ...
      EKAT_ERROR_MSG("Error! Invalid/unsupported extrapolation type.\n"
                     " - input value : " +
                     s +
                     "\n"
  ...
  auto vremap =
      std::make_shared<VerticalRemapper>(m_grid_after_hremap, m_model_grid);
  ...
  // Set the mask value only if needed. RemapData has a default that is invalid
  // for VerticalRemapper
  ...
  //  - p_data is the full 3d pressure where data is defined, while p_file is
  //  the field
  //    we read from file. For Static1D and Dynamic3D they are the same, but for
  //    Dynamic3DRef, p_file is the surf pressure (2d), while p_data is the full
  //    3d pmid
  auto p_layout = m_vr_type == Static1D
                      ? m_grid_after_hremap->get_vertical_layout(true)
                      : m_grid_after_hremap->get_3d_scalar_layout(true);
  auto &p_data  = m_helper_pressure_fields["p_data"];
  p_data        = Field(FieldIdentifier("p_data", p_layout, ekat::units::Pa,
                                        m_grid_after_hremap->name()));
  p_data.get_header().get_alloc_properties().request_allocation(
      SCREAM_PACK_SIZE);
  ...
  if (m_vr_type == Dynamic3D) {
    ...
    // We load the surface pressure, and reconstruct p_data via p=ps*hybm(k) +
    // p0*hyam(k)
    auto &ps = m_helper_pressure_fields["p_file"];
    ps       = Field(FieldIdentifier(data.pname,
                                     m_grid_after_hremap->get_2d_scalar_layout(),
    ...
    auto hyam       = m_grid_after_hremap->create_geometry_data(
        "hyam", layout, nondim, real_t, SCREAM_PACK_SIZE);
    auto hybm = m_grid_after_hremap->create_geometry_data(
        "hybm", layout, nondim, real_t, SCREAM_PACK_SIZE);
    AtmosphereInput hvcoord_reader(m_time_database.files.front(),
                                   m_grid_after_hremap, {hyam, hybm}, true);
    hvcoord_reader.read_variables();
  } else if (m_vr_type == Static1D) {
    // Can load p now, since it's static
    AtmosphereInput p_data_reader(m_time_database.files.front(),
                                  m_grid_after_hremap,
                                  {p_data.alias(data.pname)}, true);
    p_data_reader.read_variables();
  }
  vremap->set_source_pressure(m_helper_pressure_fields["p_data"],
                              VerticalRemapper::Both);
...
void DataInterpolation::register_fields_in_remappers() {
  // Register fields in the remappers. Vertical first, since we only have
  // model-grid fields
  ...
  if (m_vr_type == Dynamic3D or m_vr_type == Dynamic3DRef) {
    ...
    m_horiz_remapper_beg->register_field_from_tgt(data_p.clone(
        data_p.name(), m_horiz_remapper_beg->get_src_grid()->name()));
    m_horiz_remapper_end->register_field_from_tgt(data_p.clone(
        data_p.name(), m_horiz_remapper_end->get_src_grid()->name()));
  }
