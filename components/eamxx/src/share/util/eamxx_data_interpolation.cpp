...
DataInterpolation::DataInterpolation(const std::shared_ptr<const AbstractGrid> &model_grid,
                                     const std::vector<Field> &fields)
    : m_model_grid(model_grid), m_fields(fields) {
  EKAT_REQUIRE_MSG(model_grid != nullptr, "[DataInterpolation] Error! Invalid grid pointer.\n");
...
int DataInterpolation::get_input_files_dimlen(const std::string &dimname) const {
  ...
  for (const auto &fname : m_time_database.files) {
    ...
    EKAT_REQUIRE_MSG(scorpio::has_dim(fname, dimname), "Error! Input file is missing '" + dimname +
                                                           "' dimension.\n"
                                                           "  - input file: " +
                                                           fname + "\n");
...
void DataInterpolation::register_fields_in_remappers() {
  ...
  if (m_vr_type == Dynamic3D or m_vr_type == Dynamic3DRef) {
    ...
    m_horiz_remapper_beg->register_field_from_tgt(
        data_p.clone(data_p.name(), m_horiz_remapper_beg->get_src_grid()->name()));
    m_horiz_remapper_end->register_field_from_tgt(
        data_p.clone(data_p.name(), m_horiz_remapper_end->get_src_grid()->name()));
  }
