...
//       such as to NOT cause unintended loss of precision. E.g., if storing 1 (an int), we can
...
//       would not cause loss of precision. Retrieving a double scalar as a float is
//       instead allowed, as this is common when dealing with single/mixed precision.
struct ScalarWrapper {
  ...
  template<typename ST>
  ST as () const {
    EKAT_REQUIRE_MSG (not is_unsafe_conversion(type,get_data_type<ST>()),
