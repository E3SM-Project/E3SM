...
inline std::vector<int> unflatten_idx(const std::vector<int> &dims,
                                      const int idx) {
  ...
  auto ind = [&](const int i) -> int & { return *(indices.rbegin() + i); };

  switch (r) {
  case 6:
    ind(5) = ((((idx / dr0) / dr1) / dr2) / dr3) / dr4; // Fallthrough
  case 5:
    ind(4) = ((((idx / dr0) / dr1) / dr2) / dr3) % dr4; // Fallthrough
  case 4:
    ind(3) = (((idx / dr0) / dr1) / dr2) % dr3; // Fallthrough
  case 3:
    ind(2) = ((idx / dr0) / dr1) % dr2; // Fallthrough
  case 2:
    ind(1) = (idx / dr0) % dr1; // Fallthrough
  case 1:
    ind(0) = idx % dr0;
    break;
  default:
...
unflatten_idx(const int idx,
              const Kokkos::View<int *, Kokkos::LayoutRight, Props...> &dims,
              int &i) {
  EKAT_KERNEL_ASSERT_MSG(dims.size() == 1,
                         "Error! Wrong overload of unflatten_idx called.\n");
  i = idx;
  EKAT_KERNEL_ASSERT_MSG(i >= 0 && i < dims[0],
                         "Error! Flatten index out of bounds.\n");
}
...
unflatten_idx(const int idx,
              const Kokkos::View<int *, Kokkos::LayoutRight, Props...> &dims,
              int &i, int &j) {
  EKAT_KERNEL_ASSERT_MSG(dims.size() == 2,
                         "Error! Wrong overload of unflatten_idx called.\n");
  ...
  EKAT_KERNEL_ASSERT_MSG(i >= 0 && i < dims[0],
                         "Error! Flatten index out of bounds.\n");
  EKAT_KERNEL_ASSERT_MSG(j >= 0 && j < dims[1],
                         "Error! Flatten index out of bounds.\n");
}
...
unflatten_idx(const int idx,
              const Kokkos::View<int *, Kokkos::LayoutRight, Props...> &dims,
              int &i, int &j, int &k) {
  EKAT_KERNEL_ASSERT_MSG(dims.size() == 3,
                         "Error! Wrong overload of unflatten_idx called.\n");
  ...
  EKAT_KERNEL_ASSERT_MSG(j >= 0 && j < dims[1],
                         "Error! Flatten index out of bounds.\n");
  EKAT_KERNEL_ASSERT_MSG(k >= 0 && k < dims[2],
                         "Error! Flatten index out of bounds.\n");
}
...
unflatten_idx(const int idx,
  ...
  EKAT_KERNEL_ASSERT_MSG(dims.size() == 4,
                         "Error! Wrong overload of unflatten_idx called.\n");
  ...
  EKAT_KERNEL_ASSERT_MSG(k >= 0 && k < dims[2],
                         "Error! Flatten index out of bounds.\n");
  EKAT_KERNEL_ASSERT_MSG(l >= 0 && l < dims[3],
                         "Error! Flatten index out of bounds.\n");
}
...
unflatten_idx(const int idx,
  ...
  EKAT_KERNEL_ASSERT_MSG(dims.size() == 5,
                         "Error! Wrong overload of unflatten_idx called.\n");
  ...
  EKAT_KERNEL_ASSERT_MSG(l >= 0 && l < dims[3],
                         "Error! Flatten index out of bounds.\n");
  EKAT_KERNEL_ASSERT_MSG(m >= 0 && m < dims[4],
                         "Error! Flatten index out of bounds.\n");
}
...
unflatten_idx(const int idx,
  ...
  EKAT_KERNEL_ASSERT_MSG(dims.size() == 6,
                         "Error! Wrong overload of unflatten_idx called.\n");
  ...
  EKAT_KERNEL_ASSERT_MSG(m >= 0 && m < dims[4],
                         "Error! Flatten index out of bounds.\n");
  EKAT_KERNEL_ASSERT_MSG(n >= 0 && n < dims[5],
                         "Error! Flatten index out of bounds.\n");
}
