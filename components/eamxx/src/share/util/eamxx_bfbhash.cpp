...
static void reduce_hash(void *invec, void *inoutvec, int *len,
...
int all_reduce_HashType(MPI_Comm comm, const HashType *sendbuf,
                        HashType *rcvbuf, int count) {
  ...
  const auto stat =
      MPI_Allreduce(sendbuf, rcvbuf, count, MPI_LONG_LONG_INT, op, comm);
  MPI_Op_free(&op);
  return stat;
}
