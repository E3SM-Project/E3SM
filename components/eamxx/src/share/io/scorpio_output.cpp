 ...
 , m_add_time_dim   (true)
{
  ...
  auto gm = gridl_mgr->get_grids_manager();
  ...
  auto fm_grid = gm->get_grid(grid_name);
...
void AtmosphereOutput::init()
{
  ...
  for (const auto& fname : m_fields_names) {
    ...
    for (int i=0; i<layout.rank(); ++i) {
      ...
      dimname += dimname=="dim" ? std::to_string(dims[i]) : "";
      ...
      if (is_partitioned) {
        EKAT_REQUIRE_MSG (m_decomp_dimname=="",
            "Error! Decomposed dimension name was already set.\n"
