 ...
 , m_add_time_dim (true)
{
    ...
    for (const auto& grid_name : io_grid_aliases) {
      if (f_pl.isSublist(grid_name)) {
        ...
        if (pl.isParameter("IO Grid Name"))
          io_grid_name = pl.get<std::string>("IO Grid Name");
        break;
      }
    }
  ...
  auto grid_after_vr = fm_model->get_grid();
  if (use_vertical_remap_from_file) {
  ...
  if (use_online_remapper || use_horiz_remap_from_file) {
    ...
    if (use_horiz_remap_from_file) {
      ...
      grid_after_hr = grids_mgr->get_grid(io_grid_name);
      // Construct a generic remapper (likely, Dyn->PhysicsGLL)
      m_horiz_remapper = grids_mgr->create_remapper(grid_after_vr,grid_after_hr);
    }
...
void AtmosphereOutput::init()
{
  ...
  // Make sure scorpio fields are inited with correct value for accumulation
    ...
    for (const auto& [fname, avg_cnt_name] : m_field_to_avg_cnt_map) {
      ...
      if (is_write_step) {
        ...
        if (output_step and m_avg_type==OutputAvgType::Average) {
          ...
          compute_mask<Comparison::LT>(count,m_avg_coeff_threshold*nsteps_since_last_output,mask);

          // To set count=fill_value (FV) where mask=1, simply add mask*fill_value to count
          // Recall, y += a*x will set y=FV if, upon entry, "x=FV", "y=FV", or "a=FV AND x!=0"
          count.update<Device,CombineMode::ScaleAdd>(mask,m_fill_value,float(1.0));
        }
      }
    }
  }
...
void AtmosphereOutput::set_avg_cnt_tracking(const std::string& name, const FieldLayout& layout)
{
  ...
  FieldIdentifier cnt_id (avg_cnt_name,layout,nondim,fm_scorpio->get_grid()->name());
  Field cnt (cnt_id);
  cnt.allocate_view();
