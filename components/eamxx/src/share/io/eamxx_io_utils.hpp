...
enum class FileType { ModelOutput, ModelRestart, HistoryRestart, Unset };
...
inline std::string e2str(const FileType avg) {
  ...
  case FT::ModelOutput:
    return "model-output";
  case FT::ModelRestart:
    return "model-restart";
  case FT::HistoryRestart:
...
enum class OutputAvgType { Instant, Max, Min, Average, Invalid };
