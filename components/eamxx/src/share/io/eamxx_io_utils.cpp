                   ...
                   const std::shared_ptr<const AbstractGrid>& grid)
{
  ...
  std::regex vert_divergence (R"((d([A-Za-z0-9_]+)(dp|dz))_vert_divergence$)");
  ...
  else if (std::regex_search(diag_field_name,matches,vert_divergence)) {
    diag_name = "VertDivergenceDiag";
    params.set("grid_name", grid->name());
    params.set<std::string>("field_name", matches[1].str());
    params.set<std::string>("divergence_method", matches[2].str());
  }
