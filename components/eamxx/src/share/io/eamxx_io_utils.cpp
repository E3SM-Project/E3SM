...
std::string find_filename_in_rpointer(const std::string &filename_prefix,
  ...
  // values of REST_N/REST_OPTION used in the previous run. Also, model restart
  // is *always* INSTANT.
  if (model_restart) {
    EKAT_REQUIRE_MSG(
        avg_type == OutputAvgType::Instant,
        "Error! Model restart output should have INSTANT avg type.\n"
        " - input avg_type: " +
            e2str(avg_type) + "\n");
    pattern_str += e2str(OutputAvgType::Instant) +
                   R"(.n(step|sec|min|hour|day|month|year)s_x\d+)";
  } else {
    EKAT_REQUIRE_MSG(control.output_enabled(),
                     "Error! When restarting an output stream, we need a valid "
                     "IOControl structure.\n"
                     " - filename prefix: " +
                         filename_prefix + "\n");
    pattern_str += e2str(avg_type) + "." + control.frequency_units + "_x" +
  ...
  if (found) {
    ...
    if (model_restart) {
      EKAT_ERROR_MSG("Error! Restart requested, but no model restart file "
                     "found in 'rpointer.atm'.\n"
      ...
      EKAT_ERROR_MSG("Error! Restart requested, but no history restart file "
                     "found in 'rpointer.atm'.\n"
                     "   hist restart filename prefix: " +
                     filename_prefix +
                     "\n"
                     "   hist restart filename pattern: " +
                     pattern_str +
                     "\n"
                     "   run t0           : " +
                     run_t0.to_string() +
                     ...
                     "\n"
                     "   output freq units: " +
                     control.frequency_units +
                     "\n"
                     "   rpointer content:\n" +
                     content +
                     "\n\n"
                     " Did you change output specs (avg type, freq, or freq "
                     "units) across restart? If so, "
                     "please, remember that "
                     "it is not allowed.\n"
                     " It is also possible you are using a rhist file create "
                     "before commit 6b7d441330d. That "
                     "commit changed how "
                     "rhist file names\n"
                     " are formed. In particular, we no longer use "
                     "INSTANT.${REST_OPTION}_x${REST_N}, but we "
                     "use the avg type, "
                     "and freq/freq_option\n"
                     " of the output stream (to avoid name clashes if 2 "
                     "streams only differ for one of "
                     "those). If you want to use "
                     "your rhist file,\n"
                     " please rename it, so that the avg-type, freq, and "
                     "freq_option reflect those of the "
                     "output stream.\n");
    }
  }
...
void write_timestamp(const std::string &filename, const std::string &ts_name,
  ...
  if (write_nsteps) {
    scorpio::set_attribute(filename, "GLOBAL", ts_name + "_nsteps",
                           ts.get_num_steps());
  }
}
...
util::TimeStamp read_timestamp(const std::string &filename,
  ...
  auto ts = util::str_to_time_stamp(
      scorpio::get_attribute<std::string>(filename, "GLOBAL", ts_name));
  if (read_nsteps and
      scorpio::has_attribute(filename, "GLOBAL", ts_name + "_nsteps")) {
    ts.set_num_steps(
        scorpio::get_attribute<int>(filename, "GLOBAL", ts_name + "_nsteps"));
  }
  return ts;
}
...
create_diagnostic(const std::string &diag_field_name,
  ...
  // Note: use raw string syntax R"(<string>)" to avoid having to escape the
  // \ character Note: the number for field_at_p/h can match positive
  // integer/floating-point numbers
  std::regex field_at_l(R"(([A-Za-z0-9_]+)_at_(lev_(\d+)|model_(top|bot))$)");
  std::regex field_at_p(R"(([A-Za-z0-9_]+)_at_(\d+(\.\d+)?)(hPa|mb|Pa)$)");
  std::regex field_at_h(
      R"(([A-Za-z0-9_]+)_at_(\d+(\.\d+)?)(m)_above_(sealevel|surface)$)");
  ...
  std::regex vert_contract(
      "([A-Za-z0-9_]+)_vert_(avg|sum)(_((dp|dz)_weighted))?$");
  ...
  if (std::regex_search(diag_field_name, matches, field_at_l)) {
    ...
    params.set<std::string>("temperature_kind", matches[1].str() != ""
                                                    ? matches[1].str()
                                                    : std::string("Tot"));
