...
minicam_data=importdata('RCP_MiniCAM_4.5_nogrowth.txt',' ');
...
for ind=1:(length(years)-1)
    ...
    for k=1:(length(annual)-1)
        ...
        dlmwrite(['processed/urban/gurbn.', num2str(annual(k)), '.txt'], GLMurbn2005,'precision','%.6f','delimiter',' ');
        dlmwrite(['processed/urban/gothr.', num2str(annual(k)), '.txt'], eval(['landarea_halfdeg-GLMpast',num2str(annual(k)),'-GLMcrop',num2str(annual(k)),'-GLMurbn2005']) ,'precision','%.6f','delimiter',' ');
    end;
...
dlmwrite('processed/urban/gurbn.2100.txt',GLMurbn2005 ,'precision','%.6f','delimiter',' ');
dlmwrite('processed/urban/gothr.2100.txt',landarea_halfdeg-GLMpast2100-GLMcrop2100-GLMurbn2005 ,'precision','%.6f','delimiter',' ');
