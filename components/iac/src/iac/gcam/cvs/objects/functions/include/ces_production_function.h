#ifndef _CES_PRODUCTION_FUNCTION_H_
#define _CES_PRODUCTION_FUNCTION_H_
#if defined(_MSC_VER)
#pragma once
#endif
...
/*! 
* \file ces_production_function.h
* \ingroup Objects
* \brief CESProductionFunction class header file.
*
...
*/
class CESProductionFunction : public AProductionFunction {
public:
...
private:
    double normalizeAlphaZero( InputSet& input, double aAlphaZero, double sigma, const int aPeriod ) const;
    
    double calcCapitalScaler( const InputSet& input, double aAlphaZero, double sigma,
                              double capitalStock, const int aPeriod ) const;
    
    double calcCapitalRateScaler( const InputSet& input, double sigma, const int aPeriod ) const;
    
    double calcFinalProfitScaler( const InputSet& input, const std::string& regionName,
                                  const std::string& sectorName, int period, double alphaZero,
                                  double sigma ) const;
    
    double calcExpProfitScaler( const InputSet& input, double aLifetimeYears,
                                const std::string& regionName, const std::string& sectorName,
                                int period, double alphaZero, double sigma ) const;
};

#endif // _CES_PRODUCTION_FUNCTION_H_
