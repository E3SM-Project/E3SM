...
!
  ...
  ! Unit Numbers
  integer, public :: logunit_atm = 6        ! "stdout" log file unit number, default is 6
...
!-----------------------------------------------------------------------
  ...
  subroutine ncd_pio_openfile(file, fname, mode)
    ...
    if (masterproc) then
       write(logunit_atm,*) trim(subname),' opening ', trim(fname), file%fh
    ...
    if(ierr/= PIO_NOERR) then
       ...
       write(logunit_atm,*) 'Opened existing file ', trim(fname), file%fh
    end if
...
!-----------------------------------------------------------------------
  ...
  subroutine ncd_pio_createfile(file, fname)
    ...
    if(ierr/= PIO_NOERR) then
       ...
       write(logunit_atm,*) 'Opened file ', trim(fname),  ' to write', file%fh
    end if
  ...
  subroutine check_var(ncid, varname, vardesc, readvar, print_err )
    ...
    if (ret /= PIO_noerr) then
       ...
       if (masterproc .and. log_err) &
            write(logunit_atm,*) subname//': variable ',trim(varname),' is not on dataset'
    end if
...
!-----------------------------------------------------------------------
  ...
  subroutine check_dim(ncid, dimname, value)
    ...
    if (dimlen /= value) then
       write(logunit_atm,*) subname//' ERROR: mismatch of input dimension ',dimlen, &
...
!-----------------------------------------------------------------------
  ...
  subroutine ncd_inqvid(ncid,name,varid,vardesc,readvar)
    ...
    if (present(readvar)) then
       ...
       if (ret /= PIO_noerr) then
          if (masterproc) write(logunit_atm,*) subname//': variable ',trim(name),' is not on dataset'
          readvar = .false.
       else
          readvar = .true.
       end if
...
!-----------------------------------------------------------------------
  ...
  subroutine ncd_defvar_bynf(ncid, varname, xtype, ndims, dimid, varid, &
    ...
    if (present(dimid)) then
       ...
       if (ndims /= 0) then
          write(logunit_atm,*) subname//' ERROR: dimid not supplied and ndims ne 0 ',trim(varname),ndims
          call shr_sys_abort()
       endif
    endif
    ...
    if (masterproc .and. debug > 1) then
       write(logunit_atm,*) trim(subname),' Defining variable = ', trim(varname)
    ...
    if (present(flag_values)) then
       ...
       if ( .not. present(flag_meanings)) then
          write(logunit_atm,*) 'Error in defining variable = ', trim(varname)
...
!-----------------------------------------------------------------------
  ...
  subroutine ncd_io_int_var1(varname, data, dim1name, flag, ncid, nt, readvar)
    ...
    if (masterproc .and. debug > 1) then
       write(logunit_atm,*) subname//' ',trim(flag),' ',trim(varname),' ',trim(dim1name)
    end if
    ...
    elseif (flag == 'write') then
       ...
       if (masterproc) then
          write(logunit_atm,*) subname//' ERROR: unsupported flag ',trim(flag)
          call shr_sys_abort()
       endif
...
!-----------------------------------------------------------------------
  ...
  subroutine ncd_io_log_var1(varname, data, dim1name, &
    ...
    if (masterproc .and. debug > 1) then
       write(logunit_atm,*) subname//' ',trim(flag),' ',trim(varname)
    end if
...
!-----------------------------------------------------------------------
  ...
  subroutine ncd_io_real_var1(varname, data, dim1name, &
    ...
    if (masterproc .and. debug > 1) then
       write(logunit_atm,*) trim(subname),' ',trim(flag),' ',trim(varname)
    endif
...
!------------------------------------------------------------------------
  ...
  subroutine ncd_getiodesc(ncid, ndims, dims, dimids, xtype, iodnum)
       ...
       if (ndims == iodesc_list(n)%ndims .and. xtype == iodesc_list(n)%type) then
          ...
          if (found) then
             ...
             if (iodnum > num_iodesc) then
                write(logunit_atm,*) trim(subname),' ERROR: iodnum out of range ',iodnum,num_iodesc
                call shr_sys_abort()
             endif
             RETURN
          endif
    ...
    if (ndims > 0) then 
       ...
       iodnum = num_iodesc
       if (masterproc .and. debug > 1) then
          write(logunit_atm,*) trim(subname),' creating iodesc at iodnum,ndims,dims(1:ndims),xtype',&
    ...
    do m = 1, lsize
      compDOF(m) = m                ! JW TODO: fix this    converting 0-based IDs to 1-based
    enddo
    if (debug > 1) then
       do m = 0,npes-1
          if (iam == m) then
             write(logunit_atm,*) trim(subname),' sizes1  = ',iam,gsize,lsize,npes
             write(logunit_atm,*) trim(subname),' compDOF = ',iam,size(compDOF),minval(compDOF),maxval(compDOF)
             call shr_sys_flush(logunit_atm)
          endif
          call mpi_barrier(mpicom_atm,status)
       enddo
    endif
