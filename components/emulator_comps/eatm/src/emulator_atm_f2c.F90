module emulator_atm_f2c_v2
   !----------------------------------------------------------------------------
   ! Fortran interface declarations for the C++ EmulatorAtm class.
   !
   ! This module provides the iso_c_binding interfaces that allow the Fortran
   ! MCT wrapper (atm_comp_mct.F90) to call the C++ EmulatorAtm implementation.
   !----------------------------------------------------------------------------
   ...
   ! Public interface for atmosphere emulator
   ...
   interface
      ...
      !---------------------------------------------------------------------------
      ! Atmosphere emulator interface
      !---------------------------------------------------------------------------

      subroutine emulator_atm_create_instance(f_comm, comp_id, input_file, &
         log_file, run_type, start_ymd, start_tod) bind(c)
         use iso_c_binding
      ...
      subroutine emulator_atm_set_grid_data(nx, ny, num_local_cols, num_global_cols, &
      ...
      subroutine emulator_atm_init_coupling_indices(export_fields, import_fields) bind(c)
         use iso_c_binding
      ...
      subroutine emulator_atm_setup_coupling(import_data, export_data, &
         num_imports, num_exports, field_size) bind(c)
         use iso_c_binding
      ...
      function emulator_atm_get_num_local_cols() result(n) bind(c)
         use iso_c_binding
         integer(c_int) :: n
      end function emulator_atm_get_num_local_cols

      function emulator_atm_get_num_global_cols() result(n) bind(c)
         use iso_c_binding
         integer(c_int) :: n
...
end module emulator_atm_f2c_v2
