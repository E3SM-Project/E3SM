 ...
 * @brief Implementation of atmosphere coupling functions.
 *
 * Provides the data transfer functions between MCT attribute vectors
 * and the internal AtmFieldManager storage.
 *
 * @see atm_coupling.hpp for function documentation
 */
...
/**
 * @brief Import fields from MCT buffer to field manager.
 *
 * MCT aVect layout is column-major (Fortran): rAttr(nfields, lsize)
 * In C this translates to: data[col * nfields + field_idx]
 */
...
/**
 * @brief Export fields from field manager to MCT buffer.
 */
