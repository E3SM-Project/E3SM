...
module aero_model
  ...
  use modal_aero_wateruptake,  only: aw_scheme, &
                                     aw_rh_max, &
                                     aw_mam_rh_min, &
                                     aw_m7_rh_min, &
                                     aw_l_hysteresis, &
                                     aw_l_rh_clearsky, &
                                     aw_l_m7_rh_min
  use aer_rad_props,  only: aw_insitu_diag
...
contains
  ...
  subroutine aero_model_readnl(nlfile)
    ...
    integer  :: aer_aw_scheme        = 0          ! 0 - default MAM scheme; 1 - k-kohler ; 2 - M7 (Jacobson 1996); 3 - M7 (Jacobson 2005)
    real(r8) :: aer_aw_mam_rh_min    = 0.35_r8    ! the minimum rh below which wet radius is equal to dry radius, used for MAM
    real(r8) :: aer_aw_m7_rh_min     = 0.45_r8    ! the minimum rh below which wet radius is equal to dry radius, used for M7
    real(r8) :: aer_aw_rh_max        = 0.98_r8    ! the maximum rh to solve for water uptake process
    logical  :: aer_aw_l_hysteresis  = .TRUE.     ! TRUE if hysteresis effect is considered for MAM
    logical  :: aer_aw_l_rh_clearsky = .TRUE.     ! TRUE if clear-sky RH is used for water uptake process
    logical  :: aer_aw_l_m7_rh_min   = .FALSE.    ! FALSE if RHmin and highest molality are not considered
    logical  :: aer_aw_insitu_diag   = .FALSE.    ! TRUE if add aerocom insitu diag 

    namelist /aerosol_nl/ &
       aer_wetdep_list, aer_drydep_list, sol_facti_cloud_borne, &
       seasalt_emis_scale, sscav_tuning, sol_factb_interstitial, &
    ...
    call mpibcast(aer_aw_scheme, 1,                                 mpiint,  0, mpicom)
    call mpibcast(aer_aw_rh_max, 1,                                 mpir8,   0, mpicom)
    call mpibcast(aer_aw_mam_rh_min, 1,                             mpir8,   0, mpicom)
    call mpibcast(aer_aw_m7_rh_min, 1,                              mpir8,   0, mpicom)
    call mpibcast(aer_aw_l_hysteresis, 1,                           mpilog,  0, mpicom)
    call mpibcast(aer_aw_l_rh_clearsky, 1,                          mpilog,  0, mpicom)
    call mpibcast(aer_aw_l_m7_rh_min, 1,                            mpilog,  0, mpicom)
    call mpibcast(aer_aw_insitu_diag, 1,                            mpilog,  0, mpicom)
#endif

    wetdep_list = aer_wetdep_list
    drydep_list = aer_drydep_list

    aw_insitu_diag   = aer_aw_insitu_diag
    aw_scheme        = aer_aw_scheme
    aw_rh_max        = aer_aw_rh_max
    aw_mam_rh_min    = aer_aw_mam_rh_min
    aw_m7_rh_min     = aer_aw_m7_rh_min
    aw_l_hysteresis  = aer_aw_l_hysteresis
    aw_l_rh_clearsky = aer_aw_l_rh_clearsky
    aw_l_m7_rh_min   = aer_aw_l_m7_rh_min
