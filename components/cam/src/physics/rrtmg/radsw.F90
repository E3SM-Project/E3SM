...
!-----------------------------------------------------------------------
   ...
   do i = 1, ncol
      ...
      call linear_interp(pmid_host(i,:),E_pmid(i,:), &
   ...
   do i = 1, ncol
!      call linear_interp(pint_host(i,1:pver),E_pint(i,1:rrtmg_levs-1), &
...
!                    r_state%tlay(i,2:pverp), tlay_five(i,2:rrtmg_levs),pver,pver_five)
      ...
      tlev_five(i,rrtmg_levs+1) = r_state%tlev(i,pverp+1)
       ...
       do k = 2,pver_five
          dy = (log(E_pint(i,k)) - log(E_pmid(i,k))) / (log(E_pmid(i,k-1)) - log(E_pmid(i,k)))
