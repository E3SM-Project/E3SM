...
!-----------------------------------------------------------------------------------------
      ...
      ! Size of segment 2
      mi1(2) = channel%seg(2)%mi1  
      mim(2) = channel%seg(2)%mim
      mip(2) = channel%seg(2)%mip   
      mj1(2) = channel%seg(2)%mj1  
      mjm(2) = channel%seg(2)%mjm    
      mjp(2) = channel%seg(2)%mjp    

      ! Size of segment 3
      mi1(3) = channel%seg(3)%mi1  
      mim(3) = channel%seg(3)%mim
      mip(3) = channel%seg(3)%mip   
      mj1(3) = channel%seg(3)%mj1   
      mjm(3) = channel%seg(3)%mjm    
      mjp(3) = channel%seg(3)%mjp    

      ! Size of segment 4
      mi1(4) = channel%seg(4)%mi1  
      mim(4) = channel%seg(4)%mim
      mip(4) = channel%seg(4)%mip   
      mj1(4) = channel%seg(4)%mj1  
      mjm(4) = channel%seg(4)%mjm    
      mjp(4) = channel%seg(4)%mjp    
      ...
      IF (PRESENT(TH3D)) THEN
        ...
        CASE(3)
          CALL HALO_GROUP3 (mi1(1),mim(1),mip(1),mj1(1),mjm(1),mjp(1), &
                            mi1(2),mim(2),mip(2),mj1(2),mjm(2),mjp(2), &
                            mi1(3),mim(3),mip(3),mj1(3),mjm(3),mjp(3), &
                            mi1(4),mim(4),mip(4),mj1(4),mjm(4),mjp(4), &
                            num_halo,NK1,                              &
                            channel%seg(1)%TH3D(:,:,2:NK2),            &
                            channel%seg(2)%TH3D(:,:,2:NK2),            &
                            channel%seg(3)%TH3D(:,:,2:NK2),            &
...
!---------------------------------
      IF (PRESENT(PSI)) THEN
        ...
        CASE(3)
          CALL HALO_GROUP3_2 (mi1(1),mim(1),mip(1),mj1(1),mjm(1),mjp(1),          &
                              mi1(2),mim(2),mip(2),mj1(2),mjm(2),mjp(2),          &
                              mi1(3),mim(3),mip(3),mj1(3),mjm(3),mjp(3),          &
                              mi1(4),mim(4),mip(4),mj1(4),mjm(4),mjp(4),num_halo, &
                              channel%seg(1)%PSI,channel%seg(2)%PSI,              &
                              channel%seg(3)%PSI,channel%seg(4)%PSI,OVERLAP=.TRUE.)
        END SELECT
        endif                            
...
!=========================================================================================      
      SUBROUTINE HALO_GROUP3 (mi1_1,mim_1,mip_1,mj1_1,mjm_1,mjp_1, &
                              mi1_2,mim_2,mip_2,mj1_2,mjm_2,mjp_2, &  
                              mi1_3,mim_3,mip_3,mj1_3,mjm_3,mjp_3, &  
                              mi1_4,mim_4,mip_4,mj1_4,mjm_4,mjp_4, &  
                              num_halo,ksize,a1,a2,a3,a4,OVERLAP)
!=========================================================================================      
!     specify the halo values of GROUP 3 channel
!     (SEG1: Face 4, SEG2: Face 5, SEG3: Face 2, SEG4: Face 6)
      
      INTEGER, INTENT(IN) :: mi1_1,mim_1,mip_1,mj1_1,mjm_1,mjp_1
      INTEGER, INTENT(IN) :: mi1_2,mim_2,mip_2,mj1_2,mjm_2,mjp_2
      INTEGER, INTENT(IN) :: mi1_3,mim_3,mip_3,mj1_3,mjm_3,mjp_3
