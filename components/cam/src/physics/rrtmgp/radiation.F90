...
logical function string_in_list(string, list)
  ...
  function rte_lw(k_dist, gas_concs, p_lay, t_lay, p_lev,    &
    ...
    error_msg = k_dist%gas_optics(p_lay, p_lev, t_lay, t_sfc, gas_concs, &
                                  ...
                                  col_dry, t_lev)
    call assert_range(optical_props%tau, 0._wp, huge(optical_props%tau), 'lw_gas_optics')
    call optical_props%set_name('lw_gas_optics')
    call handle_error(optical_props%validate())
    
    if (error_msg /= '') return
    ! ----------------------------------------------------
    ! Clear sky is gases + aerosols (if they're supplied)
    !
    if(present(aer_props)) call assert_range(aer_props%tau, 0._wp, huge(aer_props%tau), 'lw_aer_optics')
    if(present(aer_props)) error_msg = aer_props%increment(optical_props)
    if(error_msg /= '') return
    call optical_props%set_name('lw_gas_and_aerosol_optics')
    call handle_error(optical_props%validate())
    ...
    !
    call assert_range(cloud_props%tau, 0._wp, huge(cloud_props%tau), 'lw_cld_optics')
    error_msg = cloud_props%increment(optical_props)
    if(error_msg /= '') return
    call optical_props%set_name('lw_gas_and_aerosol_and_cloud_optics')
    call handle_error(optical_props%validate())
  ...
  function rte_sw(k_dist, gas_concs, p_lay, t_lay, p_lev, &
    ...
    error_msg = k_dist%gas_optics(p_lay, p_lev, t_lay, gas_concs,  &
                                  ...
                                  col_dry)
    if (error_msg /= '') return
    call assert_range(optical_props%tau, 0._wp, huge(optical_props%tau), 'sw_gas_optics')
    call optical_props%set_name('sw_gas_optics')
    call handle_error(optical_props%validate())
    !
    ...
    ! Clear sky is gases + aerosols (if they're supplied)
    !
    if(present(aer_props)) call assert_range(aer_props%tau, 0._wp, huge(aer_props%tau), 'sw_aer_optics')
    if(present(aer_props)) error_msg = aer_props%increment(optical_props)
    if(error_msg /= '') return
    call optical_props%set_name('sw_gas_and_aerosol_optics')
    call handle_error(optical_props%validate())
    ...
    ! All-sky fluxes = clear skies + clouds
    !
    call assert_range(cloud_props%tau, 0._wp, huge(cloud_props%tau), 'sw_cld_optics')
    error_msg = cloud_props%increment(optical_props)
    if(error_msg /= '') return
    call optical_props%set_name('sw_gas_and_aerosol_and_cloud_optics')
    call handle_error(optical_props%validate())
