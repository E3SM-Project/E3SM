...
#ifdef FIVE
   integer, parameter :: pverp_micro = pverp!_five +for initial testing purposes set equal
   integer, parameter :: pver_micro = pver!_five + for initial testing purposes set euql 
#else
...
subroutine micro_mg_cam_tend(state, ptend, dtime, pbuf)
   ...
   real(r8) :: rho(state%psetcols,pver_micro)
  ...
  real(r8), pointer :: frzdep(:,:)
   ...
   real(r8) :: rho_grid(pcols,pver_micro)
   real(r8) :: liqcldf_grid(pcols,pver_micro)
   real(r8) :: qsout_grid(pcols,pver_micro)
   real(r8) :: ncic_grid(pcols,pver_micro)
   real(r8) :: niic_grid(pcols,pver_micro)
   real(r8) :: rel_fn_grid(pcols,pver_micro)    ! Ice effective drop size at fixed number (indirect effect) (microns) - on grid
   real(r8) :: qrout_grid(pcols,pver_micro)
   real(r8) :: drout2_grid(pcols,pver_micro)
   real(r8) :: dsout2_grid(pcols,pver_micro)
   real(r8) :: nsout_grid(pcols,pver_micro)
   real(r8) :: nrout_grid(pcols,pver_micro)
   real(r8) :: reff_rain_grid(pcols,pver_micro)
   real(r8) :: reff_snow_grid(pcols,pver_micro)
   real(r8) :: cld_grid(pcols,pver_micro)
   real(r8) :: pdel_grid(pcols,pver_micro)
   real(r8) :: prco_grid(pcols,pver_micro)
   real(r8) :: prao_grid(pcols,pver_micro)
   real(r8) :: icecldf_grid(pcols,pver_micro)
   real(r8) :: icwnc_grid(pcols,pver_micro)
   real(r8) :: icinc_grid(pcols,pver_micro)
   ...
   real(r8) :: nc_grid(pcols,pver_micro)
   real(r8) :: ni_grid(pcols,pver_micro)
   real(r8) :: qr_grid(pcols,pver_micro)
   real(r8) :: nr_grid(pcols,pver_micro)
   real(r8) :: qs_grid(pcols,pver_micro)
   real(r8) :: ns_grid(pcols,pver_micro)
...
!   levels to be used here
#ifdef FIVE
   nlev_micro = nlev!_five for testing purposes, just use same levels
   top_lev_micro = top_lev!_five(1) ! testing
#else
   nlev_micro = nlev
   ...
   call five_syncronize_e3sm(state_loc,dtime,p0_five_ref,pint_five,pmid_five,&
...
#ifdef FIVE
!   packed_p = packer%pack(pmid_five) uncomment in future
...
#ifdef FIVE
!   packed_pdel = packer%pack(pdel_five) uncomment in future
...
#ifdef FIVE
!   packed_pint = packer%pack_interface(five_pint) uncomment in future
   ...
   ! +PAB, for FIVE, this will need to be modified to use FIVE variables
