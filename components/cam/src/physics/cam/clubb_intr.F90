...
! numbers are coefficients of the empirical equation
 ...
 !  Initialize physics tendency arrays, copy the state to state1 array to use in this routine
   ...
   if (.not. micro_do_icesupersat) then    
     call physics_ptend_init(ptend_loc, 'clubb_ice1', ls=.true., lu=.true., lv=.true., lq=lq)
   endif
   ...
   if (micro_do_icesupersat) then
     ...
     call physics_ptend_init(ptend_all, 'clubb_ice2')
   endif
...
!PMA adds fields for gustiness
   ...
   if (micro_do_icesupersat) then
     ...
     call physics_ptend_init(ptend_loc, 'iceadj', ls=.true., lq=lq2 )
   ...
   if (micro_do_icesupersat) then
     call physics_ptend_init(ptend_loc, 'clubb_ice3', ls=.true., lu=.true., lv=.true., lq=lq)
   endif
...
!                     rtpthlp_in = rtpthlp_in + rtpthlp_mc_out * dtime
   ...
   if (.not. micro_do_icesupersat) then
      call physics_ptend_init(ptend_all, 'clubb_ice4')
   endif
   call physics_ptend_sum(ptend_loc,ptend_all,ncol)
   call physics_update(state1,ptend_loc,hdtime)
   ...
   call physics_ptend_init(ptend_loc, 'clubb_det', ls=.true., lq=lqice)
...
!-------------------------------------------------------------------------------
    ...
    lq(:) = .TRUE.
    call physics_ptend_init(ptend, 'clubb_srf', lq=lq)
