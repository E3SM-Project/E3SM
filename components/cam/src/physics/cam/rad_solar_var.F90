  ...
  subroutine rad_solar_var_init( )
    ...
    if ( do_spctrl_scaling ) then
       ...
       if ( .not.has_ref_spectrum ) then
          call endrun('rad_solar_var_init: solar input file must have reference irradiance spectrum')
       endif
       ...
       call integrate_spectrum( nbins, nradbins, we, radbinmin, radbinmax, ssi_ref, ref_band_irrad)
