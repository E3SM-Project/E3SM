...
!    write(*,*) 'gxlin-test1, lq = ', ptend%lq(49)
    call outfld('bc_bf_chem', state%q(:,:,18), pcols, lchnk   )
    !Guangxing Lin, end
...
!    write(*,*) 'gxlin-test3, lq = ', ptend%lq(49)
    call outfld('bc_af_chem', state%q(:,:,18), pcols, lchnk   )
    !Guangxing Lin, end
...
if (l_tracer_aero) then
    ...
    call outfld('bc_bf_drydep', state%q(:,:,18), pcols, lchnk   )
    !Guangxing Lin, end
    ...
    !Guangxing Lin
    call outfld('bc_af_drydep', state%q(:,:,18), pcols, lchnk   )
    !Guangxing Lin, end
...
if (l_tracer_aero) then
    ...
    elseif( microp_scheme == 'MG' ) then
       ...
       !Guangxing Lin    
       call outfld('bc_bf_ndrop', state%q(:,:,18), pcols, lchnk   )
       !Guangxing Lin    
...
!!== KZ_WATCON 
        ...
        if (l_st_mic) then
          ...
          !Guangxing Lin
          ! call outfld('bc_bf_ndrop', state%q(:,:,44), pcols, lchnk   )
          !Guangxing Lin, end
          ! Have to scale and apply for full timestep to get tend right
          ! (see above note for macrophysics).
          call physics_ptend_scale(ptend, 1._r8/cld_macmic_num_steps, ncol)
          ...
          !Guangxing Lin
          ! call outfld('bc_af_ndrop', state%q(:,:,44), pcols, lchnk   )
          !Guangxing Lin, end
        ...
        ! If microphysics is off, set surface cloud liquid/ice and rain/snow fluxes to zero
          ...
          !Guangxing Lin
           call outfld('bc_af_ndrop', state%q(:,:,18), pcols, lchnk   )
          !Guangxing Lin, end
...
if (l_tracer_aero) then
    ...
    if ( .not. deep_scheme_does_scav_trans() ) then
       ...
       call outfld('bc_bf_wetdep', state%q(:,:,18), pcols, lchnk   )
       !Guangxing Lin, end
       call aero_model_wetdep( ztodt, dlf, dlf2, cmfmc2, state, sh_e_ed_ratio,       & !Intent-ins
          ...
          !Guangxing Lin
           !call outfld('so4_test2', ptend%q(:,:,44), pcols, lchnk   )
          !Guangxing Lin, end
       call physics_update(state, ptend, ztodt, tend)
       !Guangxing Lin
       call outfld('bc_af_wetdep', state%q(:,:,18), pcols, lchnk   )
       !Guangxing Lin, end
