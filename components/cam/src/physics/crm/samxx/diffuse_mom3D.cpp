...
void diffuse_mom3D(real5d &tk) {
  ...
  parallel_for(
      Bounds<4>(nzm, ny, nx + 1, ncrms), YAKL_LAMBDA(int k, int j, int i, int icrm) {
        int jb = j - 1;
        int kc = k + 1;
        ...
        fu(k, j + 1, i, icrm) =
            -2.0 * tkx * (u(k, j + offy_u, ic - 1 + offx_u, icrm) - u(k, j + offy_u, i - 1 + offx_u, icrm));
        ...
        fv(k, j + 1, i, icrm) =
            -tkx * (v(k, j + offy_v, ic - 1 + offx_v, icrm) - v(k, j + offy_v, i - 1 + offx_v, icrm) +
        ...
        fw(k, j + 1, i, icrm) =
            -tkx * (w(kc, j + offy_w, ic - 1 + offx_w, icrm) - w(kc, j + offy_w, i - 1 + offx_w, icrm) +
  ...
  parallel_for(
      Bounds<4>(nzm, ny, nx, ncrms), YAKL_LAMBDA(int k, int j, int i, int icrm) {
        int kc = k + 1;
        int ib = i - 1;
        dudt(na - 1, k, j, i, icrm) =
            dudt(na - 1, k, j, i, icrm) - (fu(k, j + 1, i + 1, icrm) - fu(k, j + 1, ib + 1, icrm));
        dvdt(na - 1, k, j, i, icrm) =
            dvdt(na - 1, k, j, i, icrm) - (fv(k, j + 1, i + 1, icrm) - fv(k, j + 1, ib + 1, icrm));
        dwdt(na - 1, kc, j, i, icrm) =
            dwdt(na - 1, kc, j, i, icrm) - (fw(k, j + 1, i + 1, icrm) - fw(k, j + 1, ib + 1, icrm));
      });
  ...
  //       for (int icrm=0; icrm<ncrms; icrm++) {
  parallel_for(
      Bounds<4>(nzm, ny + 1, nx, ncrms), YAKL_LAMBDA(int k, int j, int i, int icrm) {
        int jc = j + 1;
        int kc = k + 1;
        int kcu = min(kc, nzm - 1);
        int ib = i - 1;
        ...
        fv(k, j, i + 1, icrm) =
            -2.0 * tky * (v(k, jc - 1 + offy_v, i + offx_v, icrm) - v(k, j - 1 + offy_v, i + offx_v, icrm));
        ...
        fu(k, j, i + 1, icrm) =
            -tky * (u(k, jc - 1 + offy_u, i + offx_u, icrm) - u(k, j - 1 + offy_u, i + offx_u, icrm) +
        ...
        fw(k, j, i + 1, icrm) =
            -tky * (w(kc, jc - 1 + offy_w, i + offx_w, icrm) - w(kc, j - 1 + offy_w, i + offx_w, icrm) +
  ...
  parallel_for(
      Bounds<4>(nzm, ny, nx, ncrms), YAKL_LAMBDA(int k, int j, int i, int icrm) {
        ...
        int kc = k + 1;
        dudt(na - 1, k, j, i, icrm) =
            dudt(na - 1, k, j, i, icrm) - (fu(k, j + 1, i + 1, icrm) - fu(k, jb + 1, i + 1, icrm));
        dvdt(na - 1, k, j, i, icrm) =
            dvdt(na - 1, k, j, i, icrm) - (fv(k, j + 1, i + 1, icrm) - fv(k, jb + 1, i + 1, icrm));
        dwdt(na - 1, kc, j, i, icrm) =
            dwdt(na - 1, kc, j, i, icrm) - (fw(k, j + 1, i + 1, icrm) - fw(k, jb + 1, i + 1, icrm));
      });

  // for (int k=0; k<nzm; k++) {
  //  for (int icrm=0; icrm<ncrms; icrm++) {
  parallel_for(
      Bounds<2>(nzm, ncrms), YAKL_LAMBDA(int k, int icrm) {
        uwsb(k, icrm) = 0.0;
        vwsb(k, icrm) = 0.0;
      });
  ...
  //       for (int icrm=0; icrm<ncrms; icrm++) {
  parallel_for(
      Bounds<4>(nzm - 1, ny, nx, ncrms), YAKL_LAMBDA(int k, int j, int i, int icrm) {
        int jb = j - 1;
        int kc = k + 1;
        int ib = i - 1;
        ...
        fw(kc, j + 1, i + 1, icrm) = -2.0 * tkz *
                                     (w(kc, j + offy_w, i + offx_w, icrm) - w(k, j + offy_w, i + offx_w, icrm)) *
                                     rho(k, icrm) * iadz;
        tkz = rdz25 * (tk(0, k, j + offy_d, i + offx_d, icrm) + tk(0, k, j + offy_d, ib + offx_d, icrm) +
                       tk(0, kc, j + offy_d, i + offx_d, icrm) + tk(0, kc, j + offy_d, ib + offx_d, icrm));
        fu(kc, j + 1, i + 1, icrm) =
            -tkz *
            ((u(kc, j + offy_u, i + offx_u, icrm) - u(k, j + offy_u, i + offx_u, icrm)) * iadzw +
             (w(kc, j + offy_w, i + offx_w, icrm) - w(kc, j + offy_w, ib + offx_w, icrm)) * dzx) *
            rhow(kc, icrm);
        tkz = rdz25 * (tk(0, k, j + offy_d, i + offx_d, icrm) + tk(0, k, jb + offy_d, i + offx_d, icrm) +
                       tk(0, kc, j + offy_d, i + offx_d, icrm) + tk(0, kc, jb + offy_d, i + offx_d, icrm));
        fv(kc, j + 1, i + 1, icrm) =
            -tkz *
            ((v(kc, j + offy_v, i + offx_v, icrm) - v(k, j + offy_v, i + offx_v, icrm)) * iadzw +
             (w(kc, j + offy_w, i + offx_w, icrm) - w(kc, jb + offy_w, i + offx_w, icrm)) * dzy) *
            rhow(kc, icrm);
  ...
  //       for (int icrm=0; icrm<ncrms; icrm++) {
  parallel_for(
      Bounds<3>(ny, nx, ncrms), YAKL_LAMBDA(int j, int i, int icrm) {
        real rdz = 1.0 / dz(icrm);
        real rdz2 = rdz * rdz * grdf_z(nzm - 2, icrm);
        real tkz = rdz2 * grdf_z(nzm - 1, icrm) * tk(0, nzm - 1, j + offy_d, i + offx_d, icrm);
        fw(nz - 1, j + 1, i + 1, icrm) =
            -2.0 * tkz * (w(nz - 1, j + offy_d, i + offx_d, icrm) - w(nzm - 1, j + offy_w, i + offx_w, icrm)) /
  ...
  parallel_for(
      Bounds<4>(nzm, ny, nx, ncrms), YAKL_LAMBDA(int k, int j, int i, int icrm) {
        ...
        dudt(na - 1, k, j, i, icrm) =
            dudt(na - 1, k, j, i, icrm) - (fu(kc, j + 1, i + 1, icrm) - fu(k, j + 1, i + 1, icrm)) * rhoi;
        dvdt(na - 1, k, j, i, icrm) =
            dvdt(na - 1, k, j, i, icrm) - (fv(kc, j + 1, i + 1, icrm) - fv(k, j + 1, i + 1, icrm)) * rhoi;
      });
  ...
  parallel_for(
      Bounds<4>(nzm - 1, ny, nx, ncrms), YAKL_LAMBDA(int k, int j, int i, int icrm) {
        ...
        dwdt(na - 1, k + 1, j, i, icrm) =
            dwdt(na - 1, k + 1, j, i, icrm) - (fw(k + 2, j + 1, i + 1, icrm) - fw(k + 1, j + 1, i + 1, icrm)) * rhoi;
      });
}
