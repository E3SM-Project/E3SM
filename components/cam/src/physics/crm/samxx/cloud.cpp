      ...
      Bounds<4>(nzm, ny, nx, ncrms), YAKL_LAMBDA(int k, int j, int i, int icrm) {
        ...
        if (q(ind_q, k, j + offy_s, i + offx_s, icrm) > qsatt) {
          ...
          do {
            ...
            dfff = dlstarn * (q(ind_q, k, j + offy_s, i + offx_s, icrm) - qsatt) +
                   dlstarp * qp(ind_qp, k, j + offy_s, i + offx_s, icrm) - lstarn * dqsat - 1.0;
            dtabs = -fff / dfff;
            niter = niter + 1;
            tabs1 = tabs1 + dtabs;
        ...
        qp(ind_qp, k, j + offy_s, i + offx_s, icrm) =
            max(0.0, qp(ind_qp, k, j + offy_s, i + offx_s, icrm)); // just in case
      });
}
