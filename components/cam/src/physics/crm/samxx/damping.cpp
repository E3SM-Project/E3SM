  ...
  parallel_for(
      Bounds<2>(nzm, ncrms), YAKL_LAMBDA(int k, int icrm) {
        ...
        if ((k <= nzm - 1) && (k >= nzm - 1 - n_damp(icrm))) {
          tau(k, icrm) = tau_min * pow((tau_max / tau_min), ((z(nzm - 1, icrm) - z(k, icrm)) /
                                                             (z(nzm - 1, icrm) - z(nzm - 1 - n_damp(icrm), icrm))));
  ...
  parallel_for(
      Bounds<4>(nzm, ny, nx, ncrms), YAKL_LAMBDA(int k, int j, int i, int icrm) {
        ...
        if (k <= nzm - 1 && k >= nzm - 1 - n_damp(icrm)) {
          ...
          t(k, offy_s + j, offx_s + i, icrm) -=
              dtn * (t(k, offy_s + j, offx_s + i, icrm) - t0loc(k, icrm)) * tau(k, icrm);
