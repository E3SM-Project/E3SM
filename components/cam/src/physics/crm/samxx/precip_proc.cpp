...
void precip_proc(real5d &q, int ind_q, real5d &qp, int ind_qp) {
  ...
  parallel_for(
      Bounds<4>(nzm, ny, nx, ncrms), YAKL_LAMBDA(int k, int j, int i, int icrm) {
        ...
        real omn, omp, omg, qcc, qii, autor, autos, accrr, qrr, accrcs, accris, qss, accrcg, accrig, tmp, qgg, dq,
            qsatt, qsat;
        ...
        if (qn(k, j, i, icrm) + qp(ind_qp, k, j + offy_s, i + offx_s, icrm) > 0.0) {
          ...
          if (qn(k, j, i, icrm) > 0.0) {
            ...
            dq = dtn * (accrr * qcc + autor * (qcc - qcw0) + (accris + accrig) * qii + (accrcs + accrcg) * qcc +
                        autos * (qii - qci0));
            ...
            q(ind_q, k, j + offy_s, i + offx_s, icrm) =
                q(ind_q, k, j + offy_s, i + offx_s, icrm) + qp(ind_qp, k, j + offy_s, i + offx_s, icrm);
        ...
        q(ind_q, k, j + offy_s, i + offx_s, icrm) =
            q(ind_q, k, j + offy_s, i + offx_s, icrm) + (dq - qp(ind_qp, k, j + offy_s, i + offx_s, icrm));
      });
}
