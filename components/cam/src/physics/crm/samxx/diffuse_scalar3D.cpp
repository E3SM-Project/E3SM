...
void diffuse_scalar3D(real4d &field, real3d &fluxb, real3d &fluxt, real5d &tkh, int ind_tkh, real2d &flux) {
  auto &dx = ::dx;
  auto &dy = ::dy;
  auto &rhow = ::rhow;
  auto &adzw = ::adzw;
  ...
  if (dosgs) {
    ...
    parallel_for(
        Bounds<4>(nzm, ny, nx, ncrms), YAKL_LAMBDA(int k, int j, int i, int icrm) { dfdt(k, j, i, icrm) = 0.0; });
    ...
    parallel_for(
        Bounds<4>(nzm, ny + 1, nx + 1, ncrms), YAKL_LAMBDA(int k, int j, int i, int icrm) {
          if (j >= 1) {
            ...
            real tkx = rdx5 * (tkh(ind_tkh, k, j + offy_d - 1, i + offx_d - 1, icrm) +
                               tkh(ind_tkh, k, j + offy_d - 1, ic + offx_d - 1, icrm));
            flx_x(k + offz_flx, j, i, icrm) = -tkx * (field(k, j + offy_s - 1, ic + offx_s - 1, icrm) -
                                                      field(k, j + offy_s - 1, i + offx_s - 1, icrm));
          }
          if (i >= 1) {
            int jc = j + 1;
            real rdy5 = 0.5 * rdy2 * grdf_y(k, icrm);
            real tky = rdy5 * (tkh(ind_tkh, k, j + offy_d - 1, i + offx_d - 1, icrm) +
                               tkh(ind_tkh, k, jc + offy_d - 1, i + offx_d - 1, icrm));
            flx_y(k + offz_flx, j, i, icrm) = -tky * (field(k, jc + offy_s - 1, i + offx_s - 1, icrm) -
                                                      field(k, j + offy_s - 1, i + offx_s - 1, icrm));
          }
        });
    ...
    //  for (int icrm=0; icrm<ncrms; icrm++) {
    parallel_for(
        Bounds<2>(nzm, ncrms), YAKL_LAMBDA(int k, int icrm) { flux(k, icrm) = 0.0; });
    ...
    parallel_for(
        Bounds<4>(nzm, ny, nx, ncrms), YAKL_LAMBDA(int k, int j, int i, int icrm) {
          if (k <= nzm - 2) {
            ...
            real tkz =
                rdz5 * (tkh(ind_tkh, k, j + offy_d, i + offx_d, icrm) + tkh(ind_tkh, kc, j + offy_d, i + offx_d, icrm));
            flx_z(k + offz_flx, j + offy_flx, i + offx_flx, icrm) =
                -tkz * (field(kc, j + offy_s, i + offx_s, icrm) - field(k, j + offy_s, i + offx_s, icrm)) * rhoi;
    ...
    parallel_for(
        Bounds<4>(nzm, ny, nx, ncrms), YAKL_LAMBDA(int k, int j, int i, int icrm) {
          ...
          dfdt(k, j, i, icrm) = dtn * (dfdt(k, j, i, icrm) - (flx_z(k + offz_flx, j + offy_flx, i + offx_flx, icrm) -
                                                              flx_z(kb + offz_flx, j + offy_flx, i + offx_flx, icrm)) *
                                                                 rhoi);
...
void diffuse_scalar3D(real5d &field, int ind_field, real3d &fluxb, real3d &fluxt, real5d &tkh, int ind_tkh,
                      real2d &flux) {
  auto &dx = ::dx;
  auto &dy = ::dy;
  auto &rhow = ::rhow;
    ...
    parallel_for(
        Bounds<4>(nzm, ny, nx, ncrms), YAKL_LAMBDA(int k, int j, int i, int icrm) {
          if (k <= nzm - 2) {
            ...
            flx_z(k + offz_flx, j + offy_flx, i + offx_flx, icrm) =
                -tkz *
                (field(ind_field, kc, j + offy_s, i + offx_s, icrm) -
                 field(ind_field, k, j + offy_s, i + offx_s, icrm)) *
                rhoi;
            yakl::atomicAdd(flux(kc, icrm), flx_z(k + offz_flx, j + offy_flx, i + offx_flx, icrm));
          } else if (k == nzm - 1) {
            real tmp = 1.0 / adzw(nz - 1, icrm);
        ...
        Bounds<4>(nzm, ny, nx, ncrms), YAKL_LAMBDA(int k, int j, int i, int icrm) {
          ...
          dfdt(k, j, i, icrm) = dtn * (dfdt(k, j, i, icrm) - (flx_z(k + offz_flx, j + offy_flx, i + offx_flx, icrm) -
                                                              flx_z(kb + offz_flx, j + offy_flx, i + offx_flx, icrm)) *
                                                                 rhoi);
          field(ind_field, k, j + offy_s, i + offx_s, icrm) =
              field(ind_field, k, j + offy_s, i + offx_s, icrm) + dfdt(k, j, i, icrm);
        });
  }
}

void diffuse_scalar3D(real5d &field, int ind_field, real4d &fluxb, int ind_fluxb, real4d &fluxt, int ind_fluxt,
  ...
  if (dosgs) {
    ...
    //  for (int icrm=0; icrm<ncrms; icrm++) {
    parallel_for(
        Bounds<2>(nzm, ncrms), YAKL_LAMBDA(int k, int icrm) { flux(ind_flux, k, icrm) = 0.0; });
    ...
    parallel_for(
        Bounds<4>(nzm, ny, nx, ncrms), YAKL_LAMBDA(int k, int j, int i, int icrm) {
          if (k <= nzm - 2) {
            ...
            flx_z(nzm - 1, j + offy_flx, i + offx_flx, icrm) =
                fluxt(ind_fluxt, j, i, icrm) * rdz * tmp * rhow(nz - 1, icrm);
