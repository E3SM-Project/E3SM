...
void accelerate_crm(int nstep, int nstop, bool &ceaseflag) {
  ...
  parallel_for(
      Bounds<4>(nzm, ny, nx, ncrms), YAKL_LAMBDA(int k, int j, int i, int icrm) {
        ...
        yakl::atomicAdd(qtbaccel(k, icrm),
                        (qcl(k, j, i, icrm) + qci(k, j, i, icrm) + qv(k, j, i, icrm)) * crm_accel_coef);
        if (crm_accel_uv) {
  ...
  if (ceaseflag) { // special case for dT/dt too large
    ...
    std::cout << "crm: nstop increased from " << nstop << " to "
              << round(nstop + (nstop - nstep + 1) * crm_accel_factor);
  ...
  parallel_for(
      Bounds<4>(nzm, ny, nx, ncrms), YAKL_LAMBDA(int k, int j, int i, int icrm) {
        ...
        t(k, j + offy_s, i + offx_s, icrm) =
            max(tmin, t(k, j + offy_s, i + offx_s, icrm) + crm_accel_factor * ttend_acc(k, icrm));
        if (crm_accel_uv) {
          u(k, j + offy_u, i + offx_u, icrm) =
              u(k, j + offy_u, i + offx_u, icrm) + crm_accel_factor * utend_acc(k, icrm);
          v(k, j + offy_v, i + offx_v, icrm) =
              v(k, j + offy_v, i + offx_v, icrm) + crm_accel_factor * vtend_acc(k, icrm);
        }
  ...
  parallel_for(
      Bounds<4>(nzm, ny, nx, ncrms), YAKL_LAMBDA(int k, int j, int i, int icrm) {
        ...
        if (qpoz(k, icrm) + qneg(k, icrm) <= 0.0) {
          ...
          micro_field(idx_qt, k, j + offy_s, i + offx_s, icrm) =
              max(0.0, micro_field(idx_qt, k, j + offy_s, i + offx_s, icrm) * factor);
          ...
          if (micro_field(idx_qt, k, j + offy_s, i + offx_s, icrm) < 0.0) {
            ...
            real qt_res =
                micro_field(idx_qt, k, j + offy_s, i + offx_s, icrm) - qcl(k, j, i, icrm) - qci(k, j, i, icrm);
