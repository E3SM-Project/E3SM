...
void diffuse_mom2D(real5d &tk) {
  ...
  if (!docolumn) {
    ...
    parallel_for(
        Bounds<3>(nzm, nx + 1, ncrms), YAKL_LAMBDA(int k, int i, int icrm) {
          int kc = k + 1;
    ...
    //    for (int icrm=0; icrm<ncrms; icrm++) {
    parallel_for(
        Bounds<3>(nzm, nx, ncrms), YAKL_LAMBDA(int k, int i, int icrm) {
          int kc = k + 1;
          int ib = i - 1;
          ...
          dwdt(na - 1, kc, j, i, icrm) =
              dwdt(na - 1, kc, j, i, icrm) - (fw(k, j, i + 1, icrm) - fw(k, j, ib + 1, icrm));
        });
  }
  ...
  //    for (int icrm=0; icrm<ncrms; icrm++) {
  parallel_for(
      Bounds<3>(nzm - 1, nx, ncrms), YAKL_LAMBDA(int k, int i, int icrm) {
        int kc = k + 1;
  ...
  //       for (int icrm=0; icrm<ncrms; icrm++) {
  parallel_for(
      Bounds<2>(nx, ncrms), YAKL_LAMBDA(int i, int icrm) {
        real rdz = 1.0 / dz(icrm);
  ...
  parallel_for(
      Bounds<3>(nzm, nx, ncrms), YAKL_LAMBDA(int k, int i, int icrm) {
        ...
        dudt(na - 1, k, j, i, icrm) =
            dudt(na - 1, k, j, i, icrm) - (fu(kc, j, i + 1, icrm) - fu(k, j, i + 1, icrm)) * rhoi;
        dvdt(na - 1, k, j, i, icrm) =
            dvdt(na - 1, k, j, i, icrm) - (fv(kc, j, i + 1, icrm) - fv(k, j, i + 1, icrm)) * rhoi;
      });
  ...
  parallel_for(
      Bounds<3>(nzm - 1, nx, ncrms), YAKL_LAMBDA(int k, int i, int icrm) {
        ...
        dwdt(na - 1, k + 1, j, i, icrm) =
            dwdt(na - 1, k + 1, j, i, icrm) - (fw(k + 2, j, i + 1, icrm) - fw(k + 1, j, i + 1, icrm)) * rhoi;
      });
}
