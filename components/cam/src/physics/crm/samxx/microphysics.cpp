...
void precip_fall(int hydro_type, real4d &omega) {
  ...
  parallel_for(
      Bounds<4>(nzm, ny, nx, ncrms), YAKL_LAMBDA(int k, int j, int i, int icrm) {
        ...
        real tmp;
        tmp = term_vel_qp(icrm, i, j, k, micro_field(1, k, j + offy_s, i + offx_s, icrm), vrain, vsnow, vgrau, crain,
                          csnow, cgrau, rho(k, icrm), tabs(k, j, i, icrm), a_pr, a_gr);
  ...
  for (int iprec = 1; iprec <= nprec; iprec++) {
    ...
    parallel_for(
        Bounds<4>(nzm, ny, nx, ncrms), YAKL_LAMBDA(int k, int j, int i, int icrm) {
          tmp_qp(k, j, i, icrm) =
              micro_field(1, k, j + offy_s, i + offx_s, icrm); // Temporary array for qp in this column
        });
    ...
    parallel_for(
        Bounds<4>(nzm, ny, nx, ncrms), YAKL_LAMBDA(int k, int j, int i, int icrm) {
          int kc = k + 1;
          tmp_qp(k, j, i, icrm) = tmp_qp(k, j, i, icrm) -
                                  (fz(kc, j, i, icrm) - fz(k, j, i, icrm)) * irhoadz(k, icrm); // Update temporary qp
        });
    ...
    parallel_for(
        Bounds<4>(nzm, ny, nx, ncrms), YAKL_LAMBDA(int k, int j, int i, int icrm) {
          ...
          www(k, j, i, icrm) = 0.5 * (1.0 + wp(k, j, i, icrm) * irhoadz(k, icrm)) *
                               (tmp_qp(kb, j, i, icrm) * wp(kb, j, i, icrm) -
    ...
    if (nonos) {
      ...
      parallel_for(
          Bounds<4>(nzm, ny, nx, ncrms), YAKL_LAMBDA(int k, int j, int i, int icrm) {
            ...
            int kb = max(0, k - 1);
            mx(k, j, i, icrm) =
                max(tmp_qp(kb, j, i, icrm), max(tmp_qp(kc, j, i, icrm), max(tmp_qp(k, j, i, icrm), mx(k, j, i, icrm))));
            mn(k, j, i, icrm) =
                min(tmp_qp(kb, j, i, icrm), min(tmp_qp(kc, j, i, icrm), min(tmp_qp(k, j, i, icrm), mn(k, j, i, icrm))));
            kc = min(nzm - 1, k + 1);
            mx(k, j, i, icrm) = rho(k, icrm) * adz(k, icrm) * (mx(k, j, i, icrm) - tmp_qp(k, j, i, icrm)) /
                                (pn(www(kc, j, i, icrm)) + pp(www(k, j, i, icrm)) + eps);
            mn(k, j, i, icrm) = rho(k, icrm) * adz(k, icrm) * (tmp_qp(k, j, i, icrm) - mn(k, j, i, icrm)) /
                                (pp(www(kc, j, i, icrm)) + pn(www(k, j, i, icrm)) + eps);
          });
      ...
      parallel_for(
          Bounds<4>(nzm, ny, nx, ncrms), YAKL_LAMBDA(int k, int j, int i, int icrm) {
            ...
            fz(k, j, i, icrm) =
                fz(k, j, i, icrm) + pp(www(k, j, i, icrm)) * min(1.0, min(mx(k, j, i, icrm), mn(kb, j, i, icrm))) -
    ...
    parallel_for(
        Bounds<4>(nzm, ny, nx, ncrms), YAKL_LAMBDA(int k, int j, int i, int icrm) {
          ...
          real flagstat = 1.0;
          micro_field(1, k, j + offy_s, i + offx_s, icrm) = micro_field(1, k, j + offy_s, i + offx_s, icrm) -
                                                            (fz(kc, j, i, icrm) - fz(k, j, i, icrm)) * irhoadz(k, icrm);
          real tmp = -(fz(kc, j, i, icrm) - fz(k, j, i, icrm)) * irhoadz(k, icrm) * flagstat; // For qp budget
          yakl::atomicAdd(qpfall(k, icrm), tmp);
          real lat_heat =
              -(lfac(kc, j, i, icrm) * fz(kc, j, i, icrm) - lfac(k, j, i, icrm) * fz(k, j, i, icrm)) * irhoadz(k, icrm);
    ...
    if (iprec < nprec) {
      ...
      //       for (int icrm=0; icrm<ncrms; icrm++) {
      parallel_for(
          Bounds<4>(ny, nx, nzm, ncrms), YAKL_LAMBDA(int k, int j, int i, int icrm) {
            real tmp = term_vel_qp(icrm, i, j, k, micro_field(1, k, j + offy_s, i + offx_s, icrm), vrain, vsnow, vgrau,
                                   crain, csnow, cgrau, rho(k, icrm), tabs(k, j, i, icrm), a_pr, a_gr);
            wp(k, j, i, icrm) = rhofac(k, icrm) * tmp;
...
void micro_init() {
  ...
  if (nrestart == 0) {
    ...
    //       for (int icrm=0; icrm<ncrms; icrm++) {
    parallel_for(
        Bounds<4>(nmicro_fields, ny, nx, ncrms), YAKL_LAMBDA(int l, int j, int i, int icrm) {
  ...
  //    for (int icrm=0; icrm<ncrms; icrm++) {
  parallel_for(
      Bounds<3>(nmicro_fields, nz, ncrms), YAKL_LAMBDA(int l, int k, int icrm) {
        mkwle(l, k, icrm) = 0.0;
        mkwsb(l, k, icrm) = 0.0;
        mkadv(l, k, icrm) = 0.0;
        mkdiff(l, k, icrm) = 0.0;
  ...
  //       for (int icrm=0; icrm<ncrms; icrm++) {
  parallel_for(
      Bounds<2>(nz, ncrms), YAKL_LAMBDA(int k, int icrm) {
        qpsrc(k, icrm) = 0.0;
        qpevp(k, icrm) = 0.0;
      });
}
