...
void advect2_mom_z() {
  ...
  if (RUN3D) {
    ...
    parallel_for(
        Bounds<4>(nzm - 1, ny, nx, ncrms), YAKL_LAMBDA(int k, int j, int i, int icrm) {
          ...
          fuz(k + 1, j, i, icrm) =
              rhoi * (w(k + 1, j + offy_w, i + offx_w, icrm) + w(k + 1, j + offy_w, i - 1 + offx_w, icrm)) *
              (u(k + 1, j + offy_u, i + offx_u, icrm) + u(kb + 1, j + offy_u, i + offx_u, icrm));
          fvz(k + 1, j, i, icrm) =
              rhoi * (w(k + 1, j + offy_w, i + offx_w, icrm) + w(k + 1, j - 1 + offy_w, i + offx_w, icrm)) *
    ...
    parallel_for(
        Bounds<4>(nzm - 1, ny, nx, ncrms), YAKL_LAMBDA(int k, int j, int i, int icrm) {
          ...
          fuz(k + 1, j, i, icrm) =
              www * (u(k + 1, j + offy_u, i + offx_u, icrm) + u(kb + 1, j + offy_u, i + offx_u, icrm));
          fvz(k + 1, j, i, icrm) =
              www * (v(k + 1, j + offy_v, i + offx_v, icrm) + v(kb + 1, j + offy_v, i + offx_v, icrm));
          yakl::atomicAdd(uwle(k + 1, icrm), fuz(k + 1, j, i, icrm));
          yakl::atomicAdd(vwle(k + 1, icrm), fvz(k + 1, j, i, icrm));
        });
  }
  ...
  parallel_for(
      Bounds<4>(nzm, ny, nx, ncrms), YAKL_LAMBDA(int k, int j, int i, int icrm) {
        ...
        fwz(k, j, i, icrm) = dz25 *
                             (w(kc, j + offy_w, i + offx_w, icrm) * rhow(kc, icrm) +
                              w(k, j + offy_w, i + offx_w, icrm) * rhow(k, icrm)) *
  ...
  parallel_for(
      Bounds<4>(nzm - 1, ny, nx, ncrms), YAKL_LAMBDA(int k, int j, int i, int icrm) {
        ...
        dwdt(na - 1, k + 1, j, i, icrm) =
            dwdt(na - 1, k + 1, j, i, icrm) - (fwz(k + 1, j, i, icrm) - fwz(kb + 1, j, i, icrm)) * rhoi;
      });
}
