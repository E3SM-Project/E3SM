...
void advect_scalar(real4d &f, real2d &fadv, real2d &flux) {
  ...
  if (docolumn) {
    ...
    parallel_for(
        Bounds<2>(nz, ncrms), YAKL_LAMBDA(int k, int icrm) { flux(k, icrm) = 0.0; });
    ...
    parallel_for(
        ...
        YAKL_LAMBDA(int k, int j, int i, int icrm) { f0(k, j, i, icrm) = f(k, j, i, icrm); });
    ...
    //  for (int icrm=0; icrm<ncrms; icrm++) {
    parallel_for(
        Bounds<2>(nzm, ncrms), YAKL_LAMBDA(int k, int icrm) { fadv(k, icrm) = 0.0; });
...
void advect_scalar(real5d &f, int ind_f, real2d &fadv, real2d &flux) {
  ...
  if (docolumn) {
    ...
    parallel_for(
        Bounds<4>(nzm, dimy_s, dimx_s, ncrms),
        YAKL_LAMBDA(int k, int j, int i, int icrm) { f0(k, j, i, icrm) = f(ind_f, k, j, i, icrm); });
...
void advect_scalar(real5d &f, int ind_f, real3d &fadv, int ind_fadv, real3d &flux, int ind_flux) {
  ...
  if (docolumn) {
    parallel_for(
        Bounds<2>(nz, ncrms), YAKL_LAMBDA(int k, int icrm) { flux(ind_flux, k, icrm) = 0.0; });
    ...
    //  for (int icrm=0; icrm<ncrms; icrm++) {
    parallel_for(
        Bounds<2>(nzm, ncrms), YAKL_LAMBDA(int k, int icrm) { fadv(ind_fadv, k, icrm) = 0.0; });
