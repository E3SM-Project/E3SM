...
void diffuse_scalar(real5d &tkh, int ind_tkh, real4d &f, real3d &fluxb, real3d &fluxt, real2d &fdiff, real2d &flux) {
  ...
  parallel_for(
      ...
      YAKL_LAMBDA(int k, int j, int i, int icrm) { df(k, j, i, icrm) = f(k, j, i, icrm); });
  ...
  //  for (int icrm=0; icrm<ncrms; icrm++) {
  parallel_for(
      Bounds<2>(nzm, ncrms), YAKL_LAMBDA(int k, int icrm) { fdiff(k, icrm) = 0.0; });
...
void diffuse_scalar(real5d &tkh, int ind_tkh, real5d &f, int ind_f, real3d &fluxb, real3d &fluxt, real2d &fdiff,
                    real2d &flux) {
  auto &ncrms = ::ncrms;
  real4d df("df", nzm, dimy_s, dimx_s, ncrms);
  ...
  parallel_for(
      Bounds<4>(nzm, dimy_s, dimx_s, ncrms),
      YAKL_LAMBDA(int k, int j, int i, int icrm) { df(k, j, i, icrm) = f(ind_f, k, j, i, icrm); });
...
void diffuse_scalar(real5d &tkh, int ind_tkh, real5d &f, int ind_f, real4d &fluxb, int ind_fluxb, real4d &fluxt,
                    int ind_fluxt, real3d &fdiff, int ind_fdiff, real3d &flux, int ind_flux) {
  auto &ncrms = ::ncrms;
  real4d df("df", nzm, dimy_s, dimx_s, ncrms);
  ...
  //  for (int icrm=0; icrm<ncrms; icrm++) {
  parallel_for(
      Bounds<2>(nzm, ncrms), YAKL_LAMBDA(int k, int icrm) { fdiff(ind_fdiff, k, icrm) = 0.0; });
