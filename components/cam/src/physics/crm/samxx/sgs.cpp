...
void kurant_sgs(real &cfl) {
  ...
  parallel_for(
      Bounds<2>(nzm, ncrms), YAKL_LAMBDA(int k, int icrm) { tkhmax(k, icrm) = 0.; });
...
void sgs_proc() {
  ...
  //       for (int icrm=0; icrm<ncrms; icrm++) {
  parallel_for(
      Bounds<4>(nzm, dimy_s, dimx_s, ncrms),
      YAKL_LAMBDA(int k, int j, int i, int icrm) { tke2(k, j, i, icrm) = sgs_field(0, k, j, i, icrm); });
  // for (int k=0; k<nzm; k++) {
  //   for (int j=0; j<dimy_d; j++) {
  //     for (int i=0; i<dimx_d; i++) {
  //       for (int icrm=0; icrm<ncrms; icrm++) {
  parallel_for(
      Bounds<4>(nzm, dimy_d, dimx_d, ncrms),
      YAKL_LAMBDA(int k, int j, int i, int icrm) { tk2(k, j, i, icrm) = sgs_field_diag(0, k, j, i, icrm); });
}

void sgs_mom() { diffuse_mom(); }
...
void sgs_init() {
  ...
  if (nrestart == 0) {
    ...
    //        for (int icrm=0; icrm<ncrms; icrm++) {
    parallel_for(
        Bounds<5>(nsgs_fields, nzm, dimy_s, dimx_s, ncrms),
        YAKL_LAMBDA(int l, int k, int j, int i, int icrm) { sgs_field(l, k, j, i, icrm) = 0.0; });
    ...
    //        for (int icrm=0; icrm<ncrms; icrm++) {
    parallel_for(
        Bounds<5>(nsgs_fields_diag, nzm, dimy_d, dimx_d, ncrms),
        YAKL_LAMBDA(int l, int k, int j, int i, int icrm) { sgs_field_diag(l, k, j, i, icrm) = 0.0; });
  }
