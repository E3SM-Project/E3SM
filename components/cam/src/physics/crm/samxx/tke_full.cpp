...
void tke_full(real5d &tke, int ind_tke, real5d &tk, int ind_tk, real5d &tkh, int ind_tkh) {
  ...
  parallel_for(
      Bounds<4>(nzm - 1, ny, nx, ncrms), YAKL_LAMBDA(int k, int j, int i, int icrm) {
        int kb, kc;
        real betdz, tabs_interface, qtot_interface, qp_interface, bbb, buoy_sgs, qctot, omn, qsat_interface, qsat_check,
            lstarn, dqsat, qsatt, qsatw, qsati, dtqsatw, dtqsati;
        ...
        tabs_interface = 0.5 * (tabs(kc, j, i, icrm) + fac_cond * qcl(kc, j, i, icrm) + fac_sub * qci(kc, j, i, icrm) +
        ...
        qtot_interface = 0.5 * (qv(kc, j, i, icrm) + qcl(kc, j, i, icrm) + qci(kc, j, i, icrm) + qv(kb, j, i, icrm) +
                                qcl(kb, j, i, icrm) + qci(kb, j, i, icrm));
        ...
        buoy_sgs = betdz * (bbb * (t(kc, j + offy_s, i + offx_s, icrm) - t(kb, j + offy_s, i + offx_s, icrm)) +
                            epsv * tabs_interface *
                                (qv(kc, j, i, icrm) + qcl(kc, j, i, icrm) + qci(kc, j, i, icrm) - qv(kb, j, i, icrm) -
                                 qcl(kb, j, i, icrm) - qci(kb, j, i, icrm)) +
        ...
        a_prod_bu_vert(k + 1, j, i, icrm) =
            -0.5 *
            (tkh(ind_tkh, kc, j + offy_d, i + offx_d, icrm) + tkh(ind_tkh, kb, j + offy_d, i + offx_d, icrm) + 0.002) *
            buoy_sgs;
        ...
        if (qctot > 0.0) {
          ...
          if (qtot_interface > qsat_check) {
            ...
            qp_interface =
                0.5 * (qpl(kc, j, i, icrm) + qpi(kc, j, i, icrm) + qpl(kb, j, i, icrm) + qpi(kb, j, i, icrm));
            ...
            bbb = 1.0 + epsv * qsatt + qsatt - qtot_interface - qp_interface + 1.61 * tabs_interface * dqsat;
            bbb = bbb / (1.0 + lstarn * dqsat);

            buoy_sgs = betdz * (bbb * (t(kc, j + offy_s, i + offx_s, icrm) - t(kb, j + offy_s, i + offx_s, icrm)) +
                                (bbb * lstarn - (1.0 + lstarn * dqsat) * tabs_interface) *
                                    (qv(kc, j, i, icrm) + qcl(kc, j, i, icrm) + qci(kc, j, i, icrm) -
                                     qv(kb, j, i, icrm) - qcl(kb, j, i, icrm) - qci(kb, j, i, icrm)) +
                                (bbb * fac_cond - (1.0 + fac_cond * dqsat) * tabs(k, j, i, icrm)) *
                                    (qpl(kc, j, i, icrm) - qpl(kb, j, i, icrm)) +
                                (bbb * fac_sub - (1.0 + fac_sub * dqsat) * tabs(k, j, i, icrm)) *
                                    (qpi(kc, j, i, icrm) - qpi(kb, j, i, icrm)));

            buoy_sgs_vert(k + 1, j, i, icrm) = buoy_sgs;
            a_prod_bu_vert(k + 1, j, i, icrm) = -0.5 *
                                                (tkh(ind_tkh, kc, j + offy_d, i + offx_d, icrm) +
                                                 tkh(ind_tkh, kb, j + offy_d, i + offx_d, icrm) + 0.002) *
                                                buoy_sgs;
          }
        }
      });
  ...
  //  for (int icrm=0; icrm<ncrms; icrm++) {
  parallel_for(
      Bounds<2>(nzm - 1, ncrms), YAKL_LAMBDA(int k, int icrm) {
  ...
  parallel_for(
      Bounds<4>(nzm - 1, ny, nx, ncrms), YAKL_LAMBDA(int k, int j, int i, int icrm) {
        real grd, Ce1, Ce2, cx, cy, cz, tkmax, smix, ratio, Cee, a_prod_sh, a_prod_bu, a_diss, tmp, buoy_sgs;
        ...
        if (buoy_sgs <= 0.0) {
          ...
          smix = min(grd, max(0.1 * grd,
                              sqrt(0.76 * tk(ind_tk, k, j + offy_d, i + offx_d, icrm) / Ck / sqrt(buoy_sgs + 1.e-10))));
        }
        ratio = smix / grd;
        Cee = Ce1 + Ce2 * ratio;
        if (dosmagor) {
          tk(ind_tk, k, j + offy_d, i + offx_d, icrm) =
              sqrt(Ck * Ck * Ck / Cee * max(0.0, def2(k, j, i, icrm) - Pr * buoy_sgs)) * smix * smix;
          ...
          a_diss = min(tke(ind_tke, k, j + offy_s, i + offx_s, icrm) / (4.0 * dt),
                       Cee / smix * pow(tke(ind_tke, k, j + offy_s, i + offx_s, icrm), 1.5));
          tke(ind_tke, k, j + offy_s, i + offx_s, icrm) =
              max(0.0,
                  tke(ind_tke, k, j + offy_s, i + offx_s, icrm) + dtn * (max(0.0, a_prod_sh + a_prod_bu) - a_diss));
