...
void pre_timeloop() {
  ...
  //       for (int icrm=0; icrm<ncrms; icrm++) {
  parallel_for(
      Bounds<4>(nzm, crm_ny_rad, crm_nx_rad, ncrms), YAKL_LAMBDA(int k, int j, int i, int icrm) {
  ...
  //  for (int icrm=0; icrm<ncrms; icrm++) {
  parallel_for(
      Bounds<2>(ny * YES3D + 1, ncrms), YAKL_LAMBDA(int j, int icrm) { fcory(j, icrm) = fcor(icrm); });
  // for (int j=0; j<ny; j++) {
  //  for (int icrm=0; icrm<ncrms; icrm++) {
  parallel_for(
      Bounds<2>(ny, ncrms), YAKL_LAMBDA(int j, int icrm) { fcorzy(j, icrm) = fcorz(icrm); });
  ...
  // for (int icrm=0; icrm<ncrms; icrm++) {
  parallel_for(
      ncrms, YAKL_LAMBDA(int icrm) { dz(icrm) = 0.5 * (z(0, icrm) + z(1, icrm)); });
  ...
  // for (int icrm=0; icrm<ncrms; icrm++) {
  parallel_for(
      ncrms, YAKL_LAMBDA(int icrm) { adzw(nz - 1, icrm) = adzw(nzm - 1, icrm); });
  ...
  parallel_for(
      Bounds<2>(nzm - 1, ncrms), YAKL_LAMBDA(int k, int icrm) {
        int kInd = k + 1;
        rhow(kInd, icrm) = (crm_input_pmid(plev - (k + 1), icrm) - crm_input_pmid(plev - (k + 1) - 1, icrm)) / ggr /
                           (adzw(kInd, icrm) * dz(icrm));
      });
  ...
  parallel_for(
      Bounds<4>(nzm, ny, nx, ncrms), YAKL_LAMBDA(int k, int j, int i, int icrm) {
        if (u(0, offy_u, offx_u, 0) == u(0, offy_u, 1 + offx_u, 0) &&
            u(1, offy_u, 2 + offx_u, 0) == u(1, offy_u, 3 + offx_u, 0)) {
  ...
  parallel_for(
      Bounds<4>(nzm, ny, nx, ncrms), YAKL_LAMBDA(int k, int j, int i, int icrm) {
        t(k, j + offy_s, i + offx_s, icrm) = tabs(k, j, i, icrm) + gamaz(k, icrm) - fac_cond * qcl(k, j, i, icrm) -
                                             fac_sub * qci(k, j, i, icrm) - fac_cond * qpl(k, j, i, icrm) -
                                             fac_sub * qpi(k, j, i, icrm);
  ...
  parallel_for(
      Bounds<2>(nzm, ncrms), YAKL_LAMBDA(int k, int icrm) {
        ...
        ttend(k, icrm) =
            (crm_input_tl(l, icrm) + gamaz(k, icrm) - fac_cond * (crm_input_qccl(l, icrm) + crm_input_qiil(l, icrm)) -
             fac_fus * crm_input_qiil(l, icrm) - t00(k, icrm)) *
            idt_gl;
        qtend(k, icrm) =
            (crm_input_ql(l, icrm) + crm_input_qccl(l, icrm) + crm_input_qiil(l, icrm) - q0(k, icrm)) * idt_gl;
        ...
        tg0(k, icrm) = crm_input_tl(l, icrm) + gamaz(k, icrm) - fac_cond * crm_input_qccl(l, icrm) -
                       fac_sub * crm_input_qiil(l, icrm);
  ...
  //---------------------------------------------------
  parallel_for(
      Bounds<2>(plev + 1, ncrms), YAKL_LAMBDA(int k, int icrm) {
        if (k < plev) {
