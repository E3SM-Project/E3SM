...
void setperturb() {
  ...
  auto gcolp = ::gcolp;
  parallel_for(
      Bounds<4>(perturb_num_layers, ny, nx, ncrms), YAKL_LAMBDA(int k, int j, int i, int icrm) {
        ...
        yakl::Random rand((size_t)(gcolp(icrm) * perturb_num_layers * ny * nx * ncrms + k * ny * nx * ncrms +
                                   j * nx * ncrms + i * ncrms + icrm));
        real rand_perturb = rand.genFP<real>(-1., 1.);
        // apply perturbation
        t(k, j + offy_s, i + offx_s, icrm) =
            t(k, j + offy_s, i + offx_s, icrm) + rand_perturb * perturb_t_magnitude * perturb_k_scaling;
