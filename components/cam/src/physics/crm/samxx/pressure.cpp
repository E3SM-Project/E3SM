...
void pressure() {
  ...
  parallel_for(
      Bounds<4>(nzslab, ny, nx, ncrms),
      YAKL_LAMBDA(int k, int j, int i, int icrm) { f(k, j, i, icrm) = p(k, j + offy_p, i + offx_p, icrm); });
  ...
  //      for (int icrm=0; icrm<ncrms; icrm++) {
  parallel_for(
      Bounds<3>(nzslab, ny, ncrms), YAKL_LAMBDA(int k, int j, int icrm) {
        real ftmp[nx + 2];
        real tmp[nx];
  ...
  if (RUN3D) {
    ...
    //      for (int icrm=0; icrm<ncrms; icrm++) {
    parallel_for(
        Bounds<3>(nzslab, nx + 1, ncrms), YAKL_LAMBDA(int k, int i, int icrm) {
          real ftmp[ny + 2];
          real tmp[ny];
  ...
  //      for(int icrm=0; icrm<ncrms; icrm++) {
  parallel_for(
      Bounds<4>(nzslab, nypp, nx + 1, ncrms),
      YAKL_LAMBDA(int k, int j, int i, int icrm) { ff(k, j, i, icrm) = f(k, j, i, icrm); });
  ...
  //     for (int i=0; i<nx+1; i++) {
  parallel_for(
      Bounds<2>(nypp, nx + 1), YAKL_LAMBDA(int j, int i) {
        int jt = 0;
        int it = 0;
  ...
  //    for (int icrm=0; icrm<ncrms; icrm++) {
  parallel_for(
      Bounds<3>(nypp, nx + 1, ncrms), YAKL_LAMBDA(int j, int i, int icrm) {
        ...
        ff(nzm - 1, j, i, icrm) =
            (ff(nzm - 1, j, i, icrm) - a(nzm - 1, icrm) * beta(nzm - 2)) /
  ...
  parallel_for(
      Bounds<4>(nzslab, nypp, nx + 1, ncrms),
      YAKL_LAMBDA(int k, int j, int i, int icrm) { f(k, j, i, icrm) = ff(k, j, i, icrm); });
  ...
  parallel_for(
      Bounds<4>(nzslab, dimy_p, nx + 1, ncrms), YAKL_LAMBDA(int k, int j, int i, int icrm) {
        int jj, ii;
