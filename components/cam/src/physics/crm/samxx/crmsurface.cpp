...
void crmsurface(real1d &bflx) {
  ...
  parallel_for(
      Bounds<3>(ny, nx, ncrms), YAKL_LAMBDA(int j, int i, int icrm) {
        ...
        fluxbu(j, i, icrm) =
            -(0.5 * (u(0, j + offy_u, i + 1 + offx_u, icrm) + u(0, j + offy_u, i + offx_u, icrm)) + ug - uhl(icrm)) /
            u_h0 * tau00;

        fluxbv(j, i, icrm) = -(0.5 * (v(0, j + YES3D + offy_v, i + offx_v, icrm) + v(0, j + offy_v, i + offx_v, icrm)) +
                               vg - vhl(icrm)) /
                             u_h0 * tau00;
