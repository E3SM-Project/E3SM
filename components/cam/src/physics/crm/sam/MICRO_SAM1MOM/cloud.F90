...
contains
  ...
  subroutine cloud(ncrms,q,qp,qn)
    ...
    integer niter, iloop, loop_size

    loop_size = nx*ny*nzm*ncrms
    an = 1./(tbgmax-tbgmin)
    bn = tbgmin * an
    ap = 1./(tprmax-tprmin)
    bp = tprmin * ap
    ...
    !$omp target teams distribute parallel do private(icrm,i,j,k,qsatt,dqsat,dtabs,tabs1,fff,dfff) 
#endif
    do iloop = 0, loop_size-1
      icrm = mod(iloop,ncrms)+1
      i = mod(int(iloop/ncrms),nx)+1
      j = mod(int(int(iloop/ncrms)/nx),ny)+1
      k = mod(int(int(int(iloop/ncrms)/nx)/ny),nzm)+1
      ...
      if(q(icrm,i,j,k).gt.qsatt) then
        niter=0
        dtabs = 100.
        do while(niter.lt.10)  !abs(dtabs).gt.0.01.and.niter.lt.10)
          if(tabs1.ge.tbgmax) then
            om=1.
            lstarn=fac_cond
            dlstarn=0.
