...
!+++ AeroCOM IND3 output
   ...
   if (prog_modal_aero) then
      ! aerosol tendencies
      call physics_ptend_init(ptend, state%psetcols, 'crm_ndrop_aero', lq=lq)
   else
      ! no aerosol tendencies
      call physics_ptend_init(ptend, state%psetcols, 'crm_ndrop')
   end if
...
! Gas tendency
   ...
   else if(SPCAM_mmf) then  ! called in the mmf part.
