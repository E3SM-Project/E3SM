...
!-----------------------------------------------------------------------
  ...
  !IMPORTANT: **- add code to apply  this tendency **
  ...
  use shr_kind_mod,     only: r8 => shr_kind_r8, cx =>SHR_KIND_CX, cs =>SHR_KIND_CS, cxx =>SHR_KIND_CXX
  ...
  use pio,              only: file_desc_t, pio_nowrite, pio_closefile, pio_inq_dimid, pio_bcast_error, &
       pio_seterrorhandling, pio_noerr, pio_inquire_dimension, pio_get_att, pio_inq_varid
  ...
  type :: forc_air_native_grid
     ...
     !Number of levels in the forcing file
     integer                               :: lev_frc
  ...
  character(len=11)    :: aero_names(N_AERO) = (/'ac_HC      ','ac_NOX     ','ac_PMNV    ',&
                          'ac_PMSO    ','ac_PMFO    ','ac_FUELBURN','CO2_flux  ','ac_H2O     ',&
                          'ac_SOX     ','ac_CO      '/)
  ...
  !Logical to control whether to horizontal grid is on native grid or not
  logical :: horz_native(N_AERO) = (/.false., .false., .false., .false., .false., &
...
!-------------------------------------------------------------------
    ...
    species_loop : do m = 1,aircraft_cnt
       ...
       if( horz_native(index_map(m))) then
          ...
          call native_grid_frc_air(m)%time_coord%initialize(native_grid_frc_air(m)%input_file)

          !-----------------------------------------------------------------------
          !       Open file
          !-----------------------------------------------------------------------
          call cam_pio_openfile(fh, native_grid_frc_air(m)%input_file, PIO_NOWRITE) 
          ...
          if(pio_inq_dimid(fh, trim(adjustl(dim1name)), dim1id) /= pio_noerr) then
             ...
             !file grid
             call endrun('grid mismatch, failed to find '//dim1name//' dimension in file:'//native_grid_frc_air(m)%input_file//' '&
             ...
             if(pio_inquire_dimension(fh, dim1id, len = dim1len) ==  pio_noerr) then
                if(dim1len /= hdim1_d ) then !compare model grid length with file's
                   write(err_str,*)'Netcdf file grid size(',dim1len,') should be same as model grid size(',&
                        hdim1_d,'), netcdf file is:'//native_grid_frc_air(m)%input_file
                   call endrun(err_str//errmsg(__FILE__,__LINE__))
                endif
             else
                call endrun('failed while inquiring dimensions of file:'//native_grid_frc_air(m)%input_file//'&
                     &'//errmsg(__FILE__,__LINE__))
             endif
          elseif( dycore_is('LR')) then
             if(pio_inq_dimid(fh, trim(adjustl(dim2name)), dim2id)) then !obtain lat dimension of model
                call endrun('failed while inquiring dimension'//trim(adjustl(dim2name))//' from file:'&
                     ' '//native_grid_frc_air(m)%input_file//' '//errmsg(__FILE__,__LINE__))
             endif
             if(pio_inquire_dimension(fh, dim1id, len = dim1len) ==  pio_noerr .and. &
                ...
                if(dim1len /= hdim1_d .or. dim2len /= hdim2_d)then
                   write(err_str,*)'Netcdf file grid size(',dim1len,' x ',dim2len,') should be same as model grid size(',&
                        hdim1_d,' x ',hdim2_d,'), netcdf file is:'//native_grid_frc_air(m)%input_file
                   call endrun(err_str//errmsg(__FILE__,__LINE__))
                endif
             else
                call endrun('failed while inquiring dimensions of file:'//native_grid_frc_air(m)%input_file//'&
          ...
          if( pio_inq_dimid(fh, 'lev', dimlevid) ==  pio_noerr ) then
             if ( pio_inquire_dimension(fh, dimlevid, len =  native_grid_frc_air(m)%lev_frc) /=  pio_noerr ) then
                write(err_str,*)'failed to obtain value of "lev" dimension from file:',native_grid_frc_air(m)%input_file,&
                     errmsg(__FILE__, __LINE__)
                call endrun(err_str)
             endif
          else
             write(err_str,*)'Dimension "lev" is not found in:',native_grid_frc_air(m)%input_file,errmsg(__FILE__, __LINE__)
             call endrun(err_str)
          endif
          ...
          if(pio_inq_varid( fh, spc_name, var_id ) == pio_noerr ) then
             if(pio_get_att( fh, var_id, 'units', native_grid_frc_air(m)%units) .ne. pio_noerr ) then
                write(err_str,*)'failed to obtain units of variable ',trim(spc_name),' in &
                     &file:',native_grid_frc_air(m)%input_file,errmsg(__FILE__, __LINE__)
                call endrun(err_str)
             endif
          else
             write(err_str,*)'variable ',trim(spc_name),' not found in:',native_grid_frc_air(m)%input_file, &
                  errmsg(__FILE__, __LINE__)
             call endrun(err_str)
          endif
    ...
    do m=1,aircraft_cnt
       ...
       if(horz_native(index_map(m))) then
          if (associated(native_grid_frc_air(m)%native_grid_flds_tslices)) &
             number_flds = 1
          !read the forcing file once
          call advance_native_grid_data(state, native_grid_frc_air(m)%pbuf_ndx, m, pbuf2d)
  ...
  subroutine aircraft_emit_adv( state, pbuf2d)
    ...
    do m = 1, aircraft_cnt
       ...
       if(horz_native(index_map(m))) then
          ...
          !field in pbuf
          call advance_native_grid_data( state, pbuf_ndx, m, pbuf2d)
       else          
  ...
  subroutine advance_native_grid_data( state, pbuf_ndx, m, pbuf2d)
    !-------------------------------------------------------------------
    !    This subroutine reads the data from the native grid, 
    !    interpolates in time and vertically. Finally the interpolated
    !    data is stored  in pbuf
    !-------------------------------------------------------------------
    ...
    type(physics_state), intent(in)    :: state(begchunk:endchunk)
    type(physics_buffer_desc), pointer :: pbuf2d(:,:)    
    integer, intent(in)                :: pbuf_ndx, m

    !local vars
    type(physics_buffer_desc), pointer :: pbuf_chnk(:)
    type(file_desc_t) :: fh

    integer  :: ind,c,ncol,i,caseid,n
    logical  :: read_data
    ...
    indx2_pre_adv = native_grid_frc_air(m)%time_coord%indxs(2)
    
    !compute weights for time interpolation (time_coord%wghts) by advancing in time
    call native_grid_frc_air(m)%time_coord%advance()
    ...
    if ( read_data ) then
       ...
       ! read time-level 1
       if (native_grid_frc_air(m)%initialized .and. native_grid_frc_air(m)%time_coord%indxs(1) == indx2_pre_adv) then
          ! skip the read if the needed vals for time level 1 are present in time-level 2
          native_grid_frc_air(m)%native_grid_flds_tslices(:,:,:,1) = native_grid_frc_air(m)%native_grid_flds_tslices(:,:,:,2)
       else
          !NOTE: infld call doesn't do any interpolation in space, it just reads in the data
          call infld(trim(spc_name_list(m)), fh, dim1name, dim2name, 'lev',&
               1, pcols, 1, native_grid_frc_air(m)%lev_frc, begchunk, endchunk, &
               native_grid_frc_air(m)%native_grid_flds_tslices(:,:,:,1), found, &
               gridname='physgrid', timelevel=native_grid_frc_air(m)%time_coord%indxs(1))
          if (.not. found) then
             call endrun(trim(spc_name_list(m)) // ' not found'//errmsg(__FILE__,__LINE__))
          endif
       endif
       ...
       call infld(trim(spc_name_list(m)), fh, dim1name, dim2name, 'lev',&
            1, pcols, 1, native_grid_frc_air(m)%lev_frc, begchunk, endchunk, &
            native_grid_frc_air(m)%native_grid_flds_tslices(:,:,:,2), found, &
            gridname='physgrid', timelevel=native_grid_frc_air(m)%time_coord%indxs(2))
    ...
    if (native_grid_frc_air(m)%time_coord%wghts(2) == 0.0_r8) then
       native_grid_frc_air(m)%native_grid_flds(:,:,:) = native_grid_frc_air(m)%native_grid_flds_tslices(:,:,:,1)
    else
       native_grid_frc_air(m)%native_grid_flds(:,:,:) = native_grid_frc_air(m)%native_grid_flds_tslices(:,:,:,1) + &
            native_grid_frc_air(m)%time_coord%wghts(2) * (native_grid_frc_air(m)%native_grid_flds_tslices(:,:,:,2) - &
            native_grid_frc_air(m)%native_grid_flds_tslices(:,:,:,1))
    endif
    ...
    !add field to pbuf
    !$OMP PARALLEL DO PRIVATE (C, NCOL, tmpptr_native_grid, pbuf_chnk, vrt_interp_field)
    do c = begchunk,endchunk
       ncol = state(c)%ncol
       pbuf_chnk => pbuf_get_chunk(pbuf2d, c)
       call pbuf_get_field(pbuf_chnk, pbuf_ndx , tmpptr_native_grid )
       tmpptr_native_grid(:ncol,:) = vrt_interp_field(:ncol,:,c)!variable afer vertical interolation
    enddo
