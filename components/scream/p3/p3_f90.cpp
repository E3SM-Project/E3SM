...
void p3_main_c(Real *qc, Real *nc, Real *qr, Real *nr, Real *th_old, Real *th,
               ...
               Real *diag_effc, Real *diag_effi, Real *diag_vmi, Real *diag_di,
               Real *diag_rhoi, Int n_diag_2d, Real *diag_2d, Int n_diag_3d,
               Real *diag_3d, bool log_predictNc, bool typeDiags_ON,
               const char **model, Real *prt_drzl, Real *prt_rain,
               Real *prt_crys, Real *prt_snow, Real *prt_grpl, Real *prt_pell,
               Real *prt_hail, Real *prt_sndp);
}

namespace scream {
namespace p3 {

FortranData::FortranData(Int ncol_, Int nlev_) : ncol(ncol_), nlev(nlev_) {
...
const FortranDataIterator::RawArray &FortranDataIterator::getfield(
    Int i) const {
...
Int check_against_python(const FortranData &d) {
  ...
  if(util::is_single_precision<Real>::value) {
    const double tol = 0;
    if(util::reldif<double>(d.birim(0, d.nlev - 1, 0), 7.237245824853744e-08) >
       tol)
      ++nerr;
    if(util::reldif<double>(d.qirim(0, d.nlev - 1, 0), 9.047746971191373e-06) >
       tol)
      ++nerr;
