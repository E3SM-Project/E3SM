...
static Int compare(const std::string &label, const Scalar *a, const Scalar *b,
                   const Int &n, const Real &tol) {
  Int nerr = 0;
  Real den = 0;
  for(Int i = 0; i < n; ++i) den = std::max(den, std::abs(a[i]));
  Real worst = 0;
  for(Int i = 0; i < n; ++i) {
...
Int compare(const std::string &label, const double &tol,
  ...
  for(Int i = 0, n = refi.nfield(); i < n; ++i) {
    ...
    nerr += compare(label + std::string(" ") + fr.name, fr.data, fd.data,
                    fr.size, tol);
  }
  return nerr;
}
...
struct Baseline {
  Baseline() { params_.push_back({ic::Factory::mixed, 1800}); }
 ...
 private:
  ...
  static void read(const FILEPtr &fid, const FortranData::Ptr &d) {
    ...
    for(Int i = 0, n = fdi.nfield(); i < n; ++i) {
      ...
      int dim, ds[3];
      util::read(&dim, 1, fid);
      scream_throw_if(dim != f.dim, "For field "
