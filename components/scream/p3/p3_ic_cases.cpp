...
FortranData::Ptr make_mixed() {
  ...
  //                                                      v (in the python)
  for(k = 0; k < 15; ++k)
    d.qitot(0, nk - 20 + k, 0) = 1e-4;  //*(1 - double(k)/14)
  for(k = 0; k < nk; ++k) d.nitot(0, k, 0) = 1e6;
  for(k = 0; k < 15; ++k)
    d.qirim(0, nk - 20 + k, 0) = 1e-4 * (1 - double(k) / 14);
  ...
  // temp.
  static constexpr double Rd = 287,  // gas constant for dry air, J/kg/K
  ...
  for(k = 0; k < nk; ++k) {
    ...
    plo = (k == 0) ? std::max<double>(
                         0, d.pres(0, 0) -
                                0.5 * (d.pres(0, 1) - d.pres(0, 0)) / (1 - 0))
                   : 0.5 * (d.pres(0, k - 1) + d.pres(0, k));
    phi = (k == nk - 1)
              ? d.pres(0, nk - 1) +
                    0.5 * (d.pres(0, nk - 1) - d.pres(0, nk - 2)) / (1 - 0)
