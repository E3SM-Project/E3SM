...
void run(std::mt19937_64& engine)
{
  ...
  auto diag = diag_factory.create("RainWaterPath",comm,params);
  diag->set_grids(gm);
  ...
  // Run tests
  {
    ...
    for (int icol=0;icol<ncols;icol++) {
      ...
      ekat::genRandArray(dview_as_real(qr),              engine, pdf_qr);
    ...
    Kokkos::parallel_for("", policy, KOKKOS_LAMBDA(const MemberType& team) {
      ...
      Kokkos::parallel_reduce(Kokkos::TeamThreadRange(team, num_levs), [&] (const Int& idx, Real& lsum) {
        ...
        lsum += qr_v(icol,jpack)[klev] * pseudo_dens_v(icol,jpack)[klev] / gravit;
      },lwp_v(icol));
      team.team_barrier();
    });
    Kokkos::fence();
