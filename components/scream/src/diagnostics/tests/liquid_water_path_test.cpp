...
void run(std::mt19937_64& engine)
{
  ...
  auto diag = diag_factory.create("LiqWaterPath",comm,params);
  diag->set_grids(gm);
  ...
  // Run tests
  {
    ...
    for (int icol=0;icol<ncols;icol++) {
      ...
      ekat::genRandArray(dview_as_real(qc),              engine, pdf_qc);
    ...
    Kokkos::parallel_for("", policy, KOKKOS_LAMBDA(const MemberType& team) {
      ...
      Kokkos::parallel_reduce(Kokkos::TeamThreadRange(team, num_levs), [&] (const Int& idx, Real& lsum) {
        ...
        lsum += qc_v(icol,jpack)[klev] * pseudo_dens_v(icol,jpack)[klev] / gravit;
      },lwp_v(icol));
      team.team_barrier();
    });
    Kokkos::fence();
