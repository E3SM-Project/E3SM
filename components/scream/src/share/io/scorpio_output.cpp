...
long long res_dep_memory_footprint () const {
  ...
  for (const auto& fn : m_field_names) {
    bool is_diagnostic = (m_diagnostics.find(name) != m_diagnostics.end());
    bool can_alias_field_view =
        m_avg_type==OutputAvgType::Instant && not is_diagnostics &&
        m_field_mgr.at(fn).get_header().get_alloc_properties().get_padding()==0 &&
        m_field_mgr.at(fn).get_header().get_parent().expired();
