...
public:
  ...
  const identifier_type& get_field_id(const std::string name,const std::string grid) const;
...
FieldRepository<RealType>::get_field (const identifier_type& id) const {
  ...
  EKAT_REQUIRE_MSG(map!=m_fields.end(), "Error! Field " + id.name() + " not found.\n");
  auto it = map->second.find(id);
  EKAT_REQUIRE_MSG(it!=map->second.end(), "Error! Field " + id.name() + " not found.\n");
  return it->second;
}
...
//  EKAT_REQUIRE_MSG(f_matches.size()==1, "Error! get_field: " + name + " found " + std::to_string(f_matches.size()) + " matches on grid " + grid + ".\n");
  auto fid = get_field_id(name,grid);
  // Use this field id to grab field itself.
  return get_field(fid);
}

template<typename ScalarType, typename Device>
const typename FieldRepository<ScalarType,Device>::identifier_type&
FieldRepository<ScalarType,Device>::get_field_id (const std::string name, const std::string grid) const {
  EKAT_REQUIRE_MSG(m_repo_state==RepoState::Closed,"Error! You are not allowed to grab fields from the repo until after the registration phase is completed.\n");
  ...
  auto& alias = map->second;
  auto& fid = alias.begin()->first;
  return fid;
}
