...
TEST_CASE("spa_read_data","spa")
{
  ...
  for (int time_index = 0;time_index<max_time; time_index++) {
    ...
    for (size_t dof_i=0;dof_i<dofs_gids_h.size();dof_i++) {
      REQUIRE(ps_h(dof_i) == ps_func(time_index,spa_horiz_interp.ncols_src));
      for (int kk=0;kk<nlevs;kk++) {
        ...
        REQUIRE(ccn3_h(dof_i,kpack)[kidx] == ccn3_func(time_index, kk, spa_horiz_interp.ncols_src));
        for (int n=0;n<nswbands;n++) {
          REQUIRE(aer_g_sw_h(dof_i,n,kpack)[kidx]   == aer_func(time_index,n,kk,spa_horiz_interp.ncols_src,0));
          REQUIRE(aer_ssa_sw_h(dof_i,n,kpack)[kidx] == aer_func(time_index,n,kk,spa_horiz_interp.ncols_src,1));
          REQUIRE(aer_tau_sw_h(dof_i,n,kpack)[kidx] == aer_func(time_index,n,kk,spa_horiz_interp.ncols_src,2));
        }
        for (int n=0;n<nlwbands;n++) {
          REQUIRE(aer_tau_lw_h(dof_i,n,kpack)[kidx] ==  aer_func(time_index,n,kk,spa_horiz_interp.ncols_src,3));
        }
      }
    }
  }
