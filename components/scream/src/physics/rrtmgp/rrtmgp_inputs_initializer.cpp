...
namespace scream {
    ...
    void RRTMGPInputsInitializer::initialize_fields () {
        ...
        // Make sure RRTMGP has been initialized
        EKAT_REQUIRE_MSG(scream::rrtmgp::initialized, "Error! RRTMGP has not been initialized!");
        ...
        // Make sure yakl is initialized
        if (!yakl::isInitialized()) { 
            std::cout << "WARNING: initializing yakl from rrtmgp_inputs_initializer!\n";
            yakl::init();
        } else {
            std::cout << "YAKL already initialized by the time we get to rrtmgp_inputs_initializer\n";
        }
        ...
        auto d_pmid = m_fields.at("pmid").get_reshaped_view<Real**>();
        auto d_pint = m_fields.at("pint").get_reshaped_view<Real**>();
        auto d_tmid = m_fields.at("tmid").get_reshaped_view<Real**>();
        auto d_tint = m_fields.at("tint").get_reshaped_view<Real**>();
        auto d_col_dry = m_fields.at("col_dry").get_reshaped_view<Real**>();
        auto d_gas_vmr = m_fields.at("gas_vmr").get_reshaped_view<Real***>();
        auto d_sfc_alb_dir = m_fields.at("sfc_alb_dir").get_reshaped_view<Real**>();
        auto d_sfc_alb_dif = m_fields.at("sfc_alb_dif").get_reshaped_view<Real**>();
        auto d_mu0 = m_fields.at("mu0").get_reshaped_view<Real*>();
        auto d_lwp = m_fields.at("lwp").get_reshaped_view<Real**>();
        auto d_iwp = m_fields.at("iwp").get_reshaped_view<Real**>();
        auto d_rel = m_fields.at("rel").get_reshaped_view<Real**>();
        auto d_rei = m_fields.at("rei").get_reshaped_view<Real**>();
        ...
        yakl::Array<double,3,memDevice,yakl::styleFortran> gas_vmr_ptr("gas_vmr", d_gas_vmr.data(), ncol, nlay, ngas);
        ...
        gas_concs.concs.deep_copy_to(gas_vmr_ptr);
    }
} // namespace scream
