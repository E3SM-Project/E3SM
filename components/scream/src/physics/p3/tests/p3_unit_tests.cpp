...
struct UnitWrap::UnitTest<D>::TestP3Saturation
{
  ...
  KOKKOS_FUNCTION  static void saturation_tests(const Scalar& temperature, const Scalar& pressure, const Scalar& correct_sat_ice_p,
    ...
    const Scalar sym_tol_p = 10.;
    const Scalar sym_tol_q = 1e-3;
    ...
    printf("===============================\n");
    printf("Checking T = %f",temperature," and Pres = %e\n",pressure);
    printf("===============================\n");

    //PMC note: only checking 1st entry in packs below b/c packs were created
    //by copying a scalar up to pack size.
      
    // Test vapor pressure against approximate symbolic implementation
    // ---------------------------------------------------------
    Scalar esi_exact=polysvp1_exact(temperature,1);
    Scalar esl_exact=polysvp1_exact(temperature,0);

    if ( std::abs(sat_ice_p[0] - esi_exact ) > sym_tol_p ) {
      printf("  Diff btwn analytic esi=%e and computed=%e bigger than sym_tol=%e\n",esi_exact,sat_ice_p[0],sym_tol_p);
      errors++;}
    if ( std::abs(sat_liq_p[0] - esl_exact ) > sym_tol_p ) {
      printf("  Diff btwn analytic esl=%e and computed=%e bigger than sym_tol=%e\n",esl_exact,sat_liq_p[0],sym_tol_p);
      errors++;}
    ...
    if ( std::abs(mix_ice_r[0] - qsi_exact ) > sym_tol_q ) {
      printf("Diff btwn analytic qsi=%e and computed=%e bigger than sym_tol=%e\n",qsi_exact,mix_ice_r[0],sym_tol_q);
      errors++;}
    if ( std::abs(mix_liq_r[0] - qsl_exact ) > sym_tol_q ) {
      printf("Diff btwn analytic qsl=%e and computed=%e bigger than sym_tol=%e\n",qsl_exact,mix_liq_r[0],sym_tol_q);
      errors++;}

    //Test mixing-ratios against Wexler approx: 
    // -------------------
    // Again, compute condition # Cond=x*f'(x)/f(x), with x=temperature, f(x)=mix_X_r[0], and
  ...
  ////////////////////////////////////////PETER HACKED IN STUFF
  static Scalar polysvp1_exact(const Scalar& t_atm, const bool& ice)
  {
    // Compute saturation vapor pressure (es) symbolically.
    // des/dT = L/[T*(1/rho_vap - 1/rho_liq] by Clausius clapeyron 
    // (Wallace + Hobbes eq 2.93). 1/rho_liq<<1/rho_vap =>
    // des/dT \approx L/[T*Rv*T/es] = L*es/(Rv*T**2.)
    // Taking the integral between some T0 and a particular T yields:
    // es(T) \approx es(T0)*exp{-L/Rv*(1/T - 1/T0)}.
    ...
    //Flatau et al 1992 table 4. Would be better to use exact obs
    ...
    Scalar result = es_T0*std::exp( -L/RV*(1/t_atm - 1/tmelt) );

    //printf("T,is_ice,es=%e %d %e\n",t_atm,ice,result);
...
TEST_CASE("p3_get_time_space_phys_variables_test", "[p3_get_time_space_phys_variables_test]"){
