...
struct UnitWrap::UnitTest<D>::TestDsd2 {
  ...
  static void run_cloud_bfb()
  {
    ...
    Kokkos::parallel_for(RangePolicy(0, 1), KOKKOS_LAMBDA(const Int& i) {
      ...
      Spack mu_c, nu, lamc, cdist, cdist1;
    ...
    // Validate results
    for (Int s = 0; s < Spack::n; ++s) {
      REQUIRE(gcdd[s].nc_out == gcdd_device(s).nc_out);
      REQUIRE(gcdd[s].mu_c   == gcdd_device(s).mu_c);
      REQUIRE(gcdd[s].nu     == gcdd_device(s).nu);
      REQUIRE(gcdd[s].lamc   == gcdd_device(s).lamc);
      REQUIRE(gcdd[s].cdist  == gcdd_device(s).cdist);
      REQUIRE(gcdd[s].cdist1 == gcdd_device(s).cdist1);
    }
  }
  ...
  static void run_rain_bfb()
  {
    ...
    Kokkos::parallel_for(RangePolicy(0, 1), KOKKOS_LAMBDA(const Int& i) {
      ...
      Spack mu_r, lamr, cdistr, logn0r;
      Functions::get_rain_dsd2(mu_r_table, gt_small, qr, nr, mu_r, lamr, cdistr, logn0r, rcldm);
    ...
    // Validate results
    for (Int s = 0; s < Spack::n; ++s) {
      REQUIRE(grdd[s].nr_out == grdd_device(s).nr_out);
      REQUIRE(grdd[s].mu_r == grdd_device(s).mu_r);
      REQUIRE(grdd[s].lamr == grdd_device(s).lamr);
      REQUIRE(grdd[s].cdistr == grdd_device(s).cdistr);
      REQUIRE(grdd[s].logn0r == grdd_device(s).logn0r);
    }
  }
