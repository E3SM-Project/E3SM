  ...
  bool& hydrometeorsPresent)
{
  ...
  Kokkos::parallel_for(
    ...
    inv_rho(k) = 1 / rho(k);
    qvs(k)     = physics::qv_sat(t(k), pres(k), false, range_mask);
    qvi(k)     = physics::qv_sat(t(k), pres(k), true,  range_mask);
  ...
  Kokkos::parallel_for(
    ...
    if (not_skip_micro.any()) {
      ...
      ice_cldliq_wet_growth(
        ...
        wetgrowth, qrcol, qccol, qwgrth, nrshdr, qcshd, range_mask, not_skip_micro);
