...
!!!!!!!	
  ...
  recursive function check_grid(x1,x2,km1,km2,ncol) result (out)
    implicit none

    integer, intent(in) :: km1, km2
    integer, intent(in) :: ncol
    real(r8), intent(in) :: x1(ncol,km1), x2(ncol,km2)
    ...
    if (km1 .eq. km2+1) then
       do k2 = 1,km2
          k1 = k2+1
          do i = 1,ncol
             if (x1(i,k1-1) .gt. x2(i,k2) .or. x1(i,k1) .lt. x2(i,k2)) &
                  print *,'check_grid: k1,k2,i,x1(i,k1-1),x2(i,k2)',k1,k2,i,x1(i,k1-1),x2(i,k2)
          end do
       end do
    else
       if (km2 .ne. km1+1) print *,'check_grid: km1,km2',km1,km2
       out = check_grid(x2,x1,km2,km1,ncol)
    end if
    out = 0
