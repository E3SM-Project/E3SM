...
struct UnitWrap::UnitTest<D>::TestSaturation
{
  ...
  KOKKOS_FUNCTION  static void saturation_tests(const Scalar& temperature, const Scalar& pressure,
    ...
    const Spack mix_ice_fr = physics::qv_sat(temps, pres, true, Smask(false), physics::Polysvp1);
    const Spack mix_liq_fr = physics::qv_sat(temps, pres, false,Smask(false), physics::Polysvp1);
    ...
    const Spack mix_ice_mkr  = physics::qv_sat(temps, pres, true,  Smask(false), physics::MurphyKoop);
    const Spack mix_liq_mkr  = physics::qv_sat(temps, pres, false, Smask(false), physics::MurphyKoop);
