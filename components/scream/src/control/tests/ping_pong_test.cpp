...
public:
  ...
  void initialize (const Comm& comm) {
    ...
    m_input_fids.emplace(in_name,layout);
    m_output_fids.emplace(out_name,layout);
  }
...
TEST_CASE("ping-pong", "") {
  ...
  factory.register_product("duMmy",&create_dummy_process<device_type,SCREAM_PACK_SIZE>);
  ...
  std::vector<FieldTag> tags = {FieldTag::Column,FieldTag::Component};
  std::vector<int> dims = {32, 2};
  FieldLayout layout (tags,dims);
  FieldIdentifier final_fid("field_" + std::to_string(atm_comm.size()-1),layout);
