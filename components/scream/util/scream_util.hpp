...
#define scream_kernel_assert(condition)         \
  do {                                          \
    if(!(condition)) Kokkos::abort(#condition); \
  } while(0)
#else
#define scream_assert(condition)
#define scream_kernel_assert(condition)
#endif
#define scream_throw_if(condition, message)                       \
  do {                                                            \
    if(condition) {                                               \
      std::stringstream _ss_;                                     \
...
#define scream_kernel_throw_if(condition, message)                             \
  do {                                                                         \
    if(condition) Kokkos::abort(#condition " led to the exception\n" message); \
  } while(0)
...
struct FILECloser {
  void operator()(FILE *fh) { fclose(fh); }
};
...
template <>
struct is_single_precision<float> {
  enum : bool { value = true };
};
template <>
struct is_single_precision<double> {
