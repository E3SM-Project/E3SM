...
#include "shr_assert.h"
  ...
  private :: downscale_atmo_state_to_topounit_glc      ! Downscale atmosperic state fields from gridcell to topounit based on glc
  private :: downscale_atmo_state_to_topounit_UoA      ! Downscale atmosperic state fields from gridcell to topounit based on UoA
  private :: downscale_longwave_to_topounit        ! Downscale precipitation field from gridcell to topounit
  private :: downscale_precip_to_topounit          ! Downscale longwave radiation field from gridcell to topounit
...
contains
    ...
    !-----------------------------------------------------------------------
  subroutine downscale_grd_to_topounit(g, i, x2l)
    !
    ! !DESCRIPTION:
...
!    logical  :: do_lapse_downscaling = .true.
	...
	do t = grc_pp%topi(g), grc_pp%topf(g)
	   if(numt > 1) then                          !downscaling is done only if a grid has more than 1 topounits    
	      topoElv  = top_pp%elevation             ! Topounit sfc elevation
		  
		  if(uoa_flag) then
			call downscale_atmo_state_to_topounit_UoA(g, i, x2l)
		  else
		    call downscale_atmo_state_to_topounit_glc(g, i, x2l)  
		  end if
		  
		  call downscale_longwave_to_topounit(g, i, x2l)  
		  ! call precipitation downscaling 
		  call downscale_precip_to_topounit(g, i, x2l,t,mxElv,uovern_t,grdElv,topoElv,rain_g, snow_g) 
  ...
  !------------------------------------------------------
  ! Downscale other atmospheric state variables based on UoA method place holder
  subroutine downscale_atmo_state_to_topounit_UoA() 
  
  end subroutine downscale_atmo_state_to_topounit_UoA
  
  !-------------------------------------------------------
  ! Downscale longwave radiation  place holder
  subroutine downscale_longwave_to_topounit()
  ...
  !-----------------------------------------------------------------------
  subroutine downscale_atmo_state_to_topounit_glc(g, i, x2l)
    !
    ! !DESCRIPTION:
...
!      call check_downscale_consistency(bounds, atm2lnd_vars)
  ...
  end subroutine downscale_atmo_state_to_topounit_glc
