...
contains
    ...
    !-----------------------------------------------------------------------
  subroutine downscale_grd_to_topounit(g, i, x2l,FrdNum, gZ, sZ,sMaxZ,gPCP)
    !
    ! !DESCRIPTION:
	...
	real(r8)                         , intent(in)    :: FrdNum(:,:)
	real(r8)                         , intent(in)    :: gZ(:,:)
	real(r8)                         , intent(in)    :: sZ(:)           ! Elevation of Topounits
	real(r8)                         , intent(in)    :: sMaxZ           ! Maximum elevation within the grid
	real(r8)                         , intent(in)    :: gPCP(:,:)       ! grid level precipitation
...
!    nstep = get_nstep()
    ...
    do t = grc_pp%topi(g), grc_pp%topf(g)
	  ...
	  call downscale_atmo_state_to_topounit()       
      call downscale_longwave_to_topounit()       
      call downscale_precip_to_topounit(g, i, x2l,FrdNum, gZ, sZ,sMaxZ,gPCP) 
...
!      call check_downscale_consistency(bounds, atm2lnd_vars)
  ...
  subroutine downscale_atmo_state_to_topounit()
  ...
  subroutine downscale_precip_to_topounit(g, i, x2l,FrdNum, gZ, sZ,sMaxZ,gPCP)
