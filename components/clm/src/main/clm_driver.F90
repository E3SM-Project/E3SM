module clm_driver
  ...
  use SedYieldMod            , only : SoilErosion, Landslide
...
contains
  ...
  subroutine clm_drv(doalb, nextsw_cday, declinp1, declin, rstwr, nlend, rdate)
    ...
    do nc = 1,nclumps
       ...
       if (use_erosion) then
          ...
          if (ero_lndsld) then
             call Landslide(bounds_clump, filter(nc)%num_hydrologyc, filter(nc)%hydrologyc, &
  ...
  subroutine clm_drv_patch2col (bounds, num_nolakec, filter_nolakec, &
    ...
    call p2c (bounds, num_nolakec, filter_nolakec, &
         waterflux_vars%qflx_dirct_rain_patch(bounds%begp:bounds%endp), &
         waterflux_vars%qflx_dirct_rain_col(bounds%begc:bounds%endc))

    call p2c (bounds, num_nolakec, filter_nolakec, &
         waterflux_vars%qflx_leafdrip_patch(bounds%begp:bounds%endp), &
         waterflux_vars%qflx_leafdrip_col(bounds%begc:bounds%endc))
