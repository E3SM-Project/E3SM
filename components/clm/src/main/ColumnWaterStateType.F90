module ColumnWaterStateType
  
# include "shr_assert.h"  ! from WaterStateType

  ! -------------------------------------------------------- 
  ! ALM sub-grid hierarchy:
  ! Define Soil column data type with state and flux type as
  ! well as related subroutines (such as Init and Clean)
  ! -------------------------------------------------------- 
  ! 10 Oct 2016, DW  
...
!!!!   ******DW******** we will need to refer to the new data types lun_ws???
...
!  use ColumnType     , only : col   
  ...
  ! declare the public instances of column water state types

  type(column_water_state), public, target :: col_ws
 ...
 subroutine init_col_ws(this, bounds, &
...
!------------------------------------------------------------------------
  subroutine initallocate_col_ws(this, bounds)
    !
    ! !DESCRIPTION:
...
!    begg = bounds%begg; endg= bounds%endg
  ...
  end subroutine initallocate_col_ws

  !------------------------------------------------------------------------
  subroutine inithistory_col_ws(this, bounds)
    !
    ! !DESCRIPTION:
    ! Initialize module data structure
    !
...
!!!!  *****DW*****   how to handle this no (begc:endc)
  ...
  end subroutine inithistory_col_ws


 !-----------------------------------------------------------------------
  subroutine initcold_col_ws(this, bounds, &
       h2osno_input_col, snow_depth_input_col, watsat_col, t_soisno_col)
    !
    ! !DESCRIPTION:
...
!!!!!!  *****DW****   how to handle this lun data structure???
  ...
  end subroutine initcold_col_ws

 !------------------------------------------------------------------------
  subroutine restart_col_ws(this, bounds, ncid, flag, &
       watsat_col)
    ! 
    ! !DESCRIPTION:
...
!         interpinic_flag='interp', readvar=readvar, data=this%qaf_lun)
  ...
  end subroutine restart_col_ws

  subroutine reset_col_ws(this, column)
    !
    ! !DESCRIPTION:
  ...
  end subroutine reset_col_ws

end module ColumnWaterStateType
