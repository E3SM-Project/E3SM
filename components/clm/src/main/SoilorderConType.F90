...
contains
  ...
  subroutine soilorderconInit()
    ...
    allocate(soilordercon%smax           (0:nsoilorder))        ; soilordercon%smax(:)         = spval
    allocate(soilordercon%ks_sorption    (0:nsoilorder))        ; soilordercon%ks_sorption(:)  = spval
    allocate(soilordercon%r_weather      (0:nsoilorder))        ; soilordercon%r_weather(:)    = spval
    allocate(soilordercon%r_adsorp       (0:nsoilorder))        ; soilordercon%r_adsorp(:)     = spval
    allocate(soilordercon%r_desorp       (0:nsoilorder))        ; soilordercon%r_desorp(:)     = spval
    allocate(soilordercon%r_occlude      (0:nsoilorder))        ; soilordercon%r_occlude(:)    = spval

    allocate(soilordercon%k_s1_biochem      (0:nsoilorder))     ; soilordercon%k_s1_biochem(:) = spval
    allocate(soilordercon%k_s2_biochem      (0:nsoilorder))     ; soilordercon%k_s2_biochem(:) = spval
    allocate(soilordercon%k_s3_biochem      (0:nsoilorder))     ; soilordercon%k_s3_biochem(:) = spval
    allocate(soilordercon%k_s4_biochem      (0:nsoilorder))     ; soilordercon%k_s4_biochem(:) = spval
