module PhosphorusStateUpdate1BeTRMod
  !-----------------------------------------------------------------------
  ! !DESCRIPTION:
...
contains
  ...
  !-----------------------------------------------------------------------
  subroutine PhosphorusStateUpdate1Soil(num_soilc, filter_soilc, num_soilp, filter_soilp, &
       cnstate_vars, col_ps, col_pf, is_decomp_on)
    !
    ! !DESCRIPTION:
...
!         nfixation_prof        => cnstate_vars%nfixation_prof_col          , & ! Input:  [real(r8) (:,:)   ]  profile over which N fixation is distributed through column (1/m)
  ...
  end subroutine PhosphorusStateUpdate1Veg


  !-----------------------------------------------------------------------
  subroutine PhosphorusStateUpdate1(num_soilc, filter_soilc, num_soilp, filter_soilp, &
       cnstate_vars, col_ps, veg_ps, col_pf, veg_pf, ldecomp_on)
    !
    ! !DESCRIPTION:
    ! On the radiation time step, update all the prognostic phosphorus state
    ! variables (except for gap-phase mortality and fire fluxes)
...
end module PhosphorusStateUpdate1BeTRMod
