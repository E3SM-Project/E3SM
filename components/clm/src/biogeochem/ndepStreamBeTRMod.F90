module ndepStreamBeTRMod
  ...
  ! ! PRIVATE TYPES
  type(shr_strdata_type)  :: sdat_noy         ! input data stream as noy
  type(shr_strdata_type)  :: sdat_nhx         ! input data stream as nh3
...
contains
  ...
  subroutine ndep_init(bounds)
   ...
   call shr_strdata_create(sdat_nhx,name="clmndep",    &
        ...
        fldListFile='NDEP_NHx_year',               &
        fldListModel='NDEP_NHx_year',              &
        fillalgo='none',                           &
        mapalgo=ndepmapalgo,                       &
   ...
   call shr_strdata_create(sdat_noy,name="clmndep",    &
        pio_subsystem=pio_subsystem,               &
        pio_iotype=shr_pio_getiotype(inst_name),   &
        mpicom=mpicom, compid=comp_id,             &
        gsmap=gsmap_lnd_gdc2glo, ggrid=dom_clm,    &
        ...
        filePath='',                               &
        filename=(/trim(stream_fldFileName_ndep)/),&
        fldListFile='NDEP_NOy_year',               &
        fldListModel='NDEP_NOy_year',              &
        fillalgo='none',                           &
        mapalgo=ndepmapalgo,                       &
        calendar=get_calendar(),                   &
        taxmode='extend'                           )
   ...
   if (masterproc) then
      call shr_strdata_print(sdat_nhx,'CLMNDEP data')
      call shr_strdata_print(sdat_noy,'CLMNDEP data')
   endif
 ...
 subroutine ndep_interp(bounds, atm2lnd_vars)
   ...
   call shr_strdata_advance(sdat_nhx, mcdate, sec, mpicom, 'ndepdyn')
   call shr_strdata_advance(sdat_noy, mcdate, sec, mpicom, 'ndepdyn')

   ig = 0
   dayspyr = get_days_per_year( )
   do g = bounds%begg,bounds%endg
      ig = ig+1
      atm2lnd_vars%forc_ndep_nh3x_grc(g) = sdat_nhx%avs(1)%rAttr(1,ig) / (secspday * dayspyr)
      atm2lnd_vars%forc_ndep_noy_grc(g) = sdat_noy%avs(1)%rAttr(1,ig) / (secspday * dayspyr)
      atm2lnd_vars%forc_ndep_grc(g) = atm2lnd_vars%forc_ndep_nh3x_grc(g) + atm2lnd_vars%forc_ndep_noy_grc(g)
   end do
