...
!    use clm_varctl, only : use_pflotran, pf_cmode, pf_hmode
 ...
 !-----------------------------------------------------------------------
    ...
    !----------------------------------------------------------------
     ...
     do j = 1, nlev
      ...
      this%nflx_input_litr_met_vr_col(c,j) =           &
        this%phenology_n_to_litr_met_n_col(c,j)        + & !phenology
        this%dwt_frootn_to_litr_met_n_col(c,j)         + & !dynamic land cover
        this%gap_mortality_n_to_litr_met_n_col(c,j)    + & !gap mortality
        this%harvest_n_to_litr_met_n_col(c,j)          + & !harvest
        this%m_n_to_litr_met_fire_col(c,j)    ! fire mortality

      !cellulose nitrogen
      this%nflx_input_litr_cel_vr_col(c,j)=  &
         this%phenology_n_to_litr_cel_n_col(c,j)       + & !phenology
         this%dwt_frootn_to_litr_cel_n_col(c,j)        + & !dynamic land cover
         this%gap_mortality_n_to_litr_cel_n_col(c,j)   + & !gap mortality
         this%harvest_n_to_litr_cel_n_col(c,j)         + & !harvest
         this%m_n_to_litr_cel_fire_col(c,j)             ! fire mortality

      !lignin nitrogen
      this%nflx_input_litr_lig_vr_col(c,j) = &
         this%phenology_n_to_litr_lig_n_col(c,j)        + &  !phenology
         this%dwt_frootn_to_litr_lig_n_col(c,j)         + &   !dynamic land cover
         this%gap_mortality_n_to_litr_lig_n_col(c,j)    + & !gap mortality
         this%harvest_n_to_litr_lig_n_col(c,j)          + & !harvest
         this%m_n_to_litr_lig_fire_col(c,j)               ! fire mortality
  ...
  do fc = 1,num_soilc
    c = filter_soilc(fc)
    this%nflx_minn_input_nh4_vr_col(c,1) = &
    this%ndep_to_sminn_col(c)                   + &
    this%fert_to_sminn_col(c)
    this%nflx_minn_input_nh4_vr_col(c,2:nlev) = 0._r8
    this%nflx_minn_input_no3_vr_col(c,1:nlev) = 0._r8
  enddo
