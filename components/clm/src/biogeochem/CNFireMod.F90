...
contains
  ...
  subroutine CNFireArea (bounds, &
    ...
    associate(                                                                & 
         ...
         deadcrootc         =>    carbonstate_vars%deadcroot_patch         , & ! Input:  [real(r8) (:)     ]  (gC/m2) dead coarse root C                        
         ...
         totvegc            =>    carbonstate_vars%totveg_patch            , & ! Input:  [real(r8) (:)     ]  (gC/m2) total vegetation carbon, excluding cpool  
         ...
         totlitc            =>    carbonstate_vars%totlit_col              , & ! Input:  [real(r8) (:)     ]  (gC/m2) total lit C (column-level mean)           
         decomp_cpools_vr   =>    carbonstate_vars%decomp_pools_vr_col     , & ! Input:  [real(r8) (:,:,:) ]  (gC/m3)  VR decomp. (litter, cwd, soil)
         rootc_col          =>    carbonstate_vars%rootc_col                , & ! Output: [real(r8) (:)     ]  root carbon                                       
         totveg_col        =>    carbonstate_vars%totveg_col              , & ! Output: [real(r8) (:)     ]  totvegc at column level                           
      ...
      call p2c(bounds, num_soilc, filter_soilc, &
           ...
           totveg_col(bounds%begc:bounds%endc))
     ...
     do fc = 1, num_soilc
        ...
        if( cropf_col(c)  <  1.0 )then
           if (trotr1_col(c)+trotr2_col(c)>0.6_r8) then
              ...
              fuelc(c) = totlitc(c)+totveg_col(c)-rootc_col(c)-fuelc_crop(c)*cropf_col(c)
 ...
 subroutine CNFireFluxes (num_soilc, filter_soilc, num_soilp, filter_soilp, &
   ...
   associate(                                                                                                   & 
        ...
        decomp_cpools_vr                    =>    carbonstate_vars%decomp_pools_vr_col                       , & ! Input:  [real(r8) (:,:,:) ]  (gC/m3)  VR decomp. (litter, cwd, soil)
        totsomc                             =>    carbonstate_vars%totsom_col                                , & ! Input:  [real(r8) (:)     ]  (gC/m2) total soil organic matter C
        ...
        cpool                               =>    carbonstate_vars%pool_patch                                , & ! Input:  [real(r8) (:)     ]  (gC/m2) C pool        

        decomp_npools_vr                    =>    nitrogenstate_vars%decomp_pools_vr_col                     , & ! Input:  [real(r8) (:,:,:) ]  (gC/m3)  VR decomp. (litter, cwd, soil)
        ...
        deadstemn                           =>    nitrogenstate_vars%deadstem_patch                          , & ! Input:  [real(r8) (:)     ]  (gN/m2) dead stem N                               
        ...
        deadcrootn                          =>    nitrogenstate_vars%deadcroot_patch                         , & ! Input:  [real(r8) (:)     ]  (gN/m2) dead coarse root N                        
        ...
        npool                               =>    nitrogenstate_vars%pool_patch                              , & ! Input:  [real(r8) (:)     ]  (gN/m2) plant pool of stored N  
        ! add phosphorus state variables - X.YANG
        decomp_ppools_vr                    =>    phosphorusstate_vars%decomp_pools_vr_col                     , & ! Input:  [real(r8) (:,:,:) ]  (gP/m3)  VR decomp. (litter, cwd, soil)
        ...
        livestemp                           =>    phosphorusstate_vars%livestem_patch                          , & ! Input:  [real(r8) (:)     ]  (gP/m2) live stem P                               
        ...
        deadstemp                           =>    phosphorusstate_vars%deadstem_patch                          , & ! Input:  [real(r8) (:)     ]  (gP/m2) dead stem P                               
        ...
        livecrootp                          =>    phosphorusstate_vars%livecroot_patch                         , & ! Input:  [real(r8) (:)     ]  (gP/m2) live coarse root P                        
        ...
        deadcrootp                          =>    phosphorusstate_vars%deadcroot_patch                         , & ! Input:  [real(r8) (:)     ]  (gP/m2) dead coarse root P                        
        ...
        ppool                               =>    phosphorusstate_vars%pool_patch                              , & ! Input:  [real(r8) (:)     ]  (gP/m2) plant pool of storage P            

        fire_mortality_c_to_cwdc            =>    carbonflux_vars%fire_mortality_to_cwd_col                , & ! Input:  [real(r8) (:,:)   ]  C flux fire mortality to CWD (gC/m3/s)
        somc_fire                           =>    carbonflux_vars%somc_fire_col                               , & ! Input:  [real(r8) (:)     ]  (gC/m2/s) fire C emissions due to peat burning
        m_leafc_to_fire                     =>    carbonflux_vars%m_leaf_to_fire_patch                       , & ! Input:  [real(r8) (:)     ]  (gC/m2/s) fire C emis. from leafc	    
        m_leafc_storage_to_fire             =>    carbonflux_vars%m_leaf_storage_to_fire_patch               , & ! Input:  [real(r8) (:)     ]  (gC/m2/s) fire C emis. from leafc_storage   
        m_leafc_xfer_to_fire                =>    carbonflux_vars%m_leaf_xfer_to_fire_patch                  , & ! Input:  [real(r8) (:)     ]  (gC/m2/s) fire C emis. from leafc_xfer	    
        m_livestemc_to_fire                 =>    carbonflux_vars%m_livestem_to_fire_patch                   , & ! Input:  [real(r8) (:)     ]  (gC/m2/s) fire C emis. from livestemc	         
        m_livestemc_storage_to_fire         =>    carbonflux_vars%m_livestem_storage_to_fire_patch           , & ! Input:  [real(r8) (:)     ]  (gC/m2/s) C emis. livestemc_storage	       
        m_livestemc_xfer_to_fire            =>    carbonflux_vars%m_livestem_xfer_to_fire_patch              , & ! Input:  [real(r8) (:)     ]  (gC/m2/s) C emis. livestemc_xfer	       
        m_deadstemc_to_fire                 =>    carbonflux_vars%m_deadstem_to_fire_patch                   , & ! Input:  [real(r8) (:)     ]  (gC/m2/s) C emis. deadstemc_xfer	       
        m_deadstemc_storage_to_fire         =>    carbonflux_vars%m_deadstem_storage_to_fire_patch           , & ! Input:  [real(r8) (:)     ]  (gC/m2/s) C emis. deadstemc_storage	       
        m_deadstemc_xfer_to_fire            =>    carbonflux_vars%m_deadstem_xfer_to_fire_patch              , & ! Input:  [real(r8) (:)     ]  (gC/m2/s) C emis. deadstemc_xfer	       
        m_frootc_to_fire                    =>    carbonflux_vars%m_froot_to_fire_patch                      , & ! Input:  [real(r8) (:)     ]  (gC/m2/s) C emis. frootc		       
        m_frootc_storage_to_fire            =>    carbonflux_vars%m_froot_storage_to_fire_patch              , & ! Input:  [real(r8) (:)     ]  (gC/m2/s) C emis. frootc_storage	       
        m_frootc_xfer_to_fire               =>    carbonflux_vars%m_froot_xfer_to_fire_patch                 , & ! Input:  [real(r8) (:)     ]  (gC/m2/s) C emis. frootc_xfer		       
        m_livecrootc_to_fire                =>    carbonflux_vars%m_livecroot_to_fire_patch                  , & ! Input:  [real(r8) (:)     ]  (gC/m2/s) C emis. livecrootc		    	
        m_livecrootc_storage_to_fire        =>    carbonflux_vars%m_livecroot_storage_to_fire_patch          , & ! Input:  [real(r8) (:)     ]  (gC/m2/s) C emis. livecrootc_storage	       
        m_livecrootc_xfer_to_fire           =>    carbonflux_vars%m_livecroot_xfer_to_fire_patch             , & ! Input:  [real(r8) (:)     ]  (gC/m2/s) C emis. livecrootc_xfer	       
        m_deadcrootc_to_fire                =>    carbonflux_vars%m_deadcroot_to_fire_patch                  , & ! Input:  [real(r8) (:)     ]  (gC/m2/s) C emis. deadcrootc		    	
        m_deadcrootc_storage_to_fire        =>    carbonflux_vars%m_deadcroot_storage_to_fire_patch          , & ! Input:  [real(r8) (:)     ]  (gC/m2/s) C emis. deadcrootc_storage	       
        m_deadcrootc_xfer_to_fire           =>    carbonflux_vars%m_deadcroot_xfer_to_fire_patch             , & ! Input:  [real(r8) (:)     ]  (gC/m2/s) C emis. deadcrootc_xfer	       
        ...
        m_cpool_to_fire                     =>    carbonflux_vars%m_pool_to_fire_patch                       , & ! Input:  [real(r8) (:)     ]  (gC/m2/s) C emis. cpool     
  
        fire_mortality_n_to_cwdn            =>    nitrogenflux_vars%fire_mortality_to_cwd_col              , & ! Input:  [real(r8) (:,:)   ]  N flux fire mortality to CWD (gN/m3/s)
        m_leafn_to_fire                     =>    nitrogenflux_vars%m_leaf_to_fire_patch                     , & ! Input:  [real(r8) (:)     ]  (gN/m2/s) N emis. leafn		  
        m_leafn_storage_to_fire             =>    nitrogenflux_vars%m_leaf_storage_to_fire_patch             , & ! Input:  [real(r8) (:)     ]  (gN/m2/s) N emis. leafn_storage	  
        m_leafn_xfer_to_fire                =>    nitrogenflux_vars%m_leaf_xfer_to_fire_patch                , & ! Input:  [real(r8) (:)     ]  (gN/m2/s) N emis. leafn_xfer	       
        m_livestemn_to_fire                 =>    nitrogenflux_vars%m_livestem_to_fire_patch                 , & ! Input:  [real(r8) (:)     ]  (gN/m2/s) N emis. livestemn	       
        m_livestemn_storage_to_fire         =>    nitrogenflux_vars%m_livestem_storage_to_fire_patch         , & ! Input:  [real(r8) (:)     ]  (gN/m2/s) N emis. livestemn_s	       
        m_livestemn_xfer_to_fire            =>    nitrogenflux_vars%m_livestem_xfer_to_fire_patch            , & ! Input:  [real(r8) (:)     ]  (gN/m2/s) N emis. livestemn_xfer       
        m_deadstemn_to_fire                 =>    nitrogenflux_vars%m_deadstem_to_fire_patch                 , & ! Input:  [real(r8) (:)     ]  (gN/m2/s) N emis. deadstemn	       
        m_deadstemn_storage_to_fire         =>    nitrogenflux_vars%m_deadstem_storage_to_fire_patch         , & ! Input:  [real(r8) (:)     ]  (gN/m2/s) N emis. deadstemn_storage    
        m_deadstemn_xfer_to_fire            =>    nitrogenflux_vars%m_deadstem_xfer_to_fire_patch            , & ! Input:  [real(r8) (:)     ]  (gN/m2/s) N emis. deadstemn_xfer       
        m_frootn_to_fire                    =>    nitrogenflux_vars%m_froot_to_fire_patch                    , & ! Input:  [real(r8) (:)     ]  (gN/m2/s) N emis. frootn	       
        m_frootn_storage_to_fire            =>    nitrogenflux_vars%m_froot_storage_to_fire_patch            , & ! Input:  [real(r8) (:)     ]  (gN/m2/s) N emis. frootn_storage       
        m_frootn_xfer_to_fire               =>    nitrogenflux_vars%m_froot_xfer_to_fire_patch               , & ! Input:  [real(r8) (:)     ]  (gN/m2/s) N emis. frootn_xfer	       
        m_livecrootn_to_fire                =>    nitrogenflux_vars%m_livecroot_to_fire_patch                , & ! Input:  [real(r8) (:)     ]  (gN/m2/s) N emis. m_livecrootn_to_fire 
        m_livecrootn_storage_to_fire        =>    nitrogenflux_vars%m_livecroot_storage_to_fire_patch        , & ! Input:  [real(r8) (:)     ]  (gN/m2/s) N emis. livecrootn_storage   
        m_livecrootn_xfer_to_fire           =>    nitrogenflux_vars%m_livecroot_xfer_to_fire_patch           , & ! Input:  [real(r8) (:)     ]  (gN/m2/s) N emis. livecrootn_xfer      
        m_deadcrootn_to_fire                =>    nitrogenflux_vars%m_deadcroot_to_fire_patch                , & ! Input:  [real(r8) (:)     ]  (gN/m2/s) N emis. deadcrootn	       
        m_deadcrootn_storage_to_fire        =>    nitrogenflux_vars%m_deadcroot_storage_to_fire_patch        , & ! Input:  [real(r8) (:)     ]  (gN/m2/s) N emis. deadcrootn_storage   
        m_deadcrootn_xfer_to_fire           =>    nitrogenflux_vars%m_deadcroot_xfer_to_fire_patch           , & ! Input:  [real(r8) (:)     ]  (gN/m2/s) N emis. deadcrootn_xfer      
        m_retransn_to_fire                  =>    nitrogenflux_vars%m_retransn_to_fire_patch                  , & ! Input:  [real(r8) (:)     ]  (gN/m2/s) N emis. retransn             
        m_npool_to_fire                     =>    nitrogenflux_vars%m_pool_to_fire_patch                     , & ! Input:  [real(r8) (:)     ]  (gN/m2/s) N emis. npooln             
       

        ! add phosphorus fluxes with fire - these will be adding to 
        ! -.XYANG 
        fire_mortality_p_to_cwdp            =>    phosphorusflux_vars%fire_mortality_to_cwd_col              , & ! Input:  [real(r8) (:,:)   ]  P flux fire mortality to CWD (gP/m3/s)
        m_leafp_to_fire                     =>    phosphorusflux_vars%m_leaf_to_fire_patch                     , & ! Input:  [real(r8) (:)     ]  (gP/m2/s) P emis. leafp		  
        m_leafp_storage_to_fire             =>    phosphorusflux_vars%m_leaf_storage_to_fire_patch             , & ! Input:  [real(r8) (:)     ]  (gP/m2/s) P emis. leafp_storage	  
        m_leafp_xfer_to_fire                =>    phosphorusflux_vars%m_leaf_xfer_to_fire_patch                , & ! Input:  [real(r8) (:)     ]  (gP/m2/s) P emis. leafp_xfer	       
        m_livestemp_to_fire                 =>    phosphorusflux_vars%m_livestem_to_fire_patch                 , & ! Input:  [real(r8) (:)     ]  (gP/m2/s) P emis. livestemp	       
        m_livestemp_storage_to_fire         =>    phosphorusflux_vars%m_livestem_storage_to_fire_patch         , & ! Input:  [real(r8) (:)     ]  (gP/m2/s) P emis. livestemp_s	       
        m_livestemp_xfer_to_fire            =>    phosphorusflux_vars%m_livestem_xfer_to_fire_patch            , & ! Input:  [real(r8) (:)     ]  (gP/m2/s) P emis. livestemp_xfer       
        m_deadstemp_to_fire                 =>    phosphorusflux_vars%m_deadstem_to_fire_patch                 , & ! Input:  [real(r8) (:)     ]  (gP/m2/s) P emis. deadstemp	       
        m_deadstemp_storage_to_fire         =>    phosphorusflux_vars%m_deadstem_storage_to_fire_patch         , & ! Input:  [real(r8) (:)     ]  (gP/m2/s) P emis. deadstemp_storage    
        m_deadstemp_xfer_to_fire            =>    phosphorusflux_vars%m_deadstem_xfer_to_fire_patch            , & ! Input:  [real(r8) (:)     ]  (gP/m2/s) P emis. deadstemp_xfer       
        m_frootp_to_fire                    =>    phosphorusflux_vars%m_froot_to_fire_patch                    , & ! Input:  [real(r8) (:)     ]  (gP/m2/s) P emis. frootp	       
        m_frootp_storage_to_fire            =>    phosphorusflux_vars%m_froot_storage_to_fire_patch            , & ! Input:  [real(r8) (:)     ]  (gP/m2/s) P emis. frootp_storage       
        m_frootp_xfer_to_fire               =>    phosphorusflux_vars%m_froot_xfer_to_fire_patch               , & ! Input:  [real(r8) (:)     ]  (gP/m2/s) P emis. frootp_xfer	       
        m_livecrootp_to_fire                =>    phosphorusflux_vars%m_livecroot_to_fire_patch                , & ! Input:  [real(r8) (:)     ]  (gP/m2/s) P emis. m_livecrootp_to_fire 
        m_livecrootp_storage_to_fire        =>    phosphorusflux_vars%m_livecroot_storage_to_fire_patch        , & ! Input:  [real(r8) (:)     ]  (gP/m2/s) P emis. livecrootp_storage   
        m_livecrootp_xfer_to_fire           =>    phosphorusflux_vars%m_livecroot_xfer_to_fire_patch           , & ! Input:  [real(r8) (:)     ]  (gP/m2/s) P emis. livecrootp_xfer      
        m_deadcrootp_to_fire                =>    phosphorusflux_vars%m_deadcroot_to_fire_patch                , & ! Input:  [real(r8) (:)     ]  (gP/m2/s) P emis. deadcrootp	       
        m_deadcrootp_storage_to_fire        =>    phosphorusflux_vars%m_deadcroot_storage_to_fire_patch        , & ! Input:  [real(r8) (:)     ]  (gP/m2/s) P emis. deadcrootp_storage   
        m_deadcrootp_xfer_to_fire           =>    phosphorusflux_vars%m_deadcroot_xfer_to_fire_patch           , & ! Input:  [real(r8) (:)     ]  (gP/m2/s) P emis. deadcrootp_xfer      
        m_retransp_to_fire                  =>    phosphorusflux_vars%m_retransp_to_fire_patch                  , & ! Input:  [real(r8) (:)     ]  (gP/m2/s) P emis. retransp             
        m_ppool_to_fire                     =>    phosphorusflux_vars%m_pool_to_fire_patch                     , & ! Input:  [real(r8) (:)     ]  (gP/m2/s) P emis. ppool     
        
        m_leafc_to_litter_fire              =>    carbonflux_vars%m_leaf_to_litter_fire_patch                , & ! Output: [real(r8) (:)     ]                                                    
        m_leafc_storage_to_litter_fire      =>    carbonflux_vars%m_leaf_storage_to_litter_fire_patch        , & ! Output: [real(r8) (:)     ]                                                    
        m_leafc_xfer_to_litter_fire         =>    carbonflux_vars%m_leaf_xfer_to_litter_fire_patch           , & ! Output: [real(r8) (:)     ]                                                    
        m_livestemc_to_litter_fire          =>    carbonflux_vars%m_livestem_to_litter_fire_patch            , & ! Output: [real(r8) (:)     ]                                                    
        m_livestemc_storage_to_litter_fire  =>    carbonflux_vars%m_livestem_storage_to_litter_fire_patch    , & ! Output: [real(r8) (:)     ]                                                    
        m_livestemc_xfer_to_litter_fire     =>    carbonflux_vars%m_livestem_xfer_to_litter_fire_patch       , & ! Output: [real(r8) (:)     ]                                                    
        m_livestemc_to_deadstemc_fire       =>    carbonflux_vars%m_livestem_to_deadstem_fire_patch         , & ! Output: [real(r8) (:)     ]                                                    
        m_deadstemc_to_litter_fire          =>    carbonflux_vars%m_deadstem_to_litter_fire_patch            , & ! Output: [real(r8) (:)     ]                                                    
        m_deadstemc_storage_to_litter_fire  =>    carbonflux_vars%m_deadstem_storage_to_litter_fire_patch    , & ! Output: [real(r8) (:)     ]                                                    
        m_deadstemc_xfer_to_litter_fire     =>    carbonflux_vars%m_deadstem_xfer_to_litter_fire_patch       , & ! Output: [real(r8) (:)     ]                                                    
        m_frootc_to_litter_fire             =>    carbonflux_vars%m_froot_to_litter_fire_patch               , & ! Output: [real(r8) (:)     ]                                                    
        m_frootc_storage_to_litter_fire     =>    carbonflux_vars%m_froot_storage_to_litter_fire_patch       , & ! Output: [real(r8) (:)     ]                                                    
        m_frootc_xfer_to_litter_fire        =>    carbonflux_vars%m_froot_xfer_to_litter_fire_patch          , & ! Output: [real(r8) (:)     ]                                                    
        m_livecrootc_to_litter_fire         =>    carbonflux_vars%m_livecroot_to_litter_fire_patch           , & ! Output: [real(r8) (:)     ]                                                    
        m_livecrootc_storage_to_litter_fire =>    carbonflux_vars%m_livecroot_storage_to_litter_fire_patch   , & ! Output: [real(r8) (:)     ]                                                    
        m_livecrootc_xfer_to_litter_fire    =>    carbonflux_vars%m_livecroot_xfer_to_litter_fire_patch      , & ! Output: [real(r8) (:)     ]                                                    
        m_livecrootc_to_deadcrootc_fire     =>    carbonflux_vars%m_livecroot_to_deadcroot_fire_patch       , & ! Output: [real(r8) (:)     ]                                                    
        m_deadcrootc_to_litter_fire         =>    carbonflux_vars%m_deadcroot_to_litter_fire_patch           , & ! Output: [real(r8) (:)     ]                                                    
        m_deadcrootc_storage_to_litter_fire =>    carbonflux_vars%m_deadcroot_storage_to_litter_fire_patch   , & ! Output: [real(r8) (:)     ]                                                    
        m_deadcrootc_xfer_to_litter_fire    =>    carbonflux_vars%m_deadcroot_xfer_to_litter_fire_patch      , & ! Output: [real(r8) (:)     ]                                                   
     ...
     m_leafn_to_litter_fire              =>    nitrogenflux_vars%m_leaf_to_litter_fire_patch              
     m_leafn_storage_to_litter_fire      =>    nitrogenflux_vars%m_leaf_storage_to_litter_fire_patch      
     m_leafn_xfer_to_litter_fire         =>    nitrogenflux_vars%m_leaf_xfer_to_litter_fire_patch         
     m_livestemn_to_litter_fire          =>    nitrogenflux_vars%m_livestem_to_litter_fire_patch          
     m_livestemn_storage_to_litter_fire  =>    nitrogenflux_vars%m_livestem_storage_to_litter_fire_patch  
     m_livestemn_xfer_to_litter_fire     =>    nitrogenflux_vars%m_livestem_xfer_to_litter_fire_patch     
     m_livestemn_to_deadstemn_fire       =>    nitrogenflux_vars%m_livestem_to_deadstem_fire_patch       
     m_deadstemn_to_litter_fire          =>    nitrogenflux_vars%m_deadstem_to_litter_fire_patch          
     m_deadstemn_storage_to_litter_fire  =>    nitrogenflux_vars%m_deadstem_storage_to_litter_fire_patch  
     m_deadstemn_xfer_to_litter_fire     =>    nitrogenflux_vars%m_deadstem_xfer_to_litter_fire_patch     
     m_frootn_to_litter_fire             =>    nitrogenflux_vars%m_froot_to_litter_fire_patch             
     m_frootn_storage_to_litter_fire     =>    nitrogenflux_vars%m_froot_storage_to_litter_fire_patch     
     m_frootn_xfer_to_litter_fire        =>    nitrogenflux_vars%m_froot_xfer_to_litter_fire_patch        
     m_livecrootn_to_litter_fire         =>    nitrogenflux_vars%m_livecroot_to_litter_fire_patch         
     m_livecrootn_storage_to_litter_fire =>    nitrogenflux_vars%m_livecroot_storage_to_litter_fire_patch 
     m_livecrootn_xfer_to_litter_fire    =>    nitrogenflux_vars%m_livecroot_xfer_to_litter_fire_patch    
     m_livecrootn_to_deadcrootn_fire     =>    nitrogenflux_vars%m_livecroot_to_deadcroot_fire_patch     
     m_deadcrootn_to_litter_fire         =>    nitrogenflux_vars%m_deadcroot_to_litter_fire_patch         
     m_deadcrootn_storage_to_litter_fire =>    nitrogenflux_vars%m_deadcroot_storage_to_litter_fire_patch 
     m_deadcrootn_xfer_to_litter_fire    =>    nitrogenflux_vars%m_deadcroot_xfer_to_litter_fire_patch    
     ...
     m_leafp_to_litter_fire              =>    phosphorusflux_vars%m_leaf_to_litter_fire_patch              
     m_leafp_storage_to_litter_fire      =>    phosphorusflux_vars%m_leaf_storage_to_litter_fire_patch      
     m_leafp_xfer_to_litter_fire         =>    phosphorusflux_vars%m_leaf_xfer_to_litter_fire_patch         
     m_livestemp_to_litter_fire          =>    phosphorusflux_vars%m_livestem_to_litter_fire_patch          
     m_livestemp_storage_to_litter_fire  =>    phosphorusflux_vars%m_livestem_storage_to_litter_fire_patch  
     m_livestemp_xfer_to_litter_fire     =>    phosphorusflux_vars%m_livestem_xfer_to_litter_fire_patch     
     m_livestemp_to_deadstemp_fire       =>    phosphorusflux_vars%m_livestem_to_deadstem_fire_patch       
     m_deadstemp_to_litter_fire          =>    phosphorusflux_vars%m_deadstem_to_litter_fire_patch          
     m_deadstemp_storage_to_litter_fire  =>    phosphorusflux_vars%m_deadstem_storage_to_litter_fire_patch  
     m_deadstemp_xfer_to_litter_fire     =>    phosphorusflux_vars%m_deadstem_xfer_to_litter_fire_patch     
     m_frootp_to_litter_fire             =>    phosphorusflux_vars%m_froot_to_litter_fire_patch             
     m_frootp_storage_to_litter_fire     =>    phosphorusflux_vars%m_froot_storage_to_litter_fire_patch     
     m_frootp_xfer_to_litter_fire        =>    phosphorusflux_vars%m_froot_xfer_to_litter_fire_patch        
     m_livecrootp_to_litter_fire         =>    phosphorusflux_vars%m_livecroot_to_litter_fire_patch         
     m_livecrootp_storage_to_litter_fire =>    phosphorusflux_vars%m_livecroot_storage_to_litter_fire_patch 
     m_livecrootp_xfer_to_litter_fire    =>    phosphorusflux_vars%m_livecroot_xfer_to_litter_fire_patch    
     m_livecrootp_to_deadcrootp_fire     =>    phosphorusflux_vars%m_livecroot_to_deadcroot_fire_patch     
     m_deadcrootp_to_litter_fire         =>    phosphorusflux_vars%m_deadcroot_to_litter_fire_patch         
     m_deadcrootp_storage_to_litter_fire =>    phosphorusflux_vars%m_deadcroot_storage_to_litter_fire_patch 
     m_deadcrootp_xfer_to_litter_fire    =>    phosphorusflux_vars%m_deadcroot_xfer_to_litter_fire_patch    
     m_retransp_to_litter_fire           =>    phosphorusflux_vars%m_retransp_to_litter_fire_patch           
     m_ppool_to_litter_fire              =>    phosphorusflux_vars%m_pool_to_litter_fire_patch
