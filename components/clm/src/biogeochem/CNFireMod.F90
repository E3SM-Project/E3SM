     ...
     do pi = 1,max_patch_per_col
        do fc = 1,num_soilc
           ...
           if (pi <=  col_pp%npfts(c)) then
              ...
              ! For crop veg types
              if( pft_pp%itype(p) > nc4_grass )then
                 cropf_col(c) = cropf_col(c) + pft_pp%wtcol(p)
              end if
              ! For natural vegetation (non-crop and non-bare-soil)
              if( pft_pp%itype(p) >= ndllf_evr_tmp_tree .and. pft_pp%itype(p) <= nc4_grass )then
                 lfwt(c) = lfwt(c) + pft_pp%wtcol(p)
              end if
           end if
        end do
     end do
     ...
     do pi = 1,max_patch_per_col
        do fc = 1,num_soilc
           ...
           if (pi <=  col_pp%npfts(c)) then
              ...
              if( pft_pp%itype(p) > nc4_grass .and. pft_pp%wtcol(p) > 0._r8 .and. leafc_col(c) > 0._r8 )then
                 fuelc_crop(c)=fuelc_crop(c) + (leafc(p) + leafc_storage(p) + &
                      leafc_xfer(p))*pft_pp%wtcol(p)/cropf_col(c)     + &
                      totlitc(c)*leafc(p)/leafc_col(c)*pft_pp%wtcol(p)/cropf_col(c)
              end if
           end if
        end do
     end do
     ...
     do pi = 1,max_patch_per_col
        do fc = 1,num_soilc
           ...
           if (pi <=  col_pp%npfts(c)) then
              ...
              if( pft_pp%itype(p)  <  nc3crop .and. cropf_col(c)  <  1.0_r8 )then
                 if( .not. shr_infnan_isnan(btran2(p))) then
                    if (btran2(p)  <=  1._r8 ) then
                       btran_col(c) = btran_col(c)+btran2(p)*pft_pp%wtcol(p)
                       wtlf(c)      = wtlf(c)+pft_pp%wtcol(p)
                    end if
                 end if
                 if( pft_pp%itype(p) == nbrdlf_evr_trp_tree .and. pft_pp%wtcol(p)  >  0._r8 )then
                    trotr1_col(c)=trotr1_col(c)+pft_pp%wtcol(p)*col_pp%wtgcell(c)
                 end if
                 if( pft_pp%itype(p) == nbrdlf_dcd_trp_tree .and. pft_pp%wtcol(p)  >  0._r8 )then
                    trotr2_col(c)=trotr2_col(c)+pft_pp%wtcol(p)*col_pp%wtgcell(c)
                 end if
                 if ( flanduse_timeseries /= ' ' ) then    !true when landuse data is used
                    if( pft_pp%itype(p) == nbrdlf_evr_trp_tree .or. pft_pp%itype(p) == nbrdlf_dcd_trp_tree )then
                 ...
                 rootc_col(c) = rootc_col(c) + (frootc(p) + frootc_storage(p) + &
                      ...
                      livecrootc_xfer(p))*pft_pp%wtcol(p)

                 fsr_col(c) = fsr_col(c) + fsr_pft(pft_pp%itype(p))*pft_pp%wtcol(p)/(1.0_r8-cropf_col(c))
                 ...
                 if( lfwt(c)  /=  0.0_r8 )then    
                    ...
                    if( hdmlf  >  0.1_r8 )then            
                       ...
                       if( pft_pp%itype(p)  /=  noveg )then
                          ...
                          if( pft_pp%itype(p)  >=  nbrdlf_evr_shrub )then      !for shurb and grass
                             lgdp_col(c)  = lgdp_col(c) + (0.1_r8 + 0.9_r8*    &
                                  exp(-1._r8*SHR_CONST_PI* &
                                  (gdp_lf(c)/8._r8)**0.5_r8))*pft_pp%wtcol(p) &
                                  /(1.0_r8 - cropf_col(c))
                             lgdp1_col(c) = lgdp1_col(c) + (0.2_r8 + 0.8_r8*   &
                                  exp(-1._r8*SHR_CONST_PI* &
                                  (gdp_lf(c)/7._r8)))*pft_pp%wtcol(p)/lfwt(c)
                             lpop_col(c)  = lpop_col(c) + (0.2_r8 + 0.8_r8*    &
                                  exp(-1._r8*SHR_CONST_PI* &
                                  (hdmlf/450._r8)**0.5_r8))*pft_pp%wtcol(p)/lfwt(c)
                          else   ! for trees
                             if( gdp_lf(c)  >  20._r8 )then
                                lgdp_col(c)  =lgdp_col(c)+0.39_r8*pft_pp%wtcol(p)/(1.0_r8 - cropf_col(c))
                             else    
                                lgdp_col(c) = lgdp_col(c)+pft_pp%wtcol(p)/(1.0_r8 - cropf_col(c))
                             end if
                             if( gdp_lf(c)  >  20._r8 )then   
                                lgdp1_col(c) = lgdp1_col(c)+0.62_r8*pft_pp%wtcol(p)/lfwt(c)
                             else
                                if( gdp_lf(c)  >  8._r8 ) then
                                   lgdp1_col(c)=lgdp1_col(c)+0.83_r8*pft_pp%wtcol(p)/lfwt(c)
                                else
                                   lgdp1_col(c)=lgdp1_col(c)+pft_pp%wtcol(p)/lfwt(c)
                                end if
                             end if
                             lpop_col(c) = lpop_col(c) + (0.4_r8 + 0.6_r8*    &
                                  exp(-1._r8*SHR_CONST_PI* &
                                  (hdmlf/125._r8)))*pft_pp%wtcol(p)/lfwt(c) 
                          end if
                       end if
                    else
                       lgdp_col(c)  = lgdp_col(c)+pft_pp%wtcol(p)/(1.0_r8 - cropf_col(c))
                       lgdp1_col(c) = lgdp1_col(c)+pft_pp%wtcol(p)/lfwt(c)
                       lpop_col(c)  = lpop_col(c)+pft_pp%wtcol(p)/lfwt(c)
                    end if
                 end if

                 fd_col(c) = fd_col(c) + fd_pft(pft_pp%itype(p)) * pft_pp%wtcol(p) * secsphr / (1.0_r8-cropf_col(c))         
              end if
           end if
        end do
     end do
     ...
     do pi = 1,max_patch_per_col
        do fc = 1,num_soilc
           ...
           if (pi <=  col_pp%npfts(c)) then
              p = col_pp%pfti(c) + pi - 1
              ! For crop
              if( forc_t(c)  >=  SHR_CONST_TKFRZ .and. pft_pp%itype(p)  >  nc4_grass .and.  &
                   kmo == abm_lf(c) .and. forc_rain(c)+forc_snow(c) == 0._r8  .and. &
                   burndate(p) >= 999 .and. pft_pp%wtcol(p)  >  0._r8 )then ! catch  crop burn time
                 ...
                 baf_crop(c) = baf_crop(c) + cropfire_a1/secsphr*fb*fhd*fgdp*pft_pp%wtcol(p)
                 if( fb*fhd*fgdp*pft_pp%wtcol(p)  >  0._r8)then
                    burndate(p)=kda
                 end if
              end if
           end if
 ...
 subroutine CNFireFluxes (num_soilc, filter_soilc, num_soilp, filter_soilp, &
   ...
   associate(                                                                                                   & 
	...
	m_veg = 1.0_r8
        ...
        m_leafc_to_fire(p)               =  leafc(p)              * f * cc_leaf(pft_pp%itype(p))
        m_leafc_storage_to_fire(p)       =  leafc_storage(p)      * f * cc_other(pft_pp%itype(p))
        m_leafc_xfer_to_fire(p)          =  leafc_xfer(p)         * f * cc_other(pft_pp%itype(p))
        m_livestemc_to_fire(p)           =  livestemc(p)          * f * cc_lstem(pft_pp%itype(p))
        m_livestemc_storage_to_fire(p)   =  livestemc_storage(p)  * f * cc_other(pft_pp%itype(p))
        m_livestemc_xfer_to_fire(p)      =  livestemc_xfer(p)     * f * cc_other(pft_pp%itype(p))
        m_deadstemc_to_fire(p)           =  deadstemc(p)          * m_veg * f * cc_dstem(pft_pp%itype(p))
        m_deadstemc_storage_to_fire(p)   =  deadstemc_storage(p)  * f * cc_other(pft_pp%itype(p))
        m_deadstemc_xfer_to_fire(p)      =  deadstemc_xfer(p)     * f * cc_other(pft_pp%itype(p))
        m_frootc_to_fire(p)              =  frootc(p)             * f * 0._r8
        m_frootc_storage_to_fire(p)      =  frootc_storage(p)     * f * cc_other(pft_pp%itype(p)) 
        m_frootc_xfer_to_fire(p)         =  frootc_xfer(p)        * f * cc_other(pft_pp%itype(p))
        m_livecrootc_to_fire(p)          =  livecrootc(p)         * f * 0._r8
        m_livecrootc_storage_to_fire(p)  =  livecrootc_storage(p) * f * cc_other(pft_pp%itype(p)) 
        m_livecrootc_xfer_to_fire(p)     =  livecrootc_xfer(p)    * f * cc_other(pft_pp%itype(p)) 
        m_deadcrootc_to_fire(p)          =  deadcrootc(p)         * m_veg * f * 0._r8
        m_deadcrootc_storage_to_fire(p)  =  deadcrootc_storage(p) * f*  cc_other(pft_pp%itype(p)) 
        m_deadcrootc_xfer_to_fire(p)     =  deadcrootc_xfer(p)    * f * cc_other(pft_pp%itype(p)) 
        m_gresp_storage_to_fire(p)       =  gresp_storage(p)      * f * cc_other(pft_pp%itype(p))
        m_gresp_xfer_to_fire(p)          =  gresp_xfer(p)         * f * cc_other(pft_pp%itype(p))


        ! nitrogen fluxes
        m_leafn_to_fire(p)               =  leafn(p)              * f * cc_leaf(pft_pp%itype(p))
        m_leafn_storage_to_fire(p)       =  leafn_storage(p)      * f * cc_other(pft_pp%itype(p))
        m_leafn_xfer_to_fire(p)          =  leafn_xfer(p)         * f * cc_other(pft_pp%itype(p))
        m_livestemn_to_fire(p)           =  livestemn(p)          * f * cc_lstem(pft_pp%itype(p))
        m_livestemn_storage_to_fire(p)   =  livestemn_storage(p)  * f * cc_other(pft_pp%itype(p))
        m_livestemn_xfer_to_fire(p)      =  livestemn_xfer(p)     * f * cc_other(pft_pp%itype(p))
        m_deadstemn_to_fire(p)           =  deadstemn(p)          * m_veg * f * cc_dstem(pft_pp%itype(p))
        m_deadstemn_storage_to_fire(p)   =  deadstemn_storage(p)  * f * cc_other(pft_pp%itype(p))
        m_deadstemn_xfer_to_fire(p)      =  deadstemn_xfer(p)     * f * cc_other(pft_pp%itype(p))
        m_frootn_to_fire(p)              =  frootn(p)             * f * 0._r8
        m_frootn_storage_to_fire(p)      =  frootn_storage(p)     * f * cc_other(pft_pp%itype(p))
        m_frootn_xfer_to_fire(p)         =  frootn_xfer(p)        * f * cc_other(pft_pp%itype(p))
        m_livecrootn_to_fire(p)          =  livecrootn(p)         * f * 0._r8 
        m_livecrootn_storage_to_fire(p)  =  livecrootn_storage(p) * f * cc_other(pft_pp%itype(p)) 
        m_livecrootn_xfer_to_fire(p)     =  livecrootn_xfer(p)    * f * cc_other(pft_pp%itype(p))
        m_deadcrootn_to_fire(p)          =  deadcrootn(p)         * m_veg * f * 0._r8
        m_deadcrootn_xfer_to_fire(p)     =  deadcrootn_xfer(p)    * f * cc_other(pft_pp%itype(p)) 
        m_deadcrootn_storage_to_fire(p)  =  deadcrootn_storage(p) * f * cc_other(pft_pp%itype(p))
        m_retransn_to_fire(p)            =  retransn(p)           * f * cc_other(pft_pp%itype(p))

        ! phosphorus fluxes
        m_leafp_to_fire(p)               =  leafp(p)              * f * cc_leaf(pft_pp%itype(p))
        m_leafp_storage_to_fire(p)       =  leafp_storage(p)      * f * cc_other(pft_pp%itype(p))
        m_leafp_xfer_to_fire(p)          =  leafp_xfer(p)         * f * cc_other(pft_pp%itype(p))
        m_livestemp_to_fire(p)           =  livestemp(p)          * f * cc_lstem(pft_pp%itype(p))
        m_livestemp_storage_to_fire(p)   =  livestemp_storage(p)  * f * cc_other(pft_pp%itype(p))
        m_livestemp_xfer_to_fire(p)      =  livestemp_xfer(p)     * f * cc_other(pft_pp%itype(p))
        m_deadstemp_to_fire(p)           =  deadstemp(p)          * f * cc_dstem(pft_pp%itype(p))
        m_deadstemp_storage_to_fire(p)   =  deadstemp_storage(p)  * f * cc_other(pft_pp%itype(p))
        m_deadstemp_xfer_to_fire(p)      =  deadstemp_xfer(p)     * f * cc_other(pft_pp%itype(p))
        m_frootp_to_fire(p)              =  frootp(p)             * f * 0._r8
        m_frootp_storage_to_fire(p)      =  frootp_storage(p)     * f * cc_other(pft_pp%itype(p))
        m_frootp_xfer_to_fire(p)         =  frootp_xfer(p)        * f * cc_other(pft_pp%itype(p))
        m_livecrootp_to_fire(p)          =  livecrootp(p)         * f * 0._r8 
        m_livecrootp_storage_to_fire(p)  =  livecrootp_storage(p) * f * cc_other(pft_pp%itype(p)) 
        m_livecrootp_xfer_to_fire(p)     =  livecrootp_xfer(p)    * f * cc_other(pft_pp%itype(p))
        m_deadcrootp_to_fire(p)          =  deadcrootp(p)         * f * 0._r8
        m_deadcrootp_xfer_to_fire(p)     =  deadcrootp_xfer(p)    * f * cc_other(pft_pp%itype(p)) 
        m_deadcrootp_storage_to_fire(p)  =  deadcrootp_storage(p) * f * cc_other(pft_pp%itype(p))
        m_retransp_to_fire(p)            =  retransp(p)           * f * cc_other(pft_pp%itype(p))

        ! mortality due to fire
        ! carbon pools
        m_leafc_to_litter_fire(p)                   =  leafc(p) * f * &
             (1._r8 - cc_leaf(pft_pp%itype(p))) * &
             fm_leaf(pft_pp%itype(p))
        m_leafc_storage_to_litter_fire(p)           =  leafc_storage(p) * f * &
             (1._r8 - cc_other(pft_pp%itype(p))) * &
             fm_other(pft_pp%itype(p))
        m_leafc_xfer_to_litter_fire(p)              =  leafc_xfer(p) * f * &
             (1._r8 - cc_other(pft_pp%itype(p))) * &
             fm_other(pft_pp%itype(p))
        m_livestemc_to_litter_fire(p)               =  livestemc(p) * f * &
             (1._r8 - cc_lstem(pft_pp%itype(p))) * &
             fm_droot(pft_pp%itype(p))    
        ...
        m_livestemc_to_deadstemc_fire(p)            =  livestemc(p) * f * &
             (1._r8 - cc_lstem(pft_pp%itype(p))) * &
             (fm_lstem(pft_pp%itype(p))-fm_droot(pft_pp%itype(p)))
        m_deadstemc_to_litter_fire(p)               =  deadstemc(p) * m_veg * f * &
             (1._r8 - cc_dstem(pft_pp%itype(p))) * &
             fm_droot(pft_pp%itype(p))    
        ...
        m_livecrootc_to_deadcrootc_fire(p)          =  livecrootc(p)         * f * &
             (fm_lroot(pft_pp%itype(p))-fm_droot(pft_pp%itype(p)))
        ...
        if (use_cndv) then
           if ( woody(pft_pp%itype(p)) == 1._r8 )then
              if ( livestemc(p)+deadstemc(p) > 0._r8 )then
                 nind(p) = nind(p)*(1._r8-1._r8*fm_droot(pft_pp%itype(p))*f) 
              else
                 nind(p) = 0._r8
              end if
           end if
           leafcmax(p) = max(leafc(p)-m_leafc_to_fire(p)*dt, leafcmax(p))
           if (pft_pp%itype(p) == noveg) leafcmax(p) = 0._r8
        end if
              ...
              if (pi <=  col_pp%npfts(c)) then
                 ...
                 if ( pft_pp%active(p) ) then
                    ...
                    fire_mortality_c_to_cwdc(c,j) = fire_mortality_c_to_cwdc(c,j) + &
                         m_deadstemc_to_litter_fire(p) * pft_pp%wtcol(p) * stem_prof(p,j)
                    fire_mortality_c_to_cwdc(c,j) = fire_mortality_c_to_cwdc(c,j) + &
                         m_deadcrootc_to_litter_fire(p) * pft_pp%wtcol(p) * croot_prof(p,j)
                    fire_mortality_n_to_cwdn(c,j) = fire_mortality_n_to_cwdn(c,j) + &
                         m_deadstemn_to_litter_fire(p) * pft_pp%wtcol(p) * stem_prof(p,j)
                    fire_mortality_n_to_cwdn(c,j) = fire_mortality_n_to_cwdn(c,j) + &
                         m_deadcrootn_to_litter_fire(p) * pft_pp%wtcol(p) * croot_prof(p,j)
                    ! add phosphorus
                    fire_mortality_p_to_cwdp(c,j) = fire_mortality_p_to_cwdp(c,j) + &
                         m_deadstemp_to_litter_fire(p) * pft_pp%wtcol(p) * stem_prof(p,j)
                    fire_mortality_p_to_cwdp(c,j) = fire_mortality_p_to_cwdp(c,j) + &
                         m_deadcrootp_to_litter_fire(p) * pft_pp%wtcol(p) * croot_prof(p,j)


                    fire_mortality_c_to_cwdc(c,j) = fire_mortality_c_to_cwdc(c,j) + &
                         m_livestemc_to_litter_fire(p) * pft_pp%wtcol(p) * stem_prof(p,j)
                    fire_mortality_c_to_cwdc(c,j) = fire_mortality_c_to_cwdc(c,j) + &
                         m_livecrootc_to_litter_fire(p) * pft_pp%wtcol(p) * croot_prof(p,j)
                    fire_mortality_n_to_cwdn(c,j) = fire_mortality_n_to_cwdn(c,j) + &
                         m_livestemn_to_litter_fire(p) * pft_pp%wtcol(p) * stem_prof(p,j)
                    fire_mortality_n_to_cwdn(c,j) = fire_mortality_n_to_cwdn(c,j) + &
                         m_livecrootn_to_litter_fire(p) * pft_pp%wtcol(p) * croot_prof(p,j)
                    ! add phosphorus
                    fire_mortality_p_to_cwdp(c,j) = fire_mortality_p_to_cwdp(c,j) + &
                         m_livestemp_to_litter_fire(p) * pft_pp%wtcol(p) * stem_prof(p,j)
                    fire_mortality_p_to_cwdp(c,j) = fire_mortality_p_to_cwdp(c,j) + &
                         m_livecrootp_to_litter_fire(p) * pft_pp%wtcol(p) * croot_prof(p,j)


                    m_c_to_litr_met_fire(c,j)=m_c_to_litr_met_fire(c,j) + &
                         ((m_leafc_to_litter_fire(p)*lf_flab(pft_pp%itype(p)) &
                         ...
                         (m_frootc_to_litter_fire(p)*fr_flab(pft_pp%itype(p)) &
                         ...
                         m_deadcrootc_xfer_to_litter_fire(p))* croot_prof(p,j))* pft_pp%wtcol(p)    
                    m_c_to_litr_cel_fire(c,j)=m_c_to_litr_cel_fire(c,j) + &
                         (m_leafc_to_litter_fire(p)*lf_fcel(pft_pp%itype(p))*leaf_prof(p,j) + &
                         m_frootc_to_litter_fire(p)*fr_fcel(pft_pp%itype(p))*froot_prof(p,j))* pft_pp%wtcol(p) 
                    m_c_to_litr_lig_fire(c,j)=m_c_to_litr_lig_fire(c,j) + &
                         (m_leafc_to_litter_fire(p)*lf_flig(pft_pp%itype(p))*leaf_prof(p,j) + &
                         m_frootc_to_litter_fire(p)*fr_flig(pft_pp%itype(p))*froot_prof(p,j))* pft_pp%wtcol(p)  

                    m_n_to_litr_met_fire(c,j)=m_n_to_litr_met_fire(c,j) + &
                         ((m_leafn_to_litter_fire(p)*lf_flab(pft_pp%itype(p)) &
                         +m_leafn_storage_to_litter_fire(p) + &
                         m_leafn_xfer_to_litter_fire(p)+m_retransn_to_litter_fire(p)) &
                         *leaf_prof(p,j) +(m_frootn_to_litter_fire(p)*fr_flab(pft_pp%itype(p)) &
                         ...
                         m_deadcrootn_xfer_to_litter_fire(p))* croot_prof(p,j))* pft_pp%wtcol(p)    
                    m_n_to_litr_cel_fire(c,j)=m_n_to_litr_cel_fire(c,j) + &
                         (m_leafn_to_litter_fire(p)*lf_fcel(pft_pp%itype(p))*leaf_prof(p,j) + &
                         m_frootn_to_litter_fire(p)*fr_fcel(pft_pp%itype(p))*froot_prof(p,j))* pft_pp%wtcol(p) 
                    m_n_to_litr_lig_fire(c,j)=m_n_to_litr_lig_fire(c,j) + &
                         (m_leafn_to_litter_fire(p)*lf_flig(pft_pp%itype(p))*leaf_prof(p,j) + &
                         m_frootn_to_litter_fire(p)*fr_flig(pft_pp%itype(p))*froot_prof(p,j))* pft_pp%wtcol(p) 

                    ! add phosphorus
                    m_p_to_litr_met_fire(c,j)=m_p_to_litr_met_fire(c,j) + &
                         ((m_leafp_to_litter_fire(p)*lf_flab(pft_pp%itype(p)) &
                         +m_leafp_storage_to_litter_fire(p) + &
                         m_leafp_xfer_to_litter_fire(p)+m_retransp_to_litter_fire(p)) &
                         *leaf_prof(p,j) +(m_frootp_to_litter_fire(p)*fr_flab(pft_pp%itype(p)) &
                         ...
                         m_deadcrootp_xfer_to_litter_fire(p))* croot_prof(p,j))* pft_pp%wtcol(p)    
                    m_p_to_litr_cel_fire(c,j)=m_p_to_litr_cel_fire(c,j) + &
                         (m_leafp_to_litter_fire(p)*lf_fcel(pft_pp%itype(p))*leaf_prof(p,j) + &
                         m_frootp_to_litter_fire(p)*fr_fcel(pft_pp%itype(p))*froot_prof(p,j))* pft_pp%wtcol(p) 
                    m_p_to_litr_lig_fire(c,j)=m_p_to_litr_lig_fire(c,j) + &
                         (m_leafp_to_litter_fire(p)*lf_flig(pft_pp%itype(p))*leaf_prof(p,j) + &
                         m_frootp_to_litter_fire(p)*fr_flig(pft_pp%itype(p))*froot_prof(p,j))* pft_pp%wtcol(p) 
