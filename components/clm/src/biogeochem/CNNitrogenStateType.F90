    ...
    do p = bounds%begp,bounds%endp
       ...
       if (lun_pp%itype(l) == istsoil .or. lun_pp%itype(l) == istcrop) then       
          if (pft_pp%itype(p) == noveg) then
             ...
             this%leafn_patch(p)         = leafc_patch(p)         / veg_vp%leafcn(pft_pp%itype(p))
             this%leafn_storage_patch(p) = leafc_storage_patch(p) / veg_vp%leafcn(pft_pp%itype(p))
          end if
          ...
          if (veg_vp%woody(pft_pp%itype(p)) == 1._r8) then
             this%deadstemn_patch(p) = deadstemc_patch(p) / veg_vp%deadwdcn(pft_pp%itype(p))
          else
          ...
          if (nu_com .ne. 'RD') then
              ...
              if (pft_pp%itype(p) .ne. noveg) then
                 this%frootn_patch(p) = frootc_patch(p) / veg_vp%frootcn(pft_pp%itype(p))
                 this%frootn_storage_patch(p) = frootc_storage_patch(p) / veg_vp%frootcn(pft_pp%itype(p))
              end if
          end if
