  ...
  subroutine NStateUpdate2(num_soilc, filter_soilc, num_soilp, filter_soilp, &
    ...
    associate(                      & 
         ...
         do j = 1, nlevdecomp
            do fc = 1,num_soilc
               ...
               ns%decomp_pools_vr_col(c,j,i_met_lit) = &
                    ns%decomp_pools_vr_col(c,j,i_met_lit) + nf%gap_mortality_n_to_litr_met_n_col(c,j) * dt
               ns%decomp_pools_vr_col(c,j,i_cel_lit) = &
                    ns%decomp_pools_vr_col(c,j,i_cel_lit) + nf%gap_mortality_n_to_litr_cel_n_col(c,j) * dt
               ns%decomp_pools_vr_col(c,j,i_lig_lit) = &
                    ns%decomp_pools_vr_col(c,j,i_lig_lit) + nf%gap_mortality_n_to_litr_lig_n_col(c,j) * dt
               ns%decomp_pools_vr_col(c,j,i_cwd)     = &
                    ns%decomp_pools_vr_col(c,j,i_cwd)     + nf%gap_mortality_n_to_cwdn_col(c,j)       * dt
            end do
         end do
      ...
      do fp = 1,num_soilp
         ...
         ns%pool_patch(p)              =  ns%pool_patch(p)      - nf%m_npool_to_litter_patch(p)      * dt
  ...
  subroutine NStateUpdate2h(num_soilc, filter_soilc, num_soilp, filter_soilp, &
    ...
    associate(                      & 
      ...
      if (.not. is_active_betr_bgc .and. &
         ...
         do j = 1,nlevdecomp
            do fc = 1,num_soilc
               c = filter_soilc(fc)
               ns%decomp_pools_vr_col(c,j,i_met_lit) = &
                    ns%decomp_pools_vr_col(c,j,i_met_lit) + nf%harvest_n_to_litr_met_n_col(c,j) * dt
               ns%decomp_pools_vr_col(c,j,i_cel_lit) = &
                    ns%decomp_pools_vr_col(c,j,i_cel_lit) + nf%harvest_n_to_litr_cel_n_col(c,j) * dt
               ns%decomp_pools_vr_col(c,j,i_lig_lit) = &
                    ns%decomp_pools_vr_col(c,j,i_lig_lit) + nf%harvest_n_to_litr_lig_n_col(c,j) * dt
               ns%decomp_pools_vr_col(c,j,i_cwd)     = &
                    ns%decomp_pools_vr_col(c,j,i_cwd)     + nf%harvest_n_to_cwdn_col(c,j)       * dt
            end do
         end do
      ...
      do fp = 1,num_soilp
         ...
         ns%pool_patch(p)      = ns%pool_patch(p)      - nf%hrv_npool_to_litter_patch(p)     * dt
