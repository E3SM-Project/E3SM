module CNCarbonFluxType
  ...
  type, public :: carbonflux_type
    ...
    !------------------------------------------------------------------------
     ...
     !array for offline litter input flux
  ...
  subroutine InitHistory(this, bounds, carbon_type)
    ...
    if (carbon_type == 'c12') then
       ...
       if ( nlevdecomp_full > 1 ) then
          ...
          this%rr_vr_col(begc:endc,:)=spval
          call hist_addfld2d (fname='RR_vr', units='gC/m^3/s', type2d='levdcmp', &
               avgflag='A', long_name='vertically resolved root respiration', &
               ptr_col=this%rr_vr_col, default='inactive')
    ...
    do j = 1,nlev
      do fc = 1,num_soilc
        ...
        this%cflx_input_litr_met_vr_col(c,j)=              &
         this%phenology_c_to_litr_met_c_col(c,j)         + & !phenology
         this%dwt_frootc_to_litr_met_c_col(c,j)          + & !dynamic land cover
         this%gap_mortality_c_to_litr_met_c_col(c,j)     + & !gap mortality
         this%harvest_c_to_litr_met_c_col(c,j)           + & !harvest
         this%m_c_to_litr_met_fire_col(c,j)              ! fire mortality

      !cellulose carbon
      this%cflx_input_litr_cel_vr_col(c,j) =                   &
         this%phenology_c_to_litr_cel_c_col(c,j)            +  &  !phenology
         this%dwt_frootc_to_litr_cel_c_col(c,j)             +  &  !dynamic land cover
         this%gap_mortality_c_to_litr_cel_c_col(c,j)        + & !gap mortality
         this%harvest_c_to_litr_cel_c_col(c,j)              + & !harvest
         this%m_c_to_litr_cel_fire_col(c,j)               ! fire mortality

      !lignin carbon
      this%cflx_input_litr_lig_vr_col(c,j) =              &
         this%phenology_c_to_litr_lig_c_col(c,j)          + & !phenology
         this%dwt_frootc_to_litr_lig_c_col(c,j)           + & !dynamic land cover
         this%gap_mortality_c_to_litr_lig_c_col(c,j)      + & !gap mortality
         this%harvest_c_to_litr_lig_c_col(c,j)            + & !harvest
         this%m_c_to_litr_lig_fire_col(c,j)                 ! fire mortality
