    ...
    associate(& 
      ...
      do j = 1, nlevdecomp
         ...
         do fc = 1,num_soilc
            ...
            ps%decomp_pools_vr_col(c,j,i_cwd) = ps%decomp_pools_vr_col(c,j,i_cwd) + pf%fire_mortality_to_cwd_col(c,j) * dt
      ...
      do fp = 1,num_soilp
         ...
         ps%leaf_patch(p)              =  ps%leaf_patch(p)      - pf%m_leaf_to_fire_patch(p)      * dt
         ps%froot_patch(p)             =  ps%froot_patch(p)     - pf%m_froot_to_fire_patch(p)     * dt
         ps%livestem_patch(p)          =  ps%livestem_patch(p)  - pf%m_livestem_to_fire_patch(p)  * dt
         ps%deadstem_patch(p)          =  ps%deadstem_patch(p)  - pf%m_deadstem_to_fire_patch(p)  * dt
         ps%livecroot_patch(p)         =  ps%livecroot_patch(p) - pf%m_livecroot_to_fire_patch(p) * dt
         ps%deadcroot_patch(p)         =  ps%deadcroot_patch(p) - pf%m_deadcroot_to_fire_patch(p) * dt

         ps%leaf_patch(p)              =  ps%leaf_patch(p)      - pf%m_leaf_to_litter_fire_patch(p)           * dt
         ps%froot_patch(p)             =  ps%froot_patch(p)     - pf%m_froot_to_litter_fire_patch(p)          * dt
         ps%livestem_patch(p)          =  ps%livestem_patch(p)  - pf%m_livestem_to_litter_fire_patch(p)       * dt
         ps%deadstem_patch(p)          =  ps%deadstem_patch(p)  - pf%m_deadstem_to_litter_fire_patch(p)       * dt
         ps%livecroot_patch(p)         =  ps%livecroot_patch(p) - pf%m_livecroot_to_litter_fire_patch(p)      * dt
         ps%deadcroot_patch(p)         =  ps%deadcroot_patch(p) - pf%m_deadcroot_to_litter_fire_patch(p)      * dt

         ! storage pools
         ps%leaf_storage_patch(p)      =  ps%leaf_storage_patch(p)      - pf%m_leaf_storage_to_fire_patch(p)      * dt
         ps%froot_storage_patch(p)     =  ps%froot_storage_patch(p)     - pf%m_froot_storage_to_fire_patch(p)     * dt
         ps%livestem_storage_patch(p)  =  ps%livestem_storage_patch(p)  - pf%m_livestem_storage_to_fire_patch(p)  * dt
         ps%deadstem_storage_patch(p)  =  ps%deadstem_storage_patch(p)  - pf%m_deadstem_storage_to_fire_patch(p)  * dt
         ps%livecroot_storage_patch(p) =  ps%livecroot_storage_patch(p) - pf%m_livecroot_storage_to_fire_patch(p) * dt
         ps%deadcroot_storage_patch(p) =  ps%deadcroot_storage_patch(p) - pf%m_deadcroot_storage_to_fire_patch(p) * dt

         ps%leaf_storage_patch(p)      =  ps%leaf_storage_patch(p)      - pf%m_leaf_storage_to_litter_fire_patch(p)      * dt
         ps%froot_storage_patch(p)     =  ps%froot_storage_patch(p)     - pf%m_froot_storage_to_litter_fire_patch(p)     * dt
         ps%livestem_storage_patch(p)  =  ps%livestem_storage_patch(p)  - pf%m_livestem_storage_to_litter_fire_patch(p)  * dt
         ps%deadstem_storage_patch(p)  =  ps%deadstem_storage_patch(p)  - pf%m_deadstem_storage_to_litter_fire_patch(p)  * dt
         ps%livecroot_storage_patch(p) =  ps%livecroot_storage_patch(p) - pf%m_livecroot_storage_to_litter_fire_patch(p) * dt
         ps%deadcroot_storage_patch(p) =  ps%deadcroot_storage_patch(p) - pf%m_deadcroot_storage_to_litter_fire_patch(p) * dt


         ! trapsfer pools
         ps%leaf_xfer_patch(p)         =  ps%leaf_xfer_patch(p)      - pf%m_leaf_xfer_to_fire_patch(p)      * dt
         ps%froot_xfer_patch(p)        =  ps%froot_xfer_patch(p)     - pf%m_froot_xfer_to_fire_patch(p)     * dt
         ps%livestem_xfer_patch(p)     =  ps%livestem_xfer_patch(p)  - pf%m_livestem_xfer_to_fire_patch(p)  * dt
         ps%deadstem_xfer_patch(p)     =  ps%deadstem_xfer_patch(p)  - pf%m_deadstem_xfer_to_fire_patch(p)  * dt
         ps%livecroot_xfer_patch(p)    =  ps%livecroot_xfer_patch(p) - pf%m_livecroot_xfer_to_fire_patch(p) * dt
         ps%deadcroot_xfer_patch(p)    =  ps%deadcroot_xfer_patch(p) - pf%m_deadcroot_xfer_to_fire_patch(p) * dt

         ps%leaf_xfer_patch(p)         =  ps%leaf_xfer_patch(p)      - pf%m_leaf_xfer_to_litter_fire_patch(p)      * dt
         ps%froot_xfer_patch(p)        =  ps%froot_xfer_patch(p)     - pf%m_froot_xfer_to_litter_fire_patch(p)     * dt
         ps%livestem_xfer_patch(p)     =  ps%livestem_xfer_patch(p)  - pf%m_livestem_xfer_to_litter_fire_patch(p)  * dt
         ps%deadstem_xfer_patch(p)     =  ps%deadstem_xfer_patch(p)  - pf%m_deadstem_xfer_to_litter_fire_patch(p)  * dt
         ps%livecroot_xfer_patch(p)    =  ps%livecroot_xfer_patch(p) - pf%m_livecroot_xfer_to_litter_fire_patch(p) * dt
         ps%deadcroot_xfer_patch(p)    =  ps%deadcroot_xfer_patch(p) - pf%m_deadcroot_xfer_to_litter_fire_patch(p) * dt
         ...
         ps%pool_patch(p)              =  ps%pool_patch(p) - pf%m_pool_to_fire_patch(p)              * dt
         ps%pool_patch(p)              =  ps%pool_patch(p) - pf%m_pool_to_litter_fire_patch(p)       * dt
      end do
