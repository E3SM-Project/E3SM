    ...
    associate(& 
          ...
          do j = 1, nlevdecomp
             do fc = 1,num_soilc
                ...
                do l = 1, ndecomp_pools
                   ...
                   ps%decomp_pools_vr_col(c,j,l) = ps%decomp_pools_vr_col(c,j,l)- pf%biochem_pmin_ppools_vr_col(c,j,l)*dt
      ...
      do j = 1, nlevdecomp
         ...
         do fc = 1,num_soilc
            ...
            ps%decomp_pools_vr_col(c,j,i_cwd) = ps%decomp_pools_vr_col(c,j,i_cwd) + pf%fire_mortality_p_to_cwdp_col(c,j) * dt

            ! pft-level wood to column-level litter (uncombusted wood)
            ps%decomp_pools_vr_col(c,j,i_met_lit) = ps%decomp_pools_vr_col(c,j,i_met_lit) + pf%m_p_to_litr_met_fire_col(c,j)* dt
            ps%decomp_pools_vr_col(c,j,i_cel_lit) = ps%decomp_pools_vr_col(c,j,i_cel_lit) + pf%m_p_to_litr_cel_fire_col(c,j)* dt
            ps%decomp_pools_vr_col(c,j,i_lig_lit) = ps%decomp_pools_vr_col(c,j,i_lig_lit) + pf%m_p_to_litr_lig_fire_col(c,j)* dt
      ...
      do l = 1, ndecomp_pools
         do j = 1, nlevdecomp
            ...
            do fc = 1,num_soilc
               c = filter_soilc(fc)
               ps%decomp_pools_vr_col(c,j,l) = ps%decomp_pools_vr_col(c,j,l) - pf%m_decomp_ppools_to_fire_vr_col(c,j,l) * dt
            end do
         end do
      end do
      ...
      do fp = 1,num_soilp
         ...
         ps%leaf_patch(p)              =  ps%leaf_patch(p)      - pf%m_leafp_to_fire_patch(p)      * dt
         ps%froot_patch(p)             =  ps%froot_patch(p)     - pf%m_frootp_to_fire_patch(p)     * dt
         ps%livestem_patch(p)          =  ps%livestem_patch(p)  - pf%m_livestemp_to_fire_patch(p)  * dt
         ps%deadstem_patch(p)          =  ps%deadstem_patch(p)  - pf%m_deadstemp_to_fire_patch(p)  * dt
         ps%livecroot_patch(p)         =  ps%livecroot_patch(p) - pf%m_livecrootp_to_fire_patch(p) * dt
         ps%deadcroot_patch(p)         =  ps%deadcroot_patch(p) - pf%m_deadcrootp_to_fire_patch(p) * dt

         ps%leaf_patch(p)              =  ps%leaf_patch(p)      - pf%m_leafp_to_litter_fire_patch(p)           * dt
         ps%froot_patch(p)             =  ps%froot_patch(p)     - pf%m_frootp_to_litter_fire_patch(p)          * dt
         ps%livestem_patch(p)          =  ps%livestem_patch(p)  - pf%m_livestemp_to_litter_fire_patch(p)       * dt
         ps%deadstem_patch(p)          =  ps%deadstem_patch(p)  - pf%m_deadstemp_to_litter_fire_patch(p)       * dt
         ps%livecroot_patch(p)         =  ps%livecroot_patch(p) - pf%m_livecrootp_to_litter_fire_patch(p)      * dt
         ps%deadcroot_patch(p)         =  ps%deadcroot_patch(p) - pf%m_deadcrootp_to_litter_fire_patch(p)      * dt

         ! storage pools
         ps%leaf_storage_patch(p)      =  ps%leaf_storage_patch(p)      - pf%m_leafp_storage_to_fire_patch(p)      * dt
         ps%froot_storage_patch(p)     =  ps%froot_storage_patch(p)     - pf%m_frootp_storage_to_fire_patch(p)     * dt
         ps%livestem_storage_patch(p)  =  ps%livestem_storage_patch(p)  - pf%m_livestemp_storage_to_fire_patch(p)  * dt
         ps%deadstem_storage_patch(p)  =  ps%deadstem_storage_patch(p)  - pf%m_deadstemp_storage_to_fire_patch(p)  * dt
         ps%livecroot_storage_patch(p) =  ps%livecroot_storage_patch(p) - pf%m_livecrootp_storage_to_fire_patch(p) * dt
         ps%deadcroot_storage_patch(p) =  ps%deadcroot_storage_patch(p) - pf%m_deadcrootp_storage_to_fire_patch(p) * dt

         ps%leaf_storage_patch(p)      =  ps%leaf_storage_patch(p)      - pf%m_leafp_storage_to_litter_fire_patch(p)      * dt
         ps%froot_storage_patch(p)     =  ps%froot_storage_patch(p)     - pf%m_frootp_storage_to_litter_fire_patch(p)     * dt
         ps%livestem_storage_patch(p)  =  ps%livestem_storage_patch(p)  - pf%m_livestemp_storage_to_litter_fire_patch(p)  * dt
         ps%deadstem_storage_patch(p)  =  ps%deadstem_storage_patch(p)  - pf%m_deadstemp_storage_to_litter_fire_patch(p)  * dt
         ps%livecroot_storage_patch(p) =  ps%livecroot_storage_patch(p) - pf%m_livecrootp_storage_to_litter_fire_patch(p) * dt
         ps%deadcroot_storage_patch(p) =  ps%deadcroot_storage_patch(p) - pf%m_deadcrootp_storage_to_litter_fire_patch(p) * dt


         ! trapsfer pools
         ps%leaf_xfer_patch(p)         =  ps%leaf_xfer_patch(p)      - pf%m_leafp_xfer_to_fire_patch(p)      * dt
         ps%froot_xfer_patch(p)        =  ps%froot_xfer_patch(p)     - pf%m_frootp_xfer_to_fire_patch(p)     * dt
         ps%livestem_xfer_patch(p)     =  ps%livestem_xfer_patch(p)  - pf%m_livestemp_xfer_to_fire_patch(p)  * dt
         ps%deadstem_xfer_patch(p)     =  ps%deadstem_xfer_patch(p)  - pf%m_deadstemp_xfer_to_fire_patch(p)  * dt
         ps%livecroot_xfer_patch(p)    =  ps%livecroot_xfer_patch(p) - pf%m_livecrootp_xfer_to_fire_patch(p) * dt
         ps%deadcroot_xfer_patch(p)    =  ps%deadcroot_xfer_patch(p) - pf%m_deadcrootp_xfer_to_fire_patch(p) * dt

         ps%leaf_xfer_patch(p)         =  ps%leaf_xfer_patch(p)      - pf%m_leafp_xfer_to_litter_fire_patch(p)      * dt
         ps%froot_xfer_patch(p)        =  ps%froot_xfer_patch(p)     - pf%m_frootp_xfer_to_litter_fire_patch(p)     * dt
         ps%livestem_xfer_patch(p)     =  ps%livestem_xfer_patch(p)  - pf%m_livestemp_xfer_to_litter_fire_patch(p)  * dt
         ps%deadstem_xfer_patch(p)     =  ps%deadstem_xfer_patch(p)  - pf%m_deadstemp_xfer_to_litter_fire_patch(p)  * dt
         ps%livecroot_xfer_patch(p)    =  ps%livecroot_xfer_patch(p) - pf%m_livecrootp_xfer_to_litter_fire_patch(p) * dt
         ps%deadcroot_xfer_patch(p)    =  ps%deadcroot_xfer_patch(p) - pf%m_deadcrootp_xfer_to_litter_fire_patch(p) * dt
         ...
         ps%pool_patch(p)              =  ps%pool_patch(p) - pf%m_ppool_to_fire_patch(p)              * dt
         ps%pool_patch(p)              =  ps%pool_patch(p) - pf%m_ppool_to_litter_fire_patch(p)       * dt
      end do
