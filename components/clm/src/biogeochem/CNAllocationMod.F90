...
!    real(r8):: sminn_tot(bounds%begc:bounds%endc)
    ...
    associate(                                                                                   &
         ...
         leafc                        => carbonstate_vars%leaf_patch                          , & ! Input:  [real(r8) (:)   ]                                          
         frootc                       => carbonstate_vars%froot_patch                         , & ! Input:  [real(r8) (:)   ]                                          
         livestemc                    => carbonstate_vars%livestem_patch                      , & ! Input:  [real(r8) (:)   ]                                          
         ...
         plant_calloc                 => carbonflux_vars%plant_alloc_patch                    , & ! Output: [real(r8) (:)   ]  total allocated C flux (gC/m2/s)        
         ...
         cpool_to_leafc               => carbonflux_vars%pool_to_leaf_patch                  , & ! Output: [real(r8) (:)   ]                                          
         cpool_to_leafc_storage       => carbonflux_vars%pool_to_leaf_storage_patch          , & ! Output: [real(r8) (:)   ]                                          
         cpool_to_frootc              => carbonflux_vars%pool_to_froot_patch                 , & ! Output: [real(r8) (:)   ]                                          
         cpool_to_frootc_storage      => carbonflux_vars%pool_to_froot_storage_patch         , & ! Output: [real(r8) (:)   ]                                          
         cpool_to_livestemc           => carbonflux_vars%pool_to_livestem_patch              , & ! Output: [real(r8) (:)   ]                                          
         cpool_to_livestemc_storage   => carbonflux_vars%pool_to_livestem_storage_patch      , & ! Output: [real(r8) (:)   ]                                          
         cpool_to_deadstemc           => carbonflux_vars%pool_to_deadstem_patch              , & ! Output: [real(r8) (:)   ]                                          
         cpool_to_deadstemc_storage   => carbonflux_vars%pool_to_deadstem_storage_patch      , & ! Output: [real(r8) (:)   ]                                          
         cpool_to_livecrootc          => carbonflux_vars%pool_to_livecroot_patch             , & ! Output: [real(r8) (:)   ]                                          
         cpool_to_livecrootc_storage  => carbonflux_vars%pool_to_livecroot_storage_patch     , & ! Output: [real(r8) (:)   ]                                          
         cpool_to_deadcrootc          => carbonflux_vars%pool_to_deadcroot_patch             , & ! Output: [real(r8) (:)   ]                                          
         cpool_to_deadcrootc_storage  => carbonflux_vars%pool_to_deadcroot_storage_patch     , & ! Output: [real(r8) (:)   ]                                          
         ...
         cpool_to_grainc              => carbonflux_vars%pool_to_grain_patch                 , & ! Output: [real(r8) (:)   ]  allocation to grain C (gC/m2/s)         
         cpool_to_grainc_storage      => carbonflux_vars%pool_to_grain_storage_patch         , & ! Output: [real(r8) (:)   ]  allocation to grain C storage (gC/m2/s) 
         ...
         plant_nalloc                 => nitrogenflux_vars%plant_alloc_patch                  , & ! Output: [real(r8) (:)   ]  total allocated N flux (gN/m2/s)        
         avail_retransn               => nitrogenflux_vars%avail_retransn_patch                , & ! Output: [real(r8) (:)   ]  N flux available from retranslocation pool (gN/m2/s)
         npool_to_grainn              => nitrogenflux_vars%pool_to_grain_patch               , & ! Output: [real(r8) (:)   ]  allocation to grain N (gN/m2/s)         
         npool_to_grainn_storage      => nitrogenflux_vars%pool_to_grain_storage_patch       , & ! Output: [real(r8) (:)   ]  allocation to grain N storage (gN/m2/s) 
         ...
         npool_to_leafn               => nitrogenflux_vars%pool_to_leaf_patch                , & ! Output: [real(r8) (:)   ]  allocation to leaf N (gN/m2/s)          
         npool_to_leafn_storage       => nitrogenflux_vars%pool_to_leaf_storage_patch        , & ! Output: [real(r8) (:)   ]  allocation to leaf N storage (gN/m2/s)  
         npool_to_frootn              => nitrogenflux_vars%pool_to_froot_patch               , & ! Output: [real(r8) (:)   ]  allocation to fine root N (gN/m2/s)     
         npool_to_frootn_storage      => nitrogenflux_vars%pool_to_froot_storage_patch       , & ! Output: [real(r8) (:)   ]  allocation to fine root N storage (gN/m2/s)
         npool_to_livestemn           => nitrogenflux_vars%pool_to_livestem_patch            , & ! Output: [real(r8) (:)   ]                                          
         npool_to_livestemn_storage   => nitrogenflux_vars%pool_to_livestem_storage_patch    , & ! Output: [real(r8) (:)   ]                                          
         npool_to_deadstemn           => nitrogenflux_vars%pool_to_deadstem_patch            , & ! Output: [real(r8) (:)   ]                                          
         npool_to_deadstemn_storage   => nitrogenflux_vars%pool_to_deadstem_storage_patch    , & ! Output: [real(r8) (:)   ]                                          
         npool_to_livecrootn          => nitrogenflux_vars%pool_to_livecroot_patch           , & ! Output: [real(r8) (:)   ]                                          
         npool_to_livecrootn_storage  => nitrogenflux_vars%pool_to_livecroot_storage_patch   , & ! Output: [real(r8) (:)   ]                                          
         npool_to_deadcrootn          => nitrogenflux_vars%pool_to_deadcroot_patch           , & ! Output: [real(r8) (:)   ]                                          
         npool_to_deadcrootn_storage  => nitrogenflux_vars%pool_to_deadcroot_storage_patch   , & ! Output: [real(r8) (:)   ]                                          
         ...
         plant_palloc                 => phosphorusflux_vars%plant_alloc_patch                , & ! Output: [real(r8) (:)   ]  total allocated P flux (gP/m2/s)        
         ...
         ppool_to_grainp              => phosphorusflux_vars%pool_to_grain_patch             , & ! Output: [real(r8) (:)   ]  allocation to grain P (gP/m2/s)         
         ppool_to_grainp_storage      => phosphorusflux_vars%pool_to_grain_storage_patch     , & ! Output: [real(r8) (:)   ]  allocation to grain P storage (gP/m2/s) 
         ...
         ppool_to_leafp               => phosphorusflux_vars%pool_to_leaf_patch              , & ! Output: [real(r8) (:)   ]  allocation to leaf P (gP/m2/s)          
         ppool_to_leafp_storage       => phosphorusflux_vars%pool_to_leaf_storage_patch      , & ! Output: [real(r8) (:)   ]  allocation to leaf P storage (gP/m2/s)  
         ppool_to_frootp              => phosphorusflux_vars%pool_to_froot_patch             , & ! Output: [real(r8) (:)   ]  allocation to fine root P (gP/m2/s)     
         ppool_to_frootp_storage      => phosphorusflux_vars%pool_to_froot_storage_patch     , & ! Output: [real(r8) (:)   ]  allocation to fine root P storage (gP/m2/s)
         ppool_to_livestemp           => phosphorusflux_vars%pool_to_livestem_patch          , & ! Output: [real(r8) (:)   ]                                          
         ppool_to_livestemp_storage   => phosphorusflux_vars%pool_to_livestem_storage_patch  , & ! Output: [real(r8) (:)   ]                                          
         ppool_to_deadstemp           => phosphorusflux_vars%pool_to_deadstem_patch          , & ! Output: [real(r8) (:)   ]                                          
         ppool_to_deadstemp_storage   => phosphorusflux_vars%pool_to_deadstem_storage_patch  , & ! Output: [real(r8) (:)   ]                                          
         ppool_to_livecrootp          => phosphorusflux_vars%pool_to_livecroot_patch         , & ! Output: [real(r8) (:)   ]                                          
         ppool_to_livecrootp_storage  => phosphorusflux_vars%pool_to_livecroot_storage_patch , & ! Output: [real(r8) (:)   ]                                          
         ppool_to_deadcrootp          => phosphorusflux_vars%pool_to_deadcroot_patch         , & ! Output: [real(r8) (:)   ]                                          
         ppool_to_deadcrootp_storage  => phosphorusflux_vars%pool_to_deadcroot_storage_patch , & ! Output: [real(r8) (:)   ]                                          
...
!         sminp_vr                     => phosphorusstate_vars%sminp_vr_col                     , & ! Input:  [real(r8) (:,:) ]  (gP/m3) soil mineral P
         ...
         leafc_storage                => carbonstate_vars%leaf_storage_patch                , &
         leafc_xfer                   => carbonstate_vars%leaf_xfer_patch                   , &
...
!
         ...
         npool                        => nitrogenstate_vars%pool_patch                        , & ! Input:  [real(r8) (:)   ]  (gN/m2) plant N pool storage
         ...
         !!! add phosphorus variables  - X. YANG 
         ppool                        => phosphorusstate_vars%pool_patch                      , & ! Input: [real(r8)       ] Plant non-structural P storage (gP/m2)
         plant_pdemand                => phosphorusflux_vars%plant_pdemand_patch               , & ! Output: [real(r8) (:)   ]  P flux required to support initial GPP (gP/m2/s)
         plant_palloc                 => phosphorusflux_vars%plant_alloc_patch                , & ! Output: [real(r8) (:)   ]  total allocated P flux (gP/m2/s)
         ppool_to_grainp              => phosphorusflux_vars%pool_to_grain_patch             , & ! Output: [real(r8) (:)   ]  allocation to grain P (gP/m2/s)
         ppool_to_grainp_storage      => phosphorusflux_vars%pool_to_grain_storage_patch     , & ! Output: [real(r8) (:)   ]  allocation to grain P storage (gP/m2/s)
         ...
         laisun                       => canopystate_vars%laisun_patch                         , & ! Input:  [real(r8) (:)   ]  sunlit projected leaf area index        
         laisha                       => canopystate_vars%laisha_patch                         , & ! Input:  [real(r8) (:)   ]  shaded projected leaf area index        
         leafc                        => carbonstate_vars%leaf_patch                          , &
         leafn                        => nitrogenstate_vars%leaf_patch                        , &
         leafp                        => phosphorusstate_vars%leaf_patch                      , &
