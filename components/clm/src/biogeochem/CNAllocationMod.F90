...
!         sminp_vr                     => phosphorusstate_vars%sminp_vr_col                     , & ! Input:  [real(r8) (:,:) ]  (gP/m3) soil mineral P
      ...
      if (.not. use_nitrif_denitrif) then
         ...
         if (nu_com .eq. 'RD') then ! 'RD' : relative demand approach is default mode
            do j = 1, nlevdecomp
               do fc=1,num_soilc
                  ...
                  sum_pdemand_vr(c,j) = col_plant_pdemand(c) * puptake_prof(c,j) + potential_immob_p_vr(c,j)
                  col_plant_pdemand_vr(c,j) = col_plant_pdemand(c) * puptake_prof(c,j)
               end do
            end do
            ...
            do j = 1, nlevdecomp
               do fc=1,num_soilc
                  ...
                  if (sum_pdemand_vr(c,j)*dt < solutionp_vr(c,j)) then
                     ...
                     actual_immob_p_vr(c,j) = potential_immob_p_vr(c,j)
                     sminp_to_plant_vr(c,j) =  col_plant_pdemand_vr(c,j)
                     supplement_to_sminp_vr(c,j) = potential_immob_p_vr(c,j) + &
                          col_plant_pdemand_vr(c,j) - (solutionp_vr(c,j)/dt)
                  else
         ...
         if (nu_com .eq. 'RD') then
            ...
            do j = 1, nlevdecomp
               do fc=1,num_soilc
                  ...
                  col_plant_ndemand_vr(c,j) = col_plant_ndemand(c) * nuptake_prof(c,j)
               end do
            end do
            ...
            do j = 1, nlevdecomp  
               do fc=1,num_soilc
                  ...
                  sum_nh4_demand_vr(c,j) = col_plant_ndemand_vr(c,j)  + potential_immob_vr(c,j) + pot_f_nit_vr(c,j)
                  sum_nh4_demand_scaled(c,j) = col_plant_ndemand_vr(c,j) * compet_plant_nh4(p) + &
                  ...
                  if (sum_nh4_demand_vr(c,j)*dt < smin_nh4_vr(c,j)) then
                     ...
                     if (sum_nh4_demand_vr(c,j) > 0.0_r8 .and. smin_nh4_vr(c,j) > 0.0_r8) then
                        ...
                        smin_nh4_to_plant_vr(c,j) = min((smin_nh4_vr(c,j)/dt)*(col_plant_ndemand_vr(c,j)* &
                             compet_plant_nh4(p) / sum_nh4_demand_scaled(c,j)), col_plant_ndemand_vr(c,j) )
                  ...
                  sum_no3_demand_vr(c,j) = (col_plant_ndemand_vr(c,j) -smin_nh4_to_plant_vr(c,j)) + &
                       (potential_immob_vr(c,j)-actual_immob_nh4_vr(c,j)) + pot_f_denit_vr(c,j)
                  sum_no3_demand_scaled(c,j) = (col_plant_ndemand_vr(c,j)-smin_nh4_to_plant_vr(c,j))*compet_plant_no3(p) + &
                  ...
                  if (sum_no3_demand_vr(c,j)*dt < smin_no3_vr(c,j)) then
                     ...
                     if (sum_no3_demand_vr(c,j) > 0.0_r8 .and. smin_no3_vr(c,j) > 0.0_r8) then
                        ...
                        smin_no3_to_plant_vr(c,j) = min((smin_no3_vr(c,j)/dt)*((col_plant_ndemand_vr(c,j)- &
                             smin_nh4_to_plant_vr(c,j))*compet_plant_no3(p) / sum_no3_demand_scaled(c,j)), &
         ...
         if (nu_com .eq. 'RD') then
            ! calculate competition coefficients
            ! Relative Demand approach, competition coefficients are all set to one
            ! POx leaching is not considered as potential competitors
            ! P
            compet_plant_p(:) = 1._r8
            compet_decomp_p = 1._r8

            ! default is relative demand approach: root nutrient uptake profile is based on nutrient concentration profile
            ! nu_com with ECA or MIC: root nutrient uptake profile is based on fine root density profile
            ! init total mineral P pools
            do fc=1,num_soilc
               c = filter_soilc(fc)
               solutionp_tot(c) = 0._r8
            end do

            ! sum up total mineral P pools
            do j = 1, nlevdecomp
               do fc=1,num_soilc
                  c = filter_soilc(fc)
                  solutionp_tot(c) = solutionp_tot(c) + solutionp_vr(c,j) * dzsoi_decomp(j)
            ...
            ! define P uptake profile for initial vertical distribution of plant N uptake, assuming plant seeks P from where it is most abundant
            do j = 1, nlevdecomp
               do fc=1,num_soilc
                  c = filter_soilc(fc)
                  if (solutionp_tot(c)  >  0.) then
            ...
            do j = 1, nlevdecomp
               do fc=1,num_soilc
                  ...
                  ! compete for phosphorus
                  sum_pdemand_vr(c,j) = col_plant_pdemand_vr(c,j) + potential_immob_p_vr(c,j)
                  sum_pdemand_scaled(c,j) = col_plant_pdemand_vr(c,j)*compet_plant_p(p) + potential_immob_p_vr(c,j)*compet_decomp_p
         ...
         ! resolving N limitation vs. P limitation for decomposition
         ! if decomposition is more N limited than P limited, excess P is redistributed into plant P uptake flux
         ! if decomposition is more p limited than N limited, excess N is redistributed into plant NH4, NO3 uptake fluxes
         if( .not.cnallocate_carbonphosphorus_only().and. .not.cnallocate_carbonnitrogen_only().and. .not.cnallocate_carbon_only() )then
            if (nu_com .eq. 'RD') then 
               ! caution: not consistent with the idea in (.not. NITRIF_DENITRIF)
               do j = 1, nlevdecomp
                  do fc=1,num_soilc
                     c = filter_soilc(fc)
         ...
         ! calculate N/P limitation factor
         do fc=1,num_soilc
            c = filter_soilc(fc)
            ! calculate the fraction of potential growth that can be
            ! acheived with the N available to plants
