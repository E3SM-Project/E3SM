               ...
               do p = col%pfti(c), col%pftf(c)
                  ...
                  if (pft%active(p).and. (pft%itype(p) .ne. noveg)) then
                        do j = 1, nlevdecomp
                           ...
                           pnup_pfrootc(p) =  pnup_pfrootc(p) + plant_ndemand_vr_patch(p,j) * fpg_vr(c,j) / (max(frootc(p), 1e-20_r8) * froot_prof(p,j))
                        end do
                  end if
               end do
            end do
            ...
            do fc=1,num_soilc
               ...
               do p = col%pfti(c), col%pftf(c)
                  ...
                  if (pft%active(p).and. (pft%itype(p) .ne. noveg)) then
                        do j = 1, nlevdecomp
                           ! derivative of linear function
                           pnup_pfrootc(p) =  pnup_pfrootc(p) + plant_nh4demand_vr_patch(p,j) * fpg_nh4_vr(c,j)  / (max(frootc(p),1e-20_r8) * froot_prof(p,j))
                           pnup_pfrootc(p) =  pnup_pfrootc(p) + plant_no3demand_vr_patch(p,j) * fpg_no3_vr(c,j)  / (max(frootc(p),1e-20_r8) * froot_prof(p,j))
                        end do
                  end if
               end do
            end do
...
!-----------------------------------------------------------------------
  ...
  subroutine dynamic_plant_alloc( nutrient_scalar, water_scalar, laindex, alloc_leaf, alloc_stem, alloc_froot)
