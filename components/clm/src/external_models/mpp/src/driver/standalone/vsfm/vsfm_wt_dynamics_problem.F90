...
module vsfm_wt_dynamics_problem
...
#include <petsc/finclude/petsc.h>
  ...
  public :: run_vsfm_wt_dynamics_problem
  public :: output_regression_vsfm_wt_dynamics_problem

contains
!------------------------------------------------------------------------

  subroutine run_vsfm_wt_dynamics_problem()
    !
#include <petsc/finclude/petsc.h>
    !
    ...
    PetscBool          :: save_initial_sat, save_final_sat
    character(len=256) :: string
    character(len=256) :: output_suffix
    PetscViewer        :: viewer
    PetscReal, pointer :: data(:),vec_p(:)
    Vec                :: variable_vec
    ...
    save_initial_sat = PETSC_FALSE
    save_final_sat   = PETSC_FALSE
    output_suffix     = ''
    ...
    call PetscOptionsGetBool(PETSC_NULL_OPTIONS,PETSC_NULL_CHARACTER,'-save_final_sat',save_final_sat,flg,ierr)
    call PetscOptionsGetBool(PETSC_NULL_OPTIONS,PETSC_NULL_CHARACTER,'-save_initial_sat',save_initial_sat,flg,ierr)
    ...
    if (save_initial_sat) then
       if (len(trim(adjustl(output_suffix))) ==  0) then
          string = trim(output_suffix) // 'initial_sat.bin'
       else
          string = 'initial_sat_' // trim(output_suffix) // '.bin'
       endif
       ...
       call VecCreateSeq(PETSC_COMM_SELF, nz, variable_vec, ierr); CHKERRQ(ierr)
       call VecGetArrayF90(variable_vec, vec_p, ierr); CHKERRQ(ierr)
       vec_p = data
       call VecRestoreArrayF90(variable_vec, vec_p, ierr); CHKERRQ(ierr)
       
       call PetscViewerBinaryOpen(PETSC_COMM_SELF,trim(string),FILE_MODE_WRITE,viewer,ierr);CHKERRQ(ierr)
       call VecView(variable_vec,viewer,ierr);CHKERRQ(ierr)
       call PetscViewerDestroy(viewer,ierr);CHKERRQ(ierr)
    ...
    if (save_initial_sat) then
       if (len(trim(adjustl(output_suffix))) ==  0) then
          string = trim(output_suffix) // 'final_sat.bin'
       else
          string = 'final_sat_' // trim(output_suffix) // '.bin'
       endif
  ...
  end subroutine run_vsfm_wt_dynamics_problem
...
#include <petsc/finclude/petsc.h>
  ...
  subroutine add_conditions_to_goveqns()
    ...
    call vsfm_mpp%soe%AddConditionInGovEqn(ieqn, COND_SS,   &
         'Source at top', 'kg/s', COND_MASS_RATE, &
         SOIL_TOP_CELLS)
  ...
  subroutine set_initial_conditions()
    ...
    do kk = 1,nz
       z            = dz/2.d0 + dz * (kk - 1)
       press_ic(kk) = 101325.d0 + (0.5d0 - z)* 997.16d0 * 9.80868d0
    enddo
  ...
  subroutine set_bondary_conditions()
    ...
    top_pressure_bc(:) = 2.5d-5 * 1d3
    bot_pressure_bc(:) = 101325.d0 + (0.5d0 - z_column/nz/2.d0*0.d0)* 997.16d0 * 9.80868d0
  ...
  !------------------------------------------------------------------------
  subroutine output_regression_vsfm_wt_dynamics_problem(filename_base, num_cells)
    !
    use MultiPhysicsProbVSFM      , only : vsfm_mpp
    use MultiPhysicsProbConstants , only : AUXVAR_INTERNAL
  ...
  end subroutine output_regression_vsfm_wt_dynamics_problem
  
end module vsfm_wt_dynamics_problem
