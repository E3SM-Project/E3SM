...
#include "shr_assert.h"
  ...
  use dynPriorWeightsMod  , only : prior_weights_type, set_prior_weights_acc
  use dynPatchStateUpdaterMod  , only : patch_state_updater_type, patch_state_set_old_weights_acc
  use dynColumnStateUpdaterMod , only : column_state_updater_type,column_state_set_old_weights_acc
  ...
  use glc2lndMod          , only : glc2lnd_type, glc2lnd_vars_update_glc2lnd_acc
...
contains
  ...
  subroutine dynSubgrid_init(bounds, glc2lnd_vars, crop_vars)
    ...
    character(len=256) :: flanduse_timeseries
    !-----------------------------------------------------------------------
  ...
  subroutine dynSubgrid_driver(bounds_proc, &
       ...
       grc_cs, grc_cf, glc2lnd_vars, crop_vars, dt)
    !
    ! !DESCRIPTION:
    ...
    use decompMod            , only : bounds_type, get_proc_clumps, get_clump_bounds_gpu
