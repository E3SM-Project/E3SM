...
#SBATCH --gpus=4
#SBATCH -t 00:30:00
#SBATCH -J p3-test-gpu-dp
#SBATCH -o p3-test-results/o.p3-test-gpu-dp.%j.txt
#
# Standalone P3 GPU test in double precision.  The machine is selected from
# NERSC_HOST so the same script can be submitted on Perlmutter or Alvarez.
# Build first on the login node with ./build-p3-test-gpu-dp.sh.
set -euo pipefail
...
exec "${REPO_ROOT}/run_p3_test_${MACHINE_NAME}-dp.sh" "$MACHINE_NAME"
