#!/bin/bash
# Profile one standalone P3 DP run on a Perlmutter GPU node.
#
...
# node with ./build-p3-profile-gpu-dp.sh. The profiling build is kept in a
# separate directory so it cannot overwrite the normal testing executable.
#
# Examples:
#   sbatch sbatch-p3-profile-pm-gpu-dp.sh
...
TEST_EXE="${REPO_ROOT}/build_p3_profile_pm-gpu-DP/src/physics/p3/tests/p3_run_and_cmp"
PROFILE_TOOL="${PROFILE_TOOL:-nsys}"
SCRATCH_ROOT="${SCRATCH:-/pscratch/sd/${USER:0:1}/${USER}}"
RESULTS_DIR="${RESULTS_DIR:-${SCRATCH_ROOT}/e3sm/p3-profile-results/pm-gpu-dp/${SLURM_JOB_ID:-manual}}"
