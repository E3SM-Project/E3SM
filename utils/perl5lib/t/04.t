...
# Test use of '+='; this should work just like '=' if this variable has not yet been given a value
$namelist = "&clm_inparm hist_fincl1+='A1' /";
$nl2 = Build::Namelist->new( $namelist );
is($nl2->get_variable_value('clm_inparm', 'hist_fincl1'), "'A1'", "+= should work like = if the variable has not yet been given a value");

# Test that a second assignment overrides the first value
$namelist = "&clm_inparm hist_fincl1='A1', hist_nhtfrq=1, hist_fincl1='B2' /";
$nl2 = Build::Namelist->new( $namelist );
is($nl2->get_variable_value('clm_inparm', 'hist_fincl1'), "'B2'", "a second = should override the first value");

# Test that += appends to an existing value
$namelist = "&clm_inparm hist_fincl1='A1', hist_nhtfrq=1, hist_fincl1+='B2' /";
$nl2 = Build::Namelist->new( $namelist );
is($nl2->get_variable_value('clm_inparm', 'hist_fincl1'), "'A1','B2'", "+= should append to an existing value");
...
my $regexpr;
$regexpr = qr/ERROR\(Build::Namelist::_parse_next\): expect a equal '=' sign or '\+=', instead got: ','B2','C3'/;
