...
set e3sm_tag       = crterai/atm/add_SHOC2_v2P3
set tag_name       = #E3SM_alpha5_59_v2candidate_NGD_Conv

### CUSTOM CASE_NAME
#set case_name = ${tag_name}.${job_name}.${resolution}.${machine} 
set case_name = E3SM_v2SHOC.${job_name}.${resolution}.${machine}  #TOCHANGE
...
set seconds_before_delete_source_dir =  3 #-1
set seconds_before_delete_case_dir   =  3 #10
set seconds_before_delete_bld_dir    =  3 #-1
set seconds_before_delete_run_dir    =  3 #-1
...
### DIRECTORIES
set code_root_dir           = /qfs/people/tera431/E3SM # user modify
set e3sm_simulations_dir    = /compyfs/tera431/E3SM_simulations # user modify
...
#set restart_files_dir           = /compyfs/shpu881/E3SM_simulations/E3SM-at-upst-merge_PR3629.FC5AV1C-L_wP3_m1.ne30_ne30.compy/run
...
if ( `lowercase $processor_config` == '1' ) then
  ...
  ${xmlchange_exe} MAX_TASKS_PER_NODE="128"
  ${xmlchange_exe} MAX_MPITASKS_PER_NODE="32"

  ${xmlchange_exe} NTASKS_ATM="5400"
  ${xmlchange_exe} ROOTPE_ATM="0"

  ${xmlchange_exe} NTASKS_LND="5400"
  ${xmlchange_exe} ROOTPE_LND="0"

  ${xmlchange_exe} NTASKS_ICE="4800"
  ${xmlchange_exe} ROOTPE_ICE="0"

  ${xmlchange_exe} NTASKS_OCN="4800"
  ${xmlchange_exe} ROOTPE_OCN="0"

  ${xmlchange_exe} NTASKS_CPL="5400"
  ${xmlchange_exe} ROOTPE_CPL="0"

  ${xmlchange_exe} NTASKS_GLC="32"
  ${xmlchange_exe} ROOTPE_GLC="0"

  ${xmlchange_exe} NTASKS_ROF="5400"
  ${xmlchange_exe} ROOTPE_ROF="0"

  ${xmlchange_exe} NTASKS_WAV="32"
  ${xmlchange_exe} ROOTPE_WAV="0"

  ${xmlchange_exe} NTHRDS_ATM="4"
  ${xmlchange_exe} NTHRDS_LND="4"
  ${xmlchange_exe} NTHRDS_ICE="2"
  ${xmlchange_exe} NTHRDS_OCN="2"
  ${xmlchange_exe} NTHRDS_CPL="4"
  ${xmlchange_exe} NTHRDS_GLC="2"
  ${xmlchange_exe} NTHRDS_ROF="4"
  ${xmlchange_exe} NTHRDS_WAV="1"

##Xue: added a custom PE layout from Kai's run script  
else if ( `lowercase $processor_config` == 'custom' ) then
####Xue 20200218: for theta-l + SL tracer transport 
    ...
    echo "AMB> ntask $ntask pcol $pcol"
    ${xmlchange_exe} --id CAM_CONFIG_OPTS --append --val="-pcols $pcol"

  ${xmlchange_exe} NTASKS_ATM="$ntask"
  ${xmlchange_exe} NTASKS_LND="$ntask"
  ${xmlchange_exe} NTASKS_ICE="$ntask"
  ${xmlchange_exe} NTASKS_OCN="$ntask"
  ${xmlchange_exe} NTASKS_CPL="$ntask"
  ${xmlchange_exe} NTASKS_GLC="$ntask"
  ${xmlchange_exe} NTASKS_ROF="$ntask"
  ${xmlchange_exe} NTASKS_WAV="$ntask"
...
# +++ 
#$xmlchange_exe --id CAM_CONFIG_OPTS --val "-mach $machine -phys default -clubb_sgs -microphys p3 -chem linoz_mam4_resus_mom_soag -rain_evap_to_coarse_aero -bc_dep_to_snow_updates -nlev 72 -cosp"
# +++

# Use atmosphere theta-l dynamical core
${xmlchange_exe} CAM_TARGET=theta-l

${xmlchange_exe} CAM_CONFIG_OPTS="-cosp" --append

# Switch radiation to RRTMGP
## [JS]:comment-out   ${xmlchange_exe} --append CAM_CONFIG_OPTS='-rad rrtmgp'
...
if ( `lowercase $debug_queue` == true ) then
  if ( $machine == quartz || $machine == syrah ) then
    ...
    $xmlchange_exe --id JOB_QUEUE --val 'slurm'
  endif
endif
...
# --- [JS]
$xmlchange_exe --id SAVE_TIMING --val "FALSE"
...
#-----------------------------------------------------------------------------------------------
  ...
  cp -s ${restart_files_dir}/${restart_case_name}.cice.r.${restart_filedate}-00000.nc $case_run_dir   # [JS]  
