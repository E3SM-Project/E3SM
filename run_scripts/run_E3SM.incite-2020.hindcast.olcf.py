...
case_list = ['INCITE2020','HINDCAST',arch,grid,compset,f'CRMNXY_{crm_nx}x{crm_ny}']

### specific case names and settings
# case = '.'.join(case_list+['CRMDX_1600','NLEV_50' ,'CRMNZ_47' ,'CRMDT_5e0','MOMFB',init_date] )
# case = '.'.join(case_list+['CRMDX_1600','NLEV_125','CRMNZ_115','CRMDT_2e0','MOMFB',init_date] )
case = '.'.join(case_list+['CRMDX_1600','NLEV_250','CRMNZ_230','CRMDT_1e0','MOMFB',init_date] )
...
if config : 
   ...
   for p in params:
      ...
      if p[0]=='CRMDT': run_cmd(f'./xmlchange --append -id CAM_CONFIG_OPTS -val \" -crm_dt {float(p[1])} \" ')
