...
# 144 "pio_support.F90.in"
  subroutine pio_writedof (file, DOF, comm, punit)
    ...
    sdof1d = -1
    sdof_tmp(1) = sdof
    call pio_fc_gather_offset(sdof_tmp, 1, MPI_INTEGER, &
...
#endif
             call MPI_SEND(dof,sdof,MPI_INTEGER8,masterproc,n,comm,ierr)
             if (ierr /= MPI_SUCCESS) call piodie(__PIO_FILE__,__LINE__,' pio_writedof mpi_send')
          endif
          if (myrank == masterproc .and. sdof1d(n) > 0) then
#ifndef _NO_FLOW_CONTROL
             call MPI_IRECV(wdof,sdof1d(n),MPI_INTEGER8,n,n,comm,rcv_request,ierr)
...
#else
             call MPI_RECV(wdof,sdof1d(n),MPI_INTEGER8,n,n,comm,status,ierr)
             if (ierr /= MPI_SUCCESS) call piodie(__PIO_FILE__,__LINE__,' pio_writedof mpi_recv')
#endif
          endif
       endif
    ...
    if (myrank == masterproc) then
       ...
       do n = 0,npes-1
          ...
          if (n == masterproc) then
             ...
             call MPI_SEND(sdof,1,MPI_INTEGER8,n,n,comm,ierr)
             if (ierr /= MPI_SUCCESS) call piodie(__PIO_FILE__,__LINE__,' pio_readdof mpi_send1')
             if (sdof > 0) then
                call MPI_SEND(wdof,sdof,MPI_INTEGER8,n,npes+n,comm,ierr)
       ...
       call MPI_RECV(sdof,1,MPI_INTEGER8,masterproc,myrank,comm,status,ierr)
       if (ierr /= MPI_SUCCESS) call piodie(__PIO_FILE__,__LINE__,' pio_readdof mpi_recv1')
       if (sdof > 0) then
          deallocate(dof)
          allocate(dof(sdof))
          call MPI_RECV(dof,sdof,MPI_INTEGER8,masterproc,npes+myrank,comm,status,ierr)
