...
!===================================================
#define _FILE_ "box_rearrange.F90"
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!
  ...
  do i=1,ndof
    ...
    if (ioproc /= -1 ) then  ! ignore sender hole
      ...
      if ( ioindex<0 .or. ioindex>=niodof ) &
        call piodie( _FILE_,__LINE__, &
                     'box_rearrange_comp2io: i=', i, &
                     'dest_ioindex(i) out of range=', ioindex )
...
subroutine box_rearrange_comp2io_real (IOsystem, ioDesc, s1, src, s2, &
  ...
  if(iosystem%intercomm/=MPI_COMM_NULL) then
     mycomm = iosystem%intercomm
  else
     mycomm = iosystem%comp_comm
  end if
  ! begin
  ...
  if (pio_option /= POINT_TO_POINT) then
    ...
    if (pio_option == COLLECTIVE) then
      ...
      call pio_swapm( nprocs, myrank,                            &
        ...
        mycomm, pio_hs, pio_isend, pio_maxreq        )
#ifdef _USE_ALLTOALLW
    endif
#endif
  ...
  if (pio_option /= POINT_TO_POINT) then
    ...
    if (pio_option == COLLECTIVE) then
      ...
      call pio_swapm( nprocs, myrank,                               &
        ...
        IOsystem%comp_comm, pio_hs, pio_isend, pio_maxreq           )
#ifdef _USE_ALLTOALLW
    endif
#endif
    call dealloc_check(a2a_sendcounts)
    ...
    do i=1,num_iotasks
      if (scount(i) /= 0) then
        ...
        call MPI_IRECV( compbuf, 1, stype(i), &                ! buf, count, type
                        io_comprank,TAG2,    &                 ! destination,tag
                        IOsystem%comp_comm,rreq(i),ierror )
        call CheckMPIReturn(subName,ierror)
      endif
    ...
    if (IOsystem%IOproc) then
      do i=1,nrecvs
        ...
        call MPI_ISEND( iobuf,1, rtype(i), &                ! buf, count, type
                        ...
                        IOsystem%comp_comm,sreq(i),ierror )        
        call CheckMPIReturn(subName,ierror)
...
# 762 "box_rearrange.F90.in"
  subroutine gindex_to_coord( gindex, gstride, ndim, gcoord )
    ...
    integer,intent(out) :: gcoord(:)       ! output global coords (0-based)
...
# 798 "box_rearrange.F90.in"
  logical function find_ioproc( gcoord, lb, ub, lstride, ndim, nioproc, &
    ...
    loop_ioproc: do i=1,nioproc
       ...
       if (found) then
          call piodie(_FILE_,__LINE__, 'multiple matches')
       endif
...
# 869 "box_rearrange.F90.in"
  subroutine compute_dest(compdof, start, count, gsize, ndim, nioproc, &
    ...
    integer, intent(out) :: dest_ioindex(:)     ! index in iobuf on that ioproc
    ...
    integer ndof
    integer gindex
    integer lb(ndim,nioproc)        ! 0-based lower bound of boxes
    integer ub(ndim,nioproc)        ! 0-based upper bound of boxes
    integer gcoord(ndim)            ! 0-based xyz coordinates
    ...
    if(Debug) print *,_FILE_,__LINE__,minval(compdof), maxval(compdof)
    do i=1,ndof
       ...
       if (compdof(i)==0) then             ! sender hole
          ...
          if (.not. find_ioproc(gcoord, lb, ub, lstride, ndim, nioproc, &
             ...
             call piodie( _FILE_,__LINE__, 'quitting' )
          endif
...
# 980 "box_rearrange.F90.in"
  subroutine box_rearrange_create(Iosystem, compdof, gsize, ndim, &
                                  nioproc, ioDesc)
    implicit none

    type (Iosystem_desc_t), intent(in) :: Iosystem
    integer, intent(in) :: compdof(:)      ! global indices for compbuf
    integer, intent(in) :: gsize(:)        ! global domain size gsize(ndim)
...
!!!!!!
    ndof = size(compdof)
    call alloc_check( ioDesc%dest_ioproc, ndof,          &
                      'box_rearrange_create dest_ioproc' )
    call alloc_check( ioDesc%dest_ioindex, ndof,         &
...
!!!!!!
    ...
    if(ndim.ne.size(iodesc%start)) then
       print *,_FILE_,__LINE__,ndim, size(iodesc%start)
       call piodie(_FILE_,__LINE__,'bad ndim size',ndim)
    end if
    ...
    if (Iosystem%IOproc) then
       ...
       if(Debug) then
          print *, _FILE_,__LINE__,iodesc%start, iodesc%count
          if(iosystem%io_rank==0) &
             print *,_FILE_,__LINE__,ndim,(i,' :', &
...
# 1093 "box_rearrange.F90.in"
  subroutine compute_counts(Iosystem, ioDesc, ndof, niodof)
    ...
    integer :: ioindex            ! offset for data to be sent to i/o task
    ...
    integer,pointer :: sindex(:)  ! sindex(ndof) is blocks of src indices
    integer,pointer :: s2rindex(:)! s2rindex(ndof) is local blocks of dest indices
    ...
    integer,pointer :: rindex(:)  ! rindex(niodof) is blocks of dest indices
    ...
    do i=1,ndof
       ...
       if (iorank /= -1) then                       ! not a sender hole
          if (iorank<1 .or. iorank>num_iotasks) &
               call piodie(_FILE_,__LINE__,'io destination out of range',iorank)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    do i=2,num_iotasks
       ...
       if (scount(i)/=0 .and. spos(i) > ndof) & 
            call piodie(_FILE_,__LINE__,'spos=',spos(i),'> ndof=',ndof)
    end do
    ...
    do i=1,ndof
       ...
       if (iorank /= -1) then                                  ! skip sender hole
          ...
          if (tempcount(iorank) > scount(iorank)) &
               call piodie(_FILE_,__LINE__,'tempcount>scount')
       endif
    end do
...
!!!!!!
    ...
    if (Iosystem%IOproc) then
       ...
       if(Debug) then
          print *, _FILE_,__LINE__,iodesc%start, iodesc%count
          if(iosystem%io_rank==0) print *,_FILE_,__LINE__,ndim,(i,' :',start(:,i), count(:,i),i=1,iosystem%num_iotasks)
       end if
       ! note that index in start,count is the io_rank not comp_rank
    endif
print *,__FILE__,__LINE__
    if(iosystem%intercomm /= MPI_COMM_NULL) then
       is_async=.true.
       call MPI_BCAST( start,ndim*Iosystem%num_iotasks,MPI_INTEGER, &    ! buf, cnt
            iosystem%iomaster, Iosystem%intercomm,ierror )
       call CheckMPIReturn(subName,ierror)
...
!!!!!!!
    ...
    root=Iosystem%iomaster
    do i=1,Iosystem%num_iotasks
!#if 0
       if(iosystem%comp_rank>=0) print *,Iosystem%comp_rank,': ',nsends(i),' sends to ',i
!#endif
       if(is_async) then
	  print *,__FILE__,__LINE__,iosystem%io_rank,i-1
          root = find_io_comprank(ioSystem,i) 
          if(iosystem%io_rank==root) then
             print *,__FILE__,__LINE__
             call MPI_REDUCE( nsends(i),ioDesc%nrecvs,1,MPI_INTEGER, &
                  MPI_SUM,MPI_ROOT,Iosystem%intercomm,ierror )
             print *,__FILE__,__LINE__, iodesc%nrecvs
          else if(iosystem%io_rank>=0) then
             print *,__FILE__,__LINE__
             call MPI_REDUCE( nsends(i),ioDesc%nrecvs,1,MPI_INTEGER, &
                  MPI_SUM,MPI_PROC_NULL,Iosystem%intercomm,ierror )
             print *,__FILE__,__LINE__
          else
             print *,__FILE__,__LINE__
             call MPI_REDUCE( nsends(i),ioDesc%nrecvs,1,MPI_INTEGER, &
                  MPI_SUM, root  ,Iosystem%intercomm,ierror )
...
!#if DEBUG
    if (Iosystem%IO_rank==i-1) &
         print *,Iosystem%io_rank,': nrecvs=',ioDesc%nrecvs
!#endif
    end do
    ...
    if(is_async) then
       call MPI_REDUCE( nsends_total,nsends_global,1,MPI_INTEGER, &
            MPI_SUM,Iosystem%iomaster,Iosystem%intercomm,ierror )
    else	
       call MPI_REDUCE( nsends_total,nsends_global,1,MPI_INTEGER, &
            MPI_SUM,Iosystem%iomaster,Iosystem%comp_comm,ierror )
    end if
    call CheckMPIReturn(subName,ierror)
    ...
    if (.not.is_async .and. Iosystem%comp_rank == Iosystem%iomaster) then
    ...
    print *,__FILE__,__LINE__
    call compute_counts(Iosystem,ioDesc,ndof,niodof, is_async)
...
# 1713 "box_rearrange.F90.in"
  subroutine compute_counts( Iosystem,ioDesc,ndof,niodof, is_async )
    ...
    integer,pointer :: rindex(:)  ! rindex(niodof)= blocks of dest indices
    ...
    integer,pointer :: srcindex(:)! srcindex(ndof)= blocks of src indices
    integer,pointer :: stype(:)
#ifdef NO_MPI2
    ...
    integer,pointer :: spos(:)      ! spos(i) = start in sindex for ith ioproc
       ...
       do i=1,nrecvs
          if(is_async) then
             call MPI_IRECV( rcount(i),1,MPI_INTEGER, &       ! buf,count,type
                  ...
                  Iosystem%intercomm,rreq(i),ierror)
          else
             call MPI_IRECV( rcount(i),1,MPI_INTEGER, &       ! buf,count,type
                  MPI_ANY_SOURCE,TAG0, &           ! source, tag
                  Iosystem%comp_comm,rreq(i),ierror)
       ...
       if (scount(i) /= 0) then
          ...
          if(is_async) then
             call MPI_ISEND( scount(i),1,MPI_INTEGER, &       ! buf,count,type
                  ...
                  Iosystem%intercomm,sreq,ierror )
          else
             ! go from 1-based io rank to 0-based rank in comp_comm
             call MPI_ISEND( scount(i),1,MPI_INTEGER, &       ! buf,count,type
                  io_comprank,TAG0,  &             ! destination,tag
                  Iosystem%comp_comm,sreq,ierror )
          end if
          call CheckMPIReturn(subName,ierror)
...
!!!!!!
    ...
    if (Iosystem%IOproc) then
       ...
       pos=1
       do i=1,nrecvs
          print *,__FILE__,__LINE__,i,nrecvs
          call MPI_WAIT( rreq(i), status, ierror )
          call CheckMPIReturn(subName,ierror)
...
!!!!!!
    ...
    do i=1,num_iotasks                   ! send indices to ioproc i
       if (scount(i) /= 0) then
          ...
          if (count/=scount(i)) call piodie(_FILE_,__LINE__,'count/=scount(i)')
          if (count>max_scount) call piodie(_FILE_,__LINE__,'count>max_scount')
