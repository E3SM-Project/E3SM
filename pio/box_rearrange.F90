...
# 853 "box_rearrange.F90.in"
  subroutine compute_dest(compdof, start, count, gsize, ndim, nioproc, &
    ...
    integer(kind=pio_offset), intent(in) :: count(:,:)       ! count(ndim,nioproc)
...
#endif
    if (Iosystem%IOproc) then
       call pio_fc_gather_offset(iodesc%start, ndim, MPI_INTEGER8, & ! sendbuf, count, type
            start, ndim, MPI_INTEGER8,             & ! recvbuf, count, type
            0, Iosystem%IO_comm                   )

       call pio_fc_gather_offset(iodesc%count, ndim, MPI_INTEGER8, & ! sendbuf, count, type
...
!       end if
    ...
    if(debug) print *,__PIO_FILE__,__LINE__,ndim,iosystem%num_iotasks,iosystem%ioranks(1),iosystem%union_comm

    call MPI_BCAST(start, ndim*Iosystem%num_iotasks, MPI_INTEGER8,   & ! buf, cnt
                   Iosystem%ioranks(1), Iosystem%union_comm, ierror    )
    call CheckMPIReturn(subName, ierror)

    call MPI_BCAST(count, ndim*Iosystem%num_iotasks, MPI_INTEGER8,   & ! buf, cnt
                   Iosystem%ioranks(1), Iosystem%union_comm, ierror    )
    call CheckMPIReturn(subName, ierror)
