...
# 1407 "piodarray.F90.in"
 subroutine write_vdc2_real(File, Vardesc, iodesc, array, iostat, tmp_lod, tmp_reflevel, tmp_ts)
    ...
    if (IOproc) then
       ...
       cptr = c_loc(iobuf(1))
       print *,__FILE__,__LINE__,file%iosystem%num_iotasks,vardesc%name(1:vlen)
       call WriteVDC2Var(cptr, start, count,  File%iosystem%IO_comm, tmp_ts, tmp_lod, tmp_reflevel, File%iosystem%num_iotasks, F_C_STRING_DUP(varDesc%name(1:vlen)))
    endif
...
subroutine read_vdc2_real(File, Vardesc, iodesc, array, iostat, tmp_lod, tmp_ref, tmp_ts)
    ...
    if(UseRearranger) then 
       if (IOproc) then 
          ...
          IOBUF= -1.0_r8
          cptr = c_loc(iobuf(1))
  	  call ReadVDC2Var(cptr, start, count,  File%iosystem%IO_comm, tmp_ts, tmp_lod, tmp_ref, File%iosystem%num_iotasks, F_C_String_dup(varDesc%name))
       else
          call alloc_check(IOBUF,0)
          IOBUF= -1.0_r8
       endif
