...
!               cm%col(i,j)%V(nlevp-k)     = v1*elemin%Dinv(1,2,i,j) + v2*elemin%Dinv(2,2,i,j)                
       ...
       do k=1,nlev
	     ...
	     if(abs(elemin%sphereP(i,j)%lat).lt. emanuel_max_lat) then
                ...
                v1                         = elemin%state%v(i,j,1,k,nm1)
                v2                         = elemin%state%v(i,j,2,k,nm1)
                cm%col(i,j)%U(nlevp-k)     = v1*elemin%D(i,j,1,1) + v2*elemin%D(i,j,1,2)            
                cm%col(i,j)%V(nlevp-k)     = v1*elemin%D(i,j,2,1) + v2*elemin%D(i,j,2,2)                
#ifdef USE_MAXLAT
		endif
#endif
             enddo
       ...
       do k=1,nlev
          do j=1,np
    	     ...
    	     if(abs(elemin%sphereP(i,j)%lat).lt. emanuel_max_lat) then
                ...
                elemin%state%v(i,j,1,k,nm1)  = u1*elemin%Dinv(i,j,1,1) + v1*elemin%Dinv(i,j,1,2)
                elemin%state%v(i,j,2,k,nm1)  = u1*elemin%Dinv(i,j,2,1) + v1*elemin%Dinv(i,j,2,2)
                elemin%state%T(i,j,k,nm1)    = cd%T(nlevp-k)
       ...
       do k=1,nlev
          do j=1,np
    	     ...
    	     if(abs(elemin%sphereP(i,j)%lat).lt. emanuel_max_lat) then
                ...
                elemin%derived%FM(i,j,1,k,nm1) = &
                elemin%derived%FM(i,j,1,k,nm1) + fu*elemin%Dinv(i,j,1,1) + fv*elemin%Dinv(i,j,1,2)
                elemin%derived%FM(i,j,2,k,nm1) = &
                elemin%derived%FM(i,j,2,k,nm1) + fu*elemin%Dinv(i,j,2,1) + fv*elemin%Dinv(i,j,2,2)
                elemin%derived%FT(i,j,k,nm1)   = &
                elemin%derived%FT(i,j,k,nm1)   +     cd%FT(nlevp-k)
                elemin%derived%FQ(i,j,k,nm1,1) = &
