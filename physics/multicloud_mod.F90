...
#ifndef CAM
  use physics_mod, only        : Prim_Condense,getsurfpress,Temp2PotTemp
#endif
...
contains
  ...
  !
  !=============================================================
  subroutine Prim_Advance_Multicloud(elem,cm,hvcoord,deriv,flt,hybrid,dt,tl,nets,nete)
    ...
    if(nstep>0 .and. filter_freq_advection > 0 .and. modulo(nstep,filter_freq_advection)==0) then
       ...
       do ie=nets,nete
          call filter_P(elem(ie)%state%qmc(:,:,nfilt),  flt)
          call filter_P(elem(ie)%state%teb(:,:,nfilt),flt)
          do j=1,np
             do i=1,np
                elem(ie)%state%qmc(i,j,nfilt) = elem(ie)%state%mv(i,j)*elem(ie)%state%qmc(i,j,nfilt)
                elem(ie)%state%teb(i,j,nfilt) = elem(ie)%state%mv(i,j)*elem(ie)%state%teb(i,j,nfilt)
             end do
          end do
          call edgeVpack(edgeMc,elem(ie)%state%qmc(:,:,nfilt),1,0,elem(ie)%desc)
          call edgeVpack(edgeMc,elem(ie)%state%teb(:,:,nfilt),1,1,elem(ie)%desc)
       end do

       call bndry_exchangeV(hybrid,edgeMc)

       do ie=nets,nete
          call edgeVunpack(edgeMc,elem(ie)%state%qmc(:,:,nfilt),1,0,elem(ie)%desc)
          call edgeVunpack(edgeMc,elem(ie)%state%teb(:,:,nfilt),1,1,elem(ie)%desc)
          do j=1,np	
             do i=1,np
                elem(ie)%state%qmc(i,j,nfilt) = elem(ie)%state%rmv(i,j)*elem(ie)%state%qmc(i,j,nfilt)
                elem(ie)%state%teb(i,j,nfilt) = elem(ie)%state%rmv(i,j)*elem(ie)%state%teb(i,j,nfilt)
             enddo
          enddo
       end do
    ...
    do ie=nets,nete
       ...
       !
       ! V1
       call VerticalProjectionVector(elem(ie)%state%v(:,:,:,:,n0v),elem(ie)%state%uproj1(:,:,:,n0v),&
            cm%D%phi(:,1),elem(ie)%state%lnps(:,:,n0v),hvcoord)
       ! V2
       call VerticalProjectionVector(elem(ie)%state%v(:,:,:,:,n0v),elem(ie)%state%uproj2(:,:,:,n0v),&
            cm%D%phi(:,2),elem(ie)%state%lnps(:,:,n0v),hvcoord)
       ! Ubar
       call VerticalProjectionVector(elem(ie)%state%v(:,:,:,:,n0v),elem(ie)%state%ubar(:,:,:,n0v),&
            cm%D%cstmode(:),elem(ie)%state%lnps(:,:,n0v),hvcoord)

       ! 01/20/2009
       do j=1,np	
          do i=1,np
             up1 = elem(ie)%state%uproj1(i,j,:,n0v)
             up2 = elem(ie)%state%uproj2(i,j,:,n0v)
             elem(ie)%state%uproj1(i,j,1,n0v)=up1(1)*cm%D%Aphi(1,1)+up2(1)*cm%D%Aphi(1,2)
             elem(ie)%state%uproj1(i,j,2,n0v)=up1(2)*cm%D%Aphi(1,1)+up2(2)*cm%D%Aphi(1,2)

             elem(ie)%state%uproj2(i,j,1,n0v)=up1(1)*cm%D%Aphi(2,1)+up2(1)*cm%D%Aphi(2,2)
             elem(ie)%state%uproj2(i,j,2,n0v)=up1(2)*cm%D%Aphi(2,1)+up2(2)*cm%D%Aphi(2,2)
          enddo
       enddo

       do j=1,np	
          do i=1,np
             q  = elem(ie)%state%qmc(i,j,n0v)
             v1 = (elem(ie)%state%uproj1(i,j,1,n0v) + cm%alpha*elem(ie)%state%uproj2(i,j,1,n0v) + 0.0D0*elem(ie)%state%ubar(i,j,1,n0v))*q
             v2 = (elem(ie)%state%uproj1(i,j,2,n0v) + cm%alpha*elem(ie)%state%uproj2(i,j,2,n0v) + 0.0D0*elem(ie)%state%ubar(i,j,2,n0v))*q
       ...
       do j=1,np	
          do i=1,np
             v1 = elem(ie)%state%uproj1(i,j,1,n0v)*cm%D%Qt1 + elem(ie)%state%uproj2(i,j,1,n0v)*cm%D%Qt2
             v2 = elem(ie)%state%uproj1(i,j,2,n0v)*cm%D%Qt1 + elem(ie)%state%uproj2(i,j,2,n0v)*cm%D%Qt2
             ...
             ! this was eliminated
             !v1 = elem(ie)%state%ubar(i,j,1,n0v)*cm%Qt0
             !v2 = elem(ie)%state%ubar(i,j,2,n0v)*cm%Qt0

             v1 = elem(ie)%state%ubar(i,j,1,n0v)
             v2 = elem(ie)%state%ubar(i,j,2,n0v)
       ...
       gradqmc(:,:,:) = gradient(elem(ie)%state%qmc(:,:,n0),  deriv)
       gradteb(:,:,:) = gradient(elem(ie)%state%teb(:,:,n0),  deriv)
       ...
       !do k=1,nlev
          pot0(:,:,:) = Temp2PotTemp(elem(ie)%state%T(:,:,:,n0),p_0(:,:,:))-elem(ie)%state%pot0(:,:,:)
          pot(:,:,:)  = Temp2PotTemp(elem(ie)%state%T(:,:,:,nm1),p(:,:,:))-elem(ie)%state%pot0(:,:,:)
          if(Debug)print *,"Potential temp     at level ",k, " is ", sum(abs(pot(:,:,k)))
          if(Debug)print *,"Potential temp t=0 at level ",k, " is ", sum(abs(elem(ie)%state%pot0(:,:,k)))
       ...
       if(Debug)print *,"QMC = ",sum(abs(elem(ie)%state%qmc(:,:,:)))
       ...
       do j=1,np	
	    ...
	    ! tebtens=0.d0
             tem     = elem(ie)%state%qmc(i,j,nm1) + cm%D%psi1avg*(t1(i,j)+ cm%alpha2*t2(i,j)) !*cm%psi2avg)

             deltem = elem(ie)%state%teb(i,j,nm1)-tem+cm%TebMinTem
             teb    = elem(ie)%state%teb(i,j,n0)
             ...
             ! At "n-1"
             Qc   = max(0.0D0,cm%D%Q0c + (1.0D0/cm%Tau_convec)*(cm%a1*elem(ie)%state%teb(i,j,nm1) &
                  + cm%a2*elem(ie)%state%qmc(i,j,nm1) -cm%a0*(t1(i,j)+ cm%gamma2*t2(i,j))))
             Hd   = ((1.0D0 -lws)/(1.0D0 - cm%lambdastar))*Qc
             D0   = (cm%D%m0/cm%Q0R1)*max(0.0D0,(cm%Q0R1 + cm%mu*(elem(ie)%state%Hs(i,j,nm1) &
                  - elem(ie)%state%Hc(i,j,nm1))))*deltem

             ! At "n"
             tem0    = elem(ie)%state%qmc(i,j,n0) + cm%D%psi1avg*(t1_0(i,j)+ cm%alpha2*t2_0(i,j)) !*cm%psi2avg)
             deltem0 = elem(ie)%state%teb(i,j,n0)-tem0+cm%TebMinTem
            ...
            ! end if
             ...
             Qc0    = max(0.0D0,cm%D%Q0c + (1.0D0/cm%Tau_convec)*(cm%a1*elem(ie)%state%teb(i,j,n0) &
                  + cm%a2*elem(ie)%state%qmc(i,j,n0) -cm%a0*(t1_0(i,j)+ cm%gamma2*t2_0(i,j)))) 
             Hd0    = ((1.0D0 -lws0)/(1.0D0 - cm%lambdastar))*Qc0
             D00    = (cm%D%m0/cm%Q0R1)*max(0.0D0,(cm%Q0R1 + cm%mu*(elem(ie)%state%Hs(i,j,n0) &
                  - elem(ie)%state%Hc(i,j,n0))))*deltem0
             ...
             if(cm%closure == 1)then
               ...
               D0 =lws0*D00
                ...
                Qcongest = max(0.0D0,cm%D%Q0c + (1.0D0/cm%Tau_convec)*(elem(ie)%state%teb(i,j,nm1) &
                   -cm%a0prime*(t1(i,j)+ cm%gamma2prime*t2(i,j))))
                Qcongest0 = max(0.0D0,cm%D%Q0c + (1.0D0/cm%Tau_convec)*(elem(ie)%state%teb(i,j,n0) &
             ...
             !Pbig = max(0.0D0,Hd*cm%psi1avgtrunc + (elem(ie)%state%Hc(i,j,nm1) - elem(ie)%state%Hs(i,j,nm1))*cm%psi2avgtrunc) ! Precipitation
             !Pbig0 = max(0.0D0,Hd0*cm%psi1avgtrunc + (elem(ie)%state%Hc(i,j,n0) - elem(ie)%state%Hs(i,j,n0))*cm%psi2avgtrunc) ! Precipitation
             Pbig  = Hd*cm%D%psi1avgtrunc  + (1.0D0-cm%csr)*(elem(ie)%state%Hc(i,j,nm1) - elem(ie)%state%Hs(i,j,nm1))*cm%D%psi2avgtrunc ! Precipitation
             Pbig0 = Hd0*cm%D%psi1avgtrunc + (1.0D0-cm%csr)*(elem(ie)%state%Hc(i,j,n0) - elem(ie)%state%Hs(i,j,n0))*cm%D%psi2avgtrunc ! Precipitation


             ! just to store the info
             ! (Pmc is one dt back)
             elem(ie)%state%Pmc(i,j) = Pbig0
             elem(ie)%state%Hd(i,j)  = Hd0
             ...
             if(cm%closure == 1)then
                elem(ie)%state%D(i,j)   = lws*D0/cm%Htall
             else
                elem(ie)%state%D(i,j)   = D0/cm%Htall
             end if
             ...
             elem(ie)%state%qmc(i,j,np1) = elem(ie)%state%mv(i,j)*(elem(ie)%state%qmc(i,j,nm1) &
                  - dt2*(div(i,j)+divubar(i,j)+divpert(i,j))*rmetdet(i,j) &
                  + dt2*(-Pbig0 + D0/cm%Htall)*elem(ie)%state%mask(i,j))             
             
             ! Boundary layer potential temperature
             
             !elem(ie)%state%teb(i,j,np1) = elem(ie)%state%mv(i,j)*(elem(ie)%state%teb(i,j,nm1) - dt2*(tebtens) &
             !     + dt2*((cm%TstarMinTeb-elem(ie)%state%teb(i,j,n0))*cm%D%Tau_evap_Inv -D0/cm%h)*elem(ie)%mask(i,j))
             
             ! added the varying Sea surface temperature:
             !elem(ie)%state%teb(i,j,np1) = elem(ie)%state%mv(i,j)*(elem(ie)%state%teb(i,j,nm1) - dt2*(tebtens) &
              !    + dt2*((elem(ie)%state%delthetasurf(i,j)-elem(ie)%state%teb(i,j,n0))*cm%D%Tau_evap_Inv -D0/cm%h)*elem(ie)%mask(i,j))
          

             !semi-analytic integration of theta_eb equation, to prevent stiffness: Oct 12, 2009
            
              Bcst    = (cm%D%m0/cm%Q0R1)*max(0.0D0,(cm%Q0R1 + cm%mu*(elem(ie)%state%Hs(i,j,n0) &
                  - elem(ie)%state%Hc(i,j,n0))))/cm%h
            
             Acst = cm%D%Tau_evap_Inv + Bcst  
             tem0 = tem0 - cm%TebMinTem
 
             elem(ie)%state%teb(i,j,np1) = elem(ie)%state%mv(i,j)*((elem(ie)%state%teb(i,j,nm1)*dexp(-Acst*dt2) +&
                                           (elem(ie)%state%delthetasurf(i,j)*cm%D%Tau_evap_Inv + Bcst*tem0)*(1.d0 - dexp(-Acst*dt2))/Acst)&
                    *elem(ie)%state%mask(i,j) - dt2*tebtens)

                 
             
             ! ODE 1
             elem(ie)%state%Hs(i,j,np1)  = (elem(ie)%state%Hs(i,j,n0) &
                  +   an*dt*(cm%alpha_strat*Hd0 -elem(ie)%state%Hs(i,j,n0))*elem(ie)%state%mask(i,j)/cm%Tau_strat &
                  + anm1*dt*(cm%alpha_strat*Hd -elem(ie)%state%Hs(i,j,nm1))*elem(ie)%state%mask(i,j)/cm%Tau_strat)

             ! ODE 2
             elem(ie)%state%Hc(i,j,np1)  = (elem(ie)%state%Hc(i,j,n0) &
                  +  an*dt*(Qcongest0*cm%alpha_congest*(lws0-cm%lambdastar)/(1.0D0-cm%lambdastar)&
                  -elem(ie)%state%Hc(i,j,n0))*elem(ie)%state%mask(i,j)/cm%Tau_congest &
                  +anm1*dt*(Qcongest*cm%alpha_congest*(lws-cm%lambdastar)/(1.0D0-cm%lambdastar)&
                  -elem(ie)%state%Hc(i,j,nm1))*elem(ie)%state%mask(i,j)/cm%Tau_congest)
          enddo
       enddo

       if(Debug)print *,"qmc = ",sum(abs(elem(ie)%state%qmc(:,:,:)))
       if(Debug)print *,"teb = ",sum(abs(elem(ie)%state%teb(:,:,:)))
       if(Debug)print *,"Hs  = ",sum(abs(elem(ie)%state%Hs(:,:,:)))
       if(Debug)print *,"Hc  = ",sum(abs(elem(ie)%state%Hc(:,:,:)))
       if(Debug)print *,"t1  = ",sum(abs(t1(:,:)))
       if(Debug)print *,"t2  = ",sum(abs(t2(:,:)))

       call edgeVpack(edgeMc,elem(ie)%state%qmc(:,:,np1),1,0,elem(ie)%desc)
       call edgeVpack(edgeMc,elem(ie)%state%teb(:,:,np1),1,1,elem(ie)%desc)
    ...
    do ie=nets,nete
       call edgeVunpack(edgeMc,elem(ie)%state%qmc(:,:,np1),1,0,elem(ie)%desc)
       call edgeVunpack(edgeMc,elem(ie)%state%teb(:,:,np1),1,1,elem(ie)%desc)

       do j=1,np	
          do i=1,np
             elem(ie)%state%qmc(i,j,np1) = elem(ie)%state%rmv(i,j)*elem(ie)%state%qmc(i,j,np1)
             elem(ie)%state%teb(i,j,np1) = elem(ie)%state%rmv(i,j)*elem(ie)%state%teb(i,j,np1)
          enddo
       enddo
    enddo
    ...
    do ie=nets,nete
       ...
       do j=1,np	
          do i=1,np
             ! clip
             !elem(ie)%state%qmc(i,j,np1)  = max(1.e-12_real_kind, elem(ie)%state%qmc(i,j,np1))

             ! Robert-Asselin filter
             elem(ie)%state%qmc(i,j,n0)  = elem(ie)%state%qmc(i,j,n0) + &
                  smooth*(elem(ie)%state%qmc(i,j,nm1) - &
                  2.0D0*elem(ie)%state%qmc(i,j,n0) + elem(ie)%state%qmc(i,j,np1))

             elem(ie)%state%teb(i,j,n0)  = elem(ie)%state%teb(i,j,n0) + &
                  smooth*(elem(ie)%state%teb(i,j,nm1) - &
                  2.0D0*elem(ie)%state%teb(i,j,n0) + elem(ie)%state%teb(i,j,np1))
          enddo
       enddo
       ...
       do k=1,nlev
          do j=1,np	
             do i=1,np
                elem(ie)%state%QHeating(i,j,k) =    (1.4D0*((Hd2D(i,j,ie)*cm%D%psitrunc(k,1) &
                     + (elem(ie)%state%Hc(i,j,n0) - elem(ie)%state%Hs(i,j,n0))*(cm%D%psitrunc(k,2) - cm%csr*cm%D%psi2avgtrunc)) - cooling(k))*elem(ie)%state%mask(i,j) &
                     +  cm%Tau_r*elem(ie)%state%pot0(i,j,k))*(p(i,j,k)/p0)**(kappa) &
       ...
       do k=1,nlev
          ...
          if(Debug) print *,"Tau_r*pot0*conversion  = ",sum(abs(cm%Tau_r*elem(ie)%state%pot0(:,:,k)*(p(:,:,k)/ps(:,:))**(kappa)))
          ...
          if(Debug) print *,"Convective heating     = ",sum(abs((Hd2D(:,:,ie)*cm%D%psitrunc(k,1) &
               + (elem(ie)%state%Hc(:,:,n0) - elem(ie)%state%Hs(:,:,n0))*cm%D%psitrunc(k,2))))/REAL(np*np,kind=real_kind)
       end do

      if(Debug)print *,"QHeating  = ",sum(abs(elem(ie)%state%QHeating(:,:,:)))
  ...
  subroutine multicloud_diffusion(elem, nets,nete,np1,deriv,dt2,hybrid)
    ...
    do ie=nets,nete
       ...
       gradqmc_tmp(:,:,:) = gradient(elem(ie)%state%qmc(:,:,np1),deriv)
       gradteb_tmp(:,:,:) = gradient(elem(ie)%state%teb(:,:,np1),deriv)
    ...
    do ie=nets,nete
       ...
       do j=1,np
          do i=1,np
             grad_qmc_np1(i,j,1,ie) = rmv(i,j)*grad_qmc_np1(i,j,1,ie)
             grad_qmc_np1(i,j,2,ie) = rmv(i,j)*grad_qmc_np1(i,j,2,ie)
             grad_teb_np1(i,j,1,ie) = rmv(i,j)*grad_teb_np1(i,j,1,ie)
             grad_teb_np1(i,j,2,ie) = rmv(i,j)*grad_teb_np1(i,j,2,ie)
          end do
       end do
    ...
    do ie=nets,nete
       ...
       do j=1,np
          do i=1,np
             lap_qmc_np1(i,j,ie)   = rmv(i,j)*lap_qmc_np1(i,j,ie)
             lap_teb_np1(i,j,ie)   = rmv(i,j)*lap_teb_np1(i,j,ie)
             elem(ie)%state%qmc(i,j,np1)   = elem(ie)%state%qmc(i,j,np1)   + nu_mc*dt2*lap_qmc_np1(i,j,ie)
             elem(ie)%state%teb(i,j,np1)   = elem(ie)%state%teb(i,j,np1)   + nu_mc*dt2*lap_teb_np1(i,j,ie)
          end do
       end do
  ...
  !
  subroutine ApplyShearDamping(elem,tau_damp,hybrid,tl,nets,nete)
    ...
    do ie=nets,nete
       ...
       call interpolate_scalar(interpdata(ie),elem(ie)%state%ubar(:,:,1,tl%nm1),np,var(st:en))
