...
!===============================================================================
  ...
  subroutine shr_strdata_init( &
       SDAT,mpicom,compid,name,scmmode,iop_scream,scmlon,scmlat, &
    ...
    logical               ,intent(in),optional :: iop_scream
    ...
    call shr_strdata_init_model_domain(SDAT, mpicom, compid, my_task, &
         ...
         scmmode=scmmode, iop_scream=iop_scream, scmlon=scmlon, scmlat=scmlat)
  ...
  subroutine shr_strdata_init_model_domain(SDAT, mpicom, compid, my_task, &
       gsmap, ggrid, nxg, nyg, nzg, scmmode, iop_scream, scmlon, scmlat, &
    ...
    if (present(iop_scream)) then
      liop_scream=iop_scream
    endif
          ...
          if (SDAT%nstreams > 0) then
             ...
             if (lscmmode) then
                call shr_dmodel_readgrid(SDAT%grid, SDAT%gsmap, SDAT%nxg, SDAT%nyg, SDAT%nzg, &
                     ...
                     scmmode=lscmmode, iop_scream=liop_scream, scmlon=scmlon, scmlat=scmlat)
             else
                call shr_dmodel_readgrid(SDAT%grid,SDAT%gsmap, SDAT%nxg, SDAT%nyg, SDAT%nzg, &
                     fileName, compid, mpicom, &
                     decomp=decomp, lonName=lonName, latName=latName, hgtName=hgtName, &
