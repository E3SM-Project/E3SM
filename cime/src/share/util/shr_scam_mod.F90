...
subroutine shr_scam_checkSurface(scmlon, scmlat, iop_scream, ocn_compid, ocn_mpicom, &
...
! !INPUT/OUTPUT PARAMETERS:
   ...
   logical,                      intent(in)  :: iop_scream ! iop_scream logical
...
!-------------------------------------------------------------------------------
   ...
   if (ocn_exists) then
      ...
      call shr_dmodel_readgrid(SCAMSDAT%grid,SCAMSDAT%gsmap,SCAMSDAT%nxg,SCAMSDAT%nyg,SCAMSDAT%nzg, &
           ...
           scmmode=.true.,iop_scream=iop_scream,scmlon=scmlon,scmlat=scmlat)
