...
module shr_ncread_mod
  ...
  end interface shr_ncread_varDimSize

  interface shr_ncread_dimSize  ; module procedure &
       shr_ncread_dimSizeName
  end interface shr_ncread_dimSize
  ...
  end interface shr_ncread_tField
...
contains
  ...
  logical function shr_ncread_varExists(fileName, varName)
    ...
    rcode = nf90mpi_inq_varid(fid,trim(varName),vid)
  ...
  subroutine shr_ncread_varDimNum(fileName, varName, ns, rc)
    ...
    rcode = nf90mpi_inquire_variable(fid,vid,ndims=ns)
  ...
  subroutine shr_ncread_field4dG(fn, fldName, rfld, ifld, &
    ...
    rCode = nf90mpi_inq_varid(fid,trim(fldName),vid)
    call shr_ncread_handleErr(rCode,subName//'inq varid vid: '//trim(fldName))
    rCode = nf90mpi_inquire_variable(fid,vid,xtype=xtype,ndims=ndims)
  ...
  subroutine shr_ncread_handleErr(rCode, str)
    ...
    if (rCode /= nf90_noerr) then
       write(s_logunit,F00) "netCDF error: ",trim(nf90mpi_strerror(rCode))
