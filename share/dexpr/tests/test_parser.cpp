...
TEST_CASE("parser: check string literals") {
  check_parse(R"(name == "hello")", R"((name=='hello'))");
  check_parse(R"(name == 'hello')", R"((name=='hello'))");
  check_parse(R"(name == "It's me")", R"((name=="It's me"))");
  check_rejected(R"(name == "say "hello"")","Unexpected trailing input");
}

TEST_CASE("some parsed expression", "[parser]") {
  check_parse("x*y.derivative(dx=dy,['col']).where(x>0)",
              "(x*y.derivative((dx=dy), ['col']).where((x>0)))");
  check_parse("x.where(condition=a < b and c != d)",
              "x.where((condition=((a<b) and (c!=d))))");
}
