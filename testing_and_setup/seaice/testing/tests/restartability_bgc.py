...
def restartability_bgc(mpasDevelopmentDir, domainsDir, domain, configuration, options, check):

    # find available directory name
    iTest = 1
    dirExists = True
    while (dirExists):
        testDir = "restartability_bgc_%i.%s.%s" %(iTest,configuration,domain)
        iTest = iTest + 1
    ...
    title = "Test: Restartability BGC, Configuration: %s, Domain: %s" %(configuration,domain)
    ...
    failed = test_summary(nErrorsNonArray, nErrorsArray, logfile, "restartability_bgc")
