...
for i in range(ntests):
    ...
    for j in range(len(grids)):
        ...
        for k in range(len(tracers)):
            tracer = tracers[k]
            sol = np.reshape(ncfileIC.variables[tracer+'Tend'][0,:,iz],[ny,nx])
            var = np.reshape(ncfile.variables[tracer+'Tend'][0,:,iz],[ny,nx])
            dif = abs(var[1:ny-1,1:nx-1]-sol[1:ny-1,1:nx-1])
            err = abs((var[1:ny-1,1:nx-1]-sol[1:ny-1,1:nx-1])/sol[1:ny-1,1:nx-1])
...
for i in range(ntests):
    ...
    for k in range(len(tracers)):
        tracer = tracers[k]
        plt.loglog(dx,difL2[i,:,k]/np.max(difL2[i,:,k]),'-x',label=tracer)
