...
fig, axs = plt.subplots(nRow,nCol,figsize=(5.3*nCol,2.0*nRow),constrained_layout=True)
...
for iRow in range(nRow):
    ...
    for iCol in range(nCol):
        ax = axs[iRow, iCol]
        dis = ax.imshow(var[iTime[iCol],0:512:4,:].T,extent=[0,120,20,0],aspect=2,cmap='jet',vmin=5,vmax=30)
        if iRow==nRow-1:
            ax.set_xlabel('x, km')
        if iCol==0:
