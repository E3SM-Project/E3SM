...
#def  EC_CellWidthVsLat(lat, varargin)
#'''
...
#densityEC = zeros(size(lat))
#cellWidthOut = zeros(size(lat))
#for j in range(length(lat)
#  if abs(lat[j])<latTransition
#    densityEC[j] = ((densityEq-densityMidLat) * (1.0 + tanh( (latPosEq - abs(lat[j]))/ latWidthEq)) / 2.0) + densityMidLat
#  else:
#    densityEC[j] = ((densityMidLat-densityPole) * (1.0 + tanh( (latPosPole - abs(lat[j]))/ latWidthPole)) / 2.0) + densityPole
#  
