...
parser = OptionParser()
parser.add_option("-m", "--mpas", dest="mpasfile", help="input MPAS netCDF file.", metavar="FILE")
parser.add_option("-t", "--triangle", dest="trifile", help="output file name template to be in triangle format (FILE.1.node, FILE.1.ele).", metavar="FILE")
...
# write node file header:   First line: <# of vertices> <dimension (must be 2)> <# of attributes> <# of boundary markers (0 or 1)> 
fnode.write( "{:d} 2 0 1\n".format(nCells) )
...
for i in range(nCells):
   if ConC[i, 0:nConC[i]].min() == 0:
      ...
      isBdy = 0
   fnode.write( "{:d} {:f} {:f} {:d}\n".format(i+1, xCell[i], yCell[i], isBdy) )
...
# write ele file header:  First line: <# of triangles> <nodes per triangle> <# of attributes>
fele.write( "{:d} 3 0\n".format( numtri ) )
...
for i in range(nVertices):
   ...
   if ConV[i,:].min() > 0:
      cnt += 1
      fele.write( "{:d} {:d} {:d} {:d}\n".format(cnt, ConV[i,0], ConV[i,1], ConV[i,2]) )
