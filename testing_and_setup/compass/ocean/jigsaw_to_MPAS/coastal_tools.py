...
Alaska = {"include": [np.array([-170.0, -141.0, 49.0, 72.0]),
                      ...
                      # Connects AK to CA
                      np.array([-129.5, -121.0, 49.0, 55.0])
                      ],
          "exclude": []}
...
def compute_cell_width(D, cell_width, lon, lat, dx_min, trans_start, trans_width, restrict_box,  # {{{
    ...
    backgnd_weight = .5 * \
        (np.tanh((D - trans_start - .5 * trans_width) / (.2 * trans_width)) + 1)
...
def get_data_inside_box(lon, lat, data, box, idx=False):
    ...
    if not idx:
        ...
        except BaseException:  # NetCDF indexing
...
def get_indices_inside_quad(lon, lat, box):
    ...
    # Initialize the coordinate vectors for points inside convex hull of quad
    # region
    r = 0.0 * R[idx]
    s = 0.0 * S[idx]
    x = X[idx]
