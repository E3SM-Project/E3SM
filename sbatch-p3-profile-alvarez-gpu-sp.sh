#!/bin/bash
# Profile one standalone P3 SP run on an Alvarez GPU node.
#
...
# node with NERSC_HOST=alvarez ./build-p3-test-gpu-sp.sh.
#
# Examples:
#   sbatch sbatch-p3-profile-alvarez-gpu-sp.sh
...
#SBATCH --gpus=4
#SBATCH -t 00:30:00
#SBATCH -J p3_profile_alvarez-gpu-sp
...
TEST_EXE="${REPO_ROOT}/build_p3_profile_alvarez-gpu-SP/src/physics/p3/tests/p3_run_and_cmp"
PROFILE_TOOL="${PROFILE_TOOL:-nsys}"
SCRATCH_ROOT="${SCRATCH:-/pscratch/sd/${USER:0:1}/${USER}}"
RESULTS_DIR="${RESULTS_DIR:-${SCRATCH_ROOT}/e3sm/p3-profile-results/alvarez-gpu-sp/${SLURM_JOB_ID:-manual}}"
