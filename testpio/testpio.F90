...
! Code name, used in debug prints and passed to called routines for flow tracing
    ...
    type (Var_desc_t)   :: vard_r8,vard_r4,vard_i4, &
...
!DBG    print *,'testpio: before call to t_initf'
#ifdef TIMING
    !---------------------------------------------------------------
    ! timing library
    !---------------------------------------------------------------
!DBG    print *,'testpio: point #1'
    call t_initf(nml_filename, logprint=.false., logunit=6, &
             mpicom=MPI_COMM_WORLD, MasterTask=log_master_task)
!DBG    print *,'testpio: point #2'
    call t_startf('testpio_total')

!DBG    print *,'testpio: after call to t_startf'
    !-----------------------------------------------------------------------------
    ! Memory test
    !-----------------------------------------------------------------------------
    call get_memusage(msize,mrss0)
!DBG    print *,'testpio: after get_memusage #3'
    allocate(mem_tmp(1024*1024))    ! 1 MWord, 8 MB
    mem_tmp = -1.0
    call get_memusage(msize,mrss1)
!DBG    print *,'testpio: after get_memusage #4'
    deallocate(mem_tmp)
    call get_memusage(msize,mrss2)
!DBG    print *,'testpio: after get_memusage #5'
    mb_blk = 0.0_r8
!DBG    print *,'testpio: after get_memusage #6'
...
!DBG    print *,'testpio: before call to readTestPIO_Namelist'
    if(my_task == master_task) then 
       call ReadTestPIO_Namelist(nml_in, nprocs, nml_filename, myname, nml_error)
    endif
!DBG print *,'testpio: before call to broadcast_namelist'
    call MPI_barrier(MPI_COMM_WORLD,ierr)
    call Broadcast_Namelist(myname, my_task, master_task, MPI_COMM_WORLD, ierr)
!DBG print *,'testpio: after call to broadcast_namelist'
...
!DBG       print *,'iam: ',PIOSYS%comp_rank,'testpio: point #1'
       call gdecomp_read_nml(gdecomp,nml_filename,'comp',PIOSYS%comp_rank,PIOSYS%num_tasks,gDims3D)
!DBG       print *,'iam: ',PIOSYS%comp_rank,'testpio: point #2'
       call gdecomp_DOF(gdecomp,PIOSYS%comp_rank,compDOF,start,count)
!DBG       print *,'iam: ',PIOSYS%comp_rank,'testpio: point #3'
    else
       call pio_readdof(trim(compdof_input),compDOF,MPI_COMM_WORLD,75)
       sdof = size(compDOF)
       start = gDims3D   ! min tmp
       count = 0         ! max tmp
       do n = 1,sdof
...
!DBG       write(6,*) trim(myname),' my_task,sdof,start,count = ',my_task,sdof,start,count
    endif
...
!DBG       print *,'iam: ',PIOSYS%comp_rank,'testpio: point #4'
           call gdecomp_read_nml(gdecomp,nml_filename,'io',PIOSYS%io_rank,PIOSYS%num_iotasks,gDims3D)
!DBG       print *,'iam: ',PIOSYS%comp_rank,'testpio: point #5'
           call gdecomp_DOF(gdecomp,PIOSYS%io_rank,ioDOF,start,count)
!DBG       print *,'iam: ',PIOSYS%comp_rank,'testpio: point #6'
...
!DBG    if(Debug) print *,'comp_rank,io_rank: ',piosys%comp_rank,piosys%io_rank,' ioDOF ',ioDOF
    ...
    if (trim(rearr) == 'box') then
       ...
       if (trim(iodof_input) == 'namelist') then
!DBG       print *,'iam: ',PIOSYS%comp_rank,'testpio: point #7'
...
!DBG       print *,'iam: ',PIOSYS%comp_rank,'testpio: point #8'
       else
...
!DBG       print *,'iam: ',PIOSYS%comp_rank,'testpio: point #9'
    ...
    enddo
!DBG       print *,'iam: ',PIOSYS%comp_rank,'testpio: point #10'
...
!DBG       print *,'iam: ',PIOSYS%comp_rank,'testpio: point #11'
...
do it=1,maxiter
   ...
   fname    = TRIM(dir)//'foo.'//TRIM(Iofmtd)
   fname_r8 = TRIM(dir)//'foo.r8.'//TRIM(Iofmtd)
   fname_r4 = TRIM(dir)//'foo.r4.'//TRIM(Iofmtd)
   fname_i4 = TRIM(dir)//'foo.i4.'//TRIM(Iofmtd)
...
!   print *, __FILE__,__LINE__,'>',fname_r4,'<'
   ...
   ! Set Frame to '1' in the PIO descriptor file
    ...
    one = 1
    call PIO_SetFrame(vard_r8,one)
    call PIO_SetFrame(vard_r4,one)
   ...
   if(iotype == iotype_pnetcdf .or. & 
      iotype == iotype_netcdf ) then
       ...
       if(TestR8) then 
          ...
          iostat = PIO_def_var(File_r8,'field',PIO_double,(/dimid_x,dimid_y,dimid_z/),vard_r8)
       ...
       if(TestR4) then 
          ...
          iostat = PIO_def_var(File_r4,'field',PIO_real,(/dimid_x,dimid_y,dimid_z/),vard_r4)
...
#endif
      call PIO_write_darray(File_r8,vard_r8, iodesc_r8, test_r8wr, iostat)
#ifdef TIMING
      call t_stopf('testpio_write')
#endif
      et = MPI_Wtime()
      dt_write_r8 = dt_write_r8 + et - st
   ...
   if(TestR4) then
      ...
      call t_startf('testpio_write')
#endif
      call PIO_write_darray(File_r4,vard_r4,iodesc_r4, test_r4wr,iostat)
#ifdef TIMING
      call t_stopf('testpio_write')
#endif
      et = MPI_Wtime()
      dt_write_r4 = dt_write_r4 + et - st
...
!   if(TestCombo) ierr = PIO_OpenFile(PIOSYS,File,iotype,fname)
   ...
   if(iotype == iotype_pnetcdf .or. &
      ...
      if(TestR8) then 
         iostat = PIO_inq_varid(File_r8,'field',vard_r8)
         call check_pioerr(iostat,__FILE__,__LINE__,' r8 inq_varid')
      endif

      if(TestR4) then 
         iostat = PIO_inq_varid(File_r4,'field',vard_r4)
...
#endif
      call PIO_read_darray(File_r8,vard_r8,iodesc_r8,test_r8rd,iostat)
#ifdef TIMING
   ...
   if(TestR4) then
      ...
      call t_startf('testpio_read')
#endif
      call PIO_read_darray(File_r4,vard_r4,iodesc_r4,test_r4rd,iostat)
#ifdef TIMING
      call t_stopf('testpio_read')
#endif
      et = MPI_Wtime()
   ...
   if(TestR8) then
      call checkpattern(fname_r8,test_r8wr,test_r8rd,iostat)
      call check_pioerr(iostat,__FILE__,__LINE__,' checkpattern r8 test')
   endif

   if(TestR4) then
      call checkpattern(fname_r4,test_r4wr,test_r4rd,iostat)
      call check_pioerr(iostat,__FILE__,__LINE__,' checkpattern r4 test')
   endif

   if(TestInt) then
      call checkpattern(fname_i4, test_i4wr,test_i4rd,iostat)
   ...
   if(TestCombo) then 
     ...
     !-----------------------------
     call checkpattern(fname,test_r8wr,test_r8rd,iostat)
     call check_pioerr(iostat,__FILE__,__LINE__,' checkpattern test_r8 ')

     call checkpattern(fname,test_r4wr,test_r4rd,iostat)
     call check_pioerr(iostat,__FILE__,__LINE__,' checkpattern test_r4 ')

     call checkpattern(fname,test_i4wr,test_i4rd,iostat)
...
enddo ! do it=1,maxiter
   ...
   if(TestR8 .and. (piosys%io_rank == 0) ) then
     call WriteTimeTrialsStats(casename,TestR8CaseName, fname_r8, Iodesc_r8, gdt_read_r8, gdt_write_r8, maxiter) 
   endif

   if(TestR4 .and. (piosys%io_rank == 0) ) then
     call WriteTimeTrialsStats(casename,TestR4CaseName, fname_r4, Iodesc_r4, gdt_read_r4, gdt_write_r4, maxiter) 
   endif

   if(TestInt .and. (piosys%io_rank == 0) ) then
     call WriteTimeTrialsStats(casename,TestI4CaseName, fname_i4, Iodesc_i4, gdt_read_i4, gdt_write_i4, maxiter) 
   endif
...
!=============================================================================
 ...
 subroutine WriteStats(CaseName, FileName, ioDesc, trialNo, dtRead, dtWrite)
  ...
  case default
	write(*,'(4a)') myname_,':: Case ',CaseName,' not supported.  Returning without writing output.'
        ...
        print *,'Record bytes: ',INT(ioDesc%glen*datumSize,kind=i8), &
	                         INT(ioDesc%glen*datumSize,kind=i8)
        print *,'-----------------------------------------'
        print *,'Type of I/O performed: ',Iofmtd
        print *,' File name: ',FileName
        print *,'-----------------------------------------'
        print *, CaseName,' Trial No. ',trialNo,' Read time: ',dtRead, &
	        'Read Mbytes/sec: ',MBYTES*ioDesc%glen*datumSize/dtRead
        print *, CaseName,' Trial No. ',trialNo,' Write time: ',dtWrite, &
	         'Write Mbytes/sec: ',MBYTES*ioDesc%glen*datumSize/dtWrite
        print *,'-----------------------------------------'
...
!=============================================================================
 ...
 subroutine WriteTimeTrialsStats(casename,TestName, FileName, ioDesc, ReadTimes, WriteTimes, nTrials) 
  ...
  TotalMBytes = MBYTES * ioDesc%glen * datumSize
