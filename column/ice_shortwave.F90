...
!-----------------------------------------------------------------------
      ...
      ! compute aerosol mass path
         ...
         if (netsw > puny) then ! sun above horizon
            ...
            endif ! end if using 5band snicar radiative subroutine
...
! author:  Bruce P. Briegleb, NCAR
!   2013:  E Hunke merged with NCAR version
!   2018:  Cheng Dang merged with SNICAR, UC Irvine
!
! Note by Cheng Dang 2018:
! This subroutine kept the radiative transfer method, snow-covered sea ice layer
! sturcture, and most of the code structures of subroutine compute_dEdd, with major
! changeds listed below to merge current snow treatments in SNICAR Model
...
! 5. This subroutine can replace subroutine compute_dEdd to compute bare sea ice
!    and ponded sea ice, after finish testing
!
! The justification and explaination for above changes can be find in paper:
!    Dang et al., 2018, in prep
!
...
! density and grain radius, along with ice and pond depths, then computes
! layer by layer Delta-Eddington reflectivity, transmissivity and combines
! layers (done by calling routine). Finally, surface albedos
! and internal fluxes/flux divergences are evaluated.
!
!  Description of the level and layer index conventions. This is
!  for the standard case of one snow layer and four sea ice layers.
...
!-----------------------------------------------------------------------
      ...
      ! FUTURE-WORK: adding 5-bd sea ice iops when become avalible
      ...
      ! FUTURE-WORK: adding 5-bd aerosol IOPs
      ! note the aerosol IOPs are avaliable from SNICAR
      ! while we need to discuss how to match aerosol types and etc
     ...
     integer (kind=int_kind) :: &
         ...
         ns_aer_tmp       ! temporary index for aersol IOPs, will be removed soon
     ...
     real (kind=dbl_kind), dimension (0:klevp) :: &
      ...
      ! solar zenith angle parameterizations
       real (kind=dbl_kind), parameter :: &
          sza_a0 =  0.085730_dbl_kind,  & ! pure ice mass density (kg/m3)
          sza_a1 = -0.630883_dbl_kind,  & ! snow grain adjustment factor max
          sza_a2 =  1.303723_dbl_kind,  & ! snow grain adjustment factor min
          sza_b0 =  1.467291_dbl_kind,  & ! pure ice mass density (kg/m3)
          sza_b1 = -3.338043_dbl_kind,  & ! snow grain adjustment factor max
          sza_b2 =  6.807489_dbl_kind,  &
          mu_75  =  0.2588_dbl_kind
       ...
       ! absorption coefficient for chlorophyll (/m)
       real (kind=dbl_kind), dimension(nspint_5bd, 0:klev) :: &
           kabs_chl_5bd
...
!      wghtns(3) = cp33 + (cp22-cp33)*(c1-fnidr)
      ...
      ! Todo-CD: see if these weights depends on fnidr similar to 3-band treatment
      ! spectral weights - 5 bands (use SNICAR for now)
...
!ech: note this is highly resolution dependent!
      ...
      ! pond IOPs is not needed for this subroutine (snow-covered sea ice only)
      ! this section of code is kept for future mearge between 5band and 3 band
      ! subroutines
      ! adjust ponded ice iops with tuning parameters -
!      if( R_pnd >= c0 ) then
!        do ns = 1, nspint
!          sigp         = ki_p_ssl_mn_5bd(ns)*wi_p_ssl_mn_5bd(ns)*(c1+fp_pnd*R_pnd)
!          ki_p_ssl_5bd(ns) = sigp+ki_p_ssl_mn_5bd(ns)*(c1-wi_p_ssl_mn_5bd(ns))
!          wi_p_ssl_5bd(ns) = sigp/ki_p_ssl_5bd(ns)
!          gi_p_ssl_5bd(ns) = gi_p_ssl_mn_5bd(ns)
!
!          sigp         = ki_p_int_mn_5bd(ns)*wi_p_int_mn_5bd(ns)*(c1+fp_pnd*R_pnd)
!          ki_p_int_5bd(ns) = sigp+ki_p_int_mn_5bd(ns)*(c1-wi_p_int_mn_5bd(ns))
!          wi_p_int_5bd(ns) = sigp/ki_p_int_5bd(ns)
!          gi_p_int_5bd(ns) = gi_p_int_mn_5bd(ns)
!        enddo
!      else !if( R_pnd < c0 ) then
!        do ns = 1, nspint
!          sigp         = ki_p_ssl_mn_5bd(ns)*wi_p_ssl_mn_5bd(ns)*(c1+fm_pnd*R_pnd)
!          sigp         = max(sigp, c0)
!          ki_p_ssl_5bd(ns) = sigp+ki_p_ssl_mn_5bd(ns)*(c1-wi_p_ssl_mn_5bd(ns))
!          wi_p_ssl_5bd(ns) = sigp/ki_p_ssl_5bd(ns)
!          gi_p_ssl_5bd(ns) = gi_p_ssl_mn_5bd(ns)
!
!          sigp         = ki_p_int_mn_5bd(ns)*wi_p_int_mn_5bd(ns)*(c1+fm_pnd*R_pnd)
!          sigp         = max(sigp, c0)
!          ki_p_int_5bd(ns) = sigp+ki_p_int_mn_5bd(ns)*(c1-wi_p_int_mn_5bd(ns))
!          wi_p_int_5bd(ns) = sigp/ki_p_int_5bd(ns)
!          gi_p_int_5bd(ns) = gi_p_int_mn_5bd(ns)
...
!      endif            ! adjust ponded ice iops
      ...
      !Todo-CD: aerosol IOPs will need to be replaced by the 5 band aerosol IOPs from CLM-SNICAR
!mgf++
      if (modal_aero) then
           do k=0,klev
              if (k < nslyr+1) then ! define indices for snow layer
      ...
      !  5-band aerosols IOPs assignment
      do k=0,klev
          tzaer_5bd(1:5,k) = (/tzaer(1,k),tzaer(2,k),tzaer(2,k),tzaer(3,k),tzaer(3,k)/)
          wzaer_5bd(1:5,k) = (/wzaer(1,k),wzaer(2,k),wzaer(2,k),wzaer(3,k),wzaer(3,k)/)
          gzaer_5bd(1:5,k) = (/gzaer(1,k),gzaer(2,k),gzaer(2,k),gzaer(3,k),gzaer(3,k)/)
      enddo
          ...
          do ns = 1, nspint_5bd
            ...
            do nsky = 1,2 ! loop for both direct beam and diffuse beam
              ...
              if (nsky == 1) then ! direc incident
                ...
                else if (ceiling(rsnw(ksnow)) - rsnw(ksnow) < 1.0e-3_dbl_kind) then
              ...
              else if (nsky == 2) then ! diffuse  incident
...
! ------------------------------------------------------------------------------
! Todo-CD: incoporating SNICAR 5-band aerosol optical properties --
          ! assign temporary index for aerosol optical property
          ! the current MPAS-Seaice simulation does not contain aerosols,
          ! so code regarding aerosols are
         if (ns == 1) then
            ns_aer_tmp = 1
         elseif (ns <= 3) then
            ns_aer_tmp = 2
...
! mgf++
   ...
   ! if na
               ...
               !interstitial BC
                  taer = taer + &
                       aero_mp(na)*kaer_bc_tab(ns_aer_tmp,k_bcexs(k))
                  waer = waer + &
                       aero_mp(na)*kaer_bc_tab(ns_aer_tmp,k_bcexs(k))* &
                       waer_bc_tab(ns,k_bcexs(k))
                  gaer = gaer + &
                       aero_mp(na)*kaer_bc_tab(ns_aer_tmp,k_bcexs(k))* &
                        waer_bc_tab(ns_aer_tmp,k_bcexs(k))*gaer_bc_tab(ns_aer_tmp,k_bcexs(k))
               elseif (na == 5)then
               !within-ice BC
                   taer = taer + &
                        aero_mp(na)*kaer_bc_tab(ns_aer_tmp,k_bcins(k))* &
                        bcenh(ns_aer_tmp,k_bcins(k),k_bcini(k))
                   waer = waer + &
                        aero_mp(na)*kaer_bc_tab(ns_aer_tmp,k_bcins(k))* &
                        waer_bc_tab(ns_aer_tmp,k_bcins(k))
                   gaer = gaer + &
                        aero_mp(na)*kaer_bc_tab(ns_aer_tmp,k_bcins(k))* &
                        waer_bc_tab(ns_aer_tmp,k_bcins(k))*gaer_bc_tab(ns_aer_tmp,k_bcins(k))
                else
                   ! other species (dust)
                   taer = taer + &
                        aero_mp(na)*kaer_tab(ns_aer_tmp,(1+(na-1)/4))
                   waer = waer + &
                        aero_mp(na)*kaer_tab(ns_aer_tmp,(1+(na-1)/4))* &
                        waer_tab(ns_aer_tmp,(1+(na-1)/4))
                   gaer = gaer + &
                        aero_mp(na)*kaer_tab(ns_aer_tmp,(1+(na-1)/4))* &
                        waer_tab(ns_aer_tmp,(1+(na-1)/4))*gaer_tab(ns_aer_tmp,(1+(na-1)/4))
                endif ! if na = 1
    ! end if na
              else
                   taer = taer + &
...
!mgf++
                 if (na==1) then
                   ! interstitial BC
                   taer = taer + &
                        (aero_mp(na+1)/rnslyr)*kaer_bc_tab(ns_aer_tmp,k_bcexs(k))
                   waer = waer + &
                        (aero_mp(na+1)/rnslyr)*kaer_bc_tab(ns_aer_tmp,k_bcexs(k))* &
                        waer_bc_tab(ns,k_bcexs(k))
                   gaer = gaer + &
                 ...
                 elseif (na==5) then
                   ! within-ice BC
                   taer = taer + &
                        (aero_mp(na+1)/rnslyr)*kaer_bc_tab(ns_aer_tmp,k_bcins(k))*&
                        bcenh(ns_aer_tmp,k_bcins(k),k_bcini(k))
                   waer = waer + &
                        (aero_mp(na+1)/rnslyr)*kaer_bc_tab(ns_aer_tmp,k_bcins(k))* &
                        waer_bc_tab(ns_aer_tmp,k_bcins(k))
                   ...
                   ! other species (dust)
                   taer = taer + &
                        (aero_mp(na+1)/rnslyr)*kaer_tab(ns_aer_tmp,(1+(na-1)/4))
                   waer = waer + &
                        (aero_mp(na+1)/rnslyr)*kaer_tab(ns_aer_tmp,(1+(na-1)/4))* &
                        waer_tab(ns,(1+(na-1)/4))
                   gaer = gaer + &
...
!mgf++
               if (modal_aero) then
                  ...
                  ! interstitial BC
                     taer = taer + &
                          aero_mp(na+2)*kaer_bc_tab(ns_aer_tmp,k_bcexs(k))
                     waer = waer + &
                          aero_mp(na+2)*kaer_bc_tab(ns_aer_tmp,k_bcexs(k))* &
                          waer_bc_tab(ns_aer_tmp,k_bcexs(k))
                     gaer = gaer + &
                          aero_mp(na+2)*kaer_bc_tab(ns_aer_tmp,k_bcexs(k))* &
                          waer_bc_tab(ns_aer_tmp,k_bcexs(k))*gaer_bc_tab(ns_aer_tmp,k_bcexs(k))
                  elseif (na==5) then
                  ! within-ice BC
                     taer = taer + &
                          aero_mp(na+2)*kaer_bc_tab(ns_aer_tmp,k_bcins(k))* &
                          bcenh(ns_aer_tmp,k_bcins(k),k_bcini(k))
                     waer = waer + &
                          aero_mp(na+2)*kaer_bc_tab(ns_aer_tmp,k_bcins(k))* &
                          waer_bc_tab(ns_aer_tmp,k_bcins(k))
                  ...
                  ! other species (dust)
                     taer = taer + &
                          aero_mp(na+2)*kaer_tab(ns_aer_tmp,(1+(na-1)/4))
                     waer = waer + &
                          aero_mp(na+2)*kaer_tab(ns_aer_tmp,(1+(na-1)/4))* &
                          waer_tab(ns_aer_tmp,(1+(na-1)/4))
                     gaer = gaer + &
...
!mgf++
                  if (modal_aero) then
                     if (na==1) then
                        ! interstitial BC
                        taer = taer + &
                             (aero_mp(na+3)/rnilyr)*kaer_bc_tab(ns_aer_tmp,k_bcexs(k))
                        waer = waer + &
                             (aero_mp(na+3)/rnilyr)*kaer_bc_tab(ns_aer_tmp,k_bcexs(k))* &
                             waer_bc_tab(ns_aer_tmp,k_bcexs(k))
                        gaer = gaer + &
                     ...
                     elseif (na==5) then
                        ! within-ice BC
                        taer = taer + &
                             (aero_mp(na+3)/rnilyr)*kaer_bc_tab(ns_aer_tmp,k_bcins(k))* &
                             bcenh(ns,k_bcins(k),k_bcini(k))
                        waer = waer + &
                             (aero_mp(na+3)/rnilyr)*kaer_bc_tab(ns_aer_tmp,k_bcins(k))* &
                             waer_bc_tab(ns_aer_tmp,k_bcins(k))
                        ...
                        ! other species (dust)
                        taer = taer + &
                             (aero_mp(na+3)/rnilyr)*kaer_tab(ns_aer_tmp,(1+(na-1)/4))
                        waer = waer + &
                             (aero_mp(na+3)/rnilyr)*kaer_tab(ns_aer_tmp,(1+(na-1)/4))* &
                             waer_tab(ns_aer_tmp,(1+(na-1)/4))
                        gaer = gaer + &
...
! Todo-CD: the above section needs to be modified to incoporate SNICAR 5-band aerosol
! optical properties
         ...
         do k=nslyr+2,nslyr+1+nilyr
            ...
            Iabs(ki) = Iabs(ki) &
                      ...
                      -(dfdir_snicar(kp)*swdr*wghtns_5bd_drc(ns)   &
                        + dfdif_snicar(kp)*swdf*wghtns_5bd_dfs(ns))
         enddo       ! k
      endif        ! ns = 1, ns > 1 check which spectra
