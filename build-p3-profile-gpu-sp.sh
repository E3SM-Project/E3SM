#!/bin/bash
# Build the standalone p3_run_and_cmp test in single precision with
...
case "${NERSC_HOST:-}" in
  perlmutter)
    MACHINE_NAME="pm-gpu"
    BUILD_DIR="${REPO_ROOT}/build_p3_profile_${MACHINE_NAME}-SP"
    ;;
  alvarez)
...
"${REPO_ROOT}/scripts_p3_test/build_p3_test.sh" \
  "$MACHINE_NAME" SP "$BUILD_DIR" Release ON
