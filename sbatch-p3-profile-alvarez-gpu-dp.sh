#!/bin/bash
# Profile one standalone P3 DP run on an Alvarez GPU node.
#
...
# node with ./build-p3-profile-gpu-dp.sh.
#
# Examples:
#   sbatch sbatch-p3-profile-alvarez-gpu-dp.sh
#   PROFILE_TOOL=ncu NCU_KERNEL_NAME='regex:.*p3_.*_disp.*' \
#     NCU_LAUNCH_SKIP=100 NCU_LAUNCH_COUNT=20 \
#     sbatch sbatch-p3-profile-alvarez-gpu-dp.sh
...
#SBATCH --gpus=4
#SBATCH -t 00:30:00
#SBATCH -J p3_profile_alvarez-gpu-dp
#SBATCH -o alvarez-gpu-p3-profile-results/o.p3_profile.%j.txt

set -euo pipefail

if [ -n "${NERSC_HOST:-}" ] && [ "$NERSC_HOST" != alvarez ]; then
  echo "Error: NERSC_HOST=${NERSC_HOST}; expected alvarez." >&2
  exit 1
fi

REPO_ROOT="${SLURM_SUBMIT_DIR:-$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)}"
TEST_EXE="${REPO_ROOT}/build_p3_profile_alvarez-gpu-DP/src/physics/p3/tests/p3_run_and_cmp"
PROFILE_TOOL="${PROFILE_TOOL:-nsys}"
SCRATCH_ROOT="${SCRATCH:-/pscratch/sd/${USER:0:1}/${USER}}"
RESULTS_DIR="${RESULTS_DIR:-${SCRATCH_ROOT}/e3sm/p3-profile-results/alvarez-gpu-dp/${SLURM_JOB_ID:-manual}}"
...
if [ ! -x "$TEST_EXE" ]; then
  ...
  echo "Build it first with ./build-p3-profile-gpu-dp.sh" >&2
  exit 1
fi
...
echo "CUDA runtime: ${CUDA_LIB_DIR}"
