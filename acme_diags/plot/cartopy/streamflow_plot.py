...
def plot_panel_seasonality_map(plot_type, fig, proj, export, color_list, panel, parameter):
    if plot_type == 'test':
        panel_index = 0
...
def plot_seasonality_map(export, parameter):
    ...
    original_file_path = os.path.join(original_output_dir, parameter.output_file_seasonality_map)
...
def plot_panel_annual_map(panel_index, fig, proj, export, bias_array, panel, parameter):
    ...
    if panel_type in ['test', 'ref']:
        ...
        if parameter.print_statements:
            ...
            print('Bias of mean annual discharge {} min={}, max={}'.format(panel_type, value_min, value_max))
    ...
    for gauge, i in zip(export, range(len(export))):
        ...
        if panel_type in ['test', 'ref']:
            ...
            normalized_value = (numpy.log10(value) - numpy.log10(value_min)) / (numpy.log10(value_max) - numpy.log10(value_min))
        elif panel_type == 'bias':
        ...
        # https://scitools.org.uk/iris/docs/v1.9.2/examples/General/projections_and_annotations.html
        # Place a single marker point for each gauge.
        plt.plot(lon, lat, marker='o',  markersize=2, color=color, transform=proj_function())
        # NOTE: the "plt.annotate call" does not have a "transform=" keyword,
        # so for this one we transform the coordinates with a Cartopy call.
        at_x, at_y = ax.projection.transform_point(lon, lat, src_crs=proj_function())
    ...
    # (left, bottom, width, height) in page coordinates
    cbax = fig.add_axes(
        (panel[panel_index][0] + 0.6635, panel[panel_index][1] + 0.0115, 0.0326, 0.1792))
    cmap = colors.ListedColormap(color_list)
    if panel_type in ['test', 'ref']:
    ...
    if panel_type in ['test', 'ref']:
        ...
        ticks = numpy.arange(int(value_min), int(value_max) + step_size, step_size)
        cbar.ax.tick_params(labelsize=9.0, length=0)
...
def plot_annual_map(export, bias, parameter):
    ...
    original_file_path = os.path.join(original_output_dir, parameter.output_file_annual_map)
...
def plot_annual_scatter(xs, ys, zs, parameter):
    ...
    ax.set_xscale('log')
    ax.set_yscale('log')
    ax.set_xlabel('{} streamflow ($m^3$/$s$)'.format(parameter.reference_title), fontsize=12)
    ...
    original_file_path = os.path.join(original_output_dir, parameter.output_file_annual_scatter)
