...
def plot_min_max_mean(tc_dict, canvas, metrics_dict, ref_test_or_diff):
    ...
    min_label = managetextcombined(
        tc_dict, plot + '_min_label', plot + '_min_label', canvas)
    max_label = managetextcombined(
        tc_dict, plot + '_max_label', plot + '_max_label', canvas)
    mean_label = managetextcombined(
        tc_dict, plot + '_mean_label', plot + '_mean_label', canvas)

    min_value = managetextcombined(
        tc_dict, plot + '_min_value', plot + '_min_value', canvas)
    max_value = managetextcombined(
        tc_dict, plot + '_max_value', plot + '_max_value', canvas)
    mean_value = managetextcombined(
        tc_dict, plot + '_mean_value', plot + '_mean_value', canvas)
...
def plot_rmse_and_corr(tc_dict, canvas, metrics_dict):
    ...
    rmse_label = managetextcombined(
        tc_dict, 'diff_plot_comment1_title', 'diff_plot_comment1_title', canvas)
    corr_label = managetextcombined(
        tc_dict, 'diff_plot_comment2_title', 'diff_plot_comment2_title', canvas)
    rmse_label.string = 'RMSE'
    corr_label.string = 'CORR'

    rmse_value = managetextcombined(
        tc_dict, 'diff_plot_comment1_value', 'diff_plot_comment1_value', canvas)
    corr_value = managetextcombined(
        tc_dict, 'diff_plot_comment2_value', 'diff_plot_comment2_value', canvas)
...
def set_levels_of_graphics_method(method, levels, data, data2=None):
    ...
    if method.levels == [[1.0000000200408773e+20, 1.0000000200408773e+20]]:
        if data2 is None:
            ...
            method.levels = vcs.mkscale(
                min(data.min(), data2.min()), max(data.max(), data2.max()))
