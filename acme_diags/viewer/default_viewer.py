...
def create_viewer(root_dir, parameters):
    ...
    set_name = parameters.sets[0]

    viewer = OutputViewer(path=root_dir)
    viewer.add_page(SET_TO_NAME[set_name], seasons_used(parameters))
    ...
    lat_lon_viewer.pass_in_constants(SEASONS, LAT_LON_TABLE_INFO)
    lat_lon_viewer.generate_lat_lon_metrics_table(viewer, root_dir, parameters)
    lat_lon_viewer.generate_lat_lon_taylor_diag(viewer, root_dir, parameters)

def seasons_used(parameters):
    """
    Get a list of the seasons used for this set of parameters.
    """
    seasons_used = set([s for s in p.season for p in parameters])
