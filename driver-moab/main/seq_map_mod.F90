...
contains
  ...
  subroutine seq_map_map( mapper, av_s, av_d, fldlist, norm, avwts_s, avwtsfld_s, &
    ...
    !=========================================================================
    ! STRATEGY SELECTION: MCT Path
    ! The following three branches handle the MCT-based mapping operations.
    ! MOAB operations for these strategies are handled AFTER this section.
    !=========================================================================

    if (mapper%copy_only) then
       !-------------------------------------------
       ! STRATEGY 1: COPY
       ! Source and destination grids are IDENTICAL (same gsmap).
       ! No interpolation needed - just copy field values directly.
       ! MCT: Uses mct_aVect_copy for efficient memory copy.
       !-------------------------------------------
       if (present(fldlist)) then
       ...
       !-------------------------------------------
       ! STRATEGY 2: REARRANGE
       ! Source and destination grids have SAME grid points but
       ! DIFFERENT MPI decomposition (different gsmap).
       ! MCT: Uses mct_rearr_rearrange for MPI all-to-all communication
       ! to redistribute data across processors.
       !-------------------------------------------
       if (present(fldlist)) then
       ...
       !-------------------------------------------
       ! STRATEGY 3: MAP (Full Interpolation/Regridding)
       ! Source and destination are DIFFERENT grids.
       ! MCT: Uses sparse matrix-vector multiplication (sMatP) with
       ! pre-computed interpolation weights from mapping files.
       ! Supports optional normalization via seq_map_avNorm:
       ! - If avwts_s provided: multiply source by weights before mapping,
       !   then divide result by mapped weights (conservative mapping)
       ! - If norm=.true.: apply self-normalization
       !-------------------------------------------
