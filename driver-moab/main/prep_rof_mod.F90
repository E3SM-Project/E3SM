...
! similar to a2o, prep_ocn
          ...
          if ((mbrxid .ge. 0) .and.  (mbaxid .ge. 0)) then
            ...
            if (samegrid_ar) then 
               ! the same mesh , atm and rof use the same dofs
               ! we do not compute intersection, so we will have to just send data from atm to rof and viceversa, by GLOBAL_ID matching
               ! so we compute just a comm graph, between atm and rof dofs, on the coupler; target is rof
               ! land is full mesh
               type1 = 3; !  full mesh for atm 
               type2 = 3;  ! fv for target rof
               ...
               if (ierr .ne. 0) then
                  write(logunit,*) subname,' error in computing comm graph , ATM-ROF'
                  call shr_sys_abort(subname//' ERROR in computing comm graph , ATM-ROF')
               endif
               ! context for rearrange is target in this case
...
! endif HAVE_MOAB
          ...
          if ((mbaxid .ge. 0) .and.  (mbrxid .ge. 0)) then
            ...
            endif ! when samegrid_ar , nothing to do, already have the compute graph
