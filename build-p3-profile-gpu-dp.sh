#!/bin/bash
# Build the standalone p3_run_and_cmp test in double precision with
# CUDA profiling support enabled.
...
case "${NERSC_HOST:-}" in
  perlmutter)
    MACHINE_NAME="pm-gpu"
    BUILD_DIR="${REPO_ROOT}/build_p3_profile_${MACHINE_NAME}-DP"
    ;;
  alvarez)
...
"${REPO_ROOT}/scripts_p3_test/build_p3_test.sh" \
  "$MACHINE_NAME" DP "$BUILD_DIR" Release ON
