...
contains
  ...
  subroutine seq_flds_set(nmlfile, ID, infodata)
    ...
    namelist /seq_cplflds_inparm/  &
         ...
         glc_nec, glc_nozc, ice_ncat, seq_flds_i2o_per_cat, flds_bgc_oi, &
    ...
    call shr_mpi_bcast(glc_nozc     , mpicom)
    ...
    call glc_zocnclass_init(glc_nozc)
  ...
  subroutine set_glc_zocnclass_field(name, attname, longname, stdname, units, fieldlist, &
    ...
    ! Sets a coupling field for all ocn z classes (1:glc_nozc) plus bare land
    ! (index 0).
    !
    ! Note that, if glc_nozc = 0, then we don't create any coupling fields
    !
    ! Puts the coupling fields in the given fieldlist, and also does the appropriate
    ! metadata_set calls.
    !
