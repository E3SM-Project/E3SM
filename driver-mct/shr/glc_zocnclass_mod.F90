...
contains
  ...
  subroutine glc_zocnclass_init_bnds()
    ...
    allocate(zocn_bnds(glc_nzoc, 2))
    zocn_bnds(1,1) = 0._r8
    zocn_bnds(1,2) = 0.5_r8 * (zocn_levels(1) + zocn_levels(2))
    do i = 2, glc_nzoc - 1
       zocn_bnds(i,1) = 0.5_r8 * (zocn_levels(i-1) + zocn_levels(i))
       zocn_bnds(i,2) = 0.5_r8 * (zocn_levels(i) + zocn_levels(i+1))
    enddo
    zocn_bnds(glc_nzoc,1) = 0.5_r8 * (zocn_levels(glc_nzoc-1) + zocn_levels(glc_nzoc))
    zocn_bnds(glc_nzoc,2) = zocn_levels(glc_nzoc) + (zocn_levels(glc_nzoc) - zocn_bnds(glc_nzoc,1))
  ...
  function glc_get_zocnclass_bounds() result(zocnclass_bounds)
    ...
    !
    ! !ARGUMENTS:
    real(r8) :: zocnclass_bounds(glc_nzoc,2)  ! function result
    !
    ! !LOCAL VARIABLES:
