...
!=== For DG 2D Version (Nair) ==================================
     ...
     !      ulatlon(:,:,1,lev)=elem(ie)%DinvJMD(:,:,1,1)*v1 + elem(ie)%DinvJMD(:,:,2,1)*v2   ! co->latlon
     !      ulatlon(:,:,2,lev)=elem(ie)%DinvJMD(:,:,1,2)*v1 + elem(ie)%DinvJMD(:,:,2,2)*v2   ! co->latlon
     !  end do
     ! Convert cotravariant wind to latlon 
        do lev=1,nlev
            v1 = elem(ie)%state%v(:,:,1,lev,n0)
            v2 = elem(ie)%state%v(:,:,2,lev,n0)
            ulatlon(:,:,1,lev)=elem(ie)%DJMD(:,:,1,1)*v1 + elem(ie)%DJMD(:,:,1,2)*v2 
            ulatlon(:,:,2,lev)=elem(ie)%DJMD(:,:,2,1)*v1 + elem(ie)%DJMD(:,:,2,2)*v2  
        end do
...
!    real (kind=real_kind), parameter  :: surf=12.566370614359172*
    ...
    do ie=nets,nete
       ...
       ulatlon(:,:,1)=elem(ie)%DJMD(:,:,1,1)*v1 + elem(ie)%DJMD(:,:,1,2)*v2   ! contra->latlon
       ulatlon(:,:,2)=elem(ie)%DJMD(:,:,2,1)*v1 + elem(ie)%DJMD(:,:,2,2)*v2   ! contra->latlon
