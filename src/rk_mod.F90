  ...
  subroutine ComputeCFL(dt_grv,pmean,mindx,n0,elem,nets,nete,hybrid,cfl,dt_adv)
    ...
    do ie=nets,nete
       do k=1,nlev
          do j=1,np
             do i=1,np
                v1     = elem(ie)%DJMD(i,j,1,1)*elem(ie)%state%v(i,j,1,k,n0)+elem(ie)%DJMD(i,j,1,2)*elem(ie)%state%v(i,j,2,k,n0)
                v2     = elem(ie)%DJMD(i,j,2,1)*elem(ie)%state%v(i,j,1,k,n0)+elem(ie)%DJMD(i,j,2,2)*elem(ie)%state%v(i,j,2,k,n0)
