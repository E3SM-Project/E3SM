! !MODULE: med_io_mod -- reads and writes driver files
!
! !DESCRIPTION:
...
! Current Problems
!  - the original use of med_io will now ONLY work with the cpl because
...
!
!    2018-May    - T. Craig migrate to ESMF/NUOPC datatypes/mediator
!
! !INTERFACE: ------------------------------------------------------------------
...
module med_io_mod
  ...
  use seq_comm_mct, only: CPLID, seq_comm_setptrs
...
!     module procedure med_io_write_avs
!     module procedure med_io_write_avscomp
...
contains
  ...
  end subroutine med_io_cpl_init

  !===============================================================================
  !BOP ===========================================================================
  !
  ! !IROUTINE: med_io_wopen - open netcdf file
  !
  ! !DESCRIPTION:
  !    open netcdf file
  !
  ...
  subroutine med_io_wopen(filename,clobber,file_ind, model_doi_url)
    ...
    if (.not. pio_file_is_open(cpl_io_file(lfile_ind))) then
       ...
       if (exists) then
          if (lclobber) then
             ...
             if(iam==0) write(llogunit,*) subname,' create file ',trim(filename)
             ...
             if(iam==0) write(llogunit,*) subname,' open file ',trim(filename)
    ...
    elseif (trim(wfilename) /= trim(filename)) then
       ...
       if(iam==0) write(llogunit,*) subname,' different  filename currently open ',trim(filename)
       if(iam==0) write(llogunit,*) subname,' different wfilename currently open ',trim(wfilename)
  ...
  !BOP ===========================================================================
  !
  ! !IROUTINE: med_io_close - close netcdf file
  !
  ! !DESCRIPTION:
  !    close netcdf file
  !
  ...
  subroutine med_io_close(filename,file_ind)
    ...
    elseif (trim(wfilename) == trim(filename)) then
       ...
       if(iam==0) write(llogunit,*) subname,' different  filename currently open, aborting ',trim(filename)
       if(iam==0) write(llogunit,*) subname,' different wfilename currently open, aborting ',trim(wfilename)
  ...
  character(len=8) function med_io_sec2hms (seconds)
    ...
    if (seconds < 0 .or. seconds > 86400) then
       write(llogunit,*)'med_io_sec2hms: bad input seconds:', seconds
    ...
    if (minutes < 0 .or. minutes > 60) then
       write(llogunit,*)'med_io_sec2hms: bad minutes = ',minutes
       call shr_sys_abort('med_io_sec2hms: bad minutes')
    end if

    if (secs < 0 .or. secs > 60) then
       write(llogunit,*)'med_io_sec2hms: bad secs = ',secs
...
80  format(i2.2,':',i2.2,':',i2.2)
  ...
  !BOP ===========================================================================
  !
  ! !IROUTINE: med_io_write_FB - write FB to netcdf file
  !
  ! !DESCRIPTION:
  ...
  subroutine med_io_write_FB(filename,FB,whead,wdata,nx,ny,nt,fillval,pre,tavg,&
...
!    call ESMF_LogWrite(trim(tmpstr), ESMF_LOGMSG_INFO, rc=dbrc)
    ...
    if (lwdata) then
       ...
       do k = 1,nf
          ...
          if (trim(itemc) /= "hgt") then
             ...
             call pio_write_darray(cpl_io_file(lfile_ind), varid, iodesc, fldptr1, rcode, fillval=lfillvalue)
             !-------tcraig
          endif
       enddo
  ...
  !BOP ===========================================================================
  !
  ! !IROUTINE: med_io_write_av - write AV to netcdf file
  !
  ! !DESCRIPTION:
  ...
  subroutine med_io_write_av(filename,gsmap,AV,dname,whead,wdata,nx,ny,nt,fillval,pre,tavg,&
       use_float, file_ind)
    ...
    integer(in) :: lnx,lny
    real(r8) :: lfillvalue
    character(*),parameter :: subName = '(med_io_write_av) '
    integer, pointer :: Dof(:)
    integer :: lfile_ind
    ...
    if (nf < 1) then
       write(llogunit,*) subname,' ERROR: nf = ',nf,trim(dname)
       call shr_sys_abort(trim(subname)//'nf error')
    endif
    frame = -1
    if (present(nt)) then
       frame = nt
    ...
    if (lnx*lny /= ng) then
       if(iam==0) write(llogunit,*) subname,' ERROR: grid2d size not consistent ',ng,lnx,lny,trim(dname)
       call shr_sys_abort(trim(subname)//'ERROR: grid2d size not consistent ')
    endif
  ...
  end subroutine med_io_write_av

  !===============================================================================
  !BOP ===========================================================================
  !
  ! !IROUTINE: med_io_write_avs - write AVS to netcdf file
  !
  ! !DESCRIPTION:
  !    Write AV to netcdf file
  !
  ...
  subroutine med_io_write_avs(filename,gsmap,AVS,dname,whead,wdata,nx,ny,nt,fillval,pre,tavg,&
       use_float,file_ind)
    ...
    character(*),parameter :: subName = '(med_io_write_avs) '
    integer, pointer :: Dof(:)
  ...
  end subroutine med_io_write_avs
  ...
  subroutine med_io_write_avscomp(filename, comp, flow, dname, &
    ...
    if (lnx*lny /= ng) then
       if(iam==0) then
          write(llogunit,*) subname,' ERROR: grid2d size not consistent ',&
  ...
  end subroutine med_io_write_avscomp
#endif

  !===============================================================================
  !BOP ===========================================================================
  !
  ! !IROUTINE: med_io_write_int - write scalar integer to netcdf file
  !
  ! !DESCRIPTION:
  ...
  subroutine med_io_write_int(filename,idata,dname,whead,wdata,file_ind)
    ...
    if (lwdata) then
       ...
       !      write(llogunit,*) subname,' wrote AV ',trim(dname),lwhead,lwdata
    endif
  ...
  !BOP ===========================================================================
  !
  ! !IROUTINE: med_io_write_int1d - write 1d integer array to netcdf file
  !
  ! !DESCRIPTION:
  ...
  subroutine med_io_write_int1d(filename,idata,dname,whead,wdata,file_ind)
  ...
  !BOP ===========================================================================
  !
  ! !IROUTINE: med_io_write_r8 - write scalar double to netcdf file
  !
  ! !DESCRIPTION:
  ...
  subroutine med_io_write_r8(filename,rdata,dname,whead,wdata,file_ind)
  ...
  !BOP ===========================================================================
  !
  ! !IROUTINE: med_io_write_r81d - write 1d double array to netcdf file
  !
  ! !DESCRIPTION:
  ...
  subroutine med_io_write_r81d(filename,rdata,dname,whead,wdata,file_ind)
  ...
  !BOP ===========================================================================
  !
  ! !IROUTINE: med_io_write_char - write char string to netcdf file
  !
  ! !DESCRIPTION:
  ...
  subroutine med_io_write_char(filename,rdata,dname,whead,wdata,file_ind)
  ...
  !BOP ===========================================================================
  !
  ! !IROUTINE: med_io_write_time - write time variable to netcdf file
  !
  ! !DESCRIPTION:
  ...
  subroutine med_io_write_time(filename,time_units,time_cal,time_val,nt,whead,wdata,tbnds,file_ind)
    ...
    if (lwdata) then
       ...
       !      write(llogunit,*) subname,' wrote time ',lwhead,lwdata
    endif
  ...
  !BOP ===========================================================================
  !
  ! !IROUTINE: med_io_read_av - read AV from netcdf file
  !
  ! !DESCRIPTION:
  ...
  subroutine med_io_read_av(filename,gsmap,AV,dname,pre)
    ...
    character(CL)  :: name1
    character(CL)  :: lpre
    character(*),parameter :: subName = '(med_io_read_av) '
    !-------------------------------------------------------------------------------
    !
    !-------------------------------------------------------------------------------
    ...
    if (iam==0) inquire(file=trim(filename),exist=exists)
    call shr_mpi_bcast(exists,mpicom,'med_io_read_av exists')
    if (exists) then
       ...
       if(iam==0) write(llogunit,*) subname,' ERROR: file invalid ',trim(filename),' ',trim(dname)
       ...
       if (rcode == pio_noerr) then
          if (k==1) then
             ...
             if (ng /= mct_gsmap_gsize(gsmap)) then
                if (iam==0) write(llogunit,*) subname,' ERROR: dimensions do not match',&
                     lnx,lny,mct_gsmap_gsize(gsmap)
                call shr_sys_abort(trim(subname)//'ERROR: dimensions do not match')
             end if
             call pio_initdecomp(cpl_io_subsystem, pio_double, (/lnx,lny/), dof, iodesc)
             deallocate(dof)
          end if
          ...
          write(llogunit,*)'med_io_readav warning: field ',trim(itemc),' is not on restart file'
          write(llogunit,*)'for backwards compatibility will set it to 0'
          av%rattr(k,:) = 0.0_r8
       end if
       call pio_seterrorhandling(pioid,PIO_INTERNAL_ERROR)
  ...
  end subroutine med_io_read_av

  !===============================================================================
  !BOP ===========================================================================
  !
  ! !IROUTINE: med_io_read_avs - read AV from netcdf file
  !
  ! !DESCRIPTION:
  !    Read AV from netcdf file
  !
  ...
  subroutine med_io_read_avs(filename,gsmap,AVS,dname,pre)
    ...
    character(CL)  :: name1
    character(CL)  :: lpre
    character(*),parameter :: subName = '(med_io_read_avs) '
    !-------------------------------------------------------------------------------
    !
    !-------------------------------------------------------------------------------
    ...
    if (iam==0) inquire(file=trim(filename),exist=exists)
    call shr_mpi_bcast(exists,mpicom,'med_io_read_avs exists')
    if (exists) then
       rcode = pio_openfile(cpl_io_subsystem, pioid, cpl_pio_iotype, trim(filename),pio_nowrite)
       if(iam==0) write(llogunit,*) subname,' open file ',trim(filename)
       call pio_seterrorhandling(pioid,PIO_BCAST_ERROR)
    ...
    do k = 1,nf
       ...
       if (rcode == pio_noerr) then
          if (k==1) then
             ...
             if (lnx*lny /= ng) then
                write(llogunit,*) subname,' ERROR: dimensions do not match',&
                     lnx,lny,mct_gsmap_gsize(gsmap)
                call shr_sys_abort(trim(subname)//'ERROR: dimensions do not match')
             end if
             if (ndims>=3) then
             ...
             if (ni /= lni) then
                write(llogunit,*) subname,' ERROR: ni dimensions do not match',ni,lni
                call shr_sys_abort(trim(subname)//'ERROR: ni dimensions do not match')
             end if
             if (ni > 1) then
                allocate(dofn(ns*ni))
                n = 0
  ...
  end subroutine med_io_read_avs
  ...
  subroutine med_io_read_avscomp(filename, comp, flow, dname, pre)
  ...
  end subroutine med_io_read_avscomp
#endif
  !===============================================================================
  !BOP ===========================================================================
  !
  ! !IROUTINE: med_io_read_int - read scalar integer from netcdf file
  !
  ! !DESCRIPTION:
  ...
  !BOP ===========================================================================
  !
  ! !IROUTINE: med_io_read_int1d - read 1d integer from netcdf file
  !
  ! !DESCRIPTION:
  ...
  subroutine med_io_read_int1d(filename,idata,dname)
    ...
    if (exists) then
       rcode = pio_openfile(cpl_io_subsystem, pioid, cpl_pio_iotype, trim(filename),pio_nowrite)
       !         write(llogunit,*) subname,' open file ',trim(filename)
       call pio_seterrorhandling(pioid,PIO_BCAST_ERROR)
       rcode = pio_get_att(pioid,pio_global,"file_version",lversion)
       call pio_seterrorhandling(pioid,PIO_INTERNAL_ERROR)
    else
    ...
    !      write(llogunit,*) subname,' read int ',trim(dname)
  ...
  !BOP ===========================================================================
  !
  ! !IROUTINE: med_io_read_r8 - read scalar double from netcdf file
  !
  ! !DESCRIPTION:
  ...
  !BOP ===========================================================================
  !
  ! !IROUTINE: med_io_read_r81d - read 1d double array from netcdf file
  !
  ! !DESCRIPTION:
  ...
  !BOP ===========================================================================
  !
  ! !IROUTINE: med_io_read_char - read char string from netcdf file
  !
  ! !DESCRIPTION:
