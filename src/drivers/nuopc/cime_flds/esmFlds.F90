...
!================================================================================
  ...
  subroutine esmFlds_Init(gcomp, phase, rc)
    ...
    character(len=CX) :: atm2ice_fmap
    character(len=CX) :: atm2ice_smap
    character(len=CX) :: atm2ice_vmap
    character(len=CX) :: atm2lnd_fmap
    character(len=CX) :: atm2lnd_smap
    character(len=CX) :: atm2ocn_fmap
    character(len=CX) :: atm2ocn_smap
    character(len=CX) :: atm2ocn_vmap
    character(len=CX) :: atm2wav_smap
    character(len=CX) :: glc2lnd_fmap
    character(len=CX) :: glc2lnd_smap
    character(len=CX) :: glc2ice_rmap
    character(len=CX) :: glc2ocn_liq_rmap
    character(len=CX) :: glc2ocn_ice_rmap
    character(len=CX) :: ice2atm_fmap
    character(len=CX) :: ice2atm_smap
    character(len=CX) :: ice2wav_smap
    character(len=CX) :: lnd2atm_fmap
    character(len=CX) :: lnd2atm_smap
    character(len=CX) :: lnd2glc_fmap
    character(len=CX) :: lnd2glc_smap
    character(len=CX) :: lnd2rof_fmap
    character(len=CX) :: ocn2atm_fmap
    character(len=CX) :: ocn2atm_smap
    character(len=CX) :: ocn2wav_smap
    character(len=CX) :: rof2lnd_fmap
    character(len=CX) :: rof2ocn_fmap
    character(len=CX) :: rof2ocn_ice_rmap
    character(len=CX) :: rof2ocn_liq_rmap
    ...
    if (phase == 'advertise') then
       ...
       call addmap(fldListFr(compatm)%flds, 'Sa_z', mapbilnr, 'one', atm2ice_smap)
    ...
    if (phase == 'advertise') then
       ...
       if (fldchk(is_local%wrap%FBImp(compatm,compatm), 'Sa_ptem', rc=rc) .and. &
          ...
          call addmap(fldListFr(compatm)%flds, 'Sa_ptem', compocn, mapbilnr, 'one', atm2ocn_smapname)
       end if
       if (fldchk(is_local%wrap%FBImp(compatm,compatm), 'Sa_ptem', rc=rc) .and. &
    ...
    do n = 1,size(iso)
       ...
       if ( fldchk(is_local%wrap%FBImp(compatm,compatm), 'Faxa_rain'//iso(n), rc=rc) .and. &
            fldchk(is_local%wrap%FBExp(compicn)        , 'Faxa_rain'//iso(n), rc=rc)) then
    ...
    do n = 1,size(iso)
       if (phase == 'advertise') then
          ...
          if ( fldchk(is_local%wrap%FBImp(compatm,compatm), 'Faxa_snow'//iso(n), rc=rc) .and. &
               fldchk(is_local%wrap%FBExp(compicn)        , 'Faxa_snow'//iso(n), rc=rc)) then
    ...
    if (phase == 'advertise') then
       ...
       if ( fldchk(is_local%wrap%FBImp(compatm,compatm), 'Faxa_lwdn', rc=rc)) then
          call addmap(fldListFr(compatm)%flds, 'Faxa_lwdn', compatm, complnd, mapconsf, 'one', atm2lnd_fmap)
          call addmap(fldListFr(compatm)%flds, 'Faxa_lwdn', compatm, compice, mapconsf, 'one', atm2ice_fmap)
          call addmap(fldListFr(compatm)%flds, 'Faxa_lwdn', compatm, compocn, mapconsf, 'one', atm2ocn_fmap)
    ...
    if (phase == 'advertise') then
       ...
       if ( fldchk(is_local%wrap%FBexp(compatm), 'Sx_avsdr', rc=rc)) then
          ...
          call addmap(fldListMed_ocnalb_o%flds,'So_avsdr', compocn, compatm, mapconsf, 'ofrac', ocn2atm_smap)
    ...
    if (phase == 'advertise') then
       ...
       if ( fldchk(is_local%wrap%FBexp(compatm), 'Sx_anidr', rc=rc)) then
          ...
          call addmap(fldListMed_ocnalb_o%flds,'So_anidr', compocn, compatm, mapconsf, 'ofrac', ocn2atm_smap)
    ...
    if (phase == 'advertise') then
       ...
       if ( fldchk(is_local%wrap%FBexp(compatm), 'Sx_avsdf', rc=rc)) then
          ...
          call addmap(fldListMed_ocnalb_o%flds,'So_avsdf', compocn, compatm, mapconsf, 'ofrac', ocn2atm_smap)
    ...
    if (phase == 'advertise') then
       ...
       if ( fldchk(is_local%wrap%FBexp(compatm), 'Sx_anidf', rc=rc)) then
          ...
          call addmap(fldListMed_ocnalb_o%flds,'So_anidf', compocn, compatm, mapconsf, 'ofrac', ocn2atm_smap)
    ...
    if (phase == 'advertise') then
       ...
       if ( fldchk(is_local%wrap%FBexp(compatm)         , 'Sx_tref', rc=rc) .and. &
          ...
          call addmap(fldListMed_aoflux_o%flds, 'So_tref', compocn, compatm, mapconsf, 'ofrac', ocn2atm_fmap) ! map ocn->atm
    ...
    do n = 1,size(iso)
       if (phase == 'advertise') then
          ...
          if ( fldchk(is_local%wrap%FBexp(compatm)         , 'Sx_qref'//iso(n), rc=rc) .and. &
             ...
             call addmap(fldListMed_aoflux_a%flds, 'So_qref'//iso(n), compatm, compocn, mapbilnr, 'one'  , atm2ocn_fmap) ! map atm->ocn
             call addmap(fldListMed_aoflux_o%flds, 'So_qref'//iso(n), compocn, compatm, mapconsf, 'ofrac', ocn2atm_fmap) ! map ocn->atm
    ...
    if (phase == 'advertise') then
       ...
       if ( fldchk(is_local%wrap%FBexp(compatm)         , 'Sl_fv', rc=rc)) then
    ...
    if (phase == 'advertise') then
       ...
       if ( fldchk(is_local%wrap%FBexp(compatm)         , 'Sl_ram1', rc=rc)) then
    ...
    if (phase == 'advertise') then
       ...
       if (fldchk(is_local%wrap%FBexp(compatm), 'So_ssq', rc=rc) .and. &
          ...
          call addmap(fldListMed_aoflux_o%flds, 'So_ssq', compocn, compatm, mapconsf, 'ofrac', ocn2atm_fmap) ! map ocn->atm
    ...
    if (phase == 'advertise') then
       ...
       if (fldchk(is_local%wrap%FBexp(compatm), 'So_re', rc=rc) .and. &
          ...
          call addmap(fldListMed_aoflux_o%flds, 'So_re', compocn, compatm, mapconsf, 'ofrac', ocn2atm_fmap) ! map ocn->atm
    ...
    if (use_med_aoflux) then
       if (phase == 'advertise') then
          ...
          if (fldchk(is_local%wrap%FBexp(compatm), 'Sx_u10', rc=rc)) then
             ...
             call addmap(fldListMed_aoflux_o%flds, 'So_u10', compocn, compatm, mapconsf, 'ofrac', ocn2atm_fmap) ! map ocn->atm
    ...
    if (phase == 'advertise') then
       ...
       if (fldchk(is_local%wrap%FBMed_aoflux_o, 'So_ustar', rc=rc) .and. &
          ...
          call addmap(fldListMed_aoflux_o%flds, 'So_ustar', compocn, compatm, mapconsf, 'ofrac', ocn2atm_fmap) ! map ocn->atm
    ...
    if (phase == 'advertise') then
       ...
       if (fldchk(is_local%wrap%FBMed_aoflux_o, 'Faox_taux', rc=rc) .and. &
          ...
          call addmap(fldListMed_aoflux_o%flds, 'Faox_taux', compocn, compatm, mapconsf, 'ofrac', ocn2atm_fmap) ! ocn->atm
       end if
    ...
    if (phase == 'advertise') then
       ...
       if (fldchk(is_local%wrap%FBMed_aoflux_o, 'Faox_tauy', rc=rc) .and. &
          ...
          call addmap(fldListMed_aoflux_o%flds, 'Faox_tauy', compocn, compatm, mapconsf, 'ofrac', ocn2atm_fmap) ! map ocn->atm
       end if
    ...
    if (phase == 'advertise') then
       ...
       if (fldchk(is_local%wrap%FBMed_aoflux_o, 'Faox_lat', rc=rc) .and. &
          ...
          call addmap(fldListMed_aoflux_o%flds,'Faox_lat', compocn, compatm, mapconsf, 'ofrac', ocn2atm_fmap) ! map ocn->atm
       end if
       ...
       if ( fldchk(is_local%wrap%FBImp(complnd,complnd), 'Fall_lat', rc=rc) .and. &
          ...
          call addmrg(fldListTo(compatm)%flds , 'Faxx_lat', &
               ...
               mrg_from2=compmed, mrg_fld2='Faox_lat', mrg_type2='merge', mrg_fracname2='ofrac')
                 ...
                 fldchk(is_local%wrap%FBexp(compatm)        , 'Faii_lat', rc=rc) .and. &
          ! NEMS frac
    ...
    if (phase == 'advertise') then
       ...
       if ( fldchk(is_local%wrap%FBMed_aoflux_o, 'Faox_sen', rc=rc) .and. &
          ...
          call addmap(fldListMed_aoflux_o%flds,'Faox_sen', compocn, compatm, mapconsf, 'ofrac', ocn2atm_fmap) ! map ocn->atm
       end if
       ...
       if ( fldchk(is_local%wrap%FBImp(complnd,complnd), 'Fall_sen', rc=rc) .and. &
          ...
          call addmrg(fldListTo(compatm)%flds , 'Faxx_sen', &
               ...
               mrg_from2=compmed, mrg_fld2='Faox_sen', mrg_type2='merge', mrg_fracname2='ofrac')
                 ...
                 fldchk(is_local%wrap%FBexp(compatm)        , 'Faii_sen', rc=rc) .and. &
          ! NEMS frac
    ...
    if (phase == 'advertise') then
       ...
       if ( fldchk(is_local%wrap%FBMed_aoflux_o, 'Faox_lwup', rc=rc) .and. &
          ...
          call addmap(fldListMed_aoflux_o%flds,'Faox_lwup', compocn, compatm, mapconsf, 'ofrac', ocn2atm_fmap) ! map ocn->atm
       end if
    ...
    do n = 1,size(iso)
       if (phase == 'advertise') then
          ...
          if ( fldchk(is_local%wrap%FBMed_aoflux_o, 'Faox_evap'//iso(n), rc=rc) .and. &
             ...
             call addmap(fldListMed_aoflux_a%flds,'Faox_evap'//iso(n), compatm, compocn, mapconsf, 'one'  , atm2ocn_fmap) ! map atm->ocn
             call addmap(fldListMed_aoflux_o%flds,'Faox_evap'//iso(n), compocn, compatm, mapconsf, 'ofrac', ocn2atm_fmap) ! map ocn->atm
          end if
    ...
    if (phase == 'advertise') then
       ...
       if ( fldchk(is_local%wrap%FBImp(compatm,compatm), 'Faxa_taux', rc=rc) .and. &
          ...
          ! NEMS orig
          call addmap(fldListFr(compatm)%flds, 'Faxa_taux', compatm, compocn, mapconsf, 'one'  , atm2ocn_fmap) ! map atm->ocn
    ...
    if (phase == 'advertise') then
       ...
       if ( fldchk(is_local%wrap%FBImp(compatm,compatm), 'Faxa_tauy', rc=rc) .and. &
          ...
          ! NEMS orig
          call addmap(fldListFr(compatm)%flds, 'Faxa_tauy', compatm, compocn, mapconsf, 'one'  , atm2ocn_fmap) ! map atm->ocn
       ...
       if (phase == 'advertise') then
          ...
          if ( fldchk(is_local%wrap%FBexp(compocn), 'Foxx_lat'//iso(n), rc=rc) .and. &
             ...
             call addmrg(fldListTo(compocn)%flds, 'Foxx_lat'//iso(n), &
                  merge_from1=compmed, merge_fld1='Faox_lat'//iso(n), merge_type1='merge', merge_fracname1='ofrac')
          else
    ...
    if (phase == 'advertise') then
       ...
       if ( fldchk(is_local%wrap%FBexp(compocn)         , 'Foxx_sen'  , rc=rc) .and. &
          ...
          ! NEMS orig
          call addmap(fldListFr(compatm)%flds, 'Faxa_sen'  , compatm, compocn, mapconsf, 'one'  , atm2ocn_fmap) ! map atm->ocn
          ...
          call addmrg(fldListTo(compocn)%flds, 'Foxx_sen', &
               ...
               mrg_from1=compice, mrg_fld1='Fioi_melth', mrg_type1='merge', mrg_fracname1='ifrac')
       ...
       if (phase == 'advertise') then
          ...
          if ( fldchk(is_local%wrap%FBexp(compocn)         , 'Fioi_melth'//iso(n), rc=rc) .and. &
             ...
             call addmrg(fldListTo(compocn)%flds, 'Fioi_melth'//iso(n), &
                  merge_from1=compice, merge_fld1='Fioi_melth'//iso(n), &
                  merge_type1='copy_with_weights', merge_fracname1='ifrac')
          end if
       end if
    end do
       ...
       if ( fldchk(is_local%wrap%FBMed_aoflux_o        , 'Faox_lwup' , rc=rc) .and. &
          ...
          call addmrg(fldListTo(compocn)%flds, 'Foxx_lwnet', &
               ...
               mrg_from2=compatm, mrg_fld2='Faxa_lwdn', mrg_type2='merge', mrg_fracname2='ofrac')
                 ...
                 fldchk(is_local%wrap%FBImp(compatm,compatm), 'Faxa_lwdn' , rc=rc))
          ! CESM (docn)
          call addmap(fldListFr(compatm)%flds, 'Faxa_lwdn', compocn, mapconsf, 'one'  , atm2ocn_fmap)
          ...
          ! will have custom merge in med_phases_prep_ocn
                 ...
                 fldchk(is_local%wrap%FBExp(compocn)        , 'Foxx_lwnet', rc=rc))
          ! NEMS-frac (mom6)
          call addmap(fldListFr(compatm)%flds, 'Foxx_lwnet', compocn, mapconsf, 'one'  , atm2ocn_fmap)
          call addmrg(fldListTo(compocn)%flds, 'Foxx_lwnet', &
    ...
    if (phase == 'advertise') then
       ...
       if ( fldchk(is_local%wrap%FBMed_aoflux_o, 'So_duu10n', rc=rc) .and. &
          ...
          call addmap(fldListMed_aoflux_o%flds, 'So_duu10n', compocn, compatm, mapconsf, 'ofrac', ocn2atm_fmap) ! map ocn->atm
    ...
    if (phase == 'advertise') then
       ...
       if ( fldchk(is_local%wrap%FBExp(complnd)         , 'Sg_icemask_coupled_fluxes', rc=rc) .and. &
            fldchk(is_local%wrap%FBImp(compglc,  'Sg_icemask_coupled_fluxes', rc=rc)) then
    ...
    if (glc_nec > 0) then
       if (phase == 'advertise') then
          ...
          do num = 0, glc_nec
             cnum = glc_elevclass_as_string(num)
             call addmap(FldListFr(complnd)%flds, 'Flgl_qice', compglc, mapconsf, 'none', lnd2glc_fmapname)
             call addmap(FldListFr(complnd)%flds, 'Sl_tsrf'  , compglc, mapbilnr, 'none', lnd2glc_smapname)
             call addmap(FldListFr(complnd)%flds, 'Sl_topo'  , compglc, mapbilnr, 'none', lnd2glc_smapname)
          end do
       end if
    end if
