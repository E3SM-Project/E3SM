module dshr_nuopc_stream_mod
  ...
  public :: dshr_nuopc_stream_init              ! initialize a stream
  public :: dshr_nuopc_stream_set               ! set stream values
  public :: dshr_nuopc_stream_default           ! set default values
  public :: dshr_nuopc_stream_parseInput        ! extract fileName,yearAlign, etc. from a string
  public :: dshr_nuopc_stream_findBounds        ! return lower/upper bounding date info
  public :: dshr_nuopc_stream_getModelFieldList ! return model      field name list
  public :: dshr_nuopc_stream_getFileFieldName  ! return k-th input-file field name
  public :: dshr_nuopc_stream_getPrevFileName   ! return previous file in sequence
  public :: dshr_nuopc_stream_getNextFileName   ! return next file in sequence
  public :: dshr_nuopc_stream_getFilePath       ! return file path
  public :: dshr_nuopc_stream_getDomainInfo     ! return the stream's domain info data
  public :: dshr_nuopc_stream_getFile           ! acquire file, return name of file to open
  public :: dshr_nuopc_stream_getNFiles         ! get the number of files in a stream
  public :: dshr_nuopc_stream_getCalendar       ! get the stream calendar
  public :: dshr_nuopc_stream_getCurrFile       ! get the currfile, fileopen, and currpioid
  public :: dshr_nuopc_stream_setCurrFile       ! set the currfile, fileopen, and currpioid
  public :: dshr_nuopc_stream_dataDump          ! internal stream data for debugging
  public :: dshr_nuopc_stream_restWrite         ! write a streams restart file
  public :: dshr_nuopc_stream_restRead          ! read  a streams restart file
...
#include "dynamic_vector_typedef.inc"
  ...
  type shr_stream_streamType
     ...
     character(CS)     :: dataSource                   ! meta data identifying data source
     character(CL)     :: filePath                     ! remote location of data files
     ...
     character(CL)     :: domFilePath                  ! domain file: file path of domain file
     character(CL)     :: domFileName                  ! domain file: name
     character(CS)     :: domTvarName                  ! domain file: time-dim var name
     character(CS)     :: domXvarName                  ! domain file: x-dim var name
     character(CS)     :: domYvarName                  ! domain file: y-dim var name
     character(CS)     :: domZvarName                  ! domain file: z-dim var name
     character(CS)     :: domAreaName                  ! domain file: area  var name
     character(CS)     :: domMaskName                  ! domain file: mask  var name
...
#include "dynamic_vector_procdef.inc"
  ...
  subroutine dshr_nuopc_stream_init(strm,infoFile,yearFirst,yearLast,yearAlign,taxMode,rc)
    ...
    if (rCode /= 0) goto 999
    call dshr_nuopc_stream_readUpToTag(nUnit,startTag,rc=rCode)
    if (rCode /= 0) goto 999
    ...
    do while (.true.)
       ...
       if (len_trim(fldNameFile)==0 .or. len_trim(fldNameModel)==0 ) then
          ...
          call dshr_nuopc_stream_abort(subName//"ERROR: reading field names")
       end if
       if (n==1) then
    ...
    if (n==0) then
       ...
       call dshr_nuopc_stream_abort(subName//"ERROR: no input field names")
    end if
    ...
    startTag =  "<fieldInfo>"
    endTag = "</fieldInfo>"
    call dshr_nuopc_stream_readUpToTag(nUnit,startTag,rc=rCode2)
    if (rCode2 /= 0)then
       rCode = rCode2
       goto 999
    end if
    ...
    startTag =  "<offset>"
    endTag = "</offset>"
    call dshr_nuopc_stream_readUpToTag(nUnit,startTag,optionalTag=.true.,rc=rCode2)
    if (rCode2 == 0) then
       !--- read data ---
    ...
    if (strm%nFiles==0) then
       ...
       call dshr_nuopc_stream_abort(subName//"ERROR: no input file names")
    end if
    ...
    if (n==0) then
       ...
       if (n==0) then
          rCode = 1
          write(s_logunit,F00) "ERROR: no input field names"
          call dshr_nuopc_stream_abort(subName//"ERROR: no time variable name")
       else
          ...
          if(len_trim(substr) <= len(strm%domTVarName)) then
             ...
             call dshr_nuopc_stream_abort(subName//"ERROR: string too long for variable name")
          endif
       endif
       ...
       if (n==0) then
          rCode = 1
          write(s_logunit,F00) "ERROR: no input field names"
          call dshr_nuopc_stream_abort(subName//"ERROR: no lon variable name")
       else
          call shr_string_listGetName (fldListFile,n,substr,rc)
       ...
       if (n==0) then
          rCode = 1
          write(s_logunit,F00) "ERROR: no input field names"
          call dshr_nuopc_stream_abort(subName//"ERROR: no lat variable name")
       else
          call shr_string_listGetName (fldListFile,n,substr,rc)
       ...
       if (n==0) then
          !         rCode = 1
          !         write(s_logunit,F00) "ERROR: no input field names"
          !         call dshr_nuopc_stream_abort(subName//"ERROR: no area variable name")
       ...
       if (n==0) then
          !         rCode = 1
          !         write(s_logunit,F00) "ERROR: no input field names"
          !         call dshr_nuopc_stream_abort(subName//"ERROR: no mask variable name")
    ...
    !-----------------------------------------------------------------------------
    call dshr_nuopc_stream_getCalendar(strm,1,calendar)
    strm%calendar = trim(calendar)

    !-----------------------------------------------------------------------------
    ! normal return or end-of-file problem?
    !-----------------------------------------------------------------------------
    call dshr_nuopc_stream_setInit(strm)
...
999 continue
    ...
    call dshr_nuopc_stream_abort(subName//"ERROR: unexpected end-of-file")
    close(nUnit)
    if ( present(rc) ) rc = rCode
    call shr_file_freeUnit(nUnit)

  end subroutine dshr_nuopc_stream_init

  !===============================================================================
  subroutine dshr_nuopc_stream_set(&
       strm, yearFirst, yearLast, yearAlign, offset, taxMode,  &
       fldListFile, fldListModel, domFilePath, domFileName,  &
       domTvarName, domXvarName, domYvarName, domZvarName,   &
       domAreaName, domMaskName,  filePath, filename, dataSource, rc)
    ...
    call dshr_nuopc_stream_default(strm)
    ...
    if (present(domAreaName  )) strm%domAreaName = trim(domAreaName)
  ...
  end subroutine dshr_nuopc_stream_set

  !===============================================================================
  subroutine dshr_nuopc_stream_default(strm, rc)
    ...
    !-----------------------------------------------------------------------------
    call dshr_nuopc_stream_clearInit(strm)
    strm%nFiles           = 0
  ...
  end subroutine dshr_nuopc_stream_default
  !===============================================================================

  subroutine dshr_nuopc_stream_readUpToTag(nUnit,tag,optionalTag,rc)
...
999 continue
    ...
    if (rc /= 0 .and. .not. localOptionalTag ) then
       ...
       call dshr_nuopc_stream_abort(subName//"ERROR: tag not found")
    end if

  end subroutine dshr_nuopc_stream_readUpToTag

  !===============================================================================
  subroutine dshr_nuopc_stream_parseInput(str,fileName,yearAlign,yearFirst,yearLast,rc)
  ...
  end subroutine dshr_nuopc_stream_parseInput

  !===============================================================================
  subroutine dshr_nuopc_stream_findBounds(strm,mDateIn, secIn, &
    ...
    if ( .not. shr_stream_isInit(strm)) then
       rCode = 1
       call dshr_nuopc_stream_abort(trim(subName)//" ERROR: trying to find bounds of uninitialized stream")
       return
    end if
    ...
    elseif (trim(strm%taxMode) == trim(shr_stream_taxis_limit)) then
       ...
       call dshr_nuopc_stream_abort(trim(subName)//' ERROR: illegal taxMode = '//trim(strm%taxMode))
    endif
    ...
    if (dYear < 0) then
       ...
       call dshr_nuopc_stream_abort(trim(subName)//' ERROR: dyear lt one')
    endif
    ...
    if (.not. strm%found_lvd) then
       A:    do k=1,strm%nFiles
          if (.not. strm%file(k)%haveData) then
             call dshr_nuopc_stream_readtCoord(strm, k, rCode)
             if ( rCode /= 0 )then
                call dshr_nuopc_stream_abort(trim(subName)//" ERROR: readtCoord1")
                return
             end if
          end if
       ...
       if (.not. strm%found_lvd) then
          ...
          call dshr_nuopc_stream_abort(trim(subName)//" ERROR: LVD not found, all data is before yearFirst")
       else
          ...
          if ( dDateL <= strm%file(strm%k_lvd)%date(strm%n_lvd) ) then
             ...
             call dshr_nuopc_stream_abort(trim(subName)//" ERROR: LVD not found, all data is after yearLast")
          end if
       end if
       if (debug>1 .and. s_loglev > 0) then
    ...
    if (strm%found_lvd) then
       ...
       call dshr_nuopc_stream_abort(trim(subName)//" ERROR: LVD not found yet")
    endif
    ...
    if (rDateIn < rDatelvd) then
       if (limit) then
          ...
          call dshr_nuopc_stream_abort(trim(subName)//" ERROR: rDateIn lt rDatelvd limit true")
          return
       endif
       ...
       if (cycle) then
          ...
          if (.not. strm%found_gvd) then
             ...
             B:          do k=strm%nFiles,1,-1
                ...
                if (.not. strm%file(k)%haveData) then
                   call dshr_nuopc_stream_readtCoord(strm, k, rCode)
                   if ( rCode /= 0 )then
                      call dshr_nuopc_stream_abort(trim(subName)//" ERROR: readtCoord2")
                      return
                   end if
                end if
          ...
          if (.not. strm%found_gvd) then
             ...
             call dshr_nuopc_stream_abort(trim(subName)//" ERROR: GVD not found1")
          endif
       ...
       if (limit) then
          ...
          call dshr_nuopc_stream_abort(trim(subName)//" ERROR: rDateIn gt rDategvd limit true")
          return
       endif
       ...
       C:    do k=strm%k_lvd,strm%nFiles
          ...
          if (.not. strm%file(k)%haveData) then
             call dshr_nuopc_stream_readtCoord(strm, k, rCode)
             if ( rCode /= 0 )then
                call dshr_nuopc_stream_abort(trim(subName)//" ERROR: readtCoord3")
                return
             end if
          end if
    ...
    call dshr_nuopc_stream_abort(trim(subName)//' ERROR: findBounds failed')
    return

  end subroutine dshr_nuopc_stream_findBounds

  !===============================================================================
  subroutine dshr_nuopc_stream_readTCoord(strm,k,rc)

    ! verify time coordinate data is OK
    ! read in time coordinates with possible offset

    ! input/output parameters:
    type(shr_stream_streamType)  ,intent(inout) :: strm ! data stream to query
    integer         ,intent(in)    :: k    ! stream index
    integer,optional,intent(out)   :: rc   ! return code
    ...
    call dshr_nuopc_stream_getFile(strm%filePath, strm%file(k)%name, fileName)
    ...
    call dshr_nuopc_stream_verifyTCoord(strm,k,lrc) ! check new t-coord data

    if (present(rc)) then
       rc = lrc
    endif

  end subroutine dshr_nuopc_stream_readTCoord

  !===============================================================================
  subroutine dshr_nuopc_stream_verifyTCoord(strm,k,rc)
    ...
    if ( .not. strm%file(k)%haveData) then
       ...
       call dshr_nuopc_stream_abort(subName//"ERROR: can't check -- file not read.")
       return
    end if
    ...
    do n=1,strm%file(k)%nt+1
       ...
       if (checkIt) then
          if ( date1 > date2 ) then
             ...
             call dshr_nuopc_stream_abort(subName//"ERROR: calendar dates must be increasing")
             return
             ...
             if ( secs1 >= secs2 ) then
                ...
                call dshr_nuopc_stream_abort(subName//"ERROR: elapsed seconds must be increasing")
                return
             end if
          end if
          if ( secs1 < 0 .or. spd < secs1 ) then
             ...
             call dshr_nuopc_stream_abort(subName//"ERROR: elapsed seconds out of range")
             return
          end if
       end if
    end do
  ...
  end subroutine dshr_nuopc_stream_verifyTCoord

  !===============================================================================
  subroutine dshr_nuopc_stream_getModelFieldList(stream,list,rc)
  ...
  end subroutine dshr_nuopc_stream_getModelFieldList

  !===============================================================================
  subroutine dshr_nuopc_stream_getFileFieldName(stream,k,name,rc)
  ...
  end subroutine dshr_nuopc_stream_getFileFieldName

  !===============================================================================
  subroutine dshr_nuopc_stream_getFilepath(strm,path)
  ...
  end subroutine dshr_nuopc_stream_getFilePath

  !===============================================================================
  subroutine dshr_nuopc_stream_getCalendar(strm,k,calendar)
    ...
    ! local
    integer                :: fid, vid, n
    character(CL)          :: fileName,strmfile,lcal
    integer                :: rCode
    ...
    call dshr_nuopc_stream_getFile(strm%filePath,strmfile,fileName)
    rCode = nf90_open(fileName,nf90_nowrite,fid)
  ...
  end subroutine dshr_nuopc_stream_getCalendar

  !===============================================================================
  subroutine dshr_nuopc_stream_getCurrFile(strm,fileopen,currfile,currpioid)
  ...
  end subroutine dshr_nuopc_stream_getCurrFile

  !===============================================================================
  subroutine dshr_nuopc_stream_setCurrFile(strm,fileopen,currfile,currpioid)
  ...
  end subroutine dshr_nuopc_stream_setCurrFile

  !===============================================================================
  subroutine dshr_nuopc_stream_getDomainInfo(strm, filePath, fileName, timeName, &
  ...
  end subroutine dshr_nuopc_stream_getDomainInfo

  !===============================================================================
  subroutine dshr_nuopc_stream_getFile(filePath,fileName,localFile,rc)

    ! Acquire file, return name of file to open
  ...
  end subroutine dshr_nuopc_stream_getFile

  !===============================================================================
  subroutine dshr_nuopc_stream_getNextFileName(strm,fn,fnNext,path,rc)
    ...
    if (.not. found) then
       ...
       call dshr_nuopc_stream_abort(subName//"ERROR: file name not in stream: "//trim(fn))
    end if
  ...
  end subroutine dshr_nuopc_stream_getNextFileName

  !===============================================================================
  subroutine dshr_nuopc_stream_getPrevFileName(strm,fn,fnPrev,path,rc)
  ...
  end subroutine dshr_nuopc_stream_getPrevFileName

  !===============================================================================
  subroutine dshr_nuopc_stream_getNFiles(strm,nfiles)
  ...
  end subroutine dshr_nuopc_stream_getNFiles

  !===============================================================================
  subroutine dshr_nuopc_stream_restWrite(strm,fileName,caseName,caseDesc,nstrms,rc)
    ...
    if (present(nstrms)) then
       if (size(strm) < nstrms) then
          ...
          call dshr_nuopc_stream_abort(subname//": ERROR: nstrms too large for strm")
       endif
       nStreams = nstrms
    else
       nStreams = size(strm)
    ...
    do k = 1,nStreams
       if (.not. shr_stream_isInit(strm(k))) then  ! has stream been initialized?
          ...
          call dshr_nuopc_stream_abort(subName//": ERROR: given uninitialized stream")
       end if
  ...
  end subroutine  dshr_nuopc_stream_restWrite

  !===============================================================================
  subroutine dshr_nuopc_stream_restRead(strm,fileName,nstrms,rc)
    ...
    if ( rCode /= 0 )then
       call shr_file_freeUnit(nUnit)
       call dshr_nuopc_stream_abort(subName//": ERROR: error opening file: "//trim(fileName) )
       if ( present(rc) ) rc = rCode
       return
    end if
    ...
    if (present(nstrms)) then
       if (nstrms /= nStreams) then
          ...
          call dshr_nuopc_stream_abort(subname//": ERROR: nstrms ne nStreams on restart")
       endif
       nStreams = nstrms
    endif
    ...
    do k = 1,nStreams
       ...
       if (.not. strm(k)%init) then
          ...
          call dshr_nuopc_stream_abort(subName//": ERROR: reading uninitialized stream")
       end if
       call dshr_nuopc_stream_setInit(strm(k))
    ...
    if (abort) then
       ...
       call dshr_nuopc_stream_abort(subName//": ERRORS Detected ABORTING NOW")
    endif
  ...
  end subroutine dshr_nuopc_stream_restRead

  !===============================================================================
  subroutine dshr_nuopc_stream_dataDump(strm)
  ...
  end subroutine dshr_nuopc_stream_dataDump

  !===============================================================================
  subroutine dshr_nuopc_stream_abort( msg )
  ...
  end subroutine dshr_nuopc_stream_abort
  ...
  !===============================================================================
  subroutine dshr_nuopc_stream_setInit(strm,rc)
  ...
  end subroutine dshr_nuopc_stream_setInit

  !===============================================================================
  subroutine dshr_nuopc_stream_clearInit(strm,rc)
  ...
  end subroutine dshr_nuopc_stream_clearInit

end module dshr_nuopc_stream_mod
