...
!mt call shr_mem_init(prt=.true.)
   ...
   ! above - cpl_deocmp is set to pass the cpl_decomp value to seq_mctext_decomp 
   ! (via a use statement)
...
subroutine ccsm_init()
 ...
 106  format( A, f23.12)
   ...
   if (iamin_CPLID) then
      call t_startf ('driver_init_aream')
      ...
      call t_stopf ('driver_init_aream')
   end if ! iamin_CPLID
   ...
   if (iamin_CPLID) then
      call t_startf ('driver_init_domain_check')
      ...
      call t_stopf ('driver_init_domain_check')
   endif ! iamin_CPLID
   ...
   call t_startf ('driver_init_areacor')
   ...
   call t_stopf ('driver_init_areacor')
   ...
   if (iamin_CPLID .and. info_debug > 1) then
      call t_startf ('driver_init_diag')
   ...
   if (iamin_CPLID) then
      call t_startf ('driver_init_fracs')
      ...
      call t_stopf ('driver_init_fracs')
   end if
   ...
   if (iamin_CPLID) then
      if (ocn_present) then
         call t_startf ('driver_init_aoflux')
         ...
         call t_stopf ('driver_init_aoflux')
      endif
   end if
   ...
   if (do_histinit) then
      if (iamin_CPLID) then
         call t_startf('driver_init_histinit')
         ...
         call t_stopf('driver_init_histinit')
      endif
   endif
   ...
   do while ( .not. stop_alarm)
      ...
      if (rof_present .and. rofrun_alarm) then
         ...
         if (iamin_CPLID .and. rof_prognostic) then
            call ccsm_comp_barriers(mpicom=mpicom_CPLID, timer='DRIVER_ROF_PREP_BARRIER')
         ...
         if (ocn_present .and. ocnnext_alarm) then
            if (iamin_CPLALLOCNID) then
               ...
               call component_exch(ocn, flow='c2x', &
                    ...
                    mpicom_barrier=mpicom_CPLALLOCNID, run_barriers=run_barriers, &
                    timer_barrier='DRIVER_OC2_BARRIER', timer_comp_exch='DRIVER_O2C', &
                    timer_map_exch='driver_o2ct_ocno2ocnx', timer_infodata_exch='driver_o2ct_infoexch')
      ...
      if (iamin_CPLID .and. ocn_present) then
