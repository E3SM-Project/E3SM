...
!   Now call add fields
    do i = 1, zt%nn
      ...
      if (len(temp1) .gt. 16) sub = temp1(1:16)
       ...
       call addfld(trim(sub),(/ 'ilev' /),&
            'A',trim(zt%f%var(i)%units),trim(zt%f%var(i)%description))
    enddo
    ...
    do i = 1, zm%nn
      ...
      call addfld(trim(sub),(/ 'ilev' /),&
           'A',trim(zm%f%var(i)%units),trim(zm%f%var(i)%description))
    enddo

    if (l_output_rad_files) then     
      do i = 1, rad_zt%nn
        call addfld(trim(rad_zt%f%var(i)%name),(/ 'ilev' /),&
           'A',trim(rad_zt%f%var(i)%units),trim(rad_zt%f%var(i)%description))
      enddo
    
      do i = 1, rad_zm%nn
        call addfld(trim(rad_zm%f%var(i)%name),(/ 'ilev' /),&
           'A',trim(rad_zm%f%var(i)%units),trim(rad_zm%f%var(i)%description))
      enddo
    endif 
    
    do i = 1, sfc%nn
      call addfld(trim(sfc%f%var(i)%name),horiz_only,&
           'A',trim(sfc%f%var(i)%units),trim(sfc%f%var(i)%description))
    enddo
