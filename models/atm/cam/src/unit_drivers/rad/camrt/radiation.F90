...
use drv_input_data,  only: do_fdh
...
!-----------------------------------------------------------------------
    ...
    call pbuf_add_field('tcorr','global',dtype_r8,(/pcols,pver/), tcorr_idx) ! longwave  radiative heating rate 
...
!-----------------------------------------------------------------------
   ...
   ! determine default variables
    ...
    if ( do_fdh) then
       call addfld('TCORR', 'K', pver, 'A','Corrected temperature',phys_decomp)
       call add_default('TCORR',1,' ')
    endif
...
!===============================================================================
  ...
  subroutine radiation_tend(state,ptend, pbuf, &
    ...
    use drv_input_data,   only: in_data_dtime
    ...
    ! fdh implementation
    real(r8) :: qrs_in(pcols,pver)   ! shortwave radiative heating rate (from unforced run)
    real(r8) :: qrl_in(pcols,pver)   ! longwave  radiative heating rate (from unforced run)
    real(r8), pointer, dimension(:,:) :: tcorr          ! temperature correction above tropopause
    real(r8) :: t_mod(pcols,pver)                       ! temperature as modified by stratospheric temperature adjustment
...
!----------------------------------------------------------------------
    ...
    if (do_fdh) then
       qrs_in(:ncol,:) = qrs(:ncol,:)
       qrl_in(:ncol,:) = qrl(:ncol,:)
    ...
    if (dosw .or. dolw) then
	  ...
	  !! use idx_sw_diag, idx_sw_diag is the index to the visible band of the shortwave
       ...
       if (dolw) then
          ...
          t_mod(:ncol,:) = state%t(:ncol,:)
          if (do_fdh) then
             do k = 1, pver
                do i = 1, ncol
                   if ( state%pmid(i,k) < p_trop(i)) then
                      t_mod(i,k) = t_mod(i,k) + tcorr(i,k)
                   endif
                enddo
             enddo
          end if
       ...
       if(do_fdh) then
          ...
          do k = 1, pver
             do i = 1, ncol
                if ( state%pmid(i,k) < p_trop(i)) then
                   tcorr(i,k) = tcorr(i,k) + in_data_dtime*(qrs(i,k) - qrs_in(i,k) + qrl(i,k) - qrl_in(i,k)) /cpair 
                endif
             enddo
          enddo
          call outfld('TCORR',tcorr(:,:),pcols,lchnk)
