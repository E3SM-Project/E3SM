...
contains
  ...
  subroutine aerosols_formation( ncol, lchnk, index_lat, index_lon, tfld, rh, qin)
    ...
    integer,  intent(in)    :: index_lat(pcols)         ! chunk lat indicies
    integer,  intent(in)    :: index_lon(pcols)         ! chunk lon indicies
    real(r8), intent(in)    :: tfld(pcols,pver)         ! temperature
    real(r8), intent(in)    :: rh(ncol,pver)            ! relative humidity
    real(r8), intent(inout) :: qin(ncol,pver,gas_pcnst) ! xported species ( vmr )
