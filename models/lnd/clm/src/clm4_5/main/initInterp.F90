...
!$OMP END PARALLEL DO
       ...
       do ji = 1, nlevo
          ...
          do no = bego,endo
             if (activeo(no)) then
                ...
                if (ni > 0) then
                   if ( copylevels) then
                      rbuf2do(ji,no) = rbuf2di(index_lower,ni)
                   else
                      rbuf2do(ji,no) = &
  ...
  subroutine check_dim_level(ncidi, ncido, dimname)
    ...
    if (trim(dimname) == 'levgrnd') then
       if (dimleni /= dimleno) then
          if (masterproc) then
             write (iulog,*) 'input and output nlevgrnd values disagree'
             write (iulog,*) 'input nlevgrnd = ',dimleni,' output nlevgrnd = ',dimleno
          end if
          if (dimleni > dimleno) then
             if (masterproc) then
                write (iulog,*) 'ERROR interpinic: nlevgrnd_i > nlevgrnd_o not supported, stopping'
             end if
             call endrun(msg=errMsg(__FILE__, __LINE__))
          end if
       end if
