module TracerStateType
  ...
  !-------------------------------------------------------------------------------
  ! Column tracer state variables structure
  !-------------------------------------------------------------------------------
...
!   real(r8), pointer :: tracer_conc_frozen_col(:,:,:)             !tracer concentration in frozen layer for unsaturated part for nonvolatile species
!   real(r8), pointer :: tracer_conc_bubble_col(:,:,:)             !a bubble pool to track the lake ebullition in freeze-thaw period, [col, levels, tracer]
...
contains
  ...
  subroutine InitHistory(this, bounds, betrtracer_vars)
    ...
    ! some modifications are needed below.
    ! in the isotope simulation, the isotope signatures are derived from the ratio
    ! between actual tracer concentrations, therefore, they are not counted as
    ! actual tracer.
